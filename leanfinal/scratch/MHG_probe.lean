...
/-!
# MHG probe (2026-08-27) — MH.8 and X1

Zero `sorry`, zero declared `axiom`.  This probe records the exact corpus statement of X1 and
machine-checks the arithmetic/side-geometry proof.  It also records the two MH.8 comparison
shapes and proves the reverse (coefficient-adic to `dvSupp`) transport from the public capped
congruence theorem.
-/
...
/-- **X1 (OPEN-4), proved.** A polynomial pure at one direction has a point side at every
distinct direction.  The conclusion is stronger than `dvSideDeg = 0`: every attaining
abscissa at the second direction is the same endpoint (`0` or `n`). -/
...
/-! ## MH.8: the bounded-window forward comparison -/
...
/-- The least Gauss valuation of the key-development digits is at most that of the assembled
polynomial.  This is the public-proof replacement for B.40's private helper. -/
...
/-- The inner finite loss: on digits of degree `< D`, stage height is at most scaled Gauss
valuation plus `h*(D-1)`. -/
...
/-- **MH.8 forward upper comparison, proved.** On `natDegree < d`, the exact fixed loss is
`ell*h*(D'-1) + u*(d-1)`, where `D'=deg(key)=e1*f1` controls the inner digit window and
`d` controls the outer development window. -/
...
/-- **MH.8 forward conversion, proved.** The bounded-window upper comparison cancels the
fixed loss and turns a sufficiently shifted `dvSupp` floor into a Gauss floor. -/
...
/-- MH.8 forward comparison, exact public statement shape: the loss is fixed on the degree
window.  `C` will be instantiated by `ell * F.h * (D' - 1) + u * d`. -/
...
/-- **MH.8 reverse transport, proved from public API.** Coefficientwise `pi^N` divisibility
pushes back to the level-two support with scale `e1*ell`. -/
...
/-- **MH.8, fully assembled.** A degree-bounded sequence Cauchy for `dvSupp` has a limit in
the same window, with the same unshifted `dvSupp` tail estimate. -/
...
#print axioms MHGCheck.dvOnSide_weight_eq
#print axioms MHGCheck.onSide_weight_le_slot
#print axioms MHGCheck.otherSlope_pointSide
#print axioms MHGCheck.inf_devGauss_le_gaussVal
#print axioms MHGCheck.stageHeight_le_gaussVal_add_loss
#print axioms MHGCheck.dvSupp_le_scaled_gaussVal_add_window
#print axioms MHGCheck.gaussVal_of_shifted_dvSupp
#print axioms MHGCheck.reverseTransport
#print axioms MHGCheck.exists_dvGradedLimit
