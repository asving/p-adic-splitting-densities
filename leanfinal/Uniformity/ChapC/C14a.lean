...
basis `{η^s}`, taken through a chosen `AdjoinRoot.mk`-preimage and B.59a's identification
`ResidueField O ≃+* resField X`.

A **section, not an invariant** (see the module docstring): junk-stable, and pinned downstream
only through H.56-shaped residue clauses about the resulting lift. -/
