...
/-!
# Uniformity.ChapB.B83d — the `q = 2` SPLIT gate (two blocks, fires B.80)

**Chapter B, NODE B.83 instance (iv)** [gate] (`blueprint/CHAP-B_leaf_layer.md` §10), ENV-C
concrete. One signed declaration: `gate_split_two`.

*The instance* (blueprint's row (iv)): `O := ℤ_[2]`, `π := 2`, `a = ![2, 3, 3]`, so
`f = monicPoly a = X³ + 3X² + 3X + 2 = (X + 2)(X² + X + 1)`; two order-0 blocks
`φ₁ = X`, `φ₂ = X² + X + 3` with peel `g₁ = X + 2`, `g₂ = X² + X + 1`, both `e = 1`; `N = 2`.

**Why `φ₂ ≠ g₂` on purpose.** `dev φ₂ g₂ 0 = −2 ≠ 0`, so the block is `Visible`; taking
`φ₂ = g₂` would make `dev = 0` and no certificate could fire (B.75's junk direction). The keys
are *lifts*, not the factors themselves.

| block | key `φ` | peel `g` | `dev₀` | heights | side | `sideDeg` | `order1Type` |
|---|---|---|---|---|---|---|---|
| 1 | `X` (`m = 1`) | `X + 2` | `2` | `(1, 0)` | `(1,1)`, argmin `{0,1}` | `1` | `⟨{(1,1)}⟩` |
| 2 | `X² + X + 3` (`m = 2`) | `X² + X + 1` | `−2` | `(1, 0)` | `(1,1)`, argmin `{0,1}` | `1` | `⟨{(1,2)}⟩` |

and `⟨{(1,1)}⟩.data + ⟨{(1,2)}⟩.data = {(1,1),(1,2)}` — B.80's `Σ`-shape at this instance.

## The A-F.12 discharge, per block, with the arithmetic displayed

B.80's `hperim` is the window-class closure **per block**; the kit's `perim_degree_bound` gives,
uniformly in the class member `gT` of that block,

```
ℓ · φ.natDegree · ψ.natDegree = gS.natDegree ≤ gT.natDegree = (g i).natDegree
```

so, with `deg ψ ≥ 1` (ψ monic irreducible) and `ℓ ≥ 1`:

```
block 1 :  m = 1, deg g₁ = 1  ⟹  1 · 1 · deg ψ ≤ ℓ · 1 · deg ψ ≤ 1  ⟹  deg ψ = 1
block 2 :  m = 2, deg g₂ = 2  ⟹  1 · 2 · deg ψ ≤ ℓ · 2 · deg ψ ≤ 2  ⟹  deg ψ = 1
```

— the SECOND disjunct in both blocks (block 1 also has the first available at `ℓ = 1`; at
`ℓ ≥ 2` no such `gS` exists, exactly as the blueprint's ⚠ says, and the display shows why:
`2 ≤ ℓ · 1 · deg ψ ≤ 1` is absurd). `B-BOX-1`-free.
...
**TEETH.** `W12-T-SIGMA` (`EFF.W12.27`) → **Lean theorem** (the two-block firing);
`W12-ORACLE` → **executable regression** retained; `W12-SHAPE`'s decided `n = 3` split row →
**Lean theorem**.

SOURCE: `EFF.W12.29`; `blueprint/CONVENTIONS_2026-08-15.md` GC-11; B.75 (the junk direction the
`φ₂ ≠ g₂` choice avoids).

## Status

Sorry-free. **Footprint: `propext`, `Classical.choice`, `Quot.sound` plus
`Uniformity.Density.Leaf.exists_slope_factorization`** (through B.80 ← B.79 ← B.63 ← B.42).
No new axiom.
-/
...
private theorem hresm2 : IsLocalRing.residue ℤ_[2] (-2) = 0 := by
  rw [map_neg, hres2, neg_zero]

private theorem hres3 : IsLocalRing.residue ℤ_[2] 3 = 1 := by
  have h : (3 : ℤ_[2]) = 1 + 2 := by norm_num
  rw [h, map_add, hres2, map_one, add_zero]

/-! ### block 1 : `φ = X`, `g = X + 2` -/

private noncomputable abbrev gA : Polynomial ℤ_[2] := X + C 2

private theorem hgAmon : gA.Monic := by unfold gA; monicity!
private theorem hgAdeg : gA.natDegree = 1 := by unfold gA; compute_degree!
private theorem hgAdegmu : gA.natDegree = 1 * (X : Polynomial ℤ_[2]).natDegree := by
  rw [hgAdeg, Polynomial.natDegree_X]

private theorem hgAc0 : gA.coeff 0 = 2 := by unfold gA; simp
private theorem hgAc1 : gA.coeff 1 = 1 := by unfold gA; simp

private theorem hgAres : gA.map (IsLocalRing.residue ℤ_[2])
    = ((X : Polynomial ℤ_[2]).map (IsLocalRing.residue ℤ_[2])) ^ 1 := by
  unfold gA; simp [hres2]

private theorem hfinA : ∀ j, j ≤ 1 →
    npHgt (X : Polynomial ℤ_[2]) gA j = ((if j = 0 then 1 else 0 : ℕ) : ℕ∞) := by
  intro j hj
  rw [npHgt_X]
  interval_cases j
  · rw [hgAc0, GateKit.padic_addVal (p := 2) (k := 1) (a := 2) (by norm_num) (by norm_num)
      (by norm_num)]
    norm_num
  · rw [hgAc1, GateKit.padic_addVal (p := 2) (k := 0) (a := 1) (by norm_num) (by norm_num)
      (by norm_num)]
    norm_num

private theorem htopA : ∀ j, 1 < j → npHgt (X : Polynomial ℤ_[2]) gA j = ⊤ := by
  intro j hj
...
private theorem hBA : ∀ j, j ≤ gA.natDegree → (npHgt (X : Polynomial ℤ_[2]) gA j).toNat ≤ 1 := by
  intro j hj
  rw [hgAdeg] at hj
  rw [hfinA j hj]
  by_cases h : j = 0 <;> simp [h]

private theorem hreadA : suppVal (X : Polynomial ℤ_[2]) gA 1 1 = ((1 : ℕ) : ℕ∞) ∧
    ∀ j, (j ∈ sideSet (X : Polynomial ℤ_[2]) gA 1 1 ↔
...
private theorem hsideA : sideSet (X : Polynomial ℤ_[2]) gA 1 1 = {0, 1} := by
  ext j
  rw [hreadA.2 j]
  constructor
  · rintro ⟨hj, hv⟩
...
private theorem hcardA : 1 < (sideSet (X : Polynomial ℤ_[2]) gA 1 1).card := by
  rw [hsideA]; decide

private theorem hsdegA : ∀ hne : (sideSet (X : Polynomial ℤ_[2]) gA 1 1).Nonempty,
    sideDeg (X : Polynomial ℤ_[2]) gA 1 1 hne = 1 := by
  intro hne
...
private theorem hslopeA : ∀ u ℓ : ℕ, 0 < ℓ → Nat.Coprime u ℓ →
    1 < (sideSet (X : Polynomial ℤ_[2]) gA u ℓ).card → u = 1 ∧ ℓ = 1 := by
  ...
  · exfalso
    have hread := GateKit.polygon_read (n := 1) (m := 1) hgAdeg (by norm_num) hfinA htopA
      ...
      ⟨1, by norm_num⟩
    have huniq : ∀ j ∈ sideSet (X : Polynomial ℤ_[2]) gA 0 1, j = 1 := by
      intro j hj
      obtain ⟨hjm, hval⟩ := (hread.2 j).1 hj
      interval_cases j <;> revert hval <;> decide
    have : (sideSet (X : Polynomial ℤ_[2]) gA 0 1).card ≤ 1 :=
      Finset.card_le_one.2 fun a ha b hb => by rw [huniq a ha, huniq b hb]
    omega
  · exact ⟨rfl, rfl⟩

private theorem hslopeFinsetA : slopeFinset (2 : ℤ_[2]) (X : Polynomial ℤ_[2]) gA = {(1, 1)} := by
  ext p
...
private theorem hordA : order1Type (2 : ℤ_[2]) (X : Polynomial ℤ_[2]) gA = ⟨{(1, 1)}⟩ := by
  have h := GateKit.order1Type_of_sideDeg_one hpi2 GateKit.isKey_X hgAmon hgAdegmu (by
    intro p hp hne
...
private theorem htermA : ¬ NeedsDescent (2 : ℤ_[2]) (X : Polynomial ℤ_[2]) gA := by
  refine GateKit.not_needsDescent_of_sideDeg_one hpi2 GateKit.isKey_X hgAmon hgAdegmu ?_
  intro u ℓ hℓ hcop hcard hne
  obtain ⟨hu, hl⟩ := hslopeA u ℓ hℓ hcop hcard
  subst hu; subst hl
  exact hsdegA hne

private theorem hvisA : Visible (2 : ℤ_[2]) (X : Polynomial ℤ_[2]) gA 2 := by
  refine ⟨0, ?_⟩
...
private noncomputable abbrev kB : Polynomial ℤ_[2] := X ^ 2 + X + C 3
private noncomputable abbrev gB : Polynomial ℤ_[2] := X ^ 2 + X + 1
...
private theorem hkBmapres : kB.map (IsLocalRing.residue ℤ_[2])
    = X ^ 2 + X + 1 := by
  unfold kB
  simp [hres3]

private theorem hkBirr : Irreducible (kB.map (IsLocalRing.residue ℤ_[2])) := by
  set e := PadicInt.residueField (p := 2) with he
  have hmap : (kB.map (IsLocalRing.residue ℤ_[2])).map (e : ResidueField ℤ_[2] →+* ZMod 2)
      = X ^ 2 + X + 1 := by
    rw [hkBmapres]; simp
  refine Polynomial.irreducible_of_degree_le_three_of_not_isRoot ?_ ?_
  · rw [hkBmon.natDegree_map, hkBdeg]; decide
  · intro x hx
    have h1 : ((kB.map (IsLocalRing.residue ℤ_[2])).map
        (e : ResidueField ℤ_[2] →+* ZMod 2)).eval (e x)
        = e ((kB.map (IsLocalRing.residue ℤ_[2])).eval x) := by
...
private theorem hgBeq : gB = C (-2) + 1 * kB := by
  unfold gB kB
  have : (C (3 : ℤ_[2]) : Polynomial ℤ_[2]) = C 2 + 1 := by
    rw [← Polynomial.C_1, ← Polynomial.C_add]; norm_num
  rw [this]
  have h2 : (C (-2 : ℤ_[2]) : Polynomial ℤ_[2]) = - C 2 := by rw [← Polynomial.C_neg]
  rw [h2]
  ring

private theorem hgBeq' : gB = C (-2) + kB := by rw [hgBeq, one_mul]

private theorem hgBres : gB.map (IsLocalRing.residue ℤ_[2])
    = (kB.map (IsLocalRing.residue ℤ_[2])) ^ 1 := by
  rw [pow_one, hgBeq', Polynomial.map_add, Polynomial.map_C, hresm2, Polynomial.C_0, zero_add]

private theorem hdevB : dev kB gB 0 = C (-2) ∧ dev kB gB 1 = 1 :=
...
private theorem hfinB : ∀ j, j ≤ 1 → npHgt kB gB j = ((if j = 0 then 1 else 0 : ℕ) : ℕ∞) := by
  ...
  · rw [npHgt, hdevB.1, GateKit.gaussVal_C,
    ...
    norm_num
  · rw [npHgt, hdevB.2, show (1 : Polynomial ℤ_[2]) = C 1 from (Polynomial.C_1).symm,
...
private theorem hslopeFinsetB : slopeFinset (2 : ℤ_[2]) kB gB = {(1, 1)} := by
  ext p
...
private theorem hordB : order1Type (2 : ℤ_[2]) kB gB = ⟨{(1, 2)}⟩ := by
  have h := GateKit.order1Type_of_sideDeg_one hpi2 hkeyB hgBmon hgBdegmu (by
    intro p hp hne
...
private theorem htermB : ¬ NeedsDescent (2 : ℤ_[2]) kB gB := by
  refine GateKit.not_needsDescent_of_sideDeg_one hpi2 hkeyB hgBmon hgBdegmu ?_
  intro u ℓ hℓ hcop hcard hne
  obtain ⟨hu, hl⟩ := hslopeB u ℓ hℓ hcop hcard
  subst hu; subst hl
  exact hsdegB hne

private theorem hvisB : Visible (2 : ℤ_[2]) kB gB 2 := by
  refine ⟨0, ?_⟩
...
private noncomputable abbrev aIV : Fin 3 → ℤ_[2] := ![2, 3, 3]
private noncomputable abbrev phiIV : Fin 2 → Polynomial ℤ_[2] := ![X, kB]
private noncomputable abbrev gfIV : Fin 2 → Polynomial ℤ_[2] := ![gA, gB]
private noncomputable abbrev eIV : Fin 2 → ℕ := ![1, 1]

private theorem hfnormIV : Uniformity.Density.monicPoly aIV
    = X ^ 3 + C 3 * X ^ 2 + C 3 * X + C 2 := by
...
private theorem hgprodIV : Uniformity.Density.monicPoly aIV = ∏ i : Fin 2, gfIV i := by
  ...
  show _ = gA * gB
  unfold gA gB
  have h3 : (C (3 : ℤ_[2]) : Polynomial ℤ_[2]) = C 2 + 1 := by
    rw [← Polynomial.C_1, ← Polynomial.C_add]; norm_num
  rw [h3]; ring

/-! ### the two blocks' perimeter closures, and the distinctness of the two keys -/

private theorem hne01 : (X : Polynomial ℤ_[2]).map (IsLocalRing.residue ℤ_[2])
    ≠ kB.map (IsLocalRing.residue ℤ_[2]) := by
  intro h
  have hdX : ((X : Polynomial ℤ_[2]).map (IsLocalRing.residue ℤ_[2])).natDegree = 1 := by
    rw [Polynomial.monic_X.natDegree_map, Polynomial.natDegree_X]
  have hdB : (kB.map (IsLocalRing.residue ℤ_[2])).natDegree = 2 := by
    rw [hkBmon.natDegree_map, hkBdeg]
  rw [h, hdB] at hdX
  omega

private theorem hperim0 : ∀ gT : Polynomial ℤ_[2], gT.Monic →
    gT.natDegree = gA.natDegree →
    (∀ k, (2 : ℤ_[2]) ^ 2 ∣ (gA - gT).coeff k) →
  ...
  have hbound := GateKit.perim_degree_bound hpi2 GateKit.isKey_X (N := 2) (by norm_num)
...
private theorem hperim1 : ∀ gT : Polynomial ℤ_[2], gT.Monic →
    gT.natDegree = gB.natDegree →
    (∀ k, (2 : ℤ_[2]) ^ 2 ∣ (gB - gT).coeff k) →
    ∀ u ℓ : ℕ, 0 < ℓ → Nat.Coprime u ℓ →
    ∀ gS : Polynomial ℤ_[2], gS.Monic → gS ∣ gT → IsPure kB gS u ℓ →
    ∀ hne' : (sideSet kB gS u ℓ).Nonempty, ∀ H₀ : ℕ,
      npHgt kB gS (sideMin kB gS u ℓ hne') = (H₀ : ℕ∞) →
      ∀ ψ : Polynomial (resField kB), ψ.Monic → Irreducible ψ →
        (∃ c : (resField kB)ˣ, resPoly (2 : ℤ_[2]) kB gS u ℓ hne' H₀ = c • ψ) →
  ...
  have hbound := GateKit.perim_degree_bound hpi2 hkeyB (N := 2) (by norm_num)
...
/-- **B.83 (iv) — the `q = 2` split gate.** The level-2 class of `![2, 3, 3]` over `ℤ_[2]` is
...
theorem gate_split_two :
    Uniformity.Density.DecidedAt ℤ_[2] 3 ⟨{(1, 1), (1, 2)}⟩ 2
      (Uniformity.Density.proj ℤ_[2] 3 2 ![2, 3, 3]) := by
  have h := decidedAt_of_order1_certificate (O := ℤ_[2]) (π := (2 : ℤ_[2])) hpi2 (n := 3)
  ...
  have hsum : ∑ i : Fin 2, (order1Type (2 : ℤ_[2]) (phiIV i) (gfIV i)).data
      = {(1, 1), (1, 2)} := by
    rw [Fin.sum_univ_two]
    show (order1Type (2 : ℤ_[2]) (X : Polynomial ℤ_[2]) gA).data
      + (order1Type (2 : ℤ_[2]) kB gB).data = _
    rw [hordA, hordB]
    rfl
  rwa [hsum] at h
...
section AxCheck
#print axioms Uniformity.Density.Leaf.gate_split_two
end AxCheck
