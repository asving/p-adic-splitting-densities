...
the three pack row-laws PACK-i/ii/iii), — added at REVISION 5, findings VC5-1/VC5-2 —
the **(H6) terminal-seam engine-conformance group** ((T-DEC)/(T-READ)/(T-VERD) +
D-11 item (vii)), and — added at REVISION 6, finding F-1 — **(H7)**, the (I-τ)/K-LOC
sibling-interface conformance pair). **THE THEOREM AS
...
> rsh_interp, **(PACK-ii)** r_bdd, **(PACK-iii)** wsh_ok, displayed at the §3.1
> row — consumed at Step 18 with the sub-attribution exact [synced at REVISION 8,
> finding VC8-2]: PACK-i/ii at K2's evaluation glue, PACK-iii the membership
> input to K3's evaluation ring-hom (with D-6) — explicit because
...
> REVISION 10, r11, from the O-11 owed-pass G-2]: exactly-one shallow
...
(K-CUT)** (§3.1) [NEW at REVISION 10, r11, from the O-11 owed-pass G-2: the
...
(the evalAt regular-subring membership, with D-6): they
...
IN-STATEMENT (the owed-pass C6 counterexample class; for them only O-12's pole/zero
