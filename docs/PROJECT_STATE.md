...
| H2L | the h ≥ 2 currency | DONE — IFCG75.lean (35/35 PURE core, from the codex design): the Bézout normalization ring (the naive order never appears); the h=1 cascade instantiated; the mult-by-h lattice; ★★ the count-preserving bezCellTransport; `SlopeBezoutFactorCorrespondenceAt` stated-not-asserted (outside the FGMN package; owner-gated) with TS/DT reductions; the (RB) criterion fired outright |
| OTAU | A-I.9 enacted (owner-authorized) | DONE (984f33ad) — the NormalizerPinned conjunct in the deepTwist socket + the ladder live guard; direction verified 3 ways; capstone footprints unchanged (full leanspec 9533 green); ★★★ BOTH vartheta halves = ONE row: `UniversalPinnedGeneratorDescent` (R5); REVIEW_QUEUE row 10 ENACTED |
| SDJ | the sector-depth joint census | DONE — IFCG74.lean: ★★ the cluster bridge (the joint census IS the assembly currency); the sector split (strays only); the grouping = the incidence decomposition (the per-σ constant refuted); the convolution row = TWO named census values (tableSlice + strays) |
| R2F | the r ≥ 2 frames | DONE — IFCG72.lean (2888 lines): ★★★ the general fiber Lean-core (+ the new any-domain exact_factorization_unique); LSF's drainage fence DISSOLVED; the frame laws on ONE census family (`SteepDecCensusLaw`; special frames PURE core; the first r=2 law closed); Finding A recorded (the naive value law FALSE at r ≥ 2) |
| GCV | genre cluster values | DONE — IFCG73.lean: ★★★ the strata masses SUM TO 1 (Σ zed·q^{−v} = q^{2n}, the primo fixed point — no DVR witness); `deepStratumDrain_all` at EVERY split (hDR gone); the convolution row = {sector × depth joint census, the block-grouping census} — both charged mechanisms honestly refuted |
| VA23 | axes 2–3 of the census | DONE — IFCG71.lean: the rationality subtlety DISCHARGED (Tannery + geometric tails, one rational witness); the cluster/collision assembly through the bridge; the honest re-base: the row = {genre-restricted cluster values, min ≥ 2 drains ((2,2)/(2,3) only), collision limits}; min-1 drains OUTRIGHT |
| GNA | the general-n assembly | DONE — IFCG70.lean (9/9 PURE core): ★★★ `zcURLim_depthDensity` at ALL (n₁,n₂) — VCL'S AXIS 1 CLOSED (the census polynomial in q; the fold = Multiset.prod in PowerSeries; the duo regression exact) |
| SFW | the witness frame laws | DONE — IFCG69.lean: ★★★ r=1 frames UNCONDITIONAL (drainage legs discharged not carried; the separation engine Hensel-free); ★★★ `leafSubfaceLaw_five` — the first non-singleton leaf law; masters at r ≥ 2 frames only; remainder = the mass-r steep census + the Bezout fiber gap |
| NA22 | the n=2 assembly | DONE — IFCG68.lean (23/23 PURE core): ★★★ the (2,2) depth density UNCONDITIONAL (the duo law; 5/5 Z₂ regressions exact vs IGV's battery); the general-n remainder = n-ary convolution + partition type-grouping + per-type primo |
| DPN | the depth pin | DONE — IFCG67.lean (pure core): ★★ TBT's ladder constant PROVED (mixDepth = h·d·r; the weighted-Leibniz Sylvester route; corner free from IsPure); the CSL couplings fired at the known depth; SubfaceFrameLaw = one census lemma + drainage at the witness frames |
| LSF | the leaf subface law | DONE — IFCG66.lean: the leaf row FRAME-PARTITIONED (finite `subFrames`, exact count, MASTER_FRAMES ×3); remainder = per-frame `SubfaceFrameLaw` + the depth pin v = h·d·(m−e'd) + drainage coupling |
| PSP | the primary-shape peel | DONE — IFCG65.lean (26/26 PURE core, no IsAdicComplete): ★★★ step (c) CLOSED — the e=1 law exact, the peel recursion solved, `primo` closed form (every primary value an N-free finite computation, cross-pinned twice); step (d) = residue-field combinatorics (n=2 one unit away) |
| CP2 | the consolidated cite package | DONE — I10CitePkg.lean: ★★★ ONE signing surface (FGMNCitePackage = 3 legs, sources pinned FGMN Thm 6.2/6.4/Cor 6.6 by PDF read, verbatim quotes in docstrings); all consumers wired + teeth; row 9 rewritten one-signature; the h ≥ 2 recentring step LOUDLY not smuggled; post-signing: hFC outright, hMixed = assembly + the h≥2 gap |
| MFL | the mixed frame laws | DONE — IFCG64.lean (pure core): HONEST NEGATIVE ×2 (CN2 is denominator-=m only; FC misses the unramified leg — the pointwise OM steps' hypothesis vectors are literature questions, NOT invented); the (h,1) COUNT layer closed transport-free; ★★ the compositum cell transport with exact normalization; row-9 cite-scope note recorded (may need the unramified leg + e' < m criterion) |
| DGT | the diagonal transcription | DONE — IFCG63.lean (20/20 PURE core): ★★ resultant = norm over ANY ring (not in mathlib) + the full binary localization (CRT convolution, Hensel stratum assembly, the shape partition — the census IS a sum over factor shapes); remainder = the primary-shape peel + the Euler/zeta assembly |
| EFR | the Eis full-span remainder | DONE — IFCG62.lean: ★★ the mixed compositum lattice (PSL3's missing instrument) + the target-generic digit criterion; ★★★ row 2 on the KERNEL-DECIDABLE `mixedFrames m` per-frame laws (exact tiling proved; m=4 = ∅ sanity; 6 = one frame; 8 = two); off-degree unconditional; remainder = the per-frame count/decision transports only |
| CCT | carrier to calculus | DONE — I10CalcBridge.lean: ★★ the generic RungPack→FGMNCalculus transcription (the graded half FREE from pack laws — Rgr_mul premise-free via the coboundary twist); the depth-4 keystone calculus re-derived through the pack route + μ-route agreement + a consumer fired; remainder = ONE carrier `PackNormalizedLeg` (the OPEN-DICT frontier); C.92 stands |
| SPB | the spine bridge | DONE — I10SpineBridge.lean: ★★ the FIRST BankWiringSupply instance; the wiring dissected by exact iffs at the ReadOneAt genre; ★★ at wired card-2 realizations the site half = THE BANK ALONE (residue vanishes); the boundary stratified; remainder tower-side (carrier→calculus transcription + PrevGrade f' ≥ 2); the meta-core blowup gotcha logged |
| R4K | the rung-4 key lift | DONE — I10RungLift4.lean (pure core, first-attempt): the μ₅ dictionary + the leaf key fired; ★★ THE KEYSTONE TOWER SIDE OF THE DEEPTWIST BANK IS COMPLETE (rungs 2/3/4 = g8/g16/μ₅-leaf; no rung-5 key exists to lift); remainder = the carrier→calculus spine bridge (realization-side) |
| IGV | the per-depth Igusa values | DONE — IFCG61.lean (17/17 PURE core): ★★ the master law Z_m(t) = ∏(1−s^jt^{j−1})/(1−s^jt^j) DERIVED + 17/17 integer-exact battery (NOVELTY UNCLAIMED — literature lookup queued); the min-reduction + the complete min=1 law + its ZcURLim in Lean; remainder = diagonal m ≥ 2 transcription + vis-landing + summed rationality |
| W3P | the S2 rung-3 plumbing | DONE — I10RungLift3W.lean (pure core): W-a/b/c ALL plumbing as forecast; ★ the keystone tower inhabited to rung 3+ (towerPacks fires; the rung-4 carrier unconditional); remainder = the rung-4 key lift's three data (the μ₅ coefficient layer D4-13..16) |
| VCL | the vis census law | DONE — IFCG60.lean: the honest exact cell count (collision-corrected; the naive identity FALSE); ★★★ singleton types fire OUTRIGHT both genres (MASTER''' — every census row guarded 2 ≤ card σ.data); the ramified remainder = three named axes (per-depth Igusa values, summed rationality, collision grouping) |
| RL3 | the rung lifts | DONE — I10RungLift3.lean (pure core): the root relation BY CONSTRUCTION; supp laws derived; ★★ the f'=1 closed form; the per-rung remainder minimal; S2 rung-3 = 3 named plumbing lemmas |
| TBT | the two-block transport | DONE — IFCG59.lean (zero new defs): the genre-free σ-decomposition transport; ★★★ LeafSectorLaw UNCONDITIONAL at every singleton type; the leaf row = LeafSubfaceLaw at 2 ≤ card σ.data; Eis-row vacuity + the VisCensus type-support constraint; v(Res) ladder constant recorded (not machine-checked) |
| PCD | the power defect law | DONE — IFCG58.lean: the replay cite-free; the proper power sector EMPTY at m ≤ 4; ★★ ONE genre-indexed `VisCensusLaw` fuses both defect premises (bridges Iff.rfl) |
| LSC | the leaf sector count | DONE — IFCG57.lean: the general digit bridge + the exact necklace count N_d(q)·q^(mK−wLeaf) (spanning half UNCONDITIONAL); the full fire refuted (m=5 witness) — row 5 = `LeafSubfaceLaw`; the two-block-transport pointer: ONE unit likely serves rows 2/3/5 |
| RCD | root counts + drainage tiling | DONE — IFCG56.lean: ★★★ CSL's drainage premise CLOSED unconditionally (via KDW's drainageAt_all — the a2 dividend); the tiling Lean-core (fibers = pairNbhd clusters on the truncation cylinder); the depth strata N-FREE digit boxes; ★★★ `splitSectorLaw_of_visCountLaw` — the split front cite-free on ONE Prop (`VisCountLaw`, m ≥ 4 ramified; everything else fired) |
| SCB | the scoreboard reconciliation | DONE — IFCG55.lean: ★★★ MASS 4 = FactorCorrespondenceAt 2 ALONE (`coneRemainderLaw_four_all`, every type; the DDT/RIS composition was a missing import); stale hBr rows killed; ★★★ `decidedSliceAt_all_of_MASTER` — the census on FIVE premises: {FC_D (staged cite), EisFullSpan m ≥ 6, Split/Power defect e ≥ 5, LeafSectorLaw e ≥ 5}; ★★★ `capstoneHypotheses_of_MASTER` — the whole remaining surface as ONE statement |
| DDT | the dd recursion terms | DONE — IFCG54.lean (PARI 9/9 + codex no-critical): ★★★ the E2 dd tower CITE-FREE (the linear-key self-similarity — transport free, geometric weights on landed counts; E2L's guesses refuted); two mass-4 rows closed outright; the front E2-FREE; mass 4 = {FC 2, E1IrrLeafDecision} — RECONCILIATION FLAGGED (RIS already fired the leaf) |
| IEC | the iface export core | DONE — I10IfaceCore.lean: the E.11 windows engine built fresh; R1 discharged at every `SteepBlock` pair (executed nonvacuity); the XL item = ONE inequality (`LadderSteepSupplier`); lb1 = hpart + R3, package = R1' + R3 |
| TAU | the tau rows' quantification | DONE — I10TauRows.lean: ★★ STOP-LINE: `UniversalKernelSpan` REFUTED at the keystone (the index-2 twist fires the LITERAL socket yet kills kernel-span) — the vartheta halves REQUIRE the O-TAU owner amendment (pin the normalizer / conjoin pinning; REVIEW_QUEUE row 10); under it the tau boundary = row R5 only; `wrapDescentAt_not_uniform` — the wrap clause machine-separated from the carrier laws |
| E2L | the E2 depth-0 limits | DONE — IFCG53.lean (2877 lines): the genre partition with EXACT counts (the digit-triple fiber engine); the ℓ=1 inert leaf UNCONDITIONAL (B.44+CN-21); the three e2 limits on the named `e2DdDecFam` terms alone, every other σ at 0 unconditionally; the mass-4 tails = ONE deep premise each; the front rewired (`decidedSliceAt_all_of_dd_limits`) |
| CEX | the C-to-E export layer | DONE — I10CExport.lean: ★ the package supplier CLOSED at every degree (the signed leg PROVED TAUTOLOGICAL — machine-verified disclosure for the review queue: the field never constrained arising data); ★ the ladder boundary = {lb1, vartheta} EXACTLY; the export core typed with content + not-vacuous teeth; remainder R1 (IfaceExportedCore at arising pairs) + R3 (owner re-signs, flagged) |
| LDR | the ladder field's legs | DONE — I10LadderLegs.lean: ★ the field ⟺ THREE suppliers (mp1 eliminated); the vartheta leg SUBSUMED into the deepTwist boundary (shared tau rows — refutation-backed: no unconditional canonical-normalizer closure); LB1G's Exported relation = its named gap (owner-gated supersession; anti-cook fence landed); open = the C-to-E export layer + owner amendment |
| ID12 | the three I-D12 bodies | DONE — ID12.lean: the three fields bind True-bodied carriers (defect I-D12) — closed at every n AS SIGNED (tripwires landed; the real-body remainders ledgered per field); ★★★ `capstoneHypotheses_of_frontier` (Lean-core): CapstoneHypotheses n = EXACTLY {ladder, deepTwist, a0}, machine-checked |
