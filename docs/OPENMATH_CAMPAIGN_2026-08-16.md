...
| OM-9 | the weld conjuncts (JD0-BOX-2 side; GENHN-BOX-2 CLOSED by WELD-ZERO pass 2 (2026-08-18)) | HYP.74, HYP.148+139 | **DONE** — chapter F's FIRST landings: §4+§5 complete in leanfinal (16 files; Display-A carriers `JD0Box2`/`GenhnBox2` + `W1Transport` at gate signatures; 9/15 axiom stubs PROVED incl. F.09 assoc + F.10b power basis; F04w strike⟹perimeter + gauged-cocycle preservation; F12w `w1Transport_iff_zero_iff` — the carrier is the transport's SIGNATURE, machine-checked; fresh `CarryCocycle.c_comm`); 95/0 battery, 8/8 mutants; all 29 theorems Lean-core; HYP.74/148 CARRY + HYP.139 MATH unchanged, residuals pinned; WELD-ZERO fence verified untouched (A-W.2) |
| OM-10 | the n≥8 box family | HYP.67–71/78–80/142/143/144-BOX-2 | **DONE** — 5 mechanism groups, batteries 569/0 + F₉ 43M exhaustive, 12/12 teeth; HYP.142 RECONCILED (GENHN governs, transcription-confirmation performed verbatim, F₉ leg = first odd-char f₁=2 count contact, NO escalation); HYP.143 branch (ii) FIRST machine contact ever (41 rows, all 4 cells, |S_r|=D′ℓd everywhere, failure mode excluded, repair-route instruments certified); HYP.71(b) witnesses REFUTED(+repair): LAW E-W (mindiff = Θ + entry excess, tight) makes Θ-attainment infeasible in-locus at all-in-grid genres; two new certified laws (P-BINOM single-slot 9/9 + pair-or-triple cascade 7/7, any-k candidate refuted at B4); f₁≥2 compound first contact (6.5 laws survive verbatim, 6 frames incl. sealed key + η≠1); HYP.68/144-BOX-2 REDUCES-TO(6.6) with first enumeration checks; all dispositions unchanged; no Lean landing (adjudicated: the C.95/C.96 signed stubs are the C fleet's §9 surface) |
...
## 2026-08-18 rider — OM-9 second leg CLOSED (WELD-ZERO pass 2, codex sol, VERIFIED)

`leanfinal/notes/WELDZERO_PASS2_2026-08-18.md`: **HYP.148 DISCHARGED** (GENHN-4 layer 1
proved without COROLLARY W-9, uniformly in all admissible stage data and residue
characteristic) and **HYP.139 RETIRED FROM THE CAPSTONE CONE** (the replacement proof
consumes no FGMN object; `TR-3′-GEN` itself remains an OFF-CONE GRTJC gap, not proved).
Independent adversarial verification (fresh-context codex sol): **NO FINDINGS** over 8
checks — scope agreement with the ledger rows, uniformity general (GENHN-LIFT supplies
every λ at every M > D′h), no circularity through W-9, the convention pin against
GENTOW5-A1, no remaining capstone consumer of the W-1 transport (the landed
`WeldObligations.w1` field stays pending its authorized amendment but has no downstream
projection use), chapter-F signatures as claimed, A-W.2 fence untouched, and the numeric
battery re-run (9,877 checks, 0 violations). HYP.74 unaffected.
