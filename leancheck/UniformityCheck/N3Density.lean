...
* **`genuineDensity_ge_of_inj`** — an injection of a finite type into the `σ`-decided level-`N`
  classes is a density lower bound `#A / q ^ (3N) ≤ genuineDensity`. (Only an injection is
...
* **`bracket_five`** — the 5-type analogue of `leanfinal`'s `bracket_two`: lower bounds on the
  five cubic types turn, through `sum_genuineDensity_le_one` (which is general in `n`), into an
  upper bound on each one.

and then the two families whose certificates live in `N3Certs`:

* `gate_linRam_lower` : `(q-1)² / q⁴ ≤ genuineDensity O 3 c3linRam`  (level 2, `q²(q-1)²` classes);
* `gate_split3_lower` : `1 / q⁹ ≤ genuineDensity O 3 c3split`         (level 3, one class);
* `gate_linInert_lower` : `1 / q³ ≤ genuineDensity O 3 c3linInert`    (level 1, one class —
...
theorem genuineDensity_ge_of_inj {n N : ℕ} {σ : FactorizationType} {A : Type*} [Finite A]
    (f : A → Coeff O n N) (hinj : Function.Injective f)
    (hf : ∀ x, DecidedAt O n σ N (f x)) :
    (Nat.card A : ℝ) / (residueCard O : ℝ) ^ (n * N) ≤ genuineDensity O n σ := by
...
theorem bracket_five {ls li lc lr lt : ℝ}
    (hs : ls ≤ genuineDensity O 3 c3split)
    (hi : li ≤ genuineDensity O 3 c3linInert)
    (hc : lc ≤ genuineDensity O 3 c3inert)
    (hr : lr ≤ genuineDensity O 3 c3linRam)
    (ht : lt ≤ genuineDensity O 3 c3ram) :
    genuineDensity O 3 c3split ≤ 1 - li - lc - lr - lt
    ∧ genuineDensity O 3 c3linInert ≤ 1 - ls - lc - lr - lt
    ∧ genuineDensity O 3 c3inert ≤ 1 - ls - li - lr - lt
    ∧ genuineDensity O 3 c3linRam ≤ 1 - ls - li - lc - lt
    ∧ genuineDensity O 3 c3ram ≤ 1 - ls - li - lc - lr := by
  classical
  have hsum : ∑ τ ∈ ({c3split, c3linInert, c3inert, c3linRam, c3ram} :
      Finset FactorizationType), genuineDensity O 3 τ ≤ 1 := sum_genuineDensity_le_one 3 _
...
/-- **G-LINRAM.** `(q-1)² / q⁴ ≤ genuineDensity O 3 c3linRam`. -/
theorem gate_linRam_lower :
    ((residueCard O : ℝ) - 1) ^ 2 / (residueCard O : ℝ) ^ 4
      ≤ genuineDensity O 3 c3linRam := by
  classical
  obtain ⟨π, hπ⟩ := IsDiscreteValuationRing.exists_irreducible O
  have hbound := genuineDensity_ge_of_inj (O := O) (n := 3) (N := 2) (σ := c3linRam)
...
/-- **G-SPLIT3.** `1 / q⁹ ≤ genuineDensity O 3 c3split`, every `O`. -/
theorem gate_split3_lower : 1 / (residueCard O : ℝ) ^ 9 ≤ genuineDensity O 3 c3split := by
  obtain ⟨π, hπ⟩ := IsDiscreteValuationRing.exists_irreducible O
  have h := genuineDensity_ge_of_decided (split3_decided hπ)
  simpa using h
...
/-- **G-LININERT (crude).** `1 / q³ ≤ genuineDensity O 3 c3linInert`, every `O`. -/
theorem gate_linInert_lower : 1 / (residueCard O : ℝ) ^ 3 ≤ genuineDensity O 3 c3linInert := by
  ...
  have hani : Anisotropic ![β₀, β₁] := by
    ...
    exact hp
  have h := genuineDensity_ge_of_decided (linInert_decided (O := O) 0 β₀ β₁ hani)
  simpa using h
