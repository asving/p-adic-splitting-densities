...
= (r1) ∧ (r2) at the REAL (bridged) instance, all n ∈ {2,3}, all primes p < 2000.
...
# ===========================================================================
# 4. THE SCAN — (r1), (r2-def), (r2-act i) at every prime p < 2000
# ===========================================================================
print()
print("=" * 74)
print("SCAN: (REG-p) at the real tables, all p < 2000, pools p^delta, delta<=4")
print("=" * 74)
...
for n in (2, 3):
    ...
    for p in PRIMES:
        ...
        if p_ok_faithful:
            n_pass += 1
    results[n] = n_pass
    print(f"n={n}: {n_pass}/{len(PRIMES)} primes < 2000 PASS (REG-p) at faithful pools "
          f"Q(p) = {{p}}")
...
_pd = f"NO FAILING PRIME — passing density {len(PRIMES)}/{len(PRIMES)} = 1"
print(f"VERDICT (faithful REG-p, p < 2000, n in {{2,3}}): "
      f"{_pd if verdict_ok else 'SEE FAILURES'}")
print("=" * 74)

out = {'id': 'N6-regp-ext2000',
...
with open(os.path.join(HERE, 'N6-regp_results_ext2000.json'), 'w') as fh:
    json.dump(out, fh, indent=1)
print("results JSON -> N6-regp_results_ext2000.json")
