#!/usr/bin/env python3
"""sdyj_checks_v2.py — (SD-YJ') machine leg, VERSION 2 (Y-r1 repair of PE1 G-1).

v2 of sdyj_checks.py (v1 committed at a44cb70, byte-untouched, and md5-pinned
in PINS below).  SOLE substantive change: the SDYJ-CASC "line = betahat"
sub-clause is DE-TAUTOLOGIZED.  In v1, bhat was DEFINED as the cascade cell's
own line (bhat := crec['line']), so that sub-clause verified nothing (PE1
finding G-1); the other CASC sub-clauses (what == u1+u1'+u0, residue != 0,
uniqueness) and all other families were real.  Here betahat is recomputed
INDEPENDENTLY from the S-L1 pool formula (SURV_PROOF_2026-08-08.md S3):

    betahat = lamhat + sum_{l=1}^{m-1} c_l * q_l ,
    lamhat  = line(a) + line(a') + gamma_{m+1} - 2*w_mPhi_m   (W-D5),
    q_l     = h_l * prod_{l<t<m} e_t ,

with the carries c_l taken from the pool arithmetic (WS.pools_of on the
anchor splits — decorrelated from the walk that produced crec['line']), and
lamhat via E.line(b0, sig) on the W-D1 formal cell exactly as the accepted
wmultdcx_derive_checks.py DCK-LINE family computes it.  The check is
crec['line'] == betahat_pool on every cascade cell.  Output filenames carry
the _v2 suffix; everything else is v1 verbatim.
...
PINS = {
    ...
    # [v2] the v1 runner, byte-untouched, pinned at its committed md5:
    "sdyj_checks.py":                 "f77500b474196e3b76a7b80c40eb0012",
}
...
def trace_row(E, sA, sB, u1a, u1b, aA, aB, su, cs, pools, Dd, kdef, key, prin,
    ...
    if crec is not None:
        ...
        # [v2, the PE1 G-1 repair] "line = betahat" DE-TAUTOLOGIZED:
        # betahat recomputed independently from the S-L1 pool formula
        # betahat = lamhat + sum_l c_l*q_l (lamhat = line(a)+line(a')
        # + gamma_{m+1} - 2*w_mPhi_m via the W-D1 formal cell; q_l =
        # h_l*prod_{l<t<m} e_t; carries c_l from WS.pools_of — pool
        # arithmetic, decorrelated from the walk).  v1 set bhat :=
        # crec['line'], making this sub-clause circular.
        qv = WD.q_quanta(E)
        lamhat = E.line(b0, sig) + E.gam[m + 1] - 2 * E.wmPhi
        bhat_pool = lamhat + sum(cs[l - 1] * qv[l] for l in range(1, m))
        if bhat != bhat_pool:
            viol("SDYJ-CASC", tag, f"cascade line {bhat} != S-L1 pool "
                 f"betahat {bhat_pool} at {key} (lamhat={lamhat}, cs={cs})")
        else:
            note("casc_pool_line_ok")
        note("casc_rows")
        acc = []
        for c in slots[jhat]:
...
def main():
    t0 = time.time()
    print("=" * 78)
    print("(SD-YJ') machine leg v2 (Y-r1: de-tautologized CASC betahat) — "
          + ("SHAKEDOWN (throwaway only)" if SHAKE else "full battery"))
    print("=" * 78)
    rowrecs, comprecs = [], []
    if SHAKE:
        run_sweep(mini=True)
    ...
    if not SHAKE:
        out = dict(final=True, violations=VIOL, findings=FINDINGS,
                   ...
                   elapsed=el)
        with open(os.path.join(OUTDIR, "sdyj_checks_v2_results.json"),
                  "w") as f:
            json.dump(out, f, indent=1, default=str)
        print("results -> sdyj_checks_v2_results.json")
