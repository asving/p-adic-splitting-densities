...
**ARC NOTE (2026-08-16, unit EWBS2 — LAW E-W CERTIFIED → PROVED).** The named open above is DISCHARGED: `lean/notes/openmath/LAWEW_PROOF_2026-08-16.md` PROVES LAW E-W at the trinomial single-crossing geography class (which contains W1/W2 — and every frame of the widened battery: `D′ ∈ {3..6}`, `i₂ ∈ {1,2}`, `m ∈ {3,4,5}`, `μ₂ ∈ {2,3}`, `p ∈ {2,3,5}`, `ω ≠ 1`). Its COROLLARY C2 proves conjunct (b)'s infeasibility for the FULL in-budget locus at μ₂ = 2 (arbitrarily many entries — the sweep had certified one and two), and COROLLARY C1 proves the T-E-BOUND tightness; **the refutation leg of the REFUTED(+repair) verdict is therefore PROVED**, and the dated repair stands on a proved base. Method record: every proof display was machine-certified BIT-EXACT before composition (`verification/openmath/ewbs2_lawew_cert.py`, GREEN 2796/0, 5/5 teeth) and the note passed adversarial verification (Codex; pass 3 clean twice, NO FINDINGS). Conjunct (a) (genre-general nonvanishing) UNTOUCHED and open; HYP.112's fence untouched. Blueprint home: NODE C.127 [A-C.4], leanspec stubs `lawEW_faithful_of_nox`/`lawEW_faithful_high`/`lawEW_pin`. Disposition UNCHANGED: MATH (conjunct (a) remains).
...
**ARC NOTE (2026-08-16, unit EWBS2 — the single-crossing classification CERTIFIED → PROVED).** LAW E-W now has a proof (`lean/notes/openmath/LAWEW_PROOF_2026-08-16.md`; every display machine-certified bit-exact, Codex pass 3 clean twice), so the classification clause of the OM-10 note above is a THEOREM: at μ₂ = 2 trinomial single-crossing geographies a member diverges iff its development carries a crossing slot; with exactly ONE crossing slot the height is Θ_j + excess at both coordinates (COROLLARY C3), and with several crossing slots of pairwise DISTINCT excesses it is Θ_j + min excess exactly. The hard residue is NARROWED from "multi-crossing / free-digit-mixing" to exactly: (i) EQUAL-EXCESS multi-crossing ties; (ii) `j′ ≥ 2` entries at μ₂ ≥ 3 — where the naive per-coordinate law is now REFUTED at p = 2 by a constructed member (battery frame V1p2m3: pin one grade high at the two-seed rows; all 92 p = 3 reads agree) — and (iii) non-trinomial keys. Blueprint home: NODE C.127 [A-C.4]. Disposition UNCHANGED: MATH (the ties remain the census hard-unit, at a tighter address).
...
**ARC NOTE (2026-08-16, unit C.128).** The CASCADE half moves **CERTIFIED → PROVED at its geography pin** (`e₂ = 1`, `e₁ = f₂ = 2` — the entire certified surface; any complete DVR, non-binomial one-sided keys `κ₁ ≠ 0` included, any odd h, μ₂ ≥ 2): `lean/notes/openmath/LAWBS2_PROOF_2026-08-16.md`, the LAW E-W census one Φ₂-division deeper — development identity → wrap-seed sum → weight grading (depth-j seeds at layer μ₂E₂ + jδ) → top-band chain lemma → EXACT three-slot census {(0,1): b₂ĉ₂²π^{2v₂}, (1,0): (b₃−(μ₂−1)b₂)ĉ₂³π^{3v₂}, (0,0): b₄ĉ₂⁴π^{4v₂} − κ₁b₃ĉ₂³π^{3v₂}} → the iff via the congruence swap (p ∣ b₂ ⇒ M ≡ b₃). Every display certified BEFORE composition (`bs2_lawbs2_cert.py` GREEN 666/0, 6 teeth; 47 binomial + 6 κ₁-frames bit-exact incl. BP9/F5m5/F5m6 — the first p = 3 and p = 5 non-attainment rows); four-pass Codex-verified to a CLEAN PASS (pass 1: two justification gaps in one height display, repaired + machine leg BS2-TAUH; pass 2: four presentational gaps incl. the P32 frozen records, repaired; pass 3: all mathematics recomputed consistent, battery re-run bit-matched, two machine-tag items closed by strengthening the teeth; pass 4: CLEAN). **The B4/B5 refutation is now EXPLAINED**: k ≥ 5 diagonals never reach the top coordinate (wrap climb bound ⌊l/2⌋ ≥ l−2 fails), k = 4 reaches only at wrap depth 2 = layer Θ + δ — the floor-loss is paid in wrap depth, not digit valuation. **The elaborated stub `lawBS2_pair_or_triple` AS STATED (D′, f₂ free) is REFUTED** on two constructed axes (D′ = 4: empty top band; D′ = 3: unit-DEPENDENT merged digit, dead at p = 13/ĉ₀ = 2 on a PARI-valid key) — narrowing flag `F.e₁ = 2` + `T.f₂ = 2` recorded at NODE C.128 (flagged, not executed). The f₂ ≥ 3 slice stays measured-open (structurally different Θ-census); the single-slot half (9/9) and the lower-coordinate rider stay as at the OM-10 ARC. Disposition UNCHANGED: MATH (the rider).
...
CLASS: corpus-consistency-item
DISPOSITION (owner-ratified 2026-08-18): **RECONCILED — GENHN governs; NO MATH escalation** (bucket move from "possible-MATH / dual-disposition" executed on Asvin's one-liner of 2026-08-18; the OM-10 ARC NOTE below is the adjudication record).
PROPOSED DISPOSITION (superseded): NODE — reconcile the two records and say which governs; escalates to MATH if the HE6R1 reading governs, since that reading owes a new count lemma. (v3 delta item 8.)
