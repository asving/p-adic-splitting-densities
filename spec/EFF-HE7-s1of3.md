# EFF-HE7-s1of3 — CANONICAL EFFECTIVE-SPECIFICATION SHARD

**COMPILED-BY: opus (2026-08-14, Asvin-authorized substitution, PROJECT_STATE #51). CODEX CROSS-READ OWED (post-reset). PROSE-GENRE PILOT.**

## 1. Shard header

**Source note:** `lean/notes/openmath/HE7_PROOF_2026-08-08.md` (2652 lines).

**HOME range:** source lines 1–1246 inclusive (front matter + `[r1]` round record + §S0 consumption + §S1 the level-2 frame + §S2 the statements + §S3 (SLOT₂) + §S4 (LIFT₂) + §S5 the transfer audit + §S6 the ladder + §S7 termination + §S8 the n = 8 closure). Later text (§S9–S11, ANNEX F-1, ANNEX R, the dated corrections) is used ONLY to resolve material homed in that range.

**HEAD blob hash:** `af1686858e7faea56549eb4a42130eeab648d61d` (clean in the working tree; last touched by commit `fe589460`).

**Compilation context:** `spec/EFF_TEMPLATE_v2.md`; temporary IDs `EFF.HE7.s1of3.<nn>`.

**GENRE: TAG-SPARSE PROSE PROOF.** `\tag{` density is **0.00** (`grep -c '\tag{'` = 0; `runs/qgen/EFFT3_crossread.md` §4.4 measures HE7 at 0.00 tags/KB and 448 declaration keywords). Statement boundaries are therefore CHOSEN, not read off display tags. §6 below is the mandatory BOUNDARY SELF-AUDIT: every boundary, with both candidates recorded where the call was genuinely arguable.

### Governing effective-text record

The note states no single "the effective text is …" sentence. It states the rule in four places, and the four agree; the effective text is DERIVED from them (template v2 header rule; the T3 precedent's "derive it and state the facts that fix it").

**(i) The accepted body is frozen at PE3** (ANNEX F-1 head, lines 1605–1610, out of HOME range):

> “HE7 was **ACCEPTED 2/2** at PE3 (report
...
**(ii) Every later repair is a dated annex naming its exact target** (ANNEX R head, lines 1982–1985):

> “The accepted body above (through ANNEX F-1) is
> BYTE-FROZEN — md5 verified identical before and after this append —
> and per the annex pattern nothing above is edited: each repair is stated as a
> dated annex definition/lemma/rider naming its exact target.”

**(iii) The note is now closed to everything but dated appends** (line 2493–2497):
...
**(iv) `[r1]`/`[r2]`/`[r3]` were applied IN-TEXT, not by append.** The front matter's own instruction (lines 17–19) is “both applied here, every site tagged `[r1]`”, and the §S9 box records the same for r2 and r3. So every `[r1]`/`[r2]`/`[r3]` bracket inside lines 1–1246 is already-landed base text, not a supersession to be applied.

**EFFECTIVE TEXT FOR THIS SHARD = lines 1–1246 exactly as written (r1/r2/r3 already landed), READ AS CORRECTED by the out-of-range dated riders wherever they name an in-range sentence.** Six such riders land in this shard's range; each is applied at its unit and recorded in §7. No in-range sentence is deleted by any later round — every correction is a supersession that quotes its target and leaves it byte-intact.

### Scope split (template rule 7 — two predicates, never conflated)

**A. ABSORPTION / supply SET** (what the note claims to prove). §S0, lines 50–51:

> “**HE6-BOX-1 (§S7.1) and its exact residue (§S7.2) are this unit's
> problem statement.**”

and, for the in-range supply proper, §S2's four displayed statements (`.13`–`.18`) plus the two named lemmas of §S1 (`.11`, `.12`). The note's own inventory sentence is out of HOME range (§S9, line 1249, homed in shard 2):
...
**B. APPEND / consumption PROTOCOL scope** (what has been landed on other notes). **HE7 lands NOTHING on any supplier.** Every defect it finds in a supplier is owned at the source; HE7's own copy carries only note-to-owner appends. Out-of-range but governing, lines 2549–2551:

> “The exact-set display correction is owned at the source by HE6's owed repair
> round (HE6-BOX-2 queue, PE2 report §S4); this append is the note-to-owner on the
> frozen copy.”

and lines 2383–2387:

> “*(Optional HE6R1-side erratum, recorded here, executed there or not at Asvin's
> discretion: LEMMA HE6R1-3's hypothesis can be widened from "Ψ | f_S" to
> "gcd(f_S, Ψ) ≠ 1" with its proof byte-near-verbatim, which is exactly
> R2-a.)*”

The two predicates are never conflated below: §S1–S8 determine what HE7 supplies mathematically; no dated "absorbed by HE7" append is claimed landed on HE6, HE6R1, GENHN, or HE3, and none is invented. **In the reverse direction HE7 *received* four dated corrections from supplier arcs (HE6 PE2 F-1, HE6 PE3 obs-1, HE6R1 PE2 F-1, HE6R1 PE3 F-1); three of the four land inside this shard's range** and are applied at their units (§7).

### Resolution rules applied

1. Lines 1–1246 supply the HOME material: front matter/arc record, §S0 consumption fences, §S1 frame + DEFINITION HE7-1 + the two CONVENTIONs + (SLOT₂)/(LIFT₂), §S2's four statements, §S3's (SLOT₂) proof, §S4's LEMMA HE7-L1 + FINDING HE7-F1 + (LIFT₂) proof/sharpness/use-site checks, §S5.1–S5.8's transfer audit, §S6's DEFINITION HE7-2 + THEOREM HE7.D + induction, §S7's LEMMA HE7-8 + THEOREM HE7.C proof, §S8's n = 8 closure.
2. `[r1]`, `[r2]`, `[r3]` brackets inside the range are ALREADY-LANDED text (rule (iv) above), applied at their own sites in chronology; the later bracket governs the earlier text it names (e.g. `[r2, HE7-PE1 F-2]` at `.32` corrects the pre-r2 (S5.2) dichotomy; `[r2, HE7-PE1 F-3]` at `.15`/`.17` inserts the integer floor ⌊μ/2⌋).
3. **ANNEX F-1 (lines 1603–1969, shard 2) riders two in-range display sentences by name and does not edit them.** Target 1 = THEOREM HE7.A(3)'s `[r2, HE7-PE1 F-1]` wiring sentence (`.14`); target 2 = §S6(iii)'s block sentence (`.51`). Both are carried as CONDITIONALITY at their units, with the annex's own "Read as" text quoted. LEMMA HE7-8's `[r2]` box (`.54`) and LEMMA HE7-12(d)'s `[r3]` parenthetical (`.44`) “inherit the same reading”.
4. **ANNEX F-1 also falsifies LEMMA HE7-13's conclusion outside its pure-node hypothesis** (line 1626: “HE7-13's conclusion-as-stated is genuinely FALSE at mixed nodes”). HE7-13 (`.45`) is carried WITH its hypothesis, and the mixed-node case is routed to ANNEX-LEMMA HE7-13′ (shard 2). This is a scope pin, not a supersession of the display.
5. **ANNEX R (lines 1973–2474, shard 3) supersedes four in-range items:** ANNEX-DEF HE7-2′ replaces DEFINITION HE7-2's normalizer line and base sentence (`.47`); R3's rider restates THEOREM HE7.C's and §S7's n = 16 sentences to the proved direction (`.15`); R4 completes LEMMA HE7-12(c) item (7)'s dependency inventory (`.44`); R5's rider replaces §S4.2's "subgroup-coset" wording (`.22`).
6. **The post-annex dated corrections (lines 2478–2652, shard 3) supersede three further in-range items:** M-B riders THEOREM HE7.D's §S2 display (`.17`) and COROLLARY HE7.B(ii)'s inductive-step clause (`.16`); the HE6 PE2 F-1 append plus the HE6 PE3 obs-1 line correct LEMMA HE7-L1's realized-set display to the res(·)-form coset (`.19`); the HE6R1 PE2 F-1 line plus the HE6R1 PE3 F-1 line correct §S5.5 [r1](b)'s “(equal outright when separable)” parenthesis to “equal up to a fixed K₂^× scalar per side”, with the cocycle exponent's argument corrected from the side's line value to the PIN height (`.39`). The `[r1]` fold-box's “verbatim” claim at `.20` is re-scoped “as-of the r1 landing”.
7. Where an out-of-range correction lands on an in-range unit, the corrected reading is carried in that unit's CANONICAL STATEMENT (when the correction is a “read X as Y” substitution on a display) or in its CONDITIONALITY (when it is a scope pin), and the superseded sentence is quoted whenever it is load-bearing — the note keeps it byte-intact above.
8. Out-of-range statements are NOT emitted as units here merely because they resolve in-range text. §S9's boxes, §S10/§S11's battery and verdict, ANNEX F-1's HE7-13′, and ANNEX R's HE7-2′/R1-a/R1-b/R1-c/R1-d/R2-a are homed in shards 2 and 3.
9. External repo records (report files, `verification/openmath/*` artifacts, supplier notes) are **reading notes only**, never edits.

### Quotation and table discipline

CANONICAL STATEMENTs are verbatim source quotation or explicitly marked `[ASSEMBLED]`; truncation inside a quotation is marked `…`. The note's ASCII/Unicode displays are reproduced with their line structure intact; the only presentational change is Markdown blockquote framing. The one source table in range (§S8's σ dictionary) is transcribed as a table and flagged `[TABLE]`; no source table is prosified. Compiler-authored ledgers (§3, §4, §5) are flagged `[TABLE — compiler ledger]` and are not source statement units.

---

## 2. Statement inventory

### EFF.HE7.s1of3.01  [changes-record]
...
### EFF.HE7.s1of3.02  [changes-record]
...
Justification tags: the K-basis expansion `by EFF.HE7.s1of3.06` (`K₂ = K(β)`, `deg r = d_r`); the per-slot lift `by EFF.HE7.s1of3.19` (LEMMA HE7-L1); the slot-value and residue readout `by EFF.HE7.s1of3.11` ((SLOT₂)); (‡₂) ⟹ (†₂) `computation`.
...
Justification tags: (1) `by EFF.HE7.s1of3.41` (LEMMA HE7-9(a) and (b)); (2) `by XREF HE6 §S6.2` (the transferred sandwich) + `by EFF.HE7.s1of3.41` (HE7-10 nonemptiness) + `by EFF.HE7.s1of3.42` (HE7-11 Galois) + `by EFF.HE7.s1of3.29` (HE7-5 local forcing) + `by EFF.HE7.s1of3.07` (the substitution table).
...
**DERIVATION:** The clause carries its own two derivations inline, both quoted above: (i) the refutation of the pre-r1 branch — “its residue would have to be a root β₂ of r₂, and β₂ ∈ K₂(β₂) ∖ K₂ is not a value of (LIFT₂)”, `by EFF.HE7.s1of3.12` ((LIFT₂) realises exactly K₂); (ii) the non-propagation arithmetic `L_{λ₂} = ℓ₂·deg R_{λ₂} ≥ ℓ₂·m₂·deg r₂ = m₂·(ℓ₂ deg r₂) ≥ 4` hence `μ₂ ≥ 4`, `by EFF.HE7.s1of3.13` (clause (1)) + `computation`, with the ⌊·⌋ floor supplied at `[r2, HE7-PE1 F-3]`. The α-refine's existence is `by EFF.HE7.s1of3.12`; its finiteness `by EFF.HE7.s1of3.53` (LEMMA HE7-8); the refined-key license `by EFF.HE7.s1of3.44` (LEMMA HE7-12); the strict slope increase `by EFF.HE7.s1of3.45` (LEMMA HE7-13). The μ₂ ≥ 4 gate for the level-3 branch is re-derived inside `.15`'s DERIVATION (§S7).
...
Justification tags: the jump-set identification `by XREF COROLLARY HE6.B [r1]` + `by XREF LEMMA HE6R1-1`; `μ ≥ 4` `computation`; the `μ₂` bound `by XREF LEMMA HE6-3(b)` + `computation` (**explicitly NOT** by the multiplicity-weighted class size, which is HE7-BOX-4's declined link); the iteration `computation`; α-refine finiteness `by EFF.HE7.s1of3.53`; the level-2 re-derivation `by EFF.HE7.s1of3.13` (clause (1)). **The closing n = 16 sentence carries R3's rider.**
...
**DERIVATION:** [ASSEMBLED — the corollary is a composition; its two derivations are displayed inline] (i) `μ ≤ ⌊n/D′⌋ ≤ ⌊15/2⌋ = 7` at `D′ ≥ 2`, then `μ₂ ≤ ⌊μ/2⌋ = 3` `by EFF.HE7.s1of3.15` + `computation` (the two ⌊·⌋ floors are the `[r2, HE7-PE1 F-3]` repair); then the σ-decision `by EFF.HE7.s1of3.13`+`.14`+`.15`; the widened branch's literal decidability `by XREF HE6R1_RECON §S3.2`; the proper-block start `by XREF LEMMA HE6R1-2`. (ii) `by EFF.HE7.s1of3.17`/`.48` (THEOREM HE7.D) — as corrected by ANNEX-COROLLARY R1-d.
...
### EFF.HE7.s1of3.20  [changes-record]
...
### EFF.HE7.s1of3.21  [changes-record]
...
**DERIVATION:** [ASSEMBLED — the two counterexample computations are displayed inline] at `(3,1,2)`: attainable heights `{3a + 2i : a ≥ 0, i < 3} = {0,2,3,4,5,…}` so `k = 1` is unattained; at `(2,2,1)`: `T(0) = T(1) = {0}`, so only `F_Q ⊊ K` is realised — both `by EFF.HE7.s1of3.19` (`T(k)` computed from LEMMA HE7-L1). Non-propagation: every HE6 use has `k = (d−t)u` with `u = ℓκ > ℓD′h ≥ D′h > (D′−1)h` `by XREF DEFINITION HE6-1` (`κ > D′h`) + `computation`, plus the `t = d` case at height 0.
...
**DERIVATION:** [ASSEMBLED] The `W₂` characterisation is “the same bookkeeping without (†₂)” — i.e. `.12`'s DERIVATION with the per-`t` full-residue requirement `m_t = m(k) − tu ∈ W₁` kept as a condition rather than implied, `by EFF.HE7.s1of3.12` + `by EFF.HE7.s1of3.19` (`W₁` is LEMMA HE7-L1's full-`T(k)` set). The “sufficient but not necessary” claim is `by` the tooth's exhaustive enumeration (Q3's level-2 table: e.g. `(2,1,1,2,2,5)` has `thr₂ = 17` with 12 non-surjective `k < thr₂`).
...
**DERIVATION:** Read off `.12`'s proof: the `g_i` within-class heights are `m₀ − t·u_i` (`t < g_i`), each needing a level-`i` full-residue height `≥ bound_i`, and `m₀ ≥ (k − (ℓ_i−1)u_i)/ℓ_i`; solving for `k` gives `bound_{i+1} = (ℓ_ig_i − 1)u_i + ℓ_i·bound_i`. `by EFF.HE7.s1of3.12` + `computation`. The two instances are `bound₁ = (D′−1)h` (`.19`) and `bound₂ = (‡₂)` (`.12`).
...
**DERIVATION:** The chain is displayed inline: `u₂ = ℓ₂κ₂ > ℓ₂T₂ ≥ T₂ = ℓd_r·u`, then `(ℓd_r − 1)u + ℓ(D′−1)h < ℓd_r·u ≤ u₂ ≤ (d₂−t)u₂`, using `ℓ(D′−1)h < ℓD′h < ℓλ = u` — `by EFF.HE7.s1of3.06` (`T₂ := ℓd_r·u`, `u = ℓλ`) + `by XREF DEFINITION HE6-1` (`λ > D′h`) + `computation`.
...
**DERIVATION:** The two displays are the derivation, both `computation` from `.06` (`ℓm₀ + s₀u = k`, `0 ≤ s₀ ≤ ℓ−1`, `T₂ = ℓd_r·u`, `u = ℓλ`) + `by EFF.HE7.s1of3.24` (`k > T₂` at every use site) + `by XREF DEFINITION HE6-1` (`λ > D′h`) + `by EFF.HE7.s1of3.19` (the sharp per-height threshold `(i₀(m_t) + e₁(f₁−1))h ≤ (D′−1)h`). The `ℓ = 1` specialisation is displayed separately and is `computation`.
...
**DERIVATION:** Definition. The two derived clauses are displayed inline: legitimacy (`β^{c_{d₂}−c_t} ∈ K₂^×` and (LIFT₂) surjective, `by EFF.HE7.s1of3.12`) and the degree count (`deg(C_tΨ^{ℓ₂t}) ≤ (D″−1) + D″ℓ₂t < D″ℓ₂d₂`, `computation`). The twist exponents `c_t` are `.08`'s cocycle exponents evaluated at `m₁ := d₂u₂`.
...
Justification tags: the level-1 half `by XREF LEMMA HE6-0′`; the `ℓ₂ | e″` step `computation` (value group + `gcd(u₂, ℓ₂) = 1`); the residue-degree half `by XREF LEMMA HE6-0′`'s proof + `by EFF.HE7.s1of3.06` (the transport `ι^{(2)}`) + `definitional` (minimal polynomial degree).
...
Justification tags: (S5.1) reduction `by EFF.HE7.s1of3.28` (LEMMA HE7-4); (S5.3) `by XREF LEMMA HE6-1` Step 1 + `by EFF.HE7.s1of3.11`(c) + `by EFF.HE7.s1of3.27`; the two-offset case split `by EFF.HE7.s1of3.31` ((S5.2) as corrected at r2); Step 2 `by EFF.HE7.s1of3.11` + `by EFF.HE7.s1of3.08` (the twist) + `by EFF.HE7.s1of3.27` (the inverse prescription).
...
### EFF.HE7.s1of3.31  [changes-record]
...
**DERIVATION:** The failure mechanism is argued inline (`by EFF.HE7.s1of3.27` + `by EFF.HE7.s1of3.41` — the flat identity of LEMMA HE7-9); the numeric witness is `by` the battery's HE7-GEN2 leg.
...
Justification tags: the bulk `by XREF HE6 §S4` under `by EFF.HE7.s1of3.07` (the substitution table); the congruence clause `computation` at `gcd(u₂, ℓ₂) = 1`; the twist carriage `by EFF.HE7.s1of3.30` Step 2.
...
**DERIVATION:** The proof is the displayed one-line argument: `dv₂(f_S(ρ)) = ∞` forces the excess in HE7-7′(b) to be infinite, hence positive — `by EFF.HE7.s1of3.34`(b) — whose positivity criterion is exactly “κ₂ is a slope λ₂ of P₂ and `ι(R_{λ₂})(β₂) = 0`”. Well-definedness `by EFF.HE7.s1of3.08` (the residual is fixed once `n₂` is).
...
**DERIVATION:** Scope/method declaration; its two substantive claims are `by EFF.HE7.s1of3.36` (f_S is a monic O-factor with all roots level-2 points) and `definitional` (monic-by-monic gives top Ψ-coefficient 1).
...
### EFF.HE7.s1of3.38  [changes-record]
...
**DERIVATION:** The failure mechanism is displayed inline (`Ψ(ρ) = 0 ⟹ dv₂(Ψ(ρ)) = ∞ ⟹ A₀^{(2)} = 0 ⟹` no j = 0 pin `⟹ Σ L = μ₂` fails), `by EFF.HE7.s1of3.13` (clause (1)) + `computation`. The disposal is `by XREF LEMMA HE6R1-3` (cited, not reproved).
...
### EFF.HE7.s1of3.39  [instance-record]
...
**DERIVATION:** Cited, not derived: the mechanism (constant `c_g`, translated polygon) is displayed inline but its justification is `by XREF LEMMA HE6R1-2`. The corrected scalar is derived in shard 3 from `by EFF.HE7.s1of3.08` (the cocycle) + ANNEX-LEMMA R1-a(iii)(iv) + LEMMA HE6R1-2(a)'s no-excess display.
...
Justification tags: `by EFF.HE7.s1of3.40` ((RES₂)); the auxiliary `r₂,₀` `by` finite-field existence of irreducibles of every degree (classical, `.03`); the test family `by EFF.HE7.s1of3.27` + `by EFF.HE7.s1of3.24`; the ξ-side `by EFF.HE7.s1of3.30`; both displays `by EFF.HE7.s1of3.34`; the (★) step `by XREF HE3-3` (convex duality); the two samplings `by XREF LEMMA HE6-3`'s own argument under `by EFF.HE7.s1of3.07`; nonemptiness by strict-vs-equal comparison `computation`.
...
Justification tags: g-invariance of v `by` uniqueness of the extension (classical, `.03`); the level-1 half `by XREF LEMMA HE6-5`; the transport equivariance `by EFF.HE7.s1of3.06` (uniqueness of the semilinear embedding).
...
### EFF.HE7.s1of3.43  [changes-record]
...
**DERIVATION:** The slot-geometry computation is displayed inline: `by XREF DEFINITION HE6-1` (the key's coefficient heights `dv(B_t) = (d_r−t)u`) + `computation` (`ℓ(d_r−t)u + ℓtu = ℓd_r·u = T₂`), and `by EFF.HE7.s1of3.11` (the slot-min definition) for `w`'s side.
...
Justification tags: (a) `by EFF.HE7.s1of3.11`(c) + `by EFF.HE7.s1of3.28` + `by EFF.HE7.s1of3.31` ((S5.2) as corrected) + `computation`; (b) `by EFF.HE7.s1of3.11` + `by EFF.HE7.s1of3.28`; (c) an eleven-item transfer audit, each item `by` the named unit; (d) `computation` (ultrametric slotwise), with the `W ≠ 0` half `by EFF.HE7.s1of3.45` (LEMMA HE7-13's strict increase — the two lemmas induct together).
...
Justification tags: package at Ψ̃ `by EFF.HE7.s1of3.44`(c); every root at λ₂ `by EFF.HE7.s1of3.41` (HE7-9(b)); the label `by EFF.HE7.s1of3.35`; the lift `by EFF.HE7.s1of3.12`; the residue cancellation `computation` (same value, same residue ⟹ strictly larger value); the new polygon `by EFF.HE7.s1of3.44`(d) + `.41`; the peel branch `by EFF.HE7.s1of3.44`(c) item (11) + `by EFF.HE7.s1of3.37`.
...
### EFF.HE7.s1of3.46  [regression-record]
...
Justification tags: (SLOT) `by EFF.HE7.s1of3.11` under `.07`'s substitution — **SUPERSEDED: this sentence is exactly what Codex F1 falsified at level ≥ 3; the operative argument is ANNEX-THEOREM R1-b**; (LIFT) `by EFF.HE7.s1of3.12` — **likewise superseded by ANNEX-THEOREM R1-c**; the seven package items `by EFF.HE7.s1of3.28`, `.34`, `.36`/`.37`, `.41`, `.42`, `.13` under `.40` ((RES_i)) and the classical list — **audited per R1.2 items 1–7**; the ξ-side recursion `by EFF.HE7.s1of3.30`/`.31`/`.33` generalised, `computation`, and it is the one step R1.2 item 1 places in the twist-free class (“values, polygons, counts, degrees, bounds … never see residues at all: twist-free, §S6 verbatim”).
...
### EFF.HE7.s1of3.49  [changes-record]
...
**DERIVATION:** A transfer audit of LEMMA HE6R1-3's own inputs against the level-i package: local forcing `by EFF.HE7.s1of3.29` at level i, Galois stability + integral closedness `by` the classical list, `disc ≠ 0` `by EFF.HE7.s1of3.05`.
...
### EFF.HE7.s1of3.50  [changes-record]
...
**DERIVATION:** The correction is an indexing computation: `(LIFT_{i−1})`'s residues lie in `K_{(i−1)+1} = K_i`, which is where the recentring constant must live; the level-2 instance is checked explicitly. `by EFF.HE7.s1of3.48` (THEOREM HE7.D's indexing) + `computation`.
...
### EFF.HE7.s1of3.51  [changes-record]
...
Justification tags: the node inequality `definitional` (the single side's geometry); the coefficient convergence `by EFF.HE7.s1of3.11` ((SLOT₂)'s slot-min formula); the limit `by` completeness of O (classical, `.03`); the contradiction `by EFF.HE7.s1of3.05` (`disc f ≠ 0`).
...
### EFF.HE7.s1of3.54  [changes-record]
...
**DERIVATION:** `by EFF.HE7.s1of3.44` (LEMMA HE7-12 at `W = w_0 + ⋯ + w_{j−1}`) + `by EFF.HE7.s1of3.45` (LEMMA HE7-13's strict increase).
...
### EFF.HE7.s1of3.55  [changes-record]
...
**DERIVATION:** Pigeonhole, displayed: each peel drops μ₂ by exactly 1 (`by EFF.HE7.s1of3.37`/`.38` and `.49`), μ₂ = 1 is decided, hence at most μ₂ − 1 peels; an infinite chain therefore has an infinite constant-μ₂ tail on a fixed separable `f_S^{(seg)} | f_S`, and `.53`'s argument applies to that tail verbatim.
...
### EFF.HE7.s1of3.57  [instance-record]
...
**DERIVATION:** Instantiation of `.06` at `(e₁,f₁,h) = (2,1,1)`, `ℓ = 2`, `d_r = 1`; `|S| = D′L_λ = 8` `by EFF.HE7.s1of3.41` (`n_λ = D′L_λ`) with `L_λ = ℓ·deg R_λ = 2·2 = 4`; no level 3 `by EFF.HE7.s1of3.15`.
...
**DERIVATION:** Case enumeration of the μ₂ = 2 polygon: one side of length 2 (with λ₂ ∉ ℤ forcing ℓ₂ = 2; with λ₂ ∈ ℤ the residual is a quadratic, inert or split or a square) or two sides of length 1 — `by EFF.HE7.s1of3.13` clause (1) (Σ L = μ₂ = 2); letters `by EFF.HE7.s1of3.13` clause (2) (`e = e₁ℓℓ₂ = 4ℓ₂`, `f = f₁d_r deg r₂ = deg r₂`); row 5 `by EFF.HE7.s1of3.14`; row 6 `by EFF.HE7.s1of3.09`/`.38`.
...
### EFF.HE7.s1of3.59  [changes-record]
...
**DERIVATION:** `by EFF.HE7.s1of3.44` (the package at Ψ − w) + `by EFF.HE7.s1of3.13` (clauses (1)(2) at the refined key) + `by EFF.HE7.s1of3.45` (strictly larger slope) + `by EFF.HE7.s1of3.53` (termination).
...
### EFF.HE7.s1of3.60  [changes-record]
...
**DERIVATION:** Comparison of `.58`'s σ column with HE6's HE6-T-CASEB observations; “no fourth value possible” is `by EFF.HE7.s1of3.58` (the enumeration is exhaustive given row 6).
...
**DERIVATION:** An input-by-input audit of GENHN.C's consumption list against the level-2 package: `by EFF.HE7.s1of3.29` ((T1)/(T2) at level 2), `by EFF.HE7.s1of3.11` (exact dv), `by EFF.HE7.s1of3.40`+`.41` (the norm count), `by EFF.HE7.s1of3.53`+`.12` (the refine step).
...
`[TABLE — compiler ledger]` Every row's count is `grep -cF '<designation>' <file>` run at HEAD. **55 distinct designations, all count ≥ 1.**

| # | Target : designation | Count | Role in this shard |
|---|---|---|---|
| 1 | `HE6_PROOF_2026-08-08.md:DEFINITION HE6-1` | 7 | The order-2 key family Ψ_{λ,r}; supplies λ > D′h and the key's slot heights |
| 2 | `HE6_PROOF_2026-08-08.md:LEMMA HE6-0` | 17 | Off-disk level-1 value δ = D′·min(dv ξ, h) |
| 3 | `HE6_PROOF_2026-08-08.md:LEMMA HE6-0′` | 5 | Level-1 local forcing (e₁ℓ \| e, f₁d_r \| f) |
| 4 | `HE6_PROOF_2026-08-08.md:LEMMA HE6-1` | 15 | The level-1 ξ-side; Step 1's estimate and Step 2's transport identity |
| 5 | `HE6_PROOF_2026-08-08.md:LEMMA HE6-1L` | 4 | The corrected level-1 (LIFT); = LEMMA HE7-L1 as of the r1 landing |
| 6 | `HE6_PROOF_2026-08-08.md:LEMMA HE6-2` | 10 | Level-1 evaluation display (× ℓ gives the level-2 disk criterion) |
| 7 | `HE6_PROOF_2026-08-08.md:LEMMA HE6-2′` | 4 | Second level-1 evaluation display (added to HE7-12(c)(7) by ANNEX R R4) |
| 8 | `HE6_PROOF_2026-08-08.md:COROLLARY HE6-2″` | 1 | Level-1 label of a root; the datum (λ, r) "arises as a label of f" |
| 9 | `HE6_PROOF_2026-08-08.md:LEMMA HE6-3` | 12 | Level-1 slope counts (SEP)-free; supplies n_λ = D′L_λ for THEOREM HE7.C |
| 10 | `HE6_PROOF_2026-08-08.md:LEMMA HE6-4` | 8 | Level-1 nonemptiness (added to HE7-12(c)(7) by R4; named in §S6(iii)) |
| 11 | `HE6_PROOF_2026-08-08.md:LEMMA HE6-5` | 5 | Level-1 Galois equivariance |
| 12 | `HE6_PROOF_2026-08-08.md:THEOREM HE6.A` | 19 | The level-1 class-size theorem HE7.A is "one level up"; its sandwich slack |
| 13 | `HE6_PROOF_2026-08-08.md:COROLLARY HE6.B` | 6 | The four-case jump enumeration, in its WIDENED [r1] form |
| 14 | `HE6_PROOF_2026-08-08.md:HE6.B` | 6 | The pre-r1 enumeration error HE7.A(3) repeats one level up |
| 15 | `HE6_PROOF_2026-08-08.md:HE6-BOX-1` | 9 | This unit's problem statement; the n = 8 bite |
| 16 | `HE6_PROOF_2026-08-08.md:HE6-T-CASEB` | 4 | HE6's own tooth; the observed three-σ alphabet |
| 17 | `HE6_PROOF_2026-08-08.md:S1` | 7 (§-anchor) | The inherited frame + ϖ-convention (see `.05`'s caveat) |
| 18 | `HE6_PROOF_2026-08-08.md:S3.3` | 2 | The disk-criterion transfer's source step |
| 19 | `HE6_PROOF_2026-08-08.md:S6.2` | 3 | The class-size sandwich transferred verbatim at HE7.A(2) |
| 20 | `HE6_PROOF_2026-08-08.md:S6.4` | 1 | HE6's own two-route decorrelation at μ ≤ 3 |
| 21 | `HE6_PROOF_2026-08-08.md:S7.2` | 5 | HE6's stated exact residue = HE7's problem statement |
| 22 | `HE6_PROOF_2026-08-08.md:S7.3` | 1 | The FGMN citation targets HE7 declares NOT consumed |
| 23 | `HE6R1_RECON_2026-08-08.md:LEMMA HE6R1-1` | 9 | The widened jump condition with identical bounds |
| 24 | `HE6R1_RECON_2026-08-08.md:LEMMA HE6R1-2` | 8 | The proper-block read off f (μ₂, sides, residual radical) |
| 25 | `HE6R1_RECON_2026-08-08.md:LEMMA HE6R1-3` | 8 | The level-2 peel at Ψ \| f_S (irreducibility half NOT reproved here) |
| 26 | `HE6R1_RECON_2026-08-08.md:FINDING HE6R1-F1` | 4 | The enumeration error found against HE7.A(3)/HE7.C |
| 27 | `HE6R1_RECON_2026-08-08.md:FINDING HE6R1-F2` | 4 | The missing Ψ ∤ f_S hypothesis |
| 28 | `HE6R1_RECON_2026-08-08.md:S3.2` | 6 | The ten-item literal-instance check; the item-2 ℓ = 1 verification |
| 29 | `GENHN_PROOF_2026-08-08.md:LEMMA GENHN-2` | 2 | THE SLOT LEMMA — the ladder's base case |
| 30 | `GENHN_PROOF_2026-08-08.md:GENHN-4` | 6 | Refine transfer — declared NOT consumed |
| 31 | `GENHN_PROOF_2026-08-08.md:GENHN.A(iv)` | 3 | The "binomial kill" LEMMA HE7-8 declares it does not need |
| 32 | `GENHN_PROOF_2026-08-08.md:GENHN.C` | 19 | §S7 trichotomy — argument template only, at §S8 only |
| 33 | `GENHN_PROOF_2026-08-08.md:[GENHN-HE(μ)]` | 1 | The program predicate COROLLARY HE7.B discharges |
| 34 | `GENHN_PROOF_2026-08-08.md:W-9` | 13 | The graded weld face declared NOT consumed |
| 35 | `HE3_PROOF_2026-08-08.md:HE3-3` | 15 | The elementary convex duality (★) of step 1 |
| 36 | `HE3_PROOF_2026-08-08.md:HE3-BOX-6` | 12 | The original defect §S5.5's reduction repairs |
| 37 | `JD0_PROOF_2026-08-08.md:J-D0` | 25 | "counts are twist-blind, letters are twisted" |
| 38 | `W12_PROOF_2026-08-08.md:W-12.D` | 19 | The conditional-stack consumer named in COROLLARY HE7.B |
| 39 | `W12_PROOF_2026-08-08.md:r4` | 59 | The Artin–Schreier trap round (descriptive; see source defect 5) |
| 40 | `GENIND_PROOF_2026-08-08.md:GENIND.B` | 19 | The conditional-stack consumer named in COROLLARY HE7.B |
| 41 | `CODEX_HE6PE1_2026-08-08.md:F2` | 4 | The Φ′ \| f stratum finding whose level-2 analogue is HE6R1-F2 |
| 42 | `CODEX_HE6PE1_2026-08-08.md:finding 4` | 1 | The (LIFT) over-reach found twice (= FINDING HE7-F1) |
| 43 | `CODEX_HE7RAT_2026-08-09.md:F1` | 4 | The CRITICAL on DEFINITION HE7-2 / THEOREM HE7.D |
| 44 | `CODEX_HE7RAT_2026-08-09.md:F4` | 5 | HE7-12(c) item (7)'s incomplete inventory |
| 45 | `CODEX_HE7RAT_2026-08-09.md:F5` | 3 | §S4.2's "subgroup-coset" mislabel |
| 46 | `HE6_passPE2_report.md:F-1` | 6 | The η_θ^{−q(k)} coset defect landing on LEMMA HE7-L1 |
| 47 | `HE6_passPE3_report.md:obs-1` | 1 | The res(·) form of the appended coset display |
| 48 | `HE6R1_passPE2_report.md:F-1` | 3 | The §S5.5 "(equal outright when separable)" scalar defect |
| 49 | `HE7_passPE1_report.md:F-1` | 4 | The refined-key read GAP closed by LEMMAS HE7-12/HE7-13 |
| 50 | `HE7_passPE1_report.md:F-2` | 2 | The false (S5.2) middle-band dichotomy |
| 51 | `HE7_passPE1_report.md:F-4` | 2 | The (LIFT_i) vs (LIFT_{i−1}) off-by-one citation |
| 52 | `HE7_passPE2_report.md:F-1` | 5 | LEMMA HE7-12(d)'s W ≠ 0 hypothesis |
| 53 | `HE7_passPE2_report.md:F-2` | 7 | Mid-chain peels interleaving the refine chain |
| 54 | `HE7_passPE3_report.md:F-1` | 8 | The mixed-residual defect (ANNEX F-1's subject) |
| 55 | `HE6R1_passPE3_report.md:F-1` | 12 | The pin-height correction of the cocycle exponent (`.39`) |

**Designations counted but NOT emitted as XREFs (3):**
- **`HMENU3-ANNEX-A`** — count **0** in `HMENU3_PROOF_2026-08-08.md` (the note's own heading is `## ANNEX A`, count **5**). The referent is unique and verified; HE7's shorthand is note-local. Cited by ANNEX F-1 and §S9 `[r3]`, both shard 2 — the row belongs to shard 2's ledger, recorded here only because the same shorthand recurs.
- **`twist (letters only)`** — count **0** in `GENHN_PROOF_2026-08-08.md`; the target's heading (line 729) is `**The twist (letters only).**` with the period inside the bold. Carried descriptively at `.03`.
- **`stage-α recentering`** / **`stage-CS`** — the HE3 method names used at `.15`/`.18`/`.37` are HE7-local descriptions, not HE3 designations. The parent designations `HE3-3` (15) and `HE3-BOX-6` (12) are verified; the method names are carried descriptively.

**Battery artifacts verified to EXIST on disk** (`verification/openmath/`): `he7_checks.py`, `he7_checks_output.txt`, `he7_checks_results.json`, `he7r1_supp.py`, `he7r2_supp.py`, `he7_pe2_fresh.py`, `he7_pe3_probe12.py`, `he7annex_supp.py`, `he7rannex_supp.py`, `he6r1_checks.py`, `he6_checks.py` — 11/11 present.
...
|---|---|---|---|---|
| HE6_PROOF | `HE6-BOX-1` (9) | `.13`–`.16`, `.57`–`.62` | **CLOSED** at n ≤ 15 and at every machine-certified configuration (`.16`(i)); at every degree modulo the arc grade (`.16`(ii)) | **Not landed.** No dated "closed by HE7" append is claimed on HE6 by this note |
| HE6_PROOF | `LEMMA HE6-1L` (4) | `.19`, `.20`, `.21` | HE7-L1 was ADOPTED BY HE6 (reverse direction: HE6 consumed HE7); HE7 now CITES | **Landed at the source, by HE6's own r1 round (R3)** — the note says “DISCHARGED at the source”, not “appended by us” |
| HE6_PROOF | `S1` (7) / (LIFT) | `.21` | FINDING HE7-F1: HE6 §S1's (LIFT) refuted as stated | **Not landed by HE7**; “The corrected hypothesis … *should be* appended to HE6 §S1's (LIFT)” — an offered erratum, executed at the source |
| HE6_PROOF | `LEMMA HE6-1L` (4) | `.19` | The exact-set display defect (coset) is HE7's display too | **Not landed by HE7**: “owned at the source by HE6's owed repair round (HE6-BOX-2 queue, PE2 report §S4); this append is the note-to-owner on the frozen copy” |
| HE6R1_RECON | `LEMMA HE6R1-3` (8) | `.09`, `.38`, `.49`, `.60`, `.62` | Consumed, not reproved; its irreducibility half is machine-evidenced only | **Not landed by HE7.** ANNEX R R2 offers the widening “gcd(f_S, Ψ) ≠ 1” as an “*Optional HE6R1-side erratum … executed there or not at Asvin's discretion*” |
| HE6R1_RECON | `LEMMA HE6R1-2` (8) | `.39`, `.51` | Consumed, not reproved; supersedes HE7-BOX-4 in read form | **Not landed by HE7**; the scalar erratum's “source-side cure is HE6R1's post-PE2 micro-annex, same wave” |
...
| GENIND_PROOF / W12_PROOF | `GENIND.B` (19), `W-12.D` (19) | `.16` | Named as the composition path to the uniformity theorem; HE7 supplies the σ-decision leg only | **Not landed by HE7** |
| HE3_PROOF | `HE3-BOX-6` (12) | `.37` | §S5.5's block-factor reduction is “the step whose absence was HE3-BOX-6's original defect” (§S9, shard 2) | **Not landed by HE7** |

**The last column is not an inference.** HE7 states the protocol three times in its own words (header §B): the finding “DISCHARGED at the source”; the erratum “executed there or not at Asvin's discretion”; the correction “owned at the source … this append is the note-to-owner on the frozen copy”. **Zero dated consumption appends are claimed landed on any supplier by this note, and none is invented here.**

---

## 4. NON-IMPORTS

`[TABLE — compiler ledger]` The note's explicit negative-import fences homed in lines 1–1246.

| Fence | Explicit material NOT imported or NOT proved | Unit |
|---|---|---|
| Base change | “**No base change.**” — re-asserted in THEOREM HE7.A, COROLLARY HE7.B, THEOREM HE7.A′ | `.04`, `.13`, `.16`, `.62` |
| Order-≥2 machinery | “**No Ore theorem, no Montes/GMN index theorem, no higher-order residual-polynomial operator, no theorem of the product** — the FGMN citation targets displayed in HE6 §S7.3 are *not* used; this note replaces them with the induction of §S6.” | `.04` |
| Irreducibility of test polynomials | “**No irreducibility of any test polynomial**, at any level”; DEFINITION HE7-1's key is “*never assumed irreducible*”; DEFINITION HE7-3's Ψ₂ “Never assumed irreducible”; §S3's Remark “**Neither requires Ψ irreducible.**” | `.04`, `.06`, `.18`, `.27` |
...
| GENHN.C citation | “We do not *cite* GENHN.C at level 2 — its statement is about a stage over a DVR, and Ψ is not known irreducible — we re-derive the same five cases from THEOREM HE7.A.” §S0: consumed “**as an argument template only**, and only at §S8” | `.03`, `.63` |
| HE3's stage route | “HE3 needed the *resolvent over a stage ring* (hence a DVR, hence irreducibility)”; “it is also the step that HE3's stage-CS route could not take, because there the inner object was the resolvent over R, whose roots are not roots of f” | `.18`, `.37` |
| Multiplicity-weighted class size | THEOREM HE7.C's proof: “the multiplicity-weighted exact value \|S_{λ,r}\| = D′ℓ·m·d_r is **not needed** — the upper bound suffices” (this is HE7-BOX-4, homed in shard 2) | `.15` |
| The gcd step | The §S1 CONVENTION's second equivalence is **NOT formal** at a reducible key — HE7 does not prove it; ANNEX-LEMMA R2-a (shard 3) supplies it | `.09` |
| Peel irreducibility | LEMMA HE6R1-3's “*irreducibility of Ψ over O of degree D″ with (e,f) = (e₁ℓ, f₁d_r)*, **which no textual argument of this note supplies**” | `.38`, `.60` |
| Level-i restatements | “**The level-i restatements of HE6R1-1/2/3 are not separately displayed here**; they are transfers of the same kind §S6 already performs” | `.51` |
| §S6 gives no bound on i | “Nothing above bounds i; §S7 does.” | `.52` |
| Discriminant-invariance route | Termination is “handled here by completeness rather than by the discriminant” — the naive `disc = A₁² − 4A₀` bound is NOT used | `.56` |
| Mixed-node continuation (post-hoc) | LEMMA HE7-13's conclusion is **NOT** claimed outside its pure-node hypothesis; ANNEX F-1 states it is “genuinely FALSE at mixed nodes”, and refutes the naive whole-f_S continuation outright | `.45` |
| DEFINITION HE7-2 not consumed at level ≤ 2 | “the level-2 core §S1–S5, §S7, §S8, THEOREM HE7.A/HE7.A′/HE7.C, COROLLARY HE7.B(i) and the whole machine record do NOT consume DEFINITION HE7-2 (§S1 defines n₂ directly)” | `.47`, `.17` |
| No invariance under CHANGING normalizer systems | R1.2 item 3: “no invariance under CHANGING systems is consumed anywhere (that stronger claim is false, which is HE7-T-BADTWIST's very tooth)” — what is consumed is COHERENCE of one fixed system | `.08`, `.48` |
...
## 6. BOUNDARY SELF-AUDIT (prose-genre pilot)

HE7 carries **zero `\tag{` displays**. Nothing in it announces "this span is one statement". What it has instead: bold `LEMMA/THEOREM/DEFINITION/COROLLARY/CONVENTION/FINDING` openers inside blockquotes (which mark *some* units), bold prose headers like `**Sharpness (the exact reachable set).**` and `**Remark (…).**` (which mark others), `[r1]`/`[r2]`/`[r3]` brackets (which mark corrections, sometimes mid-sentence), and long unmarked prose. So every boundary below was **chosen**.

### 6.0 The rule applied

> **A unit is the smallest span carrying one complete claim together with its proof obligations.**

Operationally, that resolved into a four-part test, applied in this order:

1. **Does the span have a proof obligation of its own?** A displayed lemma, a derivation, a case enumeration, or a machine verdict — yes. A pointer sentence, a heading, or a restatement — no (it folds into the unit it points at).
2. **Is the span cited BY DESIGNATION from elsewhere?** `(S5.2)`, `(RES₂)`, `(S5.8.1)`, `(†₂)`, `(‡₂)`, `HE7-13′`, `§S6(iii)` are all cited by name from other spans. A named, separately-cited object is addressable, therefore a unit — even when it is physically inside another unit's proof.
3. **Does the span have its OWN supersession history?** This is the strongest splitter in a note with four post-freeze annex layers. Two clauses of one blockquote that are riddered by different annexes must be separate units, or the CONDITIONALITY field lies about one of them.
4. **Does the span have its own conditionality or its own consumer set?** Two claims with different hypotheses, or with disjoint downstream consumers, split.

Proofs are NOT units: template v2 rule 1 makes DERIVATION a *field*, so §S3, §S4.2's (LIFT₂) proof, §S5.7's proof of HE7.A(1)(2), and §S7's proof of THEOREM HE7.C are all carried inside their statements' units.

### 6.1 Boundaries drawn, with the arguable ones recorded both ways

**1. Front matter → `.01` (title + Status) / `.02` (the `[r1]` round record).** Test 3. `.01`'s grade sentence is superseded five times over; `.02`'s substantive content (both findings applied in-text) is what makes resolution rule 2 sound and is superseded only in its grade clause. *Alternative:* one front-matter unit. *Rejected because* the r1 record is the load-bearing evidence that there is no separate round-block to apply — a reader of `.01` alone would not know that.

**2. §S0 → `.03` (positive imports) / `.04` (negative fence).** Test 4, and template rule 3: NON-IMPORTS is a section of its own, so the negative bullet must be independently addressable. *Not arguable* — the two bullets state opposite predicates.

**3. §S1 → five units (`.05` frame, `.06` DEFINITION HE7-1, `.07` substitution table, `.08` twist CONVENTION, `.09` Ψ ∤ f_S CONVENTION).** Test 1 and 3. `.07` is a display with its own exactness verification and is the engine of every §S5 "transfers verbatim" verdict; `.08` and `.09` are conventions with different histories (`.08` is extended by ANNEX-LEMMA R1-a; `.09` is *licensed* by ANNEX-LEMMA R2-a and was flagged a GAP). **Arguable: should `.07` be folded into `.06`?** *Candidate A:* fold — it is "attached objects" continued. *Candidate B (chosen):* separate — the table is cited as an object ("with the substitution table", §S2, §S6) and is what §S6's induction generalises; folding it would hide the note's own claim that the substitution is *exact*.

**4. The one-sentence lead-in `.10` ("The two lemmas HE6 §S7.2 named as its exact residue, stated here in full").** **Genuinely arguable.** *Candidate A:* not a unit — a connective. *Candidate B (chosen):* a unit — it is a complete claim (HE7's problem statement equals HE6's stated residue) with a proof obligation discharged by `.11`/`.12`, and it is the exact claim §S9 later qualifies with "one thing it under-counted". Under test 1 a claim that a later section corrects is not a connective. *Cost of the choice:* one very short unit.

**5. (SLOT₂)/(LIFT₂) statements (`.11`, `.12`) separated from §S3/§S4.2's proofs.** Forced by template rule 1 (DERIVATION is a field). *No alternative was available*, but note the consequence: §S3 and §S4.2 contribute NO units of their own except the two spans the test pulls out (`.18`, `.22`, `.23`).

**6. THEOREM HE7.A split into `.13` (clauses (1)(2) + the closing fence) and `.14` (clause (3)).** **The shard's most consequential boundary call, and the clearest instance of test 3.** *Candidate A:* one unit — it is one blockquote, one theorem, one name. *Candidate B (chosen):* two units. **Decided by three independent facts, any one of which suffices:** (i) ANNEX F-1 riders *clause (3)'s `[r2]` wiring sentence* by name and explicitly leaves (1)(2) alone; (ii) ANNEX R's BOX-1 append lists "THEOREM HE7.A" as untouched by all five findings while separately annotating "HE7.A(3)'s level-3 branch"; (iii) clause (3) carries its own non-propagation derivation, consumed by COROLLARY HE7.B(i) and re-derived in §S7. Under candidate A a single CONDITIONALITY field would have to say both "untouched by all five findings" and "carries the ratification annotation" — i.e. it would lie about one half. *Cost:* the blockquote is quoted in two pieces; both pieces are byte-verbatim and their union is the whole display minus nothing.

**7. §S3's Remark → `.18`.** Test 1 and 4: "Neither requires Ψ irreducible" is a NON-IMPORT with DAG consequences, and the HE3 contrast is the note's own account of why its method exists. *Alternative:* fold into `.11`'s DERIVATION. *Rejected* because the fence is re-asserted at `.62`/`.63` and belongs in §4's ledger.

**8. §S4.1 → three units (`.19` LEMMA HE7-L1, `.20` the fold box, `.21` FINDING HE7-F1).** Test 3, decisively: `.19` is corrected twice from shard 3 (coset, then res(·)-form); `.20`'s "verbatim" is re-scoped by a *third* dated line; `.21` is closed and untouched. *Candidate A:* one §S4.1 unit. *Rejected* — three different supersession histories in 50 lines.

**9. (LIFT₂)'s Sharpness (`.22`) and the bound recursion (`.23`) split out of `.12`.** Test 2 and 3: `.22`'s wording is riddered by ANNEX R R5 while `.12` is untouched; `.23` is cited by designation from §S6 and re-affirmed unchanged by ANNEX-THEOREM R1-c. *Alternative:* both inside `.12`'s DERIVATION. *Rejected* — R5's rider would then have no addressable target.

**10. §S4.3 → `.24` (the (‡₂) chain) / `.25` (the `[r1]` level-1 use-site check).** Test 4: two different thresholds, two different suppliers ((LIFT₂)'s own vs HE6-1L's), and `.25` is the unit the shard-3 coset correction cites as its non-propagation ground. *Alternative:* one §S4.3 unit. *Weakly arguable* — they share a purpose. Chosen split because only `.25` is load-bearing for the shard-3 correction.

**11. §S5's preamble → `.26`.** **Arguable.** *Candidate A:* a scope heading, not a unit. *Candidate B (chosen):* a unit — "**Exactly one item is in the third class**" is a falsifiable count, and §S9 falsifies it (twice at `[r1]`, once more at `[r2]`). A claim the note itself corrects is a claim.

**12. (S5.2) split out of LEMMA HE7-7's proof → `.31`.** Test 2: `(S5.2)` is cited by designation from `.44`'s proof ("by the case split of (S5.2) [r2]") and from the machine leg ("the (S5.2) [r2] middle band"). *Candidate A:* leave it inside `.30`'s DERIVATION. *Rejected* — a designation cited from two other units with a correction of its own must be addressable. *Cost:* `.30`'s DERIVATION is quoted with a bracketed hand-off `[offsets ε₀, ε₁ and (S5.2): see .31]`, the only place in this shard where a quoted proof is interrupted.

**13. The two §S5.3 Remarks kept separate (`.32`, `.33`).** Test 4: one is a definitional-correctness record with a machine witness; the other is an architectural claim consumed by §S6's induction. *Not seriously arguable.*

**14. §S5.6 kept as ONE unit (`.41` = LEMMA HE7-9 + LEMMA HE7-10).** **The clearest MERGE decision, and the one that most tests the rule.** *Candidate A:* two units — both are cited independently and often (HE7-9(a), HE7-9(b), HE7-10 each appear in several other units' DERIVATIONs). *Candidate B (chosen):* one unit. **Reason:** the rule says *smallest span carrying one complete claim TOGETHER WITH ITS PROOF OBLIGATIONS*. LEMMA HE7-10's proof obligations are not discharged anywhere outside LEMMA HE7-9's (†₂-count) setup — the note writes one proof for both and the HE7-10 half literally begins "For LEMMA HE7-10, take Ψ₂ := …" inside it. Splitting would either duplicate the setup (two quotations of one span) or leave `.41b`'s DERIVATION pointing at `.41a` for its own premises, which is exactly the failure mode the DERIVATION field was added to prevent. *Cost:* a caller who wants only HE7-10 must read the whole unit; mitigated by naming both lemmas in the unit's clause structure.

**15. (RES₂) split out → `.40`, breaking source order once.** Test 2: `(RES₂)` is cited by designation from `.48` (as `(RES_i)`) and `.63`, and HE7-12(c) item (8) lists it as a key-free input. It is physically the first sentence of §S5.6's proof, i.e. *after* `.41`'s statements. **This is the shard's one deliberate source-order break** and it is flagged at `.40` itself. *Alternative:* number it `.41` and §S5.6 `.40`, preserving order by first-line. *Rejected* because a reader scanning the inventory would then meet the lemma before the identity its proof opens with; the identity is logically prior.

**16. §S5.5 → three units (`.37` reduction, `.38` `[r1]`(a), `.39` `[r1]`(b)).** Test 3 and 4: (a) is disposed by LEMMA HE6R1-3 and level-generic at `.49`; (b) is disposed by LEMMA HE6R1-2, is **the target of two shard-3 dated corrections**, and is the shard's one signed proof-only unit. *Candidate A:* one "[r1] TWO ADDITIONS" unit, as the source's own bold header suggests. *Rejected* — the source header is a formatting convenience; the two additions share no supplier, no consumer and no correction history.

**17. LEMMA HE7-12 kept WHOLE (`.44`, clauses (a)–(d) + the 11-item audit).** *Candidate A:* split (a)/(b)/(c)/(d), or at least split the 11-item audit of (c). *Candidate B (chosen):* one unit. **Reason:** (a)–(d) share one proof and one hypothesis set, (c) is *literally* an audit of the other clauses' consequences, and (d)'s `W ≠ 0` argument depends on (a)–(c). The two annex riders that touch it name sub-clauses ((c) item (7); (d)'s parenthetical) and are carried as two separate CONDITIONALITY paragraphs inside the one unit — which the field can express without a split. *Cost:* the longest unit in the shard.

**18. LEMMA HE7-8 split FOUR ways (`.53` statement+proof, `.54` `[r2]` box, `.55` `[r3]` peel clause, `.56` characteristic-free note).** Test 3 and 2. ANNEX F-1 names the `[r2]` box explicitly ("LEMMA HE7-8's [r2] box … inherit the same reading"), so `.54` must be addressable; the charge for this queue names the `[r3]` mid-chain peel clause specifically, and it is a self-contained pigeonhole argument with its own machine status (unexercised); `.56` cites an external note (W-12) and makes a characteristic-uniformity claim the lemma statement does not. *Candidate A:* one LEMMA HE7-8 unit with three sub-paragraphs. *Rejected* on test 3 for `.54` and test 1 for `.55`. **`.56` is the weakest of the four** — a parenthetical remark — kept because it is the note's only statement about characteristic 2 and it feeds OPEN-CALL 5.

**19. §S6's `[r1]` paragraph split into (i)/(ii)/(iii) → `.49`/`.50`/`.51`.** Test 3: (iii) is ANNEX F-1's rider target 2 by name; (ii) carries the `[r2]` off-by-one repair; (i) carries neither. *Candidate A:* one "[r1] level-generic" unit matching the source's single bold header. *Rejected* — the rider would have no target.

**20. `.52` ("Nothing above bounds i; §S7 does") as a unit.** **The most arguable inclusion in the shard.** Two lines, no proof. *Candidate A:* a transition sentence; fold into `.51` or drop. *Candidate B (chosen):* a unit — it is a NEGATIVE claim about §S6's reach, i.e. a non-import in the DAG sense (THEOREM HE7.D alone does not bound the ladder), and template rule 3 makes negative fences load-bearing. *Honest cost:* if the cross-read judges two-line fences to be noise, this is the unit to cut; nothing else depends on it.

**21. THEOREM HE7.D emitted TWICE (`.17` = §S2's display, `.48` = §S6's display).** **Genuinely arguable, and decided by an external fact.** *Candidate A:* one unit — same theorem, same name; §S2 is a forward summary. *Candidate B (chosen):* two units. **Reason:** dated correction M-B (shard 3) riders *§S2's* clause "all hold, with the SAME proofs" by section number, and ANNEX R R1.1's preamble separately riders *§S6's* twin sentences "(SLOT). §S3 verbatim …" / "(LIFT). §S4.2 verbatim". Two displays, two riders, two different quoted targets. Under candidate A the unit's CONDITIONALITY would have to attach both riders to one text, and the merge run would not be able to tell which sentence each rider edits. **Format consequence: in prose notes a theorem stated twice at different granularities is TWO units when different riders name each display.** The relation is recorded at both units.

**22. §S8 split into seven units (`.57`–`.63`).** Test 1 for the frame, template rule 5 for the table, test 3 for the two riders (`[r2]` upgrades row 5, `[r1]` adds row 6 — different rounds, different subjects), test 1 for `.61`/`.62`/`.63`. *Arguable:* whether `.57` and `.58` should be one unit, since the frame paragraph ends with a colon introducing the table. *Chosen split* because template rule 5 makes tables units, and because `.57`'s "the block is everything" claim is cited independently.

**23. Proofs NOT emitted as units.** §S3 Steps 1–3 → `.11`'s DERIVATION; §S4.2's (LIFT₂) proof → `.12`'s; §S5.7's *Proof of THEOREM HE7.A(1)(2)* → `.13`'s; §S7's *Proof of THEOREM HE7.C [r1]* → `.15`'s. Two of these are physically remote from their statements (the HE7.A(1)(2) proof is 590 lines after the statement; the HE7.C proof is 880 lines after). **Format observation: in a prose note the DERIVATION field routinely reaches across hundreds of lines, and the RESOLUTION TRACE must therefore carry TWO line ranges per unit (statement span, proof span). The v2 template's trace field assumes one.**

### 6.2 Merges deliberately NOT made

- The (S5.3) estimate stays inside `.30`'s DERIVATION rather than becoming a unit: it is cited by number from `.44`'s proof, but only as "LEMMA HE7-7 Step 1's estimate (S5.3)", i.e. through its parent. Test 2 requires an *independent* designation; this one is always parented.
- (S6.1)/(S6.2)/(S6.3) stay inside `.48`'s DERIVATION: ANNEX R R1.2 item 1 cites them as a block ("(S6.1)–(S6.3)"), never individually.
- (S5.8.1) and (S5.8.2) stay inside `.44` and `.45`: each is displayed inside the statement of its own lemma and is cited as "the off-disk inequality (S5.8.1)" / "the strict slope increase (S5.8.2)" — parented in every occurrence.
- (†₂)/(‡₂) stay inside `.12`: they are the lemma's own hypothesis labels.
- The 11 items of LEMMA HE7-12(c) stay inside `.44`: item (7) alone is riddered, and a CONDITIONALITY paragraph names it precisely.

### 6.3 Which v2 rules fought the prose genre

Recorded as **format evidence for GRTJB, GRTJC and GENIND** (the remaining tag-sparse prose notes), one line each plus the fix.

1. **CANONICAL STATEMENT "verbatim-or-[ASSEMBLED]" assumes the statement is a display.** In HE7 roughly a third of the units are *bold-prose-headed paragraphs* (`**Sharpness (…).**`, `**Remark (…).**`, `**[r1] TWO ADDITIONS …**`) with no display at all; quoting them verbatim quotes ordinary prose, which is correct but makes "the statement" and "the exposition" typographically indistinguishable in the spec. **Fix: add a `FORM:` sub-field — `display | bold-headed paragraph | bracketed rider | inline sentence` — so a downstream chapter cut can tell a theorem from a remark without re-reading the note.**
2. **The single-span RESOLUTION TRACE fights statement/proof separation.** See §6.1 item 23: statements and proofs are up to 880 lines apart. **Fix: trace takes `statement lines` + `proof lines` + `correction sites`, three slots, not one range.**
3. **The type enum still has no slot for a note's own FINDING-against-a-supplier.** `FINDING HE7-F1` (`.21`) is not a lemma, not an instance record, not a regression record, and not a scope record: it is a *defect report against another note, discharged at the source*. Filed as `[changes-record]` faute de mieux. **Fix: add `supplier-finding` to the enum** — the tagged genre had none of these; prose proof notes have many (HE7 alone carries F1 plus four inbound supplier corrections).
4. **"Tables are units" (rule 5) under-specifies ASCII displays.** §S8's dictionary is a fixed-width ASCII block inside an indented code fence, not a Markdown table. Transcribing it as a Markdown table loses the source bytes; quoting it verbatim violates rule 5. **`.58` does BOTH** (verbatim display, then `[TABLE]` transcription). **Fix: make the double-emission the rule for ASCII tables, explicitly.**
5. **Rule 2 (XREF grep-verification) has no vocabulary for a designation that exists only with different case or punctuation.** Three of HE7's citations fail fixed-string grep purely on capitalisation or a period inside bold (`twist (letters only)`, `finding 2`, `HMENU3-ANNEX-A`). Emitting them as OPEN-CALLs would be over-reporting; suppressing them would lose real DAG edges. **Fix: a `NEAR-MISS` disposition — "designation absent as written; verified referent at `<file>:<line>` under `<actual string>`, count N" — distinct from both a clean XREF and an OPEN-CALL.**
6. **Rule 6 (teeth inverse table) assumes every guarded unit has a tooth.** In a prose proof note the *proof-only* units are the interesting ones. **Fix: require an explicit `PROOF-ONLY` row-class in the inverse table, quoting the note's own coverage-hole sentence** (this shard has exactly one, `.39`, and the note names it).
7. **Rule 7 (scope split) assumed the note has an absorption set.** HE7 absorbs nothing; it *discharges a box* and *receives* four corrections from suppliers. The A/B split still works, but predicate B ("what has been landed on other notes") is answered "nothing, deliberately, three times over". **Fix: allow predicate B to be answered by an explicit NO-APPENDS declaration with its quotes, and add predicate C: inbound corrections received.** Four of this shard's six cross-shard supersessions are inbound supplier corrections, and the v2 header has no place for them.
8. **The rider-vs-supersession distinction is missing and the prose genre is full of riders.** ANNEX F-1's "Read as" (a scope pin: the display stays true, its *wiring* is re-routed), ANNEX R R3/R5's "Rider: read X as Y" (a wording substitution), and M-B's "Read … as …" (a proof-provenance substitution) are three different operations, all phrased identically. **Fix: `SUPERSESSION KIND: replacement | wording-rider | scope-pin | provenance-rider | as-of scoping`** — this shard needed all five.

---

## 7. SHARD NOTES

- **Cross-shard supersessions applied (six, all inbound to this range).**
  (1) ANNEX F-1 → `.14` (HE7.A(3)'s `[r2]` wiring sentence: pure-node verbatim, mixed-node routed through ANNEX-LEMMA HE7-13′) and → `.51` (§S6(iii): "per distinct factor, REPEATED OR NOT — mixed nodes included"), with `.54` and `.44`(d) inheriting the same reading; plus the scope pin on `.45` (HE7-13's conclusion FALSE at mixed nodes).
  (2) ANNEX R R1 → `.47` (ANNEX-DEF HE7-2′ replaces DEFINITION HE7-2's normalizer line and base sentence) and → `.17`/`.48` (THEOREM HE7.D re-proved via R1-b/R1-c/R1.2/R1-d, at annex grade).
  (3) ANNEX R R3 → `.15` (both n = 16 sentences read as the necessary direction only).
  (4) ANNEX R R4 → `.44` (HE7-12(c) item (7)'s inventory completed; conclusion unaffected).
  (5) ANNEX R R5 → `.22` ("proper subgroup-coset" → "punctured proper F_Q-linear subspace").
  (6) The dated corrections → `.19` (the η_θ^{−q(k)} coset, in res(·) form), `.20` (the fold-box's "verbatim" re-scoped as-of the r1 landing), `.39` (the §S5.5 scalar, with the exponent's argument corrected from the side's line value to the PIN height), `.16`/`.17` (M-B's proof-provenance rider).
  ANNEX-LEMMA R2-a is a *license*, not a supersession: it supplies the underived gcd step of `.09`'s equivalence chain, which `.35` and `.41` consume.
- **Range-edge audit.** Line 1243 is §S8's closing sentence ("The agreement of the two routes is an internal decorrelation, exactly as HE6 §S6.4 was for μ ≤ 3."); line 1244 is blank; line 1245 is `---`; line 1246 is blank; line 1247 opens `## S9.`. **No source statement unit is cut at the edge.** The shard boundary falls exactly on a section break.
- **Outside-HOME material used only as resolver, never inventoried here:** §S9's honest-status list and the five boxes, §S10's battery description, §S11's verdict tables and seven disclosures, ANNEX F-1 in full (including ANNEX-LEMMA HE7-13′), ANNEX R in full (ANNEX-DEF HE7-2′, R1-a, R1-b, R1-c, R1-d, R2-a, R3–R5), and the six dated corrections/lines. Shards 2 and 3 own them.
- **Protocol versus supply.** §S1–S8's supply set is inventoried; **no consumption append is claimed landed on any supplier note**, and none is invented. Four inbound supplier corrections are recorded at their targets.
- **Numbering.** Temporary IDs `EFF.HE7.s1of3.01`–`.63`, to be reconciled with shards 2 and 3 at the merge run. **Merge hazard to flag:** ANNEX-LEMMA HE7-13′ (shard 2) and ANNEX-DEF HE7-2′/R1-a/R1-b/R1-c/R1-d/R2-a (shard 3) are the *effective forms* of statements homed here; the merge must wire `.45 → HE7-13′`, `.47 → HE7-2′`, `.11 → R1-b`, `.12 → R1-c`, `.17`/`.48 → R1-d`, `.09 → R2-a` as explicit edges, or a chapter cut will read the frozen bodies as current.

---

## 8. Self-audit and OPEN-CALLS

### OPEN-CALL 1 — the §S1 CONVENTION's grade is flagged for Asvin, and the note says so

ANNEX R R2's closing paragraph (lines 2389–2395) reads: *“As stated the finding is a GAP (an underived equivalence at a standing convention consumed at level 2 everywhere); by arc precedent (PE2 F-1's W ≠ 0, PE3 F-1 — verified-true, in-toolkit, display-scale) this exact shape was twice graded MINOR. The repair above is identical under either grade; the grade decides only whether the ratification CHALLENGE cites one heavy finding or two.”* The BOX-1 append adds: *“Asvin adjudicates: the CHALLENGE's final disposition and F2's grade.”* **The compiler cannot decide this**, and the choice changes `.09`'s CONDITIONALITY wording (GAP vs MINOR) and the ratification summary the merged spec carries. **Decision needed: Asvin.** No silent choice was made — `.09` records the defect and the license, and quotes the grade flag.

### OPEN-CALL 2 — two dated riders point opposite ways on the same phrase

`.22`'s phrase “a proper subgroup-coset of K₂” is (a) declared a **mislabel** by ANNEX R R5, to be read as “the nonzero elements of a proper F_Q-linear subspace”, and (b) declared **coset-correct as written** by the HE6 PE2 F-1 dated append's non-propagation item (ii). Both are true of different objects: R5 speaks of the *shape inside the braces* (a punctured subspace, not a group coset), the HE6-side append of the *external* `η_θ^{−q(k)}` multiplier (a genuine coset factor). **The fully corrected object is `η_θ^{−q(k)} · (punctured proper F_Q-linear subspace)` — a coset of a punctured subspace — but no line of the note states that composite.** The compiler has emitted both riders verbatim at `.22` and refused to synthesise the composite. **Decision needed: whether the merged spec carries the composite reading (and, if so, whether that is a note-level erratum owed to HE7 or a reading note).**

### OPEN-CALL 3 — LEMMA HE7-12(d) and LEMMA HE7-13 are mutually dependent, and the note resolves it by fiat

`.44`(d)'s `[r3]` argument for `W ≠ 0` cites LEMMA HE7-13's strict slope increase; `.45`'s proof cites LEMMA HE7-12(c)(d) for the package at `Ψ̃`. The note breaks the circle in one parenthesis: *“the two lemmas induct together along the chain, the base W = w₁ being a single nonzero (LIFT₂) polynomial”*. That is a correct *sketch* of a simultaneous induction on chain length, but **the induction is not displayed** — no statement of the joint hypothesis, no base/step separation. PE2 graded the underlying issue MINOR and it was repaired by exactly this parenthesis. **Not a claimed error**; recorded because a formalization blueprint cannot build a mutual recursion from a parenthesis. **Decision needed at 0b/chapter time: whether the joint induction must be displayed before formalization.**

### OPEN-CALL 4 — R4's completed inventory may or may not apply to §S6(iii)

ANNEX R R4 completes the dependency inventory of **LEMMA HE7-12(c) item (7)**'s sentence about LEMMA HE6R1-2. **§S6(iii) (`.51`) makes the same claim about the same lemma in different words** (“whose proof likewise consumes only items in the per-level package ((LIFT_i), the two evaluation displays, the slope counts, nonemptiness, and the level-i analogue of LEMMA HE6-4)”) and R4 does not name it. The two inventories differ: §S6(iii) already lists LEMMA HE6-4 (which R4 adds at item (7)) but omits (LIFT₂)/DEFINITION HE7-3/HE7-9(a)/THEOREM HE7.A(1) (which R4 adds). **Decision needed: whether the merged spec reads R4's completion onto §S6(iii) as well.** Carried at `.51` as an explicit flag; no silent completion was made.

### OPEN-CALL 5 — characteristic 2 is claimed covered and exercised nowhere

`.56` claims the termination argument “does not use 2 ∈ O^×, so it covers the equal-characteristic-2 Artin–Schreier corner”. The claim is textually sound (the proof routes through completeness). But **`p = 2` appears in no row of any HE7 leg**: the sealed battery runs `q ∈ {3,5,7}` in char 0 and `q ∈ {3,5}` in char p; `he6r1_checks.py` runs `q ∈ {3,5,7}`/`{3,5}`; `he7annex_supp.py` runs `p ∈ {5,7}`; `he7rannex_supp.py` runs `p ∈ {5,7}`. HE7-BOX-2's coverage list (shard 2) does **not** name characteristic 2 among the unexercised items. **Decision needed: whether "characteristic 2 unexercised anywhere in the HE7 stack" is a coverage-hole row the merged spec must add to HE7-BOX-2.**

### Recorded source defects (flagged, not silently repaired)

1. **`.03`:** the GENHN paragraph is cited as “its "twist (letters only)" paragraph”; fixed-string `twist (letters only)` has count **0** in `GENHN_PROOF_2026-08-08.md` because the target's heading (line 729) is `**The twist (letters only).**` — period inside the bold. Referent unique and verified; carried descriptively.
2. **`.15`, `.18`, `.37`:** “HE3's stage-α recentering” and “HE3's stage-CS route” are HE7-local method names, not HE3 designations. Parent designations `HE3-3` (15) and `HE3-BOX-6` (12) verified; the method names carried descriptively.
3. **`.26`:** “**Exactly one item is in the third class (§S5.3)**” is falsified by the note's own §S9 under-count list (three items at base, two more at `[r1]`) and by §S5.8's `[r2]` addition. True of §S5.1–S5.7 as originally scoped; false of the section's final content. The note corrects itself; the display is not edited.
4. **`.09`:** the note writes “Codex PE1 finding 2”; the target report (`CODEX_HE6PE1_2026-08-08.md`) numbers it `F2` in its repair queue and writes `Finding 2` with a capital. Fixed-string `finding 2` count **0**. XREF emitted at `F2` (count 4).
5. **`.56`:** “W-12's r4 repair” — `r4` has count **59** in `W12_PROOF_2026-08-08.md`, i.e. the designation is a bare round tag matching every `[r4]` bracket, not an addressable statement. The specific repair is carried descriptively; a precise designation would have to come from W-12's own spec.
6. **`.59` / `.60` / `.62`:** **the phrase “the fifth row” denotes two different rows.** `[r2]` (line 1190) means the α-refine row, physically fifth in `.58`'s display; `[r1]` (line 1199) means the PEEL row, physically sixth. THEOREM HE7.A′ compounds it by saying “the **five**-row dictionary above” of a six-row display. All three referents are unambiguous from their own content; no statement moves. Recorded because a merge or a chapter cut that greps “fifth row” will collide.
7. **Refine-count slip (shard-2 homed, quoted here):** §S11's Q1 sentence originally said the α-refine fired on **41** members; the `[r2]` disclosure recounts the frozen artifact's refine histograms to **42** and classifies the original as “a display slip of the note (the artifact was always 42)”. `.46` and `.59` in this shard quote **42**, the corrected figure. Recorded because the number appears in three places and only one of them carries the correction.

### Fidelity audit

- Every CANONICAL STATEMENT is a verbatim source quotation or is explicitly marked `[ASSEMBLED]`. **No inverse, sign, quantifier, index, level, inequality direction, or implication direction was altered anywhere.** The one place a quoted proof is interrupted is `.30`, marked inline with a bracketed hand-off to `.31`.
- Truncations inside quotations: **none used in this shard.** Where a source sentence continues past what a unit needs, the whole sentence is quoted.
- Terminal punctuation inside quoted displays is preserved as written (the EFF-T3 cross-read's minor 9); the note's own typographic quotes and ASCII displays are reproduced with line structure intact. Markdown blockquote framing (`> `) is the only presentational addition.
- The one source table (§S8's dictionary) is emitted **twice** — verbatim as the source's ASCII display, then as a `[TABLE]` Markdown transcription — and is flagged. No source table is prosified.
- **55 external XREF designations, every one grep-verified with a fixed-string count ≥ 1 recorded inline and in §3.1.** Three candidate designations with count 0 are explicitly NOT emitted as XREFs and are recorded as source defects 1, 2 and 4 (with verified substitute designations where they exist). One §-anchor XREF (`HE6:S1`) carries a caveat that its count is an upper bound.
- Status and mathematical content are kept as distinct predicates: the acceptance chronology governs grade (`.01`), the note's own no-append protocol governs consumption (header §B, §3.2), and no append is claimed landed.
- Out-of-range material is used only to resolve in-range HOME units (resolution rule 8); shards 2 and 3 own §S9–S11 and both annexes.
- **Honest limitation of this shard:** four of the six inbound corrections and both annexes are physically in other shards. Every affected unit quotes the governing corrective text verbatim, so shard 1 is self-contained as a *reading*, but the merge run must still install the edges listed in §7.

EFF-HE7-s1of3 COMPILED: 63 statements / 55 xrefs verified / 5 open calls
