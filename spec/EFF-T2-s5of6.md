# EFF-T2-s5of6 — CANONICAL EFFECTIVE-SPECIFICATION SHARD
...
**HOME range:** source lines 4019–4442 inclusive (the r10 counter; r11 in full; post-pass rider block 2; r12 in full). The destination section of an edit, not the physical location of the edit directive, determines HOME; directives aimed at base S0–S8 resolve `EFF.T2.s1of6.<nn>` units and are routed, not re-emitted (§7).

**HEAD blob hash:** `9cc9063028c598a90cd98a022ac07694e6dc8a2a`

**Compilation context:** `spec/EFF_TEMPLATE_v2.md`; temporary IDs `EFF.T2.s5of6.<nn>`. Conventions follow `spec/EFF-T2-s1of6.md`; the HOME routing audit follows `spec/EFF-T1-s2of3.md`.

**This shard carries the last setting of several effective objects.** r12 is the final repair round; the accepted text of record is “the effective text as of r12, plus the first post-pass rider block, plus the second post-pass rider block, plus post-pass rider 3”. Consequently the **final pin inventory (37, taxonomy \(20+4+13\))**, the **final cured-consumption census (ten)** and the **final five Source-pin lines** are all set inside this range, and nothing after it amends them.
...
The acceptance record confirms this range's two pin cures and its final inventory:

> “LEMMA HE6-2's β-definedness guard (r11 — `HE6-BETA-GUARD`); and HETOW's post-PE2 correction C-A with S1.8B's imported degree conjunct (r12 — `HETOW-KEY-BRIDGE-C-A`, and the conjunct re-derived master-side).”

> “That sweep was run whole at **37** pins by pass 2 … and re-resolved to the same \(20+4+13=37\) inventory by pass 1, which additionally certified the new span `HETOW-KEY-BRIDGE-C-A` [548, 569] minimal under S0.2.”

Every counter state set in this range (“RESET to 0 of 2” twice; “STANDS AT 1 OF 2” once) is superseded by “**The clean-pass counter reaches 2 OF 2**”.

### Scope split

**ABSORPTION/supply SET.** Membership unchanged; the hypothesis suite is unchanged (r11 and r12 each book “No theorem-statement change”), with **one recorded addition to carrier data**:

> “One item of CARRIER DATA is made explicit rather than left to a phrase: repair 4(b) writes S1.1's "integer-normalized value \(d\)" out as \(d=c_{\mathcal C}v\) for a positive integer \(c_{\mathcal C}\), with the valuation properties that follow. This is an addition to what the carrier displays and is recorded as one; it is discharged as written in all five instance boxes … so no instance record is amended”

and **one imported conjunct replaced by a master-side derivation** (the \(\deg w<D''\) bound in S1.8B's scope clause).

**APPEND/consumption PROTOCOL scope.** The gated relation is restated in this range, and it is consumption:

> “Consumption, not citation, is the gated relation: S0.2's cure clause … and S7's standing check … are both worded on it, and r11 repair 1(f)'s asymmetry test is how a directive is classified.”

Two sweeps of record run here — HE6's dated appends re-examined (r11) and the **first exhaustive dated-append sweep over the five non-HE6 sources** (r12) — both source-side. Downstream consumption checks remain queued by the acceptance record.

### Resolution rules applied

1. The acceptance record governs status; counter states in this range are historical. Pin counts and censuses reach their **final** values here (37; \(20+4+13\); ten).
2. A directive homed in base S0–S8 is routed (§7), with the two exceptions declared in shard 3: (i) reading directives, adjudications, declinations and verification records whose destination is a `(PINS)` row or a pin name; (ii) a multi-leg derivation with quoted source legs whose shard-1 destination unit compresses it to a single clause. Exception (ii) is used at `.09` only. **No statement, display, clause or instance box of shard 1 is re-emitted.**
3. Intra-shard supersession is applied and only the replacement is stated as effective: Rider 2.1 supersedes r11's “Those are the only two sites.”; r12 repair 1(e)–(f) supersedes r11's counts and two Source-pin lines; r12 repair 4(d) supersedes one clause of r9 repair 1(b)'s lift-freedom rider (which is homed in shard 4).
4. Cross-shard supersession is applied in both directions. **Downward:** none — no later text amends anything homed here except the acceptance record's status. **Upward:** r12 repair 4(d) amends a clause of the derivation compiled at shard 4 `.05`; r12 repair 2 amends base S3.1 (shard 1 `.33`); r12 repair 3 amends base S5 item 3 (shard 1 `.40`); r12 repair 4(a)–(b) amend base S1.4 and S1.1 (shard 1 `.02`, `.04`, `.09`). Shard 4 `.05` was amended at compilation of this shard to carry r12 repair 4(d)'s completed step.
5. The **asymmetry test** — “does obeying it change something this master reads?” — is the effective classifier for a dated source append, and is stated here for the first time (r11 repair 1(f)) and applied corpus-wide (r12 repair 1(g)).
6. Every `grep -cFx` count recorded in this range was **re-run at compilation** (§3): 22 source lines across four files, plus 18 master-internal landmark checks, all at the recorded values and line numbers, including the two multi-occurrence census strings (3 at bound 4031, 4 at bound 4202).
7. Post-pass rider 3 and the ACCEPTANCE RECORD (4443–4507) lie outside this range and are compiled in shard 6.

### Quotation and table discipline

Source statements are verbatim excerpts or marked `[ASSEMBLED]`; abridgement inside a quoted block is marked with an ellipsis. Source lines are reproduced byte-exactly. Compiler ledgers are separately marked. The composed final Source-pin lines at `.15` are marked `[ASSEMBLED]` and their three constituent rounds are named.

---

## 2. Statement inventory

### EFF.T2.s5of6.01  [changes-record]

**CANONICAL STATEMENT:** `[ASSEMBLED — the r10 counter; the r10 header and repairs are homed at EFF.T2.s4of6.10–.15]`
...
### EFF.T2.s5of6.02  [changes-record]
...
**CONDITIONALITY:** Historical as to counter and pin count (36 → 37). Its own census sentence (“Those are the only two sites.”) is superseded inside this shard by Rider 2.1 (`.05`).
...
**CONDITIONALITY:** The pin is a reading directive, not an independent assertion: exactly one thing is consumed from it. The source's own downstream site check is explicitly not consumed — a classification Rider 2.1 later tests and upholds (`.05`). The pin count it sets (36) is superseded to 37 inside this shard.
...
### EFF.T2.s5of6.06  [acceptance-box]
...
### EFF.T2.s5of6.07  [changes-record]
...
### EFF.T2.s5of6.09  [lemma]

**CANONICAL STATEMENT:** `[ASSEMBLED — the master-side degree bound replacing the imported conjunct in S1.8B's composed-key scope clause (r12 repair 1(b)–(c)); destination base S1.8B, shard 1 EFF.T2.s1of6.24]`
...
### EFF.T2.s5of6.11  [changes-record]
...
### EFF.T2.s5of6.13  [changes-record]
...
**(d) The elided \(h(E)<\lambda\) step**, completing the lift-freedom rider compiled at shard 4 `.05`:
...
**RESOLUTION TRACE:** r12 repair 4(a)–(d) (4387–4423); four anchors landmark-verified at 1 under bound 4202 (lines 384, 157, 2522, 3739). Destinations: base S1.4 → shard 1 `.09`; base S1.1 → shard 1 `.04` (which quotes the \(d=c_{\mathcal C}v\) paragraph verbatim); r4 repair 9's parenthetical (shard 2 `.05` context); r9 repair 1(b) → shard 4 `.05`.
...
**Method.** Every source line, source line **number** and master-internal landmark below was **re-searched at compilation time (2026-08-14)** — exact-line comparison in the named accepted source file (part A) and fixed-string containment under the round's own bound (part B). No count and no line number differed from the note's record, including the two multi-occurrence census strings.
...
|---|---|---|---:|---|
| V01 | `HE6-BETA-GUARD` first delimiter | HE6 | 1036 | `**Correction 2 (PE4 F-2 — LEMMA HE6-2's β-definedness guard).** The` |
| V02 | `HE6-BETA-GUARD` last delimiter | HE6 | 1047 | `lemma's conclusion and its consumers are unaffected by this re-scoping.` |
| V03 | the operative directive inside the span | HE6 | 1041 | `the guard as **"when δ = κ"** — the defining case, where the element is a` |
| V04 | preceding sibling (boundary witness, outside the span) | HE6 | 1022 | `**Correction 1 (PE4 F-1 — the §S9 headline "1,385 members over six` |
| V05 | next unit of equal rank (boundary witness, outside) | HE6 | 1051 | `### Dated corrections (2026-08-12 — sol certification A3: the R6 chain challenge adjudicated at HEAD)` |
| V06 | the source's own site-check line (classified, not consumed) | HE6 | 1044 | `passPE4's site check, HE6-3/HE6-4/THEOREM HE6.A read ε through that` |
| V07 | transitive route (a), inside `HE6-ACCOUNT` | HE6 | 545 | `* ρ-side: by LEMMA HE6-2 (with r₀ never a label's factor)` |
| V08 | transitive route (b), inside `HE6-ACCOUNT` | HE6 | 589 | `* ρ-side: by LEMMA HE6-2, Σ_ρ dv(Ψ(ρ)) = ℓd_r·Σ_λ′ n_λ′ min(λ′,λ) +` |
| V09 | the enclosing section heading (boundary context, outside the span) | HETOW | 539 | `### Dated corrections (2026-08-09, post-PE2)` |
| V10 | `HETOW-KEY-BRIDGE-C-A` first delimiter | HETOW | 548 | `**C-A (cures PE2 F-A — LEMMA HETOW-2's F-2 paragraph, §S2).** Two` |
| V11 | the re-scoping directive | HETOW | 558 | `closing "Either way LEMMA HE7-12 [r2, r3] applies" is RE-SCOPED to` |
| V12–V17 | the corner diagnosis and the preserved accounting core | HETOW | 560–565 | `the F-4 branch, **w = 0**: at the (e₂ = 1, t = f₂−1) corner a` · `junk-augmented coefficient's moved piece x^{r′}π^{·}Φ′ lands at slot` · `e₂t + 1 = e₂f₂, the LEADING slot, so w would acquire an` · `x-degree-≥ D″ term and HE7-12's hypothesis "deg w < D″" (HE7_PROOF` · `§S5.8) fails there. The accounting core stands: value monotone, raise` · `exactly u₂ − e₂D′h, residue unchanged via the Ô[x] identity —` |
| V18–V21 | the correction's own consumer trace (ending at the span's last delimiter) | HETOW | 566–569 | `machine-exact at frame R with r′ = e₁−2 maximal. PE2's consumer` · `trace found the two in-repo consumers (HETOW.A(i); the GENHN dated` · `erratum's display) both pinned to the (LIFT) form — neither reaches` · `the withdrawn exemplar or the corner.` |
| V22 | next sibling unit (boundary witness, outside) | HETOW | 571 | `**C-B (cures PE2 F-B — the §S2 preamble's mechanism sentence; the` |
| V23 | next unit of higher rank (boundary witness, outside) | HETOW | 608 | `### Dated acceptance record + correction (2026-08-09, post-PE3)` |
| V24 | the governed licence sentence, inside `HETOW-KEY-BRIDGE` | HETOW | 261 | `> T₂. Either way LEMMA HE7-12 [r2, r3] applies (its W = 0 split` |
| V25 | the sweep's HETOW:837 row | HETOW | 837 | `### Dated absorption append (2026-08-12 — S-2/LEMMA COCYCLE-TELESCOPE)` |
| V26 | LEMMA HE7-12's hypothesis line | HE7 | 814 | `> with key Ψ (DEFINITION HE7-1), and let w ∈ O[x], w ≠ 0, with deg w < D″` |
| V27–V28 | LEMMA HE7-12(d), the fold | HE7 | 842–843 | `> (d) *(chains fold into one application)* if w₁, …, w_j each have degree` · `> < D″ and slot value > T₂ then W := w₁ + ⋯ + w_j has deg W < D″ and` |
| V29–V30 | `GENTOW-4-1`(c), the value disjunct only | GENTOW4 | 184–185 | `> holds verbatim: Phi2 = Psi_{kappa2,rtilde} − w with w = 0 or` · `> dv2(w) > T2, so by LEMMA HE7-12 the §S5 package, THEOREM` |

All 30 lines returned exactly **1**, each at the line number the note records.

### B. Master-internal landmarks (fixed-string, under the round's own bound)

| Landmark | Bound | Line(s) | Count |
|---|---|---|---:|
| `**The 2026-08-09 post-PE4 correction governs.**` | 4031 | 3892 | 1 |
| item-2 tail: (266). → `HE6-TEST-PACKAGE`. | 4031 | 3870 | 1 |
| `The 35 pins are the following existing-text pairs` | 4031 | 3929 | 1 |
| `Eight consumptions have been cured this way.` | 4031 | 3955 | 1 |
| `so the count now reads two of eight.` | 4031 | 3965 | 1 |
| S3.1 search string (`HE6-ROOT-LABEL`, and `HE6-ACCOUNT`.) | 4031 | 3981 | 1 |
| S3.2 search string (`HE6-ACCOUNT`, and `HE3-A-PROOF`.) | 4031 | 3983 | 1 |
| level-two tail string (`HE6-TEST-PACKAGE`, `HE6-ROOT-LABEL` and `HE6-ACCOUNT`.) | 4031 | 3985, 3987, 3989 | **3** |
| `Those are the only two sites.` | 4162 | 4088 | 1 |
| `The unit's remaining content, the source's own site check over HE6-3, HE6-4 and THEOREM HE6.A` | 4162 | 4070 | 1 |
| `The 36 pins are the following existing-text pairs` | 4202 | 4090 | 1 |
| `Nine consumptions have been cured this way.` | 4202 | 4122 | 1 |
| `so the count now reads two of nine.` | 4202 | 4138 | 1 |
| same level-two tail string, at r12's bound | 4202 | 3985, 3987, 3989, 4140 | **4** |
| `The source proves more than the master requests by having an irreducible local key.` | 4202 | 832 | 1 |
| `3. Every cited source unit has a named paired begin/end pin.` | 4202 | 1194 | 1 |
| `There is no all-height lift hypothesis.` | 4202 | 384 | 1 |
| `Every \(\xi\in\operatorname{Pt}\) has a \(k_0\)-embedding` | 4202 | 157 | 1 |

The two multi-occurrence results reproduce the note's own arithmetic exactly: r11 records **3** at its bound 4031 and amends all three identically; r12 records **4** at bound 4202, “the fourth occurrence being line 4140, r11 repair 1(e)'s own quotation of it, which r11's own bound 4031 excluded”.

**XREF result:** 30 source lines at count 1 with line numbers confirmed; 18 master-internal landmark checks at their recorded values, including both multi-occurrence censuses. **0 unverifiable designations; 0 compiler `OPEN-CALL`s.**
...
|---|---|---|---|
| r10/r11/r12 round records (.01, .02, .07) | Historical as to counter | Obligation set final at two | Source-side gates only |
| `HE6-BETA-GUARD` (.03) | Effective, final | Reading directive for `HE6-TEST-PACKAGE`; one thing consumed | On all five Source-pin lines |
| β-guard consumption analysis (.04) | Effective | No fence added; conclusion byte-unchanged | n/a |
| Consuming-site census (.05) | Effective in the widened form | Two direct sites + one transitive route | Coverage unaffected |
| Rider block 2 (.06) | Historical as to counter (1 of 2) | Fold rule effective | n/a |
| `HETOW-KEY-BRIDGE-C-A` (.08) | Effective, final; certified minimal at acceptance | Reading directive for `HETOW-KEY-BRIDGE`; \(w\ne0\) branch takes nothing from the governed sentence | On S3.4 and S3.5 only |
| Master-side degree bound (.09) | Effective | Presupposes the instance boxes' monicity | Removes an import; adds no source consumption |
| Five-source dated-append sweep (.10) | Effective, declared incomplete | One row's ground re-grounded at rider 3 | Gate re-runs it |
| S3.1 export correction (.11) | Effective | Original-key only; `(MP1)` fences the recentered case | n/a |
| S5 item 3 restated (.12) | Effective | Four admissible citation classes, each instanced | This is the gate's own wording |
| Nit cures (.13) | Effective | (b) is the round's only carrier-data addition | n/a |
| Inventory bookkeeping (.14) | **Final**: 37, \(20+4+13\), census ten | Confirmed at acceptance | Pin sweep run whole at acceptance |
| Final Source-pin lines (.15) | **Final** | Per-record; recentered-chain pins unconsumed over \(\Phi'\) | Downstream re-checks queued |

**Reverse-XREF audit.** No unit homed here names a downstream consumer note/designation pair. HETOW:837's absorption append names an S-2 consumer of HETOW, not of T2, and is recorded as not consumed rather than promoted to a reverse-XREF row.
...
|---|---|
| Uncorrected β-guard | Do not read LEMMA HE6-2's dichotomy under its as-written guard (“denominator dividing \(\ell\)”); at \(\delta\ne\kappa\) with \(\ell\delta\in\mathbb Z\) the \(\beta\) it names is undefined. Read it under `HE6-BETA-GUARD`. |
| The source's own site check | Do not consume HE6's post-PE4 verdict on its downstream sites; only the corrected guard is consumed. |
| Naming as the gated relation | Do not enumerate consuming sites by where the master *names* a directive; consumption includes transitive routes through pinned proof text. |
| HE7-12 licence at \(w\ne0\) | Do not use LEMMA HETOW-2's closing “Either way LEMMA HE7-12 applies” in the \(w\ne0\) branch; C-A re-scopes it to \(w=0\). |
| `GENTOW-4-1`(c) as a degree bound | Do not read clause (c) as supplying \(\deg w<D''\); it states the value disjunct only. |
| Junk-augmented presentations | Do not treat an object carrying an \(x\)-degree-\(\ge D''\) term as the composed key; the instance boxes substitute a monic \(\Phi_2\) of degree \(D_2\). |
| Unchecked accumulated increment | Do not assume the degree bound for \(W=w+w_1+\dots+w_j\) from the bound on \(w\); it is checked from `(LIFT)` and corroborated by HE7-12(d). |
| “Forgets that strength” | Do not read S3.1 as exporting no irreducibility: \(\Phi'\)-irreducibility is exported as the item-5 discharge, at the original key only, and enters no step of the master proof. |
| Citation as the ledger test | Do not read S5 item 3 as requiring a pin for every *cited* unit; four classes of cited-but-unpinned material are admissible, and the r1 begin/end marker vocabulary is dead. |
| \(L\) collision | Do not conflate the legal lift \(L_k(c)\) with the side length \(L_\lambda\); the argument decides. |
| HE7-L1's proof | Do not consume LEMMA HE7-L1's §S4.1 display; it lies inside no span and is named as corroboration only. |
| HETOW:837 | Do not read the S-2 absorption append as changing HETOW-4's supply; the lemma statement stays byte-frozen and authoritative and the append is not consumed. |
| Superseded counter states | Do not cite “RESET to 0 of 2” or “STANDS AT 1 OF 2” as status; the accepted counter is 2/2. |
...
|---|---|---|
| S7 pin gate, conditions 1–4 | Two final pins added with adjudicated conditions; minimal-span argument recorded | .03, .08 |
| Standing S7 consumption sweep | Directive-consumption test applied twice; census widened to a transitive route | .04, .05, .10, .12 |
| Dated-append sweep (HE6) | Six appends classified; one pinned | .03, .04 |
| Dated-append sweep (five non-HE6 sources) | Eleven units enumerated; exactly one hit, pinned | .08, .10 |
| Composed-key transport / HETOW frame gate | \(w\ne0\) branch re-based master-side; no re-scoped licence consumed | .09 |
| Accumulated-degree tooth | Sum bound checked from `(LIFT)`, corroborated by HE7-12(d) | .09 |
| GENTOW4 perimeter gate | Clause (c) over-read corrected to the value disjunct | .09 |
| Reducible-key boundary / `(MP1)` fence | S3.1's export stated precisely; recentered key does not inherit irreducibility | .11 |
| Pin-note directive coherence | Two new pin notes; four admissible citation classes named with instances | .03, .08, .12 |
| S7 Pass 1 theorem gate | Carrier datum \(d=c_{\mathcal C}v\) displayed where it is used; the \(h(E)<\lambda\) step completed | .13 |
| Pin-sweep leg of the acceptance gate | Inventory closes at 37 with taxonomy \(20+4+13\) and census ten | .14, .15 |
| Five-instance perimeter | Final per-record Source-pin lines composed and grounded | .15 |
| S7 acceptance gate | Two resets and one non-reset recorded; superseded to 2/2 | .01, .02, .06, .07 |
...
|---|---|---|
| 4019–4031, r10 counter | Credit, no-statement-change booking, 35 pins, counter reset superseding the rider | **Local: .01** |
| 4035–4041, r11 header | Arm, pass of record, protocol, verification list | **Local: .02** |
| 4045–4055, r11 repair 1(a) | Why the β-guard correction is consumed | **Local: .04** |
| 4057–4070, r11 repair 1(b) | `HE6-BETA-GUARD` declared with its conditions and pin note | **Local: .03**; `(PINS)` row → shard 1 `.03` |
| 4072–4088, r11 repair 1(c) | Two citation sites amended; census sentence | **Local: .03, .05** |
| 4090–4138, r11 repair 1(d) | Pin count 35→36; taxonomy; census nine | **Local: .14**; counts → shard 1 `.03`, `.42` |
| 4140, r11 repair 1(e) | `HE6-BETA-GUARD` onto all five Source-pin lines | **Local: .15**; destination S3.1–S3.5 → shard 1 `.33`–`.37` |
| 4142–4146, r11 repair 1(f) | Sweep re-run; the asymmetry test stated | **Local: .10** (the test), `.05` |
| 4150–4162, r11 counter | Round record | **Local: .02** |
| 4166–4176, rider-2 header | Fold rule, scope, landmark convention | **Local: .06** |
| 4178–4198, Rider 2.1 | Census widened to the transitive route; pin note classified | **Local: .05** |
| 4200–4202, rider-2 counter state | NOT reset; stands 1 of 2 | **Local: .06** |
| 4206–4212, r12 header | Arm, pass of record, protocol, verification list | **Local: .07** |
| 4216–4224, r12 repair 1(a) | What C-A says and which sentence it governs | **Local: .08** |
| 4226–4234, r12 repair 1(b) | Degree conjunct supplied master-side; corner analysis; accumulated increment | **Local: .09** |
| 4236–4242, r12 repair 1(c) | S1.8B scope clause amended | Base S1.8B → shard 1 `.24`; **local: .09** |
| 4244–4259, r12 repair 1(d) | `HETOW-KEY-BRIDGE-C-A` declared; minimal-span argument; pin note | **Local: .08**; `(PINS)` row → shard 1 `.03` |
| 4261–4309, r12 repair 1(e) | Pin count 36→37; taxonomy; census ten | **Local: .14** |
| 4311–4327, r12 repair 1(f) | `HETOW-KEY-BRIDGE-C-A` onto S3.4/S3.5; the r11 tail-string arithmetic | **Local: .15** |
| 4329–4345, r12 repair 1(g) | Five-source dated-append sweep | **Local: .10** |
| 4351–4365, r12 repair 2 | S3.1 export sentence corrected | Base S3.1 → shard 1 `.33`; **local: .11** |
| 4369–4383, r12 repair 3 | S5 item 3 restated on consumption | Base S5 → shard 1 `.40`; **local: .12** |
| 4387–4423, r12 repair 4 | Four nit cures (L-notation; \(d=c_{\mathcal C}v\); HE7-L1 attribution; the \(h(E)<\lambda\) step) | Base S1.4/S1.1 → shard 1 `.09`, `.04`; r9 repair 1(b) → shard 4 `.05`; **local: .13** |
| 4427–4439, r12 counter | Theorem-statement status; final counts; counter reset | **Local: .07** |
...
|---|---|
| .03, .04, .05 | One repair, three functions: the pin (what is declared), the analysis (why it is consumed), and the census (where). The census is separated because a later rider widens it and the other two are untouched by that. |
| .06 | A rider block sets counter state; it is a status record and must not be merged with either neighbouring round. |
| .08, .09 | The pin and the derivation are separated because r12 explicitly keeps them independent: “The correction is pinned regardless, because pinning is owed for the directive itself and not only for the mathematics it would have blocked.” |
| .10 | The sweep is a corpus-wide classification record with its own classifier (the asymmetry test); folding it into either pin would hide that it enumerates eleven units and hits one. |
| .11, .12 | Two honesty corrections to base sections, kept apart because they correct different things: an instance record's export claim, and the ledger's statement of the gated relation. |
| .13 | The four nit cures are one unit because r12 folds them as one repair and (b) and (d) are connected — (d) uses the ultrametric law (b) displays. |
| .14, .15 | Counts and lines are separated because they close differently: the counts are a single arithmetic chain, the lines are five per-record declarations composed from three rounds. |

**Leading edge.** Line 4018 is blank; line 4019 opens `## r10 counter`. The r10 round is split across the edge: header and repairs in shard 4, counter here; both halves name each other.

**Trailing edge.** Line 4441 is `---` and 4442 is blank; line 4443 opens `## Post-pass rider 3`. The r12 round is complete within this range — header, four repairs and counter — so the trailing edge falls between whole blocks. Post-pass rider 3 and the ACCEPTANCE RECORD are compiled in shard 6.

---

## 9. SHARD NOTES

- **Cross-shard supersessions applied (downward):** only the acceptance record's status (counter 2/2) and its confirmations of this range's final figures. **No later text amends any locally homed unit's content** — r12 is the last repair round, and post-pass rider 3 touches exactly one clause, the *ground* of the HE7:2478/M-B row in `.10`, leaving its disposition unchanged.
- **Cross-shard supersessions applied (upward):** r12 repair 4(d) completes the lift-freedom step of the derivation compiled at shard 4 `.05`; shard 4 `.05` was amended at compilation of this shard to carry it. r12 repairs 2, 3 and 4(a)–(b) amend base sections compiled in shard 1 (`.33`, `.40`, `.09`, `.04`); shard 1 already quotes the r12 forms of the \(d=c_{\mathcal C}v\) paragraph and of S5 item 3, so no upward amendment to shard 1 was required.
- **Intra-shard supersessions applied:** Rider 2.1 over r11's census sentence; r12 repair 1(e)–(f) over r11's counts and two Source-pin lines. Only replacement text is stated as effective.
- **This shard carries the final settings.** `.14` and `.15` are the last word on the pin inventory and the five Source-pin lines; no other shard carries either. The merge should treat `.15` as the effective S3.1–S3.5 pin declarations and attach them to shard 1 `.33`–`.37`.
- **Exception (ii) accounting.** Used once, at `.09`, whose destination is base S1.8B (shard 1 `.24`); shard 1 compresses the derivation to one sentence. No shard-1 statement, display, clause or instance box is re-emitted.
- **Merge flags:** (i) the final Source-pin lines (`.15`) have no shard-1 counterpart. (ii) The five-source dated-append sweep (`.10`) has no shard-1 counterpart; shard 1 `.40` records only the four admissible citation classes. (iii) Shard 1 `.24` does not carry the master-side degree derivation — supplied here as `.09`.
- **Verification independence:** all 30 source lines were re-verified with their line numbers, and both multi-occurrence census strings reproduce the note's own arithmetic (3 at bound 4031; 4 at bound 4202). The `grep -lF 'T2V2PIN'` claim of the r12 protocol is consistent with the acceptance record's independent report of zero `T2V2PIN` lines in the six sources.
- **Protocol versus absorption:** the two sweeps here are source-side classification, not downstream consumption checks. The acceptance record still queues every downstream re-check.
...
| Check | Result |
|---|---|
| Statement inventory | 15 temporary-ID units, all with HOME in source lines 4019–4442 |
| Required fields | 15/15 have `CANONICAL STATEMENT`, `DERIVATION`, `CONDITIONALITY`, `RESOLUTION TRACE`, and `TEETH` |
| XREF verification | 30 source lines at count 1 with recorded line numbers confirmed; 18 master-internal landmark checks at recorded values, including two multi-occurrence censuses |
| Compiler open calls | 0 |
| Source open obligations | `(LB1)` and `(MP1)` only, unchanged by every round here; no obligation opened or closed; no fence added |
| Supersession | Intra-shard (rider 2 over r11; r12 over r11) and upward cross-shard applied; superseded counts recorded as historical only |
| Protocol/supply split | Two source-side sweeps separated from the queued downstream consumption checks |
| Tables | Source enumerations as `[TABLE]`; the composed Source-pin lines marked `[ASSEMBLED]` with their three constituent rounds named; compiler ledgers marked |
| Fidelity | Quotations verbatim or explicitly `[ASSEMBLED]`, abridgements marked; source lines byte-exact |
| NON-IMPORTS | Present, 13 rows, all from locally homed text |
| Teeth inverse | Both directions checked; zero orphans |
| HOME routing | All 24 physical blocks accounted for; 15 homed locally, the rest routed (four upward) |
| Shard edges | Leading edge splits r10 with shard 4; trailing edge falls between whole blocks; no unit truncated |
| Padding discipline | Exception (ii) used once with its destination named; no shard-1 statement re-emitted |

EFF-T2-s5of6 COMPILED: 15 statements / 30 xrefs verified / 0 open calls
