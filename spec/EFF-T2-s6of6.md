...
and the block defines the text of record:
...
**ABSORPTION/supply SET.** T2 supplies one master theorem and five instance records, all inventoried at their base HOME in shard 1. What this block adds is the **status** under which they are supplied — an acceptance that is explicitly not a clearance:
...
and a grade that is explicitly **not** transitive to the composed instances:

> “**Acceptance of this note does NOT make those composed instances \(2/2\).** It is acceptance of the composition, not an upgrade of the level-one supplier; \(g_{\mathrm{master}}=2/2\) removes the master's own factor from the minimum and nothing else.”

**APPEND/consumption PROTOCOL scope.** Separately and explicitly outstanding:
...
2. The two post-acceptance predicates are kept apart: mathematical acceptance (2/2, with the ledger) and downstream consumption/repricing (queued).
3. `(MP1′)` is not a standing conditionality; the four that are, are quoted in full at `.05`.
...
Every unit below is verbatim or explicitly `[ASSEMBLED]` from a single block; abridgement is marked with an ellipsis. Compiler ledgers are separately marked.
...
**CONDITIONALITY:** Confined to one clause of one bullet in r12's sweep record. It amends the ground of the row compiled at shard 5 `.10`; that row's disposition stands.
...
### EFF.T2.s6of6.03  [acceptance-box]
...
**CONDITIONALITY:** “Zero CRITICAL across all thirteen” is a statement about findings, not a claim of correctness; the standing conditionalities at `.05` are what remain open after it.
...
**RESOLUTION TRACE:** source lines 4492–4499. Cross-shard: `(LB1)` is homed at base S1.7B (shard 1 `.18`); `(MP1)` at S1.8A (shard 1 `.23`); `(SEC-RANK)`'s discharge map at shard 2 `.08`; `(RISE)`'s at shards 3 `.03` and 4 `.03`/`.05`; the `(MP1′)` retirement at shard 3 `.04`.
...
### EFF.T2.s6of6.06  [acceptance-box]
...
**CONDITIONALITY:** This is source-side evidence only. It certifies that every consumed source unit lies inside a resolvable span and that no source was edited; it certifies nothing about downstream consumers, whose checks remain queued (`.09`).
...
### EFF.T2.s6of6.09  [changes-record]
...
| Claim in the block | Re-run | Result |
|---|---|---|
| “**Zero `T2V2PIN` lines in the six accepted sources**” | `grep -lF 'T2V2PIN'` over the six | **no hits** — claim reproduced |
| “`git status --porcelain lean/notes/openmath/` was empty at pass 2” | `git status --porcelain` on the six sources, and on the note | **empty** for all seven — claim reproduced (at compilation date) |
| “`runs/comp13/T2SOL5_output.log` … **0 CRITICAL + 0 GAP + 0 minor**” | read the log's verdict line | `VERDICT: CLEAN \| FINDINGS: 0C + 0G + 0m` — reproduced |
| “`runs/comp13/T2P10_output.log` … **0 CRITICAL + 0 GAP + 1 minor**” | read the log's verdict line | `VERDICT: CLEAN \| FINDINGS: 0C + 0G + 1m` — reproduced |
| “eight Opus/Claude-arm passes (`runs/comp13/T2P3`–`T2P10`) and five gpt-5.6-sol-high passes (`runs/comp13/T2SOL`–`T2SOL5`)” | directory listing | the T2 logs named across the arc are present in `runs/comp13/` |
| Rider 3.1: “`grep -n 'HE7\.D\|HE7\.B'` over the master returns exactly two lines: 2136 and 4333” | re-run **under the rider's own bound 1–4439** | **exactly [2136, 4333]** — reproduced |
...
|---|---|---|---|
| Rider 3 fold + counter (.01) | Effective; sets 2/2 | Rider unread by a third fresh context | n/a |
| Rider 3.1 re-grounding (.02) | Effective | Disposition unchanged; ground corrected | Amends shard 5 `.10`'s ground |
| The arc (.03) | Effective | Zero CRITICAL is a findings statement, not a correctness claim | n/a |
| The two passes (.04) | Effective | Pass 2's four nits live and uncured | Pass 2 ran the 37-pin sweep (source-side) |
| Four standing conditionalities (.05) | **Effective and final** | Two fence reads; two are carrier hypotheses | Inherited by every consumer |
| Machine evidence (.06) | Effective; re-verified here | Source-side only | Downstream unchecked |
...
`[TABLE — negative perimeter declared by this block]`

| NON-IMPORT | Effective prohibition |
|---|---|
| Grade transitivity | Do not infer that a composed instance is \(2/2\) from the master's acceptance; \(g_{\mathrm{master}}=2/2\) only removes the master's factor from the minimum. |
| The HE3 cap | Do not read S3.1, S3.2, or any level-two chain consuming an HE3 pin through S1.8C, above HE3's HEAD grade \(0/2\) (clean count \(1/2\)). |
| Acceptance as clearance | Do not read acceptance as discharging `(LB1)`, `(MP1)`, or the two stipulated carrier hypotheses: “Acceptance is acceptance of the composition WITH this ledger, not clearance of it.” |
| A fifth conditionality | Do not carry `(MP1′)` forward as an obligation; it is retired and denotes a consequence of the pins. |
| Battery evidence | Do not expect or cite a machine battery for T2: none is owed, and S7's gate is the pin sweep. |
| Downstream status | Do not infer that any downstream consumption check has passed or that any consumer has been re-priced; both are queued. |
| In-place edits | Do not edit the accepted text: from here the note takes dated appends only, and a rewrite reopens the hostile arc. |
| Naming as consumption | Do not read the master's naming of a source unit as consumption; the M-B disposition turns on exactly that distinction. |
...
`[TABLE — compiler inverse; T2 owes no numbered battery, and this block says why]`
...
| Pin-note directive coherence | Seven reading directives audited at pass 2 | .04 |
| `(OCC)` demand at the refinement height | Closed master-side at both levels by pass 2 | .04 |
| Fence-leakage attacks | Four standing conditionalities displayed with their fences and discharges | .05 |
| Grade-laundering hazard | `(GRADE)` minimum restated; HE3 cap held; transitivity denied in terms | .07 |
...
**Zero-orphan result:** 0 statement teeth orphaned; 0 inverse teeth orphaned; **0 of 0 numbered battery rows — and this shard carries the note's own statement that no battery is owed** (`.06`).
...
|---|---|---|
| 4443–4445, rider-3 header | Fold rule, scope, the two passes named, counts held at 37 / \(20+4+13\) / ten | **Local: .01** |
| 4447–4453, rider-3 landmark convention | Bound 1–4439; no source count owed | **Local: .01** |
| 4455–4471, Rider 3.1 | The M-B disposition re-grounded on consumption | **Local: .02**; amends shard 5 `.10`'s ground |
| 4473–4475, counter state | Counter NOT reset; stands at 2 OF 2; gate MET | **Local: .01** |
| 4479–4481, acceptance: the arc | Thirteen passes, two arms, zero CRITICAL, compressed GAP history | **Local: .03** |
| 4483–4490, the two passes + the rider that postdates them | Pass records and the counter reaching 2 of 2 | **Local: .04** |
| 4492–4499, standing conditionalities | Four rows plus the no-fifth sentence | **Local: .05** |
| 4501, machine evidence | Pin sweep at 37; zero markers; zero edits; census ten | **Local: .06** |
| 4503, grade | \(0/2\to2/2\); `(GRADE)` binds; HE3 cap | **Local: .07** |
...
|---|---|
| .01, .02 | The rider's status effect (it sets the counter) is separated from its one substantive cure, because the first governs the whole note and the second governs one clause of one bullet in shard 5. |
| .03, .04 | The arc (what happened across thirteen passes) is separated from the two passes of record (what satisfies the gate). Only the second is gate-bearing; the first is history with a per-round index. |
| .05 | The conditionality ledger is one unit because acceptance is defined as acceptance *with* it — splitting the four rows would let a consumer inherit three. |
| .06, .07 | Evidence and grade are different predicates: the sweep says the source-side gate ran; the grade says what the acceptance is worth downstream, and the two are joined only by the HE3 cap being a supplier fact rather than a gate result. |
| .08, .09 | The freeze governs the note's future text; the queue governs other notes' future work. Collapsing them would let “byte-frozen” read as “finished”. |
