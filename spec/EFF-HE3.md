# EFF-HE3 — CANONICAL EFFECTIVE-SPECIFICATION (MERGED)
...
**HEAD blob hash:** `79bbbb638c39cf4a9658e466210626bf5d6c90a5` — cited identically by both source
...
```
$ git hash-object lean/notes/openmath/HE3_PROOF_2026-08-08.md
79bbbb638c39cf4a9658e466210626bf5d6c90a5
```

**No divergence found — this was the merge run's STOP condition and it did not trigger.**
Both shards additionally record the file's last-touching commit as `62400944`.

**Compilation context:** `spec/EFF_TEMPLATE_v2.md`.

**Merged from:** `spec/EFF-HE3-s1of2.md` (HOME lines 1–1275; 38 statements, 38 xrefs,
6 open calls), `spec/EFF-HE3-s2of2.md` (HOME lines 1276–1853; 34 statements, 23 xrefs,
6 open calls). **72 statements, 61 raw XREF citations, 12 raw open calls total before
merge-time dedup.** This file supersedes the two shards as the citable unit for all
downstream consumption; any consumer citing `EFF.HE3.*` from this point on cites the
contiguous IDs below, not the shard-local `EFF.HE3.sKof2.*` IDs. The two shard files are
...
unit-boundary candidate was chosen and why) lives in full; this merged file's own
boundary section (§7) carries forward only the cross-shard edge list and the
format-level conclusions (the SERIES, the SUPERSESSION KIND vocabulary), not the
twenty-plus individual boundary arguments each shard records for its own range.

### SHARD-MAP (old shard-local ID → new contiguous ID)

Shard 1's offset is **0** (its own numbering, 01–38, is already contiguous from 1).
Shard 2 carries a **+38** offset (01–34 → 39–72). Every `EFF.HE3.sKof2.NN` occurrence —
full form (including the short `s1of2.NN`/`s2of2.NN` form used inside `TARGETS:` fields)
and the bare `` `.NN` `` shorthand used throughout each shard's own prose — has been
rewritten below to the new contiguous ID, under the following disambiguation rule
recovered from the shards' own citation discipline:
...
> names the other shard** — a `shard 1`/`shard 2` label, or (in one aggregating
> `§7 SHARD NOTES` bullet and one predicate-C table row of shard 2) a topic phrase
> naming a single shard for a whole semicolon-separated list — **and that override
> persists across commas within the same still-labeled list, resetting at the next `·`
> field-separator or at an explicit competing label** (e.g. shard 2's own `in-range`
> marker). This is verifiable independently: every `(shard 1 `.NN`)` annotation in
> shard 2 carries a line-range that lands inside shard 1's own 1–1275 span whenever one
> is given, and every un-labeled bare `.NN` in shard 2's own RESOLUTION TRACE fields
> carries a line-range inside 1276–1853 — checked exhaustively below, zero mismatches.
> **A second, orthogonal exception:** a bare `` `.NN` `` immediately following a citation
> of `spec/EFF-GENHN-s1of2.md` or `spec/EFF-GENHN-s2of2.md` is **not an HE3 ID at all** —
> it is a citation into GENHN's own (separately merged) shard numbering and is left
> completely untouched by this merge, exactly as both shards wrote it.
...
`+38` offset. The rewrite was cross-checked unit by unit against this compiler's own
reading of both shards' full text (not by a blind global substitution), and audited
afterward by exhaustive regex sweep: **zero leftover `sKof2` substrings within the merged
statement inventory (§3)** — the handful that survive elsewhere in this header and in §7
are deliberate quotations of the original shard-local notation, illustrating the rewrite
rule itself — and **zero out-of-range renumbered IDs** (every resolved ID lies in 1–72).
...
the file's last-touching commit (`62400944`) — confirmed by this merge against the
...
**GENRE: TAG-SPARSE PROSE PROOF, shifting from in-place strike-through repair (shard 1's
range) to a records-and-appends discipline (shard 2's range).** `grep -c '\tag{'` = **0**
over the whole note (re-verified fresh at merge time). Shard 1's HOME range (§S0–S6.5)
applies repairs `[r1]`/`[r2]` **in place**, with `~~struck~~` refuted text preserved
byte-adjacent to its replacement. Shard 2's HOME range (§S7–S9′ plus the two dated append
blocks) is where the discipline **switches to dated appends**: line 1589 — "The body
above, §S1 (iii) included, stays BYTE-FROZEN; the correction lives here" — and line
1631–1632 — "Everything above this heading is byte-untouched by this append." **Reading
rule for strike-through (compiler-declared, since the note does not state one, carried
unchanged from shard 1's header): `~~X~~ **[rN, Fm: Y]**` means X is REFUTED and Y is the
current reading.** The struck text is quoted at its unit whenever load-bearing, but it is
never the CANONICAL STATEMENT.

### Governing effective-text record (unified from both shards)

**FREEZE PREDICATES (rule 21 — THREE, unusually asymmetric across the two shards):**

**(F1) NO TEXT-FREEZE on the body during r1/r2 (shard 1's range) — repairs are IN-PLACE**,
**switching to BYTE-FROZEN dated-append discipline starting in shard 2's range** (line
1589, quoted above). The two shards together supply the whole predicate: shard 1 states
the in-place mechanism and its own byte-freeze exemption clause (three consumer-pinned
displays, (F3) below); shard 2 states where the freeze begins and reasserts it.

**(F2) INSTRUMENT-FREEZE — the sealed §S8 battery, byte-frozen through every round.** The
one exception is disclosed and quantified across both shards identically: the
`he3r1_supp.py` docstring changed at `[r2]`, with both md5s displayed
(`9cd132a8ce174f5703542757675f7960` → `93681e8d47d447dcbfd46db0ddfe0c55`) and the stdout
certified byte-identical — **the cleanest instrument-freeze accounting in the corpus**,
per both shards' own words.

**(F3) CONSUMER-PIN FREEZE — three specific displays are byte-frozen because frozen
consumers pin them** (DEFINITION 1, HE3-2(b)'s consumed above-λ clause, the (★) display),
stated in shard 1's header and exercised throughout shard 1's unit boundaries (§6.1 item 2
there) and referenced from shard 2 (`.05`, `.27`'s reasoning). This is a **third, distinct**
freeze predicate from (F1)/(F2): it protects three named spans against the note's OWN
in-place repair mechanism, because HE6 and HE7 are frozen at `HE3 @ 7a95449` and consume
them.

**EFFECTIVE TEXT = the whole 1,853-line file exactly as written** — `[r1]`/`[r2]` already
landed in place in shard 1's range (struck text is REFUTED text preserved for the record,
never current), READ AS CORRECTED by every later dated rider/annex that names an earlier
sentence, with no earlier sentence ever deleted. **Six dated corrections land inside shard
1's HOME range** (one from PE3, five from the 2026-08-12 A5 wave: R8-1..R8-5); each is
applied at its unit and listed in §7 below.

### Scope of record — mandatory THREE-predicate split (unified from both shards)

**A. ABSORPTION / mathematical supply SET.** The note's own inventory is its title and
§S1: **THEOREM HE3.A** (the stage-σ recursion), **COROLLARY HE3.B** ([GENHN-HE(μ)] for all
μ; unconditional at μ ≤ 3), **COROLLARY HE3.C** (the program consequence), supported by
**DEFINITION 1/2** and **LEMMAS HE3-0, HE3-0′, HE3-1, HE3-1L, HE3-1′, HE3-2, HE3-2′, HE3-3,
HE3-4, HE3-5, HE3-6**, plus **three genuinely new lemmas landed by the 2026-08-12 A5 append
wave**: LEMMA HE3-4U (`.68`), LEMMA HE3-4D1 (`.69`), and R8-1's recursive DECIDED criterion
(`.67`). The claimed absorption target is GENHN's open box: the title says "**[GENHN-HE(μ ≥
3)] CLOSED AT ITS FIRST-LIVE CASE (μ = 3, degrees n ≤ 7: UNCONDITIONAL) AND REDUCED TO ONE
NAMED BRANCH AT μ ≥ 4 (HE3-BOX-6)**." **Two of the five A5 corrections (R8-2, R8-3) NARROW
HE3-BOX-6** — the only box-narrowing events in the note, and this merge's second headline
item (see the Constrained item below).

**B. APPEND / consumption PROTOCOL scope — what HE3 lands on other notes.** **Nothing.**
HE3 *reports* on its consumers rather than appending to them — §S0′'s CONSUMER TRACE
(`.06`) is a table of four consumers with a SAFE/N-A verdict each, computed by HE3 and
landed nowhere: "no consumption site of HE3 found in `GENIND_PROOF_2026-08-08.md` (grep,
this round)" is its own words for the GENIND row. **Zero dated consumption appends are
claimed landed on any consumer, and none is invented here.** Crucially, **HE3's own
consumer trace is demonstrably two edges short of the true outbound picture** — per
template rule 10 (a cross-note status claim must be checked against the full append chain,
not the frozen consumer-trace sentence alone), this merge's own re-verification (§4, §5)
confirms both **HE7 consumes `HE3-BOX-6`** and **T2's `(MP1′)` retirement rests on HE3's
own stage-α passage** — neither edge appears in `.06`'s four-consumer table, and shard 2's
own compiler pass surfaced both independently (its `.44` and its §3.2 reverse-edge row).
Both are carried below as explicit reverse-XREF rows in §5 and both associated OPEN-CALLs
(shard 2's OPEN-CALL 1 and OPEN-CALL 3) are carried forward unresolved in §8.

**C. INBOUND CORRECTIONS RECEIVED.** Six streams, all landing inside shard 1's HOME range
(four discovered/applied while shard 1 was itself being drafted, two dated appends applied
afterward from shard 2's own range):
...
| 3 | Fable PE3 (`HE3_passPE3_report.md`) | dated append (homed in shard 2, `.65`) | 1 MINOR (M-1) | `.12` |
| 4 | sol certification A5 (2026-08-12), re-adjudicating the R8 chain read | dated appends (homed in shard 2, `.66`–`.72`) | 5, all REAL | `.01`, `.17`, `.25`, `.28`, `.29`, `.32`–`.35`, `.44`, `.55`, `.67` |
...
1. Shard 1's lines 1–1275 supply the base statements. `[r1]`/`[r2]` tags are
...
2. **Post-PE3 corrections are dated appends and never edit; they quote and rider.** Six
   land on shard-1-homed units, all physically drafted in shard 2's range (§7 below).
3. **The three consumer-pinned displays are byte-frozen even against the note's own
   in-place mechanism** — where a repair was needed at a pinned display, it is landed
   *adjacent* rather than *inside* (`.14`, DEFINITION 1's `[r2]` construction pin, is the
   pattern). A merge that flattened `.13` into `.14` would violate the consumers' pins;
   they remain two units.
4. **Two out-of-range appends PROVE previously boxed material** (R8-2 proves branch (i);
   R8-3 proves the D′ = 1 case). They are **licenses**, not supersessions, and each
   *narrows* HE3-BOX-6 rather than widening it. Applied at `.44` with the box's own
   post-narrowing scope re-stated alongside its original displayed text (never silently
   rewritten).
5. Out-of-range statements (relative to either shard's own HOME) are not emitted as
   separate units merely because they resolve in-range text; each of shard 2's six boxes,
   the battery, the verdict blocks, and all six dated corrections are homed exactly once,
   in shard 2.
6. Every cited commit hash was verified with `git cat-file -t`; every cited artifact was
   verified to exist; hashes deferred to a commit message (never displayed in the note's
   own text) are marked **UNPINNED** (rule 23).
7. **HOME is determined by an edit's destination, not physical drafting location** — the
   same rule T1's and HE7's merges used. All six of shard 2's dated corrections are
   physically drafted in shard 2's range but resolve shard-1-homed units (five of the six)
   or in-range units (`.44`, `.55`, `.67`); §7 below verifies each resolves to exactly one
   place in the merged inventory.

### Quotation and table discipline

Unchanged from both shards: CANONICAL STATEMENTs are verbatim source quotations or
explicitly marked `[ASSEMBLED]`; truncations marked `…`; strike-through markers reproduced
exactly; every statement unit carries `FORM:`. Every source table is transcribed as a
table and marked `[TABLE]`; the note's several fixed-width ASCII displays (rule 14) are
emitted **twice** per both shards' own extension of the rule — verbatim as the source's
display, then again as a `[TABLE]` Markdown transcription. Compiler-authored ledgers (XREF
tables, consumption ledgers, TEETH inverse tables, the SERIES/edge tables of §7) are marked
...
`[TABLE — compiler ledger, consolidated from shard 1 §4 (14 rows) and shard 2 §4 (15
rows) = 29 raw rows, 0 dedup pairs found → 29 distinct rows below]`

**Dedup count for this section: 0.** Unlike T1/HE7, no fence in either HE3 shard
restates a near-identical negative claim already stated by the other shard — the two
ranges' NOT-consumed/NOT-claimed fences are about disjoint material (shard 1: GENHN-4,
Montes/GMN order-≥2, the graded-frame fence, base-change-freedom at μ ≤ 3; shard 2:
machine-coverage holes, the char-p oracle asymmetry, the two owed teeth, HE-BND's
generality). Every row below is specific to its own shard and asserted once.

| Fence | Explicit material NOT imported, NOT proved, or NOT claimed | Source |
|---|---|---|
| GENHN-4 / the W-9 weld face | "**GENHN.A(iv)/LEMMA GENHN-4 is NOT consumed**, hence **the W-9 graded weld face (GENHN-BOX-2) is NOT inherited**" | s1 `.04` |
| Montes/GMN order-≥2 | "named in GENHN's box as the "faithful-cite target" — is **not** cited" | s1 `.04` |
| SIGMALAW / JC-LOAD / DULEMMA | "not consumed" | s1 `.04` |
| Graded frame / digit-space bijection | "**No graded frame and no digit-space bijection is used: (a)–(c) are statements about valuations and residues only.**" | s1 `.27` |
| GENHN-2 at non-roots | `[r1, F4]`: GENHN-2 "is stated at roots θ of Φ′ only …; the paste above overconsumed it" — the ξ-form is HE6-0″, not GENHN-2 | s1 `.03`, `.25` |
| GENHN.C as input | "consumed as a CROSS-CHECK only: §S6 re-derives it as the μ = 2 case of HE3.A, **so no circularity**" | s1 `.03`, `.36` |
| The node, in the Ψ-comparison | "the lemmas cited consume only dv(Φ″ − Φ′) > D′h and the locus, **never this node** — no circularity" | s1 `.11` |
| The letter-starved corner and μ ≥ 4 in the node derivation | "are NOT closed by this display; there the node is CONSUMED from GENHN's [r1] ERRATUM" | s1 `.11`(iii) |
| (SEP) at general μ | "For general μ, (SEP) is a real hypothesis, and lifting it needs the same enlarged test family as HE3-BOX-6" | s1 `.29` |
| Base change at μ ≤ 3 | "μ ≤ 3 IS BASE-CHANGE-FREE; the two base-change reductions below are μ ≥ 4-only and are BOXED" | s1 `.33` |
| Both base-change branches (as originally displayed) | "**Both branches below are therefore to be read as BOXED (HE3-BOX-6), not proved.**" — later narrowed to (ii) alone by R8-2 | s1 `.33` |
| Exact summand valuations in the recentering | `[REPAIR, PE1 finding 4]`: "Only the INEQUALITY in (a) survives summand-wise" | s1 `.27` |
| The ℓ ≥ 2 residue read at μ ≤ 3 | "no μ ≤ 3 conclusion consults this read" | s1 `.25` |
| "UNCONDITIONAL" ≠ graded | "It does NOT upgrade the grade: the note is at attempt 0/2 and every consequence carries HE3-BOX-1's supplier-stack conditionality." | s1 `.17` |
| A composer-context pass | "The earlier Codex pass adjudicated in the composer's own context … **does not count toward the grade**" | s1 `.02` |
| The naive R_λ read | "What is NOT in the class is the naive per-slot read … the naive assembly can have a DIFFERENT factorization type" | s1 `.14`, `.24` |
| GENHN-4 / the W-9 weld face (restated) | "HE3 does not consume LEMMA GENHN-4, so the W-9 weld face (GENHN-BOX-2) is absent from the σ chain — it survives only inside GENHN's COUNT laws" | s2 `.39` |
| μ ≥ 4 as a graded claim | "μ ≥ 4 is BOXED, not graded." | s2 `.39` |
| Machine coverage | "NOT exercised by machine: f₁ ≥ 2 stage genres at μ = 3 …, μ ≥ 4 …, d ≥ 2 ambients, and the char-p side of the resultant leg" | s2 `.41` |
| An independent char-p oracle | "PARI's `factorpadic` σ-oracle exists only on the ℤ_p side … the char-p rows are scored against the reader plus the internal resultant identity, **not against an independent oracle**" | s2 `.42` |
| Two promised teeth | "HE-T-PSI (corrupted slope set) and HE-T-CERT (embedded uncertified tail) are OWED." | s2 `.43` |
| A promised embedded row | "no embedded n = 7 row" | s2 `.43` |
| HE-T-NODE as a tooth | "**HE-T-NODE as coded does not test what its name claims** … it should be read as a diagnostic, not a tooth" | s2 `.43` |
| Branch (ii), post-narrowing | "the only place the argument leaves the ground it stands on … must be read as CONDITIONAL on it" | s2 `.44` |
| The repair route | "IDENTIFIED REPAIR ROUTE (**not executed here**) … That is a genuine further step, not a rewording." | s2 `.44` |
| Machine grade vs acceptance | "(instrument grade, not acceptance) … nothing here upgrades the conditionality display of COROLLARY HE3.C" | s2 `.56` |
| A general boundary law | "**No single "one more window unit" law, nor one unstratified exact q-power, follows from HE3-5.**" | s2 `.67` |
| HE-BND's generality | "it does not verify a general two-step boundary law" | s2 `.67`, `.55` |
| The split-key count | "**No claim that a single split key still contributes D′ roots is used.**" | s2 `.68` |
| P4's independence | "P4 verifies the T(κ) BOOKKEEPING …, **not the residue arithmetic**; the r1 sentence … overstated the leg's independence" | s2 `.62` |
| The N1 demo's geography | "a geography where the N1 gap is **not LIVE**" | s2 `.60` |
| Weakening any accepted display | "no accepted display of THEOREM HE3.A or COROLLARY HE3.B is weakened" | s2 `.66` |

---

## 3. Statement inventory (72 units, contiguous, in source order)

### EFF.HE3.01  [changes-record]
...
**RESOLUTION TRACE:** statement line 1 · correction sites 1579–1583 (grade, `.64`), 1691–1734 (R8-2, `.68`), 1736–1772 (R8-3, `.69`), 1851–1853 (ledger, `.72`).
...
### EFF.HE3.02  [changes-record] · SERIES: HE3-GRADE-ARC, member 1 of 4
...
|---|---|---|---|
| 1 | composed; PE1 NOT CLEAN (2C + 4G + 2M); `[r1]` repairs | **this unit**, lines 3–20 | 0/2 |
| 2 | PE2 NOT CLEAN (0C + 2G + 3M); `[r2]` repairs | `.07`, lines 120–139 | 0/2 |
| 3 | HE3-BOX-1's grade box | `.39`, lines 1278–1294 | 0/2 |
| 4 | **PE3 CLEAN (0C + 0G + 1M)** — "The consecutive-clean count moves 0/2 → **1/2** and HOLDS at 1/2 through this correction" | `.64`, lines 1579–1583 | **1/2 (TERMINAL)** |
...
**RESOLUTION TRACE:** statement 3–20 · correction sites 120–139 (`.07`), 1278–1294 (`.39`), 1579–1583 (`.64`), 1851–1853 (`.72`).
...
**TEETH:** the sealed battery (`.03`'s instrument) plus the additive `he3r1_supp.py` leg; both inventoried in §6 and homed as verdict units in the shard-2-derived range. **`disclosed non-repair`:** "4 wired teeth" against five promised — the shortfall is disclosed at HE3-BOX-4b (`.43`), and one of the four (HE-T-NODE) "does not test what its name claims".
...
**RESOLUTION TRACE:** statement 24–77 · re-derivations 169–237 (`.11`) · correction sites 1795–1819 (R8-5's provenance fix at §S4, `.71`).
...
### EFF.HE3.04  [fence]  — the NOT-consumed fence
...
**RESOLUTION TRACE:** statement 79–88 · discharge 700–809 (`.24`) · restatement 1291–1294 (`.39`).
...
### EFF.HE3.05  [table]  — §S0′'s `[r1]` repair record (nine rows)
...
**CONDITIONALITY:**
(a) **F5's row is the shard's most consequential disposition** and it is honest about *why* the defect survived: two false displays that "mutually cancel". `.32` carries the note's own explanation — "which is why the sealed battery never caught either (PE1 F5)". A defect invisible to the instrument because two errors cancel is exactly the class rule 27's AUDIT disposition exists for.
...
**RESOLUTION TRACE:** statement 92–108 · sites as listed above · later re-audit 1691–1734 (`.68`), 1795–1819 (`.71`) — R8-2, R8-5 re-open F5-adjacent and F4-adjacent material.
...
**TEETH:** `he3r1_supp.py` P1–P4 — the additive leg built to verify the r1 repairs; inventoried in §6, homed in the shard-2-derived range. **Disposition: executable regression built from a repair round.**

---

### EFF.HE3.06  [table]  — §S0′'s CONSUMER TRACE (four rows)
...
**CONDITIONALITY:** **This table is a cross-note claim in four parts and rule 10 requires it be checked against each target's full state, not against a pinned sentence. This compiler checked all four, and — per rule 10 more broadly — the merge run additionally re-checked the table's own completeness (it omits two real edges; see below):**
...
(b) **HE7 row — CONFIRMED at the designation level.** `grep -cF 'HE3-3' HE7_PROOF_2026-08-08.md` = **2** ✓, matching the (★)-of-HE3-3 citation. **But this trace omits a second, larger HE7 edge**: HE7 also consumes `HE3-BOX-6` (verified fresh at merge time; see §4's headline finding — the count both shards claimed, 12, is itself a discovered drift, corrected to **1** at merge time). Recorded as source defect 1.
...
(e) **A fifth, entirely absent row: T2.** `spec/EFF-T2.md`'s `EFF.T2.25` records T2's `(MP1′)` obligation RETIRED on the strength of `HE3-A-PROOF` (verified fresh: `grep -cF 'HE3-A-PROOF' lean/notes/openmath/T2_SIGMA_LADDER_MASTER_2026-08-12.md` = **65**), which is `.34`'s own stage-α passage (lines 1149–1166). This table names four consumers; T2 is not one of them. Recorded as source defect 1 (continued) and carried as an explicit reverse-XREF row in §5.
...
### EFF.HE3.07  [table]  — §S0″'s `[r2]` repair record (five rows)
...
**RESOLUTION TRACE:** statement 120–139 · sites F-1 → `.14`, `.22`, `.23`, `.24`, `.25`, `.27`, `.32`; F-2 → `.11`; F-3 → `.29`; F-4/F-5 → §S9′ (the shard-2-derived range). **All five located.**
...
### EFF.HE3.08  [hypothesis]  — §S1's standing frame
...
**RESOLUTION TRACE:** HE3-side statement 1, 369–378 · GENHN-side status `GENHN_PROOF_2026-08-08.md` 1029–1042, 1449–1453 · HE3's grade 1579–1583 (`.64`) · the ledger line 1851–1853 (`.72`).
...
**TEETH:** NONE (status record). The mathematical claim's teeth are at `.15`–`.17`; the machine confirmation of all five μ = 3 stage types is §S9's Q3 (`.52`).

---

### EFF.HE3.10  [definition]  — the stage ring, re-carried through L (the `[r1, F7]` correction)
...
**SUPERSESSION KIND: replacement** (the carrier object). `TARGETS: .10` itself; the rider enumerates its own downstream sites (`.20`'s norm, `.34`'s stage-CS step).
...
**Two independent corroborations:** (i) GENHN's own `[e2]` ERRATUM E3 carries the identical correction one level up — HE3 says so, and it checks (`GENHN_PROOF` 470–527: "R = Ô[x]/(Φ′) = Ô[θ] is in general the NON-MAXIMAL order Ô[θ], not a DVR"). (ii) HE3-BOX-6 (`.44`) independently rests on the same fact: "**no element of R has non-integer dv** (dv is ℤ-valued on R)".
...
**SUPERSESSION KIND: replacement** (the node), with the derivation itself **replaced a second time** at `[r2]`. **CHAIN: sealed `S`-node → `[r1]` `D′h`-node with a derivation that assumed attainment → `[r2]` Ψ-comparison derivation. TERMINAL: the `[r2]` derivation.** `TARGETS: .03` (the §S0 import), `.13` (DEFINITION 2's node name), `.20`, `.24`, `.26`, `.27` (every threshold in the note).
...
(b) **The letter-starved corner is excluded** — and part (iii)'s parenthetical listing that corner is **corrected by the post-PE3 M-1 append** (`.65`), which finds *neither* of its two readings matches the displayed list: under `d_λ ≤ μ` the list must add `(4,1)`; under the in-context `d_λ ≤ μ − 1` the listed `(2,2)` is unreachable. **The frozen fence nonetheless stands** because the consumed GENHN corner `{(2,1),(3,1),(2,2)}` is a superset of the in-context reachable corner `{(2,1),(3,1)}` — conservativity. `SUPERSESSION KIND: counter re-reading`. See `.12`.
(c) **Non-circularity is asserted and checkable**: "the lemmas cited consume only dv(Φ″ − Φ′) > D′h and the locus, never this node". **This compiler checked it:** LEMMA HE3-1 (`.20`) requires `dv(Φ″ − Φ′) > D′h`; LEMMA HE3-1′ (`.22`) requires `κ > D′h`; the convex duality (★) (`.31`) is pure convexity. **None consumes the node** ✓ — the non-circularity claim holds.
...
- **The starved-corner bound.** `|K^×| = Q^{f₁} − 1 ≤ d_λ` with `d_λ ≤ μ = 3` gives `Q^{f₁} ≤ 4`, i.e. `Q^{f₁} ∈ {2,3,4}` ⟹ `(Q,f₁) ∈ {(2,1),(3,1),(2,2),(4,1)}` ✓ — **four pairs, and the display lists three.** The M-1 append (`.65`) reaches the identical conclusion ✓ **this compiler's independent recomputation confirms the append's first bullet exactly.**
...
**RESOLUTION TRACE:** statement 163–167 · derivation 169–228 (`[r2]` re-display) · scope 218–228 · locus scoping 230–237 · correction sites 1591–1618 (post-PE3 M-1, `.65`).
...
### EFF.HE3.12  [scope-record]  — §S1 (iii)'s starved-corner parenthetical
...
**CONDITIONALITY:** **The parenthetical is WRONG under both of its readings, and the post-PE3 append says so while leaving it frozen.** Quoted from `.65`:
...
and the conservativity finding at `.65`:
...
**RESOLUTION TRACE:** statement 242–251 · construction pin 253–286 (`.14`) · consumers' pin `7a95449` ✓ commit · byte-frozen per §1.
...
### EFF.HE3.14  [definition]  — the `[r2, F1]` CONSTRUCTION PIN for R_λ (the coherent assembly)
...
**SUPERSESSION KIND: license** (supplies a construction DEFINITION 1 presupposes) — deliberately NOT a replacement, because (F3) freezes the definition. `TARGETS: .13`, and by the rider's own pointer list `.22` (§S3 HE3-1′'s proof), `.25` (§S4 (ii)), `.27` (§S5 (b)), `.32` (§S6.2 HE3-4).
...
(d) Untouched by the 2026-08-12 A5 wave except through R8-5, which re-points HE3-0's residual sentence to the matching coherent normalizers (`.71`).
...
**RESOLUTION TRACE:** statement 253–286 · arithmetic and witness 588–642 (`.23`, `.24`) · pointer sites 576–582, 655–658, 726–728, 787–793, 983–984 · later touch 1810–1815 (`.71`).
...
**TEETH:** **`he3_pe2fresh.py`'s member `ma`** — the twist adjudicator, PARI σ = {(2,4)}, siding with the coherent read. **Re-confirmed at PE3 at three further geographies**: "with the coherent-vs-naive flip machine-adjudicated at each of the three, siding with the coherent read each time" (`.64`). **Disposition: planted mutant (naive vs coherent), fired four times across two passes.**

---

### EFF.HE3.15  [definition]  — DEFINITION 2 (the stage read; decided leaves)
...
**RESOLUTION TRACE:** statement 288–310 · node source 163–237 (`.11`) · stage-α consistency 1149–1169 (`.34`) · recursive re-reading 1634–1689 (`.67`, R8-1).

XREF: `GENHN_PROOF_2026-08-08.md:GENHN.A(v)` — count **6**.
XREF: `GENHN_PROOF_2026-08-08.md:GENHN-3(a)` — count **11**.

**TEETH:** **HE-BND** — the battery's decided/undecided profile vs window (`.55`) measures exactly this definition's condition (b). **Disposition: measurement.** HE3-BOX-5 item (4) (`.45`) names the open question: "whether "decided leaf" as defined in DEFINITION 2 coincides exactly with LEMMA HE3-5's window condition on every row (the battery measures both)".

---

### EFF.HE3.16  [theorem]  — THEOREM HE3.A (the stage-σ recursion)
...
(a) At the time of writing, LEMMA HE3-4 carried **two** boxed base-change branches (§S6.2, `.33`: "Both branches below are therefore to be read as BOXED (HE3-BOX-6), not proved"), while §S1 and HE3-BOX-6 said only branch (ii) was boxed — **an internal contradiction the A5 wave named**. R8-2 (`.68`) resolves it by PROVING branch (i) as LEMMA HE3-4U. `SUPERSESSION KIND: license`.
...
**RESOLUTION TRACE:** statement 312–331 · proof 1092–1205 · dependency corrections 1691–1734 (`.68`, R8-2), 1736–1772 (`.69`, R8-3), 1774–1793 (`.70`, R8-4).

XREF: `GENIND_PROOF_2026-08-08.md:GENIND.A(IV)` — count **5**.

**TEETH:** **HE-SIG** — "PARI `sig(f,p)` = factorpadic-200 + idealprimedec, the pinned w12_checks question, against the HE3.A dictionary", scored at §S9 Q3: **947 PARI jobs, 0 bad, ALL FIVE μ = 3 stage types realized and confirmed** (`.52`). Planted mutant **HE-T-SIG**: "6/6 flipped dictionaries caught by PARI on each of the two rows". **Disposition: executable regression against an independent oracle + planted mutant, both fired.** The headline row is the 3-label type `(1,1)+(1,1)+(1,1)` — "**25 sextics that split into three ramified quadratics, each PARI-confirmed** — the first machine confirmation of a 3-label stage σ anywhere in this program."

---

### EFF.HE3.17  [theorem]  — COROLLARY HE3.B ([GENHN-HE(μ)] for all μ) with its `[r1]` headline rescope
...
**RESOLUTION TRACE:** statement 333–367 · proof inline + 1207–1214 (`.36`) · correction sites 1691–1734 (`.68`, R8-2), 1736–1772 (`.69`, R8-3), 1579–1583 (grade, `.64`).

XREF: `GENIND_PROOF_2026-08-08.md:GENIND.A(IV)` — count **5**.
XREF: `GENHN_PROOF_2026-08-08.md:[GENHN-HE` — count **19**.
XREF: `HE3_PROOF_2026-08-08.md:HE3-BOX-6` — self-count **12** (the box is named twelve times in its own note).

**TEETH:** **PROOF-ONLY at μ ≥ 4** — HE3-BOX-3 (`.41`): "NOT exercised by machine: … μ ≥ 4 (where LEMMA HE3-4's ramified base-change branch and the stage-CS recursion first bite)". At μ ≤ 3 the dictionary is toothed by HE-SIG (947 PARI jobs, all five types).

---

### EFF.HE3.18  [theorem]  — COROLLARY HE3.C (the program consequence)
...
(a) **"conditional exactly on {GENHN's S9 boxes} ∪ {arc stack}" omits `[GENHN-TOW-1]`.** GENHN's own corrected conditionality display (its lines 352–371, compiled at `spec/EFF-GENHN-s1of2.md` `.18` — GENHN's own shard numbering, untouched by this merge) reads: at `n ≥ 8`, P(n) rests on `{[GENHN-HE(μ ≥ 3)]} ∪ {[GENHN-TOW-1]} ∪ {S9 boxes} ∪ {arc stack}` — and `[GENHN-TOW-1]` **is** one of GENHN's §S9.1 boxes, so the phrase "GENHN's S9 compression/coverage boxes" arguably covers it. **But `[GENHN-TOW-1]` is not a compression or coverage box — it is a MATHEMATICS box** (GENHN's own §S9.2 files it under "MATHEMATICS (new theorems needed)" as item 2b). Under GENHN's own taxonomy, HE3.C's phrase under-describes it. Recorded as source defect 3; the compiler does not rewrite the clause.
...
**RESOLUTION TRACE:** statement 369–378 · GENHN-side display `GENHN_PROOF` 352–371 · grade 1579–1583 (`.64`).
...
### EFF.HE3.19  [scope-record]  — §S2, the mechanism and why the μ = 2 argument stops
...
- `Σ e_sf_s = 3` admits `{(3,1)}, {(1,3)}, {(1,1),(2,1)}, {(1,1),(1,2)}, {(1,1),(1,1),(1,1)}` — **five** ✓ **exactly the five types §S9's Q3 table lists** (`.52`) ✓ **cross-check passes.**
...
- **HE-T-UNDET's count.** §S9 Q5 (`.54`) reports "**10** nonnegative solutions (all class sizes multiples of D′e_sf_s = 2, summing to D′μ = 6)". Recompute: solutions `(n₁,n₂,n₃)` with each a nonnegative multiple of 2 summing to 6, i.e. `(a,b,c)` nonnegative integers with `a+b+c = 3` (after dividing by 2): the count is `C(3+2, 2) = C(5,2) = 10` ✓ **exactly 10** ✓ and the truth `(2,2,2)` corresponds to `(1,1,1)` ✓. The examples given — `(6,0,0), (4,2,0), (2,4,0)` — are `(3,0,0), (2,1,0), (1,2,0)` ✓ all valid ✓. **The tooth's number is exactly right.**

**RESOLUTION TRACE:** statement 382–444 · qualifications 757–776 (`.27`'s char repair), 892–935 (`.29`'s (SEP)), 1691–1734 (`.68`, R8-2) · tooth 1448–1455 (`.54`).

XREF: `GENHN_PROOF_2026-08-08.md:THEOREM GENHN.C` — count **11**.
XREF: `HE3_PROOF_2026-08-08.md:HE-T-UNDET` — self-count **3**.

**TEETH:** **HE-T-UNDET — "fired, and it is the mechanism certificate"** (`.54`, §S9 Q5): the μ = 2 method is not merely unproven at μ = 3 but "**information-theoretically insufficient**", with the 10 spurious solutions exhibited. **Disposition: planted mutant / measurement; fired.** This is the strongest possible justification for the note's existence and it is machine-certified.

---

### EFF.HE3.20  [lemma]  — LEMMA HE3-1 (the resultant identity at every test key)
...
### EFF.HE3.21  [lemma]  — LEMMA HE3-1L (the letter lift at level 1)
...
### EFF.HE3.22  [lemma]  — LEMMA HE3-1′ (the evaluation display)
...
**RESOLUTION TRACE:** statement 548–559 · proof 561–586 · `[r2]` correction 576–582 · Remark 588–642 (`.23`).
...
### EFF.HE3.23  [scope-record]  — the `[r2, F1]` twist Remark (what the twist does and does not do)
...
**SUPERSESSION KIND: replacement** (a false sentence struck and replaced). `TARGETS: .23` itself; the two bullets pin `.22` and `.24`.
...
*Arithmetic audit (rule 22) of the non-affinity instance, computed fresh.* `(e₁, h) = (2,1)`, `λ = 3`, `κ₀ = 9`, `u = λℓ`… with `λ = 3` an integer, `ℓ = 1` and `u = 3`. Line heights at `k = 0..3`: `κ₀ − ku = 9, 6, 3, 0`. `i₀(m)` solves `i·h ≡ m (mod 2)`, i.e. `i ≡ m (mod 2)`, `0 ≤ i < 2`: `i₀(9) = 1`, `i₀(6) = 0`, `i₀(3) = 1`, `i₀(0) = 0`. Also `i₀(u) = i₀(3) = 1`, `i₀(κ₀) = i₀(9) = 1`. Then `W(k) = (i₀(κ₀−ku) + k·i₀(u) − i₀(κ₀))/e₁`:
- `k=0`: `(1 + 0 − 1)/2 = 0` ✓
- `k=1`: `(0 + 1 − 1)/2 = 0` ✓
- `k=2`: `(1 + 2 − 1)/2 = 1` ✓
- `k=3`: `(0 + 3 − 1)/2 = 1` ✓
...
### EFF.HE3.24  [run-record]  — THE WITNESS (PE2's adjudicated member, re-derived)
...
**TEETH:** **Planted mutant (naive vs coherent), FIRED, PARI-adjudicated**, with the above-λ length measured at all 8 letters of `F₉^×` (all 0, refuting the phantom refine). **Re-fired at three further geographies at PE3** (`.64`). **Disposition: planted mutant; fired four times.**

---

### EFF.HE3.25  [lemma]  — LEMMA HE3-0 (the label of a root)

**CANONICAL STATEMENT:** verbatim, lines 648–671 (three clauses, with two in-place correction brackets).
...
(c) **The proof sketch's provenance is corrected by R8-5** (`.71`): the sentence "(i)/(ii): the SLOT LEMMA (GENHN-2) is stated for any ξ with (T1)/(T2)" is **stale** — GENHN-2 is stated at roots θ only, as `.03`'s `[r1, F4]` already established. R8-5 re-points it to HE6-0″. `SUPERSESSION KIND: provenance-rider`. **The A5 append's own words: "This is a provenance correction only: the required ξ-form lemma was already consumed at §S0 [r1, F4], but the HE3-0 proof sentence had not been updated to cite it."** So the r1 round fixed the import and missed one downstream sentence; A5 found it.
(d) **The definition of `S_r` that consumes this clause is separately repaired by R8-4** (`.70`), which installs the same piecewise `β_ρ` into LEMMA HE3-4's display — "this correction propagates it to the one display the r1 round missed".
...
**RESOLUTION TRACE:** statement 648–671 · proof 673–682 · correction sites 1774–1793 (`.70`, R8-4), 1795–1819 (`.71`, R8-5).
...
### EFF.HE3.26  [lemma]  — COROLLARY HE3-0′ (the local forcing)
...
### EFF.HE3.27  [lemma]  — LEMMA HE3-2 (recentering: valuations and residuals)
...
### EFF.HE3.28  [lemma]  — COROLLARY HE3-2′ (separability preserved along stage-α chains)
...
**TEETH:** Indirect — the termination claim is exercised by every decided leaf in the enumeration (72,602 reader classifications, §S9).

---

### EFF.HE3.29  [lemma]  — LEMMA HE3-3 (the slope counts, Ψ-inversion) with (SEP)
...
### EFF.HE3.30  [lemma]  — the `[r1-N1]` consecutive-triple supplement
...
**RESOLUTION TRACE:** statement 937–961 · machine record 1520–1535 (§S9′, `[r2] F-5` scope disclosure) · independent cover: `HE6_PROOF` LEMMA HE6-3.
...
### EFF.HE3.31  [scope-record]  — the integer-sampling Remark and the corrected recipe
...
**TEETH:** **HE-PSI verifies (★) directly** — “the support-function identity (★) holds at every sampled κ on every in-window polygon”, 0 violations over 43,528 checks (`.51`, Q2). **Disposition: executable regression.**

---

### EFF.HE3.32  [lemma]  — LEMMA HE3-4 (the residue peel)
...
**CONDITIONALITY:** **The displayed definition of `S_r` is ILL-DEFINED at `ℓ ≥ 2` and is replaced by a dated append.** R8-4 (1774–1793, `.70`):
...
`SUPERSESSION KIND: replacement` (the definition). **CHAIN: the frozen `res(Φ′(ρ)/n(λ)(ρ))` display → R8-4's piecewise `β_ρ`. TERMINAL: R8-4 (`.70`).** The A5 verification note (1843–1847, `.72`) states exactly why the r1 round missed it: the same repair was installed at HE3-0(iii)/HE3-0′ (`.25`, `.26`) but “this correction propagates it to the one display the r1 round missed”.
...
**RESOLUTION TRACE:** statement 983–989 · proof 991–1020 · μ ≤ 3 scope 1022–1058 (`.33`) · boxed reductions 1060–1086 · consistency check 1088–1090 · correction sites 1691–1734 (R8-2, `.68`), 1774–1793 (R8-4, `.70`).
...
### EFF.HE3.33  [scope-record]  — μ ≤ 3 is base-change-free; the two reductions and their boxing
...
**CONDITIONALITY:** **“Both branches … BOXED” is SUPERSEDED — R8-2 proves branch (i).** From 1691–1734 (`.68`):
...
`SUPERSESSION KIND: license` + **inventory completion** (the box shrinks). **CHAIN: §S1/HE3-BOX-6's “ONE branch” → §S6.2's “BOTH … BOXED” (internal contradiction) → R8-2's LEMMA HE3-4U → “branch (ii) alone”. TERMINAL: R8-2 (`.68`), awaiting sol discharge-confirmation.**
**The A5 verification note names the contradiction explicitly** (1830–1833, `.72`): “§S1's branch-(ii)-only reservation (L350–353) vs §S6.2's "BOTH are stated too cheaply" (L1049) confirmed as the internal contradiction”.
...
**RESOLUTION TRACE:** statement 1022–1086 · consistency check 1088–1090 · correction site 1691–1734 (R8-2, `.68`).
...
### EFF.HE3.34  [theorem]  — §S6.3, the proof of THEOREM HE3.A (four steps)
...
**RESOLUTION TRACE:** statement (the theorem) 312–331 · proof 1092–1205 · Galois lemma 1118–1140 (`.35`) · correction sites 1691–1734 (R8-2, `.68`, at the peel it consumes), 1774–1793 (R8-4, `.70`, at `S_r`'s definition).
...
### EFF.HE3.35  [lemma]  — LEMMA HE3-6 (Galois equivariance of labels)
...
**It is consumed a second time, out of range**, by R8-2's LEMMA HE3-4U (`.68`): “LEMMA HE3-6 shows that these conjugate componentwise classes assemble to the original Galois-stable label class” — so the equivariance lemma is what makes the unramified split-sum repair legitimate.
Untouched by `[r2]` and by the A5 wave (`.66`–`.72`, verified).
...
**RESOLUTION TRACE:** statement 1118–1121 · proof 1123–1140 · consumed at 1102–1108 (`.34`'s base step) and 1724–1728 (R8-2, `.68`).
...
### EFF.HE3.36  [theorem]  — §S6.4, the proof of COROLLARY HE3.B and the μ = 2 cross-check
...
- **μ = 3's five types.** `Σ e_sf_s = 3`: `{(3,1)}, {(1,3)}, {(1,1),(2,1)}, {(1,1),(1,2)}, {(1,1),(1,1),(1,1)}` ✓ **five** ✓, matching §S9's Q3 table (`.52`) row for row ✓.
...
**RESOLUTION TRACE:** statement 1207–1226 · the induction 349–367 (`.17`) · GENHN.C's dictionary `GENHN_PROOF` 259–273 · the machine confirmation 1420–1432 (`.52`, Q3).

XREF: `GENHN_PROOF_2026-08-08.md:THEOREM GENHN.C` — count **11**.
XREF: `GENIND_PROOF_2026-08-08.md:GENIND.A(IV)` — count **5**.

**TEETH:** **Q3's 947 PARI jobs realize and confirm ALL FIVE μ = 3 types**, including “**25 sextics that split into three ramified quadratics, each PARI-confirmed** — the first machine confirmation of a 3-label stage σ anywhere in this program” (`.52`). **Disposition: executable regression against an independent oracle; the cross-check itself is `foreign-note evidence` consumed as decorrelation.**

---

### EFF.HE3.37  [lemma]  — LEMMA HE3-5 (the σ-resolution boundary)
...
**CONDITIONALITY:** **The lemma's two clauses stand; its three bullets are REPLACED by the R8-1 dated correction, and the note's introductory “has an exact answer” is re-read.** From 1634–1689 (`.67`):
...
**Every step reproduces, including the A5 verification note's own hand-check (1826–1830, `.72`), which this compiler independently confirms.**

**RESOLUTION TRACE:** statement 1238–1258 · proof 1260–1265 · preamble 1231–1236 · correction 1634–1689 (R8-1, `.67`).

XREF: `GENHN_PROOF_2026-08-08.md:GENHN-3(a)` — count **11**.

**TEETH:** **HE-T-CAP — “added mid-run, the strongest tooth”** (`.54`, Q5): “the earlier reader variant that dropped out-of-window recentered digits disagrees with the sound reader on 17 (p = 5) and 43 (p = 7) members — **the soundness bug of LEMMA HE3-5 is real and is caught**.” **Disposition: planted mutant, fired, and the mutant is the note's own earlier reader.** **HE-BND's two-step table is RE-SCOPED by R8-1** to “evidence for the selected aligned rows only” — `stale self-description` on the general law.

---

### EFF.HE3.38  [scope-record]  — the Interpretation (the honest reading of the undecided mass)
...
(a) **“PE3 is next” and “Attempt grade 0/2”** — PE3 ran CLEAN and moved the counter to **1/2** (`.64`). SERIES `HE3-GRADE-ARC`: member 1 = the title/status block (`.02`, 0/2); member 2 = the `[r2]` record (`.07`, 0/2); **member 3 = this box** (0/2); **member 4 = the post-PE3 append (`.64`, 1/2 — TERMINAL)**. **The arc stops at 1/2; HE3 is NOT accepted.**
...
**RESOLUTION TRACE:** statement 1278–1294 · prior members 3–20, 120–139 · terminal member 1579–1583 (`.64`) · supplier movement `GENHN_PROOF` 1941–1955.
...
### EFF.HE3.40  [scope-record]  — HE3-BOX-2 (the embedded-genre certification perimeter)
...
**CONDITIONALITY:** A clean inheritance with an explicit fence (“a statement about window data, not about THEOREM HE3.A”). **Its supplier is GENHN's S11.F, which is derived rather than merely observed** (`spec/EFF-GENHN-s2of2.md` `.20`), so the inheritance is of a proved perimeter. Untouched by every append (verified). **One gap:** the box says “The battery gates its σ legs at that perimeter **on embedded rows**”, but **HE3's battery has no embedded rows** — `.41` lists only full-side genres, and HE3-BOX-4b (`.43`) records “no embedded n = 7 row”. So the gating clause is **vacuous for this note's own instrument.** Recorded as source defect 9.

**DERIVATION:** Inherited; derived at GENHN S11.F.

**RESOLUTION TRACE:** statement 1295–1302 · supplier `GENHN_PROOF` 1352–1377, 1777–1798 · the vacuity 1321–1333 (`.43`).

XREF: `GENHN_PROOF_2026-08-08.md:GENHN-BOX-4` — count **3** · `S11.F` — count **10**.

**TEETH:** **`signed vacuity disclosure`** — the tooth the box names (HE-T-CERT, the embedded uncertified tail) is **OWED and never wired** (`.43`). Disposition: inherited perimeter, unexercised here.

---

### EFF.HE3.41  [scope-record]  — HE3-BOX-3 (machine coverage)
...
**CONDITIONALITY:** **This box is the reason every f₁ ≥ 2 repair in this note can say “invisible to the sealed battery”**, and it is the coverage-hole sentence quoted at nine PROOF-ONLY rows across the merged inventory. **Partially closed post-seal by additive legs**: `he3r1_supp.py` P2/P3 run `f₁ ∈ {2,3}` sweeps (`.59`, `.61`) and `he3_pe2fresh.py` runs the first `e₁ ≥ 2 ∧ f₁ ≥ 2` geography — **so the arc's coverage exceeds the sealed battery's, and the box's own sentence stays true about the sealed rows.** `SUPERSESSION KIND: as-of scoping`. **μ ≥ 4 remains unexercised by anything.**
**One claim is FALSE as stated at f₁ ≥ 2**: “The PROOFS are uniform in all of these” — at `μ ≥ 4` the ramified branch (ii) is boxed, not proved, so the proofs are *not* uniform in μ. The box's own next clause names branch (ii) as unexercised, so the two sentences sit in tension. Recorded as source defect 10.
...
**RESOLUTION TRACE:** statement 1303–1312 · additive legs 1490–1573 (`.57`–`.63`) · the μ ≥ 4 box 1334–1368 (`.44`).
...
### EFF.HE3.42  [scope-record]  — HE3-BOX-4 (characteristic coverage of the machine leg)
...
**CONDITIONALITY:** **An honest asymmetry disclosure, and it is corpus-wide** (“an oracle asymmetry every prior unit in this program inherits”) — GENHN's own §S11.C makes the same disclosure (“ORACLE: PARI legs on all Zp sinks”). **The proof-side claim is qualified by `.27`'s PE1-F4 repair**: the characteristic-freedom is at the *residue* level, since binomial coefficients do appear in the recentering expansion. That qualification is not repeated here. Recorded as source defect 11.
...
**RESOLUTION TRACE:** statement 1313–1320 · the proof-side qualification 757–776 (`.27`) · §S9's ℤ_p-only rows 1390–1395 (`.47`).
...
### EFF.HE3.43  [instrument-record]  — HE3-BOX-4b (SEALED-VS-DELIVERED)
...
**Net wired teeth that ARE teeth: THREE** (UNDET, SIG, CAP). §S9's Q5 (`.54`) nonetheless reports “all wired teeth fired”, which is true of the four as wired ✓ but a reader counting teeth must use three. Recorded.
**The exculpating clause is honest and checkable**: “Nothing in §S3–S6 depends on the missing rows” ✓ — the missing rows are coverage, and the shard-1-homed proofs cite no machine result as a premise ✓ verified.
...
**RESOLUTION TRACE:** statement 1321–1333 · Q5's scoring 1448–1461 (`.54`) · CAP's origin 1231–1236 (`.37`'s preamble).
...
### EFF.HE3.44  [scope-record]  — HE3-BOX-6 (THE ONE SURVIVING GAP)
...
**(3) R8-3 supplies the box's own missing derivation.** The box writes “i.e. degree n ≥ e₁f₁μ ≥ 8” — which needs `D′ ≥ 2`, undisplayed. R8-3 (`.69`) proves the `D′ = 1` case directly (LEMMA HE3-4D1) and concludes: “The unresolved ramified base-change branch can occur only after excluding this direct case, so there D′ ≥ 2; together with μ ≥ 4 this gives n = D′μ ≥ 2·4 = 8. **Therefore the advertised unconditional range n ≤ 7 survives, now with the missing derivation displayed.**”

**(4) The box is therefore STRICTLY NARROWER at HEAD than as displayed**, and its `n ≤ 7` consequence is *better* supported, not worse.

**CHAIN (rule 25):** box as displayed (“ONE gap”, contradicted by §S6.2's “BOTH”) → **R8-2** (`.68`; branch (i) proved; “branch (ii) alone is BOXED”) → **R8-3** (`.69`; `D′ ≥ 2` supplied; `n ≤ 7` survives). **TERMINAL: branch (ii) alone, at `μ ≥ 4` and `D′ ≥ 2`, i.e. `n ≥ 8` — awaiting sol discharge-confirmation** (1851–1853).
...
(a) **The mechanism of the gap is sharp and self-diagnosed**: “no element of R has non-integer dv … so NO test key of the form Φ′ + w, w ∈ R, can sit at such a side's height”. This is the same fact `.10` records about `R` being an order.
...
**RESOLUTION TRACE:** statement 1334–1368 · the contradicting §S6.2 verdict 1057–1058 (`.33`) · corrections 1691–1734 (R8-2, `.68`, the charge's L1694), 1736–1772 (R8-3, `.69`) · verification 1830–1841 (`.72`).

XREF: `GENHN_PROOF_2026-08-08.md:[GENHN-HE` — count **19**.
XREF: `HE3_PROOF_2026-08-08.md:HE3-BOX-6` — self-count **12**.
XREF: `HE7_PROOF_2026-08-08.md:HE3-BOX-6` — **count 1, CORRECTED at this merge** (both shard 2's own XREF row and this merge's assignment brief recorded **12**; fresh `grep -cF 'HE3-BOX-6' lean/notes/openmath/HE7_PROOF_2026-08-08.md` at merge time returns **1**, at line 1263 — "this is the step whose absence was HE3-BOX-6's original defect". The **12** is HE3's own SELF-count of the designation within its own note (`grep -c 'HE3-BOX-6' HE3_PROOF_2026-08-08.md` = 12, confirmed), evidently conflated with the reverse-direction count at some point upstream. **The edge itself is real and independently confirmed** — HE7 consumes this box, exactly once, and `spec/EFF-HE7.md` `.37` (HE7's own merged numbering) is built on that single occurrence — only the citation *count* was wrong. See source defect 16 and §4's sample re-verification. **Reverse edge, carried explicitly at §5 below, with the corrected count.**)

**TEETH:** **PROOF-ONLY and entirely unexercised.** HE3-BOX-3 (`.41`): “NOT exercised by machine: … μ ≥ 4 (**where LEMMA HE3-4's ramified base-change branch and the stage-CS recursion first bite**)”. The nearest machine contact is `he3r1_supp.py` P2's “an ℓ = 2 member (λ = 7/2, block (2,2))” (`.59`) — but that member has `d = 1`, i.e. it is the *reachable* half, not branch (ii) ✓ (`ℓd = 2 ≤ 3`, so `d = 1`). **Disposition: PROOF-ONLY; the gap is self-found (`decorrelated-model audit` is not needed — the note found it “before any hostile pass”), and its narrowing is `decorrelated-model audit` (sol A5).**

---

### EFF.HE3.45  [scope-record]  — HE3-BOX-5 (what a hostile pass should attack first)
...
- **(1) was attacked and PARTIALLY RESOLVED**: R8-2 (`.68`) proved the *unramified* half; the ramified branch (ii) stands ✓ **the priority was correct.**
...
- **(3) was attacked at PE1 (finding 3) and closed by (SEP) + the N1 supplement** ✓ (`.29`, `.30`).
- **(4) REMAINS OPEN** — and R8-1 (`.67`) makes it sharper, not softer, by showing HE3-5 is recursive. See OPEN-CALL 4.
...
**RESOLUTION TRACE:** statement 1369–1379 · outcomes: (1)/(2) at 1691–1734 (`.68`); (3) at 892–961; (4) open, sharpened at 1634–1689 (`.67`).

**TEETH:** NONE (a preregistration about future passes, not about data). **Disposition: `preregistration` (rule 19) — and this unit scores it above, which is what the rule's pairing asks for.**

---

### EFF.HE3.46  [instrument-record]  — §S8's instrument
...
**CONDITIONALITY:** **The “no twist bookkeeping” design is what makes the battery immune to the `[r2]` F-1 coherent/naive defect** — `.24` says so: “the battery detects residual roots by measuring dv(B₀), which consults the coherent object automatically (the measurement IS LEMMA HE3-1′'s evaluation identity, whose right side is the coherent R_λ(s))”. **That immunity is a design property, not luck, and it is worth preserving.**
**Two dependencies:** the reader implements HE3-1′/HE3-2, so **it cannot independently test them** — it tests the *dictionary* they feed. And “enumerated in STAGE COORDINATES (legitimate by LEMMA GENHN-1's digit bijection)” means the enumeration's legitimacy rides GENHN-1, whose node display HE3 itself corrects (`.11`). **At f₁ = 1 the two nodes coincide, so the enumeration is sound on every row** ✓ — the note says so at `.11`'s LOCUS SCOPING.

**DERIVATION:** Instrument description.

**RESOLUTION TRACE:** statement 1381–1388 · immunity 638–642 (`.24`) · node soundness 236–237 (`.11`).

XREF: `GENHN_PROOF_2026-08-08.md:LEMMA GENHN-1` — count **4**.
ARTIFACT: `verification/openmath/he3_checks.py` ✓ present.

**TEETH:** the instrument itself; the census tie (`.53`) is its decorrelation.

---

### EFF.HE3.47  [instrument-record]  — §S8's rows
...
**CONDITIONALITY:** All rows are `f₁ = 1` and `d = 1` — the coverage bound of `.41`. **No embedded row** (`.43`).

**DERIVATION:** Row list.

**RESOLUTION TRACE:** statement 1390–1395 · census tie 1434–1446 (`.53`) · boundary 1463–1477 (`.55`).

**TEETH:** the rows are the substrate. `signed vacuity disclosure` per `.41`.

---

### EFF.HE3.48  [instrument-record]  — §S8's legs
...
**CONDITIONALITY:** **HE-SIG reuses W-12's pinned oracle question** (“the pinned w12_checks question”), which is a genuine decorrelation: the σ-oracle is not written for this note. `grep -cF 'w12_checks' W12_PROOF_2026-08-08.md` = **8** ✓ verified. **HE-NORM uses exact integer resultants (Bareiss/Sylvester), no sympy** — and the `[r2]` F-4 fix corrected a docstring that wrongly said sympy (`.62`).

**DERIVATION:** Leg description.

**RESOLUTION TRACE:** statement 1397–1402 · scored 1412–1477 (`.50`–`.55`).

XREF: `W12_PROOF_2026-08-08.md:w12_checks` — grep-verified count **8**.

**TEETH:** the four legs; inverse table at §6.

---

### EFF.HE3.49  [run-record]  — §S9's verdict headline and totals
...
**CONDITIONALITY:**
(a) **“all wired teeth fired” is true of the four as wired, but one of the four is mis-specified** (`.43`). A reader counting genuine teeth must use three.
(b) **The artifact hashes are NOT in the note** — “hashes in the commit message”. **UNPINNED from the note's own text** (rule 23), unlike `he3r1_supp.py`'s three full md5s. Recorded as source defect 12.
(c) **Instrument grade only** — `.56` says so explicitly.
...
**RESOLUTION TRACE:** statement 1404–1410 · Q-blocks 1412–1477 · machine-leg scope 1479–1488 (`.56`).

ARTIFACT: `verification/openmath/he3_checks_output.txt` ✓ · `he3_checks_results.json` ✓.

**TEETH:** this unit is the run-record half; the leg descriptions (`.48`) are the other half.

---

### EFF.HE3.50  [run-record]  — Q1 (HE-NORM)
...
**CONDITIONALITY:** **This is the strongest machine result for `.20`** — the resultant identity holds exhaustively on every enumerated member. The claim “The engine of the proof is machine-real” is warranted: HE3-1 is the identity every other lemma rides. **Scope:** `f₁ = 1` rows only (`.41`), so the `[r1, F2]` band repair is invisible here — that repair's evidence is PE1's PARI-exact counter-instances (`.20`).

**DERIVATION:** Run record.

**RESOLUTION TRACE:** statement 1412–1414 · the lemma 450–490 (`.20`).

**TEETH:** **Disposition: executable regression, exhaustive on the enumerated rows.** Exactness method: “exact Bareiss/Sylvester integer resultants” (`.48`) — no floating point, no sympy.

---

### EFF.HE3.51  [run-record]  — Q2 (HE-PSI)
...
**RESOLUTION TRACE:** statement 1416–1418 · the lemma 548–586 (`.22`) · (★) 846–857 (`.31`).
...
### EFF.HE3.52  [run-record]  — Q3 (HE-SIG) and the five μ = 3 stage types
...
- **The five types are exhaustive for `Σ e_sf_s = 3`** ✓ (audited at `.19`): `{(3,1)}, {(1,3)}, {(1,1),(2,1)}, {(1,1),(1,2)}, {(1,1)³}` ✓ **exactly five, no omissions.**
...
**CONDITIONALITY:** **The strongest positive result in the note**, and it is scoped: `f₁ = 1`, `e₁ = 2`, ℤ_p only. The 3-label type is the one “with NO μ = 2 analogue”, i.e. the exact configuration `.19` shows the μ = 2 method cannot decide.

**DERIVATION:** Run record against the HE3.A dictionary.

**RESOLUTION TRACE:** statement 1420–1432 · the dictionary 312–331 (`.16`) · the cross-check 1207–1226 (`.36`) · the underdetermination it answers 414–422 (`.19`).

XREF: `GENHN_PROOF_2026-08-08.md:THEOREM GENHN.C` — count **11** (the μ = 2 dictionary the μ = 3 table extends).

**TEETH:** **Disposition: executable regression against an independent oracle (PARI factorpadic-200 + idealprimedec, the pinned W-12 question).** Planted mutant **HE-T-SIG**: “6/6 flipped dictionaries caught by PARI on each of the two rows” (`.54`).

---

### EFF.HE3.53  [run-record]  — Q4, the census tie
...
- **The 72,134 figure.** `39,366 + 32,768 = 72,134` ✓ **exactly the enumerated-member total at `.49`** ✓ — **so Group A alone accounts for every enumerated member, and Group B's 468 constructed members are the extra reader classifications** ✓ confirming `.49`'s reconciliation independently.
...
**RESOLUTION TRACE:** statement 1434–1446 · GENHN's side `GENHN_PROOF` 1827–1830 · the enumeration's legitimacy 1386–1388 (`.46`).
...
### EFF.HE3.54  [run-record]  — Q5 (teeth)
...
- **Why both identities are satisfied by all ten.** Identity 1: `Σn_i = 6` ✓ by construction. Identity 2 at a single slope `λ`: `Σn_iλ = λ·6` ✓ **independent of the split** ✓ — exactly `.19`'s degeneracy ✓.
- **HE-T-CAP's disagreement counts.** 17 (p = 5) and 43 (p = 7). Cross-check against `.55`'s never-decided residuals (28 and 51) and against the N = 5→6 increments (`96 − 84 = 12` at p = 5; `291 − 258 = 33` at p = 7): neither matches 17/43 exactly. **No cross-check is available; recorded as unverifiable-but-unclaimed.** The tooth's *nature* is what matters: the mutant is the note's own earlier reader.
- **Teeth accounting.** Fired: UNDET, SIG, CAP, NODE = 4 ✓ matching `.43`'s “FOUR teeth wired”; genuine: 3 ✓; owed: PSI, CERT = 2 ✓.
**The one derivable number (10) reproduces exactly.**

**CONDITIONALITY:** **HE-T-UNDET is not a regression test — it is a proof-of-necessity**, and it is the single strongest justification for the whole note. **HE-T-CAP is the strongest tooth and it caught a real soundness bug in the note's own earlier reader.** **HE-T-NODE is disclosed mis-specified** (`.43`); **two teeth are owed.** The Q5 verdict phrase “all wired teeth fired” at `.49` must be read with `.43`'s accounting.

**DERIVATION:** Run record.

**RESOLUTION TRACE:** statement 1448–1461 · the underdetermination 399–422 (`.19`) · the soundness bug 1231–1236 (`.37`'s preamble) · the accounting 1321–1333 (`.43`).
...
### EFF.HE3.55  [run-record]  — HE-BND (the boundary law)
...
**CONDITIONALITY:** **“PREDICTION CONFIRMED” is RE-SCOPED by R8-1 (`.67`)** (1686–1689):
...
**CONDITIONALITY:**
(a) **`STALE-SELF-DESCRIPTION` twice**: “attempt 0/2” (now 1/2) and “**no hostile arc has run**” — three passes ran (PE1, PE2, PE3). **This sentence dates from before the arc**, and the surrounding `[r1]`/`[r2]` text was updated while it was not. Recorded as source defect 13.
(b) **Item 4 of the list is RE-SCOPED by R8-1** — “the σ-resolution boundary law of LEMMA HE3-5 with its predicted two-step” is exactly what R8-1 withdraws as a general law (`.55`, `.67`).
...
**DERIVATION:** Summary of `.50`–`.55`.

**RESOLUTION TRACE:** statement 1479–1488 · grade 1579–1583 (`.64`) · re-scoping 1686–1689 (`.67`).

**TEETH:** NONE (summary).

---

### EFF.HE3.57  [run-record]  — §S9′'s verdict and the runner pins
...
**RESOLUTION TRACE:** statement 1490–1500 · the F-4 finding 137 (`.07`) · the legs 1502–1562 (`.58`–`.62`).

ARTIFACT: `verification/openmath/he3r1_supp.py` ✓ · `he3r1_supp_output.txt` ✓ · `he3r1_supp_results.json` ✓ — 3/3 present.
**Full md5s: 3 PINNED** (`93681e8d…`, `9cd132a8…`, `65ed06e4…`), in contrast to §S8's UNPINNED-from-the-note hashes (`.49`).

**TEETH:** the leg itself; P1–P4 at `.58`–`.62`.

---

### EFF.HE3.58  [run-record]  — §S9′ P1 (the repaired peel at PE1's counter-instance)
...
**CONDITIONALITY:** **The decisive machine evidence for the `[r1] F1` CRITICAL repair (LEMMA HE3-1L).** Two things it establishes that nothing else does: the two `K∖F₃` letters are *carriable* (the old display could not reach them), and the spurious splits PE1 exhibited are *excluded by measurement*, not by argument.
...
**RESOLUTION TRACE:** statement 1502–1511 · the lemma 498–546 (`.21`) · the peel 991–1020 (`.32`) · PE1's finding 100 (`.05`).
...
### EFF.HE3.59  [run-record]  — §S9′ P2 (the f₁ = 2 sweep)
...
**CONDITIONALITY:** **The sweep that gives the `f₁ = 2` repairs their machine contact**, and it is the same frame `Φ′ = x²+9` that `.10` uses for the non-DVR counter-instance and `.20` for the band counter-instances — **so one frame carries three distinct findings** ✓.
**Coverage note:** `e₁ = 1` throughout (`Φ′ = x²+9` at `(e₁,f₁,h) = (1,2,1)`), which is exactly the geography where the `[r2]` F-1 twist seam is INVISIBLE (`W ≡ 0` at `e₁ = 1`, `.14`). **So P2 could not have caught the coherent/naive defect** — and did not. The `[r2]` coverage note (`.63`) says so.

**DERIVATION:** Run record.

*Arithmetic audit (rule 22).* Member count: 3 (rational configs) + 1 + 1 + 1 + 1 + 1 = **8** ✓ **matching “8/8 members”** ✓. Frame: `Φ′ = x²+9` over ℤ₃ ⟹ `e₁ = 1, f₁ = 2, D′ = 2, h = 1` ✓ (audited at `.10`). σ checks: `(1,2)+(1,4)` sums to `2 + 4 = 6` ✓; `(1,6)` → 6 ✓; `(2,2)` block → 4, and with `μ`… the ℓ = 2 member at `λ = 7/2` has `ℓ = 2`, so `e_s = 2`, and `(e₁e_s, f₁f_s) = (2, 2)` ✓ **matching “block (2,2)”**, degree `4` ✓; mixed `(1,2)+(2,2)` sums to `2 + 4 = 6` ✓. **All degree-consistent.** The three-consecutive-slope member has slopes `3,4,5` ✓ with `D′h = 2`, so all three exceed the band floor ✓ legal.

**RESOLUTION TRACE:** statement 1512–1519 · the geographies it covers: 149–163 (`.10`), 450–478 (`.20`), 937–961 (`.30`), 648–671 (`.25`) · the coverage note 1564–1573 (`.63`).
...
### EFF.HE3.60  [run-record]  — §S9′ P2-N1 (the supplement's two branches) with its `[r2]` geography disclosure
...
**The FR-D frame is worth noting**: `Φ′ = x − 2` over ℤ₂ gives `D′ = 1`, i.e. `e₁ = f₁ = 1` — **the `D′ = 1` case R8-3 later proves directly** (`.69`). So the N1 supplement's live-geography evidence sits in exactly the configuration whose general theory was the last A5 gap.
...
**RESOLUTION TRACE:** statement 1520–1535 · the supplement 937–961 (`.30`) · the F-5 finding 138 (`.07`).
...
### EFF.HE3.61  [run-record]  — §S9′ P3 (the f₁ = 3 sweep and the 26-letter peel)
...
**RESOLUTION TRACE:** statement 1536–1542 · the lemma's fullness clause 508–513 (`.21`).
...
### EFF.HE3.62  [run-record]  — §S9′ P4 with its `[r2]` scope disclosure
...
**A second, unrelated disclosure is folded in**: the docstring's “sympy integer resultants” was a slip; the code is the no-sympy Sylvester/Bareiss route. That is the docstring-only change (F2)'s exception accounts for (`.57`).
...
**RESOLUTION TRACE:** statement 1543–1562 · the lemma 498–534 (`.21`) · the F-4 finding 137 (`.07`) · the runner pin 1494–1500 (`.57`).
...
### EFF.HE3.63  [scope-record]  — §S9′'s coverage note
...
**This is where the twist seam had to surface**: `i₀(κ) cycles` only at `e₁ ≥ 2`, and `W(k)` is built from `i₀` — so the first `e₁ ≥ 2` row is exactly the first row that could see the coherent/naive difference ✓ **and it did** (`.24`). The coverage note and the F-1 finding are the same event seen from two sides.

**DERIVATION:** Coverage record.

*Arithmetic audit (rule 22).* The PE2 frame `Φ′ = x⁴ + 9` over ℤ₃ gives `(e₁,f₁) = (2,2)` ✓ (audited at `.24`), `D′ = 4`, and a `μ = 3` member has degree `D′μ = 12` ✓ **matching “a μ = 3 degree-12 member”** ✓. `|F₉^×| = 8` for the peel profiles ✓.

**RESOLUTION TRACE:** statement 1564–1573 · the twist finding 253–286, 615–642 (`.14`, `.24`) · the F-1 row 134 (`.07`).
...
### EFF.HE3.64  [acceptance-box] · SERIES: HE3-GRADE-ARC, member 4 of 4 (TERMINAL)
...
**CONDITIONALITY:** **TERMINAL member of the HE3-GRADE-ARC SERIES: 1/2, NOT accepted.** Acceptance needs two consecutive clean passes; HE3 has one. **And five REAL findings landed after it** (`.65`–`.71`), none of which resets the counter but all of which are “awaiting sol discharge-confirmation”.
...
**One of the three geographies is `Q = 4` over `F₄[[t]]`** — which is exactly the case `.12`'s starved-corner parenthetical omits, and PE3's M-1 finding is about that omission ✓ **the pass that found the omission also ran the omitted case.**
...
**RESOLUTION TRACE:** statement 1577–1589 · prior members 3–20, 120–139, 1278–1294 · the M-1 correction 1591–1618 (`.65`) · the later A5 wave 1622–1853.
...
### EFF.HE3.65  [changes-record]  — M-1 (the §S1 (iii) starved-corner parenthetical)
...
**SUPERSESSION KIND: counter re-reading** (the list re-counted under two explicit bounds) + **scope-pin** (the correction bites only for a future consumer at `d_λ = μ`). `TARGETS: .12, .11(iii)`.
...
*Arithmetic audit (rule 22), both bullets independently recomputed.*
...
**RESOLUTION TRACE:** statement 1591–1618 · target 218–222 (`.12`) · PE3's Q = 4 leg 1584–1588 (`.64`).
...
### EFF.HE3.66  [changes-record]  — the A5 adjudication header (the R8 umbrella)
...
**The wave's own status:** “Ledger: HE3's row → 5 repairs folded, **awaiting sol discharge-confirmation**” (1851–1853). See OPEN-CALL 8.

*Arithmetic audit (rule 22).* Five findings ✓ and five dated corrections below (`.67`–`.71`) ✓ **one block per finding, no orphans** ✓. Of the five: R8-1 replacement, R8-2 license, R8-3 license, R8-4 replacement, R8-5 provenance-rider — **2 licenses + 2 replacements + 1 provenance-rider = 5** ✓.

**DERIVATION:** Adjudication framing.

**RESOLUTION TRACE:** statement 1622–1632 · the five blocks 1634–1819 · verification 1821–1853 (`.72`).
...
### EFF.HE3.67  [changes-record]  — R8-1 (HE3-5 is recursive, not a one-unit bound)
...
**SUPERSESSION KIND: replacement** (the three bullets and the introductory phrase) + **scope-pin** (the HE-BND table). `TARGETS: .37 (the bullets), .38 (the Interpretation's framing), .55 (the table's generality)`.
...
(i) **The recursive criterion is now the definition of DECIDED for read histories** — and it *agrees* with DEFINITION 2 (`.15`), whose condition (b) already quantified over every event. So R8-1 does not contradict DEFINITION 2; it corrects HE3-5's *bound*.
(ii) **HE3-BOX-5 item (4)'s open question is sharpened, not closed** (`.45`).
(iii) **The HE-BND table survives as a measurement and loses its generality** (`.55`).
...
- **Why char 5 and `f₁ = 2`?** `e₁ = 1` makes the normalizers π-powers with no wrap ✓ (the A5 verification note says so at `.72`'s 1826–1827 ✓), keeping the example clean. **This compiler's independent recomputation matches the verification note's hand-check line for line.**
**The counterexample is exact.**

**RESOLUTION TRACE:** statement 1634–1689 · targets 1238–1258, 1267–1274 (`.37`, `.38`), 1463–1477 (`.55`) · verification 1823–1830 (`.72`).
...
### EFF.HE3.68  [lemma]  — R8-2: LEMMA HE3-4U (the unramified split-sum)
...
**SUPERSESSION KIND: license** (a boxed branch becomes proved) + **inventory completion** (HE3-BOX-6's item list shrinks to one). `TARGETS: .33 (the "BOTH … BOXED" verdict), .44 (the box), .17 and .01 (the "ONE named branch" claim, now accurate)`.
...
**It consumes LEMMA HE3-6** (`.35`) to assemble the componentwise classes — so the r1 equivariance discharge is load-bearing here too.
**Its own status:** awaiting sol discharge-confirmation.
...
**Every step reproduces, and this compiler's recomputation matches the A5 verification note's own check (`.72`'s 1833–1837) line for line.**

**RESOLUTION TRACE:** statement 1691–1734 · targets 1022–1086 (`.33`), 1334–1368 (`.44`), 333–367 (`.17`) · verification 1830–1839 (`.72`).
...
**TEETH:** **PROOF-ONLY.** Branch (i) is reachable only at `μ ≥ 4` in the *boxed* sense (at `μ ≤ 3` the subtraction argument of `.33` handles non-rational factors without base change), and `μ ≥ 4` is unexercised by any row (HE3-BOX-3, `.41`). Coverage-hole sentence: “NOT exercised by machine: … μ ≥ 4”. **Disposition: `decorrelated-model audit` for the finding; PROOF-ONLY for the lemma.**

---

### EFF.HE3.69  [lemma]  — R8-3: LEMMA HE3-4D1 (the linear-key case)
...
**SUPERSESSION KIND: license** (an undisplayed hypothesis is supplied and the excluded case proved) + **inventory completion**. `TARGETS: .44 (HE3-BOX-6's "n ≥ 8" derivation), .17 and .01 (the "n ≤ 7" range)`.
...
**The `D′ = 1` case is not exotic** — PE2's FR-D leg runs at `Φ′ = x − 2` over ℤ₂, i.e. exactly `D′ = 1` (`.60`) ✓. So the case R8-3 closes had machine contact all along.
**Its own status:** awaiting sol discharge-confirmation.
...
- **The invariants.** A terminal side `λ = u/ℓ` with separable irreducible residual factor `r` of degree `d` produces a factor of degree `ℓd` ✓ (classical Newton/Hensel) ✓; its roots have `ℓ | e` and `d | f` ✓, and `ef = ℓd` ✓ forces `(e,f) = (ℓ,d)` exactly ✓ — **the same forcing pattern as `.34`** ✓.
...
**Every step reproduces, and this compiler independently identified the same gap at `.17`'s audit before reading this append.**

**RESOLUTION TRACE:** statement 1736–1772 · target 1349–1353 (`.44`), 353–355 (`.17`) · the `D′ = 1` machine contact 1530–1535 (`.60`) · verification 1839–1842 (`.72`).
...
### EFF.HE3.70  [changes-record]  — R8-4 (piecewise definition of ramified label classes)
...
**SUPERSESSION KIND: replacement** (the `S_r` display). `TARGETS: .32`.

**CONDITIONALITY:** **A propagation fix, not a new idea** — the A5 verification note says so (`.72`'s 1843–1847): the piecewise `β_ρ` is “confirmed IDENTICAL to the r1 F6 repair already installed in HE3-0(iii)/HE3-0′ … this correction **propagates it to the one display the r1 round missed**”. `SUPERSESSION KIND` is therefore replacement at *display* scope with no content change.
**The closing clause bounds the blast radius**: “All later occurrences S_{Z−s} lie in the integer-slope case and are unchanged” ✓ — verified: `.32`'s peel argument and `.34`'s stage-α step both use `S_{Z−s}` at integer slopes ✓.
**Its own status:** awaiting sol discharge-confirmation.

**DERIVATION:** The `dv = 0` checks are displayed.

*Arithmetic audit (rule 22).* `ℓ = 1`: `dv(y_ρ) = λ` and `dv(n(λ)(ρ)) = λ` ⟹ quotient `dv = 0` ✓. `ℓ ≥ 2`: `dv(y_ρ^ℓ) = ℓ·λ = ℓ·(u/ℓ) = u` ✓ and `dv(n(u)(ρ)) = u` ⟹ `dv = 0` ✓ **exactly the displayed check** ✓. **Both residues exist** ✓. Identity with the r1 F6 form: `.25`'s display is “β_ρ := res(y_ρ/n(λ)(ρ)) at ℓ = 1, β_ρ := res(y_ρ^ℓ/n(u)(ρ)) at ℓ ≥ 2” ✓ **character-identical** ✓ confirming the verification note's claim.

**RESOLUTION TRACE:** statement 1774–1793 · target 983–989 (`.32`) · the identical r1 repair 660–669 (`.25`) · verification 1842–1848 (`.72`).

**TEETH:** **PROOF-ONLY** — the `ℓ ≥ 2` case with `d ≥ 2` is branch (ii), unexercised; the `ℓ ≥ 2, d = 1` case has one machine contact (`he3r1_supp.py` P2's `λ = 7/2` member, `.59`) ✓. **Disposition: `decorrelated-model audit` for the finding.**

---

### EFF.HE3.71  [changes-record]  — R8-5 (HE3-0 provenance correction)
...
**SUPERSESSION KIND: provenance-rider** (the claim stands; its cited source is replaced). `TARGETS: .25`.

**CONDITIONALITY:** **A stale citation surviving a repair round, found by a fresh model 20 days later.** The `[r1] F4` round corrected the §S0 *import* (`.03`) but left this one downstream *proof sentence* citing GENHN-2 at points where GENHN-2 does not apply. **No mathematical content moves** — the note says so and it checks: HE6-0″ supplies exactly the consumed form ✓.
**This is the clearest instance in the sweep of a repair round's incomplete propagation**, and it is the same failure mode as R8-4 (`.70`, the r1 F6 repair not reaching one display).
**Its own status:** awaiting sol discharge-confirmation.

**DERIVATION:** Provenance substitution; the replacement text is HE6-0″'s statement.

*Arithmetic audit (rule 22).* The replacement's displayed identity `dv(A_j(ρ)) = min_i(e₁v(a_{j,i}) + ih) = dv(A_j(θ))` ✓ is exactly the slot formula of `.03`'s import, with `ξ = ρ` ✓ — and the equality with `dv(A_j(θ))` holds because the formula's right side depends only on the coefficients, not on the point ✓ **which is the whole content of the ξ-form** ✓.

**RESOLUTION TRACE:** statement 1795–1819 · target 673–682 (`.25`) · the import it should have cited 42–60 (`.03`) · verification 1848–1851 (`.72`).
...
### EFF.HE3.72  [instrument-record]  — the A5 verification parenthetical
...
**One line-number pin is checkable and checks out:** “the stale GENHN-2 sentence located (L677)” — HE3 line 677 is inside the HE3-0 proof sketch ✓ (`.25`'s DERIVATION spans 673–682) ✓. Likewise “HE3-5's three bullets located verbatim (L1249–1258)” ✓ (`.37` spans 1238–1258, bullets at 1249–1258) ✓ and “§S1's branch-(ii)-only reservation (L350–353)” ✓ (`.17` spans 333–367, the reservation at 349–355) ✓ **all three line pins land in the right spans.**
**One external dependency is disclosed**: “HE6-0″ (**as corrected at this campaign's A3 fold**) supplies exactly the consumed form” — so HE3's most load-bearing citation depends on an HE6-side correction from the same campaign. Recorded; not verified here (OPEN-CALL 10).

*Compiler cross-check (rule 22).* This unit independently recomputed **all five** of the verification claims: R8-1's counterexample (`.67`) ✓; R8-2's split-sum arithmetic (`.68`) ✓; R8-3's `D′ ≥ 2` gap (`.69`, and independently at `.17`'s audit before reading the append) ✓; R8-4's `dv = 0` checks and the identity with the r1 F6 form (`.70`) ✓; R8-5's slot identity (`.71`) ✓. **Five of five reproduced. No discrepancy between the parenthetical's claims and this compiler's independent recomputation.**
...
against the target note, plus one commit-pin check (`git cat-file -t`) and one
artifact-existence check, per rule 23. **61 raw XREF citations across the two
shards (38 from shard 1, 23 from shard 2 — matching each shard's own closing
line, not the header sentences inside each shard's own §3.1, which undercount
their own tables: shard 1's §3.1 opens "26 distinct designations" against a
38-row table, shard 2's opens "19 distinct designations" against a 23-row
table — both flagged as source defects below, not silently reconciled) reduce
to 46 distinct `target:designation` pairs; 15 duplicate instances (15 distinct
designations cited identically by both shards) are marked `**DEDUP**` and
removed as separate rows.** Every duplicate's count agrees exactly across both
shards — **zero count mismatches among the 15 dedup pairs** (checked row by
row below). **One designation's count, however, does NOT survive fresh
re-verification** — `HE7_PROOF_2026-08-08.md:HE3-BOX-6`, one of the two
designations this merge was specifically charged to carry forward as an
explicit reverse-XREF row. Shard 2 recorded **12**; this merge's fresh grep at
HEAD returns **1**. This is a genuine mismatch, not sharding noise, and it is
**not silently corrected without comment** — see the row below, the sample
re-verification table, and source defect 16.

`[TABLE — compiler ledger]`

| # | Target : designation | Count | Role | Source shard(s) | Dedup |
|---:|---|---:|---|---|---|
| 1 | `GENHN_PROOF_2026-08-08.md:THEOREM GENHN.CLASS` | 7 | The genre datum, consumed as the definition layer / the genre notation of `.41`'s coverage box | s1+s2 | **DEDUP** |
| 2 | `GENHN_PROOF_2026-08-08.md:LEMMA GENHN-1` | 4 | The Φ′-development and (pre-correction) the node / the digit bijection legitimising the stage-coordinate enumeration (`.46`) | s1+s2 | **DEDUP** |
| 3 | `GENHN_PROOF_2026-08-08.md:LEMMA GENHN-2` | 2 | THE SLOT LEMMA — at roots θ only; the ξ-form is HE6-0″ | s1 | — |
| 4 | `GENHN_PROOF_2026-08-08.md:LEMMA GENHN-3` | 3 | The window ledger; `GENHN-3(a)` (row 13) is the lift-stability clause | s1 | — |
| 5 | `GENHN_PROOF_2026-08-08.md:LEMMA GENHN-4` | 3 | Declared NOT consumed — the fence at `.04` / declared NOT consumed at `.39` | s1+s2 | **DEDUP** |
| 6 | `GENHN_PROOF_2026-08-08.md:THEOREM GENHN.C` | 11 | Consumed as a CROSS-CHECK only, re-derived at `.36` / the μ = 2 dictionary Q3's table (`.52`) extends | s1+s2 | **DEDUP** |
| 7 | `GENHN_PROOF_2026-08-08.md:GENHN.A(v)` | 6 | The (μ,K)-cluster read DEFINITION 2 instantiates | s1 | — |
| 8 | `GENHN_PROOF_2026-08-08.md:S11.F` | 10 | The extraction-certified perimeter, the machine-side gate / its derivation | s1+s2 | **DEDUP** |
| 9 | `GENHN_PROOF_2026-08-08.md:GENHN-BOX-4` | 3 | The same perimeter as a box; HE3-BOX-2 (`.40`) inherits it | s1+s2 | **DEDUP** |
| 10 | `GENHN_PROOF_2026-08-08.md:GENHN-BOX-2` | 5 | The W-9 weld face declared NOT inherited / absent from HE3's σ chain (`.39`) | s1+s2 | **DEDUP** |
| 11 | `GENHN_PROOF_2026-08-08.md:LEMMA GENHN-T(b)` | 11 | Cited at `.34`'s stage-CS step — the lemma GENHN later REFUTED (source defect 6) | s1 | — |
| 12 | `GENHN_PROOF_2026-08-08.md:THEOREM GENHN.B` | 7 | The clause list HE3.C discharges into | s1 | — |
| 13 | `GENHN_PROOF_2026-08-08.md:GENHN-3(a)` | 11 | Lift-stability iff dv < e₁N / consumed by HE3-5 (`.37`) and R8-1 (`.67`) | s1+s2 | **DEDUP** |
| 14 | `GENHN_PROOF_2026-08-08.md:ERRATUM 2026-08-08` | 1 | GENHN's `[r1]` node erratum, consumed at `.11`(iii) / the GENHN corner M-1 (`.65`)'s conservativity rests on | s1+s2 | **DEDUP** |
| 15 | `GENHN_PROOF_2026-08-08.md:ERRATUM E3` | 4 | The same carrier correction one level up (`.10`) | s1 | — |
| 16 | `GENHN_PROOF_2026-08-08.md:[GENHN-HE` | 19 | The box this note claims to close / the box HE3-BOX-6 (`.44`) scopes | s1+s2 | **DEDUP** |
| 17 | `GENHN_PROOF_2026-08-08.md:[GENHN-TOW-1]` | 19 | The tower box HE3.C's conditionality phrase under-describes | s1 | — |
| 18 | `GENIND_PROOF_2026-08-08.md:GENIND.A(I)` | 3 | The species partition | s1 | — |
| 19 | `GENIND_PROOF_2026-08-08.md:GENIND.A(IV)` | 5 | The descent engine — abscissa length ≥ 4 ⟹ μ ≥ 4 | s1 | — |
| 20 | `GENIND_PROOF_2026-08-08.md:LEMMA GENIND-3` | 6 | Sibling fibration for embedded genres; also consumed by R8-2 (`.68`) / the sibling splitting HE3-4U imports | s1+s2 | **DEDUP** |
| 21 | `GENIND_PROOF_2026-08-08.md:GENIND-H(n)` | 7 | The package HE3.C reports on | s1 | — |
| 22 | `HE6_PROOF_2026-08-08.md:HE6-0″` | 7 | The single most load-bearing citation — the ξ-form slot lemma / the ξ-form lemma R8-5 (`.71`) re-points to | s1+s2 | **DEDUP** |
| 23 | `HE6_PROOF_2026-08-08.md:LEMMA HE6-1L` | 4 | The mechanism HE3-1L back-propagates | s1 | — |
| 24 | `HE6_PROOF_2026-08-08.md:LEMMA HE6-3` | 12 | The (SEP)-free slope counts; independent cover for N1 / HE6's frozen battery, the foreign e₁ ≥ 2 contact | s1+s2 | **DEDUP** |
| 25 | `HE6_PROOF_2026-08-08.md:LEMMA HE6-5` | 5 | The template for HE3-6 | s1 | — |
| 26 | `HE6_PROOF_2026-08-08.md:HE6-0′` | 5 | The ℓ ≥ 2 residue repair form | s1 | — |
| 27 | `HE6_PROOF_2026-08-08.md:HE6-2″` | 2 | HE6's own re-proof of HE3-0(iii)'s ℓ ≥ 2 read (not consumed) | s1 | — |
| 28 | `HE6_PROOF_2026-08-08.md:HE3-0` | 9 | **Reverse edge: HE6 consumes HE3-0** | s1 | — |
| 29 | `HE6_PROOF_2026-08-08.md:DEFINITION 1` | 1 | **Reverse edge: the consumer pin** | s1 | — |
| 30 | `HE6_PROOF_2026-08-08.md:HE3-2(b)` | 1 | **Reverse edge: the above-λ clause pin** | s1 | — |
| 31 | `HE6_PROOF_2026-08-08.md:HE3.B` | 2 | **Reverse edge: the descent** | s1 | — |
| 32 | `HE6_PROOF_2026-08-08.md:HE3-1` | 1 | **Reverse edge: the resultant identity** | s1 | — |
| 33 | `HE7_PROOF_2026-08-08.md:HE3-3` | 2 | **Reverse edge: the (★) convex duality** | s1 | — |
| 34 | `GRTJC_PROOF_2026-08-08.md:JC-LOAD` | 31 | Declared NOT consumed (both shards, independently) | s1+s2 | **DEDUP** |
| 35 | `SIGMALAW_PROOF_2026-08-08.md:(EC-q)` | 26 | Declared NOT consumed | s1 | — |
| 36 | `DULEMMA_PROOF_2026-08-08.md:DULEMMA` | 1 | Declared NOT consumed | s1 | — |
| 37 | `CODEX_HE3PE1_2026-08-08.md:HE3` | 15 | The uncounted Codex pass; source of the (SEP) counterexample / the uncounted composer-context pass | s1+s2 | **DEDUP** |
| 38 | `spec/HYPOTHESIS_LEDGER.md:HYP.81` | 10 | The ledger row for `[GENHN-HE(μ≥3)]` | s1 | — |
| 39 | `HE7_PROOF_2026-08-08.md:HE3-BOX-6` | **1 (CORRECTED — shard 2 recorded 12; see note above and source defect 16)** | **Reverse edge: HE7 consumes the box** — omitted from HE3's own consumer trace (`.06`). Confirmed at HE7 line 1263: "this is the step whose absence was HE3-BOX-6's original defect" | s2 | — |
| 40 | `W12_PROOF_2026-08-08.md:w12_checks` | 8 | The pinned σ-oracle question HE-SIG (`.52`) reuses | s2 | — |
| 41 | `W12_PROOF_2026-08-08.md:W-12.D` | 19 | The supplier stack row | s2 | — |
| 42 | `W11_PROOF_2026-08-08.md:W-11` | 18 | The supplier stack row (not consumed) | s2 | — |
| 43 | `T2_SIGMA_LADDER_MASTER_2026-08-12.md:HE3-A-PROOF` | 65 | **Reverse edge: T2's `(MP1′)` retirement rests on HE3's stage-α passage** (`.34`, lines 1149–1166) — absent from HE3's own consumer trace | s2 | — |
| 44 | `T2_SIGMA_LADDER_MASTER_2026-08-12.md:HE6-SLOT-SEAM` | 56 | A co-certifier of the same retirement | s2 | — |
| 45 | `T2_SIGMA_LADDER_MASTER_2026-08-12.md:HE6-FAMILY` | 61 | Ditto | s2 | — |
| 46 | `T2_SIGMA_LADDER_MASTER_2026-08-12.md:HE6-PROOF` | 66 | Ditto | s2 | — |

**Designations counted but NOT emitted as XREFs (5 distinct, no overlap between the two shards' lists):**

- **`ϖ-CONVENTION`** — count **0** in `HE6_PROOF_2026-08-08.md`; the target's own heading is `**CONVENTION (ϖ-normalizers: twist-free).**` at line 104, verified. Carried descriptively at `.03`. (Shard 1.)
- **`GENHN §S4`, `GENH4 S5`-style section pointers** — §-anchor citations, not designations; carried descriptively. (Shard 1.)
- **`HE6-3(a)`** — the sub-clause cited at `.11`(iii); the parent `LEMMA HE6-3` (row 24, count 12) is emitted instead. (Shard 1.)
- **`(MP1′)`** — count **0** in `HE3_PROOF_2026-08-08.md`, count **37** in `T2_SIGMA_LADDER_MASTER_2026-08-12.md` (fresh: `grep -c 'MP1' HE3_PROOF_2026-08-08.md` = **0**, re-verified). HE3 does not name it; the edge is one-directional and is recorded descriptively at §5 and OPEN-CALL 9. (Shard 2.)
- **`§-anchor pointers`** (`§S1 (iii)`, `§S6.2`, `L1249–1258`, `L677`, `L350–353`, `L1049`) — line/section pins, verified to land in the right spans (`.72`), not designations. (Shard 2.)

**A note on "T2" as a bare substring (compiler finding, not in either shard).** The assignment brief for this merge states "HE3 itself names neither T2 nor `(MP1′)` (`grep -c 'MP1'` = 0, `grep -c 'T2'` = 0 …)". Fresh-checked: `grep -c 'MP1'` = **0** ✓, but a literal `grep -c 'T2'` against `HE3_PROOF_2026-08-08.md` returns **21**, not 0 — every one of the 21 lines is an occurrence of **`(T2)`**, the note's own hypothesis label (paired with `(T1)`) used throughout §S0–S9 for GENHN-2's/HE6-0″'s two matching conditions, and has nothing to do with the T2 sigma-ladder note. A targeted check for an actual T2-note citation — `grep -n 'T2_SIGMA_LADDER\|T2_PROOF\|note T2\|EFF-T2'` — returns **0** in `HE3_PROOF_2026-08-08.md`. **The substantive claim (HE3 does not cite the T2 note or its retirement obligation by name) is confirmed; the literal grep pattern in the brief is confounded by an unrelated label collision.** Recorded as source defect 17, not silently corrected.

**Commit pins verified with `git cat-file -t` (all → `commit`) — 5 distinct, consolidated (shard 1 recorded 5, shard 2 recorded 2, both subsets of the same 5, dedup 2):** `7950ee4` (PE1), `6d0ff09` (PE2), `7a95449` (the consumers' HE3 pin), `f2a7164` (HE3's own HEAD pin at composition), `4682bcf` (GENIND) — **re-verified fresh at merge time, 5/5 → commit.**

**Artifacts/reports/logs verified present on disk — 25 distinct, consolidated (shard 1's 17-file list and shard 2's 17-file list are byte-identical; reports and logs likewise overlap in full; only shard 1 additionally checks 2 corpus records):** `he3_checks.py`, `he3_checks_output.txt`, `he3_checks_results.json`, `he3r1_supp.py`, `he3r1_supp_output.txt`, `he3r1_supp_results.json`, `he3_pe1fresh_ab.py`, `he3_pe1fresh_ab_output.txt`, `he3_pe1fresh_c.py`, `he3_pe1fresh_c_output.txt`, `he3_pe2fresh.py`, `he3_pe3fresh.py`, `he3_pe3fresh_output.txt`, `he3_pe3fresh_results.json`, `he3_pe4fresh.py`, `he3_pe4fresh_output.txt`, `he3_pe4fresh_results.json` (17, both shards independently) · `HE3_passPE1_report.md`, `HE3_passPE2_report.md`, `HE3_passPE3_report.md`, `CODEX_HE3PE1_2026-08-08.md` (4) · `runs/residues/R8_he3_output.log`, `runs/cert/A5_he3_output.log` (2) · `runs/qgen/WELD_FACE_AUDIT.md`, `spec/HYPOTHESIS_LEDGER.md` (2, shard 1 only) = **25/25 present.** **Note carried from shard 2 (source defect 15 below):** `he3_pe4fresh.py` (+ output + results) exists on disk and the note itself never references a PE4 — the grade arc stops at PE3.

**Hash pinning quality (rule 23), consolidated.** 3 full md5s (`he3r1_supp.py` old `9cd132a8ce174f5703542757675f7960` → new `93681e8d47d447dcbfd46db0ddfe0c55`, plus its output `65ed06e49f4c54abf1b143513633f143`) — **PINNED**, with byte-identical stdout certified across the docstring-only change. The §S8 battery's own artifact hashes are deferred to the commit message and are **UNPINNED from the note's own text** (source defects 4/12).

### Sample re-verification (10 of 46, grep-rerun at merge time against the working tree — MUST include both reverse-edge designations per the merge charge)

`[TABLE — compiler ledger]`

| Target : designation | Recorded count | Re-verified count | Match |
|---|---:|---:|---|
| `HE7_PROOF_2026-08-08.md:HE3-BOX-6` (reverse edge 1) | 12 | **1** | **NO — CORRECTED, source defect 16** |
| `T2_SIGMA_LADDER_MASTER_2026-08-12.md:HE3-A-PROOF` (reverse edge 2) | 65 | 65 | OK |
| `GENHN_PROOF_2026-08-08.md:THEOREM GENHN.CLASS` | 7 | 7 | OK |
| `GENHN_PROOF_2026-08-08.md:LEMMA GENHN-2` | 2 | 2 | OK |
| `HE6_PROOF_2026-08-08.md:HE6-0″` | 7 | 7 | OK |
| `HE6_PROOF_2026-08-08.md:LEMMA HE6-3` | 12 | 12 | OK |
| `GRTJC_PROOF_2026-08-08.md:JC-LOAD` | 31 | 31 | OK |
| `W12_PROOF_2026-08-08.md:w12_checks` | 8 | 8 | OK |
| `GENIND_PROOF_2026-08-08.md:LEMMA GENIND-3` | 6 | 6 | OK |
| `spec/HYPOTHESIS_LEDGER.md:HYP.81` | 10 | 10 | OK |

**9 of 10 sampled counts match exactly; the tenth (reverse edge 1, `HE7_PROOF_2026-08-08.md:HE3-BOX-6`) does not, and is corrected in §4's main table above rather than silently reproduced.** The sample deliberately covers both mandated reverse-edge designations, four dedup pairs, and both external-file-outside-`lean/notes/openmath` targets (`spec/HYPOTHESIS_LEDGER.md`). Blob hash of the source note re-verified identical to the header's pin: `git hash-object lean/notes/openmath/HE3_PROOF_2026-08-08.md` = `79bbbb638c39cf4a9658e466210626bf5d6c90a5` ✓ (STOP condition did not trigger).

### Sample statement-quote re-verification (5 of 72, checked verbatim against the source note by line number)

`[TABLE — compiler ledger]`

| Statement | Quoted fragment | Source lines | Match |
|---|---|---|---|
| `.01` (the title headline) | "# HE3 — [GENHN-HE(μ ≥ 3)] CLOSED AT ITS FIRST-LIVE CASE … grade stays 0/2, PE3 next]**" | 1 | OK, byte-exact |
| `.35` (LEMMA HE3-6) | "**LEMMA HE3-6 [r1] (Galois equivariance of labels).** For g ∈ Gal(K̄₀/K₀) … Galois-stable, at every f₁." | 1118–1121 | OK, byte-exact |
| `.44` (HE3-BOX-6 opening) | "**HE3-BOX-6 (THE ONE SURVIVING GAP — found by this unit's own review … LEMMA HE3-4's branch (ii) …" | 1334–1338 (of 1334–1368) | OK, byte-exact |
| `.67` (R8-1 opening) | "### Dated correction (2026-08-12, post-R8) — R8-1: HE3-5 is recursive, not a one-unit bound … whether the multiplicity of a residual root at the current node is" | 1634–1637 (of 1634–1689) | OK, byte-exact |
| `.72` (A5 verification parenthetical opening) | "*(Corrections R8-1..R8-5 derived by the A5 certification run (gpt-5.6-sol high, runs/cert/A5_he3_output.log, 2026-08-12); orchestrator verification before transcription — R8-1: …" | 1821–1824 (of 1821–1853) | OK, byte-exact |

**All 5 sampled quotes are byte-identical to the source note at the cited lines; no drift introduced by the renumbering pass.** The sample deliberately covers both shards' HOME ranges, the merge's own corrected-XREF unit (`.44`), and both endpoints of the R8-1/A5 dated-append pair.

---

## 5. Local supply/consumption ledger

HE3's own protocol (header predicate B, stated in the note's own words at `.06`): **it reports on its consumers rather than appending to them.** §S0′'s CONSUMER TRACE (`.06`) computes SAFE/N-A verdicts for four consumers and lands nothing. **Zero dated consumption appends are claimed landed on any consumer by this note, and none is invented here.** The table below merges both shards' independent §3.2 tables (shard 1: 6 rows; shard 2: 5 rows) into one consumer-direction ledger, deduping the three suppliers both shards separately tracked (HE6, HE7, GENHN's box).

### 5.1 Consumer-direction ledger (mathematical supply versus append/consumption protocol, kept strictly distinct per the header's predicate split)

`[TABLE — compiler ledger]`

| Consumer | Verified designation(s) | HE3 EFF units (supply) | Mathematical status | Append/consumption-protocol status |
|---|---|---|---|---|
| HE6 (frozen @ HE3 `7a95449`) | `HE3-0` (9), `DEFINITION 1` (1), `HE3-2(b)` (1), `HE3.B` (2), `HE3-1` (1) | `.13`, `.15`, `.17`, `.20`, `.25`, `.27`, `.31` | HE3's own trace verdicts SAFE; three displays byte-frozen for exactly this reason (F3) | **Not landed by HE3.** The trace is computed here and appended nowhere |
| **HE7 (frozen @ HE3 `7a95449`)** | `HE3-3` (2) · **`HE3-BOX-6` (1, CORRECTED at this merge — §4)** | `.13`, `.15`, `.31` (the DEF/(★) edge) · `.44` (the box) | SAFE on DEF 1/2 and (★); **on the box, HE7 consumes HE3's own *conditionality*, which R8-2/R8-3 have since NARROWED to branch (ii) alone** | **Not landed either way.** HE3's own consumer trace (`.06`) omits the BOX-6 edge entirely (source defect 1); no append tells HE7 the box shrank. **Reverse edge #1, carried explicitly per this merge's charge.** |
| GENHN (reverse direction: GENHN consumes HE3) | `HE3-0` cited in GENHN's own `[r1]` node erratum | `.25` | GENHN consumes HE3-0; HE3's corrected node MATCHES GENHN's erratum ✓ verified | **Landed on GENHN by GENHN**, in its own `[r1]` erratum — not by HE3 |
| GENHN (the box) | `[GENHN-HE` (19) | `.09`, `.17`, `.56` | HE3 CLAIMS the discharge at μ ≤ 3 unconditionally | **NOT landed.** GENHN's §S9.2 still calls it "THE one load-bearing open lemma of the whole composite program" (OPEN-CALL 2) |
| GENIND (accepted) | — | — | "no consumption site of HE3 found" ✓ verified (GENIND's own note, grepped) | N/A |
| **T2 (accepted, 2/2)** | **`HE3-A-PROOF` (65)** | `.34`'s stage-α passage (source lines 1149–1166: "Apply the inductive hypothesis to the sub-block: it is the label-class of a multiplicity-m system over the SAME stage ring, with the same (e₁, f₁)") | **T2's `(MP1′)` obligation is RETIRED on the strength of it** — `spec/EFF-T2.md` `EFF.T2.25`: "`HE3-A-PROOF` transports the refined sub-block over the same stage ring" | **Not landed and NOT RECORDED.** HE3 names neither T2 nor `(MP1′)` by designation (both 0, §4); its own consumer trace (`.06`) lists exactly four consumers (HE6, HE7, GENHN, GENIND) and T2 is not one of them. **Reverse edge #2, carried explicitly per this merge's charge.** |
| W-12 | `w12_checks` (8) | `.48` | HE-SIG reuses W-12's pinned σ-oracle question — a consumption in the instrument direction only | Not landed; a decorrelation, not a mathematical dependency |
| `spec/HYPOTHESIS_LEDGER.md` | `HYP.81` (10) | `.09` | The ledger indexes `[GENHN-HE(μ≥3)]` as a live obligation | Ledger-side, done outside HE3 |

**The protocol column is not an inference.** HE3 states the "report, don't append" protocol in its own words (`.06`'s framing, `.44`'s "IDENTIFIED REPAIR ROUTE (not executed here)"); **zero dated consumption appends are claimed landed on any consumer by this note, and none is invented here.**

### 5.2 Predicate-B discussion: HE3's own consumer trace is two edges short

`.06`'s CONSUMER TRACE (§S0′, verbatim-quoted at that unit) lists exactly **four** consumers — HE6, HE7, GENHN, GENIND — each with a computed SAFE/N-A verdict, and its own words are explicit about the method: "no consumption site of HE3 found in `GENIND_PROOF_2026-08-08.md` (grep, this round)". **Per template rule 10 — a cross-note status claim must be checked against the full append chain, not the frozen trace sentence alone — this merge's own re-verification (§4 above, this table) confirms the trace is demonstrably two edges short of the true outbound picture:**

1. **HE7 also consumes `HE3-BOX-6`** (row above; corrected count 1, not the 12 either shard's own numbers implied). This is a *mathematical* edge, not merely a citation: HE7 consumes HE3's *conditionality*, and that conditionality has since narrowed under R8-2/R8-3 (`.68`, `.69`) in ways `.06`'s trace — computed before the A5 wave — cannot reflect.
2. **T2's `(MP1′)` retirement rests on HE3's own stage-α passage** (`.34`, lines 1149–1166), verbatim-identical to the passage T2's own note names by designation (`HE3-A-PROOF`, count 65). This is the more consequential of the two: **an accepted note (T2) has retired a standing obligation on the strength of a passage whose own home note (HE3) sits at grade 1/2, not accepted, and whose surrounding lemma set (HE3-4, HE3-4U, HE3-4D1) was corrected five times on 2026-08-12** — after T2's own citation, if T2's citation predates the A5 wave (not independently dated by this compiler; recorded, not resolved). The stage-α passage itself (`.34`'s "Apply the inductive hypothesis to the sub-block…") is **not** among the five R8 corrections' targets (verified: none of R8-1 through R8-5 touches lines 1149–1166), so the retirement's *foundation* is unmoved even though its *neighbourhood* was repaired five times in one day.

**Neither edge is invented here — both are grep-verified, both are named by the consuming note's own text, and neither is claimed as a defect in T2 or HE7 (they cited correctly; HE3's own trace is what is incomplete).** Both are carried forward as explicit reverse-XREF rows above and as unresolved OPEN-CALLs (§8, OPEN-CALL 7 for the HE7/BOX-6 edge, OPEN-CALL 9 for the T2 edge).

---

## 6. TEETH inverse table (unified from both shards; presented as two sub-tables by shard-of-origin — shard 1 carries the core sealed battery, shard 2 carries the additive `[r1]` legs, the Q1–Q5 verdicts, and the A5-wave findings — with one combined zero-orphan statement closing both)

### 6.1 Shard 1's core battery and additive legs (IDs unchanged, offset 0)

`[TABLE — compiler ledger]` Battery row / tooth → guarded unit(s) → disposition.

| Battery row / tooth | Guarded unit(s) | Disposition |
|---|---|---|
| **HE-NORM** (LEMMA HE3-1 at the entry key on EVERY member; 72,134 entry-key resultant identities, exact Bareiss/Sylvester) | `.50`, `.20`, `.49`(i) | Executable regression, exhaustive on the enumerated rows; "The engine of the proof is machine-real." |
| **HE-PSI** (the identity at EVERY test key (κ,s) + HE3-1′'s inequality + the convex duality (★); 43,528 checks, 0 violations) | `.51`, `.22`, `.29`, `.31` | Executable regression — tests the inequality and (★), not the equality-vs-cancellation dichotomy |
| **HE-SIG** (PARI factorpadic-200 + idealprimedec vs the HE3.A dictionary; 947 jobs, 0 bad, all five μ = 3 types) | `.52`, `.16`, `.34`, `.36` | Executable regression against an independent oracle (W-12's pinned question) |
| **HE-BND** (the decided/undecided profile vs window; 3,744 boundary reads) | `.55`, `.15`, `.37`, `.38` | Measurement; **RE-SCOPED by R8-1** to the aligned rows only — `stale self-description` on the general two-step law |
| **HE-T-UNDET** (the μ = 2 method's 10 spurious solutions at a 3-label member) | `.54`, `.19` | Planted mutant / measurement; **fired — "the mechanism certificate"** |
| **HE-T-SIG** (6/6 flipped dictionaries caught by PARI on each of two rows) | `.54`, `.16` | Planted mutant; fired |
| **HE-T-CAP** (the out-of-window-dropping reader disagrees on 17 (p=5) / 43 (p=7) members) | `.54`, `.37` | Planted mutant; **fired — added mid-run, "the strongest tooth"**; the mutant is the note's own earlier reader |
| **HE-T-NODE** | `.43`, `.54` | **MIS-SPECIFIED**: "does not test what its name claims … read as a diagnostic, not a tooth" — `stale self-description` |
| **HE-T-PSI, HE-T-CERT** | `.43`, `.40` | **OWED** (never wired) — `disclosed non-repair` |
| **Q4, the census tie** (the fresh reader reproduces GENHN's committed E3 rows exactly, 72,134 members) | `.53`, `.46` | **Foreign-note decorrelation** — "Two independently written readers agreeing to the digit on 72,134 members" |
| **`he3r1_supp.py` P1** (the repaired peel at PE1's counter-instance; m_s = 1 at all 3 roots incl. both K∖F₃ letters, 0 at the other 5; PARI on two variants) | `.58`, `.21`, `.32`, `.33` | Executable regression, exhaustive over the letter set; **excludes by measurement the splits PE1 showed the old text could not** |
| **`he3r1_supp.py` P2** (f₁ = 2 sweep, 8/8 members incl. an ℓ = 2 member and the consecutive-triple geography) | `.59`, `.10`, `.20`, `.25`, `.30` | Executable regression; **e₁ = 1 only — the twist seam is structurally invisible** |
| **`he3r1_supp.py` P2-N1** (both branches of the N1 supplement) | `.60`, `.30` | Executable regression **at the WRONG geography** — `signed vacuity disclosure` at `[r2]` F-5; cured by PE2's FR-D |
| **`he3r1_supp.py` P3** (f₁ = 3 sweep, 4/4 + the 26-letter peel over F₂₇^×) | `.61`, `.21`, `.35` | Executable regression, exhaustive over the letter set |
| **`he3r1_supp.py` P4** (LEMMA HE3-1L's exact set) | `.62`, `.21` | **Quasi-tautological, DISCLOSED** at `[r2]` F-4 — `signed vacuity disclosure`; decorrelated substitute is PE2's FR-C (`nfeltval`/`nfmodpr`) |
| **`he3_pe2fresh.py`** (GREEN, 7/7 PARI members at the first e₁ ≥ 2 ∧ f₁ ≥ 2 geography; member `ma` the twist adjudicator; FR-C, FR-D) | `.60`, `.62`, `.63`, `.14`, `.24`, `.30` | Executable regression on a decorrelated instrument; **the twist mutant fired** |
| **PE3's fresh route** (GREEN, 0 violations, three disjoint geographies: the (2,1)/(3,1) starved corners, a μ = 3 coherent family at (e₁,f₁) = (3,1), and non-prime Q = 4 over F₄[[t]]) | `.64`, `.65`, `.12`, `.14` | Executable regression on a decorrelated instrument; **the coherent-vs-naive flip adjudicated at each of the three, siding with coherent each time** |

**PROOF-ONLY rows (shard 1, rule 16), with the note's own coverage-hole sentence:**

| Unit | Coverage hole, in the note's words |
|---|---|
| `.10` (the L/O_L carrier) | The battery enumerates in stage coordinates and never consults R's ring structure; the affected site is the μ ≥ 4 stage-CS step — HE3-BOX-3 (`.41`): "NOT exercised by machine: … μ ≥ 4" |
...
| `.29` ((SEP) at general μ) | Needs `μ ≥ 9` to fail; HE3-BOX-3 (`.41`): "μ ≥ 4 … NOT exercised by machine" |
| `.33` (branch (ii)) | HE3-BOX-3 (`.41`): "μ ≥ 4 (where LEMMA HE3-4's ramified base-change branch … first bite)" |
...
**AUDIT-disposition rows (shard 1, rule 27) — findings no battery row could have produced:**
...
### 6.2 Shard 2's Q1–Q5 verdicts and the A5-wave findings (renumbered, `.39`–`.72`)

**PROOF-ONLY rows (shard 2, rule 16), with the note's own coverage-hole sentence:**

| Unit | Coverage hole, in the note's words |
|---|---|
| `.40` (the embedded perimeter) | HE-T-CERT is OWED and there is "no embedded n = 7 row" — the gating clause is vacuous for this instrument |
| `.44` (branch (ii)) | HE3-BOX-3 (`.41`): "NOT exercised by machine: … μ ≥ 4 (where LEMMA HE3-4's ramified base-change branch … first bite)" |
| `.68` (LEMMA HE3-4U) | Same — the boxed branch (i) is reachable only at μ ≥ 4 |
| `.69` (LEMMA HE3-4D1) | PROOF-ONLY as a lemma; the `D′ = 1` case has contact at PE2's FR-D |
| `.70` (the piecewise `S_r`) | `ℓ ≥ 2, d ≥ 2` is branch (ii); only `ℓ = 2, d = 1` has contact (P2's λ = 7/2 member) |
| `.71` (the provenance fix) | No instrument can see a citation-scope defect |
| `.42` (char-p σ) | "not against an independent oracle" |

**AUDIT-disposition rows (shard 2, rule 27):**

| Finding | Unit | Class |
|---|---|---|
| The starved-corner list matches neither reading | `.65` | arithmetic recount (PE3 M-1) |
| HE3-5 is recursive; the "one more window unit" bullet is FALSE | `.67` | decorrelated-model audit + constructed counterexample |
| §S6.2 contradicts §S1 on how many branches are boxed | `.68`, `.44` | decorrelated-model audit — an internal contradiction |
| "n ≥ D′μ ≥ 8" consumes an undisplayed `D′ ≥ 2` | `.69`, `.44` | decorrelated-model audit |
| `S_r`'s display is still ill-defined at ℓ ≥ 2 | `.70` | decorrelated-model audit (an r1-repair propagation miss) |
| HE3-0's proof sentence still cites GENHN-2 | `.71` | decorrelated-model audit (an r1-repair propagation miss) |
| P4 is quasi-tautological as a residue check | `.62` | **self-found at `[r2]`** — in-house instrument audit |
| The N1 demo ran where the gap is not live | `.60` | **self-found at `[r2]`** |
| HE-T-NODE does not test what its name claims | `.43` | **self-found** — in-house instrument audit |
| Two promised teeth were never wired; a promised row never ran | `.43` | **self-found** sealed-vs-delivered audit |

### 6.3 Combined zero-orphan statement

**Both-direction audit, unified.** Every battery row in either sub-table guards at least one unit in the merged 72-unit inventory; every unit whose `TEETH` field names a battery row appears in some row above. **Units with `TEETH: NONE`** — `.01`, `.03`, `.04`, `.06`, `.08`, `.09`, `.13`, `.18` (shard 1) and `.39`, `.45`, `.56`, `.66`, `.71` (shard 2, renumbered) — are declarations, fences, status records, preregistrations, framing paragraphs, or provenance fixes, and are outside the orphan count. **Units flagged `TEETH: Indirect`** — `.25`, `.26`, `.28` — are guarded compositely through HE-SIG. **Zero battery-row orphans; zero toothed-unit orphans across all 72 units; sixteen signed PROOF-ONLY units (9 shard 1 + 7 shard 2); twenty-five AUDIT-class findings (15 shard 1 + 10 shard 2), five of them SELF-FOUND (1 shard 1 — the N1 gap — + 4 shard 2 — the sealed-vs-delivered shortfall, HE-T-NODE's mis-specification, P4's quasi-tautology, and the N1 demo's dead geography).**

---

## 7. SERIES, cross-shard supersession edges, and boundary resolution

### 7.1 The dated-arc SERIES: HE3-GRADE-ARC (4 members, TERMINAL — HE3 is NOT accepted)

Both shards independently name this construct (shard 1's header rule 28; shard 2's SHARD NOTES item (f): "SERIES HE3-GRADE-ARC has members in both shards") and each supplies exactly the links inside its own HOME range. Assembled here as one block, per the merge charge, cross-checked against `.02`'s own CONDITIONALITY table (reproduced verbatim in the merged inventory) and against `.39`'s and `.64`'s own confirmations — **all three independent descriptions name exactly the same four links, in the same order, with no duplication and no gap.**

`[TABLE — compiler ledger, assembled from EFF.HE3.02's CONDITIONALITY field plus its own confirmations at EFF.HE3.39 and EFF.HE3.64]`

| Link | Unit | Round | Grade movement | Content (quoted from the unit or its own resolution) |
|---:|---|---|---|---|
| 0 (origin) | `.02` | base + `[r1]` | **0/2** | "PE1 NOT CLEAN (2 CRITICAL + 4 GAP + 2 MINOR); `[r1]` repairs" applied in place |
| 1 | `.07` | `[r2]` | **stays 0/2** | PE2 NOT CLEAN (0 CRITICAL + 2 GAP + 3 MINOR — the r1 repairs and core lemmas HELD); repaired by the `[r2]` round |
| 2 | `.39` (HE3-BOX-1, shard 2's HOME) | base (box re-stated) | **0/2 (re-affirmed)** | "Attempt grade 0/2 on the μ ≤ 3 statement … PE3 is next" — the strike-through additionally **LOWERS** an earlier 1/2 (a Codex composer-context pass no longer counts) |
| 3 (**TERMINAL**) | `.64` | dated append (post-PE3) | **0/2 → 1/2** | "PE3 verdict … CLEAN — 0 CRITICAL, 0 GAP, 1 MINOR (M-1, corrected below). The consecutive-clean count moves 0/2 → 1/2 and HOLDS at 1/2 through this correction." |

**The arc stops at 1/2 — acceptance needs two consecutive clean passes, and HE3 has exactly one. HE3 is NOT accepted.** This is the load-bearing fact for every downstream conditionality display (`.01`(c), `.09`, `.56`). **Unlike the sibling GENHN/GRTW2 merges, which reach 2/2, this SERIES terminates at 1/2 — the two must not be conflated.** Five further REAL findings (the A5 wave, `.66`–`.72`) landed *after* this terminal link; none of them resets or advances the counter (`.64`'s own words: "HOLDS at 1/2 through this correction"), and their own status is "awaiting sol discharge-confirmation" (OPEN-CALL 8).

**Uniqueness verified:** an exhaustive scan of every CONDITIONALITY field in the merged 72-unit inventory for the string `HE3-GRADE-ARC` confirms exactly four member-declarations (`.02` "member 1", the CONDITIONALITY table itself naming all four, `.39`'s heading "member 3 of 4", `.64`'s heading "member 4 of 4 (TERMINAL)") and no fifth candidate anywhere else. **No duplicate link, no skipped link, TERMINAL uncontested.**

### 7.2 Cross-shard supersession edges (consolidated, with SUPERSESSION KIND)

The header records that **six dated corrections land inside shard 1's HOME range**, all physically drafted in shard 2's range (rule 7: HOME is determined by an edit's *destination*, not its physical drafting location — the rule T1's and HE7's merges also used). The table below lists every such edge, plus the in-range (shard 2 → shard 2) edges of the same six corrections where a correction has more than one target, using the vocabulary already exercised throughout the merged inventory: **replacement | license | wording-rider | provenance-rider | scope-pin | as-of scoping | counter re-reading | inventory completion** — all eight kinds named in the template appear somewhere in this note (shard 1's own format-note tally, `.33`/`.44`/`.65` supplying `inventory completion` and `counter re-reading`, the only two not otherwise exercised at the unit level).

`[TABLE — compiler ledger, consolidated from shard 1 SHARD NOTES's 6-item cross-shard list and shard 2 SHARD NOTES's own "riders landing on shard 1" enumeration — both name the identical six corrections]`

| From | To | Kind | Content |
|---|---|---|---|
| `.65` (M-1, PE3) | `.12` | counter re-reading | The starved-corner parenthetical re-counted under two explicit bounds (four-element list vs. two-element reachable corner); frozen fence stands by conservativity |
| `.65` (M-1) | `.11`(iii) | scope-pin | The imprecision bites only for a future consumer re-using the parenthetical at a site where `d_λ = μ` |
| `.67` (R8-1) | `.37` | replacement | The three bullets following LEMMA HE3-5 replaced by the recursive DECIDED criterion; "one more window unit" refuted by a constructed counterexample |
| `.67` (R8-1) | `.38` | scope-pin | The Interpretation's "exact q-power" framing survives only under the stratified reading R8-1 supplies |
| `.67` (R8-1) | `.55` (in-range) | scope-pin | HE-BND's "PREDICTION CONFIRMED" re-scoped to "evidence for the selected aligned rows only" |
| `.68` (R8-2, LEMMA HE3-4U) | `.33` | license | Branch (i) of the base-change reduction PROVED, not boxed; "BOTH … BOXED" corrected to "branch (ii) alone is BOXED" |
| `.68` (R8-2) | `.44` (in-range) | inventory completion | HE3-BOX-6's item list shrinks from two branches to one — **chained, see below** |
| `.68` (R8-2) | `.17` | license | COROLLARY HE3.B's single μ ≥ 4 reservation becomes accurate again |
| `.68` (R8-2) | `.01` | license | The headline's "ONE named branch" claim (contradicted by `.33` pre-repair) becomes true |
| `.69` (R8-3, LEMMA HE3-4D1) | `.44` (in-range) | license, **chained; TERMINAL** | Supplies the box's missing `D′ ≥ 2` hypothesis; the `D′ = 1` case proved unconditional at every μ — **TERMINAL narrowing of HE3-BOX-6, together with `.68`: branch (ii) alone, at μ ≥ 4 and D′ ≥ 2, i.e. n ≥ 8** |
| `.69` (R8-3) | `.17` | license | The advertised `n ≤ 7` unconditional range survives, now with the missing derivation displayed |
| `.69` (R8-3) | `.01` | license | Same, at the headline |
| `.70` (R8-4) | `.32` | replacement | LEMMA HE3-4's `S_r` display, ill-defined at `ℓ ≥ 2`, replaced by the piecewise `β_ρ` definition |
| `.71` (R8-5) | `.25` | provenance-rider | The stale citation of GENHN-2 in HE3-0's proof sketch re-pointed to HE6-0″ (the claim itself does not move) |

**CHAIN, marked TERMINAL (rule 25):** HE3-BOX-6's displayed text (`.44`, "the only place the argument leaves the ground it stands on") is contradicted pre-repair by `.33`'s "Both branches … BOXED" → **`.68`/R8-2** proves branch (i), correcting `.33` to "branch (ii) alone is BOXED" → **`.69`/R8-3** supplies the undisplayed `D′ ≥ 2` hypothesis the box's own `n ≥ 8` derivation needed. **TERMINAL: branch (ii) alone, at μ ≥ 4 and D′ ≥ 2 (n ≥ 8) — awaiting sol discharge-confirmation**, per both `.44`'s and `.68`'s own CHAIN fields. **Uniqueness verified:** no third correction targets `.33`/`.44`'s branch-boxing claim anywhere in the merged inventory (an exhaustive scan of every CONDITIONALITY/RESOLUTION TRACE field for `EFF.HE3.33` and `EFF.HE3.44` as a supersession *target* confirms only `.68` and `.69`).

**Total: fourteen distinct supersession edges** (all six corrections land at least once inside shard 1's HOME range; four of the fourteen are in-range shard-2-to-shard-2 edges of the same six corrections, listed for completeness since a correction with multiple targets is one unit, not one edge). **All fourteen resolve to a unit that exists in the merged inventory; zero dangling targets.**

### 7.3 Boundary-unit resolution (merge rule 4/7)

Per rule 7 (HOME follows edit destination, not drafting location), all six corrections above are **HOMED in shard 2** (drafted and homed at `.65`, `.67`–`.71`) even though five of their six target-sets land on shard-1-HOME units. **Verified: none of the six is duplicated as a second unit inside shard 1's range** — an exhaustive scan of the merged inventory's 72 unit headers (§3) confirms `.65` and `.67`–`.71` appear exactly once each, at their shard-2-derived numbers, and no unit numbered `.01`–`.38` independently re-states any of M-1, R8-1, R8-2, R8-3, R8-4, or R8-5 as its own content (they are only *cited*, by ID, from the shard-1-HOME units they target). Neither shard's own §6 BOUNDARY SELF-AUDIT names any unit drafted in shard 1 but homed in shard 2 (the traffic is one-directional in this note, unlike GENHN's/HE7's own boundary audits). **Every unit named in either shard's own §6 as "drafted elsewhere, HOME-routed" appears exactly once in the merged inventory.**

### 7.4 Range-edge audit

Both shards independently audited the physical line boundary and both report the same clean break: shard 1's own note — "Line 1274 is the Interpretation's last sentence … line 1275 is blank; line 1276 opens `## S7. BOXES, GRADE, AND COVERAGE`. No source statement unit is cut at the edge." — and shard 2's own note — "Line 1275 (shard 1's last) is blank; line 1276 opens `## S7. …`; the range ends at line 1853, the note's last line." **Both descriptions name the identical boundary (1275/1276) and agree it falls on a section break.** The file's own end (source line 1853) is shard 2's own last line, closing the A5 verification parenthetical's final `)*`. **Zero units span the shard boundary.**

---

## 8. Self-audit and OPEN-CALLs

**12 raw OPEN-CALLs (6 per shard) carry forward as 12 distinct.** No pair is *self-flagged* as a duplicate by either shard (unlike the sibling GENHN/HE7/T1 merges, where the source shards explicitly wrote "this is the same call as…"); this merge checked for such flags with a fresh grep (`same call`, `self-identified duplicate`, `recorded in both`, `same as shard`, `identically as shard`, `same question` — none found across either shard) and found none, so **per the standing charge ("dedup self-flagged duplicate pairs"), nothing here is dedup'd.** One pair (OPEN-CALL 5 and OPEN-CALL 8 below) is substantively close — both concern the five A5 repairs' unconfirmed status — but neither shard cross-references the other's copy, so both are carried forward separately with the overlap noted, not merged. **None of the 12 is discharged by the act of merging** — every one is carried forward open.
...
`.06` asserts, for HE6 and HE7 at pin `7a95449`, that specific clauses are "byte-unchanged". **This merge verified that every consumed designation EXISTS in each target (grep-checked, all ≥ 1) but did NOT diff the clauses against the `7a95449` blob.** Doing so would require checking out the pinned blob and comparing spans. **Decision needed: whether a future pass performs the byte-diff, or the trace is recorded as an inherited assertion.** Two related gaps surfaced while checking, one of them **corrected at this merge**: **HE7 also consumes `HE3-BOX-6`** — recorded by shard 2 (and by this merge's own assignment brief) at count **12**, but fresh re-verification finds count **1** (§4; source defect 16) — which `.06`'s own four-consumer trace still does not list either way (source defect 1); and the trace's GENIND row was confirmed by grep ✓.

### OPEN-CALL 4 — "decided leaf" has two candidate definitions and the note lists the question as open

`.15` (DEFINITION 2) requires terminal separability + lift-stability at every event; `.37` (LEMMA HE3-5) gives a window criterion; `.38` asserts they coincide ("The theorem's hypothesis "DECIDED leaf" is precisely LEMMA HE3-5's condition"); **HE3-BOX-5 item (4) (`.45`) lists the coincidence as an open check** the battery measures both sides of; and **R8-1 (`.67`) makes HE3-5 recursive**, which is what would make the coincidence true. **Decision needed: whether the merged spec asserts the coincidence under R8-1's recursive reading, or carries it as open.** No silent choice was made.

### OPEN-CALL 5 — the A5 wave's five repairs are all "awaiting sol discharge-confirmation"

R8-1..R8-5 (`.67`–`.71`) close with: "Ledger: HE3's row → 5 repairs folded, **awaiting sol discharge-confirmation**." Two of them (R8-2 `.68`, R8-3 `.69`) narrow HE3-BOX-6 and are load-bearing for the headline's `n ≤ 7` range; one (R8-1 `.67`) refutes a displayed bullet of a lemma the battery's HE-BND leg was scored against. **Decision needed: whether a chapter cut may consume landed-but-unconfirmed repairs as current** — the same question `spec/EFF-GENHN.md`'s own OPEN-CALL raises for GENHN's A2 wave (formerly `spec/EFF-GENHN-s2of2.md` OPEN-CALL 4), and the two should be answered together.

### OPEN-CALL 6 — §S6.2's "both branches boxed" contradicted §S1 for the whole pre-A5 period, and the corpus consumed HE3 during it

`.33` displayed "**Both branches below are therefore to be read as BOXED**" while `.17` and HE3-BOX-6 (`.44`) said only branch (ii) was. The A5 verification note (`.72`) names it as "the internal contradiction". **The note was consumed by HE6 and HE7 (frozen at `7a95449`) during that period**, and `.06`'s trace — computed at `[r1]`, before A5 — does not mention it. **Decision needed: whether the consumer trace must be re-run against the post-A5 text.** Recorded, not resolved.

### OPEN-CALL 7 — HE3-BOX-6's displayed scope and its HEAD scope differ, and a consumer pins the display

`.44`'s frozen text says branch (ii) is "the only place the argument leaves the ground it stands on", while `.33` said BOTH branches were boxed — an internal contradiction the A5 wave named (OPEN-CALL 6 above). **R8-2 (`.68`) makes the box's own title claim true; R8-3 (`.69`) supplies its missing `D′ ≥ 2`.** Meanwhile **HE7 consumes `HE3-BOX-6`** (count **1**, corrected at this merge — see §4 and OPEN-CALL 3) and was frozen at `HE3 @ 7a95449`, i.e. **before both narrowings**. **Decision needed: whether the merged spec carries the box at its displayed scope (what HE7 pinned) or its HEAD scope (what R8-2/R8-3 leave), and whether HE7 is owed a note.** This merge carries the HEAD scope with the CHAIN recorded (§7.2).

### OPEN-CALL 8 — all five A5 repairs are landed-but-unconfirmed (closely related to OPEN-CALL 5, not merged with it — neither shard cross-flags the other)

"Ledger: HE3's row → 5 repairs folded, **awaiting sol discharge-confirmation**" (`.72`). Two of the five (R8-2 `.68`, R8-3 `.69`) are load-bearing for the headline's `n ≤ 7` range; one (R8-1 `.67`) refutes a bullet the battery's HE-BND leg (`.55`) was scored against. **Decision needed: whether a chapter cut may consume landed-but-unconfirmed repairs as current** — the same question `spec/EFF-GENHN.md`'s own OPEN-CALL raises, and the two should be answered together. **This call is carried separately from OPEN-CALL 5 because neither source shard names the other's copy as identical**, though a future audit may wish to fold the two.

### OPEN-CALL 9 — T2 retired a standing obligation on the strength of an HE3 passage, and HE3 does not record it

`spec/EFF-T2.md` `EFF.T2.25` records that T2's `(MP1′)` obligation is **RETIRED**, resting on four certifiers of which the first is **`HE3-A-PROOF`** (count **65** in `T2_SIGMA_LADDER_MASTER_2026-08-12.md`, re-verified fresh — §4): "`HE3-A-PROOF` transports the refined sub-block over the same stage ring." **That is HE3's stage-α passage at lines 1149–1166 verbatim** (`.34`). **HE3 names neither T2 nor `(MP1′)` by any recognizable designation** — `(MP1′)` is verified absent (count 0, fresh); a literal `grep -c 'T2'` is confounded by 21 unrelated `(T2)` hypothesis-label occurrences and is **not** a valid test (§4, source defect 17), but a targeted search for `T2_SIGMA_LADDER`/`T2_PROOF`/`EFF-T2` also returns 0 — **HE3 genuinely does not cite the T2 note by name.** Its CONSUMER TRACE (`.06`) lists four consumers, none of them T2. **So an accepted note has retired an obligation on the strength of a passage whose own home note sits at grade 1/2 (not accepted), and whose surrounding lemma set was corrected five times on 2026-08-12** (though none of R8-1..R8-5 touches the stage-α passage itself, lines 1149–1166 — verified — so the retirement's foundation is unmoved even though its neighbourhood was repaired). **Decision needed: whether T2's retirement must be re-checked against HE3 at HEAD, and whether HE3's consumer trace is owed a fifth row.** Nothing was invented; the edge is recorded at §5.

### OPEN-CALL 10 — HE3's most load-bearing citation depends on an HE6-side correction from the same campaign

`.72` discloses: "HE6-0″ (**as corrected at this campaign's A3 fold**) supplies exactly the consumed form." HE6-0″ is HE3's ξ-form slot lemma — **the single most load-bearing import** (`.03`), riding every slot read at `ρ` and `θ″`. **This merge did not verify the A3 fold's content** (out of scope: HE6's own note, not HE3's). **Decision needed: whether a future pass checks HE6-0″ at HEAD against what HE3 consumes.**

### OPEN-CALL 11 — the note's arc has three passes and the disk has four fresh legs

`verification/openmath/he3_pe4fresh.py`, `he3_pe4fresh_output.txt` and `he3_pe4fresh_results.json` all exist (confirmed present, §4), and **no line of HE3 references a PE4**. The grade arc terminates at PE3 (`.64`). **Decision needed: whether a fourth pass ran and is unrecorded, or whether the artifact belongs to another unit's arc.** Recorded as source defect 14; not resolved.

### OPEN-CALL 12 — the instrument's tooth count has three defensible answers

Promised **5**, wired **4**, genuine **3** (`.43`), and `.49` reports "all wired teeth fired". **Decision needed: which number the merged spec reports.** This merge reports all three with the accounting (§6.1).

### Recorded source defects (flagged, not silently repaired; 15 raw from the two shards + 2 found at this merge = 17 distinct)

1. **`.06`:** the CONSUMER TRACE's HE7 row lists "DEF 1/2; the convex duality (★)" but **omits `HE3-BOX-6`**. Shard 2 recorded this designation's cross-note count as 12 (citing what was then `spec/EFF-HE7-s1of3.md` §3.1, drafted before HE7's own merge); this merge's fresh grep against `HE7_PROOF_2026-08-08.md` finds **1** occurrence (line 1263), now consolidated in `spec/EFF-HE7.md`'s own unit `.37`. The trace is incomplete for HE7 either way — see source defect 16.
...
6. **`.34`:** the stage-CS step cites "the multiplicative composition of LEMMA GENHN-T(b), **here PROVED for σ rather than assumed**" — and GENHN's `[r2]` **REFUTED** T(b) at the leaf level, replacing it with T(b)′. HE3's σ-composition claim is compatible with what GENHN says survives, **but the citation names a refuted lemma.** No re-pointing was invented.
7. **§S9 (`.49`):** the sealed battery's artifact hashes are given as "hashes in the commit message", i.e. **not in the note** — UNPINNED from the note's own text, unlike `he3r1_supp.py`'s three full md5s.
8. **`.11` vs `.12`:** the starved-corner list is wrong under both readings (post-PE3 M-1, `.65`) and the frozen text stands; the fence survives only by conservativity, and a future consumer at `d_λ = μ` must use the four-element list.
9. **`.40`:** HE3-BOX-2 says "The battery gates its σ legs at that perimeter **on embedded rows**", but **the battery has no embedded rows** (`.43`, `.47`). The gating clause is vacuous for this instrument.
10. **`.41`:** HE3-BOX-3's "The PROOFS are uniform in all of these" is false at `μ ≥ 4`, where branch (ii) is boxed — in tension with the same box's own "NOT exercised … μ ≥ 4 (where LEMMA HE3-4's ramified base-change branch … first bite)".
11. **`.42`:** HE3-BOX-4's "characteristic-free as PROVED" is not repeated with `.27`'s PE1-F4 qualification (the freedom is at the residue level; binomial coefficients do appear).
12. **`.49`:** §S9's artifact hashes are deferred to a commit message — **UNPINNED from the note**, unlike `he3r1_supp.py`'s three full md5s (duplicate of item 7 above, both shards independently found it; carried once, cross-referenced twice).
13. **`.56`:** "no hostile arc has run" is stale by three passes; the surrounding `[r1]`/`[r2]` text was updated and this sentence was not.
14. **§4 (artifact ledger):** `he3_pe4fresh.py` (+ output + results) exists on disk and **the note has no PE4** — see OPEN-CALL 11.
15. **`.52`/`.55`:** the Q3 member totals (96 / 291) and the N = 6 decided counts (96 / 291) are **identical**, and the note states neither the totals nor the coincidence. Recorded because it is a strong internal consistency result a chapter cut would otherwise lose.
16. **[Found at this merge, not by either shard]** `.44`'s XREF `HE7_PROOF_2026-08-08.md:HE3-BOX-6` was recorded as count **12** by shard 2 (and repeated verbatim in this merge's own assignment brief). Fresh `grep -cF` at HEAD returns **1**. The **12** is HE3's own SELF-count of the designation within `HE3_PROOF_2026-08-08.md` (independently confirmed at 12), evidently conflated with the reverse-direction (HE7-citing-HE3) count somewhere upstream of this merge. **The edge itself is real** (HE7 line 1263 does cite HE3-BOX-6, once) — only the citation count was wrong. Corrected at `.44`, §4, §5, and OPEN-CALL 7; not silently fixed without this note.
17. **[Found at this merge]** The assignment brief's `grep -c 'T2'` = 0 claim for `HE3_PROOF_2026-08-08.md` does not survive a literal re-run: fresh count is **21**, every occurrence being the note's own `(T2)` hypothesis label (paired with `(T1)`, used throughout for GENHN-2's/HE6-0″'s matching conditions), not a citation of the T2 sigma-ladder note. A targeted search (`T2_SIGMA_LADDER`, `T2_PROOF`, `EFF-T2`) confirms **0** genuine citations, so the substantive claim survives; only the literal grep pattern in the brief was confounded by the label collision. See §4 and OPEN-CALL 9.

### Fidelity audit

- Every CANONICAL STATEMENT across all 72 units is a verbatim source quotation or is explicitly marked `[ASSEMBLED]` (one unit, `.09`, carried unchanged from shard 1). **No inverse, sign, quantifier, index, level, inequality direction, or implication direction was altered anywhere**, in either the original shard compilations or this merge's renumbering pass.
- Every source table is marked `[TABLE]`; the note's fixed-width ASCII displays are each emitted twice (verbatim display + Markdown transcription) per rule 14, exactly as both shards' own extension of the rule required; no source table was prosified at merge time.
- **46 distinct external XREF designations** (from 61 raw citations, 15 dedup'd) have fixed-string counts ≥ 1, with **one exception found and corrected at this merge** (`HE7_PROOF_2026-08-08.md:HE3-BOX-6`, 12 → 1; source defect 16). A 10-designation sample was re-grepped at merge time against the working tree (§4) — 9/10 matched, the tenth is the corrected one. A 5-quote sample was re-checked verbatim against the source note by line number and matched byte-exact (§4). Five commit hashes were independently re-confirmed real commits via `git cat-file -t`.
- **The ID-renumbering rewrite was performed unit by unit against this compiler's own reading of both shards' full text** (not a blind global substitution), tracking the header's own disambiguation rule (a bare `.NN` defaults to its own shard; an explicit "shard 1"/"shard 2" label — or, once, a topic phrase naming a whole list — overrides it until the next `·` field-separator). An exhaustive regex sweep confirms **zero leftover `sKof2` substrings within the merged statement inventory (§3)** — the handful surviving in the header (§1) and in cross-citations to `spec/EFF-GENHN-s1of2.md`/`spec/EFF-GENHN-s2of2.md` are deliberate, per the header's own second exception clause (a bare `.NN` following a GENHN-shard citation is a GENHN ID, not an HE3 one, and is left untouched) — and **zero out-of-range renumbered IDs** (every resolved ID lies in 1–72, confirmed by an explicit scan).
- The SERIES (§7.1) is assembled from all four of its links with no duplication and no gap, cross-checked against three independent internal descriptions (`.02`'s own table, `.39`'s confirmation, `.64`'s confirmation). Its TERMINAL (1/2, not accepted) is uncontested — no fifth link exists anywhere in the merged inventory.
- Absorption status (predicate A), the append/consumption protocol (predicate B — "report, don't append", stated in the note's own words and never contradicted), and the inbound-corrections predicate (predicate C, four streams) are kept distinct everywhere below, matching both source shards' own discipline.
- **Two newly-surfaced outbound edges — HE7 ← HE3-BOX-6 and T2 ← HE3's stage-α passage — are carried as explicit reverse-XREF rows (§5) and their associated OPEN-CALLs (7 and 9) are carried forward unresolved**, per the standing charge. The first edge's citation count required a correction at this merge (source defect 16); the second re-verified exactly as recorded (count 65).
- **Twelve raw OPEN-CALLs carry forward as twelve distinct** — no self-flagged duplicate pair exists between the two shards (checked by grep, §8 preamble); one substantively-close pair (5/8) is flagged as related but not merged, matching the charge's instruction to dedup only self-flagged pairs.
- **Honest limitation of this merge, stated once:** the twenty-plus individual BOUNDARY SELF-AUDIT arguments in each shard's own §6 (which unit-boundary candidate was chosen and why) are not reproduced here — only their format-level conclusions (the SERIES construct, the SUPERSESSION KIND vocabulary, the two v2-gap proposals each shard's own §6.3 catalogued: `consumer-pin freeze` as a fourth freeze predicate, a `box-narrowing`/`RETIRES:` field, and an `instrument-audit` type for self-found instrument defects) survive here. A reader who needs the per-boundary reasoning for a specific unit should consult that unit's home shard file directly (`spec/EFF-HE3-s1of2.md` for `.01`–`.38`, `spec/EFF-HE3-s2of2.md` for `.39`–`.72`); the shard files are retained for exactly this purpose (§1). **Codex cross-read remains owed** (header, PROJECT_STATE #51) — this merge performed the fold and the fresh-verification pass, not the adversarial read.

---

EFF-HE3 MERGED: 72 statements / 46 xrefs / 12 open calls
