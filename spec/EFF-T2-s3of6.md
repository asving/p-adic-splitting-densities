...
Two of the acceptance record's four standing conditionalities are settled inside this range, and its wording is the governing statement of both:
...
> “**This round makes a theorem-statement change**, and one only: (RISE) joins the root-preserving continuation assertions hypothesised by HE7.A, and clause 3's product-\(1\) transition is thereby narrowed to the refine step at the side's own slope, with `(OCC)` read at \(\lambda\in H_{\mathrm{lift}}(B)\). A hypothesis is added; no case and no conclusion is added.”

— and it **removes one obligation** from the ledger, r6 repair 2's retirement of `(MP1′)`, which is a narrowing of the fenced set, not a widening of the supply set.

**APPEND/consumption PROTOCOL scope.** No round in this range runs a downstream consumption check. Each declares the source-side protocol it does discharge, r7's being the strictest form:

> “No source file is edited by this round; the expected source edit count remains zero, and no pin is added or removed, so the expected pin count remains 32.”
...
**RESOLUTION TRACE:** source lines 3285–3293; header at 2916–2925 (shard 2, `.13`); superseded in substance by r7's counter (3499–3509) and as to status by the acceptance record.
...
**DERIVATION:** `[MASTER-SIDE IMPLICATION]` Three master data compose: `(SIDE-COUNT)` fixes \(d(\Phi(\rho))=\lambda\) on the class; `(SLOT-V)` with `(LIFT)` fixes \(d(L_k(s)(\rho))=k\); the ultrametric equality-at-unequal-values then leaves \(\min(\lambda,k)\le\lambda\) at every \(k\ne\lambda\). Since (RISE) demands a value strictly above \(\lambda\), only \(k=\lambda\) survives. The direction of the implication is load-bearing: the derivation runs (RISE) ⇒ (HT) and never the reverse, which is what makes the level-one instance discharge non-circular (.03).
...
|---|---|---|---|
| Z01 | `(REF-HT)` corroboration, the height line | `HE7_PROOF_2026-08-08.md` | `> s₂ ≠ 0). Let w be a (LIFT₂) polynomial at height λ₂ with residue s₂` |
| Z02 | `(REF-HT)` corroboration, the existence line | `HE7_PROOF_2026-08-08.md` | `> (it exists: λ₂ > T₂ satisfies (‡₂) by §S4.3) and Ψ̃^{(1)} := Ψ̃ − w.` |
| Z03–Z11 | S1.8C stage-α passage (9 lines) | `HE3_PROOF_2026-08-08.md` | `*Stage-α step.* If the read refines at (λ, s) (a residual root of` · `multiplicity m ≥ 2 — so the leaf is not decided at this node), then by` · `LEMMA HE3-2(b) the recentered key Φ″ has a polygon whose portion above` · `λ has length exactly m, the corresponding block of D′m roots is` · `own labels are read at Φ″ — **[r1, F5: specifically, at the part of` · `above-λ blocks' labels are NOT readable at Φ″ — they are read at Φ′` · `Apply the inductive hypothesis to the sub-block: it is the` · `label-class of a multiplicity-m system over the SAME stage ring, with` · `the same (e₁, f₁), so its pieces compose into the dictionary as claimed.` |
| Z12–Z14 | `HE6-SLOT-SEAM` slot law (3 lines) | `HE6_PROOF_2026-08-08.md` | `> ξ be ANY point of K̄₀ satisfying (T1) and (T2). Then` · `> dv(A(ξ)) = min_i(e₁v(a_i) + ih) EXACTLY, and the residue of the attaining` · `> class, read after division by ϖ(ξ)^{dv(A)}, is ι_ξ(γ)·η_ξ^{−q} **[r2 sign` |
| Z15–Z18 | `HE6-SLOT-SEAM` key-freeness and its correction record (4 lines) | `HE6_PROOF_2026-08-08.md` | `> irreducible ψ of degree f₁); it never uses Φ′(ξ) = 0. ∎* **Codex PE1` · `> finding 6 is the observation that GENHN-2 as PASTED is stated at roots θ of` · `> Φ′ only, while this note applies it at roots of f and at roots of the` · `> (generally reducible) test polynomials — an overconsumption cured by this` |
| Z19–Z25 | `HE6-FAMILY` recipe and assembly (7 lines) | `HE6_PROOF_2026-08-08.md` | `> **DEFINITION HE6-1 (THE ENLARGED TEST FAMILY).** Let κ = u/ℓ ∈ ℚ in lowest` · `> terms with κ > D′h, and let r = Z^{d} + Σ_{t<d}c_tZ^t ∈ K[Z] be monic` · `> irreducible with r(0) ≠ 0. Choose, by (LIFT), B_t ∈ O[x] with` · `> deg B_t < D′, dv(B_t) = (d−t)u and res(B_t(θ)/ϖ(θ)^{(d−t)u}) = c_t (and` · `>     Ψ_{κ,r} := Φ′^{ℓd} + Σ_{t<d} B_t·Φ′^{ℓt}  ∈ O[x],` · `> monic of degree D″ := D′ℓd (each B_tΦ′^{ℓt} has degree < D′ℓd). For ℓ = 1` · `> this is HE3's recentered-key family enlarged in DEGREE; for ℓ ≥ 2 it is a` |
| Z26–Z29 | `HE6-PROOF` closing transfer paragraph (4 lines, = the span's last line at Z29) | `HE6_PROOF_2026-08-08.md` | `**This is HE3's LEMMA HE3-4 in full generality**, so HE3's THEOREM HE3.A` · `assembly (§S6.3 there: base read + stage-α + stage-CS + embedded genres) may` · `be run with HE3-4 replaced by THEOREM HE6.A — with the one caveat of §S7 for` · `histories containing an ℓ ≥ 2 side with a repeated residual factor.` |
| Z30 | `HE6-PROOF` first delimiter | `HE6_PROOF_2026-08-08.md` | `### S6.2 PROOF OF THEOREM HE6.A` |
| Z31 | `HE6-PROOF` boundary witness (next sibling unit) | `HE6_PROOF_2026-08-08.md` | `### S6.3 Multiplicity ≥ 2 on an ℓ = 1 side (the stage-α node) is unaffected` |
| Z32–Z34 | (RISE) level-one legs | `HE3_PROOF_2026-08-08.md` | `λ has length exactly m, the corresponding block of D′m roots is` · `P(F″) STRICTLY ABOVE λ, which by the corrected LEMMA HE3-2(b) is the` · `Termination: each refine strictly increases the floor λ and dv-heights` |
| Z35–Z38 | (RISE) level-two legs, LEMMA HE7-13 | `HE7_PROOF_2026-08-08.md` | `> **LEMMA HE7-13 [r2] (the strict slope increase along the refine chain —` · `> DERIVED, not assumed).** Let Ψ̃ = Ψ − W with W = 0 or as in LEMMA` · `> Then EVERY root ρ of f_S has` · `>     dv₂(Ψ̃^{(1)}(ρ)) > λ₂    (possibly = ∞) ,                     (S5.8.2)` |
| Z39 | (RISE) chain convention, LEMMA HE7-8 | `HE7_PROOF_2026-08-08.md` | `> Ψ^{(j+1)} = Ψ^{(j)} − w_j with dv₂(w_j) = λ₂^{(j)} and` |
| Z40–Z41 | `HE3-A-PROOF` delimiters (re-verified in this range) | `HE3_PROOF_2026-08-08.md` | `### S6.3 PROOF OF THEOREM HE3.A` · `leaf's history, so THEOREM HE3.A is proved. ∎` |
| Z42–Z43 | `HE7-REF-TERM` delimiters (re-verified) | `HE7_PROOF_2026-08-08.md` | `> **LEMMA HE7-8 ((REF-TERM) — α-refine chains are FINITE, characteristic-free).**` · `μ₂ ≥ 2 this makes disc f_S = 0. ∎` |
| Z44 | r8 repair 1(a) family line already at Z25; r8's second HE3 leg | `HE3_PROOF_2026-08-08.md` | `*Stage-α step.* If the read refines at (λ, s) (a residual root of` (re-verified at its r8 use) |
...
|---|---|---:|
| `then \(e'=1\) and \(r'=Y-s\) for some \(s\in K\). The required refinement height lies in` | 1–2912 | 1 |
| `### The one configuration the transport does not reach:` | 1–2912 | 1 |
| `The two are independent of the level-two derivation above` | 1–2912 | 1 |
| `Each of the six master-internal landmarks this repair supersedes or anchors on` | 1–2912 | 1 |
| `> Then \(G'\in O[x]\) is monic,` | 1–2912 | 1 |
| `the one hypothesis of HE6R1-3 that mentions the key at all` | 1–2912 | 1 |
| `> &\texttt{HE6R1-3}&\text{LEMMA HE6R1-3}\\` | 1–2912 | 1 |
| `The refinement height is that slope:` | 1–3293 | 1 |
| `> \boxed{k=\lambda .}` | 1–3293 | 1 |
| `` `(REF-HT)` is forced, not chosen, and the forcing uses master data only. `` | 1–3293 | 1 |
| `at every root of the class: at \(k>\lambda\) the value does not move at all` | 1–3293 | 1 |
| `The configuration is this master's, exactly.` | 1–3293 | 1 |
| `Since the S3.1 record is exactly the claim that HE3.A's data instantiate this master's S1.2–S1.7` | 1–3293 | 1 |
| ``so they inherit whichever of `(MP1)`, `(MP1′)` the state carries without further amendment`` | 1–3293 | 1 |
| ``Three further pins, `HE3-R8-2`, `HE3-R8-3` and `HE3-R8-4`, carry a quoted`` | 1–3293 | 1 |
| `The other 14 pins are description-only at both ends.` | 1–3293 | 1 |
| ``supplied inside the existing pin `HE3-A-PROOF`, whose §S6.3 stage-α step is the passage S1.8C quotes`` | 1–3509 | 1 |
| `A recentering at \(k\ne\lambda\) is excluded there by the display above rather than by fiat` | 1–3509 | 1 |
...
| r6/r7 round records (.01, .07) | Historical as to counter | Obligation set final at two | n/a |
| r8 header (.08) | Historical as to counter; boundary unit | Repairs 2–4 and counter in shard 4 | n/a |
| Withdrawals register (.09) | Effective | Each withdrawal removes a support, not a conclusion | Withdrawn citations must not re-enter the sweep |

**Reverse-XREF audit.** No unit homed in this range names a downstream consumer note/designation pair; the acceptance record addresses consumers generically. No reverse-XREF row is invented.
...
|---|---|
| (HT) without (RISE) | Do not read `(REF-HT)` as a single stipulation or as a derived fact: (HT) is derived **from** (RISE), and (RISE) is assumed. |
| (RISE) from the master | Do not derive (RISE) from the “new valid development” sentence, from `(SEC-RANK)`, or from an unnamed assertion “hypothesised by HE7.A”. All three supports are withdrawn. |
...
**Zero-orphan result:** 0 statement teeth orphaned; 0 inverse teeth orphaned; 0 of 0 numbered battery rows.
...
|---|---|---|
| 2926–3005, r6 repair 1 | (a) `(REF-HT)` installed in S1.8; (b) same binding in the S2 clause-3 proof; (c) S1.8B Step 1 rebuilt and the corroboration demoted | Base S1.8, S2 proof, S1.8B → shard 1 (.19, .24, .31); the derivation and the withdrawal **local: .02, .09** |
| 3007–3141, r6 repair 2 | S1.8C installed in place of the `(MP1′)` subsection; four propagation amendments; `HE6-PROOF` delimiters; taxonomy update | Base S1.8C, S3.1/S3.2, S4.1, S4.2, S5 → shard 1 (.25, .33, .34, .38–.40); the adjudication **local: .04**; the delimiters and totals **local: .06** |
| 3145–3159, r6 repair 3 | S1.7B unconditionality sentence scoped | Base S1.7B → shard 1 (.18, which quotes the r6 form verbatim) |
| 3163–3179, r6 repair 4 | “six” → “seven” in r5 repair 2's verification remark | Destination is r5 repair 2's remark (shard 2 range, recorded at shard 2 §3B); bookkeeping only, no unit |
| 3183–3209, r6 repair 5 | (a) two paragraphs relocated inside S1.8; (b) the third `###` unit numbered S1.8C | Base S1.8 → shard 1 (.23, .25) |
| 3213–3241, r6 repair 6 | (a) S1.8B scope clause; (b) r5 repair 6's justification sentence re-pointed | (a) **local: .05**; (b) destination shard 2 `.12`, where the supersession is applied |
| 3245–3281, r6 repair 7 | (a) item-(11) phrase; (b) `(MID-MASS)` comma; (c) `HE6R1-3` row description; (d) asymmetry adjudicated moot | (a),(b) base S1.8/S1.8B → shard 1 (.23, .24); (c) **local: .06**; (d) recorded in .04's conditionality |
| 3285–3293, r6 counter | Two objects, two obligations, counter reset | **Local: .01** |
| 3297–3303, r7 header | Arm, pass of record, protocol, GAP framing | **Local: .07** |
| 3307–3365, r7 repair 1 | `(REF-HT)` split into (RISE)+(HT); three supports withdrawn; honest status of (RISE); non-circularity | Base S1.8 → shard 1 (.19); derivation **local: .02**; discharge map **local: .03**; withdrawals **local: .09** |
| 3369–3395, r7 repair 2 | `HE6-FAMILY` booked first-only; taxonomy 15+4+13 | **Local: .06** |
| 3399–3413, r7 repair 3 | Stale `(MP1′)` disjunct removed from r5 repair 2's closing paragraph | Destination shard 2's r5 repair 2 text; recorded in .04's resolution trace |
| 3417–3433, r7 repair 4 | Configuration clause rebuilt; LEMMA HE3-2(b) claim withdrawn | Base S1.8C → shard 1 (.25); **local: .04, .09** |
| 3437–3465, r7 repair 5 | S1.8C's four pins added to the three level-two Source-pin lines | Base S3.3–S3.5 → shard 1 (.35–.37); settings superseded at r10–r12; ground recorded in .04 |
| 3469–3493, r7 repair 6 | (a) frame framing; (b) engine sentence re-based on r1 repair 2's credit | Base S1.8C → shard 1 (.25); **local: .04** |
| 3497–3509, r7 counter | Statement-change booking; obligations; landmark census; reset | **Local: .07** |
| 3513–3519, r8 header | Arm, pass of record, protocol, GAP framing, correction to the report's cure | **Local: .08** |
| 3523–3567, r8 repair 1 | (a) level-one discharge restated step-first; (b) two height grounds; (c) S1.8C honest-status clause; (d) non-circularity restated | Base S1.8/S1.8C → shard 1 (.19, .25); **local: .03, .04, .09** |
...
|---|---|
| .02, .03 | Split at the derivation/premise seam, which is the whole point of r7: `.02` is the master's implication (RISE) ⇒ (HT), `.03` is who supplies the premise. Merging them would restore exactly the conflation r7 was written to remove. |
| .03, .04 | Separate objects at the same instance: `.03` supplies a hypothesis clause of HE7.A, `.04` supplies a frame that lets a state instantiate HE7.A at all. r8 repair 1 amends both, which is why the boundary is stated rather than assumed. |
| .04, .05 | S1.8C and S1.8B are declared independent by the source itself (“This subsection and S1.8B are independent”), and a state may be in both configurations at once. |
| .06 | Bookkeeping is separated from the adjudications that produced it because its totals are superseded downstream while the per-pin operations are not. |
| .01, .07, .08 | One unit per round record; r6's header sits in shard 2 and is cross-referenced rather than duplicated. |
| .09 | The withdrawals register is a cross-repair unit by construction: its function is only visible when the six retractions are seen together. |

**Leading edge.** Line 2925 is blank; line 2926 opens `## r6 repair 1`, the first repair of a round whose header (2916–2925) is compiled in shard 2 as a flagged boundary unit. The round is therefore split across the edge by design, and both halves name each other.

**Trailing edge.** Line 3567 is `---` and 3568 is blank; line 3569 opens `## r8 repair 2`. The r8 round is likewise split: its header and repair 1 are compiled here, its repairs 2–4 and counter in shard 4. No statement unit is physically truncated: r8 repair 1 ends at 3565 with its landmark remark.

---

## 9. SHARD NOTES

- **Cross-shard supersessions applied (downward):** the acceptance record's counter (2/2), its `(RISE)` and `(MP1′)` bullets, the pin inventory (37) and taxonomy (\(20+4+13\)); r10 repair 1(f), r11 repair 1(e) and r12 repair 1(f)'s re-settings of the Source-pin lines that r7 repair 5 amends here; r12 repair 1's master-side re-derivation of the \(w\ne0\) degree conjunct that `.05` consumes from `HETOW-KEY-BRIDGE`; r9's `(IDX-TWIST)` and Rider 1's lift-choice qualifier, which later govern the print/coherent reading of the same recentering step `.03` exhibits.
- **Cross-shard supersessions applied (upward, into shard 2):** r6 repair 4 corrects a count word in r5 repair 2's verification remark, and r6 repair 6(b) replaces one justification sentence of r5 repair 6. Both were applied to shard 2's units at compilation of this shard; shard 2 `.12` now carries the r6 replacement.
