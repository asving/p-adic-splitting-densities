...
| HYP.08 | cubic tail recurrence | **DISCHARGED-BY** (landed) `Uniformity.Density.rate_three` (`ChapH/H97b.lean`, constants `(1,0,3)`; the actual-set recurrence is `undecidedCount_three_recursion`) | STALE-ROW CORRECTION 2026-08-24 (U10 audit, orchestrator-verified Lean-core). H.97's literal `(1,1,0)` sharpening remains available as 2 BLUEPRINT nodes in `N3_DRAINAGE_BLUEPRINT_2026-08-24.md` — only needed if a consumer requires that exact species. Math record: `N3_DRAINAGE_PROOF_2026-08-24.md` (u_N ≤ q⁻ᴺ + q⁻⁵u_{N−3}, uniform incl. p = 2, 3). |
| HYP.09 | cubic triple-root drainage | **DISCHARGED-BY** (landed) `Uniformity.Density.drainageAt_three : DrainageAt 3` (`ChapH/H97b.lean`) | STALE-ROW CORRECTION 2026-08-24: unconditional, Lean-core footprint (orchestrator re-verified). The P5 set (HYP.04–06) discharged as recorded; nothing remains open in this row. The Haar-null refinement is OPEN (no measure carrier) and NOT needed for `DrainageAt 3`. |
