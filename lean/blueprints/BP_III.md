...
| III-U3b | induction step, candidate-pair identification (order ≤ 1 outright via A6/G-layer; ≥ 2 fires hGRB + hFRESH) — LANDED 2026-08-01, records synced 2026-08-02 (CU1.lean: `CandidatePairAt` + `cu1_stepPair_le1`/`_ge2` + `cu1_stepPair` + GD-2 instance + `ol6Gate` positive gate, all proved, Lean-core; the ≥ 2 leg fires ONLY `FRESH.childDetermined` (guarded form) + `GRB.residualNormalForm` — H2-U10's twin field list is thereby FIXED, consumption-shape display in the `cu1_stepPair_ge2` docstring) | (c1)/(c2) → window principal face + ψ̂; GD-2/GD-3 instances | III-A6, III-G21, III-H1, III-H2 | HARD | CU1 §3 step |
