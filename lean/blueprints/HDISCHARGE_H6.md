...
the built corpus on 2026-08-05 (machine clock 2026-08-01), transient probe files
(deleted; imports: `Scaffold.DictIII.{CU2t, Hyps, CU1}`, `MovesU.DefsLedger`,
`MovesT.V9_irrSat`, `Scaffold.ValueSide.Hyps`, `HC2.Defs`) — FOUR probe rounds:
draft statements, the D-9 self-catch repair, the machineEHist footprint repair,
and the FOLD-ROUND restatements after the Codex review (A5/A6/A6b-signature/A7/
B0/B1-body/EngineCovers/TerminalSeamCore/TerminalSeamRows/TDecRow + the
S5b-shaped binder context). Zero errors each round (B1's probe used placeholder
law proofs; one probe-local `sorry` in a placeholder body — no landing content).

**STATUS: fold round 1 EXECUTED (all 16 Codex findings folded — §5); the
repaired blueprint owes a FRESH Codex re-review (pass 2) BEFORE any prover
fan-out. Verdict history: pass 1 = REJECT (9 criticals, all accepted-and-folded
or cured-pre-review); pass 2 = OWED.**
...
* **D-8 (`DecIrr` index/position drift).** Landed `DecIrr`: ∃ position k ≥ 1
  ...
  reads with THE LAST saturated. (a) [SUSPECT — downgraded at fold round 1,
  Codex finding 14: needs the base-row representation lemma (does the level-1
  read sit at list position 0?); unit R1 gains that check] `1 ≤ k` at 0-indexed
  lookup would exclude single-read (k′ = 1) saturated seams IF position 0 is the
  level-1 read; (b) [STANDS] ∃-any-position vs last-position (a mid-history
  μ = 1 selection at a non-final node satisfies the landed form). Whether the
  W-laws collapse (b) is NOT on record; as bare defs on `EHist` drift (b) stands.
...
**τ-hen leg (REPAIRED at fold round 1, Codex finding 1 — the first-draft a₀ ≥ 2
guard silently MOVED the a₀ = 1 obligation out of the group, a fiat-discharge).**
The base booking (a₀ = 1 or j₀ = 1) is the root reduction datum (MovesT §2.3
(τ-hen)); at a₀ = 1 the seam object is (ρ₀) + machine record with NO terminal
ENodeData (CUC §9.1 carrier note, R4 M-γ: W3 permits a terminal NODE only at
a₀ ≥ 2). The repair: the emission interface carries a SECOND, verdict-pair-only
channel `emitsHen : f → 𝐇° → (ℕ × ℕ) → Prop` (the machine-record booking — no
node asserted), and `tDECdec`'s hen leg demands `∃ EF, emitsHen f H EF` at EVERY
DecHenSeam INCLUDING a₀ = 1, with `tVERDhen` pinning that pair to (1, f₀). The
obligation stays in this group at the correct carrier; nothing is guarded away.
...
ns" lemma exists on record is UNVERIFIED — if absent, B3b states it as a NAMED
HYPOTHESIS ROW `LeafCatalogue` (displayed Prop, part of this group's residue —
Codex findings 9/13: a sorry there would silently discharge (T-DEC-cor); a named
row keeps the conditional honest) with the inventory routed to the GD-4 owner
brief (CUC §9.4 scope note: the §9.2a inventory closure rides the same note). (b) corner-unreachability
...
the failure is a compiled obstruction to report, not to paper over. MED-HARD
(retagged at fold round 1, Codex finding 12: until the root-node/dressed-history
reconciliation is CHECKED, B2, tVERD's irr face, and every downstream row remain
conditional — the unblock path must not be advertised as quick).
...
(exactly the escalation the ROOT row fences). (ii) A constructor-chosen
six-Prop-slot structure — REJECTED AT FOLD ROUND 1 (Codex finding 8, CRITICAL,
accepted): caller-selected Prop slots + a conjunction field are True-instantiable
BY THE PROVER, so as an assumed row the structure asserts NOTHING about the
built pack — unlike the RootHyps precedent, where the bare Prop is a FIELD of an
assumed record (fixed by the assumer, opaque to the prover). (iii) CHOSEN
(post-fold): item (vii) keeps its ONLY honest Lean face at HEAD — the ALREADY
LANDED bare field `RootHyps.h6_vii : Prop` (assumed, never constructed) — and
this group lands NO FenceVII structure until the value-side H-LIST/pack-builder
carrier exists. Unit D2 reduces to a DOCUMENTATION unit (the intended content
displayed at the h6_vii docstring level: six-fence roster, sealed flags,
escalation fence) + a BLOCKED-ON-CARRIER record for the typed row; the eventual
typed `FenceVII` must state a FIXED external proposition about the BUILT pack
(each fence a closed predicate of the pack/H-LIST datum, no caller-selected
slots), and its general-n proof remains [M] regardless. The n = 3 sealed
...
docstring verbatim. Consumption keying: Step 18's K3-a — at HEAD the row rides
ONLY as `RootHyps.h6_vii` (bare named assumption, assumed never constructed);
the typed `FenceVII` and BP_V's `RootRows.FenceVII n` body are
BLOCKED-ON-CARRIER (units D2/D3, fold round 1).
...
File plan: `Emission.lean` (A1–A7 + A4b + A6b), `EngineConform.lean` (B0–B3),
`TerminalRead.lean` (C0–C2), `RowsK4.lean` (D1a/D1b), `Fence.lean` (D2,
documentation-only), `RowStatements.lean` (D3–D4), `Reconcile.lean` (R1). Standing variable block per file:
...
proposal the III-H9 BLOCKED block requested — and, since the parallel BP_III
wave-5 landing of 2026-08-05 (commit aa5d50e, 3/13 units), ALSO the answer to
`DictIII/CU2t.lean`'s BLOCKED(III-S2)/(III-S3) records, which hit the same
accE/accF carrier seam; the BP_III resume record defers that mop-up to this
lead):
```lean
...
**A4b (EASY, added at fold round 1 — Codex finding 4):** the two read notions
cohere: `terminalDatumD_slope_spec` — under `ReadThroughIota f H D` + the III-S4
forcing, the side list at level k′+1 is a singleton, so the `head?`-read slope
...
```
(Read clause in the NONEMPTY + ∀-SIDES form — fold round 1, Codex finding 3;
consumed under `ReadThroughIota`, so the sides are the real read's.) Deps: A4. Non-circularity displayed in the docstring (D is polygon data of f
...
round 1 repair of Codex finding 1; see §3.1.) Deps: —.

**A6b (MED, PINNED-SIGNATURE — body is the unit's E-phase duty under §4 C0-style
constraints).** The GMN-through-ι certificate (fold round 1 repair of Codex
finding 2 — without it every ∀-D row is refutable at lawless `GMNData`):
```lean
def ReadThroughIota (f : Polynomial ℤ_[p]) (H : EHist p F)
    (D : GMNData f (Theta H)) : Prop := …
```
Pinned content: D's side lists at levels 0..(H.nodes.length) are the REAL
polygon reads of f over Θ(𝐇°) through ι — level 0 = the corpus base Newton
polygon w.r.t. the ψ̂₀-key (order-0, corpus-constructible: `OM/` polygon API +
O2aOrder1's constructor — REAL content, no freedom), levels ≥ 1 given OL-1
(the CUC (c1) bracket verbatim: "level 1 unconditional, levels ≥ 2 given
OL-1"). Pinning demands (per the C0 rule): inputs (f, H, D), the defining
clauses quantifier-explicit, a positive gate (the canonical order-≤ 1 GMNData
satisfies it) and a negative gate (the lawless `⟨fun _ => [], …⟩` fails it) land
WITH the unit. Every A7 row takes `ReadThroughIota f H D` + `ConsF f H D R` as
antecedents (the CUC standing hypotheses "Cons_f(𝐇°)" + "GMN objects through ι"
transcribed).
...
```
Deps: A2, A3, A4, A6, A6b. FOLD ROUND 1 restatement (Codex findings 1/2/3):
every row carries the CUC standing hypotheses (`ReadThroughIota` + `ConsF` —
"Cons_f(𝐇°)" and "GMN objects through ι" transcribed; kills the lawless-D
refutations); tDECdec covers BOTH channels including a₀ = 1 (via emitsHen);
tREAD's conclusion is the NONEMPTY + ∀-SIDES form (immune to junk-extra-side
satisfaction — with the forcing lemma III-S4 the list is a singleton, so this
is exactly "the fields are THE unique side's datum" without presupposing the
forcing); tVERDhen pins the machine-record channel to (1, f₀). Field ↔ CUC row
map in the docstring; the RC tie (emits + DecSeam + tREAD + tVERD ⇒
`RCConsistentD`) is unit D1's lemma. These rows are [M]: statement-only here;
discharge = waves B/C at the canonical engine + the ROOT-level adjudication.
...
**B1 (MED).** The canonical site/verdict emission — EXACT relational body
displayed (fold round 1, Codex finding 10: "existentially free" was not a
compile-probed spec; this is, probe GREEN):
```lean
noncomputable def engineEmissionSV (n p : ℕ) [Fact p.Prime]
  ...
  emitsHen f H EF := H.nodes = [] ∧ EF = (1, H.psi0.natDegree) ∧
      ∃ M : MovesC.History p F, MovesC.HistoryCoherent M ∧
        ...
        (machineEHist M).continuingPart = H
  emits_terminal := …  -- (h.1)
  emits_reaches := …   -- project the ∃-witness
  emitsHen_reaches := …
  reaches_continuing := …  -- from machineEHist.continuingPart (unit proof)
```
HONESTY DISPLAY (unchanged in substance): ν's SHAPE fields are UNCONSTRAINED in
this SV body — every sel-none ν is admitted at a decided irr seam. Consequences,
displayed: `tREAD` at `engineEmissionSV` is REFUTABLE (junk shapes) — the trio
is NOT claimed at SV; SV exists to discharge tDECdec/tDECcor/tVERD-shaped work
early. The C0 upgrade `engineEmission` (ν := the TRM record) is where tREAD
becomes provable. Prove the four interface laws + `EngineTied n p F
(engineEmissionSV n p F)`. Deps: B0.
...
home CUC §9.4 says is "purely owed"). FOLD ROUND 1 (Codex finding 11 accepted —
unconstrained signature freedom would let the def manufacture a record from a
selected D and prove a tautological self-spec): the OBSERVABLE I/O IS PINNED
HERE, only implementation is free. PINNED: inputs = `(M : MovesC.History p F)`
(the leaf's machine history) + `(hM : HistoryCoherent M)` + `(hR :
LeanUrat.MovesJ.ReadsOf p F n f M)` + f itself — NO GMNData input (the record
must be READ FROM f's development in the terminal frame, not copied from
semantic data); output = `ν : ENodeData` with `ν.sel = none`; demanded theorems
= (spec-1) `SideReads`-style conformance of ν's (e, h, ℓ, s, u) to f's
development in frame `M.nodes.length` (the corpus `IsDevelopment` + side-read
vocabulary at the terminal index, no successor key), and (spec-2) the emission
tie: `engineEmission`'s emits holds exactly at (ν = terminalReadRecord …) —
upgrading B1's SV body; `EngineTied` re-proof rides along. tREAD's discharge
then = spec-1 + the A6b certificate bridging development-reads to
`D.principalSides` (C1/C2's content). Touches NO fenced MovesC/MovesT
statement. Deliverables: def + spec-1 + spec-2 + upgraded emission.
...
**D1a (EASY).** THE SEMANTIC (H6) CORE ROW — the discharge statement (fold
...
PROVISIONAL syntactic unblocker, D1b). The first-draft ∀-over-EngineTied form
was REFUTABLE (self-caught pre-review as defect D-9; independently found as
Codex finding 5 on the pre-patch text — `EngineTied` deliberately leaves shape
fields untied, and also admits no-emission interfaces): the closed faithful
form is EXISTENTIAL-WITH-COVERING —
```lean
...
`gate_readsOf_inert2` anchors non-vacuity; adversarial junk emissions are not
demanded to conform, only the WITNESS is; discharge shape: C0's TRM + waves B/C
construct the witness). Plus the RC tie lemma
`rc_of_rows : TerminalSeamHypsE … → ReadThroughIota f H D → ConsF f H D R →
E.emits f H ν EF → DecSeam f H D → RCConsistentD f H D ν EF`
(the §9.4 tie — the repaired III-S8). Deps: A7, B0.

**D1b (EASY, PROVISIONAL — labeled so in its docstring, per the Codex D1-keying
recommendation adopted verbatim).** The K4-facing syntactic unblocker:
```lean
...
```
Probed GREEN in the S5b-shaped binder context `(seam : TreeSeam n p X FF)
(K4 : TerminalSeamRows n p X FF)`. The (X, FF) keys are consumption-site keys
ONLY — `ClassifierSpec` is abstract (no run structure), so the label tie ("X's
leaf labels are the engine's announced verdicts") is NOT statable at HEAD; it is
a REGISTERED FORTHCOMING FIELD: when Movement I/BP_VI exposes the classifier's
run structure, `TerminalSeamRows` GAINS the label-tie field (a consumer-visible
strengthening — the BP_IV seam is flagged NOW, in this blueprint and in the
docstring, so S5b/D4 land against the binder knowing the field is coming; they
consume the row opaquely, so re-landing is a re-elaboration, not a re-proof).
Until that field exists, the ROOT-level conditionality display must keep naming
the label tie as carried by the BP_VI instantiation seam, NOT by this row —
docstring carries this sentence verbatim. Deps: D1a.

**D2 (MECH, DOCUMENTATION-ONLY — restructured at fold round 1, Codex finding 8
accepted; the six-Prop-slot device is WITHDRAWN).** Item (vii)'s Lean face at
HEAD stays the LANDED bare field `RootHyps.h6_vii : Prop` (assumed, never
constructed — the honest device). This unit lands: (a) the documentation block
(at `Scaffold/HDischarge/H6/Fence.lean` as comment + a re-displayed intended
content note for the h6_vii docstring owner): the six-fence roster with H.x
pointers ((1) XHD-d-EX(∂) H.2.3 · (2) M4b-T-AUD H.5.1 · (3) U-R-COUNT
H.4.2/H.5.3 · (4) XHD-u-X H.4.3 · (5) H-LIST-GEN beyond (TRI) · (6) E-POS
⟨w_E, v⟩ ≥ 1), the sealed-flag note (V-n3 178/178, H-n3 151/151, off-census
q = 16/25 — evidence, not proofs), the escalation fence; (b) the
BLOCKED-ON-CARRIER record for the typed row: the eventual `FenceVII` must be a
FIXED closed predicate of the BUILT pack/H-LIST datum (no caller-selected Prop
slots), constructible only when the value-side H-LIST/pack-builder carrier
lands (D-11/BP_IV territory); its general-n proof remains [M] regardless. NO
new Lean declaration lands in this unit. Deps: —.
...
to re-export when it lands; NO local alias of any BP_V name). FOLD ROUND 1
(Codex finding 6 accepted — the first-draft ∀-over-EngineTied bodies inherited
the D-9/finding-5 error): the three faces are now ∃-covering forms sharing the
core's witness SHAPE (probed at `TDecRow`):
`TDecRow n := ∀ (p) [Fact p.Prime] (F' : Type) [Field F'] [Finite F'], ∃ E,
EngineTied n p F' E ∧ EngineCovers n p F' E ∧ (A7's tDECdec + tDECcor faces)`;
`TReadRow n` / `TVerdRow n` likewise at the tREAD resp. tVERD + tVERDhen faces.
Glue lemma `rows_of_core : TerminalSeamCore n p → (each face)` — one witness
projects to all three. DISPLAYED SEAM NOTE for the BP_V lead: the three faces
at INDEPENDENT witnesses are jointly weaker than `TerminalSeamCore` (different
E per face); if any Step-18 consumption site needs the JOINT witness, it must
consume `TerminalSeamCore` — route the check to the BP_V spine before HypRows
lands. `FenceVIIRow n`: POSTPONED with D2 (BLOCKED-ON-CARRIER; BP_V's
`HypRows.fenceVii` field keeps its own bare named row per ITS charter until the
typed carrier exists — its call, our record). E-phase probes each body before
landing. Deps: A7, B0, D1a, D2.
...
* **G1 (non-vacuity countermodel, MED, lands after A6b — retagged at fold
  round 1, Codex finding 15):** a compiled `E_junk` with
...
* **G2 (positive gate, MED-HARD, lands after B1 + A6b):** the SAME certified
  k′ = 0 seam with an emission satisfying every row NON-VACUOUSLY — a witnessed
  reach AND a witnessed emitsHen with EF = (1, f₀) (HK23_twoNodeGatePos style).
  The all-False emission satisfies `TerminalSeamHypsE` vacuously, so the gate
  MUST exhibit the positive witnesses; it additionally needs `EngineTied` +
  `EngineCovers` versions at a realized `ReadsOf` history (U31's
  `gate_readsOf_inert2` anchor) for the D1a-shape non-vacuity.
...
A4 (A1,A2) → A4b (A4, after III-S4) ; A5 (A4)
A6b (A2; corpus OM/O2aOrder1)
A7 (A2,A3,A4,A6,A6b) → G1 (A7,A6b) ; R1 (A2)
B0 (A2,A6) → B1 (B0) → G2 (B1,A6b) → B2 (A1,B1) ∥ B3a (B1) → B3b (B1,B3a)
C0 (B1) → C1 (C0,A6b) ∥ C2 (C0)
D1a (A7,B0) → D1b (D1a) → BP_IV S5b/D4 UNBLOCK NOTICE to the value-side owner
D3 (A7,B0,D1a,D2) → D4 (D3)
```
Critical path to unblocking BP_IV: A-wave → A7 → D1a → D1b (EASY chain, but
A6b is MED and on it — budget accordingly). The open-mathematics core: C0 → C1
(HARD; (T-READ)'s k′ = 0 leg). Item (vii)'s general-n content acquires NO proof
unit here (correct: it is [M]; §3.5).

**Unit count: 22** (A1–A7 + A4b + A6b [9], B0/B1/B2/B3a/B3b [5], C0–C2 [3],
D1a/D1b/D2/D3/D4 [5, D2 documentation-only], R1 [1] — count corrected at fold
round 1, Codex finding 16: the first draft said 18 while listing 20) **+ 2
compiled gates (G1/G2, now units in the graph) + the G3 documentation flag.**

---

## §5 CODEX ADVERSARIAL REVIEW — PASS 1 + FOLD ROUND 1 (2026-08-05)

Review ran fresh-context on the PRE-fold draft (charge: quote-and-classify,
fiat-discharge hunt, quantifier audit, defect-census check, both design
questions; full text archived at `lean/notes/openmath/H6_bp_review.md`).
**Verdict: REJECT — 9 CRITICAL, 6 GAP, 1 ADVISORY.** All findings folded below;
the repaired blueprint owes a fresh pass 2 before prover fan-out. Timeline
note: finding 5 duplicates the lead's OWN pre-review self-catch (defect D-9,
patched before the review completed — Codex reviewed the pre-patch text; the
convergence is recorded, not claimed as a rebuttal).

| # | severity | finding (compressed) | disposition |
|---|---|---|---|
| 1 | CRITICAL | tDECdec's a₀ ≥ 2 guard silently MOVES the a₀ = 1 hen obligation out of the group — fiat-discharge; the carrier note justifies a different CARRIER, not removal | **ACCEPTED-FIXED**: `TerminalEmission` gains the verdict-pair-only channel `emitsHen`; tDECdec's hen leg fires at EVERY DecHenSeam incl. a₀ = 1; new row `tVERDhen` pins (1, f₀). §3.1 + A6/A7 restated, probed |
| 2 | CRITICAL | lawless `GMNData` breaks the rows both ways (trivial DecHenSeam witnesses; universal-D tDECcor refutable at empty-side D) | **ACCEPTED-FIXED**: new certificate unit A6b `ReadThroughIota` (GMN-through-ι, k′ = 0 face corpus-real, ≥ 1 face OL-1-conditional — the CUC (c1) bracket) + `ConsF` as standing antecedents on EVERY row; A7 restated, probed |
| 3 | CRITICAL | tREAD's ∃-side form is weaker than (T-READ) ("some listed side"; R unused; no forcing available to rc_of_rows) | **ACCEPTED-FIXED**: tREAD and RCConsistentD's read clause restated in the NONEMPTY + ∀-SIDES form (= "THE unique side's datum" once III-S4's forcing lands, without presupposing it); consumed under ReadThroughIota so sides are the real read's; probed |
| 4 | CRITICAL | terminalDatumD's head?-slope vs tREAD's witness — two disconnected read notions; no uniqueness supplied | **ACCEPTED-FIXED**: new unit A4b `terminalDatumD_slope_spec` (coherence under ReadThroughIota + III-S4, displayed obligation until III-S4 lands); RCConsistentD's ∀-form pins the same side |
| 5 | CRITICAL | ∀-over-EngineTied TerminalSeamRows is generally false (no-emission and junk-emission interfaces are EngineTied) | **CURED PRE-REVIEW (D-9 self-catch) + COMPLETED**: the ∃-covering form (`EngineCovers` + existential witness) already replaced the ∀-form before the review returned; the no-emission face is killed by covering + tDECdec. D1a carries the full record |
| 6 | CRITICAL | RootRows bodies inherit the ∀-over-EngineTied error; B/C waves prove only the canonical emission | **ACCEPTED-FIXED**: D3 bodies restated as ∃-covering faces of the shared core + `rows_of_core` glue; independent-witness weakness DISPLAYED as a seam note to the BP_V lead (joint-witness sites must consume `TerminalSeamCore`) |
| 7 | CRITICAL | phantom (X, FF) keys are not semantic signature fidelity — the same proof applies to any classifier; the label tie is lost | **ACCEPTED-FIXED per Codex's own recommendation**: D1 split — D1a `TerminalSeamCore` (no keys, THE discharge statement) + D1b `TerminalSeamRows` (explicitly PROVISIONAL syntactic unblocker; label-tie field REGISTERED as a forthcoming consumer-visible strengthening; ROOT display keeps the label tie at the BP_VI seam) |
| 8 | CRITICAL | FenceVII's constructor-chosen Prop slots are True-instantiable — as an assumed row it asserts nothing about the built pack; ≠ the RootHyps precedent | **ACCEPTED-FIXED**: the device is WITHDRAWN; item (vii) keeps the landed bare `RootHyps.h6_vii` as its only HEAD-honest Lean face; D2 reduced to documentation + BLOCKED-ON-CARRIER record; the eventual typed row must be a FIXED closed predicate of the built pack |
| 9 | CRITICAL | an honest sorry at B3b's catalogue lemma would silently discharge (T-DEC-cor) | **ACCEPTED-FIXED**: no sorry — `LeafCatalogue` lands as a NAMED HYPOTHESIS ROW (part of this group's residue), B3b's theorem conditional on it, routed to the GD-4 owner |
| 10 | GAP | B1's "existentially free" emission is not a compile-probed spec | **ACCEPTED-FIXED**: the exact relational body displayed at B1 and probed (junk-shape admission EXPLICIT; tREAD-at-SV refutability displayed) |
| 11 | GAP | C0 signature freedom lets the def prove a tautological self-spec | **ACCEPTED-FIXED per Codex's recommendation verbatim**: C0's observable I/O pinned (inputs M/hM/hR/f, NO GMNData input; output sel-none ν; spec-1 SideReads-conformance to f's development; spec-2 the emission tie); implementation only is free |
| 12 | GAP | B2's f₀ reconciliation understated as MED with a quick unblock path | **ACCEPTED**: retagged MED-HARD; downstream conditionality displayed at §3.4 |
| 13 | GAP | the leaf catalogue is part of H6's residue, not an implementation detail | **ACCEPTED**: folded with finding 9 (named row, priced in this table) |
| 14 | GAP | defect D-8(a) (index off-by-one) presumes the base-row representation; only D-8(b) is established by the quoted sources | **ACCEPTED**: D-8(a) downgraded to SUSPECT; unit R1 gains the representation-lemma check; D-1..D-7 + D-8(b) stand as accused |
| 15 | GAP | G1/G2 under-specified (existence of certified witnesses; vacuous-gate risk) | **ACCEPTED-FIXED**: explicit witness constructions demanded (k′ = 0, a₀ = 1, psi0 = X seam; certified D via A6b's positive face; EF = (2,2) countermodel resp. EF = (1, f₀) positive emission); G2's non-vacuity demand made structural |
| 16 | ADVISORY | unit count said 18, listed 20 | **ACCEPTED-FIXED**: recounted post-fold — 22 units + 2 compiled gates + 1 documentation flag |

**Design-question answers (Codex), adopted:** (D1 keying) do NOT land phantom
keys as the semantic row — split provisional unblocker from discharge statement
(EXECUTED, D1a/D1b). (C0 signature) pin the observable I/O now (EXECUTED, C0).

**Residue after fold round 1 (the honest state):** the four (H6) members remain
[M] hypotheses — nothing in this blueprint discharges them; the blueprint's own
NEW named rows (`LeafCatalogue`; A6b's OL-1-conditional face; A4b's displayed
coherence obligation) are priced as the group's Lean-side residue; item (vii)
has NO typed Lean carrier until the H-LIST/pack-builder lands. Pass 2 of the
Codex review is OWED on this repaired text before any prover fan-out; the
BP_IV unblock notice (D1b) and the BP_V seam notes (D3) go out only after
pass 2 accepts.
