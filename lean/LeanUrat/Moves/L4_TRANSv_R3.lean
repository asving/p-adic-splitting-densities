...
**Repair (MANIFEST_REPAIR L4.TRANSv, fence-change APPROVED).**  Statement amended with
`hcore : StageCore σ`; import re-pointed to the CLEAN `L4_TRANSviii_b_R4` (the
machine-checked-finding-1 correction, whose `prevIaug` law `e·wPrev(Φ) < h` clears the
(S6b) threshold).  TRANSv is a pure corollary, so it must carry the same `hcore`.
...
theorem L4_TRANSv {p : ℕ} [Fact p.Prime] {F : Type*} [Field F] [Finite F] (σ : Stage p F) (hcore : StageCore σ) (ψ : Polynomial ↥σ.K) (g : ℕ) (hg : ψ.natDegree = g) (hmon : ψ.Monic) (hψ : Irreducible ψ) (hψz : ψ ≠ Polynomial.X) (Φhat : Polynomial ℤ_[p]) (hlift : IsStandardLift σ ψ g Φhat) (zbar : Fˣ) (hzbar : Polynomial.eval₂ σ.K.subtype (zbar : F) ψ = 0) (a : F) (ha : a ∈ σ.nextField zbar) : ∃ B : Polynomial ℤ_[p], inC Φhat B ∧ σ.digPrime zbar B = a ∧ (a ≠ 0 → B ≠ 0) := by
  obtain ⟨B, hmem, hdig, hnz⟩ :=
    L4_TRANSviii_b σ hcore ψ g hg hmon hψ hψz Φhat hlift zbar hzbar (σ.w Φhat + 1) (by omega) a ha
