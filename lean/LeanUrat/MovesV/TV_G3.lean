...
kcol-guarded, and kcol outcomes are continuing). -/
...
discipline: this def may be filled only after the census verdict). -/
noncomputable def scsMemberOf {n : ℕ} (C : CtsFamily n)
    (T : MovesS.TableShape n) (hp : TablePins C T)
    (e : ℕ) (τ : T.State e) (o : T.Out e τ) :
    (Fin ((scsSupplyCore C T hp).flankCount e τ o) ⊕
      Fin (((scsSupplyCore C T hp).resFactors e τ o).erase
        ((scsSupplyCore C T hp).g e τ o,
         (scsSupplyCore C T hp).μsel e τ o)).card) →
    Fin (T.odata e τ o).mem.length :=
  sorry

/-- TV-G3 roster law 1 (mirrors `SCSData.memberOf_inj`). -/
theorem scs_memberOf_inj {n : ℕ} (C : CtsFamily n)
    (T : MovesS.TableShape n) (hp : TablePins C T) :
    ∀ e (τ : T.State e) (o : T.Out e τ),
      Function.Injective (scsMemberOf C T hp e τ o) := by
  sorry

/-- TV-G3 roster law 2 (mirrors `SCSData.memberOf_ne_sel`). -/
theorem scs_memberOf_ne_sel {n : ℕ} (C : CtsFamily n)
    (T : MovesS.TableShape n) (hp : TablePins C T) :
    ∀ e (τ : T.State e) (o : T.Out e τ) i,
      scsMemberOf C T hp e τ o i ≠ scsSelIdx C T hp e τ o := by
  sorry
...
Sum.inr status through the TablePins roster tie. -/
...
(skBlk sk = mu.size) + TV-G2's stage_D/stage_W. -/
