...
PROOF NOTE (this round). The route is NOT `L5_recLiftIndep_R4` (whose `RecenterSubstCore`
input is StageCore-gated and unavailable here); instead:
* `Φ' = Φ − lift` is the `g = 1` STANDARD LIFT of `ψ = X − C center` in the PARENT stage
  (the `L5_recVV` construction, rebuilt here without `StageCore.R_neg`: the sign
...
theorem NA_transport_recentering {p : ℕ} [Fact p.Prime] {F : Type*} [Field F] [Finite F]
  ...
  linear_combination hgt - (H.strFrame i : ℚ) * hLU
    + ((ν.μ : ℚ) - ((ν.s0 : ℚ) + (ν.wSide : ℚ))) * hsl
