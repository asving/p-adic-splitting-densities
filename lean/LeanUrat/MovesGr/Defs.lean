...
* **`GenuineStageModel σ`** is the R5.3 (1)-(4) bundle: the `SideVal` built from `σ.w`, the graded
  ...
  lacked. `L₀ ≅ ↥σ.K[z^{±1}]` is the last standing fiat — and (corrected 2026-07-30 verify-2
  fold-in) it is HARD-CODED in the codomain type of `Θ` (`LExt ↥σ.K = LTwo ↥σ.K`): the bundle has
  NO `hL0` field, so the identification of the degree-zero subring with `↥σ.K[z^{±1}]` is a silent
  codomain choice, not separately stated or checked (MOVES_LEAN_SEMAUDIT_GR_2026-07-26.md §A.2).
...
* `Θ` — **(3)-(4a)** the D.1(e) ring iso `Θ : L ≅ L₀[T^{±1}] = LTwo ↥σ.K`. PLAIN DISCLOSURE
  (corrected 2026-07-30 verify-2 fold-in; earlier text claimed an explicit `hL0` hypothesis):
  there is NO `hL0` field in this structure. The `L₀ = K[z^{±1}]` identification — the last
  standing fiat — is HARD-CODED into the codomain type of `Θ` (`LExt ↥σ.K`), i.e. it IS a silent
  codomain choice, not a dischargeable hypothesis, and the identification of the localization's
  degree-zero subring with `↥σ.K[z^{±1}]` is not separately stated or checked
  (MOVES_LEAN_SEMAUDIT_GR_2026-07-26.md §A.2);
