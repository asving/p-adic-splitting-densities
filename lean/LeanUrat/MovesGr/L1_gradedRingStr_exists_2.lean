...
/-- HEq of two quotient classes across an index equality, given equal underlying
representatives (subtype-element form, for the associativity leg). -/
private lemma mk_heq_of (S : SideVal p) {i j : ℤ} (h : i = j)
...
private lemma coe_mulToGe (S : SideVal p) {γ δ : ℤ}
    (a : S.ge γ) (b : S.ge δ) :
...
/-- (L1_pmul_assoc, inlined Rg-free) associativity of the piece product, as HEq across
`(γ+δ)+ε = γ+(δ+ε)`. The on-disk unit `L1_pmul_assoc` carries a `GradedRingStr` argument
its proof never uses; that argument would be circular here, so the statement is restated
without it and proved identically. -/
private lemma pmul_assoc_aux (S : SideVal p) (γ δ ε : ℤ)
    (x : S.grPiece γ) (y : S.grPiece δ) (z : S.grPiece ε) :
    HEq (S.pmul (γ + δ) ε (S.pmul γ δ x y) z) (S.pmul γ (δ + ε) x (S.pmul δ ε y z)) := by
...
private noncomputable instance gmonI : GradedMonoid.GMonoid (fun γ => S.grPiece γ) where
  ...
  mul_assoc := by
    ...
    exact pmul_assoc_aux S i j k x y z
