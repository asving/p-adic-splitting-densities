...
`notes/WILD_WAVE2_BLUEPRINT_2026-07-21.md` §2 W2b)
...
* `om_leaf_faithful` — **[CITE GMN]** every genuine monic `f : ℤ_[p][X]` that the level-`N`
  classifier ACTUALLY sends to a shape of the σ-keyed menu `WildMenu.omMenuW n σ` has a genuine
  ℚ_p-factorization of type σ. MENU-FIBER-SCOPED; existence only; no uniformity / rationality /
  measure / density content.
* `menuFiber_hasType` — the Wave-2 faithfulness theorem (footprint = core + the axiom), and its
  σ-specialized corollary `gateFiber_hasType` at the Wave-1 wild gate
...
**Why `0 < N` (no decision-threshold hypothesis) is sound** (audit note): a nonempty
`B.classify`-fiber of a menu shape forces the level to exceed the shape's decision threshold.
The wild literal `Tselfloop n ((0, fSum σ), (n, 0)) (wShape σ)` carries its side in the emitted
face payload, and the classifier only emits sides read from the box Newton polygon, whose support
dots have box valuation `< N` (`PadicLift.zmodValuation_lt`); a left vertex `(0, fSum σ)` in the
fiber therefore forces `fSum σ < N`, i.e. exactly the `MenuPath` range in which
`classify_eq_selfloop_iff` proves the fiber IS the genuine stratum, whose digit conditions decode
to genuine `ℤ_[p]`-valuation data (`QpType.valuation_eq_of_zmodValuation_lt`). Below the
threshold the fiber is EMPTY and the axiom instance is vacuous. The order-0 separable leg reads
only the mod-`p` reduction, decided at every `N ≥ 1`.

Leaf module: no existing file is modified (the census append to `AxChk_baseline.lean` is Part D
of the wave). Exactly ONE new axiom, zero `sorry`.
-/
...
that the level-`N` classifier `B.classify` ACTUALLY sends to a shape `T` of the CURRENT σ-keyed
Wave-1 menu `WildMenu.omMenuW n σ` (order-0 separable `sepShape`s ∪ the canonical constant-e
minimal-H `Tselfloop` stratum — nothing else). A nonempty menu fiber forces the level past the
shape's decision threshold (module docstring), where the banked fiber characterizations
...
`{(1, dᵢ)}`), resp. single-side polygon `(0, fSum σ)–(n, 0)` with all-multiplicity-1 residual of
degrees `fᵢ` (⟹ Thm 1.19/Cor 1.20 leaves, type `{(n/fSum σ, fᵢ)} = σ`).
...
Mutation probe (§C of the wave): `gateFiber_nonempty` + the recorded MUTATED-axiom `False`
derivation (comment block below) + the positive check `gate_axiom_coherent`. -/
...
Lean core + `om_leaf_faithful` (see the census). -/
theorem menuFiber_hasType (p : ℕ) [Fact p.Prime] (n N : ℕ) (hN : 0 < N)
    (σ : FactorizationType) (T : ClusterShape) (hT : T ∈ WildMenu.omMenuW n σ)
    (f : Polynomial ℤ_[p]) (hf : f.Monic) (hdeg : f.natDegree = n)
    (hfib : B.classify p n N (PadicLift.toBox p n N hN f hf hdeg) = T) :
    ∃ F : QpType.QpFactorization p f, QpType.qpType p F = σ :=
  om_leaf_faithful p n N hN σ T hT f hf hdeg hfib
...
**Mutation probe evidence (machine-checked 2026-07-21, scratch NOT committed).** The following
scratch module — the REAL axiom's hypotheses verbatim, conclusion mutated to the SPLIT type
`{(1,1),(1,1)}` — was compiled against this module and `lake env lean` accepted it with zero
errors, i.e. `False` DERIVES from the mutated axiom + `gateFiber_nonempty` +
`QpType.wildGateFiber_eisenstein`. A wrong-(e,f) payload at the gate is therefore
machine-refutable, not silently absorbable: the axiom's conclusion is load-bearing at its
concrete instances.
...
/-! ## Block 6 — axiom census (this wave declares EXACTLY ONE axiom)

Expected footprints: `om_leaf_faithful` = itself; `menuFiber_hasType` /
...
`RealInstanceV2.montes_unconditional`) do NOT consume this module — their core-only footprints
are re-printed in `AxChk_baseline.lean` (Part D of the wave). -/
