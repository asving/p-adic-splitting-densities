...
/-!
# HC1/V34_readKernel — the shared V3/V4 read-frame kernel at `ReadFrame.pack` (SYN-M7)

The file-private engine kernel previously duplicated BYTE-IDENTICALLY between
`HC1/V3_readLift.lean` (lines 75–933 pre-migration) and `HC1/V4_readLanding.lean`
(lines 45–832, the documented verbatim copy — golf HC1#99, drift risk now RETIRED),
hoisted ONCE (C1 cluster; concordance `lean/notes/SYN_E0_CONCORDANCE_2026-07-30.md`).

Contents and per-lemma records:
* `ReadFrame.pack` — the second `ResValPack` carrier (SYN-E4): `w := childW σ σ.Φ e★ h★`,
  `R := V.D.Rv`, laws from the frame fields + `ReadResLaws` conjuncts 2–5.
* `v3_wv_one/v3_wv_neg/v3_wv_pow/v3_rv_one/v3_rv_pow/v3_wv_sum_ge` — statements VERBATIM
  from the V3 kernel; bodies re-based on the pack (`ResValPack.*`), the carrier-translated
  instances of the SYN-E0 arithmetic families (single proof source).
* `v3_GRf` — statement verbatim; delegates to `Moves.LaurentOrd.GRf_priv` (SYN-E0: ID).
* everything else (`v3_coeff_extract`, `v3_dev_slot_bound`, `v3_slot_mono`, `v3_epos`,
  `v3_decomp_sum`, `v3_devForm`, `v3_width`, `v3_psiNotDvd`, `v3_lift_monic`,
  `v3_lift_core`, `v3_cslot`, `v3_key_no_cancel`, `v3_minsum_facts`) — VERBATIM hoists
  from V3 (`private` dropped; ReadFrame-specific, not pack-generic: they consume
  `V.D.hS5`/`V.D.hRPhi`/`V.hwPhi`/`V.hStretch`/`IsReadLift`).
V4's pure-Laurent tail (`dvd_of_dvd_mul_T` … `pow_order_unique`) is NOT here — it lives
at `Moves/LaurentOrd.lean`; V4 opens that namespace.
-/
...
lemma v3_coeff_extract {ι : Type*} {A : Type*} [AddCommMonoid A]
...
theorem v3_GRf {K : Type*} [Field K] (ψ : Polynomial K) (hψ : Irreducible ψ)
    (hψz : ψ ≠ Polynomial.X) (c : ℕ → LaurentPolynomial K) (S : Finset ℕ) (hne : S.Nonempty)
    (hc : ∀ j ∈ S, c j ≠ 0 ∧ ¬ (Polynomial.toLaurent ψ ∣ c j)) :
    (∑ j ∈ S, c j * (Polynomial.toLaurent ψ) ^ j) ≠ 0 :=
  Moves.LaurentOrd.GRf_priv ψ hψ hψz c S hne hc
...
is zero. -/
lemma v3_dev_slot_bound (Φ : Polynomial ℤ_[p]) (hΦ : Φ.Monic)
...
/-- **The `ReadFrame` instance of the residual-valuation pack** (SYN-E4): the second
carrier of the shared engine — `w := childW σ σ.Φ e★ h★` (frame fields `hwmul`/`hwult`),
`R := V.D.Rv` (laws = `ReadResLaws` conjuncts 2–5, HC1 V1/V2's proved content). -/
noncomputable def _root_.LeanUrat.HC1.ReadFrame.pack (V : ReadFrame σ estar hstar sstar tstar) :
    Moves.ResValPack p ↥σ.K where
  w := childW σ σ.Φ estar hstar
  R := V.D.Rv
  hwmul := V.hwmul
  hwult := V.hwult
  hRne := V.laws.2.1
  hRmul := V.laws.2.2.1
  hRadd := V.laws.2.2.2.1
  hRlt := V.laws.2.2.2.2.1

/-- `wV 1 = 0`. -/
lemma v3_wv_one (V : ReadFrame σ estar hstar sstar tstar) :
    childW σ σ.Φ estar hstar 1 = 0 := V.pack.w_one

/-- `wV(−f) = wV f`. -/
lemma v3_wv_neg (V : ReadFrame σ estar hstar sstar tstar) (f : Polynomial ℤ_[p])
    (hf : f ≠ 0) : childW σ σ.Φ estar hstar (-f) = childW σ σ.Φ estar hstar f :=
  V.pack.w_neg f hf

/-- `wV(f^n) = n·wV f`. -/
lemma v3_wv_pow (V : ReadFrame σ estar hstar sstar tstar) (f : Polynomial ℤ_[p])
    (hf : f ≠ 0) (n : ℕ) :
    childW σ σ.Φ estar hstar (f ^ n) = (n : ℤ) * childW σ σ.Φ estar hstar f :=
  V.pack.w_pow f hf n

/-- `RV 1 = 1`. -/
lemma v3_rv_one (V : ReadFrame σ estar hstar sstar tstar) :
    V.D.Rv (1 : Polynomial ℤ_[p]) = 1 := V.pack.R_one

/-- `RV(f^n) = RV(f)^n`. -/
lemma v3_rv_pow (V : ReadFrame σ estar hstar sstar tstar) (f : Polynomial ℤ_[p])
    (hf : f ≠ 0) (n : ℕ) : V.D.Rv (f ^ n) = (V.D.Rv f) ^ n := V.pack.R_pow f hf n

/-- Ultrametric finite-sum lower bound for `wV`. -/
lemma v3_wv_sum_ge (V : ReadFrame σ estar hstar sstar tstar)
    (S : Finset ℕ) (a : ℕ → Polynomial ℤ_[p]) (m : ℤ)
    (hm : ∀ j ∈ S, a j ≠ 0 → m ≤ childW σ σ.Φ estar hstar (a j))
    (hsum : (∑ j ∈ S, a j) ≠ 0) :
    m ≤ childW σ σ.Φ estar hstar (∑ j ∈ S, a j) :=
  V.pack.w_sum_ge S a m hm hsum

/-- **Slot monomial** (the S5-shape at the regrade): for `B ∈ C_Φ∖0`,
`RV(B·Φ^j) = single(−t★·σ.w(B) + j·s★, c)` with `c` a unit. -/
lemma v3_slot_mono (V : ReadFrame σ estar hstar sstar tstar)
...
lemma v3_epos (V : ReadFrame σ estar hstar sstar tstar)
...
and is nonzero. -/
lemma v3_decomp_sum (V : ReadFrame σ estar hstar sstar tstar)
...
lemma v3_devForm (V : ReadFrame σ estar hstar sstar tstar)
    (hbez : (estar : ℤ) * sstar + (hstar : ℤ) * tstar = 1)
...
lemma v3_width (V : ReadFrame σ estar hstar sstar tstar)
    (hee : 1 ≤ estar)
    (hbez : (estar : ℤ) * sstar + (hstar : ℤ) * tstar = 1)
...
theorem v3_psiNotDvd (V : ReadFrame σ estar hstar sstar tstar)
    (hee : 1 ≤ estar)
    (hbez : (estar : ℤ) * sstar + (hstar : ℤ) * tstar = 1)
...
`e★·g·deg Φ`. -/
lemma v3_lift_monic (hee : 1 ≤ estar)
...
lemma v3_lift_core (V : ReadFrame σ estar hstar sstar tstar)
...
noncomputable def v3_cslot (V : ReadFrame σ estar hstar sstar tstar) (g : ℕ)
...
`v3_GRf` + `v3_psiNotDvd`. -/
lemma v3_key_no_cancel (V : ReadFrame σ estar hstar sstar tstar)
    (hee : 1 ≤ estar)
    (hbez : (estar : ℤ) * sstar + (hstar : ℤ) * tstar = 1)
...
lemma v3_minsum_facts (V : ReadFrame σ estar hstar sstar tstar)
    (hee : 1 ≤ estar)
    (hbez : (estar : ℤ) * sstar + (hstar : ℤ) * tstar = 1)
    (ψ : Polynomial ↥σ.K) (g : ℕ) (hg : ψ.natDegree = g) (hmon : ψ.Monic)
    (hψ : Irreducible ψ) (hψz : ψ ≠ Polynomial.X)
