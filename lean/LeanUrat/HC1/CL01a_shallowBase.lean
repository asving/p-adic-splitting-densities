...
card computation). AS-BUILT (SIDE BRANCH, 2026-07-30): all bodies PROVED, Lean-core
axioms only — the recommended `S1_transHypGate_a.choose` route, exactly as sketched.
...
tree. THIS COMPILED FILE LIVES ON THE SIDE BRANCH ONLY (the live tree keeps the
sorried skeleton); no negation composite is compiled anywhere in the CL-01 portfolio —
the composition against `C6_forces_unconditional_fullness` lands atomically with
CL-02's fence transition, retargeted at the frozen
