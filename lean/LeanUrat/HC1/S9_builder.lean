...
/-!
# HC1.S9_builder — the SHARED child-stage construction consumed by S9a/S9b/S9c

C4 HOIST RECORD (2026-07-30, SYNTHESIS_PASS1 C4b, ratified in BRIDGE_ADJUDICATIONS
"C1+C4" cluster; golf HC1#96): S9a_stageLiteral / S9b_childWeightSet /
S9c_coreAssembly carried the SAME ~200-line construction (Bézout split → S9w lawful
carrier → S2/S3/S5 childW plumbing → the slotwise-development hwult tower →
L4_TRANSviii_b hS6b′ → V7 strip-discharged coset hS6a′ → the Stage literal) as three
near-verbatim copies, plus S9c's private bare-Stage/grRes/coeff-core helper block.
Hoisted HERE once, BEFORE the queued consumers (BP2 HK-15/HK-16, BP3 TV-D6)
instantiate the chain at the F4 gate. The three S9 mains are now PROJECTIONS of
`S9_builder` — their public statements byte-identical to the pre-hoist forms.

VERBATIM LEDGER (the SharedRecenter protocol):
* the private helper block (`w_one'` … `coeffCore_of_stage`) = S9c_coreAssembly's
  lines, byte-verbatim (still `private`; only this file's builder consumes them);
* the proof body of `S9_builder` = S9c_coreAssembly's proof body, byte-verbatim
  EXCEPT the single witness-packaging line
  (`refine ⟨σ', tc, pin, fun f _ => rfl, ?_⟩` →
   `refine ⟨σ', tc, pin, fun f _ => rfl, rfl, ?_⟩`) — the inserted `rfl` closes the
  new conjunct 4, definitional on the literal (`weightSet := childWeightSet σ Φhat`);
* `S9_builder`'s STATEMENT is the one NEW public assertion (the SharedRecenter
  `recentering_scaffold` precedent). PER-CONJUNCT RECORD — the conjunction is exactly
  the UNION of the three ratified ∃-bodies at the same literal witness and the same
  hypotheses (σ, hσ, th, hEG : 1 < σ.e * g — the adjudicated MOVES 3755–3757 note
  guard carried by all three):
  1. `TransitionCoreL σ σ' Φhat e' h' zbar` — S9c conjunct 1 verbatim; its `.base`
     projection is S9a/S9b conjunct 1 (`TransitionData`) verbatim;
  2. `VertexPin σ σ' Φhat zbar` — conjunct 2 of all three, verbatim;
  3. the childW tie `∀ f, f ≠ 0 → σ'.w f = childW σ Φhat e' h' f` — conjunct 3 of
     all three, verbatim;
  4. `σ'.weightSet = childWeightSet σ Φhat` — S9b conjunct 4 verbatim;
  5. `StageCore σ'` — S9c conjunct 4 verbatim.
  NO conjunct is generalized, weakened, or strengthened beyond that union; the
  corner-refutation kernels (`S9{a,b,c}_ungated_corner_refuted`) STAY in their
  original files, statements untouched.
-/
...
/-- **The S9 shared builder** (C4 hoist, 2026-07-30 — see the header's per-conjunct
record): at every INCREMENT read (`1 < σ.e * g`), the child Stage literal exists
carrying, at ONE witness, the union of the S9a/S9b/S9c conjuncts — the full
round-5 transition record (whose `.base` is S9a/S9b's `TransitionData`), the
D.7(vi) vertex pin, the childW valuation tie, the pinned `weightSet = 𝒜′`, and
`StageCore`. S9a/S9b/S9c project from here; HK-15/HK-16 (BP2) and TV-D6 (BP3)
instantiate it at the F4 gate. -/
theorem S9_builder (σ : Stage p F) (hσ : StageCoreL σ) {ψ : Polynomial ↥σ.K} {g : ℕ}
    ...
    ∃ σ' : Stage p F,
      ...
      σ'.weightSet = childWeightSet σ Φhat ∧
      StageCore σ' := by
  classical
  ...
  refine ⟨σ', tc, pin, fun f _ => rfl, rfl, ?_⟩
  exact
...
#print axioms LeanUrat.HC1.S9_builder
