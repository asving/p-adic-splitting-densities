...
hypothesis_fields: explicit `RegP D` only (ROUND-2 CRITICAL 4 schema).
-/
import Mathlib
...
theorem RegP.entry_agree (h : RegP D) {q₀ : ℕ} (hq : q₀ ∈ D.Pool) (e : D.Block)
...
/-- (r1): the full determinant (junk blocks included) is defined and ≠ 0 at every
    pool value. -/
theorem RegP.det_ne_zero (h : RegP D) {q₀ : ℕ} (hq : q₀ ∈ D.Pool) (e : D.Block) :
    letI := D.instBi e; letI := D.instBd e
    DefinedAt (Matrix.det (1 - D.K e)) (q₀ : ℚ) ∧
      (Matrix.det (1 - D.K e)).eval (RingHom.id ℚ) (q₀ : ℚ) ≠ 0 :=
  (h q₀ hq e).1

/- ROUND-2 RETYPE (CRITICAL 4): the former `RegP.R_defined`/`RegP.R_agree`
   accessors read the carried strengthening conjuncts, which are KILLED — both
   facts are now DERIVED from the typed assembly as `SolveSeam.R_defined` and
   `SolveSeam.R_agree` (DefsLedger). -/

/-- (r2) at the K_e family: each matrix entry. -/
theorem RegP.K_agree (h : RegP D) {q₀ : ℕ} (hq : q₀ ∈ D.Pool) (e : D.Block)
    (i j : D.bidx e) :
...
theorem RegP.bterm_agree (h : RegP D) {q₀ : ℕ} (hq : q₀ ∈ D.Pool) (e : D.Block)
    (i : D.bidx e) :
...
theorem RegP.bsplit_agree (h : RegP D) {q₀ : ℕ} (hq : q₀ ∈ D.Pool) (e : D.Block)
    (i : D.bidx e) :
...
theorem RegP.Jcell_agree (h : RegP D) {q₀ : ℕ} (hq : q₀ ∈ D.Pool) (e : D.Block)
    (j : D.JIdx e) :
...
theorem RegP.iota_agree (h : RegP D) {q₀ : ℕ} (hq : q₀ ∈ D.Pool) (e : D.Block)
    (i : D.bidx e) :
...
theorem RegP.Wcoef_agree (h : RegP D) {q₀ : ℕ} (hq : q₀ ∈ D.Pool) (w : D.WIdx) :
...
theorem RegP.betaLeg_agree (h : RegP D) {q₀ : ℕ} (hq : q₀ ∈ D.Pool) (e : D.Block)
