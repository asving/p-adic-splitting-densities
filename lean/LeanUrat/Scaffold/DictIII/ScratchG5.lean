...
leaf #4).  This file currently holds units III-G1 (`devCoeffX`) and III-G2
...
`divByMonic`": `devCoeffX Φ B j` is the `j`-th coefficient `C_j` in
...
noncomputable def devCoeffXX {O : Type*} [CommRing O] (Φ : Polynomial O) :
    Polynomial O → ℕ → Polynomial O
  | B, 0 => B %ₘ Φ
  | B, j + 1 => devCoeffX Φ (B /ₘ Φ) j
...
theorem devCoeffX_mapX {O S : Type*} [CommRing O] [CommRing S] (f : O →+* S)
    {Φ : Polynomial O} (hΦ : Φ.Monic) (B : Polynomial O) (j : ℕ) :
    (devCoeffX Φ B j).map f = devCoeffX (Φ.map f) (B.map f) j := by
  induction j generalizing B with
  | zero => simpa [devCoeffX] using Polynomial.map_modByMonic f hΦ
  | succ j ih =>
    simp only [devCoeffX]
...
theorem devg_congrX {O} [CommRing O] [IsDomain O] [IsDiscreteValuationRing O]
    ...
    ∀ j k, (devCoeffX Φ B j - devCoeffX Φ B' j).coeff k ∈
      (Ideal.span {π}) ^ L := by
  intro j k
  ...
  have hdev : (devCoeffX Φ B j - devCoeffX Φ B' j).map (Ideal.Quotient.mk I)
      = 0 := by
    rw [Polynomial.map_sub, devCoeffX_mapX _ hΦ, devCoeffX_mapX _ hΦ, hmap,
      sub_self]
  have hz : (Ideal.Quotient.mk I)
      ((devCoeffX Φ B j - devCoeffX Φ B' j).coeff k) = 0 := by
...
The verbatim display is
  ...
  def gaussWX {O} [CommRing O] [IsDomain O] [IsDiscreteValuationRing O]
