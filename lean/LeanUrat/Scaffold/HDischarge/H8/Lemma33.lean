...
/- Tonelli factorization `tsum_pi_prod` MOVED VERBATIM to
`Scaffold/Shared/ENNRealTonelli.lean` (SYNTHESIS_PASS3 F4 dedup, 2026-08-01);
used below as `Shared.tsum_pi_prod`. -/
...
private lemma tsum_subtype_pi_group {k : ℕ} {V : Type*} (D : Fin k → Type*)
  ...
  refine Eq.trans ?_ (Shared.tsum_pi_prod _ _ (fun i (d : {d : D i // dv i d = vt.1 i}) => w i d.1))
