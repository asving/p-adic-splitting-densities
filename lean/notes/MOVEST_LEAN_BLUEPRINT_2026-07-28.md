...
(`MOVEST_AUDIT_FABLE2_2026-07-28.md`). **Rev 3 DUAL-REJECTED** — Codex 18 crit /
5 gap (`MOVEST_AUDIT_CODEX3_2026-07-28.md`) + Fable 2 crit / 4 gap
(`MOVEST_AUDIT_FABLE3_2026-07-28.md`). **Rev 4 DUAL-REJECTED** — Codex 9 crit /
4 gap (`MOVEST_AUDIT_CODEX4_2026-07-28.md`) + Fable 1 crit / 1 gap
(`MOVEST_AUDIT_FABLE4_2026-07-28.md`); the round-4 verdicts AGREE that the ENTIRE
round-3 union was genuinely repaired — Fable-4 walked both rev-4 headline repairs
at the built signatures (the wchain counter-instance dies on the w-mult clause;
the capped ladder is satisfiable at presented jets), re-verified the LedgerIV
import, and certified the §7 table quote-faithful with the tally exact — and
disagree on the residual layers: Fable-4's one fresh critical is T-V8's THIRD
...
strengthening. **Rev 6 DUAL-REJECTED** — Codex 8 crit / 6 gap
(`MOVEST_AUDIT_CODEX6_2026-07-29.md`) + Fable 2 crit / 3 gap
(`MOVEST_AUDIT_FABLE6_2026-07-29.md`); the round-6 verdicts CONVERGED for the
first time on a headline row (Codex-6 #1 = Fable-6 GAP-1: `root_total`'s untied
∃) and split on the rest: Fable-6 walked the entire §7 table as textually real
(15/15 rows present) but found the rev-6 toys' `cellOf .red` guards and
`cellLevel := 3` decide-FALSE (two criticals in the closed-form tables), while
Codex-6 re-verified only 8 of the 11 claimed F repairs, rejected BOTH standing
BDY holds (W4-1; ns_bridge), and landed the OmSat degree-(> n) escape, the
non-closed accessors (`windowBlockHts`/`slot0Block`/`bandFresh`), the toy DATA
fields, and the stale post-ratification seam names. **Rev 7 DUAL-REJECTED** — Codex 5 crit / 1 gap
(`MOVEST_AUDIT_CODEX7_2026-07-29.md`) + Fable 2 crit / 2 gap
(`MOVEST_AUDIT_FABLE7_2026-07-29.md`); 15 of the 19 round-6 rows were verified
REAL by BOTH auditors, and the residue CONVERGED to five items (both-leg
convergences, orchestrator-adjudicated in `MOVEST_REV8_CHARGE_2026-07-29.md`):
`ChildCover` defined but not INSTALLED in `CellAssign` or any displayed
consumer; `NsBridgeStmt` FALSE as stated (no decay premise — Fable-7's
in-corpus counter-model: childless tower + junk μ·Dwidth = 0 node gives
μcap ≡ 1, μclass = 1 with every premise satisfied); T-F4 FALSE at the empty
shape against the built ∅-dispatch (1 = 0 + 0, `PrefIdx` empty at reads = []);
the toy node roster UNLAWFUL (e·g = 1 leaves pinned `.increment` against the
built `hspecInc`; γ pins violating the built γ-tie; `line` intercept unpinned;
`Ranch := 0`/`ψ := X`/μ ≥ 1 jointly unsatisfiable against the built Node
...
The REV-8 changes (carried, still binding):
 (viii-1) **`ChildCover` is INSTALLED**: the W4-1 interface `CellAssign` is
   ...
   mechanical rename, §2.9) and `CellAssign … χ trackOf extends CellData`
   carries the mandatory field `child_cover : ChildCover T toCellData χ
   trackOf`. The five displayed consumers (T-E5, `TreeExpFin`, `TreeNStmt`,
   `VPPinned.CAat`, `rs1GivenOfMovesT`/`xrbOfMovesT`) take the FULL
   `CellAssign`; the constant-false-child model now has NO CellAssign
   instance at all (`constFalse_no_cellAssign`, T-D2). The TOYS instantiate
   `CellData` ONLY — re-fenced HONESTLY with the computed failure displayed
   (carrier A fails `child_cover` at g = (1,0): f̄ = x²+1 has X+1 repeated
   and NO realized child there — deliberately partial, §T-G1);
 ...
 (viii-3) **T-F4 gains the `hne` premise** (the same premise the built D11a
   ...
   its built name;
 (viii-4) **the toy roster is LAWFUL**: leaves re-pinned `.recentering`
   (built `hspecInc` forbids `.increment` at e·g = 1), γ re-pinned 3/4/6
   (the built γ-tie's own values), `line` FULLY pinned (intercepts 3/4/6 via
   the built `hLineU`), and the anchored-residual data re-pinned
   (ψ = X − C 1, center = 1, pat/Ranch per `hRanch`, ord_ψ = μ exact) with
   the FULL Node proof-field walk displayed; `trackOf` on the toys is the
   constant X (no longer read off `Node.ψ` — a type mismatch); census
   integers and every decide check re-verified unchanged;
 (viii-5) **OmSat gains the TOTALITY law `OmSatTot`** (a `VPPinned` leg:
   every `ReadsOf`-realized saturated history HAS an fη with the
   `ClusterFactorOf` package) and record 19 is RE-ATTRIBUTED: the junk-
   trackOf protection is ruling 2's SHARED `trackOf`
   (`KBTotTower.root_total` makes `VPPinned` uninhabitable under junk
   values); the membership conjunct's genuine role is the wrong-reduction/
   over-strength direction;
 (viii-6) **`hRes`'s exactness claim is SCOPED to lawful histories** (the
   accessor stays closed and total);
 (viii-7) **§2.11 CLOSURE SUPPLEMENTS**: every previously bracket-only
   identifier consumed by displayed Lean gets a displayed closed decl
   (`NodeExtends`, `SameClass`, `ClassPrefixOf`, `ramIdx`/`resDeg`, the jet
   accessors `M₀`/`scaleTop`/`interiorFreshIdx`/`digitAt`, …), per the §8
   manifest rule.
The REV-7 changes (carried, still binding):
 (i) **the W4-1 interface gains the (c2) covering clause `child_cover`**
   (ruling 1, the 5th-round OVERTURN of the four-round BDY hold): the note's
   covering case-analysis lives AT this layer (7112–7119), so `CellAssign`
   carries the per-track root covering — a repeated reduction factor forces a
   realized root child IN that track's `.red` branch set (D4R.0-K(a)'s
   L1-totality warrant); the constant-false-child/unit-cell model is now FALSE
   at the W4-1 interface ITSELF (`constFalse_childCover_false`, T-D2). This is
   a CONTRACT CHANGE, recorded for the MovesD-side W4-SYNC (§5 W4-1 row +
   `W4_SYNC_DELTAS_2026-07-29.md` entry 10). [REV 8, Codex-7 #1: the rev-7
   wiring was INCOMPLETE — `ChildCover` was standalone and no displayed
   consumer took it; it is now a mandatory FIELD of the bundled `CellAssign`
   (viii-1 above), and the toys instantiate `CellData` only, disclosed];
 (ii) **`root_total` is PER-TRACK** (ruling 2 — Codex-6 #1 = Fable-6 GAP-1,
   converged): ∀ ψ repeated, ∃ ν with `trackOf ν = ψ` carrying the child;
   `KBTotTower` gains the `trackOf` parameter ("one track per root cluster" —
   the untied ∃ let one child witness every repeated factor); the childless
   tower still dies at the zero member;
 (iii) **the toy `winC` guards pin the reduction coords** (Fable-6 CRIT-1, fix
   verbatim): `x0 = 0 ∧ x1 = 0` joins BOTH carriers' `.red` guards —
   `child_cell_red` was decide-FALSE at g ≠ toyG (witness g = (1,0),
   x = (1,0,0,…)); `cellLevel := 4` (Fable-6 CRIT-2 — at 3, coords 4/5 sat at
   level 3 outside `cell_local`'s premise while `.st` cells read them); all
   census integers recomputed unchanged (§T-G1);
 (iv) **`OmSat`'s track value is REALIZED** (Codex-6 #3): `ClusterFactorOf`
   ties `trackOf` at H's head to membership in the realized f̄-factor multiset
   (`normalizedFactors (f.map toZMod)` at the `ReadsOf`-realized f) — the
   monic-irreducible-degree-(> n) escape (∣ fails on every cluster degree ≤ n,
   witness-free again) — [REV 8, Codex-7 #2: the "dies at the membership
   conjunct" claim is RETIRED (a conjunct in an antecedent cannot cure
   implication-vacuity); the escape dies at the shared-accessor pair
   root_total + om_total — (viii-5) above, record 19 re-attributed];
 (v) **every load-bearing accessor is CLOSED** (Codex-6 #4/#7):
   `windowBlockHts`/`hScale`/`hRes` defined over the built `History.htH`/
   `History.floorH`/`Node.μ·Dwidth` (§2.5a); `bandFresh`/`shapeExp` and T-D7's
   `bandLevels` defined over the built `ShapePrefix.bandS`/`NPband`/`Nshape`
   (§2.5) — no E-phase name-binding remains in the principal non-vacuity
   mechanism or in `hdict`'s checkability chain;
 (vi) **`ns_bridge` is the TYPED seam `NsBridgeStmt`** (Codex-6 #5, hold
 ...
 (vii) **the toy node tables pin EVERY data field** (Codex-6 #6): species/σ/
   ...
   any DATA position — the one non-literal pin (σ := the level-0 stage) carries
   its displayed read-surface obligation (`card ↥σ.K = 2`);
 (viii) **the seam ledger is re-keyed to the post-ratification corpora**
   (Codex-6 #8 + g2–g6 = Fable-6 GAP-2/GAP-3, per
   `W4_SYNC_DELTAS_2026-07-29.md`): W4-4 → the built `NPIDPin`/`NthrSup`/
   `noteNP`/`D4R4_all_noteRange` (T-D7 re-keyed, imports by name); the W4-5 row
   ADDED (`OffsetPPin`/`CD_canonical`, consumer duty adjudicated); S-8 →
   `wsh17_pin` the projection-def = `W17ii` (duty = PROVE, owner MovesV);
   S-10 → `RS4Chain.legs_read` + the `deltaFactors` continuing-members
   narrowing; T-F4/the CONSUMES roster keyed to `Presented.event`/`mult` under
   the IN-FLIGHT ∅-shape redefinition (eventO/multO deleted); census 19 → 20;
 (ix) **the F1 joint WITNESS is a disclosed E-phase obligation** (Codex-6 g1):
   the 5^18 identity is cardinal compatibility, not existence — the
   CellAssign/ledger/scaffold witness at the F1 ten-pin window is now §6's
   obligation row, never silently claimed.
...
* `LeanUrat.MovesD` — `Box`, `topLocus`, `ShapeRead`/`ShapePrefix`/`ShapeWF`/`Shape`,
  ...
  REV 7 (Codex-6 #8, W4-SYNC delta 8): keyed to the IN-FLIGHT MovesD repair-round-2
  contract, where the ∅-shape dispatch (reads = [] ↦ univ/1) moves INSIDE
  `event`/`mult` and `eventO`/`multO` are DELETED; every MovesT consumer reads
  `event`/`mult` under the NEW semantics, and the D14c laws' content
  (`A'(∅) = 0`, univ/1 at the empty shape) rides the redefined names),
  ...
  the D4R.4/L12 threshold apparatus — REV 7 (Codex-6 g2, W4-SYNC delta 6): the
  BUILT `ShapePrefix.bandS`/`NPband`/`Nshape`, `Shape.NP`, and the typed NP-ID
  family `NPIDPin`/`NthrSup`/`noteNP`/`D4R4_all_noteRange` (NPv_npid.lean),
  imported BY NAME (the rev-4 rule); the pre-repair "D4R4all" pointer is retired.
...
* `LeanUrat.MovesS` — NEW at REV 3 (the S-3 re-key): `TableShape`/`MeasuredSide`/
  ...
  (EvalReDef.lean) + REV 4 (Codex-3 #18): **`LedgerIV` (Defs.lean 210, a Prop
  structure)** — BUILT, imported by §2.10/T-D15 only.
* `LeanUrat.HC2` — NEW at REV 3 (the Wall-A pin): `ReadsOf` (Defs.lean 283) —
  BUILT, referenced by `WallAReconciled` and (REV 5, Codex-4 #7) by the displayed
  `ClusterFactorOf` (both spec-level; fence notes at §2.9/§2.7).

BUILD-CHECK RECORD (REV 4, Codex-3 #19's evidentiary gap — verified on this
machine 2026-07-28, not re-argued in prose): all four consumed files carry built
artifacts — `.lake/build/lib/lean/LeanUrat/MovesS/{Interfaces,EvalReDef,Defs}.olean`
and `.lake/build/lib/lean/LeanUrat/HC2/Defs.olean` present — and a code-level
`sorry` grep on the four files is clean (the two `HC2/Defs.lean` hits at lines
21/41 are comment text). `MovesS.LedgerIV` verified at `MovesS/Defs.lean:210`
(`structure LedgerIV (T : TableShape n) (M : MeasuredSide T) : Prop`).
...
| **(OM-SAT)** | 7262–7273 declared statement | `OmSat` (Defs §2.7) — REV 5 (Codex-4 #7): the factor-assignment carrier `ClusterFactorOf` is DISPLAYED (monic + cluster-degree tie + track tie via `trackOf` + divides a `ReadsOf`-realized monic degree-n f), so the `False`-reading vacuity is dead; the rev-4 "consumed by nothing" language is RETRACTED — `VPPinned.om_sat` consumes it. REV 6 (Codex-5 #4): the TRACK VALUE ITSELF is pinned — monic ∧ irreducible ∧ degree ∣ cluster degree on `trackOf` at H's head; `trackOf := 0`/junk made the track-power clause witness-free and `OmSat` VACUOUS (§0 record 15). REV 7 (Codex-6 #3): the track value is tied to the REALIZED factorization — membership in `normalizedFactors (f.map toZMod)` at the `ReadsOf`-realized f; rev 6's abstract lawfulness still admitted a degree-(> n) monic irreducible `trackOf` (divides NO cluster degree ≤ n — witness-free, vacuous again, §0 record 19). REV 8 (Codex-7 #2 = Fable-7 GAP-1): the conjunct cannot cure IMPLICATION-vacuity (an emptier antecedent makes `OmSat` easier); the note's EXISTENCE assertion is now its own typed law `OmSatTot` (§2.7, consumed as `VPPinned.om_total`), and the junk-accessor protection is RE-ATTRIBUTED to the shared `trackOf` (`root_total` + `om_total` — record 19 re-attributed); the conjunct's role = the wrong-reduction direction | VP-SOUND citation obligation (GMN/Montes cite; axiomatization would need the faithfulness-entry process). Hypothesis fields only (`om_sat` + REV-8 `om_total`) |
...
| **Node-data lawfulness** | realizable reads have e,g,μ ≥ 1, K-card a p-power (L3/§B2-DEF ledger data) | `NodeDataLawful` (Defs §2.2) — per-node only. REV 4 (Codex-3 #1): NO LONGER a conjunct of `IrrHalts` — the note's τ-irr rule is "pure node/cell data" with no lawfulness clause; lawfulness is STANDING PERIMETER (this row), consumed via `HistLawful` where a theorem needs it | owner HC-2 (node-ledger content) |
| **History lawfulness fragment** | the L3-dictionary laws of a realizable history — per-node data + the slope/γ/anchor ties + width/window/w-chains + w₀ (card K₀ = p EXACTLY) + W-MULTIPLICATIVITY (card K_{i+1} = card K_i ^ g_i — REV 4) + the MONIC-LEAD law (root read at the monic slot has top side digit 1 — REV 5) — the KEYING-CONSTANT-FREE fragment | `HistLawful` (Defs §2.2) — REV 3 (Fable-2 CRIT-2): T-V8's premise. REV 4 (Fable-3 CRIT-1 = Codex-3 #4's wchain leg): the w-mult clause is ADDED — it is the only possible source of the real `ShapeWF.wchain`, and its HC home is the 4th-conjunct KEYING legs (TransitionCoreL's field growth F_{i+1} = F_i[z]/(ψ_i)), so the fragment is NO LONGER described as "child-keying-free": it is KEYING-CONSTANT-FREE (no recentering/lift/Bézout constant is pinned — Q1's V10-inconsistent target is exactly those constants, so Q1-INSENSITIVITY survives), and the Q1 fence GROWS to cover the clause's discharge. REV 5 (Fable-4 CRIT-1): the MONIC-LEAD clause is ADDED — the only possible source of the built `Matches` 13th conjunct (`monicTop = true → sideDigit len = 1`; `readOf` does not copy `pat`, so no other clause reaches the digit); it is the §0.5 BOX-NORMALIZATION law (a monic box member's degree-n slot digit IS 1), node-side DIGIT data pinning no recentering/lift/Bézout constant — keying-constant-free, Q1-insensitive; its discharge rides the same Q1 queue with the fragment. HONESTY (carried): `HistoryCoherent` alone does NOT imply the fragment (no HC clause forces μ ≥ 1, the K-card p-power, card K₀ = p, or the monic-lead digit); the intended discharge is HistoryCoherent-at-the-regraded-value + the node/root ledger data + the keyed field-growth law + the L3 root-box data, QUEUED ON Q1 — until then an explicit fenced premise | hypothesis `hlaw` on T-V8, T-V9; owner HC-2, discharge fenced-Q1 (§4) |
...
TWENTY-THREE IN-CORPUS REFUTATION RECORDS (the audit rounds' constructive finds, carried
...
`toy_henflip_unrealizable`, `toy_state_cell` — the ONE-SIDE carrier); record 2
lives on toy carrier B (`toy_jcmulti_site` — the TWO-SIDE carrier, REV 5: rev 4's
single pinned toy could not host both a one-side and a multi-side site); records
5+7 live on the separate 𝔽₄-type carrier (`toy_v8_wchain`, one gate, the roster
channel); record 9 lives on the separate p = 3 carrier (`toy_v8_monic`, REV 5 —
the same roster channel); records 8/10/11/12's regression protection is
TYPE-LEVEL, not a gate (8: the `Mlev` cap sits in the `slot0`/`slot0_grid`/T-C5
...
sits in `KBTotTower`'s type (REV 7: PER-TRACK, with `trackOf` in the tower's
signature); 15: the track-lawfulness conjuncts sit in `ClusterFactorOf`'s type;
16 lives on carrier A (the REVERSED flip gate coexisting with `toy_fiber_ne` at
build). REV-7 RECORDS: 17/18 live on BOTH toy carriers (the corrected winC
guards' and cellLevel-4's G1a decide obligations — `child_cell_red` and
`cell_local` now decide TRUE, re-run displayed at T-G1); 19's protection is —
RE-ATTRIBUTED at REV 8 (Codex-7 #2 = Fable-7 GAP-1(b)) — ruling 2's SHARED
`trackOf`: `KBTotTower.root_total` inside `VPPinned.covering` makes the whole
vp value uninhabitable under junk accessors, PLUS the REV-8 totality leg
`om_total` (`OmSatTot`, §2.7) which types the existence a junk accessor
cannot supply; the realized-membership conjunct in `ClusterFactorOf`'s type
protects the WRONG-REDUCTION/over-strength direction only (rev 7 credited it
with the vacuity protection — wrong: adding a conjunct to an antecedent makes
the implication EASIER, never harder). REV-8 RECORDS: 20–23 below:
...
* rev 3's `HistLawful` roster (without w-multiplicativity) cannot source
  `ShapeWF.wchain`: F := 𝔽_{p²}, two-node H with card K₀ = p, card K₁ = p²
  (a p-power — every rev-3 clause satisfiable), g₀ = 1 gives `shapeOfH` w₁ = 2 ≠
  1 = w₀·g₀ — `HistLawful` held while `ShapeWF` failed, T-V8 FALSE as stated
  (Fable-3 CRIT-1; Codex-3 #4's wchain leg concurring). The rev-4 roster adds
  card K_{i+1} = card K_i ^ g_i; gate `toy_v8_wchain` pins the counter-instance.
* rev 3's unbounded slot-0 ladder (`∀ M ≥ M₀, M − M₀ ≤ pinCount`) is
  UNSATISFIABLE at every fixed level — `Locus p m` pins per-coordinate, so
  pinCount ≤ m; at M := M₀ + m + 1 the demand fails, making `KBTot`/`VPPinned`
  uninhabitable and T-C5 refutable by cardinality (Fable-3 CRIT-2; Codex-3 #3
  concurring: "a fixed JetSetup and finite Box cannot supply arbitrarily many
  independent fresh coordinates as M → ∞"). The rev-4 forms are CAPPED at
  `Mlev J N`, the note's own profinite→finite transcription boundary (the
  Scale map, 7205–7207), and the falsifier is tower-scoped (§2.4).
* rev 4's `HistLawful` roster (without the monic-lead law) cannot source the
  built `Matches` 13th conjunct (`monicTop = true → ν.sideDigit R.len = 1`,
  MovesD/Defs 313–318): p = 3, F = 𝔽₃, ONE-node H at the monic slot
  (s0 + wSide = n) with pattern top digit `pat(wSide/e) := 2` — every rev-4
  clause holds (chain clauses vacuous; `hpatTop` demands only ≠ 0), `readOf`'s
  species guard decides `monicTop = true`, and `Matches` demands 2 = 1 — T-V8
  FALSE as stated, its THIRD refutation at a THIRD clause (Fable-4 CRIT-1). The
  rev-5 roster adds the monic-lead clause; gate `toy_v8_monic` pins the
  counter-instance (record 9).
* rev 4's fixed-level `KBTot` is degenerately inhabitable: mem-everywhere/
  child-nowhere with a jet CHOSEN to have `Mlev J N = M₀ J`, `interiorFreshIdx
  = ∅`, `L M = univ` satisfies `ns_lumps` (capped ladder vacuous, `tie` trivial)
  — the childless/constant model inhabits `KBTot` and hence rev 4's
  `VPPinned.covering` (Codex-4 #1). The rev-5 forms pin the accessors
  (`ScaleFaithful`: the jet's `Mlev`/`M₀` are the level's own (H, N)-determined
  values) and pin the LEVEL FAMILY (`KBTotTower`), so the rejection is internal
  to the consumed object (record 10; type-level protection).
* rev 4's `hMgrow` (`Mlev → ∞`) does not imply the sketch's diverging lower
  bound `Mlev − M₀`: the counter-assignment `M₀ (Jat N) = Mlev (Jat N) = N`
  satisfies `hMgrow` while the capped ladder supplies ZERO growing fresh
  equations — T-C7's conclusion unreachable as sketched (Codex-4 #2). The rev-5
  premise is the GAP form `Tendsto (Mlev − M₀) atTop atTop` on every consumer
  (record 11; type-level protection).
* rev 4's `ClassTree.leafV : List` admits association-list PERMUTATION: two
  Nodup lists differing by swapping two entries give extensionally equal
...
* rev 5's `child_cell` at o = none is JOINTLY UNSATISFIABLE with
  ...
  at F1 the joint row is CARDINALLY CONSISTENT (REV 7, Codex-6 g1 — the rev-6
  "SATISFIABLE" wording is DOWNGRADED: numerical compatibility ≠ existence):
  stateEvent(head) = the 14-pin stratum, entEvent(.red g X) = the 4-pin
  reduction cell, and on 5²² boxes 5^{22−14} · 5^{10} = 5^{18} = card(rootCell)
  ✓ — (JC-single) arithmetic at the 10-pin head window, with the .amb cell no
  longer forced to decide window digits; the concrete
  CellAssign/ledger/scaffold WITNESS is the §6 E-phase obligation row, owed,
  not claimed (record 13; type-level + carrier gates).
...
* rev 6's toy `cellOf .red` guards read NEITHER g NOR the reduction coords
  x0/x1 (Fable-6 CRIT-1): at g = (1,0), x = (1,0,0,…,0) ∈ rootCell χ g, carrier
  A's cell was winC (only ψ = X ∧ x2…x5 = 0 checked), so tAν₁ was eligible
  while `toyChildA none tAν₁ x` demands x0 = 0 — `child_cell_red` (∀ χ g)
  decide-FALSE on BOTH carriers, `toyCA`/`toyCAB` uninstantiable as displayed,
  and the rev-6 "hred is instantiable" walk-through unreal at any g ≠ toyG.
  The rev-7 guards pin x0 = 0 ∧ x1 = 0 on both carriers; violation counts
  24 → 0 (A) and 192 → 0 (B); cellEventE at the true entrance and every census
  integer unchanged (arithmetic displayed at T-G1) (record 17; carrier gates).
* rev 6's `cellLevel := fun _ => 3` made `cell_local` decide-FALSE on both
  carriers (Fable-6 CRIT-2): levelIdx (n = 2) puts coords 4/5 at level 3, so
  the premise constrained only c ∈ {0…3} while the .st cells read x4/x5 —
  witness x = 0⃗ vs x' = 0⃗-except-x'4 = 1 (premise satisfied, splitC ≠ junk).
  The rev-7 value 4 covers levelIdx c < 4 ⟺ c < 6 = every read coordinate;
  no census integer moves (record 18; carrier gates).
* rev 6's `ClusterFactorOf` lawfulness was still ABSTRACT (Codex-6 #3): a
  monic irreducible `trackOf` of degree > n divides NO cluster degree
  (1 ≤ deg fη ≤ n), so the divisibility conjunct made `ClusterFactorOf`
  witness-free and `OmSat` vacuous AGAIN — the same channel as record 15 one
  degree up. The rev-7 conjunct ties the head's track value into
  `normalizedFactors (f.map toZMod)` at the `ReadsOf`-realized f — the
  REALIZED roster, not a degree bound. REV 8 (Codex-7 #2 = Fable-7 GAP-1):
  the conjunct CANNOT be this record's protection — inside an implication's
  antecedent it makes `OmSat` EASIER to hold, and a degree-(> n) `trackOf`
  still discharges it vacuously; the protection is the SHARED-accessor pair
  (`root_total`'s per-track ∃ + the REV-8 `om_total` totality leg), and the
  conjunct's genuine role is the wrong-reduction direction (record 19;
  type-level, re-attributed).
* rev 7's `NsBridgeStmt` omitted the note's DECAY legs and was FALSE as
  stated (Codex-7 #3 = Fable-7 CRIT-1): childless tower + junk μ·Dwidth = 0
  node ⇒ interior roster ∅, L ≡ univ, μcap ≡ 1, μclass = 1 — every premise
  held, conclusion 1 = 0. The rev-8 type carries the C(b) pricing
  (μcap M = p^{−Z M}) + the growth (Z → ∞); the counter-model dies at that
  pair (re-run displayed §2.10) (record 20; type-level).
* rev 7's T-F4 was FALSE at the empty shape (Codex-7 #5 = Fable-7 CRIT-2):
  built `mult` = 1 at reads = [] while `PrefIdx` is empty — 1 = 0 + 0. The
  rev-8 form carries D11a's own `hne` + the displayed ∅-shape companion
  (record 21; statement-level, the built `mult_empty_shape`/`mult_sum_opt`
  cited by name).
* rev 7's toy roster was UNLAWFUL against the built Node/ShapeRead
  (Codex-7 #4 = Fable-7 GAP-2): `.increment` leaves at e·g = 1 (hspecInc),
  γ = 1/2/3 against the γ-tie's 3/4/6, slope-only line pins, and the
  (ψ = X, Ranch = 0, μ ≥ 1) triple jointly unsatisfiable
  (hRanch/hOrd/hpat0/hzbarRoot). The rev-8 tables re-pin all four families
  and DISPLAY the full proof-field walk; protection = the REV-6 walk-pins
  audit rule + the displayed roster (record 22; table-level).
* rev 7's `ChildCover` was defined but NOT INSTALLED (Codex-7 #1): no field,
  no displayed consumer — the constant-false-child model still instantiated
  the displayed interface. The rev-8 `CellAssign` bundles the field;
  `constFalse_no_cellAssign` (T-D2) is the installed rejection
  (record 23; interface-level).
...
Imports (REV 4, Codex-3 #19 — the NORMATIVE full roster; rev 3's list omitted the
§2.10/§2.9 consumers): `Mathlib`, `LeanUrat.MovesD.Defs`, `LeanUrat.MovesSp.Defs`
(MovesC comes through MovesD), `LeanUrat.MovesS.Interfaces` (which transitively
carries `MovesS.Defs` — `LedgerIV` lives there), `LeanUrat.MovesS.EvalReDef`,
`LeanUrat.HC2.Defs` (spec-only consumers: `WallAReconciled` + REV 5's
`ClusterFactorOf` — fence notes §2.9/§2.7).
Opens: `Polynomial LeanUrat.Moves LeanUrat.MovesC LeanUrat.MovesD`.
...
/-- REV 3 (Fable-2 CRIT-2), REV 4 (Fable-3 CRIT-1 = Codex-3 #4's wchain leg),
REV 5 (Fable-4 CRIT-1 — the Matches leg): **the KEYING-CONSTANT-FREE FRAGMENT of
history lawfulness** — T-V8's premise, strong enough to source BOTH conjuncts of
T-V8's conclusion. The rev-3 sin was a ShapeWF sourcing claim with one clause
unsourced; the rev-4 sin was auditing the ShapeWF conjunct's 12 clauses while
leaving the Matches conjunct's 13 UNAUDITED — the same wrong-warrant channel,
one conjunct over. Both rosters are now displayed.
**ShapeWF ROSTER (all 12 real clauses, each named to its source):** species_iff ←
`History.root_iff` (carrier law, free); monic ← `readOf`-definitional; dchain0 ←
the root Φ-degree-1 clause + the Node FIELD `hDwidth : Dwidth = σ.Φ.natDegree`
(Codex-3 #4's "missing initial Dwidth = 1" leg is REFUTED by this built signature —
Fable-3 §1(2) computed it); dchain ← the Dwidth chain; window ← the window clause;
gamTie ← the γ-tie; anchorTie ← the anchor clause; edvd/gmu ← the per-node ledger
clauses (gmu REV 4: stated on `ν.wSide / ν.e` — rev 3 read a NONEXISTENT `ν.len`;
...
copies ν's like-named fields — the eleven copies are the only genuinely
definitional part of the rev-4 warrant); conjunct 12 (pool card:
`Nat.card ↥ν.σ.K = p ^ R.w`) ← `NodeDataLawful`'s p-power clause with the w-guard
dead (`readOf_guards_dead`, T-V8); **conjunct 13 (the §0.5 monic-lead tie:
`R.monicTop = true → ν.sideDigit R.len = 1`) ← the REV-5 MONIC-LEAD clause
below** — rev 4's roster provably could not source it (§0 refutation record #9:
`readOf` does not copy `pat`, and the species guard ARMS the tie at every genuine
monic root read via `History.root_iff`, so the p = 3 pat-top-2 one-node instance
was lawful-but-mismatched). The clause is the L3 dictionary's §0.5 law — a monic
box member's degree-n slot digit IS 1 — stated at the ROOT node on the monic slot
(s0 + wSide = n; `readOf`'s `monicTop` decides true exactly there, r = 0 + root
species forced by the carrier law), on the real accessor `Node.sideDigit ν
(ν.wSide / ν.e) = ((ν.pat (wSide/e) : ↥ν.σ.K) : F)` (MovesD/Defs 301).
HONESTY OF THE FENCE (the round-3 adjudication, extended at REV 5): the w-mult
clause's HC home is the 4th-conjunct KEYING legs (TransitionCoreL's field growth
F_{i+1} = F_i[z]/(ψ_i)) — it IS keying content, so the fragment is no longer sold
as "child-keying-free"; the monic-lead clause's home is the L3 ROOT-BOX data
(box-normalization, not keying). NEITHER pins a keying CONSTANT (no recentering
core, no lift, no Bézout datum — exactly Q1's V10-inconsistent targets), so
Q1-INSENSITIVITY holds and the Q1 fence covers both clauses' discharge (§4).
Remaining clauses = `HistoryCoherent`'s conjuncts 1–3 VERBATIM + the 4th
conjunct's non-keying trio (window, Dwidth chain, slope increase) +
`NodeDataLawful` + the two SW1 side conditions [E-phase: each bound to its MovesC
`Node` accessor; BOTH rosters below are NORMATIVE — a clause neither SW1b nor the
Matches leg consumes is dropped at E-phase with a MANIFEST deviation, never
silently added; a clause EITHER leg needs beyond the rosters is ADDED with a
MANIFEST deviation — the hatch covers both conjuncts, closing the channel Fable-4
named]. -/
...
-- clause forces μ ≥ 1, K-card p-powers, card K₀ = p, or the monic-lead digit); the
-- discharge from the re-keyed HistoryCoherent + node/root ledger data + the keyed
-- field-growth law (the REV-4 w-mult clause's HC source) + the L3 root-box data
-- (the REV-5 monic-lead clause's source) is QUEUED ON Q1 (§4). Until then
-- `hlaw : HistLawful p n H` is an explicit hypothesis — honest conditionality.
-- COROLLARY carried for `readOf`'s guard (REV 4, Fable-3 GAP-2): w₀ = 1 + w-mult
-- + g ≥ 1 give card K_i ≥ p at every read, so `(Nat.card ↥ν.σ.K).factorization p
-- ≥ 1` on the lawful domain — T-V8's `readOf_guards_dead` displays it.

/-- (τ-irr) FIRES — **the note's rule VERBATIM** (REV 4, Codex-3 #1): "A branch whose
accumulated invariants E := ∏_r e_r, F := ∏_r g_r … SATURATE the cluster degree —
E·F = d(cluster) — halts … (cluster degree = key degree · μ = E·F·μ)" (7074–7080).
In node data the latest read's key degree is `ν.childWidth` (= e·g·Dwidth, MovesC
465), so d(cluster) = `ν.childWidth · ν.μ` and the rule is the displayed EQUATION.
REV-4 CHANGES against rev 3, both per Codex-3 #1: (a) the SATURATION EQUATION is
the definition (rev 3 carried only its μ = 1 shadow); (b) the rev-3 `NodeDataLawful`
conjunct is REMOVED — the note's rule is "pure node/cell data" with NO lawfulness
clause (lawfulness is standing perimeter, §0 row; folding it in silently
strengthened τ). The note's own parenthetical equivalence — saturation ⟺ μ = 1 at
the latest read — is the THEOREM T-V9 (`irr_iff_mu_one`, under `HistLawful`:
accE·accF = childWidth makes the equation collapse to μ = 1), never a silent claim.
Pure node/cell data: no box input, no f. -/
...
family at a jet of the state (§2.5a, with the REV-4 CAPPED slot-0 ladder) that
MEETS ITS LEVEL (`M₀ J ≤ Mlev J N` — REV 4) and whose cap accessors are
SCALE-FAITHFUL (`ScaleFaithful J N`, §2.5a — REV 5, Codex-4 #1: the jet's
`Mlev`/`M₀` are the LEVEL'S OWN (H, N)-determined Scale/entrance values, per
their pinned defining clauses "M(N) := the largest window height with base level
< N — the Scale map" (7205–7207) and the ladder "one coordinate per level,
heights unit-spaced" (7227–7229); the built chart is unique per level,
`coordOf_sorted`, so every presented jet satisfies this for free) — so
childlessness at a realized state is pinned by `NsLumpFamily.tie` to the
digit-vanishing datum of the state's OWN window, and the note's (τ-ns) =
"J(f) = ∅, the exact key power" (7081–7083), never arbitrary absence of children.
FALSIFIER (REV 5 — now FIXED-LEVEL at every scale-positive level; Codex-4 #1's
counter-model dies at `ScaleFaithful`): a model with `mem` everywhere and `child`
nowhere forces, at any state and any supplied family, ∀ x, ∀ M, x ∈ L M (`tie`);
Codex-4's degenerate jet CHOSE `Mlev J N = M₀ J` as free data — `ScaleFaithful`
removes that freedom, so at any level where the level's own Scale value exceeds
the entrance resolution (`M₀ < Mlev` FORCED), the capped `slot0` makes
`interiorFreshIdx J (M₀ J + 1)` NONEMPTY and `vanish` pins a digit to 0 — some
box member violates it (p ≥ 2), so L (M₀ J + 1) ≠ univ: `ns_lumps` FAILS AT THAT
FIXED LEVEL, pricing-free. DISCLOSED RESIDUE (the honest finite-level
transcription boundary, UNCHANGED in content, NARROWED in scope): at a
SCALE-DEGENERATE level (the level's own Scale value = the entrance resolution —
a level below the window's first fresh height), all ladder clauses are vacuous
and a junk full-box family satisfies the per-level bundle; NO finite-level
statement can reject childlessness there. That residue is closed INTERNALLY by
`KBTotTower` below (Codex-4 #1's "the tower-scoped rejection is external to
KBTot itself" — accepted: the tower is now a displayed consumed object, not
commentary), which is what `VPPinned.covering` consumes (§2.10). FIXED-LEVEL
INHABITATION (carried): T-C5 discharges the capped `slot0` at every presented
jet; `hdi : KBTot T` is instantiable. Owner HC-2/D4R0K; NEVER proved here. -/
structure KBTot (T : TreeModel p F n N m pol) : Prop where
  ...
  ns_lumps : ∀ (H : History p F), (∃ x, T.mem (some H) x) →
    ...
    -- the family's `tie` + capped `slot0` fields (§2.5a) carry the content; the ∃
    -- is the realized-state supply (HC-2's presented faces / D4R0K B(iii)); the
    -- `M₀ ≤ Mlev` conjunct is T-C7's own `hM₀` at the instance (REV 4); the
    -- `ScaleFaithful` conjunct pins the cap accessors to the level's own values
    -- (REV 5, Codex-4 #1 — free at every presented jet, fatal to the degenerate
    -- jet choice)

/-- **KB-TOT ALONG THE PINNED LEVEL FAMILY** — NEW at REV 5 (Codex-4 #1): the
profinite Theorem-B totality claim transcribes to finite level as the PER-LEVEL
...
reduction. REV 7 (ruling 2 — Codex-6 #1 = Fable-6 GAP-1, the round's CONVERGED
finding): the rev-6 leg concluded only ∃ SOME root child from ∃ SOME repeated
factor — one unrelated child could witness EVERY repeated factor (a model with
f̄ = ψ₁²ψ₂² realizing only ψ₁'s track child inhabited the closure while the
note's display is PER LIVE TRACK: "fix f and a live track … m_i ≥ 2 opens the
window — total", and "one track per root cluster", 7069–7071). The leg is now
PER-TRACK — ∀ ψ repeated, ∃ ν with `trackOf ν = ψ` carrying the child — so the
structure gains the shared `trackOf` accessor (the SAME parameter
`RedCellPartition`/`ClusterFactorOf`/`VPPinned` read). This closure is the
object `VPPinned.covering` consumes (§2.10): the childless tower STILL fails
`root_total` at the zero member (f̄ = xⁿ, X repeated, n ≥ 2 — no ν at all, a
fortiori none on track X; the record-14 refutation stays dead, verified by the
round-6 auditor) — and root children feed `mem_single` → realized states →
`scale_grow`'s antecedent, so the deep-level machinery engages; the
childless/constant model BELOW a realized history fails as at REV 5
(`ScaleFaithful` + the per-level falsifier). The scale-degenerate fixed-level
residue of `KBTot` is thereby closed inside the consumed Prop, not by external
commentary. Owner HC-2/D4R0K; NEVER proved here. -/
structure KBTotTower (pol : CanonPolicy p F) {N₀ : ℕ}
    (Tat : ∀ N', N₀ ≤ N' → TreeModel p F n N' (n * N') pol)
    (χat : ∀ N', Fin n → Fin (n * N'))
    (trackOf : Node p F → Polynomial (ZMod p)) : Prop where  -- REV 7 (ruling 2)
  perLevel : ∀ N' h, KBTot (Tat N' h)
  root_total : ∀ N' (h : N₀ ≤ N') (x : Box p (n * N')),   -- REV 6 (Codex-5 #1);
    ∀ ψ : Polynomial (ZMod p),                             -- REV 7 (ruling 2):
    ...
    -- "m_i ≥ 2 opens the window — total" (7113–7115), PER LIVE TRACK: every
    -- window-opening track has a realized root child OF THAT TRACK at every
    -- level; the childless tower dies HERE, and the one-child-for-all-tracks
    -- model (Codex-6 #1's witness) now fails at the second repeated factor
...
-- [`JetTower` — T-C7's truncation-tower def, HOISTED per the declaration order
...
ledger gains `sides`/`splitAt`/`state_cell` (Fable-2 CRIT-3). REV 4: `fiberAt`
gains clause (vi) + the `heads`/`trackDeg` carriers (Codex-3 #2); `SideSplit`
gains the side-SPAN fields typing the display's "DISJOINT slot spans" (Codex-3
#8's tie, note text quoted there); `SiteLedger.hparent`'s `.st` leg is the
IMMEDIATE parent (Codex-3 #16); `readOf`'s junk guards are displayed (Fable-3
GAP-2, dead-on-lawful-domain at T-V8's `readOf_guards_dead`). REV 5: `SideSplit`
gains the POLYGON tie — `hullSlope`/`hslope`/`hside_read`, reading each branch's
own `line.slope` — and the ledger gains the count tie `hsplit_k` (Codex-4 gap
#11); the recursion frames' priced events move to T-E7's `SubtreeFiber`/`scope`
(Codex-4 #3).
...
/-- the tree's HEAD chains (REV 4, Codex-3 #2): the length-1 chains — `hclosed`
puts every chain's one-node prefix in `chains`, so these enumerate the tree's
window-opening TRACKS. -/
noncomputable def VTree.heads (Tr : VTree p F) : Finset (History p F) :=
  Tr.hfin.toFinset.filter (fun H => H.nodes.length = 1)

/-- the PER-TRACK leaf-degree total (REV 4, Codex-3 #2): the summed verdict degrees
of the leaves lying over one head — a track's whole-cluster degree account
(non-leaves contribute 0 through `leafV`'s `none`). -/
...
d = n" (7062–7063) summed over the complete tree's leaves + hen tracks; (vi) — NEW
at REV 4 (Codex-3 #2, the PER-TRACK completeness face) — the multiset of per-head
leaf-degree totals equals the multiset {m_i·deg ψ_i} over the REPEATED reduction
factors of x: the note's per-cluster conservation (VP-1) summed track-by-track —
"one track per root cluster" (τ-hen's display), a track with m_i ≥ 2 opening the
window with cluster degree m_i·deg ψ_i. Clause (vi) is the all-and-only tree GRAPH
face: it kills track-dropping AND cross-track compensating errors (a dropped track
fails the multiset cardinality; a degree-shifted pair of tracks fails the multiset
values). ADJUDICATION NOTE (the two round-3 verdicts, both honored): Fable-3
verified (v) FAITHFUL to (TGT)/VP-1's typing — it stays; Codex-3 #2's residue
(compensation between leaves) is (vi)'s content; and (v) is now DERIVED strength —
(iii)+(vi) imply it (henV's simple-factor degrees + the repeated-factor totals sum
to n = deg(f̄), a one-line lemma recorded at T-D2), so no theorem is "smuggled":
the load-bearing clause is graph-shaped. Per-LEAF cluster-degree correctness is
NOT a fiber clause — it is the τ-rule itself (`IrrHalts`'s REV-4 saturation
equation) plus T-V9's lawful-domain theorem, exactly where the note derives
conservation. -/
    ...
    = ((UniqueFactorizationMonoid.normalizedFactors (redPoly χ x)).toFinset.filter
        ...
        (fun ψ => Multiset.count ψ
          (UniqueFactorizationMonoid.normalizedFactors (redPoly χ x))
            * ψ.natDegree))
-- [(vi) E-phase caveat, recorded: distinct tracks are assumed to carry distinct
--  HEAD NODES (the node's frame datum individuates its track — L1's level-0
--  dictionary); if the canonical instance ever merges two tracks' head values the
--  multiset equality fails-safe (cardinality mismatch), never silently passes.]
...
REV 4 (Codex-3 #8, adjudicated WITH the note's text): the display's own structure is
"the joint cell's literal system is the union of the k per-side systems over
DISJOINT slot spans (hinge vertices shared, values consistent)" (7477–7479) — the
partition IS that structure (hinge sharing is a shared-VERTEX/value-consistency
side condition, never a shared free clause), and the per-side/side-j TIE the finding
demanded is now typed: each side carries its SLOT SPAN, spans pairwise disjoint,
side clauses supported in their own span [E-phase: `sideSpan` bound to the hull
datum's per-side slot range — same owner as `splitAt`, HC-2's presented face].
REV 5 (Codex-4 gap #11 — "no field connects them to a hull-side datum"): the sides
are now tied to the POLYGON ITSELF through the branches' own line data: each side
carries its HULL SLOPE, slopes strictly increasing across sides (the polygon's
side order — a hull with k sides has k distinct slopes), and every branch's own
`line.slope` (a REAL `Node` accessor — the same one `HistLawful`'s slope tie
reads) EQUALS its side's hull slope. An arbitrary regrouping of clauses now fails
`hside_read`/`hslope` unless it reproduces the polygon's slope structure; the
count tie `k = sides H` is the ledger's (`hsplit_k` below, same finding). -/
structure SideSplit (T : TreeModel p F n N m pol) (CA : CellData p F n N m pol T)
  ...
  hullSlope : Fin k → ℚ                            -- REV 5: the side's polygon slope
  hslope : StrictMono hullSlope                    --   (k sides, k distinct slopes,
                                                    --    in the hull's side order)
  hside_read : ∀ ν ∈ CA.branchSetOf c,             -- the branch-to-side tie reads the
    ν.line.slope = hullSlope (sideOf ν)            --   branch's OWN line datum
...
structure SiteLedger (Tr : VTree p F) (T : TreeModel p F n N m pol)
  ...
  hparent : ∀ H ∈ Tr.chains, match parentSt H with
    ...
    -- REV 4 (Codex-3 #16): the entrance is the IMMEDIATE parent — the length-(k−1)
    -- prefix, which `hclosed` guarantees is a chain. Rev 3's "any earlier prefix"
    -- let T-D14's telescope skip/duplicate conditioning states while still summing
    -- every read's exponent — the displayed telescope was false for such ledgers.
  ...
  hsplit_k : ∀ H (hH : H ∈ Tr.chains) (h2 : 2 ≤ sides H),
    ...
    -- REV 5 (Codex-4 gap #11): the exhibited decomposition's side COUNT is the
    -- ledger's own hull-side count — `splitAt` can no longer regroup into a
    -- different k than the polygon has
...
fields. REV 4 (Fable-3 GAP-2 — the real `ShapeRead` carries PROOF fields the rev-3
values could not discharge on junk nodes): the two JUNK GUARDS are displayed IN the
value spec, each DEAD on the lawful domain (T-V8's `readOf_guards_dead`): `w` gets
`max … 1` (the field `hw : 1 ≤ w` — dead under `HistLawful`: w₀ = 1 + w-mult + g ≥ 1
give card K_i ≥ p) and `monicTop` gains the root-species conjunct (the field
`hmonicRoot : monicTop = true → species = root` — dead via the carrier law
`History.root_iff`). [REV 2, Codex 37: the mapping is displayed; the
structure-literal field ROSTER is bound at E-phase to `MovesD.ShapeRead`'s actual
field names — any mismatch is a MANIFEST deviation, the VALUES above are the spec.
`IsRootSpecies` := the root-species predicate `hmonicRoot` demands, E-phase-bound
to MovesD's species carrier.] -/
...
noncomputable def bandFresh (R : ShapePrefix) (n : ℕ) : Finset (ℕ × ℕ) :=
  -- REV 7 (Codex-6 #7): CLOSED over the BUILT MovesD band apparatus — no E-phase
  -- name-binding remains. The fresh coordinates (level, base) of R's LAST read's
  -- band system: `ShapePrefix.bandS` is the built per-read band predicate
  -- (MovesD/Defs 227–229), `Nshape` the built a-priori level bound (every band
  -- level sits below it, L12) — the same summand `ShapePrefix.A` counts at
  -- r = last.
...
-- the body (a card of the CLOSED `bandFresh`) and the mstar clause above are
-- NORMATIVE; `hdict` (T-E10) is now checkable against a fully displayed chain
-- (Codex-6 #7's demand: `bandFresh` gated `hdict`'s checkability).
```

### 2.5a the (ns) lump family (REV 2, Codex 38/39 — the B(iii) carrier, non-circular)

REV 4 (Fable-3 CRIT-2 = Codex-3 #3): `Mlev` is HOISTED here from T-C7 (declaration
order updated below) — every ladder clause in this section is CAPPED at it. The
note's ladder display is PROFINITE (all heights exist, 7216–7231); a finite level
carries only the heights below the Scale cap, and the rev-3 uncapped ∀-M forms were
UNSATISFIABLE at every fixed level (pinCount ≤ m — §0 refutation record #8).
...
/-- H's window-block (height, base level) roster — REV 7 (Codex-6 #4: rev 6's
"E-phase binds the NAME" left the principal non-vacuity mechanism resting on
undefined accessors; both are now CLOSED over the BUILT MovesC history chart).
The entrance window's interior is base index b < μ·D of H's LAST read (the
(ns-null) display's own convention, 7218); each interior block carries one
coordinate per level ℓ, whose absolute height is the built `History.htH`
(MovesC/Defs: ht(ℓ, b) = ℓ + Σ_{r} innerslot_r(b)·κ_r). Entries: (⌈height⌉₊, ℓ)
over ℓ < N', b < μ·D. -/
...
/-- the level's own SCALE value, H-SIDE, CLOSED — REV 6 (Codex-5 gap #7) body;
REV 7 (Codex-6 #4) closure. Body = the note's defining clause verbatim: "M(N) :=
the largest window height with base level < N — the Scale map" (7205–7207): the
sup of H's window-block heights whose base level sits below the cap. -/
noncomputable def hScale (H : History p F) (N' : ℕ) : ℕ :=
  ((windowBlockHts H N').filter (fun bh => bh.2 < N')).sup (fun bh => bh.1)
-- the roster is already level-capped, so the filter is definitionally total on
-- it — kept because the quoted clause's "base level < N" is the normative shape

/-- the ENTRANCE RESOLUTION, H-SIDE, CLOSED — REV 6 (same finding) body; REV 7
(Codex-6 #4) closure: the slot-0 block is BASE INDEX 0 ("slot 0's block b₀ (base
index 0 < μ·D — the window interior is never empty)"), and the floor value is
the BUILT cumulative staircase `History.floorH` at H's full prefix — no
`slot0Block` name remains to bind. Defining clauses, verbatim: "for each cap
M ≥ M₀ (the entrance resolution)" (7216–7217 — REV 7 cite fix, was mis-cited
7213); "heights ≤ F(b) are already zeroed by Σ's downset: inherited, not fresh"
(7220–7222) — so the fresh ladder starts strictly above the slot-0 floor:
"… one coordinate per level, heights unit-spaced, so |Z_M| ≥ ⌊M − M₀⌋"
(7227–7229). -/
noncomputable def hRes (H : History p F) : ℕ :=
  ⌈(H.floorH H.nodes.length 0).unbot' 0⌉₊
-- `floorH` returns `WithBot ℚ`; the def is TOTAL (`unbot' 0` defaults junk to
-- 0) and ⌈·⌉₊ is the height-grid reading (heights unit-spaced per level, the
-- ladder's own convention). EXACTNESS SCOPE (REV 8, Codex-7 gap: the rev-7
-- "exact on every nonempty history" claim exceeded the displayed support —
-- `floorH ≠ ⊥` at nonempty prefixes needs the last window's
-- positivity/interiority facts, i.e. node `staircase` values realized): the
-- `unbot'` is claimed exact on LAWFUL histories only (`HistLawful`/realizable
-- — where the staircases are genuine and ⊥ occurs only at prefix length 0);
-- on junk histories `hRes` is a total default, and its only consumer chain
-- (`ScaleFaithful` → `ns_lumps`/`KBTotTower`) meets it at PRESENTED jets,
-- whose `JetSetup.coherent`/`realizable` fields put them on the lawful
-- domain. The accessor itself stays closed and total — no statement change.
...
structure NsLumpFamily (T : TreeModel p F n N m pol) {H : History p F}
  ...
  slot0 : ∀ M, M₀ J ≤ M → M ≤ Mlev J N → M - M₀ J ≤ (interiorFreshIdx J M).card
    ...
    -- REV 4 (Fable-3 CRIT-2 = Codex-3 #3): CAPPED at `Mlev J N` — the profinite
    -- all-heights ladder transcribes to finite level only below the Scale cap;
    -- the uncapped form demanded unboundedly many pinned coordinates in a Fin-m
    -- box (unsatisfiable, §0 record #8). As a capped FIELD it still makes the
    -- family non-degenerate wherever M₀ J < Mlev J N (the tower falsifier, §2.4);
    -- T-C5 PROVES it for the real jets, so the field is discharged at every
    -- presented instance — the fixed-level inhabitation story Codex-3 #3 demanded.
...
structure ZCPack {H : History p F} (J : JetSetup H n N m) : Prop where
  ...
  slot0_grid : ∀ ht, M₀ J ≤ ht → ht ≤ Mlev J N →
    ...
    -- REV 4: capped like `slot0` — one interior block per height BELOW THE SCALE
    -- CAP; the uncapped grid needed unboundedly many distinct blocks (§0 record #8)
...
`EntSt`/`entEvent`/`embE` + §2.9's `CellData` (+ the bundled `CellAssign`)/
...
(T-E5), `SubtreeFiber`/`SplitSiteData` (T-E7 — REV 5), `TreeScaffold`/`multiSites`
(T-E8), the truncation carriers (T-C7; `JetTower` itself hoisted at (5) — REV 5),
`ReadLedgerT`/`readLedgerOf` (T-D14),
...
/-- `SubtreeTransport` — REV 4 (Fable-3 GAP-1 = Codex-3 #20: rev 3 consumed this
name inside PCI's value with NO definition anywhere): the "SAME PRESCRIBED SUBTREE
read at the two entrances" — the two designated chain sets are prefix-order
isomorphic with IDENTICAL read lists strictly above the shared child root ν (the
child-CTS-state identification, [2r]/CTS-M(iv)-REP's face). -/
...
-- [`stripAbove ν H` := H's read list strictly above its ν-headed child root —
--  E-phase: `List.drop` at the child-root length (`(childHist … ν …).nodes.length`);
--  the reads above the root are what "the same prescribed subtree" prescribes.]
...
and rev 4's "consumed by nothing" claim was WRONG — `VPPinned.om_sat` and the
re-keyed `vp` package consume the Prop; both errors are repaired, the language
RETRACTED). fη is a monic factor, of exactly H's cluster degree (d(cluster) =
childWidth·μ at the last read — `IrrHalts`'s own vocabulary), of SOME monic
degree-n integral polynomial whose run realizes H (the BUILT `HC2.ReadsOf` —
spec-only reference, same fence posture as `WallAReconciled`: OmSat is a
citation-obligation hypothesis leg, never a unit premise, so no unit premises
`HistoryCoherent` through it), and fη's level-0 reduction is its OWN track's
...
(§0 record 15); the head's track value is now monic ∧ IRREDUCIBLE with its
degree DIVIDING the positive cluster degree, so the power exponent is ≥ 1 and
pinned. REV 7 (Codex-6 #3): rev 6's abstract lawfulness still admitted a
DEGREE-(> n) monic irreducible `trackOf` — its degree divides NO cluster degree
(1 ≤ deg fη ≤ n), so `ClusterFactorOf` was witness-free and `OmSat` vacuous
AGAIN; the track value is now tied to the REALIZED factorization itself —
membership in `normalizedFactors (f.map toZMod)` at the very `ReadsOf`-realized
f (the same roster genre as `RedCellPartition`'s first conjunct, now at the
realized polynomial, not an abstract degree). Every genuine head track
satisfies it (the L1 frame factor IS a factor of its own f̄); every unrelated
assignment — degree > n, or a factor of a DIFFERENT reduction — fails the
membership conjunct. Every clause is displayed. -/
def ClusterFactorOf (n : ℕ) (trackOf : Node p F → Polynomial (ZMod p))
  ...
  ∃ hne : H.nodes ≠ [],
    ...
    ∃ f : Polynomial ℤ_[p], f.Monic ∧ f.natDegree = n ∧ fη ∣ f ∧
      ...
      trackOf (H.nodes.head hne)                            -- REV 7 (Codex-6 #3):
        ∈ (UniqueFactorizationMonoid.normalizedFactors      -- the REALIZED head-
            (f.map PadicInt.toZMod)).toFinset               -- track membership tie
-- consequence, recorded (not a clause): the membership bounds deg ψ ≤ n and
-- forces ψ into the realized f̄'s own factor roster; with deg ψ ∣ deg fη ≥ 1 the
-- exponent is ≥ 1, and fη ∣ f + the power clause make ψ the realized cluster's
-- track — the degree-(> n) and wrong-reduction escapes both die here.
...
/-- **(OM-SAT), the TOTALITY direction** — NEW at REV 8 (Codex-7 #2 =
Fable-7 GAP-1, part (a)): the note's display ASSERTS the cluster factor's
EXISTENCE for a realized saturated branch ("a saturated branch's cluster
factor f_η IS irreducible …", 7262–7273 — the definite article presupposes
the factor); `OmSat` alone is an IMPLICATION, and a junk `trackOf` (degree
> n, or a wrong-reduction value) makes `ClusterFactorOf` witness-free — so
`OmSat` was dischargeable by vacuity at every junk accessor. This law types
the existence: every `ReadsOf`-realized saturated history HAS an fη carrying
the full `ClusterFactorOf` package dividing ITS OWN realizing f. Realization
is expressed through the BUILT `HC2.ReadsOf` (spec-only, the same fence
posture as `ClusterFactorOf`/`WallAReconciled` — a citation-obligation leg,
never a mass-unit premise). Consumed as `VPPinned.om_total` (§2.10).
VACUITY ACCOUNTING, re-attributed (part (b) — §0 record 19): a junk
`trackOf` still empties THIS law's conclusion supply — what makes junk
accessors GLOBALLY uninhabitable at the consumed seam is ruling 2's SHARED
`trackOf`: `KBTotTower.root_total` (a `VPPinned.covering` leg over the SAME
accessor) demands ∀ ψ repeated, ∃ ν with `trackOf ν = ψ` at every level —
under a constant/junk accessor the ∃ fails at the second distinct repeated
factor (or at every ψ ≠ the junk value), so `VPPinned` as a WHOLE has no
instance; the realized-membership conjunct's genuine role is the
WRONG-REDUCTION/OVER-STRENGTH direction (it stops a lawful-looking fη being
certified against a factor of a DIFFERENT reduction). -/
def OmSatTot (p : ℕ) [Fact p.Prime] (F : Type*) [Field F] [Finite F] (n : ℕ)
    (trackOf : Node p F → Polynomial (ZMod p)) : Prop :=
  ∀ (H : History p F) (f : Polynomial ℤ_[p]),
    f.Monic → f.natDegree = n → LeanUrat.HC2.ReadsOf p F n f H →
    IrrHalts H →
...
structure CellData (p : ℕ) [Fact p.Prime] (F : Type*) [Field F] [Finite F]
  ...
  -- REV 8 (Codex-7 #1, the rename): this is the former rev-7 `CellAssign`
  -- field roster — the cell DATA/locality/child-law layer, which the site
  -- vocabulary (§2.5/§2.6) and the TOYS read; the W4-1 INTERFACE `CellAssign`
  -- is the bundled structure below, which additionally carries the mandatory
  -- covering field. Every displayed binder in this file formerly reading
  -- `CellAssign p F n N m pol T` now reads `CellData p F n N m pol T`
  -- (mechanical rename, NORMATIVE, applied throughout).
  Cell : Type
...
/-- **the reduction-cell branch PARTITION** — NEW at REV 4 (Codex-3 #7: rev 3's
`child_cell_red` stated only the ⊆ face, and the all-and-only converse — "at a
reduction-cell entrance the note's branch set is ALL eligible branches of the
selected track" — was "absent from every E-layer hypothesis"). The note's per-track
structure at level 0: "one track per root cluster" (τ-hen's display, 7069–7071);
the root's realized children partition BY TRACK, and a `.red g ψ` cell's branch set
is exactly its track's slice. REV 5 (Codex-4 gap #10: rev 4's bare `∃! ψ` had
"nothing relating ψ to the reduction factor encoded by ν, so a child can be
assigned uniquely to the WRONG factor"): the partition is now ANCHORED to each
child's OWN root track through `trackOf : Node p F → Polynomial (ZMod p)` — THE
SHARED L1 frame accessor (the node's level-0 frame datum individuates its track,
fiberAt (vi)'s caveat; the same parameter `ClusterFactorOf` reads, §2.7)
[E-phase: bound to the node's level-0 frame factor; owner HC-2 — track membership
is keying content, Q1-adjacent]. Hypothesis `(trackOf, hred)` on
T-S1/T-E8/T-E10/T-E11 and inside the §2.10 closures. -/
...
-- the second conjunct = `child_cell_red`'s ⊆ face (kept there for the CA-field
-- consumers); the first is the all-and-only converse ANCHORED (REV 5): every
-- realized root child sits in its OWN track's reduction-cell branch set — a
-- member of x's factor roster — and in NO other; the rev-4 wrong-factor
-- assignment now fails the middle conjunct

/-- **the (c2) COVERING clause AT THE W4-1 INTERFACE** — NEW at REV 7 (ruling 1,
the FIFTH-round adjudication OVERTURNING the four-round W4-1 BDY hold): the
note's covering case-analysis lives AT this layer — "fix f and a live track. At
level 0, m_i = 1 gives τ-hen and m_i ≥ 2 opens the window — total" (7112–7119),
the continuation existing by L1 totality (D4R.0-K(a), note-DISCHARGED). At a
realized window-opening read (a repeated reduction factor ψ), a root child
EXISTS carrying that track's data: it sits in the track-ψ reduction cell's OWN
branch set. `RedCellPartition` above is the all-and-only SORTING law
(conditional on children/members existing); THIS is its nonemptiness/covering
companion — together they are the W4-1 contract's (b)-union at this layer. The
constant-false-`child`/`Cell := Unit`/empty-`branchSetOf` model (the instance
the contract text forbids, pressed at rounds 3–6) is now FALSE at the W4-1
interface ITSELF: at f̄ = xⁿ (n ≥ 2) the ∃ fails — the in-corpus theorem
`constFalse_childCover_false` (T-D2) displays it, no `HistoryCoherent` fence
crossed. CONTRACT CHANGE, recorded for the MovesD-side W4-SYNC (§5 W4-1 row;
`W4_SYNC_DELTAS_2026-07-29.md` entry 10). Owner HC-2/D4R0K (L1 totality) —
NEVER proved here. REV 8 (Codex-7 #1, ruling: rev 7 left this a STANDALONE
predicate no displayed consumer took — "constant-false `child` can still
instantiate the displayed CellAssign; it only fails an optional predicate
that no shown package requires"; correct): it is now the mandatory FIELD
`child_cover` of the bundled `CellAssign` below — the interface ITSELF
rejects the constant-false-child model. -/
...
-- per-track, same quantifier shape as `KBTotTower.root_total` (§2.4 — its
-- tower form drops the CA conjunct); the branch-set conjunct is what makes
-- this a W4-1-LAYER law rather than a bare tree law

/-- **THE W4-1 INTERFACE, BUNDLED** — NEW at REV 8 (Codex-7 #1, the round-7
convergence: the rev-7 `ChildCover` was defined but not INSTALLED — no field,
no displayed consumer). `CellAssign` is now `CellData` PLUS the mandatory
per-track covering field; it takes the chart and the shared L1 track accessor
as PARAMETERS (the covering is stated at the designated chart and the shared
`trackOf` — a ∀-χ or ∀-trackOf field would be false/unsatisfiable at the
canonical instance, and an ∃ would untie the accessor the other laws share).
CONSUMERS (displayed): T-E5's `fiber_root_split`, `TreeExpFin`, `TreeNStmt`,
`VPPinned.CAat`, `rs1GivenOfMovesT`/`xrbOfMovesT` (§2.10) — the package/mass
layer takes the FULL interface, so the covering can no longer be dropped by
omission; the site vocabulary and every other displayed law read the
`CellData` layer (through `toCellData` at consumption — E-phase coercion).
The TOYS instantiate `CellData` ONLY (re-fenced HONESTLY, T-G1: carrier A
FAILS `child_cover` — at g = (1,0), x = (1,0,0,…) the reduction x² + 1 =
(X+1)² has X+1 repeated, but `toyChildA none` demands x0 = 0: NO realized
child exists, the ∃ fails; the toys are deliberately partial, one realized
branch on one track over the pinned sub-stratum). The canonical instance's
`child_cover` discharge is owner HC-2/D4R0K (L1 totality), like every other
canonical-instance law. -/
...
structure VerdictModelT (T : TreeModel p F n N m pol)
  ...
  capIrr_law : CapIrrLaw p F n pol capIrr   -- DISPLAYED below (REV 3, Codex-2
...
"one cap per class across ALL presented N" (W4-2(b)'s addendum), QUANTIFIED over
the presented family. REV 4 (Codex-3 #5): the BOX SIZE is quantified too — the
level-N' cylinders TREE-N actually consumes live in `Box p (n·N')`, a box that
GROWS with the level, so a law at one fixed m was not one cap across that family;
the cap function takes no N and no m (uniformity by type), and the law now demands
it at every presented level IN ITS OWN BOX. (Fable-3 had verified the rev-3 shape
against the addendum; Codex-3's growing-box residue is honored as an ADDITIVE
strengthening — the m'-quantifier — with nothing weakened.) -/
...
/-- the WITHDRAWN uniform cap-envelope locality — DISPLAYED at REV 4 (Fable-3
GAP-4's fourth slip: T-D4's falsifier targeted an undisplayed name): ONE cap for
`cellOf` at ALL entrance states. T-D4 exhibits its failure — the (b)/(c)
conflation the contract records (W4-2(b)(ii)). Falsifier-only vocabulary. -/
...
structure CapData (T : TreeModel p F n N m pol) (CA : CellData p F n N m pol T)
  ...
  cap_law : CapIrrLaw p F n pol capIrrOf     -- discharged by T-C2's family theorem
```
...
hCellFin/cell_local` → `CellData` (the per-state-level `cell_local`, the
...
def TreeExpNs (pol : CanonPolicy p F) : Prop :=
  ∀ (Tr : VTree p F) (H : History p F), (H ∈ Tr.chains ∧ Tr.nsLeaf H) →
    ∀ (N₀ : ℕ) (Tat : ∀ N', N₀ ≤ N' → TreeModel p F n N' (n * N') pol)
      ...
      (∀ N' h, ZCPack (Jat N' h)) →                    -- REV 4 (Codex-3 #6)
      (∀ N' h, M₀ (Jat N' h) ≤ Mlev (Jat N' h) N') →
      Filter.Tendsto                                    -- REV 5 (Codex-4 #2): GAP form
        (fun N' : {k // N₀ ≤ k} => Mlev (Jat N'.1 N'.2) N'.1 - M₀ (Jat N'.1 N'.2))
        Filter.atTop Filter.atTop →
...
sentence, stated over an ABSTRACT cap-mass field in the corpus's own carriers:
`μcap M` = the conditional mass μ(Σ ∩ L_M │ Σ) of the cap-M lump, `μclass` =
the class's conditional mass — both ABSTRACT reals the owner supplies —
carrying the laws the note's ∎ actually uses (7216–7233): (i) antitone in
the cap (nested lumps); (ii) THE C(b) PRICING WITH THE LADDER GROWTH — a
fresh-pin count Z with μcap M = p^{−Z M} (the note's (ii): "Theorem C(b)
with Z = Z_M prices the lump exactly, μ(Σ ∩ L_M) = μ(Σ)·p^{−|Z_M|}") and
Z → ∞ (the note's (iii): "|Z_M| ≥ ⌊M − M₀⌋ → ∞"); (iii) CONTINUITY FROM
ABOVE — the class = ∩_M (Σ ∩ L_M), its mass THE limit of the cap masses
(the genuinely profinite step — μclass's identification as the (τ-ns) class
mass IS this limit reading, the owner's profinite-μ business, riding the S-2
row); (iv) each cap mass READ OFF the finite levels — the ε-limit of the
...
REV 5 (Codex-4 #1): the structure is TOWER-KEYED — (c2)'s covering is Theorem-B
TOTALITY, a profinite claim whose honest finite transcription is `KBTotTower`
(per-level faces + the pinned level family, §2.4); rev 4's fixed-level
`covering : KBTot T` was degenerately inhabitable (the childless/constant model at
a scale-degenerate level), so the seam VALUE now carries the internalized tower
rejection. The (c1)/(c3-a) verdict models ride per level along the same family;
the two cites are level-free. `trackOf` = the shared L1 frame accessor (§2.7/§2.9). -/
structure VPPinned (pol : CanonPolicy p F) {N₀ : ℕ}
    ...
    Prop where
  -- REV 8 (Codex-7 #1): `CAat` is the FULL W4-1 interface per level (the
  -- covering field rides in the vp value; binder order: the chart family and
  -- the shared accessor precede the structures that read them)
  model : ∀ N' h,
    Nonempty (VerdictModelT (Tat N' h) (CAat N' h).toCellData (χat N'))
  covering : KBTotTower pol Tat χat trackOf
    -- REV 6 (Codex-5 #1): the tower reads the chart family — its `root_total`
    -- leg states the level-0 covering clause through `redPoly ∘ χat`; REV 7
    -- (ruling 2): the tower reads `trackOf` too — `root_total` is PER-TRACK
  hen_lift : HenLift p
  om_sat : OmSat p F n trackOf
  om_total : OmSatTot p F n trackOf
    -- REV 8 (Codex-7 #2 = Fable-7 GAP-1, part (a)): the TOTALITY direction —
    -- the note's existence assertion typed (§2.7); without it `OmSat`'s
    -- implication shape is dischargeable by witness-freeness alone
...
class, or `reprOf`-compatibility"; those bridges are exactly this bundle). Every
realized chain is CLASSIFIED — it lies in the PrefSet of its own shape with a
class index whose REPRESENTATIVE transports the chain's tree data: the same
membership event, the same child events, the same halting rule value, the same
verdict values; and the class of a nonempty prefix is a class-prefix (so
addresses of a tree's chains are prefix-closed). This is the η-class semantic
surface (L4/L6 — τ and verdicts are cell/class data, 7595–7598), the same owner
family as `EtaLoc`/`PreHaltClassData`/`PrefixCoherentRepr`: HC-2
(PresentNorm/Galois). Consumed by T-D11's `fibOf_partition` ONLY. -/
def ClassCover (T : TreeModel p F n N m pol) : Prop :=
  -- (the policy rides the standing binder through T's type — no re-binding; the
  --  Fable-3 GAP-4 binder-order class is checked)
...
-- ===== S-3's re-keyed premise, over the IMPORTED MovesS objects (REV 3/4) =====
-- imports: `LeanUrat.MovesS.Interfaces` (BUILT — `RS1Meas`, `βmeas`, `evalRe`;
-- transitively `MovesS.Defs` — `LedgerIV`, REV 4)
...
list + the ledger + `hxrbStmt` — never the bare `W2_xrb` implication. REV 4
(Codex-3 #18, the half-vindicated reversal COMPLETED): the ledger premise is the
REAL IMPORTED **`MovesS.LedgerIV T' M`** (Defs.lean 210, a Prop structure, BUILT —
build-check record above §0) — rev 3's `ledger_iv : Prop` parameter was "a slot
name for that future value, not the value", the exact round-2 failure pattern; the
value exists and is now consumed by name. (Fable-3's round-3 S-3 verification —
`hxrbStmt` verbatim the real `RS1Bundle.xrb` row, the conclusion verbatim the real
`recursion_meas` row incl. the `evalRe` callback and `activeState` guard — stands
untouched; both audits confirmed the βmeas typing.) -/
...
### 2.11 CLOSURE SUPPLEMENTS (NEW at REV 8 — the §8 manifest rule)

Round after round, "closed" defs turned out to be prose; the §8 CLOSURE
MANIFEST now requires every identifier consumed by displayed Lean to have a
displayed decl or a built-source line. This section closes every name that was
previously a bracket note. Each def is NORMATIVE in its VALUES; proof-term
plumbing inside bodies is E-phase (MANIFEST deviations as usual). Declaration
order: these HOIST per §2.5's order note (species/carrier helpers with (1);
jet accessors with (4); class helpers with (7)).
...
/-- the root-species predicate `readOf`'s monicTop guard reads. -/
def IsRootSpecies (s : ReadSpecies) : Prop := s = ReadSpecies.root
/-- a site's child ROOT datum: at the model root, a root-species node (the
track's first window read); at a state, a snoc-extension datum. -/
...
anonymous-constructor form the built `mem_single` itself displays — and the
snoc at a state). [E-phase: the two `⟨…⟩` proof slots.] -/
def childHist : ∀ (o : Option (History p F)) (ν : Node p F),
    ChildRoot o ν → History p F
  | none, ν, h => ⟨[ν], (by simp), (by simp [h, IsRootSpecies])⟩
  | some H, ν, h => H.snoc ν h
def oneNode (ν : Node p F) (h : ChildRoot none ν) : History p F :=
  childHist none ν h
-- CONSEQUENT RE-DISPLAY (T-E5): `RootSplitData.hcov` now reads
-- `hcov : ∀ i, ∃ h : ChildRoot none (headOf i), oneNode (headOf i) h ∈
-- Tr.chains` — the head-is-a-chain content unchanged, the root-species datum
-- made explicit (heads ARE root reads — `History.root_iff` at length-1).
/-- the length-k nonempty prefix of H, as an Option (consumed by
`chainReads`). [E-phase: the take-proofs.] -/
def prefixAt (H : History p F) (k : ℕ) : Option (History p F) :=
  if h : 0 < k ∧ k ≤ H.nodes.length
  then some ⟨H.nodes.take k, (by …), (by …)⟩ else none
/-- H's read list strictly above its ν-headed child root (SubtreeTransport's
"same prescribed subtree" comparator). [E-phase caveat: the child-root head
occurs once — the frame datum individuates it, fiberAt (vi)'s caveat.] -/
def stripAbove (ν : Node p F) (H : History p F) : List (Node p F) :=
  (H.nodes.dropWhile (· ≠ ν)).drop 1
/-- the henflip record-update (T-G1's gate). -/
def VTree.withHenV (Tr : VTree p F) (v : Multiset Vd)
    (hv : ∀ w ∈ v, ∃ g : ℕ, 1 ≤ g ∧ w.1 = {(1, g)}) : VTree p F :=
  { Tr with henV := v, hhen := hv }

-- ===== class helpers (the η-class vocabulary over the BUILT MovesD index) ==
/-- H lies in class i: its η-datum IS the class value (`PrefIdx` is the
subtype of `etaData '' PrefSet` — MovesD/Defs 414). -/
def SameClass {P : Shape n} {pol : CanonPolicy p F}
    (i : PrefIdx n pol P) (H : History p F) : Prop :=
  etaData (P : ShapePrefix) H = i.1
/-- the class-level prefix relation: some member of i' is a history-prefix of
some member of i. -/
...
-- ===== jet-chart accessors (over the BUILT `JetSetup` — coordOf/Sigma/fresh,
-- MovesC/Defs 792–860 — and the built `htH`/`floorH` chart the §2.5a
-- accessors already read; `Coord = (level, base)`, MovesC/Defs 217) =====
...
noncomputable def floorAt {H : History p F} (J : JetSetup H n N m)
    (j : Fin m) : ℚ := (H.floorH H.nodes.length (baseOf J j)).unbot' 0
...
/-- a DIG lead index: the chart-order lead of some clause of J's fresh
system (the ≺-largest support member — `coordOf_sorted` makes Fin-order the
global order). [E-phase caveat: if MovesC's (ZC) layer exposes a designated
lead accessor, bind to it with a MANIFEST deviation; this closed form is the
spec.] -/
...
/-- the CAP-M INTERIOR FRESH ROSTER: interior-base chart coordinates whose
height sits strictly above the floor and at most M (`NsLumpFamily`'s carrier
— "interior = base index < μ·D, fresh heights F(b) < ht ≤ M"). -/
...
/-- the level's Scale value READ OFF J's OWN CHART (jet-side; `ScaleFaithful`
ties it to the H-side `hScale` — nontrivial exactly for junk charts, free at
presented jets per `coordOf_sorted`). -/
noncomputable def scaleTop {H : History p F} (J : JetSetup H n N m)
  ...
  (Finset.univ.filter (fun j : Fin m =>
    ...
    (fun j => ⌈heightOf J j⌉₊)
/-- the ENTRANCE RESOLUTION read off J's own state locus: the largest
Σ-pinned slot-0 height (jet-side; `ScaleFaithful` ties it to `hRes`). -/
noncomputable def M₀ {H : History p F} (J : JetSetup H n N m) : ℕ :=
  (Finset.univ.filter (fun j : Fin m =>
      pinnedOnState J j ∧ baseOf J j = 0)).sup (fun j => ⌈heightOf J j⌉₊)
/-- Z's pins count / Z is EXACTLY the cap-M lump system (T-C5's carriers). -/
...
/-- the (level, base) support of J's pinned equations at Z (T-D7's own
Finset — exists in no built corpus, honestly this corpus's). -/
...
--  equations", 7178–7180 — T-C2's capIrrOf reads this)

-- ===== the S-11 carrier core (T-D13's target — exists in NO built corpus;
-- the FULL TreeIface is MovesV/[1v] vocabulary, §5 S-5/S-11) =====
...
-- ===== gate plumbing (the T-G1/T-G2/T-S2 names; node/tree VALUES are the
-- T-G1 closed-form tables — this block closes the surrounding plumbing) =====
noncomputable def polTriv : CanonPolicy 2 (ZMod 2) :=
  { liftOf := fun _ => Polynomial.X, … }
  -- [the constant-lift toy policy; remaining CanonPolicy fields (MovesD/Defs
  --  345) bound at E-phase to the same constant genre — MANIFEST deviations]
def toyχ : Fin 2 → Fin 9 := fun b => ⟨(b : ℕ), by omega⟩
def toyG : Fin 2 → ZMod 2 := fun _ => 0
def toyTrackOf : Node 2 (ZMod 2) → Polynomial (ZMod 2) :=
  fun _ => Polynomial.X          -- REV 8: the toys' constant track binding
def toyHcrA : ChildRoot none tAν₁ := rfl   -- tAν₁.species = .root (table pin)
noncomputable def toyTracksA : RootSplitData toyTreeA toyG :=
  { t := 1, trackChains := fun _ => {tA1, tA2a, tA2b},
    headOf := fun _ => tAν₁, hpart := (by …), hhead := (by …),
    hinj := (by …), hne := (by …), hcov := (by …) }
noncomputable def toyLedgerA : SiteLedger toyTreeA toyModel toyCA toyχ :=
  { sys := (the per-chain (Locus, FreshData) pins: head ↦ the {x0,x1}-pinned
      locus + the {x2…x5} 4-clause system; leaves ↦ the {x0…x5}-pinned locus
      + the EMPTY system — mstar 0),
    cellAt := (head ↦ .winC; leaves ↦ .splitC),
    parentSt := (head ↦ .red toyG Polynomial.X; leaves ↦ .st tA1),
    sides := fun _ => 1, freshCoords := (head ↦ {2,3,4,5}; leaves ↦ ∅), … }
  -- [proof fields G1a; the VALUES are the carrier-A table's own columns]
noncomputable def toyVdict :
    Option (History 2 (ZMod 2)) → Box 2 9 → Option Vd :=
  fun o x => (the τ-emission at the tables: some (irrVerdictOf tA2a/tA2b) at
    halted leaf states with x in their stratum; none elsewhere)
def toyTrees : Finset (VTree 2 (ZMod 2)) := (the singleton {toyTreeA})
...
/-- T-G2's below-halt surrogate: the SYNTACTIC continuation tA3 :=
tA2a.snoc tAνa (legal — species ≠ root) extends BELOW tA2a's τ-irr halt, so
...
noncomputable def twoNodeKcardH : History 2 (ZMod 2) := (§0 record #7's
  two-node instance: card K₀ = 2, card K₁ = 4 (the 𝔽₄-type stage), g₀ = 1,
  every other field the carrier-A head/leaf pins — VALUES per the record)
noncomputable def oneNodePatH : History 3 (ZMod 3) := (§0 record #9's p = 3
  one-node monic-slot instance: s0 + wSide = toyN3 · e = 1, pat top digit 2 —
  VALUES per the record; its own p = 3 carrier)
-- [the two record histories are ROSTER-PINNED like the T-G1 tables — each
--  field a literal; displayed compactly here by their defining records, the
--  full field lists at the records' §0 bullets + T-G1's gate rows]
```
...
**T-V8 `MovesT.shapeOfH_matches` · `V8_shapeOf.lean` · medium — REV 4 (Fable-3
CRIT-1 = Codex-3 #4's wchain leg: the w-multiplicativity clause joined the roster,
§0 record #7). REV 5 (Fable-4 CRIT-1 — the unit's THIRD refutation, at a THIRD
clause, this time on the MATCHES conjunct: the built `ShapeRead.Matches` carries
13 conjuncts (MovesD/Defs 313–318) and the 13th — the §0.5 monic-lead tie
`R.monicTop = true → ν.sideDigit R.len = 1` — was sourced by NOTHING: `readOf`
does not copy `pat`, and the species guard ARMS the tie at every genuine monic
root read, so the p = 3 one-node pat-top-2 instance was `HistLawful`-lawful while
`MatchesHist` FAILED — §0 record #9. The rev-4 sketch's warrant "componentwise
Matches is definitional" was TRUE for the eleven copied fields and the pool-card
clause and FALSE at exactly the 13th conjunct — the same asserted-not-audited
warrant channel as rev 3's, one conjunct over. The rev-5 premise is `HistLawful`
WITH the MONIC-LEAD clause (§2.2 — box-normalization content, keying-constant-
free, Q1-queued with the fragment, §0 row); BOTH conclusion conjuncts now carry
displayed rosters in §2.2's docstring (ShapeWF: 12 clauses; Matches: 13
conjuncts), each named to its source, with the E-phase hatch covering both legs.
Prior-round records carried: Codex-3 #4's Dwidth leg stays REFUTED by the built
`Node.hDwidth`; Fable-4 verified the other 12 Matches conjuncts check and that
the real SW1 side conditions (`hmt0` ENC-MONIC / `hw0`) both discharge — the
rev-5 hole was strictly the Matches DIGIT conjunct, now clause-sourced.**
```lean
...
theorem readOf_guards_dead (H : History p F) (n : ℕ) (hlaw : HistLawful p n H)
    ...
    max ((Nat.card ↥(H.nodes[i]'hi).σ.K).factorization p) 1
      = (Nat.card ↥(H.nodes[i]'hi).σ.K).factorization p
-- REV 4 (Fable-3 GAP-2): the `readOf` junk guards are DEAD on the lawful domain —
-- the w-guard by w₀ = 1 + w-mult + g ≥ 1 (card K_i ≥ p); the monicTop
-- species-guard by the carrier law `History.root_iff` (no theorem needed — recorded
-- in the docstring, consumed at SW1b's monic clause).
```
moves_ref: §D4-R L3 dictionary (TB-CAP(irr) and thr(T) read thresholds off it,
7562–7566); the monic-lead law is §0.5's (a monic box member's degree-n slot digit
IS 1). deps: Defs, MovesD (NEW QUEUED UNIT on the MovesD side, additive-only:
`SW1b ShapeWF_of_lawful` — the SW1 proof refactored to consume exactly the
keying-constant-free fragment; NO statement change to the existing
`ShapeWF_of_matches`; recorded in MovesD's manifest as a wave-4 companion).
sketch — the MATCHES LEG, per §2.2's REV-5 13-conjunct roster (the rev-4 warrant
"componentwise Matches is definitional" is CORRECTED — it covered 12 of 13):
conjuncts 1–11 definitional (readOf copies ν's like-named fields); conjunct 12
(pool card p^w) from the fragment's p-power clause with the guard dead by
`readOf_guards_dead`; conjunct 13 (the monic-lead tie) from the REV-5 MONIC-LEAD
clause DIRECTLY — `readOf`'s `monicTop` decides true exactly at (r = 0 ∧
s0 + wSide = n ∧ root species), root species holds by `History.root_iff`, and the
clause supplies `sideDigit (wSide/e) = 1` on that exact trigger. The SHAPEWF LEG
via SW1b — each ShapeWF clause named to its `HistLawful` source per §2.2's roster:
species_iff ← `History.root_iff`; monic ← readOf-definitional; dchain0 ← root
...
keying CONSTANT — the V10/Q1 steep-leg inconsistency (whose content is exactly
those constants) cannot make this unit vacuous; no unit of this corpus premises
`HistoryCoherent` (sweep carried; `WallAReconciled` references it inside the BUILT
`HC2.ReadsOf` as spec-only reconciliation target, §2.9 fence note). InBox is
subsumed: `HistLawful`'s root-box clause carries the L3 bound. REGRESSION GATE:
`toy_v8_monic` (T-G1) pins §0 record #9's counter-instance — with the honest
scope Fable-4 recorded: gates catch over-strengthening and roster REMOVAL, not
under-sourcing; the roster re-audit above is the channel that catches the latter,
which is why it now covers BOTH conjuncts.
hypothesis_fields: `hlaw : HistLawful` (owner HC-2, discharge fenced-Q1 — §0).

**T-V9 `MovesT.irr_saturation` · `V9_irrSat.lean` · medium — REV 4 (Codex-3 #1's
counterpart): with `IrrHalts` now the note's saturation EQUATION (§2.2), this unit
is the note's own parenthetical, stated ON `IrrHalts` — the equivalence with μ = 1
under lawfulness, no longer a detached arithmetic identity.**
```lean
...
`HistLawful` telescopes it to accE·accF (`acc_childWidth`); substituting into
`IrrHalts`'s equation accE·accF = childWidth·μ collapses it to
childWidth = childWidth·μ ⟺ μ = 1 (childWidth ≥ 1 under the fragment: root
Dwidth = 1, e·g ≥ 1, the chain). Together with T-V3's species leg this displays
BOTH equalities of the note line with no contentless conjunct (the rev-2 T-V3
defect). deps: Defs §2.2. sketch: list telescope on the Dwidth chain; Nat
arithmetic for the iff. hypothesis_fields: `hlaw` (§0).
...
-- REV 4 (Fable-3 GAP-4): the unused `{P : Shape n}` binder is dropped.
...
-- REV 4 (Codex-3 #5): the BOX SIZE m' is quantified — TREE-N's level-N' cylinders
-- live in Box p (n·N'), so one fixed m was not the family the consumer prices;
-- `capIrr_uniform` already quantified {N N' m m'} across jets, so the cap VALUE is
-- family-uniform and the strengthening is free. The reference-jet choice J₀ is
-- IMMATERIAL by the same theorem (any two jets give the equal cap) — Codex-3 #5's
-- "supplied reference-jet choice" is answered by `capIrr_uniform`, not by a new
-- definition (the realized-halting-class function it asked for IS this constant).
...
**T-C4 `MovesT.ns_ladder` · `C4_nsLadder.lean` · medium — REV 4 (Fable-3 CRIT-2's
tower form): the ∀-M ladder input is RETIRED with the uncapped ladder (§0 record
#8); the squeeze is restated over an arbitrary index with a diverging lower bound
— exactly the shape T-C7 consumes (index = the tower's levels, lower bound
Mlev − M₀, whose divergence IS T-C7's premise — REV 5, Codex-4 #2: the gap-form
`hMgrow`, never an implication from `Mlev → ∞` alone).**
```lean
theorem ns_ladder {ι : Type*} [Preorder ι] (Zcard lower : ι → ℕ)
    ...
    Filter.Tendsto (fun k => (p : ℝ) ^ (-(Zcard k : ℤ))) Filter.atTop (nhds 0)
```
moves_ref: 7227–7229 ("|Z_M| → ∞" — along the tower at finite level). deps:
Mathlib only. sketch: p ≥ 2, zpow-antitone squeeze below p^{−lower}.
hypothesis_fields: none (`hlad`/`hgrow` discharged by T-C5 + `hMgrow` at
consumption in T-C7).

**T-C5 `MovesT.nsZ_free` · `C5_nsFree.lean` · hard [pre-approved split C5a/C5b] —
REV 3 (Codex-2 #6: the (ZC) setting is the explicit displayed premise `hzc :
ZCPack J`, §2.5a). REV 4 (Fable-3 CRIT-2): the LEVEL CAP joins the premise row —
without it the conclusion was refutable by cardinality (pinCount ≤ m at any
`Locus p m`, but the ∀-M form demanded M − M₀ pins at every M); the (ZC) grid the
capped `slot0_grid` supplies runs exactly to `Mlev J N`.**
```lean
...
Scale map is unbounded along the tower — C.1.5, owner HC-2's instance; REV 5,
Codex-4 #2: stated in GAP form `Mlev − M₀ → ∞` — rev 4's `Mlev → ∞` did NOT
imply the sketch's diverging lower bound, counter-assignment
`M₀ (Jat N) = Mlev (Jat N) = N`; at ScaleFaithful jets with H fixed, M₀ is the
H-determined constant, so the gap form is exactly Scale unboundedness — recorded,
not assumed); (d) the
...
part of rev 2 was honest and stays). REV 4 (Codex-3 #6): the (ZC) admissibility
premise `hzc` joins the row — T-C5's construction REQUIRES `ZCPack (Jat N' h)` and
rev 3 consumed it silently ("`NsLumpFamily.vanish`/`slot0` alone don't imply the
equations are free on the state or priced by Theorem C(b)" — correct); `Mlev` is
now §2.5a's (hoisted, unchanged), and the capped ladders are consumed at
M = Mlev — exactly the cap, so the rev-3 consumption survives verbatim (Fable-3's
CRIT-2 re-check).**
```lean
-- `Mlev` — see §2.5a (HOISTED at REV 4; definition unchanged).
...
theorem ns_null {H : History p F} (N₀ : ℕ)
    ...
    (htw : JetTower Jat)
    (hzc : ∀ N' h, ZCPack (Jat N' h))     -- REV 4 (Codex-3 #6): T-C5's own premise
    (hM₀ : ∀ N' h, M₀ (Jat N' h) ≤ Mlev (Jat N' h) N')
    (hMgrow : Filter.Tendsto                       -- REV 5 (Codex-4 #2): GAP form
      (fun N' : {k // N₀ ≤ k} => Mlev (Jat N'.1 N'.2) N'.1 - M₀ (Jat N'.1 N'.2))
      Filter.atTop Filter.atTop)
...
T-C5 AT M = Mlev (its `hMcap` is `le_refl` there; `hzc` supplies the (ZC) setting
— REV 4) constructs the admissible Z; T-C6 gives ratio = p^{−|Z_{M(N')}|}
(denominator > 0 by `hne`); the tower-form T-C4 with lower := Mlev − M₀, whose
divergence IS `hMgrow` verbatim (REV 5 — no implication step remains where rev 4
had one), closes the squeeze (`htw` aligns the per-level systems so the counts
compare). hypothesis_fields: the jet family `Jat` + `htw`/`hzc`/`hMgrow`/`hne`
(HC-2's PRESENT-EXIST faces; `hzc` = the (ZC) standing perimeter 7232–7233) + the
...
theorem sibCount_pair_form (T : TreeModel p F n N m pol)
    ...
    (trackOf : Node p F → Polynomial (ZMod p))       -- REV 5 (Codex-4 #10)
    (hred : RedCellPartition T CA χ trackOf)   -- REV 4 (Codex-3 #7): at the F1 `.red`
...
theorem sib_falsifier : ∃ (T : TreeModel 2 (ZMod 2) 2 1 2 polTriv)
    (CA : CellData 2 (ZMod 2) 2 1 2 polTriv T), ¬ SibCount T CA toyχ
```
...
REV 4 (Codex-3 #10): `RootSplitData` is no longer handed in — `rootSplit_exists`
CONSTRUCTS it from the tree's own structure (displayed below); only the shared
...
structure RootSplitData (Tr : VTree p F) (g : Fin n → ZMod p) where
  ...
  hcov : ∀ i, ∃ h : ChildRoot none (headOf i),   -- each head IS a chain of Tr
    oneNode (headOf i) h ∈ Tr.chains             -- (REV 8: `oneNode` CLOSED,
                                                  --  §2.11 — root-species
                                                  --  datum explicit)
-- τ-hen tracks carry NO chain (7069):
...
/-- REV 4 (Codex-3 #10: "no theorem/hypothesis states that every `hreal` actually
supplies these values" — the handed-in root data are now CONSTRUCTED): the head
partition, roster, injectivity, nonemptiness, and coverage all come from the tree's
OWN structure — group `Tr.chains` by head (finite by `hfin`; heads are chains by
`hclosed`; distinct groups have distinct heads by construction). The ONE datum that
stays a hypothesis is `hg` (all fiber members share the root cell g): it is model
semantics (the tree's tracks pinning the reduction), not tree combinatorics —
owner-tagged, consumed as T-E5's `hfib`/the scaffold's `hg`. -/
...
theorem fiber_root_split (Tr : VTree p F) (T : TreeModel p F n N m pol)
    (χ : Fin n → Fin m) (trackOf : Node p F → Polynomial (ZMod p))
    (CA : CellAssign p F n N m pol T χ trackOf)      -- REV 8 (Codex-7 #1): the
      -- FULL W4-1 interface — `CA.child_cover` rides in; the site vocabulary
      -- below reads `CA.toCellData`
    (hχ : Function.Injective χ) (hrc : RootCellsOf T CA.toCellData χ)
    (hred : RedCellPartition T CA.toCellData χ trackOf) -- REV 6 (Fable-5
      -- CRIT-1's re-route: step (2) below)
...
The conclusion is the CONJUNCTION: (A) the SIB product at the cell event; (B)
...
splits at .red entrances). REV 5 (Codex-4 #3: rev 4's `hdecomp` equated the
WHOLE-TREE fiber with the site's LOCAL events — at a split nested inside one root
branch, the LHS carried the OTHER root siblings' conditions while the RHS could
not, so "a canonical nested tree generally cannot supply this field", making
`TreeScaffold` vacuous on genuinely nested sibling trees): the frame now prices
its OWN `scope` — the subtree event the enclosing recursion frame hands down (the
note's recursion is ON SUBTREES: "μ(fiber T │ Σ)" at a nested step reads T = the
CURRENT subtree, Σ = ITS root state, 7503–7508) — and `hdecomp` is restated over
...
structure SplitSiteData (Tr : VTree p F) (T : TreeModel p F n N m pol)
  ...
  -- the first-split identification AT THIS FRAME'S OWN SCOPE (REV 5): both sides
  -- are local to the site — the rev-4 whole-fiber LHS carried outside-sibling
  -- conditions the RHS could not, Codex-4 #3's nested counter-model; a FIELD here
  -- so E7 states one recursion frame; E8 receives the per-site family through its
  -- SCAFFOLD hypothesis (REV 3, Codex-2 #13 — no longer "proved in E8" by promise)
...
theorem fiber_first_split (Tr : VTree p F) (T : TreeModel p F n N m pol)
    ...
    -- μ(subtree│Σ) = jvol(c│Σ)·∏ μ(S_j│Σ_c), division-free:
...
structure TreeScaffold (Tr : VTree p F) (T : TreeModel p F n N m pol)
  ...
  hframe : ∀ H hH h2, (splitFrame H hH h2).es = L.parentSt H ∧
    ...
    -- REV 4 (Codex-3 #9): the EXPONENT TIE — T-E7 prices with the frame's
    -- `jointExp`, TREE-EXP's conclusion sums the ledger's `siteExp`; without this
    -- conjunct "the induction sketch cannot produce T-E8's displayed exponent as
    -- stated" (correct). One-side: both are the system's mstar; multi-side: the
  ...
  hscope : ∀ H hH h2, (splitFrame H hH h2).scope
    ...
    -- frame's `subtree_part` value S ν and the child frame's scope coincide by
    -- construction (both read Tr.chains below the same entrance with the tree's
    -- own specs), so the recursion telescopes without a separate tie field.
  subtree_part : ∀ H hH h2, ∀ ν ∈ CA.branchSetOf (L.cellAt H),
    ...
    -- the child-subtree partition, REV 4 (Codex-3 #11's witness face): the FIXED
    -- ContFiber witness is identified — the chain subset Cν, the child root hν,
    -- and the TREE'S OWN `leafV`/`nsLeaf` as the specs (rev 3's inner ∃ left
    ...
    -- instance. Two-verdict note: the note proves the identification INSIDE
    -- TREE-EXP for the canonical process; this corpus prices ABSTRACT models and
    -- receives the canonical facts as typed owner data — the campaign's standing
    -- genre (same as `presents`/`state_cell`), not a fresh evasion.

theorem treeExp (Tr : VTree p F) (T : TreeModel p F n N m pol)
    (CA : CellData p F n N m pol T) (χ : Fin n → Fin m)
    -- (REV 8: the unit prices over the DATA layer; its ∀-closure `TreeExpFin`
    --  (§2.10) carries the FULL W4-1 interface — Codex-7 #1's consumer)
...
and each frame prices its OWN `scope` per `sc.hscope` — REV 5: the whole-tree
fiber enters ONLY at the root step, through E5's decomposition; nested frames
never mention it, which is what kills Codex-4 #3's outside-sibling counter-model).
...
every component — funext/propext + structure eta) + T-E9 + `Realizes`: distinct
class members have disjoint NONEMPTY fibers in the finite box, so the class
injects into the box's subsets; (b) `hA` is DELETED — the note licenses constancy
"GIVEN its side-product form", i.e. THROUGH the dictionary, not as a bare
assumed equation: the premise is now the NAMED dictionary law `hdict` (per-site
exponents read off the SHAPE — `shapeExp`, the L3 dictionary's per-read
fresh-exponent, E-phase-bound to the same shape-side apparatus as
`thr`/`Nshape`), and the constancy `AofTr Tr (Lat Tr h) = AofTr Tr₀ L₀` is the
DERIVED lemma `AofTr_shape_const` (ShapeEquiv's shape-matching chain bijection
transports the per-site exponents term by term); (c) the T-E8 application is
fully supplied — `trackOf`/`hred` join the premise row (rev 4's sketch applied
T-E8 while supplying no `hred`); and the `hjcm` prose placeholder is TYPED
(Codex-4 gap #12).**
```lean
...
theorem vtree_ext_eq (Tr Tr' : VTree p F) (h : VTree.ext Tr Tr') : Tr = Tr'
-- REV 5 (Codex-4 #4's hfin leg, the enabling rigidity lemma): `hleaf` forces
-- `leafV = none` off `chains` and `hns_leaf` forces `¬ nsLeaf` there, so the ext
-- tuple (chains, henV, on-chain leafV/nsLeaf) determines EVERY component;
-- funext + propext + proof irrelevance + structure eta. (T-E9's "up to ext" is
-- thereby literal equality — recorded; no other unit's statement changes.)

theorem shapeClass_finite (Tr₀ : VTree p F) (T : TreeModel p F n N m pol)
    (χ : Fin n → Fin m) : (shapeClass Tr₀ T χ).Finite
-- REV 5: DERIVED, never assumed — each member's fiber is NONEMPTY (`Realizes` is
-- a class clause); distinct members have DISJOINT fibers (a shared member gives
-- `ext` by T-E9, hence equality by `vtree_ext_eq`); so Tr ↦ its fiber set injects
-- the class into the finite box's subsets.
...
theorem perShape_law (Tr₀ : VTree p F) (T : TreeModel p F n N m pol)
    (CA : CellData p F n N m pol T) (χ : Fin n → Fin m)
    (hχ : Function.Injective χ) (hrc : RootCellsOf T CA χ)
    (trackOf : Node p F → Polynomial (ZMod p))       -- REV 5 (Codex-4 #4's
    (hred : RedCellPartition T CA χ trackOf)          -- missing-hred leg)
    (hsib : SibCount T CA χ)
    (hns₀ : NsFree Tr₀) (hreal₀ : Realizes T χ Tr₀)  -- REV 4 (Codex-3 #12): the
      -- representative is IN its own class (`ShapeEquiv` refl) — the empty-class
...
representative's `AofTr`, constant across the class BY `AofTr_shape_const` from
the dictionary law — the note's own conditionality ("shape-determined GIVEN its
side-product form") now enters through the NAMED `hdict` premise (owner: the
shape-side theory through T-V8's dictionary), never as an assumed bare constancy
of the conclusion's exponent. deps: T-E8, T-E9, T-V8, MovesD (shape exponents).
sketch: every summand tree is realizable + ns-free, so T-E8 applies (hχ/hrc/
trackOf/hred/hsib/scat/hjcm — the FULL premise row, REV 5) with the SAME exponent
by `AofTr_shape_const`; sum the constant (D10's idiom); disjointness never needed
for the SUM but IS the no-multiplicity-weights content — cite T-E9 in the
...
theorem treeN (Tr : VTree p F) (T : TreeModel p F n N (n * N) pol) -- m := n·N level
    ...
    -- (REV 8: the unit prices over the DATA layer; its ∀-closure `TreeNStmt`
    --  (§2.10) carries the FULL W4-1 interface — Codex-7 #1's consumer)
    (hχ : Function.Injective χ) (hrc : RootCellsOf T CA χ)
    (trackOf : Node p F → Polynomial (ZMod p))       -- REV 5 (Codex-4 #10)
    (hred : RedCellPartition T CA χ trackOf)          -- REV 4 (Codex-3 #7)
...
theorem treeN_ns (Tr : VTree p F) (H : History p F)
    ...
    (Lat : ∀ N' h, NsLumpFamily (Tat N' h) (Jat N' h))
    (htw : JetTower Jat)
    (hzc : ∀ N' h, ZCPack (Jat N' h))     -- REV 4 (Codex-3 #6, threaded from T-C7)
    (hM₀ : ∀ N' h, M₀ (Jat N' h) ≤ Mlev (Jat N' h) N')
    (hMgrow : Filter.Tendsto                -- REV 5 (Codex-4 #2): GAP form, as T-C7
      (fun N' : {k // N₀ ≤ k} => Mlev (Jat N'.1 N'.2) N'.1 - M₀ (Jat N'.1 N'.2))
      Filter.atTop Filter.atTop)
    (hne : ∀ N' h, (stateTruncAt Jat N').Nonempty)
...
theorem preHalt_dict_sum {P : Shape n} (S : Presented p F n N m pol P)
    ...
    (heta : EtaLoc T P) (hib : InBoxOfMem T n)
    (hph : PreHaltClassData (P := P) pol)
    (𝒯 : Finset (VTree p F))                 -- REV 4 (Fable-3 GAP-4): 𝒯 binds
    (hsep : ∀ Tr ∈ 𝒯, (T-F2's per-tree hsep row at Tr))   -- BEFORE hsep reads it
...
the note's EXACT below-halt decomposition, not an inequality. REV 8 (Codex-7 #5
= Fable-7 CRIT-2, converged): the `hne` premise joins — the SAME premise the
built D11a `mult_sum` took at the ∅-dispatch (`D11a_multSum.lean` 38, its
header: "mult_sum carries hne (reads ≠ []) since D4″ 2026-07-29"). The rev-7
"E-phase check 1 = 1 + 0" expectation was WRONG against the LANDED semantics:
at reads = [], the built `mult x = 1` (`Presented.mult_empty_shape`,
MovesD/Defs.lean 537) while `PrefIdx` is EMPTY (the History type has no empty
chain — `Shape.NP`'s own docstring records it), so the two-count decomposition
computes 1 = 0 + 0 — FALSE at every empty-shape P. The ∅-shape truth is the
displayed COMPANION below (the note-side 1 = 1 lives at the option/designated
level, where the single root class is counted by the OBJECT, not by
`PrefIdx`): the built uniform form is `mult_sum_opt` (`D11a_multSum.lean` 68 —
Σ_x mult x through `emptyFiber` at the ∅-shape, the additive option-level
incidence form; cited BY NAME, already landed).**
```lean
theorem belowHalt_decomp {P : Shape n} (S : Presented p F n N m pol P)
    (T : TreeModel p F n N m pol) (χ : Fin n → Fin m) (x : Box p m)
    (hne : (P : ShapePrefix).reads ≠ []) :          -- REV 8: D11a's own premise
    S.mult x
...
theorem belowHalt_empty_shape {P : Shape n} (S : Presented p F n N m pol P)
    ...
    S.mult x = 1 ∧ IsEmpty (PrefIdx n pol P)
-- the ∅-shape display (REV 8): `mult x = 1` is the built
-- `Presented.mult_empty_shape` VERBATIM; the `PrefIdx` emptiness is the
-- reason the two-count decomposition CANNOT be stated there (its RHS is
-- 0 + 0) — the root's mark is carried by the designated object (L5/L12's
-- "Pref(∅) = {∅}", priced inside `mult`'s own dispatch), and the uniform
-- all-shapes incidence form is MovesD's `mult_sum_opt`, not this unit's.
```
+ the ≤ corollary (rev 1's form) kept as `belowHalt_excess`, same `hne`.
moves_ref: 7606–7611 "The unrestricted total … is ≥ this, WITH EXCESS EXACTLY
THE BELOW-HALT MARKS — §D4-R bookkeeping that NO consumer may convert to a
per-f quantity without first restricting to Pref^τ. Conversion then runs
through TREE-EXP's fiber expansion ONLY." deps: Defs §2.8, MovesD
(`Presented.mult` with the LANDED ∅-dispatch, `mult_of_ne`/`mult_empty_shape`
— MovesD/Defs.lean 516/527/537; `eventO`/`multO` deleted). sketch: rewrite by
`mult_of_ne hne`; classical partition of the finite index subtype by
`PreHalt (reprOf i)`; the companion is `mult_empty_shape` + the built
∅-`PrefIdx` emptiness. THE FENCE rides in the docstring: the conversion ban
(the pass-8 critical-1 channel, closed by display). hypothesis_fields: none
beyond `hne` (a shape-side fact, decidable per instance).
...
theorems hold over ANY CellData — the definitional leg of W4-1; the CANONICAL
...
-- input, for EVERY model and EVERY CellData. (The contract's "realizable input"
...
theorem eligible_nonvacuity_toy : ∃ (T : TreeModel …toy…) (CA : CellData …)
    (o ν x), eligibleT T CA o ν x ∧ RejectsRootOnlyChildless T CA
theorem constFalse_childCover_false :          -- NEW at REV 7 (ruling 1): the
    ∀ (T : TreeModel p F n N m pol)            -- W4-1-LAYER falsification, in-
      (CA : CellData p F n N m pol T)          -- corpus, fence-free
      (χ : Fin n → Fin m) (trackOf : Node p F → Polynomial (ZMod p)),
    (∀ ν x, ¬ T.child none ν x) →
    (∃ (g : Fin n → ZMod p) (x : Box p m), x ∈ rootCell χ g ∧
      ∃ ψ, ψ ∈ UniqueFactorizationMonoid.normalizedFactors (redPoly χ x) ∧
        2 ≤ Multiset.count ψ
          (UniqueFactorizationMonoid.normalizedFactors (redPoly χ x))) →
    ¬ ChildCover T CA χ trackOf
theorem constFalse_no_cellAssign :             -- NEW at REV 8 (Codex-7 #1):
    ∀ (T : TreeModel p F n N m pol)            -- the INSTALLED form — the
      (χ : Fin n → Fin m) (trackOf : Node p F → Polynomial (ZMod p)),
    (∀ ν x, ¬ T.child none ν x) →
    (∃ (g : Fin n → ZMod p) (x : Box p m), x ∈ rootCell χ g ∧
      ∃ ψ, ψ ∈ UniqueFactorizationMonoid.normalizedFactors (redPoly χ x) ∧
...
-- the constant-false-child model (a fortiori `Cell := Unit` with every
-- branchSetOf empty) FAILS the W4-1 interface's covering clause on every
-- window-opening input — one line from the ∃ in `ChildCover`; the
-- window-opening supply (f̄ = xⁿ, n ≥ 2) is the same input family
-- `rootOnly_childless_rejected` already uses. No `HistoryCoherent`, no
-- `ReadsOf`: the falsification lives entirely at the interface. REV 8: the
-- second theorem is the INSTALLATION check Codex-7 #1 demanded — the model
-- forbidden by the contract text admits NO instance of the W4-1 interface
-- AT ALL (`child_cover` is a mandatory field and `constFalse_childCover_-
-- false` refutes every candidate value); no consumer wiring can drop it.
```
...
claims only: not rejectable by fiberAt ALONE. REV-4 LEMMA NOTE (fiberAt (v) from
(vi)): `rootOnly_childless_rejected` may equally run through clause (vi) — chains
= ∅ makes `heads` = ∅, so (vi) forces NO repeated factors, contradicting `hrep`
directly; and (iii)+(vi) imply (v) (simple-factor degrees + repeated-factor
totals sum to n = deg f̄) — the one-line lemma `typemult_deg_of_tracks`, recorded
here so clause (v)'s derived status is checkable.
**W4-1 NON-VACUITY ADJUDICATION (REV 4, Codex-3 #14 — the charge: "apply the
note-supported non-vacuity law if one exists, else record the declared
boundary").** The finding is CORRECT that no fiberAt theorem falsifies the W4-1
SEMANTIC LAYER (`eligible`/`child_iff`) on a childless model — `eligibleT_iff_child`
is definitionally satisfied there. The note-supported law AT THAT LAYER is the
contract's own (b)-union text: "L2's ALL-AND-ONLY children ('ALL next-node data
ν = (side s, its digit tuple, ψ, canonical lift) where s is an eligible side … ψ
ranges over the eligible factors')" (MovesD §2.5 W4-1(b), quoted) — i.e. branch
sets must enumerate L2's next-node data, whose corpus face is `WallAReconciled`:
on any input whose decoded run has a realized window read, L1/L2 totality supplies
a `ReadsOf`-realized child, and the childless instance's empty branch set FALSIFIES
the iff. That law is FENCED (spec-only — `ReadsOf` carries `HistoryCoherent`, the
Q1 surface), so the corpus cannot state it as a unit without breaching its own
fence. DECLARED BOUNDARY (D9-style, on record): the W4-1-layer falsification of
...
in-corpus rejection perimeter NARROWED by one more layer (`KBTotTower.root_total`
inside the consumed vp value) while the (b)-demand AT the W4-1 layer stayed
recorded as unmet. **REV-7 OVERTURN (ruling 1 — the FIFTH round; verifiers split,
orchestrator direct read decides):** the boundary declaration is WITHDRAWN. The
note's covering case-analysis (c2) lives AT this layer (7112–7119), and its
m_i ≥ 2 clause is fence-free (L1 totality, D4R.0-K(a) — no `HistoryCoherent`
content): the W4-1 contract type gains the per-track covering clause
`ChildCover` (§2.9), and THIS unit's `constFalse_childCover_false` is the
in-corpus falsification of the forbidden constant-false-child/unit-cell model at
the interface itself. What remains fenced is ONLY the all-and-only ROSTER law
(branch sets = L2's full next-node enumeration — `WallAReconciled`, genuinely
`ReadsOf`-keyed); the EXISTENCE half never needed the fence, which is exactly
what four rounds of hold missed. The §5 W4-1 row carries the five-round
comparison; the contract change is recorded for the MovesD-side W4-SYNC.
...
noncomputable def bandLevels (R : ShapePrefix) (n : ℕ) : Finset (ℕ × ℕ) :=
  -- REV 7 (Codex-6 g2 = Fable-6 GAP-2, the REV-4 import-by-name rule): CLOSED
  -- over the BUILT MovesD band vocabulary — the (level, base) grid below the
  -- built `NPband` where SOME read's built `bandS` fires
...
theorem np_id_pin {P : Shape n}                -- NEW at REV 7: the BUILT pin
    (Nthr : PrefIdx n pol P → ℕ)               -- supplied — W4-4's as-built
    (hNthr : ∀ (i : PrefIdx n pol P) (J : JetSetup (reprOf i) n N m),
      Nthr i = 1 + (transportedLevels J (topLocus p m)).sup Prod.fst) :
    NPIDPin n pol P Nthr
```
[`transportedLevels J Z` := the (level, base) support of J's pinned equations at
Z — MovesT's OWN Finset over MovesC's fresh apparatus (`J.fresh i`, `totalPins`'s
summand supports; it exists in NO built corpus, so it is honestly this corpus's
def); `bandLevels` is now CLOSED over the built `bandS`/`NPband` (above — rev 6's
E-phase bracket RE-BRACKETED built content, Codex-6 g2). `np_id` is W4-4(a)
VERBATIM; `np_id_pin` is the AS-BUILT consumption target: MovesD now ships the
typed `NPIDPin n pol P Nthr` (NPband = NthrSup on the inhabited branch,
NPv_npid.lean 51–53) with `noteNP`/`NthrSup` and the conclusion
`D4R4_all_noteRange (hpin : NPIDPin …)` — the SUM ∧ EV identity on L12's own
range `noteNP ≤ N`. `hNthr` instantiates L12's `N(η_i, ⊤)` = 1 + the largest
transported base level (the C-cutoff's own formula) — jet-free by `np_id`.]
moves_ref: MovesD §2.5 W4-4(a)/(c) "NP-ID becomes a wave-4 theorem from mstar_eq +
fresh_band/fresh_cover + D8/D4b over the MovesT-presented classes" + W4-SYNC
delta 6 (the built types). deps: MovesD.D8_bandShape, MovesD.D4b_chartCount,
MovesD.NPv_npid (`NPIDPin`/`NthrSup`/`noteNP`/`D4R4_all_noteRange` — BY NAME),
MovesC (mstar/fresh apparatus), T-V8. sketch: D7a (⊆): every pinned equation sits
in a fresh band at its read (fresh_band/mstar_eq — per-J theorems, so the ∀-J
form is the natural strength); D7b (⊇): every band level is hit (fresh_cover +
D8); `np_id_pin` from np_id: all classes' transported level sets coincide with
`bandLevels`, so `NthrSup Nthr = 1 + max band level = NPband` (the D8 band-
ceiling identity). REV 7: the stale "landing DELETES D4R4_all's conditionality
pointer" claim is RETIRED — the pointer is already deleted corpus-side; landing
now SUPPLIES `hpin` to the built `D4R4_all_noteRange`. T-C2's NPband corollary
...
**T-D10 `MovesT.classTree_countable` · `D10_classTreeCtbl.lean` · medium — REV 4
(Codex-3 #15 + #21): `ClassTree` is a STRUCTURE carrying the data rev 3's pair
could not: the ns-MARK as a component, key uniqueness, the maximal-address domain
condition, and the ROOT PAYLOAD (henV) as its own component. REV 5 (Codex-4 #8):
`leafV` is a **key-unique `Finset`**, not a `List` — rev 4's association list
admitted PERMUTATION (two Nodup lists differing by swapping two entries yield
identical lookup functions, hence extensionally equal `fibOf` trees on unequal
`ClassTree` values — §0 record #12); a `Finset` of pairs is order-canonical
(`Vd` has DecidableEq — a subtype of `Multiset (ℕ × ℕ)`), so equal leaf DATA now
means equal component, and `fibOf_fiber_disjoint`'s ct ≠ ct' genuinely forces a
readable difference.**
```lean
structure ClassTree (n : ℕ) (pol : CanonPolicy p F) where
  addrs : Finset (Σ P : Shape n, PrefIdx n pol P)
  haddr : PrefixClosed addrs
  leafV : Finset ((Σ P : Shape n, PrefIdx n pol P) × Vd)   -- REV 5: Finset, order-
    -- canonical (Codex-4 #8's association-list permutation dies by carrier choice)
  hkeys : (leafV.image Prod.fst).card = leafV.card ∧       -- key uniqueness
    ∀ av ∈ leafV, av.1 ∈ addrs ∧ MaximalAddr addrs av.1
  nsMark : Finset (Σ P : Shape n, PrefIdx n pol P)     -- the (ns)-marked leaves
  hns : ∀ a ∈ nsMark, ∃ v, (a, v) ∈ leafV              --   — DATA, never inferred
  rootV : Multiset Vd                                   -- the henV payload
  hroot : ∀ v ∈ rootV, ∃ g : ℕ, 1 ≤ g ∧ v.1 = {(1, g)}
-- [`PrefixClosed`/`MaximalAddr` key to MovesD's class-level prefix relation
--  `ClassPrefixOf` — same accessor as §2.10's `PrefixCoherentRepr`.]
...
data (a structure of Finsets/Lists over countable carriers). hypothesis_fields:
none.
...
-- ct ≠ ct' forces some DATA component to differ, and each component is read off
-- `fibOf` injectively (addresses → representative chains via `hrep`'s coherence
-- + `hwd`; leaf entries → leafV values at the representative; marks → nsLeaf;
...
-- bridges are EXACTLY `ClassCover`'s clauses. Sketch: take x's tree Tr (T-F1);
-- `hcov` classifies each chain (realized at x through fiberAt (i)) — addrs :=
...
def rs1GivenOfMovesT (T : TreeModel p F n N m pol)
    (χ : Fin n → Fin m) (trackOf : Node p F → Polynomial (ZMod p))
    (CA : CellAssign p F n N m pol T χ trackOf)     -- REV 8 (Codex-7 #1):
      -- the FULL W4-1 interface at the fixed level AND per level below —
      -- `child_cover` rides into the package values
    {N₀ : ℕ} (Tat : ∀ N', N₀ ≤ N' → TreeModel p F n N' (n * N') pol)
    (χat : ∀ N', Fin n → Fin (n * N'))
    (CAat : ∀ N' h,
      CellAssign p F n N' (n * N') pol (Tat N' h) (χat N') trackOf)
    -- REV 5 (Codex-4 #1): the vp leg is TOWER-KEYED (VPPinned, §2.10) — the
    -- fixed-level (T, CA, χ) feeds the per-level fields; the presented tower
    -- (Tat, CAat, χat — the SAME family vocabulary as TreeExpNs/NsAmendedPair)
    -- feeds vp; trackOf = the shared L1 frame accessor (§2.7/§2.9)
    (rel2_a rel2_b rel2_c rel2_d rel2_e : Prop) : RS1GivenPackage := {
  tree_exp_fin := TreeExpFin T χ trackOf CA, -- §2.10, displayed (FULL interface)
  ...
  sib := SibCount T CA.toCellData χ,          -- §2.6, displayed
  tb_cap := TBCapPinned pol m ∧ NsAmendedPair pol,   -- §2.10, both displayed
  vp := VPPinned pol Tat χat trackOf CAat,  -- §2.10, five named legs (REV 8:
                                             --   + om_total), tower-keyed
  pci := PCI T CA.toCellData χ,               -- §2.6, displayed
...
def xrbOfMovesT (T : TreeModel p F n N m pol)
    ...
    (CAat : ∀ N' h,
      CellAssign p F n N' (n * N') pol (Tat N' h) (χat N') trackOf)
    -- REV 7 (Codex-6 #5, the round-5/6 BDY hold REJECTED): the `(ns_bridge :
    -- Prop)` binder is DELETED — the bridge slot's value is the TYPED seam
    -- `NsBridgeStmt pol` (§2.10), the note's μ-sentence AS the type; the owner
    -- (MovesS wave 5 / [2b]) now discharges the STATEMENT, not an arbitrary
    -- Prop of its choosing; `ns_bridge := True` is no longer expressible
  ...
  jc_multi := (∀ (Tr : VTree p F) (L : SiteLedger Tr T CA.toCellData χ),
    ∀ H ∈ multiSites Tr T CA.toCellData χ L, ∀ h2 : 2 ≤ L.sides H,
...
def wsh17OfMovesT (T : TreeModel p F n N m pol)
    (CA : CellData p F n N m pol T) (χ : Fin n → Fin m)   -- REV 4 (Fable-3
    (c15 cts : Prop) : Wsh17Package :=                       -- GAP-4): χ bound
...
re-keying must carry the S.2 GIVEN list AND the nine-input ledger. REV 4 (Codex-3
#18's pattern, applied here too): the ledger slot is the REAL IMPORTED
`MovesS.LedgerIV` — no Prop parameter remains where the value exists. -/
def w1mObligation {n' : ℕ} (T' : MovesS.TableShape n') (M : MovesS.MeasuredSide T')
    (Π' : RS1GivenPackage) (concl : Prop) : Prop :=
  RS1GivenHolds Π' → MovesS.LedgerIV T' M → concl
```
...
`isEntrance`), quoted as the S-6 residual. REV 4, both Codex-3 fixes: (#16) the
telescope's conditioning states can no longer skip or duplicate — `hparent`'s
`.st` leg is the IMMEDIATE parent (§2.5), so T-E6 iterates step-by-step along the
chain (rev 3's any-earlier-prefix ledgers falsified the displayed telescope);
(#17) the (BDY) index convention fixed at `readLedgerOf` — L = the last read
index, not the count (the phantom terminal read is dead).**
```lean
...
theorem chainReads_ne (Tr : VTree p F) (H : History p F) (hH : H ∈ Tr.chains) :
    (chainReads Tr H) ≠ []
-- REV 4: H's own read is a member (r = length − 1 gives the full prefix H itself),
-- so the last-read index below is well-defined on chains.
...
REV 4 (Codex-3 #17 — the (BDY) INDEX CONVENTION): the contract's inclusive
`Finset.Icc 0 L` treats **L as the LAST READ INDEX, not the read count** — rev 3
set L := length, creating a phantom terminal read of charge 0 and admitting
fe = length; now L := length − 1 (valid on chains by `chainReads_ne`), the
position range is exactly 0…L, and `hfe : fe ≤ L` excludes the phantom. -/
...
**T-D15 `MovesT.w3_rekey` · `D15_w3Rekey.lean` · easy — REV 4 (Codex-3 #18, the
central round-3 adjudication COMPLETED): the ledger premise is the REAL imported
`MovesS.LedgerIV T' M` — rev 3's `ledger_iv : Prop` was the exact "slot name for a
future value" pattern, and the ruling ("PARTIALLY JUSTIFIED — `hxrbStmt` right,
`w3Rekeyed` incomplete") is accepted in full. TWO-VERDICT RECORD: Fable-3's S-3
verification (both contract preamble sentences verbatim; `hxrbStmt` = the real
`RS1Bundle.xrb` row; the conclusion = the real `recursion_meas` row incl. the
`evalRe` callback shape and `activeState` guard; ℝ-valued βmeas) is COMPUTED
evidence and stands; Codex-3's residue was precisely the one slot Fable-3 did not
type-check — both audits are honored by importing the value.**
```lean
...
theorem w3_rekey_typed {n' : ℕ} (T' : MovesS.TableShape n')
    ...
    (hdc : MovesS.DegCons T') (Π' : RS1GivenPackage)
    (B₀ : MovesS.RS1Meas T' M) :
    w3Rekeyed RB hdc (RS1GivenHolds Π') B₀
      = (RS1GivenHolds Π' → MovesS.LedgerIV T' M → hxrbStmt T' M B₀ →
          ∀ e (he : e ∈ Finset.Icc 1 n') (τ : T'.State e) (σ' : Multiset T'.VType)
            (q₀ : ℚ) (hq : q₀ ∈ M.Pools), M.activeState q₀ e τ →
            ∀ h_ent, B₀.βmeas e he h_ent τ σ' q₀
...
G1a surrogate-decide / G1b classification bridge] — REV 5 (Codex-4 gap #13: the
rev-4 `toyModel`/`toyCA`/`toy_e5_instance` ellipses are REPLACED by closed-form
defining equations and a full statement; Fable-4 GAP-1: rev 4's ONE pinned toy
could not host BOTH the one-side site `toy_state_cell` needs AND "the toy's one
multi-side site" `toy_jcmulti_site` names — there are now TWO PINNED CARRIERS,
and every gate is named to its carrier).**

**CARRIER A (one-side — hosts records 1/3/4/6's gates).** p = 2, n = 2, N = 3,
m = 9. Reduction f̄ = x² at toyG := (0,0) (one repeated factor — one window
track, t = 1). ONE head read (one-side window, fresh digits {2,3,4,5} — mstar
4), then a depth-1 split cell with TWO branches ON THE SAME SIDE (the note's F1
z−1/z−2 shape — "every SIB-gate cell … is single-side", 7471–7472), each
halting τ-irr at its read, which reads NO fresh digit (the joint cell already
pins the whole line tuple — "one side ⟹ any branch selection pins ALL on-line
...
2³·(2⁷)¹·2² = 2¹² = 2⁹·2³). REV-7 RE-RUN (Fable-6 CRIT-1/CRIT-2, the writer's
own recomputation displayed): `child_cell_red` at the OLD winC guard — carrier
A's winC event was the 2⁵ = 32-member {x2…x5 = 0} stratum, of which only the
2³ = 8-member {x0…x5 = 0} stratum carries the child: 32 − 8 = 24 violating
members (witness g = (1,0), x = (1,0,0,…) among them); NEW guard: winC event =
the 8-member stratum EXACTLY — 0 violations. Carrier B: old winC = {x2 = 0} =
2⁸ = 256 members vs child's 2⁶ = 64: 256 − 64 = 192 violations; new winC =
{x0 = x1 = x2 = 0} = 64 — 0 violations. `cellEventE` at the true entrance
UNCHANGED on both (A: {x0 = x1 = 0} ∩ old-winC = 2⁷⁻⁴ = 8 = new-winC; B:
{x0 = x1 = 0} ∩ {x2 = 0} = 2⁶ = 64 = new-winC). `cell_local` at cellLevel 4:
levelIdx c < 4 ⟺ c < 6 ⊇ every read coordinate (A reads x0…x5, B reads
x0…x4) — the rev-6 witness pair (0⃗, 0⃗-except-x4) now differs INSIDE the
premise, refutation dead. Census re-verified: 8 = 2^{9−6} ✓, toyA = 2 +
(4+0+0) = 6 ✓, 2³·2⁷·2² = 2¹² = 2⁹·2³ ✓, carrier-B JC-multi 2⁴·2² = 2⁶ ✓ —
neither fix touches any counted event. REV-8 RE-RUN (Codex-7 #4's four
re-pins — species/γ/line/anchored-residual — plus the CellData re-fence):
every counted event is a DIGIT STRATUM (toyMem/toyChild/toyCell read only
x0…x5 and node identity); species, γ, line, ψ, pat, Ranch, center enter NO
guard and NO counted set, so every decide surface and every census integer is
UNCHANGED — 8 = 2^{9−6}, toyA = 6, 2¹² = 2⁹·2³, 2⁴·2² = 2⁶, violation counts
0/0, cellEventE 8/64 — re-verified at the new tables. TABLE-CONSISTENCY NOTE (REV 5, recorded — found while replacing the
...
`SiteLedger.state_cell` (their state events would be overlapping non-fibers of
the one `cellOf (.st tA1)` partition) AND the census exponent (each sibling's
`presents` would force the shared window's full mstar onto BOTH chains,
double-counting in `AofTr`: 2+2+2+2 = 8 ≠ 6). The REV-5 tables use the
consistent form above (window digits on the head read; leaves mstar 0), which
keeps every sealed integer. E-PHASE WATCH ITEM, flagged for T-E8: if the
CANONICAL instances need per-side/per-branch sub-system ledgers at split
children (the F1 14-pin shape suggests deeper per-branch windows), the per-chain
`AofTr` accounting at SHARED split cells and `hframe`'s `jointExp = siteExp` tie
are the clauses to re-derive — recorded here so the tension surfaces at the
gate, not in a mass proof.
...
```lean
-- CLOSED-FORM TABLES (REV 5 — no `…` in any defining equation; REV 7, Codex-6
-- #6: EVERY Node DATA field is now PINNED — the rev-6 "remaining Node fields
-- are E-phase-bound junk consistent with HistLawful" clause is DELETED: junk in
-- DATA positions could feed NodeExtends/HistLawful/shapes/tracks/ledgers):
--   carrier A: tA1 := ⟨[tAν₁]⟩, tA2a := tA1.snoc tAνa, tA2b := tA1.snoc tAνb
--   carrier B: tB1 := ⟨[tBν₁]⟩, tB2c := tB1.snoc tBνc, tB2d := tB1.snoc tBνd
--   CORE COLUMNS (e, g, μ, Dwidth, sides, freshDigits) — REV 6 (Codex-5 gap
--     #11): Dwidth = 1 throughout (childWidth = e·g·Dwidth = 1 everywhere, so
--     `IrrHalts` is table-computable: leaves 1·1 = 1·1 ✓ fire, heads
--     1 ≠ 1·2 ✓ don't):
--     tAν₁ ↦ (1,1,2,1,1,{2,3,4,5});  tAνa/tAνb ↦ (1,1,1,1,1,∅) (leaf reads at
--       the joint cell — mstar 0);
--     tBν₁ ↦ (1,1,2,1,1,{2});  tBνc/tBνd ↦ (1,1,1,1,2,{3})/(1,1,1,1,2,{4})
--   h / line — REV 8 (Codex-7 #4c: rev 7 pinned only the slope; `line` is a
--     `Line` STRUCTURE, intercept + slope — MovesC/Defs 255): heads h = 1,
--     line = ⟨3, 1⟩; leaf a h = 2, line = ⟨4, 2⟩; leaf b h = 3, line = ⟨6, 3⟩
--     (intercept, slope). Slope ties exact at e = STR = Dwidth = 1 (1·1 = 1,
--     2·1 = 2, 3·1 = 3), strict chain 1 < 2, 1 < 3, StrictMono 2 < 3; the
--     INTERCEPTS are the built `hLineU`'s own values — line.at((s0+wSide)·
--     Dwidth) = u*: heads 3 − 1·2 = 1 = u* ✓, leaf a 4 − 2·1 = 2 = u* ✓,
--     leaf b 6 − 3·1 = 3 = u* ✓;
--   species — heads `.root`, leaves `.recentering` — REV 8 (Codex-7 #4a =
--     Fable-7 GAP-2 tail: the built `hspecInc : species = increment →
--     1 < e·g` REFUTES the rev-7 `.increment` pin at the leaves' e·g = 1;
--     `hspecRec` demands e = 1 ∧ g = 1 — exactly the leaves' values ✓).
--     SPECIES-DISPATCH RE-WALK (every consumed law that reads species):
--     History.root_iff — heads root at j = 0, leaves non-root ✓; mem_snoc's
--     hν : species ≠ root ✓; readOf's monicTop guard — leaves r = 1 →
--     decide false regardless ✓; ShapeRead.hspecRec at the copied values —
--     e = g = 1 ✓; Node.hspecRecCenter — ψ = X − C center ∧ zbar = center,
--     satisfied at the REV-8 pins below ✓; HistoryCoherent's chain clause
--     dispatches on the PARENT's species (heads, `.root`) — the recentering
--     leg never fires on the toys ✓; MovesC's kappaH/innerslotH convention
--     (recenterings create no level, childWidth = Dwidth) — leaves
--     childWidth = 1·1·1 = 1 = Dwidth ✓;
--   σ — the level-0 stage of polTriv's run (K = ZMod 2); the DISPLAYED
--     READ-SURFACE OBLIGATION (the one non-literal pin, per the charge): the
--     only σ-datum any consumed obligation reads is `Nat.card ↥σ.K = 2`
--     (readOf's w = max(factorization 2 2, 1) = 1 — the w-chain seed), a G1a
--     decide clause; no consumed statement (fiberAt/IrrHalts/shapeOfH/
--     NodeExtends/ledgers) reads any other σ-component — roster-audited at
--     readOf's own field list;
--   (s, t) — Bézout e·s + h·t = 1 at e = 1: (1, 0) throughout (1·1 + h·0 = 1 ✓
--     for every h);   a — 0 throughout (no recentering offset);
--   (s0, wSide) — heads (0, 2) (the root read spans the monic box, s0 + wSide
--     = n = 2, monicTop TRUE with pat top digit 1 — record 9's honest value);
--     leaves (0, 1) (non-root, monicTop false by species);
--   ustar / gam — REV 8 (Codex-7 #4b = Fable-7 GAP-2: rev 7's "γ = e·u* +
--     j*·h at j* = 0" treated j* as free; the BUILT γ-tie (HistoryCoherent,
--     MovesC ~641–644, = HistLawful §2.2) pins the coefficient to s0+wSide:
--     gam = e·(STR·u*) + (s0+wSide)·h): heads u* = 1, γ = 1·(1·1) + 2·1 = 3;
--     leaf a u* = 2, γ = 1·(1·2) + 1·2 = 4; leaf b u* = 3, γ = 3 + 3 = 6 —
--     the charge's lawful values, ties EXACT. NO gate or census consumes
--     `gam` (re-verified: every counted event is a digit stratum; γ enters
--     only the lawfulness walks) — integers unchanged;
--   zbar = 1, center = 1 (leaves; heads carry the same K-element datum),
--     lift = Polynomial.X, ψ = Polynomial.X - Polynomial.C 1 (= X + 1 over
--     K = ZMod 2) — REV 8 (Codex-7 #4d, the ψ-order/pat facts): the rev-7
--     pins ψ = X, Ranch = 0 were JOINTLY UNSATISFIABLE against the built
--     Node proof roster — `hRanch` forces Ranch = Σ_{k ≤ wSide/e} C(pat k)Xᵏ
--     ≠ 0, `hzbarRoot` forces ψ(zbar) = 0 with zbar ∈ Fˣ = {1} (so ψ = X
--     needed the non-unit root 0), and `hOrd` (ord_ψ Ranch = μ) at ψ = X
--     contradicts `hpat0` (pat 0 ≠ 0 ⟹ ord_X = 0 < μ). LAWFUL RE-PINS:
--     pat — heads (1, 0, 1) (fun k => if k = 1 then 0 else 1), leaves
--     fun _ => 1; Ranch — heads X² + 1 (= (X+1)² over ZMod 2), leaves X + 1
--     — the `hRanch`-determined values. THE ψ-ORDER WALK (displayed, per the
--     charge): heads ord_{X+1}((X+1)²) = 2 = μ ✓ (deg 2 < 3 kills (X+1)³);
--     leaves ord_{X+1}(X+1) = 1 = μ ✓; hpat0 = 1 ≠ 0 ✓, hpatTop (pat 2 = 1
--     heads / pat 1 = 1 leaves) ≠ 0 ✓ — record 9's honest monic-lead value
--     pat(wSide/e) = 1 PRESERVED on the heads (sideDigit 2 = 1); hzbarRoot
--     (X+1)(1) = 0 over ZMod 2 ✓; hspecRecCenter (leaves) X − C 1 ∧
--     zbar = center = 1 ✓; hbez/hbezCanon (s,t) = (1,0): 1+0 = 1, 0 ≤ 0 < 1
--     ✓; hAnchor e·a = s0 − t·γ: 0 = 0 − 0·γ ✓ (t = 0 — γ-independent, and
--     HistLawful's bezT 1 h = 0 likewise); hψmonic/hψdeg (deg 1 = g)/hψirr
--     (degree 1) ✓; hEdvd 1 ∣ wSide ✓; hDwidth = deg Φ₀ = 1 ✓; hLineU above
--     ✓ — the FULL built proof-field roster (he/hh/hcop/hbez/hbezCanon/hg/
--     hμ/hEdvd/hDwidth/hψmonic/hψdeg/hψirr/hRanch/hpat0/hpatTop/hAnchor/
--     hLineU/hOrd/hzbarRoot/hspecInc/hspecRec/hspecRecCenter) checks at the
--     pinned values — the rev-7 "only proof fields (he/hh/hcop)" claim is
--     RETIRED as wrong against the built Node (MovesC/Defs 398–459);
--   trackOf (toy binding) — the CONSTANT fun _ => Polynomial.X : the toys'
--     one live track (f̄ = x², X repeated). REV 8: NO LONGER "reads Node.ψ"
--     — that binding was a TYPE mismatch (Node.ψ : Polynomial ↥σ.K, trackOf
--     lands in Polynomial (ZMod p)) and its value is now X + 1; the constant
--     satisfies `RedCellPartition`'s membership (X ∈ factors(x²)) and the
--     toyCell ψ-guards (the EntSt.red ψ datum, a level-0 object) verbatim
...
--     are G1a decide obligations over the pinned chains
inductive ToyCell | rootC | winC | splitC | junk
  deriving DecidableEq
def toyMemA : Option (History 2 (ZMod 2)) → Box 2 9 → Prop
  | none, _ => True
  | some H, x => (H = tA1 ∨ H = tA2a ∨ H = tA2b) ∧
      x 0 = 0 ∧ x 1 = 0 ∧ x 2 = 0 ∧ x 3 = 0 ∧ x 4 = 0 ∧ x 5 = 0
      -- the head read pins the whole window tuple; the leaf reads add nothing
...
def toyModel : TreeModel 2 (ZMod 2) 2 3 9 polTriv :=
  { mem := toyMemA, child := toyChildA, … }   -- [proof fields: root_mem/
    -- mem_single/mem_snoc are G1a decide obligations over the tables above —
    -- closed-form, enumerable. REV 8 (Codex-7 #4d, honesty split):
    -- `mem_realizable` is NOT decide-closable — the built field demands
    -- `HistoryCoherent H ∧ Realizable H ∧ pol.IsCanonPres H` (MovesD/Defs
    -- 631–632) for every realized state. Its NODE-LEVEL legs are now walked
    -- lawful at the table (the REV-8 proof-field roster above: γ-tie, slope
    -- tie, anchor, ψ-order, hLineU — the HistoryCoherent conjuncts 1–3 hold
    -- at the pinned values, displayed); its TRANSITION legs (the 4th
    -- conjunct's IsNodeLift/TransitionCoreL at head → leaf, Realizable's
    -- TransitionAdmissible, polTriv's IsCanonPres) are G1b HAND-BRIDGE
    -- obligations at the pinned stages (construct the leaf stage as the
    -- transition child of the level-0 stage at ψ = X − C 1) — a DISCLOSED
    -- §6 obligation row, never claimed decided; if the G1b construction
    -- fails at these pins, the table re-pins at the MANIFEST, never
    -- silently]
def toyCellA : EntSt 2 (ZMod 2) 2 → Box 2 9 → ToyCell
  ...
  | .red _ ψ, x => if ψ = Polynomial.X ∧ x 0 = 0 ∧ x 1 = 0
      ...
      -- REV 7 (Fable-6 CRIT-1, fix verbatim): the REDUCTION COORDS x0 = x1 = 0
      -- join the guard — rev 6 read NEITHER g NOR x0/x1, so at g = (1,0),
      -- x = (1,0,0,…) the cell was winC with tAν₁ eligible while
      -- toyChildA none tAν₁ x demands x0 = 0: `child_cell_red` (∀ χ g) was
      -- decide-FALSE (§0 record 17). With the pin, x ∈ rootCell χ g ∧
      -- cellOf (.red g ψ) x = winC forces g = toyG — the ∀-g leg is vacuous
      -- off the true track and exact on it; cellEventE at the true entrance
      -- (.red toyG X) is UNCHANGED (its members already had x0 = x1 = 0)
...
def toyCA : CellData 2 (ZMod 2) 2 3 9 polTriv toyModel :=
  -- REV 8 (Codex-7 #1's consequence, the HONEST RE-FENCE): the toys
  -- instantiate the DATA layer `CellData`, NOT the full W4-1 `CellAssign` —
  -- `child_cover` FAILS on both carriers, CHECKED (the charge's "verify"):
  -- at g = (1,0), x = (1,0,0,…) ∈ rootCell χ g, redPoly = x² + 1 = (X+1)²
  -- has ψ = X+1 repeated, but toyChildA none demands ν = tAν₁ ∧ x0 = 0 —
  -- NO realized child, the ∃ fails; and even at g = toyG, members with
  -- x2 ≠ 0 carry no child (the toy realizes ONE branch on ONE track over
  -- the pinned sub-stratum — deliberately partial). Every toy gate is
  -- CellData-keyed (SibCount/JCmultiAt/cellEventE/child_cell_red/cell_local
  -- all read the data layer), so all gates survive; no toy gate ever
  -- premised the covering. Disclosed at §2.9, §6, and the §5 W4-1 row.
  { Cell := ToyCell, cellOf := toyCellA, branchSetOf := toyBranchA,
    cellLevel := fun _ => 4, levelOf := levelIdx, … }   -- [REV 7 (Fable-6
    -- CRIT-2): cellLevel 4 — at rev 6's 3, coords 4/5 sit at level 3 (levelIdx,
    -- n = 2: coords 2ℓ−2, 2ℓ−1 at level ℓ), so `cell_local`'s premise
    -- constrained only c ∈ {0…3} while toyCellA reads x4/x5 at .red/.st —
    -- decide-FALSE at x = 0⃗ vs x' = 0⃗-except-x'4 = 1 (§0 record 18); at 4 the
    -- premise covers levelIdx c < 4 ⟺ c < 6 = EVERY read coordinate, and no
    -- census integer moves (cellLevel enters only cell_local's premise).
    -- The remaining fields are PROOF fields (cell_local/child_cell/
    -- child_root_sub/child_cell_red/hCellFin): G1a decide obligations over the
    -- closed-form tables — every DATA field is displayed above; only proofs
    -- sit under the `…`.
    ...
    -- gone); `RedCellPartition`, RE-WALKED AT ALL g (REV 7 — the rev-6
    -- walk-through checked only g = toyG, Fable-6 CRIT-1's charge): FIRST
    -- CONJUNCT at g = toyG — trackOf tAν₁ = X ∈ x's factor roster ✓, winC
    -- answers only ψ = X (middle ✓), and UNIQUENESS: child(none, tAν₁, x) pins
    -- x0…x5 = 0, so for ANY ψ with tAν₁ ∈ branchSet(cellOf (.red toyG ψ) x)
    -- the REV-7 guard forces ψ = X = trackOf tAν₁ ✓; at g ≠ toyG — the
    -- antecedent x ∈ rootCell χ g contradicts toyChildA's x0 = x1 = 0 pin, so
    -- no realized child exists and the conjunct is VACUOUS ✓. SECOND CONJUNCT
    -- (⊆ face) at ALL g: the guard's x0 = x1 = 0 with x ∈ rootCell χ g forces
    -- g = toyG, and then x0…x5 = 0 → child(none, tAν₁, x) ON THE NOSE ✓; at
    -- g ≠ toyG the branch set is ∅ (guard fails) — vacuous ✓. hred is
    -- instantiable, now at EVERY g]
...
def toyCellB : EntSt 2 (ZMod 2) 2 → Box 2 9 → ToyCell
  ...
  | .red _ ψ, x => if ψ = Polynomial.X ∧ x 0 = 0 ∧ x 1 = 0 ∧ x 2 = 0
      then .winC else .junk
      -- REV 6: the ψ-guard, as on carrier A; REV 7 (Fable-6 CRIT-1): the
      -- reduction coords x0 = x1 = 0 join, same witness/same fix as carrier A —
      -- cellEventE at (.red toyG X) unchanged (the 2⁶ stratum)
  | .st H, x => if H = tB1 ∧ x 3 = 0 ∧ x 4 = 0 then .splitC else .junk
def toyBranchB : ToyCell → Finset (Node 2 (ZMod 2))
  | .rootC => {tBν₁}
  | .winC => {tBν₁}
  | .splitC => {tBνc, tBνd}          -- one branch per side of the 2-side hull
  | .junk => ∅
def toyModelB : TreeModel 2 (ZMod 2) 2 3 9 polTriv :=
  { mem := toyMemB, child := toyChildB, … }               -- [proof fields: G1a,
    -- EXCEPT mem_realizable — the same REV-8 G1b hand-bridge split as
    -- toyModel's bracket above (node legs walked lawful; transition legs
    -- disclosed obligations)]
def toyCAB : CellData 2 (ZMod 2) 2 3 9 polTriv toyModelB :=
  -- (CellData, same REV-8 re-fence as toyCA — carrier B fails child_cover
  --  identically at g = (1,0))
  { Cell := ToyCell, cellOf := toyCellB, branchSetOf := toyBranchB,
    cellLevel := fun _ => 4, levelOf := levelIdx, … }     -- [proof fields: G1a;
    -- REV 7 (Fable-6 CRIT-2): cellLevel 4, as on carrier A — toyCellB's .st
    -- guard reads x3/x4 and rev 6's 3 left x4 (level 3) outside cell_local's
    -- premise; levelIdx c < 4 ⟺ c < 6 covers every read coordinate]

theorem toy_gate : (Nat.card ↥{x | toyTreeA.fiberAt toyModel toyχ x}) * 2 ^ 6
    = 2 ^ 9                                   -- carrier A; via G1a decide + G1b
-- (NO tree-level gate for carrier B — its acceptance integers are the JC-multi
--  identity's own; the shared-split AofTr accounting is the flagged watch item)
theorem toy_sib : SibCount toyModel toyCA toyχ     -- carrier A: the rev-2 kernel
  -- quantifies over the toy's FINITELY MANY cell events (not all subsets —
...
theorem toy_fiber_ne : ∃ x, toyTreeA.fiberAt toyModel toyχ x   -- S-7's witness
theorem toy_e5_instance :                -- carrier A, the FULL statement (REV 5 —
    Nat.card ↥{x | toyTreeA.fiberAt toyModel toyχ x}   -- Codex-4 #13's third
        * (Nat.card ↥(rootCell toyχ toyG)) ^ 1 * 2 ^ 2  -- ellipsis filled): E5's
      = 2 ^ 9 * Nat.card ↥(rootCell toyχ toyG ∩          -- identity at t = 1 —
          trackEvent toyTreeA toyModel toyTracksA 0 toyHcrA)
...
theorem toy_jcmulti_site :               -- record 2's gate — CARRIER B (REV 5,
    JCmultiAt toyModelB toyCAB toyχ      -- Fable-4 GAP-1: the site is now a
      (.st tB1) .splitC toySplitB        -- GENUINE two-slope hull, one branch per
  -- side, sideExp 1 + 1; the identity decides as 2⁴·2² = 2⁶ on the enumerated
  -- events (entEvent = mem tB1 = 2⁶, cellEventE = the joint stratum = 2⁴) —
...
theorem toy_state_cell :                 -- record 6's gate — CARRIER A: the
    stateEvent toyModel (some tA2a)      -- one-side state↔cell tie computes as an
      = cellEventE toyModel toyCA toyχ (toyLedgerA.parentSt tA2a)
          (toyLedgerA.cellAt tA2a)       -- EQUALITY of enumerated events (and the
  -- two-branch-sets counter-instance's union computes ≠ — both faces decided).
theorem toy_v8_wchain :                  -- records 5+7's gate — the SEPARATE
    ¬ HistLawful 2 toyN twoNodeKcardH    -- 𝔽₄-type carrier: the two-node K-card
  -- counter-instance (card K₀ = p, card K₁ = p², g₀ = 1 — §0 record #7) FAILS
  -- the REV-4 w-mult clause; removing the clause makes this gate fail to prove.
theorem toy_v8_monic :                   -- record 9's gate — NEW at REV 5
    ¬ HistLawful 3 toyN3 oneNodePatH     -- (Fable-4 CRIT-1): the p = 3 one-node
  -- monic-slot instance with pattern top digit 2 (s0 + wSide = toyN3, sideDigit
  -- (wSide/e) = 2 ≠ 1) FAILS the REV-5 monic-lead clause; removing the clause
  -- makes this gate fail to prove — the Matches-roster regression is pinned.
  -- [`oneNodePatH` := the record-#9 history on its own p = 3 carrier (𝔽₃);
  --  surrogate + bridge per the architecture. HONEST SCOPE (Fable-4's ruling,
  --  recorded): gates of this genre catch roster REMOVAL and over-strengthening;
  --  they could NOT have caught the original under-sourcing (an honest toy has
  --  pat_top = 1) — the §2.2 both-conjunct roster audit is that channel.]
```
moves_ref: §T.6's gate DISCIPLINE; the Fable-confirm p = 2 census "N(fiber) = 8 =
2^{9−6}, 0/10,000 stability" (9208–9209) is carrier A's Lean-scale target —
carrier A encodes a 2-adic one-side depth-1 split with the SAME ledger shape;
carrier B's integers are DESIGN-DERIVED and surrogate-sealed (declared honestly:
no census row seals them; the p-power bookkeeping above is the design's own
arithmetic, checkable by hand). deps: Defs, T-E8's statement (gate checks the
STATEMENT's instance, not via the theorem). sketch: hand-build both carriers +
enumerate; G1a decides the surrogate identities (incl. the SibCount kernel at
each of carrier A's cell events and the JCmultiAt identity at carrier B's split);
G1b classifies. PASS ≠ closing (SIB) and PASS ≠ closing (JC-multi) — scope
recorded verbatim (7699–7700; (JC-multi) remains census-untested at production
scale, 7481–7482 — a 2⁹ toy gate is a TYPE/SHAPE check, not census evidence).
DAY-ONE DUTY (build order §6): if `fiberAt`/`SibCount` over-strengthen, the toys
die first — the U31 lesson. hypothesis_fields: none.
...
gate.** DESIGN: `CellData.child_cell` (§2.9; REV 6: some-state-scoped — the
...
CellData/CellAssign INSTANCE changes; every MovesT unit is stated over the abstract CA and
...
shape). REV-4 ADDITION (the fence GROWS — Fable-3 CRIT-1's ruling): the w-mult
clause (card K_{i+1} = card K_i ^ g_i, §2.2) is KEYING-DERIVED — its HC source is
`TransitionCoreL`'s field growth F_{i+1} = F_i[z]/(ψ_i), a 4th-conjunct keying
leg — so its discharge is Q1-queued WITH the rest of the fragment; what keeps the
fragment Q1-INSENSITIVE is that the clause pins no keying CONSTANT (no recentering
core / lift / Bézout datum — the V10-inconsistent targets), so no MovesT statement
changes when the re-key's constants move. REV-5 ADDITION (Fable-4 CRIT-1's
ruling): the MONIC-LEAD clause (root read at the monic slot has top side digit 1,
§2.2) rides the SAME queue — its source is the L3 ROOT-BOX data (§0.5's
box-normalization law: a monic box member's degree-n slot digit IS 1), which is
box content, not keying content; it too pins no keying constant, so the
Q1-insensitivity claim is unchanged and the fragment's discharge remains ONE
queued implication covering all clauses. The 2-NODE READSOF GATE becomes a Layer-G unit spec (pre-written, NOT
...
WELL-DEFINED. MovesT reserves the NAMED conjunct slot: `CellData` gains
...
carrier). Until then: `CellData`/`CellAssign` as in §2.9 (no vertex field); HC-2's canonical
...
## REV 4 — NO status moves; REV 5 — NO status moves: the 1/4/12/2 tally, dual-
...
COUNT: **20 seam entries** = 5 (MovesD W4-1…5 — REV 7, Codex-6 g3 = Fable-6
GAP-3: the ratification repairs SHIPPED W4-5, so the rev-6 "19" and "(4
entries)" were stale) + 11 (MovesS S-1…S-11) + 2 (MovesS ratification addenda)
+ 2 (HC-2 Walls A/B); PLUS 2 fenced Q1/Q2 queue-item DESIGNS (§4 — not seam
entries, not discharge targets). Total rows below: 22. Status tally over the 20
— ROW-FAITHFUL (REV 3, Codex-2 #25/Fable-2 GAP-1: rev 2's tally line
...
(S-1, both addenda, S-2, S-3, S-5, S-6, S-7, S-8, S-9, S-10, S-11) · 3 NOT-OURS
(S-4, W4-5, Wall B).** Check: 1+4+12+3 = 20 ✓ (re-counted from the rows below).
REV-7 STATUS NOTE: W4-1 stays REDUCED but its residue CHANGES CONTENT (the
five-round overturn — the covering half is now in-corpus via
`ChildCover`/`constFalse_childCover_false`; only the all-and-only roster law
remains fenced); W4-4 stays DISCHARGED with the obligation RE-KEYED to the
built `NPIDPin` target; W4-5 enters as NOT-OURS with its consumer-duty
adjudication on the row.
...
| **W4-4** NP-ID | (a)'s typed form stated VERBATIM (∀ i, ∀ J — rev 2, Codex 24: no `S.jet` weakening); (c): "becomes a wave-4 theorem from mstar_eq + fresh_band/fresh_cover + D8/D4b" — **REV 7 (Codex-6 g2 = Fable-6 GAP-2, W4-SYNC delta 6): the contract side is now TYPED corpus-side** — MovesD ships `NPIDPin` (NPband = NthrSup, NPv_npid.lean 51–53), `NthrSup`, `noteNP` (L12's piecewise threshold verbatim) and `D4R4_all_noteRange` (the SUM ∧ EV conclusion on L12's own range `noteNP ≤ N`, premised on `hpin : NPIDPin`) | T-D7 (`np_id` + REV 7 `np_id_pin`, split D7a/D7b) | **DISCHARGED**, RE-KEYED (REV 7): the obligation is now `np_id_pin : NPIDPin n pol P Nthr` at the tree-instantiated family (Nthr i = L12's N(η_i, ⊤) = 1 + the largest transported base level), derived from the per-class `np_id`; the built names import BY NAME (`bandLevels` re-defined CLOSED over the built `bandS`/`NPband` — rev 6's bracket RE-BRACKETED built content, the REV-4 rule); the stale "landing deletes D4R4_all's conditionality pointer" sentence is RETIRED (the pointer is already gone corpus-side — landing supplies `hpin` to `D4R4_all_noteRange`); T-C2's NPband corollary consumes D7a explicitly |
| **W4-5** POL-PIN (NEW row, REV 7 — Codex-6 g3 = Fable-6 GAP-3, W4-SYNC delta 7: the entry EXISTS corpus-side, the rev-6 census pre-dated it) | MovesD ships `OffsetPPin` (Type-valued; `Realizes` predicate field + laws `total`, `realizes_ne_zero`; non-vacuity witness `OffsetPPin.lift_ne_zero`) + `CD_canonical` (the D2a class count at a pinned policy) — being STRENGTHENED in MovesD repair round 2 with support/determinism laws (in flight, noted) | — (no MovesT unit) | **NOT-OURS**, with the wave-4 CONSUMER DUTY ADJUDICATED (the round-6 open question "polTriv vs `lift_ne_zero`"): MovesT's statements are policy-GENERIC (every unit binds `pol : CanonPolicy` as a parameter and never reads `pol.liftOf` or evaluates `CD`), and the toy carriers' `polTriv` is GATE-LOCAL — their gates never evaluate `CD` or any lift, so no `OffsetPPin polTriv` instance is owed (the gates' non-vacuity is carried by their own decide obligations, not by `lift_ne_zero`). DUTY RECORDED: any FUTURE MovesT/wave-5 re-key that routes `CD`/class-count values through this corpus must premise the strengthened `OffsetPPin pol` BY NAME — a rider, not a present obligation; MovesV's CD-consuming units own the present duty (their blueprint's business) |
...
13E + 5F + 15D + 2G = 55. REV-4 DELTA: NO unit-count change (55 stands) — the
round-3 repairs are in-unit statement additions (`rootSplit_exists` at T-E5,
`readOf_guards_dead` at T-V8, `chainReads_ne` at T-D14, three gates at T-G1) and
Defs-layer objects (`SubtreeTransport`, `RedCellPartition`, `heads`/`trackDeg`,
`CapEnvelopeCellLocal`, the hoisted `Mlev`). REV-5 DELTA: NO unit-count change
(55 stands) — the round-4 repairs are in-unit statement additions
(`vtree_ext_eq`/`shapeClass_finite`/`AofTr_shape_const` at T-E10, `toy_v8_monic`
+ carrier B + the closed-form tables at T-G1) and Defs-layer objects (the
`HistLawful` monic-lead clause, `ScaleFaithful`, `KBTotTower`, `SubtreeFiber` +
...
ψ-guards + Dwidth/slope columns, the reversed henflip gate). REV-7 DELTA: NO
unit-count change (55 stands) — the round-6 repairs are Defs-layer objects
(`ChildCover`, `NsBridgeStmt`, the CLOSED `windowBlockHts`/`hScale`/`hRes`/
`bandFresh`/`bandLevels`, the per-track `root_total` + `KBTotTower`'s `trackOf`
parameter, the `ClusterFactorOf` realized-membership conjunct) and in-unit
statement/table edits (T-D2's `constFalse_childCover_false`, T-D7's
`np_id_pin`, the toy winC x0/x1 guards + cellLevel 4 + the full-roster node
table, T-F4's in-flight event/mult keying, the §5 re-keys). REV-8 DELTA: NO
unit-count change (55 stands) — the round-7 repairs are the `CellData`/
`CellAssign` split with the mandatory `child_cover` field (§2.9) + the five
full-interface consumers, the decay-premised `NsBridgeStmt` (§2.10), the
`OmSatTot` totality law + `VPPinned.om_total` (§2.7/§2.10), T-F4's `hne` +
...
`HistoryCoherent`'s 4th conjunct + `Realizable` + `polTriv.IsCanonPres` at
the pinned tables (the node-level legs are walked lawful at T-G1; failure
re-pins at the MANIFEST, never silently). E-PHASE OBLIGATION
ROW (REV 7, Codex-6 g1 — disclosed, never claimed): the F1 ten-pin joint
WITNESS — construct the concrete `CellAssign`/`SiteLedger`/`TreeScaffold`
instance at the note's F1 head window (5²² box, 14-pin state, 10-pin window,
4-pin reduction cell) and machine-check the joint row it inhabits; the §0
record-13 arithmetic (5^{22−14}·5^{10} = 5^{18} = card(rootCell), 0 < 5⁸ <
5^{18}) is CARDINAL COMPATIBILITY ONLY — numerical compatibility ≠ existence,
and "the joint row is SATISFIABLE" is hereby DOWNGRADED to "cardinally
consistent; witness owed at E-phase". Zero axioms; zero
`sorry` in the
...
* no `HistLawful` discharge (explicit fenced premise, queued on Q1 — §0/§4;
  REV 4: incl. the keying-derived w-mult clause; REV 5: incl. the box-derived
  monic-lead clause — the fence grew twice, honestly) and no in-corpus
  `state_cell`/`RedCellPartition`/`hdecomp`/`hscope`/`hdict` derivation (owner
  HC-2/shape-side presented-face data, Q1/Q2-adjacent);
* no lawfulness conjunct in τ-irr's RULE (REV 4 — the note's own text: the rule
  is the saturation equation, "pure node/cell data"; lawfulness is perimeter);
* no childless-model falsifier for the per-level `KBTot` at SCALE-DEGENERATE
  levels (REV 5: at scale-POSITIVE levels the falsifier is now fixed-level —
  `ScaleFaithful` pins the cap accessors; the degenerate-level residue is closed
  INTERNALLY by `KBTotTower`, which is what `VPPinned.covering` consumes — the
...
* no ALL-AND-ONLY W4-1 roster law inside the fence — REV 7 (ruling 1): the
  ...
  Codex-7 #1: INSTALLED as the mandatory `child_cover` FIELD of the bundled
  `CellAssign`, with `constFalse_no_cellAssign` the interface-level
  rejection); what remains fenced is exactly the roster half (branch sets =
  L2's full next-node enumeration — `WallAReconciled`, Q1), plus
  `ChildCover`'s canonical-instance discharge (owner HC-2/D4R0K); the toys
  instantiate `CellData` ONLY — they FAIL `child_cover` (checked, computed at
  T-G1: no realized child at g = (1,0) or off the pinned sub-stratum —
  partial by design, disclosed);
* no decide-closure of the toys' `mem_realizable` — the built field demands
  `HistoryCoherent ∧ Realizable ∧ IsCanonPres`; the node-level legs are
  walked lawful at the REV-8 tables, the transition legs are the disclosed
  G1b obligation row above (REV 8, Codex-7 #4d);
* no F1 joint WITNESS yet — the record-13 arithmetic is cardinal compatibility;
  the concrete CellAssign/ledger/scaffold instance at the F1 head window is the
  §6 E-phase obligation row (REV 7, Codex-6 g1);
...
name-for-value regressions). REV-4 ADDITIONS: every accessor read in a displayed
statement must EXIST on the consumed structure (the `ν.len` class — check
against the real field rosters); every ∀-clause over a resolution/height variable
must carry its finite-level cap or a tower quantifier (the slot-0 class:
satisfiability at a fixed level is part of statement review); every audit-named
missing VALUE that exists in a built corpus must be imported by name, never
slotted as a Prop parameter (the LedgerIV class); pushbacks must quote the note's
or contract's own sentence at the row (the two REV-4 pushbacks, #8/#13, model
the required form). REV-5 ADDITIONS: every unit whose conclusion is a CONJUNCTION
must carry a per-conjunct sourcing roster — auditing one conjunct's roster while
asserting the other "definitional" is the T-V8 channel that produced three
consecutive refutations (the Matches class); every accessor whose value the
statement's satisfiability depends on must carry its PINNED defining clause as a
conjunct or a displayed law, never as a bracket remark (the ScaleFaithful class);
every `Tendsto` premise must state the EXACT quantity the sketch's bound needs —
...
slope-0/1 class). REV-7 ADDITIONS: every ∃-witness in a covering/totality law
carries the note's own PER-CASE tie (∀ case, ∃ witness OF THAT CASE — the
untied-∃ class, root_total); toy guard functions must READ every datum their
law's ∀-quantifiers range over (walk-throughs at the pinned instance alone are
the g = toyG class); locality levels must COVER the guards' read-coordinate
roster, checked coordinate-by-coordinate (the cellLevel class); a lawfulness
clause on a shared accessor must tie it to the REALIZED object, not to an
abstract invariant a junk value can also satisfy (the degree-(> n) class);
load-bearing accessors are CLOSED defs at blueprint time — an E-phase
name-binding inside a non-vacuity mechanism or a checkability chain is itself
a finding (the windowBlockHts/bandFresh class); every claimed-fixed §7 row
must be textually REAL in this file (round 6 burned six claimed-fixed rows;
the disposition table is audited row-against-text before any re-submission);
seam rows are re-judged against the AS-BUILT corpora at every round
(`W4_SYNC_DELTAS` is part of the audit input). REV-8 ADDITIONS: a repair
clause is real only when INSTALLED — a defined predicate that no displayed
field or consumer signature takes is an unwired repair (the ChildCover
class); a typed seam's premises must IMPLY its conclusion over the abstract
carriers, checked by RE-RUNNING every on-record counter-model against the
restated type (the NsBridgeStmt class — a false seam hands the owner an
undischargeable statement); landed dispatch semantics are re-keyed by
COMPUTING the checked instance at the built definition, never by assuming
the note-side value survives the carrier (the ∅-shape/PrefIdx class); toy
pins are walked against the FULL built proof-field roster of every consumed
structure, not a remembered subset (the Node-roster class — "only proof
...
own displayed decl in this file (section ref) or (B) a built-source path:line
(verified by grep against the working tree, 2026-07-29). An identifier with
neither is a shipping-blocker defect. SCOPE: Lean-core/Mathlib names
(`Multiset`, `Finset`, `Polynomial`, `Nat.card`, `Filter.Tendsto`, `ZMod`,
`ℤ_[p]`, `PadicInt.toZMod`, `UniqueFactorizationMonoid.normalizedFactors`,
`Ideal.ramificationIdx`/`inertiaDeg`, `AdjoinRoot`, `IsLocalRing`, …) are
library-resolved and excluded; bound variables and structure-field accesses
of in-scope structures resolve through their structure's entry.

**(A) DECLARED IN THIS FILE** (each name's decl is displayed at the cited section):

| section | identifiers |
|---|---|
| §2.1 | `VerdictOK` · `Vd` · `Vd.deg` · `VdAt` · `henVerdict` · `irrVerdict` · `nsVerdict` |
| §2.2 | `accE` · `accF` · `NodeDataLawful` · `HistLawful` · `IrrHalts` · `irrVerdictOf` · `nsVerdictOf` |
| §2.3 | `redPoly` · `rootCell` · `henDegrees` · `henPayload` |
| §2.4 | `NsHalts` · `BranchDichotomy` · `KBTot` · `KBTotTower` · `PrunedMem` |
...
| identifiers | built source |
|---|---|
| `Box` | MovesD/Defs.lean:56 |
| `topLocus` | MovesD/Defs.lean:59 |
| `ShapeRead` (fields incl. `hspecInc`/`hspecRec`/`hmonicRoot`) | MovesD/Defs.lean:70 |
| `ShapeRead.len` | MovesD/Defs.lean:95 |
| `ShapePrefix` | MovesD/Defs.lean:111 |
| `ShapeWF` | MovesD/Defs.lean:161 |
| `ShapePrefix.bandS` / `A` / `Nshape` / `NPband` | MovesD/Defs.lean:227/235/239/247 |
| `Node.sideDigit` | MovesD/Defs.lean:313 |
| `ShapeRead.Matches` / `ShapePrefix.MatchesHist` | MovesD/Defs.lean:325/332 |
| `CanonPolicy` / `IsCanon` / `IsCanonPres` | MovesD/Defs.lean:345/353/367 |
| `InBox` | MovesD/Defs.lean:374 |
| `PrefSet` / `PrefSetAny` / `etaData` / `PrefIdx` / `Shape.NP` / `reprOf` / `Shape.CD` | MovesD/Defs.lean:379/385/402/414/425/442/448 |
| `Shape` (the WF subtype) | MovesD/Defs.lean:189 |
| `Presented` (+ `fiber`/`jet` fields) / `Threshold` | MovesD/Defs.lean:476/494/485 |
| `Presented.event` / `mult` / `event_of_ne` / `mult_of_ne` / `event_empty_shape` / `mult_empty_shape` | MovesD/Defs.lean:507/516/522/527/532/537 |
| `History.snoc` / `History.IsPrefixOf` | MovesD/Defs.lean:585/603 |
| `TreeModel` (structural fields incl. `mem_realizable`) | MovesD/Defs.lean:615 |
...
| `Line` / `Line.at` | MovesC/Defs.lean:255/260 |
| `LevelClause` (this file's `FreshClause`, §2.11) / `FreshData` (+ `.mstar`/`.sat`) | MovesC/Defs.lean:297/317 |
| `Node` (22 proof fields walked at T-G1) / `Node.childWidth` / `Node.staircase` | MovesC/Defs.lean:361/465/494 |
| `History` (+ `root_iff`/`nonempty`) / `strFrame` / `floorH` / `kappaH` / `htH` | MovesC/Defs.lean:565/577/583/589/605 |
| `HistoryCoherent` (incl. the γ-tie) / `TransitionAdmissible` / `Realizable` | MovesC/Defs.lean:634/677/693 |
| `JetSetup` (`coordOf`/`Sigma`/`fresh`/`coherent`/`realizable` fields) | MovesC/Defs.lean:792 |
| `JetSetup.SHZ` / `AdmissibleZ` / `totalPins` / `boxMass` | MovesC/Defs.lean:956/967/972/235 |
| `C4_conditionalMass` | MovesC/C4_conditionalMass.lean:148 |
| `C6_thmC_b` | MovesC/C6_thmC_b.lean:155 |
| `OrdPsiPoly` / `Stage` | Moves/Defs.lean:261/106 |
| `ReadSpecies` | MovesC/Defs.lean:347 |
| MovesSp: `Species` / `Coherent` / `RootStage` / `StageLaws` / `CatalogueWord` / `CanTreeModel` | MovesSp/Defs.lean:37/71/98/109/246/183 |
| MovesS: `TableShape` / `DegCons` / `MeasuredSide` / `LedgerIV` / `RatBurdens` | MovesS/Defs.lean:66/139/149/210/342 |
| MovesS: `RS1Meas` (+ `βmeas` field :36) / `LegAgree` / `RS4Chain` (+ `legs_read` :141) / `wsh17_pin` | MovesS/Interfaces.lean:35/80/121/187 |
| MovesS: `evalRe` | MovesS/EvalReDef.lean:63 |
| `HC2.ReadsOf` | HC2/Defs.lean:283 |

DEFECTS FOUND AND FIXED BY THIS SWEEP (the rule's first enforcement): (1)
`ramIdx`/`resDeg` — E-phase names inside displayed Lean (OmSat's conclusion)
since REV 4 → CLOSED (§2.7); (2) `FreshClause` — no built referent (built name
`LevelClause`) → abbrev + coercion note (§2.11); (3) the jet accessors
(`M₀`/`scaleTop`/`interiorFreshIdx`/`digitAt`/`interiorIdx`/`pinnedOnState`/
`heightOf`/`floorAt`/`digLead`/`interiorBase`/`baseOf`) — bracket-bound since
REV 3/4 inside `NsLumpFamily`/`ZCPack`/`Mlev`'s displayed fields → CLOSED
(§2.11); (4) `NodeExtends`/`IsRootSpecies`/`ChildRoot`/`childHist`/`oneNode`/
`prefixAt`/`stripAbove`/`SameClass`/`ClassPrefixOf`/`PrefixClosed`/
`MaximalAddr`/`withHenV`/`SystemTruncatesTo`/`transportedLevels`/
`jetTopLevel`/`IsLumpLocus`/`pinCount` — bracket/E-phase names in displayed
Lean → CLOSED (§2.11); (5) `TreeIfaceCore` — cited as if contract-side but
built NOWHERE → declared this corpus's own (§2.11); (6) the T-G2 plumbing
(`toyTrees`/`toyFiber`/`ToyDecided`/`toyBelowHaltExcess`) and the record
carriers (`twoNodeKcardH`/`oneNodePatH`) — gate names with no decl → CLOSED
(§2.11, values per their records). RESIDUAL, disclosed: `polTriv`,
`toyLedgerA`, `toyVdict`, `toyTracksA`, and the §2.11 record histories carry
