...
BYTE-FROZEN at seal a77c376 (every finding lives off the exercised strata).
...
>
> **LEMMA HE6-1L [r1] ((LIFT), corrected = LEMMA HE7-L1).** Fix k ∈ ℤ_{≥0}
> and let i₀ = i₀(k) be the unique 0 ≤ i₀ < e₁ with i₀h ≡ k (mod e₁). The
> set of residues realised as res(C(θ)/ϖ(θ)^k) by C ∈ O[x] with deg C < D′
> and dv(C) = k is
>
>     { Σ_{t ∈ T(k)} c_t·η_θ^{t} : c_t ∈ F_Q, not all 0 },
>     T(k) := { t < f₁ : k ≥ (i₀ + e₁t)·h } ,
>
> which is ALL of K^× iff T(k) = {0,…,f₁−1} iff **k ≥ (i₀(k) + e₁(f₁−1))h**;
> a k-uniform sufficient condition is **k ≥ (D′−1)h**; if T(k) = ∅ the
> height k is not attained at all. *(Proof: HE7 §S4.1. Codex's general
> failure condition "k < (i(k) + e₁(f₁−1))h" IS this threshold — the two
> findings name ONE defect, and HE7's is the sharper statement, so it is the
> one adopted; both finders are cited.)*
>
...
> class, read after division by ϖ(ξ)^{dv(A)}, is ι_ξ(γ)·η_ξ^{q} where
> γ = Σ_t res(a_{i₀+e₁t})η_θ^{t} ∈ K is the level-1 slot residue at θ and
> η_ξ^{q} is the RIDER's universal twist. *Proof: GENHN-2's proof uses (T1)
...
* **HE6-BOX-2 (grade) [r1].** Attempt grade **0/2** for this note — the PE1
  ...
  the counter stays at **0/2 post-r1**. THEOREM HE6.A inherits the minimum over its suppliers
