# O4T — the O-4 drainage transfer (DELTA-2 path step) — Phase-B attempt, REV 2 (Fable, 2026-07-31)
...
The self-contained brief is `O4T_phaseB_verifybrief_rev2.md` (hand THAT to the hostile
pass; rev 1 = `O4T_phaseB_verifybrief.md`, audited at V30). This attempt records what
was achieved, the design decisions, the falsifier record, and the honest status.

(REV 2: the V30 hostile audit (`V30_o4t.jsonl`; Codex, fresh context, no-repository)
returned UNSOUND on rev 1 — the core n = 2, 3 ceiling argument confirmed sound
including wild primes, but: TR-Q's "`Γ⁻(N) → ∞`" claim FALSE without an unboundedness
hypothesis on `Γ` (CRITICAL — the package's one false unconditional theorem); Lemma
REC's `z₀ = 0` reasoning invalid with a false "either way" sentence (justification
gap — rescued by the verifier's own observation, now displayed as Corollary RES-0:
zero is never a residual root); the "NP/SF/HEN alone" dependence claim an undercount
(eight further classical facts consumed — now the named §3.1 imports EXT/EF/D/GRD/
UCT/HRG with pinned sources); "n ≤ 3" headlines overstating the proved 2 ≤ n ≤ 3; and
one backwards floor-direction sentence in N3-BRACKET's prose. ALL repaired in the
rev-2 brief — its §0-pre disposition table maps each finding to its fix; everything
V30 confirmed is carried byte-stable there.)
...
* **The transfer skeleton, unconditional** (brief §2): TR-0 (a.e.-finite decision
  ...
  UNBOUNDED majorant `N(f) ≤ Γ(v_p(disc f))` converts M05's verified Lemma-C tail
  into an explicit envelope for the classifier; unboundedness added at rev 2 — V30
  finding 5 exhibited the constant-`Γ` counterexample to rev 1's statement).
* **THE NEW THEOREM (N3, brief §3): the O-4 transfer is UNCONDITIONALLY DISCHARGED at
  2 ≤ n ≤ 3 for the canonical mathematical classifier** (n = 1 trivial, NOT covered
  by the stated theorem — the rev-2 scope pin, V30 finding 3). The canonical order-≤1
  ...
  the DISPLAYED classical-import inventory of brief §3.1 (NP/SF/HEN + rev 2's named
  EXT/EF/D/GRD/UCT/HRG — V30 finding 6 enlarged the list from rev 1's "NP/SF/HEN
  alone" undercount) — no CU-*, no GD-≥2, no BNS (V30 confirmed this scope audit).
...
**PROVED-UNVERIFIED (rev 2; rev 1 audited UNSOUND at V30, every finding repaired —
see the rev-2 brief's §0-pre disposition table):** TR-0/FLOOR/TR-Q(rev 2) + Theorem
N3 + corollaries, awaiting a clean hostile pass on the rev-2 text. TR-OM remains
CONDITIONAL-OPEN on (AGR) with the seam decomposition displayed — this unit does NOT
close O-4 at general n; it closes the mathematical half at 2 ≤ n ≤ 3 and supplies the
general-n skeleton + the sharp target shape Γ(v) = v+1. Verification protocol: hand
`O4T_phaseB_verifybrief_rev2.md` ALONE to a fresh hostile verifier; priority charges
= brief §7 items 1–3 (the shortcut certificate, the case-(b) ceiling corner, the
RES-0/REC repair) and 7–8 (the rev-2 TR-Q hypotheses; no silent (AGR) consumption,
import inventory complete).

## 5. Files

* `lean/notes/openmath/O4T_phaseB_verifybrief_rev2.md` — the repaired self-contained
  brief (REV 2 — hand THIS to verification; §0-pre = the V30 disposition table).
* `lean/notes/openmath/O4T_phaseB_attempt_rev2.md` — this record.
* `lean/notes/openmath/V30_o4t.jsonl` — the V30 hostile-audit transcript (UNSOUND on
  rev 1; the findings rev 2 repairs).
* `lean/notes/openmath/O4T_phaseB_verifybrief.md` — the rev-1 brief (superseded).
* `lean/notes/openmath/O4T_phaseB_attempt.md` — the rev-1 record (superseded).
