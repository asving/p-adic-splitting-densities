# Dfloor — the assembly floor: D-1 (MEAS-BRIDGE) + D-2 (SUM-ONE) + D-4 (BASE-CASES) — Phase-B self-contained verification brief, REV 2

(Fable, 2026-07-31. Companion record: `Dfloor_phaseB_attempt_rev2.md`.
Falsifier — run FIRST, before rev 1 was finalized: `verification/openmath/
...
(This is REVISION 2, repairing the V17 hostile audit (`V17_dfloor.jsonl`,
verdict UNSOUND on rev 1). V17 CONFIRMED: the Haar construction's algebra /
compactness / π–λ bookkeeping, the cylinder mass law, MEAS-BRIDGE(iv)'s EXACT
strict equality and the lax bracket, Lemma EX's role, TRANSFER, SUM-ONE, the
elementary n = 1 root argument BASE-1(a)–(d), and the import-by-import scope
audit — those passages are BYTE-STABLE here. It found and this rev repairs:
(1) Prop H1's singleton clause is FALSE at n = 0 — now a displayed n ≥ 1
hypothesis, reconciled with BASE-0(b)'s Dirac mass; (2) BASE-1(e)'s
algorithmic "1×1 solve / full (U-GEN)+(U-DENS)" clauses do NOT follow from
this package — (e) is rescoped to the proved SEMANTIC clauses and the
classifier-side residue is named and fenced in a new BASE-1(f); (3) Lemma
ADD's proof invoked TYPE before establishing gh separable — reordered,
separability first; (4) C_N(x) is the CLOSED ball; (5) the T-F n = 3
interpolation certificate is now auditable IN-PACKAGE — the five sealed R_σ
displayed with the cleared-numerator degree bound.)
...
* **Theorems BASE-0 / BASE-1, Lemmas ADD / LIN, conventions (C0)–(C2)** =
  D-4 in full; the uniformity root's SEMANTIC clauses at n = 1 proved
  outright, with the classifier-side (algorithmic) half explicitly fenced
  to the classifier nodes — BASE-1(f).
...
d(a, b) = max_i |a_i − b_i|_p, C_N(x) is the CLOSED ball of radius p^{−N}
(the set d(·, x) ≤ p^{−N}) about any of its points — closed, not open, under
the usual convention; being an ultrametric ball of positive radius it is
clopen either way (H0).
...
classes it contains)·p^{−nN} at any common level N;

    (2′)  IF n ≥ 1, singletons are μ-null

(the hypothesis n ≥ 1 is NOT removable: at n = 0 the group G = ℤ_p⁰ = {()}
is a single point, μ is the Dirac mass δ_{()} — consistent with Theorem
BASE-0(b) — and the singleton {()} has mass 1, while p^{−nN} = 1 for all N);
...
SINGLETONS (2′), n ≥ 1 only. {a} ⊆ C_N(a mod p^N) gives μ{a} ≤ p^{−nN} → 0
as N → ∞; the convergence to 0 is exactly where n ≥ 1 enters (for n = 0,
p^{−nN} = 1 identically and indeed μ{()} = 1).
...
* General n (the root's intended use): once T-U + its kernel ledger supply
  ...
  denominators; the all-denominator strengthening is node D-6/O-12). Note
  (REG-p) is an EXTERNAL predicate — defined at nodes T-U/D-6/O-12, not in
  this package; it enters here only as the name of the hypothesis set P.
  This instance is CONDITIONAL exactly on the T-U ledger; the transfer
  theorem itself is unconditional.
...
**Theorem BASE-1 (= D-4's first clause; the uniformity root's SEMANTIC
clauses at n = 1, with the algorithmic half fenced in (f)).**
[(b)] For every prime p:
...
(e) [SEMANTIC clauses only] with R_{(1,1)} := 1 ∈ ℚ(t): α(1, σ; p) = R_σ(p)
    at every prime, the bracket is exact at every level, drainage is
    trivial, and Σ_σ R_σ = 1 — i.e. the SEMANTIC content of the uniformity
    root at n = 1 ((U-GEN)(i)–(ii)'s measure clauses read with the semantic
    counts (5), and (U-DENS)'s density identity α = R(p)) holds
    UNCONDITIONALLY;
(f) [FENCE — NOT PROVED HERE; the classifier-side residue, named] the
    ALGORITHMIC half of (U-GEN)/(U-DENS) at n = 1 — that the OM
    classifier/engine at n = 1 has exactly ONE state, that its producing
    finite solve of (U-GEN)(iii) is the 1×1 system (1)·C = (1) with unique
    solution C = 1 = R_{(1,1)} (pivot 1, p-independent, pole-free), and
    that its decided counts are FAITHFUL to the semantic counts (5) — is
    classifier-side content and does NOT follow from this package: "one
    semantic type" (clause (a)) does not by itself yield "one algorithmic
    state". That residue lives at the tree's classifier nodes — O-1/O-2/
    O-3/O-5 with O-4's OL1-ii agreement seam for the counts, and T-8's
    block solve for the producing system — exactly as §4.3 and §8 fence
    them. Consequently the FULL (U-GEN)+(U-DENS) at n = 1 (semantic AND
    algorithmic clauses together) is NOT claimed by this brief.
...
the singleton Σ₁ is 1. ((f) is a fence, not a claim; nothing to prove.) ∎
...
has type {(1,1)} and density 1 = R_{(1,1)}(q). So the SEMANTIC n = 1
instance of the root's unramified-base clause holds too (the (f) fence
applies verbatim to its algorithmic half); this feeds node D-3 but is
recorded here because it is free.
...
*Proof.* SEPARABILITY OF gh FIRST — so that TYPE may then be applied to gh
(TYPE's hypothesis is separability; establishing it first removes any
circularity). The roots of gh in ℚ̄_p, with multiplicity, are the roots of g
together with the roots of h. Within each factor the roots are pairwise
distinct (g and h are separable by hypothesis). Across the factors: a common
root α of g and h would have its minimal polynomial over ℚ_p dividing both g
and h, hence dividing gcd(g, h) = 1 — impossible. So all deg g + deg h roots
of gh are pairwise distinct: gh is monic separable.

THE TYPE IDENTITY. TYPE now applies to gh. Concatenating monic-irreducible
factorizations of g and h gives one of gh; by uniqueness of monic-irreducible
factorization in the UFD ℚ_p[X] (the monic-normalization argument displayed
inside TYPE's proof — this uses only unique factorization, not separability),
the factor multiset of gh IS the multiset union of those of g and h. Hence
σ(gh) = σ(g) ⊎ σ(h) by (4) applied to the concatenated factor multiset.
(Pairwise distinctness of the concatenated factors holds automatically by
TYPE applied to the now-known-separable gh; directly: within g's (resp. h's)
block the factors are distinct since g, h are separable, and a factor common
to both blocks would divide gcd(g, h) = 1.) Finite J: induction on |J|;
J = ∅ is BASE-0(a): σ(1) = ∅. ∎
...
BASE-1(e) supply the unit row and the n = 1 SEMANTIC values; the solve
STRUCTURE at n = 1 (state count, matrix, faithfulness) is classifier-side —
BASE-1(f)'s fence. D-1's level-0
...
* **T-F** (SUM-ONE point checks): Σ_σ R_σ = 1 EXACTLY at q = 2..200 for the
  n = 2 table and for the sealed n = 3 five-type table [I-5(b)]. To make the
  interpolation step auditable IN-PACKAGE (rev 1 only cited the sealed
  functions), here they are. With P4 := q⁴+q³+q²+q+1, P5 := (q+1)·P4:

      R_{(1,1)³}       = (q⁵ − q⁴ + q³) / (6·P5)
      R_{(1,1)+(1,2)}  = (q⁵ + q⁴ + q³) / (2·P5)
      R_{(1,3)}        = (q⁴ + q³) / (3·P4)
      R_{(1,1)+(2,1)}  = (q⁴ + q² + q) / P5
      R_{(3,1)}        = (q² + 1) / P4

  Over the common denominator 6·P5 (note P4 | P5), the cleared numerator
  A := 6·P5·(Σ_σ R_σ − 1) is a sum of six displayed polynomial terms each of
  degree ≤ 5: (q⁵−q⁴+q³) + 3(q⁵+q⁴+q³) + 2(q⁴+q³)(q+1) + 6(q⁴+q²+q) +
  6(q²+1)(q+1) − 6·P5. So deg A ≤ 5, and by Lemma ID's root count A vanishing
  at ANY 6 checked points q with P5(q) ≠ 0 (P5(q) > 0 for all q ≥ 2) forces
  A = 0, i.e. Σ_σ R_σ = 1 in ℚ(t); the falsifier checked 199 such points
  (and this rev re-verified A(q) = 0 at q = 2..11 with exact integer
  arithmetic). Same shape at n = 2: over 2(q+1) the cleared numerator of
  1/(q+1) + q/(2(q+1)) + q/(2(q+1)) − 1 has degree ≤ 1 and is identically 0
  (the direct check displayed in §5). ALL PASS.
...
* **PROVED HERE OUTRIGHT** (given only the listed imports): H0/H1/H2 —
  H1's singleton clause (2′) under its DISPLAYED n ≥ 1 hypothesis;
  ...
  TRANSFER; BASE-0/BASE-1(a)–(e) (the SEMANTIC clauses; (f) is a fence);
  ADD; LIN; (C0)/(C1) with consistency proofs; Prop N2 (falsifier support).
  Claimed grade: PROVED-UNVERIFIED until fresh-context adversarial passes
  accept this brief.
...
* **NOT CLAIMED:** anything about the OM classifier's decided counts or
  ...
  ALGORITHMIC half of (U-GEN)/(U-DENS) at n = 1 — one-state classifier, the
  1×1 solve (1)·C = (1), count faithfulness — BASE-1(f)'s fence (same
  classifier nodes plus T-8's block solve); the α = R_σ(p) tie at general n
  (T-U + kernel ledger); classifier rates (OL2); the unramified-base package
  beyond the free n = 1 remark (D-3); the Lean `decided`-field faithfulness
  (K-12's audit duty).
...
finite sets are μ₁-null (singletons are null by Prop H1 (2′) — the n = 1
instance, so the n ≥ 1 hypothesis of (2′) is satisfied). Step m > 1: write
...
classical rule only. (viii) [rev 2] H1 (2′): confirm the n ≥ 1 hypothesis
is displayed wherever singleton-nullity is consumed (Appendix A's m = 1
base), and that no claim at n = 0 conflicts with BASE-0(b)'s Dirac mass.
(ix) [rev 2] BASE-1: confirm (e) claims only SEMANTIC content and that
every algorithmic assertion sits inside the (f) fence. (x) [rev 2] ADD:
confirm the proof establishes gh separable BEFORE invoking TYPE on gh.
(xi) [rev 2] T-F: recompute the displayed cleared numerator A and its
degree bound ≤ 5; confirm 6 points suffice and 199 were checked.
