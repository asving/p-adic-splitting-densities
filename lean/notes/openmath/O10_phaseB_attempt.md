# O10_phaseB_attempt — O-10: sibling independence at adapted cells (the D-8-guarded form) + the K-COUNT lemma

Date: 2026-08-01. Prover: Fable (fresh context). Charge (tree §3 item 11 +
...
regime: there is NO small-N anomaly — the product law needs only the
read-depth guard, and the 2ρ buffer buys the explicit fiber value p^ρ
(§5.2). Numerics: `verification/o10_kcount_harness.py`, 68 PASS / 0 FAIL
...
membership depends only on g mod p^K. Box-side counts: at the pinned chart
each monic F mod p^N corresponds to exactly one box point pattern of its
n·N coefficient digits, so all cardinalities below transport verbatim to
box counts (M07 brief §2.1).
...
consequently the product law holds at EVERY N ≥ τ under (A1)–(A3), with no
lower N-guard beyond read depth. (2) Empty degeneracies are consistent: if
...
Then for every N ≥ thr(c) + 2ρ(c):
  ...
  (c) the level-N factor extraction is a constant-fiber surjection onto
      ...
      explicit;
  (d) the same product law (b) holds already for every N ≥ τ(c) provided
      only the reads stay ≤ N − ρ(c) (Theorem 2) — the guard's 2ρ(c)
      buffer is what makes the fiber VALUE p^{ρ(c)} and gives the reads ρ(c)
      headroom, not what makes the law true.
...
behavior. Answer, from Theorem 1(i) + Theorem 2: within the adapted regime
there is NO small-N anomaly to guard against — fibers are constant at
every N ≥ τ (value p^{s(N−τ)}, confirmed exactly by harness T1 sub-guard
rows, e.g. sizes 1/4/8/16 along N = 5..8 at B5), and the product law holds
at every N ≥ τ with reads ≤ N − e_max. What the D-8 guard buys is (i) the
fiber value p^{ρ(c)} (the explicit Jacobian normalization that mass
computations consume) and (ii) ρ(c) digits of read headroom. So the
adopted statement discipline is: the PRODUCT LAW carries the (I-τ)
read-depth guard; the FIBER-VALUE clause carries the N ≥ thr + 2ρ guard;
below it the value is p^{s(M)} — still constant, still cancelling. D-8r
...
(standard). Literature flags for the LITUNIT session: (i) quantitative
