...
PROVED at the w12 seal). dv := 2v₁ (genre E, doubled units); genre F
...
> parameter (h resp. k) admissible over the chain, stage history
...
> INERT, Q−1 for RAM and per 2SIDED pin, 1 for tails; X = #(free
...
>     genre F:  UND(k; H₂) = (q²−1)^t (q−1)^r [q^{k−1+2⌊N/2⌋+k}
>                + 1_{2|N} (q−1) q^{k−1+2N−2⌊N/2⌋̄…}]  — precisely:
>     per (ψ, H₂): q^{2⌊N/2⌋+2k−1} + 1_{2|N}(q−1)q^{comp-priced
>     boundary term} (S6.3 displays both terms slot-by-slot; at odd N
>     it is the single q-power q^{2(N−1)/2+2k−1} = q^{N+2k−2}).
>
...
> (the reader's dv0 = N → UND exit). Hence the model's hidden-class
...
> UND; F's u = N residual band → UND). Every digit CONSULTED by any
...
>   readable event (u < N; the u = N band exits UND by GENH4-CAP).
...
comp-weighted exponents and F_{q²} censuses.) Each is manifestly ONE
...
(machine leg GH-PD):** at q even, '1sq' ⟺ S₁ = 0 (Artin–Schreier
degeneration: T² + S₀ is always a square in char 2), so on the
CONSTRUCTED sub-locus {A₁ ≡ 0} every even-dv read is forced to
refine-or-drain — the cascade P-D described, realized by
conditioning rather than by slot death — while at odd q the same
locus splits '11'/'2'/'1sq' by the square-class of −S₀^eff. The
battery runs both (Fqt q = 2 / Zp p = 2 vs q = 3) and checks the
separator pointwise; plus the liveness scan over a large (h, N, dμ)
grid as an integer check of this lemma's inequalities.
...
With Φ₂ := q²+q+1, Φ₄' := q⁴+q³+q²+q+1 as needed, the sealed engine
computes and this note displays [values inserted at the seal from the
symbolic derivation; the sealed run re-verifies them — commit-2
verdict confirms byte-identity]:

    R_{(1,1)^4}      = (q-1)^3·q^3·(q^3+2q^2+2q+3) / (24(q+1)^2·(q^2+1)·Φ₂·(q^4+q^3+q^2+q+1)/(…))
    [THE ELEVEN CLOSED FORMS ARE PRINTED BY THE ENGINE AND PASTED
     HERE AT THE SEAL — see the sealed block S9.4b below]

    Σ_τ R_τ = 1 exactly (symbolic); numerical anchors at q = 2, 3
    displayed in S9.4b; sanity: E[#roots] = q/(q+1)·(…)-law and the
    unramified mass check.

### S9.4b The sealed density block

(inserted at seal — the engine's `--densities` output, exact ℚ(q)
forms, with the q = 2, 3 anchor vectors and the partial-sum sandwich
row-by-row: for each full-sweep row and each τ, decided_τ(N) ≤
R_τ·q^{4N} ≤ decided_τ(N) + UND(N) as exact integers.)
...
* **GENH4-BOX-1 (grades).** 0/2, composed this session, NO hostile
  ...
  exactly that stack — displayed, not upgraded.
...
* GH-FRESH [SAME] — fresh never-measured rows, pinned reader vs my
  laws, both directions: consE (Zp,7,4,h1), (Fqt,5,4,h1),
  (Fqt,3,5,h1), (Zp,2,11,h5) [deep entry h = 5 — first ever];
  consF (Zp,7,5,k1), (Zp,2,9,k2) [first k = 2]; FULL sweep
  (Fqt,5,3) [fresh full row: partition + all laws + XCHAR-vs-
  committed-(Zp,5,3)… (Zp,5,3) is committed: gives a NEW xchar pair];
  GH-CROSS: FULL sweep (Zp,2,7) [16.7M — the committed consE
  (2,7,1)/(2,7,3) + consF (2,7,1) rows were never cross-checked
  against a full sweep: run one, restrict, compare key-by-key]
  (dropped to optional if the cost probe exceeds budget — disclosed
  either way at seal).
...
* GH-DENS [SYM/IND] — S9: partition of unity for ν₄'s family table
  ...
  ν₂/ν₃ ties (engine, md5-pinned); level-0 census Σ = q⁴; the ELEVEN
...
* GH-SIGMA [IND] — PARI factorpadic σ per decided composite member,
  sampled: fresh (Zp,7,4,h1)E, (Zp,7,5,k1)F, (Zp,2,11,h5)E,
...
**PRE-SEAL SMOKE + preregistered spot predictions: recorded in the
runner docstring at commit 1; predictions P-1..P-9 below.**

* P-1 [NONE] GH-PIN 0/6.
* P-2 [SAME] GH-JSONTIE 0 violations over all 23 committed rows
  (≈ 600+ composite keys + order-1 keys).
* P-3 [SAME] GH-FRESH/GH-PRE2 0 violations; spot values (computed
  from the S6 laws before any run): consE (Zp,7,4,h1): entries
  6·7⁶ = 705,894; INERT(3) = 21·7⁶·… per-key table in the runner;
  consE (Zp,2,11,h5): total (q−1)q^{4·11−28} = 2¹⁶ = 65,536, UND
  per (center,letter,H₂) = 2^{N+h−1} = 2¹⁵ = 32,768 — wait: h = 5,
  N = 11: q^{N+h−1} = 2¹⁵ = 32,768 vs total 65,536: the deep-entry
  drainage HALF the stratum (the ladder has one point: N−1−2h = 0 …
  exact numbers in the runner's prereg block); consF (Zp,2,9,k2):
  total 2⁴·… = 4,096; pre2 (Zp,2,11): 2²⁰ per letter-pair = 1,048,576.
...
## S13. VERDICT (commit 2 — from the committed artifacts)

(appended at commit 2)
