# O4T — the O-4 drainage transfer — Phase-B verify brief, REV 4 (Fable, 2026-08-02)
...
(This is REVISION 4, repairing the pass-3 hostile audit (`O4T_pass3_verify.md`; Fable,
fresh context, standalone read of rev 3, full clean-pass charge including all 11 §7
attack items and a full harness re-run: verdict **SOUND-WITH-GAPS — zero critical
errors**, two minor justification gaps, "both localized to single sentences whose
surrounding conclusions are true and independently re-derivable". The ENTIRE
mathematical core — "the pinned §3.2 policy and its stability, CERT/RES/RES-0/SIDE/REC,
both (CEIL) chains at every probed corner including wild p = 2, 3, the `e·d ≥ 4`
exclusion, TR-0/FLOOR/TR-Q with the unboundedness hypothesis, N3-ENV, the corrected
N3-BRACKET (count inequality + normalization identity + density identity), the
every-lift separability chain, the eight-Fact import inventory with its count
reconciliation, and the §4 rescope" — was upheld line-by-line, all 11 charges PASS,
and the harness reproduced byte-exact modulo the five timing fields. Both gaps are
repaired below; everything pass 3 upheld is carried BYTE-STABLE. The rev-3 changelog
below is history, kept byte-stable.)

## 0-pre-4. Rev-4 changelog (pass-3 findings → dispositions → repair sites)

| pass-3 finding | class | disposition | repair site |
|---|---|---|---|
| GAP-1: (I1)'s step-0 proof sentence is false as written — "reducing to `r` ⟺ `v(α − r) ≥ 1`" fails because root differences take FRACTIONAL valuations in `(0, 1)` (refutation: `f = x² − p`, both roots `±√p` reduce to `0` with `v = 1/2`; the `≥ 1` criterion counts 0 roots against m = 2); the invariant's STATED conclusion is true via the correct criterion `v(α − r) > 0`, which is also the only form anything downstream consumes | gap (minor) | REPAIRED — the step-0 criterion is now `> 0`, with the verifier's one-line derivation displayed (`ᾱ = r ⟺ α − r ∈ 𝔪 ⟺ v(α − r) > 0`); a note pins that no later step uses `b_j ≥ 1` (the case (a)/(b) arithmetic consumes `b_2 > λ_prev` only) | §3.3 Invariant (I1) |
| GAP-2: the §3.1 closing parenthetical "I-TAIL remains the one EXTERNAL named import" undercounts — M05 Lemma B (separable lifts exist; makes `c^sem` a well-defined FUNCTION; consumed by FLOOR, hence N3-BRACKET's left clause) and M05 Lemma D (Krasner–Hensel local constancy; underwrites `c^K`'s soundness and `U^sem_N ⊆ U^K_N` — though NOT the (K) identity the terminal clause consumes) are also external displayed imports; scoped strictly to §3's PROOFS the sentence was defensible, as an unscoped package-level claim inaccurate | gap (minor, prose) | REPAIRED — the sentence now counts THREE external displayed imports (I-TAIL + M05 Lemmas B/D), each with its exact, limited consumption sites, and states the §3-proofs-only scoping under which I-TAIL is the only one | §3.1 closing note |
| observation (not a finding): condition (C1) is logically REDUNDANT — every hull-defining resolved point has height `< k`, so (C1) can never fail; the certificate's operative gate is `x₀ ≤ 1` + (C2) | note | RECORDED — a rev-4 note after the §3.2 walk puts the observation on the record; the pinned policy itself is UNCHANGED (byte-stable), since (CEIL) is stated, proved, and tested for the displayed walk | §3.2 note |
| core: everything else — §§1–6 in full, all 11 §7 charges, the harness (re-run in full: five configs, 2,534,851 classes, ALL PASS, byte-identical results modulo timings) | UPHELD | byte-stable | — |
...
Status protocol: rev 4 is PROVED-UNVERIFIED until hostile fresh-context passes accept
...
rev 3 SOUND-WITH-GAPS at pass 3 — ZERO criticals, the full core and all 11 charges
upheld, two single-sentence gaps, both repaired in this rev: per the adjudication the
unit is one confirming clean pass from acceptance).
...
**External-import count (rev 4 — the pass-3 GAP-2 repair; rev 3's "I-TAIL remains the
one EXTERNAL named import" was accurate only when scoped to §3's proofs, and inaccurate
as the unscoped package-level claim it read as).** The package's EXTERNAL,
quoted-not-proved imports are THREE, all displayed in §1 with named pins and
verification tags, each with its exact, limited consumption sites:
* **I-TAIL** (M05 Lemma C; Lean count form `MovesX.XF7.tailCountBound`) — consumed by
  TR-Q's envelope, hence by N3-ENV and TR-OM's envelope clause. Scoped strictly to the
  §3 proofs this is the ONLY external import (V30: "as acknowledged").
* **M05 Lemma B** ("separable lifts always exist") — what makes `c^sem` a well-defined
  FUNCTION (without it, an all-inseparable class would satisfy the deciding condition
  vacuously for every σ); consumed by FLOOR and hence by N3-BRACKET's left clause.
* **M05 Lemma D** (Krasner–Hensel local constancy) — underwrites `c^K`'s soundness and
  the §1 remark `U^sem_N ⊆ U^K_N`; notably NOT the (K) identity that N3-BRACKET's
  terminal clause actually consumes — that follows from `c^K`'s DEFINITION plus
  class-measurability alone (consumption audit confirmed by pass 3).
No §3.3 lemma or (CEIL) step touches Lemma B or D: the §3 proofs stand on NP/SF/HEN +
the eight Facts above, plus I-TAIL for N3-ENV via TR-Q.
...
(Rev-4 record note — a pass-3 OBSERVATION, not a finding, displayed so that no future
reader credits (C1) with gating work it does not do: **(C1) is logically REDUNDANT**.
Every hull-defining resolved point has height `< k` at scan level `k`, so
`H_res(i) < k` everywhere, and an unresolved point — height `≥ k` — sits strictly
above the resolved hull AUTOMATICALLY: (C1) can never fail, at any scan level. The
certificate's operative content is `x₀ ≤ 1` plus (C2)'s two inequalities; the §3.3
soundness proofs use the true fact "unresolved ⇒ height `≥ k > H_res(i)`", which is
unaffected. The pinned policy is left BYTE-STABLE — (CEIL) is stated, proved, and
harness-tested for the walk exactly as displayed above.)
...
one root (`f` separable). *Proof of (I1) at step 0*: `ᾱ = r ⟺ α − r ∈ 𝔪 ⟺
v(α − r) > 0` (`𝔪` the maximal ideal of the integral closure), so the roots reducing to
the repeated residue `r` are exactly those with `v(α − r) > 0 = λ_prev`; multiplicity
of `r` in `f̄` = the count. *Propagation*: below (Lemma REC). ∎
(Rev-4 note, pass-3 GAP-1: rev 3 wrote the step-0 criterion as `v(α − r) ≥ 1` — FALSE
in general, since root differences take FRACTIONAL valuations in `(0, 1)`; the
verifier's refutation is `f = x² − p`, where both roots `±√p` reduce to `0` with
`v = 1/2`, so the `≥ 1` criterion counts 0 roots against multiplicity m = 2. The
corrected `> 0` form above is what (I1) STATES and the only form anything downstream
consumes — the case (a)/(b) arithmetic uses `b_2 > λ_prev` only; no later step uses
`b_j ≥ 1`.)
...
* Rev-4 (pass-3) repairs are prose/display-level ONLY: the (I1) step-0 criterion
  corrected to `> 0` (GAP-1) and the external-import count corrected to THREE — I-TAIL
  + M05 Lemmas B/D, each with its consumption sites (GAP-2); no displayed theorem
  statement, (CEIL) chain, or downstream consumer changed; the (C1)-redundancy
  observation recorded at §3.2 with the policy byte-stable.
...
12. **The rev-4 repairs themselves** — (i) (I1) step 0: verify the corrected `> 0`
    criterion and its one-line derivation, and confirm nothing consumes the retired
    `≥ 1` form (the case (a)/(b) chains must use only `b_2 > λ_prev`); (ii) the
    three-import external count in §3.1: check every §1 external quote against the
    corrected sentence and each import's stated consumption sites (Lemma B → FLOOR /
    N3-BRACKET's left clause; Lemma D → `c^K`'s soundness and the `U^sem ⊆ U^K`
    remark only, NOT the (K) identity); (iii) the §3.2 (C1)-redundancy note: confirm
    it is purely an observation — the pinned policy, (CEIL), and the harness
