# O2a — verification brief REV 3 (self-contained; for a fresh-context hostile verifier)
...
and continue); fix nothing. This is REV 3, repairing the V14 hostile
verification of rev 2 (verdict UNSOUND; 1 critical, 1 justification gap);
rev 2 had repaired the V10 verification of rev 1 (UNSOUND; 9 critical, 3
gaps). Everything V10/V14 confirmed is carried byte-stable; every finding is
repaired at a flagged location (changelogs below). The FIVE open lemmas
OL-1..OL-5 of §4 are DECLARED — they are not findings; an UNDECLARED
dependence is.
...
3. **Theorem 2** (all orders): well-posedness, invariant matching, order
   ...
   predicate and on five EXACTLY-STATED open lemmas (OL-1..OL-5 below) —
...
OL-4/OL-5 explicitly assert, injectivity of Θ on histories with a terminal last
...
## 4. The five open lemmas (exact statements; NOT proved anywhere in the note)

OL-2/OL-3/OL-4/OL-5 are statements about the project engine's stage flow — an
