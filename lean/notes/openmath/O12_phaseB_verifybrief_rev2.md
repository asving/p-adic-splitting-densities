# O12 — verification brief REV 2 (self-contained; for a fresh-context hostile verifier)
...
(This is revision 2. A prior audit confirmed the determinant algebra, the
subring lemmas, and the numerical identities, and found three defects, all
repaired here: the entry lists E(e) are now an explicit FINITE list (§2.3);
the organization-O2 solve is no longer presented as a division by 1 − κ₀ of a
row containing the current unknown — the no-feedback and re-entrant readings
are separated and both solved explicitly (§2.4, Theorem 1(3), Theorem 2 step
(g7)); and Lemma L6 now proves the cell-exponent claim over ALL coefficient
columns, including the semantic identification of the polygon cell and the
positivity c ≥ 1 of every depth coefficient (§3, L6a–L6c).)
...
list (§2.3). Write 𝔅_n for this family of tables. The project's concrete
...
**CLAIM (the theorem to verify).** For every n ≥ 2, every booking, and EVERY
prime p, the table satisfies (REG-p). Stronger, and what is actually proved:
...
of the state law is:
    ...
    C_e \ R_e (Newton polygon with minimal slope in (0,1) — Lemma L1):
          partitioned by the polygon P and residual shape (§3, L6a/L6c);
...
### 2.3 The table rows: an explicit finite list, and the grammar

The rows of block e are the CLOSED masses (depth sums performed) of the
stratum families of §2.2 — finitely many:

* the kernel entry (per booking, §2.4);
* one closed mass per divisor pattern π (finitely many π for each e);
* one closed mass per polygon family (face-kind sequence + residual shape;
  finitely many, §3 L6; the depth sums close by L6b/L6′);
* the terminal, split and junction rows: finite ℚ(q)-combinations of the
  above with products of lower-block β-legs;
* the entrance and shape weights: the normalizations of (g2) below;
* the β-legs q ↦ Σ_{σ′} β_{e′,τ′}(σ′)(q^δ) for e′ < e, δ ∈ Δ, and block e's
  own solved β_e(σ) (verdicts σ of degree ≤ n: finitely many).

E(e) := this finite list. (Δ is any finite subset of ℤ_{≥1} containing the
consumed depths, all ≤ n; its exact contents are irrelevant — the claim is
proved for ALL δ ≥ 1.)

The grammar 𝒢 below is the PROOF DEVICE (not the definition of E(e)): every
member of E(e) is exhibited as built by finitely many applications of
...
of face j, nonzero ends) depends only on κ — not on the slopes — and every
residual-shape probability is ONE fixed element of ℚ(q), a count ratio of
the forms (g1)/(g2), constant along the depth cone.
...
(face-kind κ + residual shape, summed over its depth cone) is a finite sum
— over L6b(ii)'s finite enumeration — of terms A·∏_{j≤k−1} q^{c_j}/(q^{c_j}−1)
with all c_j ∈ ℤ_{≥1} and A of the forms (g1)–(g3), w-independent (cell part
from L6a at the nested minima; residual factor from L6c; the conditional
normalization q^{+e} is (g1)). These are exactly the (g4) shapes. The only
...
including every solved β_e and every β-leg at every depth δ ≥ 1, lies in ℛ.
Hence (L7(i)) no entry has a pole at any rational x with |x| ≥ 2; in
particular every entry is defined at every prime power q₀ ≥ 2.

*Proof.* Bottom-up induction on e; each E(e) member is exhibited in its 𝒢
generation (§2.3), and ℛ absorbs every step: (g1) ℚ[q] ⊂ ℛ. (g2) q^{-b} ∈ ℛ;
(q^d − q^{d−1})^{-1} = q^{1−d}(q−1)^{-1} ∈ ℛ; (q−1)/(q^{n+1}−1) ∈ ℛ.
(g3) (1 − q^{-1})^V q^{-N} ∈ ℛ. (g4) by L6′ every depth sum in a 𝔅_n stratum
family is a finite sum of A·q^c/(q^c − 1) with c ∈ ℤ_{≥1} and A ∈ ℛ — in ℛ;
this covers the polygon cones (exponents c_j ≥ 1 by L6b) and the (O3)
in-place resummation (ratio q^{−E}). (g5) L7(ii). (g6) ℛ is a ring.
(g7) the non-returning row of block e is generated by (g1)–(g6) from closed
masses and from β_{e′}(q^δ), e′ < e — in ℛ by the induction hypothesis and
(g5) — and the solved form is β_e = u^{−1}·(row) with u = 1 − q^{1−E}
(bookings O1, O2′, O3, via Theorem 1(3)'s explicit solution of the balance
equation — NOT a division of a row containing β_e) or u = 1 − q^{−E}
(booking O2, whose rows contain no β_e by definition). Both u are units of ℛ
(L7(iii)), so β_e ∈ ℛ. σ′-sums are finite; the kernel is 1×1, so no adjugate
denominators arise beyond u. ∎
...
* Degree-2 formal instance (the project's machine-checked n = 2 table):
  ...
  q²+q+1 (a divisor of q³−1). This is (O2) at e = 2, E = 3: kernel
  q^{-E} = q^{-3}; the terminal masses are grammar elements (1 − q^{-1},
  q^{-1} − q^{-3}) summing with the kernel to 1 — the κ₁ mass sits inside
  1/q − q^{-3} = (q^{-1} − q^{-2}) + κ₁ — and the recorded solve is EXACTLY
  the no-feedback division: (1 − 1/q)/(1 − q^{-3}) = q²/(q²+q+1) and
...
8. §2.3: that E(e) is genuinely finite (patterns, face-kinds, residual
   shapes, verdicts, Δ all finite) and every listed member's 𝒢-generation
   is exhibited.
9. The scope declaration (§1): the claim is about the §2 tables; value
...
10. Quantifiers: the claim is ∀ prime p, ∀ δ ≥ 1, ∀ e — verify no step
