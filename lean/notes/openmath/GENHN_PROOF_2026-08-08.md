...
## S2. PROOF OF THEOREM GENHN.CLASS

*(i) The datum.* By GENIND-1(i) (@ pin, proved at general (m, d)),
every state of every cluster frame is exactly one of DRAIN / DEC /
α / β / CS, and a CS event is by definition a repeated residual
factor (ψ, μ), μ ≥ 2, on a side of slope h/e₁ (lowest terms) with
e₁·deg ψ ≥ 2. Set f₁ := deg ψ. The datum G lists exactly the local
invariants of that event; the constraint e₁f₁μ ≤ m_frame is the
abscissa-length budget of GENIND S5.1 (the side spans e₁·g ≥ e₁f₁μ
abscissa units inside a polygon of span m_frame), and m_frame·d ≤ n
along any chain (fracture only shrinks degree). The entry pattern —
which vertex the side hangs off, the siblings (split pieces, β-child
blocks), the prehistory chain — is frame data OUTSIDE the (ψ, μ)
factor.

*(ii) Finiteness.* The triples are bounded as displayed. For the
entry patterns: at fixed ambient degree the frame shapes around the
side are the finitely many depth-0-polygon families of W-12.A plus
the α/β prefix grammar, each (A1)-admissible; LEMMA GENIND-0 closes
products/unions/reparameterizations. (The n = 4, 5 instances are
QSCOUT22's Q22-A table and GENIND S5.3's V/FULL tables.)

*(iii) Uniformity in n.* Three supplier mechanisms, composed:
1. **Fracture permanence (GENIND-1(iii), @ pin, general m):** after
   the frame's polygon separates the (ψ, μ)-block from its siblings,
   every later event inside the block consults only the block's own
   state; separation digits are never touched; sibling blocks evolve
   independently. So the stage's read is a function of the stage
   state alone.
2. **The fibration (GENHN.A(i), S3):** the stage state space is the
   Φ′-development digit space at explicit floors — coordinates
   intrinsic to (Q, e₁, f₁, μ, h, N), with the ambient contributing
   only the opening fiber (the entry letters and the siblings' own
   coordinates, which multiply as LEMMA GENIND-3's route fibers).
3. **Letter-blindness (J-D0 @ pin):** the residual letters and the
   twist gauge (S4's normalizer cocycle) act by type-preserving
   bijections on stage states, so per-key counts do not depend on
   which letters the ambient handed the stage.
Hence two CS events at ANY two degrees n, n′ with the same
(Q, e₁, f₁, μ, h, stage window) have count-isomorphic stage systems:
the genre's law is a function of G alone. ∎
...
## S3. THE STAGE RING AND THE FIBRATION (GENHN.A(i))

### S3.1 The stage ring at general (e₁, f₁)
...
## S4. SLOTS AND NO-CANCELLATION (GENHN.A(ii))
...
## S5. THE WINDOW LEDGER (GENHN.A(iii))
...
### S6.3 Drainage at general genres (GENH4.B's generalization)
...
## S7. PROOF OF THEOREM GENHN.C (σ at every μ = 2 stage)
...
μ = 2 stage leaf. The (2,1,2)/(1,2,2) instances are THEOREM GENH4.C
verbatim ({(4,1)}/{(2,1)²}/{(2,2)} and {(2,2)}/{(1,2)²}/{(1,4)});
the (3,1,2) instance gives {(6,1)}/{(3,1)²}/{(3,2)} — first tested
...
(stage-e = 2, stage-μ = 2, stage-CS abscissa budget 4 ≤ 4 ✓) has
...
THEOREM GENHN.CLASS + GENHN.A TO THAT SYSTEM (all of S2–S6 used
only: complete-DVR base, finite residue field, integer value
ladder, window with upward carries — all held by the stage system,
with the ragged band handled by GENHN-3 at each level): the inner
stage re-bases with (e₂, f₂) relative to K and dv, i.e. ladder
density e₁e₂ and census field F_{Q^{f₁f₂}} over the base —
multiplicative composition. σ-dictionaries compose because (T1)/(T2)
stack: an inner root has v ∈ (1/e₁e₂)ℤ-classes and residue
generating F_{Q^{f₁f₂}}. Twist layer: the composed normalizer
cocycles multiply as z-power products; their coherence across
levels is the (EC-q) ladder grammar (SIGMALAW @ accepted pin — the
supplier built for exactly this composition), consumed at the
LETTER layer only; counts remain twist-blind at every level (J-D0).
GRADE: inherits GENHN.A's clause grades AT EACH LEVEL (in
particular layer-1 consumption of the graded frame, and
...
For any such \(m\), the congruence

\[
i_0h\equiv m\pmod {e_1},
\qquad 0\le i_0<e_1,
\]

has a unique solution because \(\gcd(e_1,h)=1\). Set
...
Since \(m>D'h\) and \(i_s<D'\), every \(r_s\) is nonnegative. LEMMA GENHN-LIFT therefore gives, for every

\[
\lambda=\sum_{s=0}^{f_1-1}\lambda_s\eta^s\in K,
\]
...
of degree \(<D'\), whose nonzero terms all have exact \(dv\)-height \(m\), and whose normalized residue is \(\lambda\).
...
Write the resulting slot series as

\[
\mathscr A_j(t)=\sum_m\lambda_{j,m}t^m.
\]

By GENHN-2, the least nonzero slot height is exactly \(dv(A_j)\), and its \(K\)-coefficient is exactly the normalized leading residue used by the stage polygon. Thus the polygon of

\[
Y^2+A_1Y+A_0
\]

and every residual quadratic read from it agree, point for point, with those of

\[
Y^2+\mathscr A_1(t)Y+\mathscr A_0(t)
\]

over the integer \(t\)-ladder with coefficient field \(K\).

GENHN-3 gives the required finite-window compatibility: decisions below the cap depend only on retained digits, and carries move upward. GENHN-CAP-GEN supplies the exact conservative rule where a coefficient window leaves only part of a slot visible. Therefore the slot bijection conjugates the GENHN reader to the finite-window \((2,K)\)-reader at node \(B\). This proves clause 1.

For completeness, the transported depth-zero counts may be read directly. Let \(b\) be a lower node. If \(A_0\) vanishes through \(M\), an exact value \(w>b\) of \(A_1\) contributes

\[
(q_K-1)q_K^{M-w}
\]

states and is `SPLTAIL` precisely when \(2w<M+1\); the remaining states are `UND`.

If \(A_0\) has exact value \(u\), its number of choices is

\[
(q_K-1)q_K^{M-u}.
\]

For every \(w>b\) with \(2w<u\), the `2SIDED` count is

\[
(q_K-1)^2q_K^{(M-u)+(M-w)}.
\]

If \(u\) is odd, the remaining states are `RAM`, with the free \(A_1\)-tail beginning at \(\lceil u/2\rceil\).

If \(u=2\kappa\), the leading residual is

\[
T^2+aT+b,\qquad a\in K,\quad b\in K^\times.
\]

The finite-field censuses are
...
Each residual choice has the common free-tail factor

\[
q_K^{(M-\kappa)+(M-2\kappa)}
=|\mathcal N_\kappa(M)|.
\tag{CR-7}
\]

These are exactly the standard \((2,K)\) depth-zero cell counts.

Now fix an alpha letter \(s\in K^\times\). Its residual is \((T-s)^2\). Since \(\kappa>B\), GENHN-LIFT supplies the required exact-height \(K\)-lift of \(s\). GENHN-4 then applies: recentering the key by that lift kills the two alpha pins, sends the slice into

\[
dv(A_1')>\kappa,\qquad dv(A_0')>2\kappa,
\]

and acts triangularly on all deeper digits. GENHN-3 shows that the induced truncated transformation respects the cap. The opposite translation is its inverse. Hence, for fixed \(s\),
...
The child reader is therefore the same reader at lower node \(\kappa\), so every recursively refined key has exactly its standard child-node cardinality. Summing (CR-8) over the \(q_K-1\) nonzero letters proves (CR-2) and clause 3. It also proves clause 2 by induction on the finite recursive history.

At a common coefficient-window boundary, let \(V_m\subseteq K\) be the visible \(F_Q\)-subspace. Its nonzero readable vectors number

\[
|V_m|-1=Q^{\dim_{F_Q}V_m}-1
       =Q^{\operatorname{comp}(m)}-1.
\]

GENHN-CAP-GEN says that an unread component is never consulted: an undercuttable value drains to `UND`; otherwise a terminal result is returned exactly when it is constant on the affine completion set; an alpha refine is performed only with a complete \(K\)-pin. This proves (CR-3) and clause 4.

Finally, for arbitrary \(M\),
...
When \(M\ge2B\), this gives (CR-5). Since \(S\le B\),

\[
\frac{|\mathcal N_S(M)|}{|\mathcal N_B(M)|}
=
q_K^{(2M-3S)-(2M-3B)}
=
q_K^{3(B-S)}.
...
Because \(B-S=e_1h(f_1-1)\), the ratio is \(1\) exactly when \(f_1=1\), and is strictly greater than \(1\) for \(f_1\ge2\). This proves (CR-4)–(CR-6). ∎
