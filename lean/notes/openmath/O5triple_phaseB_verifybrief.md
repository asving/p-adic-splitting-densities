# O5triple — O-5 + D-7a/b/c (the leaf/factor bijection triple) — Phase-B verification brief

**SELF-CONTAINED. Fable prover, 2026-07-31.** Verification target: hand THIS document to a
fresh-context adversarial verifier with the standing charge (quote each offending passage;
classify *critical error* vs *justification gap*; fix nothing).
...
brief answers that finding by (i) proving an unconditional SEMANTIC leaf/factor bijection
theorem (Theorem A, §4) over the pinned GMN floor, and (ii) deriving the full triple from
...
**Two NEW literature-reading obligations declared by this brief** (both inside GMN;
transcription duties per §4 rule 5 of the tree, NOT new mathematics):

* **(GMN-COH)** (flow coherence; the content of (L11)'s warrant sentence): for a sprouted
  type t′ = (t; λ_S, ψ) of order r (sprouted because a(S,ψ) ≥ 2), the (L9) dissection
  factor G_{S,ψ} of f equals f_{t′}, the type-t′ cluster of f, and ω_{r+1}(f) (w.r.t. t′
  and its representative) = a(S,ψ). [This is the per-node identity underlying eq. (37);
  the exact GMN §3 statement is to be transcribed verbatim at declaration.]
* **(R-1)** (representative shape; the content of (L4)'s "of type t"): the constructed
  φ_r has one-sided N_i(φ_r) of slope λ_i with R_i(φ_r) ∼ ψ_i at every i ≤ r−1, top
  multiplicity 1. [GMN §2.1's chain display states this for the type's own φ_{i+1}-chain;
  Def 2.12's "plays the analogous role…" sentence carries it for representatives.]
...
its distinct residual irreducibles. **Seam flag (R-2), the (L3+)_r convention:** when
...
ℓ(S). [The j₀ = 1 convention at order 1 is exactly O2a rev-2's (L3+), falsifier-covered
...
*Proof.* (a): REP-IRR. (b): v_r is integer-valued on O[x]∖{0} [(L5)/Def 2.5 vocabulary],
so the length-1 side from (0, v_r(a₀)) to (1, u₁) has integral slope: e_S = 1, d(S) = 1;
NODE(iv)/(L8) gives irreducibility with (e, f) = (e₁⋯e_{r−1}·1, f₀⋯f_{r−1}). Equal to
(a)'s by inspection. ∎
...
## 4. Theorem A — the semantic leaf/factor bijection (unconditional over §2)
...
irreducible factors). Depth: an active node of order r corresponds to a sprouted
non-complete type; (L13) gives r* with every type in t_{r*}(g) g-complete, so no active
node has order ≥ r*; depth ≤ r* + 1 < ∞.
...
**Theorem B (transport bijectivity; the checksum-coverage mechanism).** Assume (I-1)(c),
(I-2), (I-3). For every monic separable lift g of x, β_g is a bijection
...
**Proved here, unconditional over the §2 floor** (with displayed reading flags (R-1),
(R-2)/(L3+)_r and the declared (GMN-COH)): Lemmas H0, REP-IRR, NODE, LEAF-INV, STEEP;
Theorem A.
**Proved here, conditional on the displayed interface (I-1)–(I-3):** Theorems B–E =
...
* (GMN-COH), (R-1), (R-2)/(L3+)_r — GMN transcription duties [LITERATURE; (R-2)'s
  order-1 case falsifier-covered by O2a F5].
* (HEN-LIFT), (DEFECTLESS), (L1)–(L13) — LITERATURE-CONDITIONAL/CANDIDATE per the
  standing pins; no new axiom is declared by this brief.
...
**O-5 + D-7a/b/c: PROVED at the displayed interface** — the semantic leaf/factor
bijection (Theorem A) is unconditional over the pinned GMN/Hensel/defectlessness floor
(two named transcription flags + (GMN-COH) declared); the classifier triple (Theorems
B–E) follows for every monic separable lift given (I-1)–(I-3), which are exactly the
tree's upstream O-2a / O-3 ⊕ O-2b faces, itemized per clause. Dependency sharpening on
record: D-7b consumes NO no-missing clause. Falsifiers: 1341/1341 with a load-bearing
negative control. Pending: fresh-context hostile verification of THIS brief.
