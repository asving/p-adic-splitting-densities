...
### Dated proof (2026-08-12 — sol certification P4: the (RM-1) interface lemma; AC-1/AC-4 discharged)
...
> \]
>
> **Strictness of the key difference.** Write
> \[
> \Phi _1^\sigma=\Phi _0^{e_0}+\sum_{k<e_0}b_k^\sigma\Phi _0^k,
> \qquad \sigma\in\{',\,''\}.
> \]
> Same residue gives
> \[
...
> \]
>
> This is the exact weight-content consequence of ITERLAWN’s \(m=1\)
> DIGIT-SPLIT \(+\) DEV-1 reduction. For completeness, run that displayed
> reduction on a current term \(a\Phi _0^m\), \(\deg a<d_0\). Substitution of
> \[
...
> \[
> e_0w_0(r)+(m-e_0+k)h_0
> \ge e_0w_0(a)+mh_0,
> \]
> and the \(q\)-child has an additional \(h_0\). Collisions cannot lower
...
>   and the prescribed \(R\)-read. All four are supplied above, so its
>   on-line \(R\)-slot, strictly-above \(Q\)-slot and
>   \(\epsilon'\)-carry/canonical-lift cancellation are unchanged.
