# O-14a′ Phase-B attempt — the RunRealizerExists residue: the (S6b′) interior support clause + the last-node flank
...
**Author**: Phase-B prover unit O14ap, 2026-07-31/08-01. **No Lean edits**; math only.
**Falsifier suite**: `O14ap_check.py` (same directory) — ALL PASS (7/7; PARI cross-check
of the countermodel frame included).

---

## 0. Verdict summary (the leaf re-cut)
...
3. **THEOREM 5 (general ambient F, deg Φ > 1): the compiled singleton-support clause is
   ...
   checks A1–A4). The FAITHFUL law — the carry-algebra (S6b′) support law: monomial
   support in ONE forced-class arithmetic progression *of the frame-creating read's
   stride e and length g, in frame-slot (Φ_prev-adic) vocabulary, recursively down the
   tower* — is stated (§5.3) and proved for the canonical offset-P realizer (§5.4).
   The compiled clause reads the progression at the recentering node's OWN pair
   (e, g) = (1, 1) and in X-monomial vocabulary: two independent transcription
   divergences, each separately fatal at deg Φ > 1.
...
Grade requested: **REPAIRED** for the leaf as a whole (the compiled general-F statement
is refuted-as-stated with the repaired faithful law proved; the consumed-instance
statement `RRE(p, ZMod p)` and the flank are PROVED outright).
...
### 5.3 The faithful (S6b′) support law (the carry-algebra statement)

Notation: a stage σ_r of the intended tower with frame key Φ_r built by the read
(e_{r−1}, h_{r−1}, g_{r−1}) from the previous key Φ_{r−1}; coefficient spaces developed
in the previous key: for t ∈ C_{Φ_r}, t = Σ_j B_j·Φ_{r−1}^j with deg B_j < deg Φ_{r−1},
j < e_{r−1}·g_{r−1} (slots). The **forced class** of a parent-scale target weight W is
j₀(W) := (h_{r−1}⁻¹·W mod e_{r−1}) ∈ [0, e_{r−1}) — well-defined since
gcd(e_{r−1}, h_{r−1}) = 1; in node vocabulary h⁻¹ mod e is the canonical Bézout
coefficient bezT(e, h).

> **(S6b′-SUPP).** Let σ_r be a stage of a coherent tower, W ∈ ℤ a parent-scale weight
> with W > wPrev_r(Φ_r) (= e_{r−1}h_{r−1}g_{r−1}, the clean threshold), and c̃ ∈ K_rˣ any
> digit scalar. Then there is a realizer t with wPrev_r(t) = W, digit scalar c̃ at the
> forced position, such that:
> (i) [level r−1] the Φ_{r−1}-slot support of t is contained in the single forced-class
> progression { j₀(W) + e_{r−1}·k : 0 ≤ k < g_{r−1} };
> (ii) [recursion] each occupied slot's coefficient B_{j₀+ek} is itself an (S6b′-SUPP)
> realizer one stage down, at its own (integral, threshold-clearing) target weight
> (W − (j₀+ek)·h_{r−1})/e_{r−1} — so the support in the tower's multi-index digit system
> is ONE forced-diagonal cylinder per level, down to the base, where realizers are
> p^m·û with û a unit lift of a prescribed base-field residue (support {0} when the
> base key is linear).
>
> At a recentering read (the CanRealizes consumer): W = w(Φ_r) = the key weight, the
> threshold is Lemma 3.1's inequality, and the progression (i) has stride e_{r−1} and
> length g_{r−1} — NOT the recentering node's own (e, g) = (1, 1). The compiled (CR2)
> reads the progression at (ν.e, ν.g) and in X-monomial vocabulary; both readings are
> refuted by §5.1 (and §5.5), and (S6b′-SUPP) is the statement the fence event should
> re-adjudicate (CR2) to. In X-vocabulary (i) specializes correctly ONLY at towers
> whose keys are X-power-pure (e.g. root key X with all lower realizers constants).

### 5.4 Proof of (S6b′-SUPP) for the canonical offset-P realizer

This is the dual-accepted (S6b′) construction (T-1, §B2-DEF D.7(viii)) with its support
content made explicit; existence/well-definedness is accepted content, the support
clause is by-construction, displayed here so the note is self-contained.

*Construction.* Write (e, h, g) := (e_{r−1}, h_{r−1}, g_{r−1}), ψ := ψ_{r−1}, z̄ := z̄_{r−1}.
Since gcd(e, h) = 1, the congruence W ≡ j·h (mod e) has the unique solution class
j ≡ j₀(W); on the progression j = j₀ + ek the prescribed lower weights
W_k := (W − (j₀+ek)h)/e are integers. The slot positions of consecutive progression
slots differ by exactly 1 in the previous stage's z (the stride rule: position
p_j = (j − t_prev·γ)/e_prev steps by 1 per e-step), so there is a forced common offset
m with slot k sitting at z-position m + k. Let P ∈ K_{r−1}[z] be the UNIQUE polynomial
of degree < g with z̄^m·P(z̄) = c̃ (K_r = K_{r−1}(z̄) = K_{r−1}[z]/(ψ), a degree-g
extension: {z̄^m·z̄^k}_{k<g} is a basis scaled by the unit z̄^m). Set

    t := Σ_{k < g, P_k ≠ 0} t_k · Φ_{r−1}^{j₀ + e·k},

where t_k is a lower-stage realizer of digit scalar P_k at parent-scale weight W_k
(recursively (S6b′-SUPP) one stage down).

*Well-definedness (the two-line mutual recursion).* Each W_k clears the lower threshold:
W_k ≥ (W − (eg−1)h)/e > (ehg − (eg−1)h)/e = h/e ≥ h/e > wPrev_{r−1}(Φ_{r−1}) — the last
inequality is (I-aug) at the read that created stage r−1, i.e. Lemma 3.1's inequality
one stage down; the recursion bottoms at the base stage, where realizers are unit lifts
times p-powers with no threshold. [This is the pass-13-confirmed recursion of the
accepted note; re-derived, not just cited.]

*Weight and digit.* Every summand has stage-r parent-scale weight
e·W_k + (j₀+ek)·h = W (the equal-weights design), so wPrev_r(t) = W and the digit of t
is the sum of the slot digits at their stride positions:
R_{r−1}-level, t reads as Σ_k P_k·z^{m+k} = z^m·P(z); after the ψ-reduction/evaluation
into K_r the digit scalar is z̄^m·P(z̄) = c̃ at the forced position. At a recentering
consumer (stage-r pair e = 1, t-Bézout = 0) the position is 0 and
R_r(t) = C(c̃)·T 0, matching (CR3) exactly.

*Support.* Clause (i) is the literal index set of the sum; clause (ii) is the recursion.
∎

*Tightness (why length g, not 1).* The §5.1 countermodel also shows the progression
LENGTH is sharp: any single-slot realizer t = B·Φ_{r−1}^j has digit scalar in the coset
K_{r−1}ˣ·⟨z̄⟩-monomials (one basis vector), and centers like (1+i)·i^m require P with TWO
nonzero coefficients. So no "monomial" strengthening of (S6b′-SUPP) is available: the
carry-algebra support law is a forced-class PROGRESSION law, and that is the faithful
reading of "monomial support" (one graded monomial PER SLOT per level — not one slot).
...
* **Proved outright (bare coherence + node/stage typing, no extra hypotheses):**
  ...
  Lemma 3.1, Lemma 3.2, Theorem 3, Corollary 4. These consume ONLY the §1 clauses; every
  use is cited by label in the proofs.
* **Proved at the intended tower (consuming T-1's dual-accepted stage constructions):**
  (S6b′-SUPP) §5.4 — existence/recursion re-derived, support by construction; the
  statement quantifies over stages of coherent towers built by the accepted §B2-DEF
  transitions (the same scope as T-1).
* **Refutations:** §5.1 (semantic countermodel at (3, 𝔽₉); stage-instance
  ...
  compiled-instance build named as follow-up falsifier); §5.2 (recorded-lift form;
  complete); §5.5 (conditional on fence lift; arithmetic machine-checked).
* **Not done here:** Lean edits (per charge); the compiled 𝔽₉ Stage instances; the
  formalization of Corollary 4 (recipe §4.2).
* **Known/new labels:** Theorem 1 — new observation (elementary, load-bearing).
  Theorem 3 — NEW theorem (the leaf's main open flank; closes it positively, upsetting
  the item-13 expectation that a countermodel or a JetSetup re-adjudication was live).
  Theorem 5 refutation — NEW countermodel. (S6b′-SUPP) — content of the dual-accepted
  T-1/(S6b′) construction with the support clause made explicit: label (b), new
  packaging/explicitation of accepted material, plus the NEW tightness observation.
* **Predictions falsifiable now:** (P1) the LIVE flank countermodel search returns
  ...
  succeeds and empties the singleton-(CR2) fiber at ν₁.

## 7. Consumed tree nodes
...
gates, U6 — none load-bearing). No other leaf is consumed; in particular NOTHING here
depends on O-1/O-2/O-3, so this unit sits at the top of the §D1.6 critical path as
required (V6 finding 22 honored: O-14a′ before O-3).
