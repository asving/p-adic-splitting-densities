...
**Date:** 2026-08-12.  
**Status:** ACCEPTED note of record — the hostile arc is closed at 2/2 and the accepted text is byte-frozen; THE ACCEPTANCE RECORD AT THE END OF THIS NOTE GOVERNS (before acceptance the governing sections were the highest-numbered r-section and the post-pass rider blocks); the pass record, the folded riders, the pin-sweep result, the counter state, the four standing conditionalities, and the `(GRADE)` cap that still holds the S3.1/S3.2 composed instances at HE3's HEAD grade are recorded in that block.
...
## Post-pass rider (2026-08-13, after the r9 clean-w-minor sol pass)

**Dated rider (2026-08-13 — the single minor of the gpt-5.6-sol high hostile pass on the r9 effective text, `runs/comp13/T2SOL2_output.log`, returning 0 CRITICAL + 0 GAP + 1 minor. Folded per the campaign's minors-only precedent — T4's post-pass gloss-direction rider, T5's two post-pass-1 locator riders, and T1's five post-pass-1 riders, each folded on a 0C + 0G pass and each subsequently confirmed to 2/2. This note carries no earlier rider precedent, every previous round having been gap-or-worse; the T1 acceptance-fold rider style is followed here. NO r-section, no grade event, NO counter reset.)** The rider QUALIFIES one display and does nothing else. It attaches a lift-choice clause to (IDX-TWIST) at the point of display, and by that clause scopes the two sentences that restate the display as an identity of SETS. No display of the statement suite S1.2–S1.8 is amended, no hypothesis is added to or removed from HE7.A's suite, no pin, fence, obligation, consumption check or source line is touched, and no new source consumption is created: the expected pin count stays 32, the expected synthetic-marker count stays zero, and the expected source edit count stays zero. In particular the level-one (RISE) discharge is UNCHANGED, and the qualifier cannot change it: that discharge consults neither the family's print label nor the recipe's lift, and its own SECOND rider already closes exactly the freedom this finding names. The pass's verdict slot reads CLEAN with one minor; the campaign precedent above governs the fold, and the counter statement at the end of this block is the operative one.

**Landmark convention (rider).** Counts are taken over the note AS IT STOOD BEFORE THIS BLOCK — lines 1–3805, i.e. through the last line of the r9 counter — because each landmark is necessarily quoted a second time here. Each master-internal count was taken on 2026-08-13 in the form

```
sed -n '1,3805p' T2_SIGMA_LADDER_MASTER_2026-08-12.md | grep -cF -- '<string>'
```

with `grep -cFx` used for the one landmark marked "whole line". Source lines are counted with `grep -cFx` in the named source file, as r9's verification protocol requires.

### Rider 1 (m-1SOL2) — the lift-choice qualifier on (IDX-TWIST)

**The finding folded.**

> HE6 says only: "Choose, by (LIFT), \(B_t\) …" with prescribed height and print residue. The seam correctly implies that \(\Lambda=-B_0\) has coherent digit \(\sigma\eta^{q(\kappa)}\), but not literally that \(\Lambda\) equals the carrier's designated \(L_\kappa(\sigma\eta^{q(\kappa)})\). Thus \(\Psi_{\kappa,Z-\sigma}=\Phi'-L_\kappa(\sigma\eta^{q(\kappa)})\) is exact only after a compatible choice \(B_0=-L_\kappa(\sigma\eta^{q(\kappa)})\). For an arbitrary permitted HE6 choice, the two increments differ by height \(>\kappa\). … The subsequent \(\Lambda\)-based argument explicitly handles this lift freedom and transfers `(RISE)` to the designated lift, so the instance discharge remains sound.

**Accepted, and it is a qualifier on a display, not a repair of an argument.** The finding is right about DEFINITION HE6-1: line 204 constrains \(B_0\) by \(\deg B_0<D'\), \(dv(B_0)=(d-t)u=u=\kappa\) at \((\ell,d)=(1,1)\), \(t=0\), and \(\operatorname{res}(B_0(\theta)/\varpi(\theta)^{\kappa})=c_0=-\sigma\) — a height and a print residue, and nothing more — while the master's \(L_\kappa(c)\) is the carrier's DESIGNATED lift at a height and a digit (S1.4's `(LIFT)`: \(\deg L_k(c)<D\), \(h(L_k(c))=k\), \(\operatorname{dig}(L_k(c))=c\)). What r9 repair 1(a) actually derives from the seam is therefore the PAIR \(h(-B_0)=\kappa\), \(\operatorname{dig}(-B_0)=\sigma\eta^{q(\kappa)}\), which is choice-independent; the boxed equality states one representative of that pair, namely the designated one. The cure states which representative, at the display.

**Cure.** In r9 repair 1(a)'s replacement text, immediately after the boxed (IDX-TWIST) display — that is, after the whole line `> \tag{IDX-TWIST}` and the `> \]` that closes it — and immediately before the paragraph beginning "The print label and the coherent digit are the same element of \(K\) only when", insert, as a new paragraph of the same blockquote:

> **Which lift the equality is read at.** (IDX-TWIST) is an equality of polynomials AT the compatible choice \(B_0=-L_\kappa(\sigma\eta^{q(\kappa)})\), and that choice is one DEFINITION HE6-1 permits: \(-L_\kappa(\sigma\eta^{q(\kappa)})\) has \(\deg<D=D'\) and \(h=\kappa\), hence \(d(B_0)=\kappa\), by S1.4's `(LIFT)`, and its print residue is \(\gamma_\kappa\bigl(-L_\kappa(\sigma\eta^{q(\kappa)})\bigr)\eta^{-q(\kappa)}=-\sigma\eta^{q(\kappa)}\eta^{-q(\kappa)}=-\sigma=c_0\) by the same `(HE6-SEAM)` read at \(\xi=\theta\) used just above — so the recipe's three conditions at line 204 are met, and the master's own recentering \(\Phi'-L_\kappa(\sigma\eta^{q(\kappa)})\) IS a member of the family at the label \(r=Z-\sigma\). For an ARBITRARY (LIFT)-permitted choice, line 204 fixes only the height and the print residue: two permitted \(B_0\) differ by an \(E\) with \(\deg E<D'\) and \(d(E)\ge\kappa\) whose digit at \(\kappa\) is \(\gamma_\kappa(B_0)-\gamma_\kappa(B_0')=0\) — equal print residues and one and the same conversion factor \(\eta^{-q(\kappa)}\), with `(GAMMA)` additive in the coefficient array at a fixed height — so \(E=0\) or \(h(E)>\kappa\), S1.2 giving a nonzero slot a digit in \(K^\times\). Read at an arbitrary permitted choice, (IDX-TWIST) therefore holds MODULO increments of height \(>\kappa\), and what is choice-independent is exactly the pair \(h(-B_0)=\kappa\), \(\operatorname{dig}(-B_0)=\sigma\eta^{q(\kappa)}\) that the seam read above produced. Nothing below needs more, and the difference is absorbed where it arises: the designated-lift transfer of (b) below, run at \(\Lambda=-B_0\), sends \(E=\Lambda-L_\kappa(\operatorname{dig}\Lambda)\) to \(E=0\) or \(h(E)>\kappa\) and then \(d(E(\rho))>\kappa\) by `(SLOT-V)`, so both clauses pass between the increment a construction actually formed and the carrier's designated recentering at the same height and digit. Accordingly the two directions of the identification below are to be read with this qualifier attached. The COVERING direction — every nonempty recentering \(\Phi'-L_\kappa(c)\) of this master at a full height \(\kappa\) is a member of the family, at the label \(r=Z-c\eta^{-q(\kappa)}\) and the permitted choice \(B_0=-L_\kappa(c)\) — is EXACT, by the permittedness just checked, and it is the direction S1.8C consumes. The CONVERSE reading — that an arbitrarily chosen member is literally the designated recentering — holds up to an increment of height \(>\kappa\), which is to say it holds exactly in the height-and-digit data that every consumer of it reads.

**The other three "member for member" sites, classified; none is amended.** The string `member for member` occurs four times in the note before this block, on lines 3677, 3709, 3755 and 3797, and each is now covered. Line 3677 (the r9 round preamble, "the family still covers every recentering of this master, member for member") and line 3755 (S1.8C's replacement sentence, "the family covers every recentering of this master at that height, member for member") both state the COVERING direction, which the qualifier certifies as exact; they stand unchanged. Lines 3709 and 3797 state the identity of SETS ("the family IS the set of this master's nonempty recenterings …"), and they are the two sentences the qualifier's closing clause scopes: exact at the compatible choice, and up to an increment of height \(>\kappa\) at an arbitrary permitted one. Their purpose — that the relabelling by the fixed unit \(\eta^{q(\kappa)}\) loses nothing, so no recentering of this master falls outside the family — is carried entirely by the covering direction and is untouched.

**S1.8C is exact as it stands, and the permittedness check is what makes it so.** S1.8C's replacement sentence names its lift explicitly ("the member it is, is \(\Psi_{\lambda,\,Z-s\eta^{-q(\lambda)}}=\Phi'+B_0\) with \(B_0=-L_\lambda(s)\)"), i.e. it is written AT the compatible choice, with \(\sigma=s\eta^{-q(\lambda)}\) and \(\sigma\eta^{q(\lambda)}=s\). What that sentence needs, and what the r9 text left implicit, is that \(B_0=-L_\lambda(s)\) is a choice line 204 permits — the print-residue computation displayed in the cure, which returns \(c_0=-s\eta^{-q(\lambda)}=-\sigma\). With that check recorded the sentence is exact as written and is NOT amended.

**No consumer's mathematics changes.** (IDX-TWIST) is consumed at four sites downstream of its display — line 3739 ("the family's print label at that same member is \(s\eta^{-q(\lambda)}\)"), line 3747 ("its print label being \(s\eta^{-q(\lambda)}\) and not \(s\)"), line 3755 (S1.8C) and line 3759 ("the index a member carries is exactly what (IDX-TWIST) computes") — and every one of them consumes the LABEL–DIGIT correspondence \(\sigma\leftrightarrow\sigma\eta^{q(\kappa)}\), which is choice-independent, not the polynomial equality. The (RISE) discharge itself asserts no height, no digit and no label of the exhibited member before deriving them from the passage's own rise, and its SECOND rider closes the lift freedom between the instance's increment and the designated lift; the qualifier restates that closure at the display rather than adding to it. The two named open obligations, `(LB1)` and `(MP1)`, are unchanged, as is the status of (RISE) as a stipulated carrier hypothesis discharged per instance.

**Landmarks.** Under the convention above, on 2026-08-13: `> \tag{IDX-TWIST}` — **1**, whole line, under `grep -cFx`; the insertion's following-paragraph delimiter `The print label and the coherent digit are the same element of \(K\) only when` — **1**; `So at each index \(\kappa\) the family IS the set of this master's nonempty recenterings at height \(\kappa\), member for member, under (IDX-TWIST)` — **1** (line 3709); `so at each height the family IS the set of this master's nonempty recenterings, member for member, and only the index NAMES differ` — **1** (line 3797); `so the member \(\Psi_{\kappa,Z-\sigma}\) is \(\Phi'-L_\kappa(\sigma\eta^{q(\kappa)})\) and not \(\Phi'-L_\kappa(\sigma)\)` — **1** (line 3797); `the member it is, is \(\Psi_{\lambda,\,Z-s\eta^{-q(\lambda)}}=\Phi'+B_0\) with \(B_0=-L_\lambda(s)\)` — **1** (line 3755, S1.8C, unamended); `SECOND, the lift freedom is immaterial.` — **1** (line 3739, the transfer the qualifier points to). Census of the two governed strings: `member for member` occurs **4** times, on lines 3677, 3709, 3755 and 3797, all four classified above; `\Psi_{\kappa,Z-\sigma}` occurs **3** times, on lines 3695 and 3699 (the lead-in and the boxed display) and 3797 (the counter's restatement). Source: `HE6_PROOF_2026-08-08.md` line 204, re-verified at **1** under `grep -cFx`, inside the `HE6-FAMILY` span (lines 201–213) — the same line r9 repair 1(a) already consumes, so this rider adds no source consumption and no pin.
...
## Post-pass rider 2 (2026-08-13, after the r11 clean-w-minor sol pass)

**Dated rider (2026-08-13 — the single minor of the gpt-5.6-sol high hostile pass on the r11 effective text, `runs/comp13/T2SOL4_output.log`, returning 0 CRITICAL + 0 GAP + 1 minor. Folded per the campaign's minors-only precedent — T4's post-pass gloss-direction rider, T5's two post-pass-1 locator riders, T1's five post-pass-1 riders, and this note's own first post-pass rider block above, each folded on a 0 CRITICAL + 0 GAP pass and none of them a grade event. NO r-section, no grade event, NO counter reset.)** The rider WIDENS one census sentence and does nothing else. It replaces r11 repair 1(c)'s closing census sentence "Those are the only two sites." by a sentence that keeps the two DIRECT citation sites and adds the TRANSITIVE consumption route the pass found — `HE6-ACCOUNT`'s proof text, which invokes LEMMA HE6-2 at arbitrary roots twice. No display of the statement suite S1.2–S1.8 is amended, no hypothesis is added to or removed from HE7.A's suite, no pin is added or removed, no fence, obligation or Source-pin line is touched, and no new source consumption is created — the two source lines the widened census names, HE6:545 and HE6:589, both lie inside the already-pinned span `HE6-ACCOUNT` [525,604], and 545 is already quoted verbatim by r10 repair 1(a) item 8. The expected pin count stays **36**, the expected synthetic-marker count stays zero, and the expected source edit count stays zero. In particular no mathematical conclusion of this master changes: what the widened census records is an additional route by which an already-pinned reading directive is consumed, not an additional source unit that is consumed. The pass's verdict slot reads CLEAN with one minor; the campaign precedent above governs the fold, and the counter statement at the end of this block is the operative one.

**Landmark convention (rider).** Counts are taken over the note AS IT STOOD BEFORE THIS BLOCK — lines 1–4162, i.e. through the last line of the r11 counter — because each landmark is necessarily quoted a second time here. Each master-internal count was taken on 2026-08-13 in the form

```
sed -n '1,4162p' T2_SIGMA_LADDER_MASTER_2026-08-12.md | grep -cF -- '<string>'
```

Source lines are counted with `grep -cFx` in `lean/notes/openmath/HE6_PROOF_2026-08-08.md`, as r11's verification protocol requires.

### Rider 2.1 (m-1SOL4) — the consuming-site census widened to the transitive route through `HE6-ACCOUNT`

**The finding folded.**

> **m-1 — the "exactly two consuming sites" claim is too narrow.** r11 says HE6-3/HE6-4's use of the correction "is the source's business and is not consumed here." But `HE6-ACCOUNT` is explicitly consumed as the pinned proof of HE6-3 and HE6-4. Its proof invokes HE6-2 for arbitrary roots at HE6:545 and again in HE6-4 at line 589. Those invocations require the corrected piecewise reading when \(\delta\ne\kappa\). Thus there is at least one additional transitive consumption route. This is minor because `HE6-BETA-GUARD` is pinned and present on all five effective Source-pin lines; no source coverage or mathematical conclusion is missing.

**Accepted, and it is a census widening, not a repair of an argument.** The finding is right that r11's sweep enumerated only the sites at which this master NAMES the correction, and that naming and consuming are not the same relation — the whole ground on which r11 itself pinned `HE6-BETA-GUARD` was that a directive the reader is told to obey is consumed material whether or not it is cited. The same test applied one step further reaches the route the pass names: this master consumes `HE6-ACCOUNT`'s proof text, that text invokes LEMMA HE6-2 at roots off the label class, and the corrected guard is what makes those invocations well posed. The finding is also right that nothing else moves: the route lands on pins and Source-pin lines that already carry `HE6-BETA-GUARD`, so the cure is confined to the census sentence.

**Cure.** In r11 repair 1(c), replace the census sentence

> Those are the only two sites.

by

> Those are the only two sites at which this master CITES the correction by name; they are not the only routes by which it is CONSUMED. One further route is TRANSITIVE, through the pinned span `HE6-ACCOUNT` (HE6 §S5 [525,604], the proof unit of LEMMA HE6-3 and LEMMA HE6-4), whose text invokes LEMMA HE6-2 at ARBITRARY roots at exactly two places. The first is `* ρ-side: by LEMMA HE6-2 (with r₀ never a label's factor)` (545, `grep -cFx` → **1**), the head of the three-line passage that r10 repair 1(a) item 8 quotes verbatim as the root-to-slope regrouping carrying this master's `(ACCOUNT)`, and that item 9 names again as the \(\rho\)-side evaluation inside the instance's pinned proof. The second is `* ρ-side: by LEMMA HE6-2, Σ_ρ dv(Ψ(ρ)) = ℓd_r·Σ_λ′ n_λ′ min(λ′,λ) +` (589, `grep -cFx` → **1**), inside the proof of LEMMA HE6-4 that r10 repair 1(a) item 3 consumes for S1.7 clause 3. Both run over roots \(\rho\) of \(f\) whose \(\delta=\lambda_\rho\) is in general \(\ne\kappa\), and both read the piecewise value \(\ell d\cdot\min(\delta,\kappa)\) there; that reading is well posed only under the corrected guard, which at \(\delta\ne\kappa\) yields \(\varepsilon(\rho)=0\) from the forward half of the dichotomy and names no \(\beta\) at all, whereas the guard as originally written would name at those very roots a \(\beta\) that is undefined. So the census reads: TWO direct citation sites, plus a transitive route at the two source lines 545 and 589 inside `HE6-ACCOUNT`. Coverage is unaffected, and nothing outside this sentence changes on its account: `HE6-BETA-GUARD` is pinned by (b), and by (e) it stands on all five Source-pin lines — the same five that already carry `HE6-ACCOUNT` — so wherever the transitive route runs, the guard is already declared alongside the span that runs it.

**The pin note at (b) is not amended, and the distinction that lets it stand.** Its closing sentence — `The unit's remaining content, the source's own site check over HE6-3, HE6-4 and THEOREM HE6.A, is the source's business and is not consumed here.` — is about the source's own VERDICT on its downstream sites (`passPE4's site check, HE6-3/HE6-4/THEOREM HE6.A read ε through that`, HE6:1044, `grep -cFx` → **1**), not about the corrected guard itself. That verdict remains unconsumed, because this master does not need it: at \(\delta\ne\kappa\) the value \(\varepsilon(\rho)=0\) used at 545 and 589 is the forward half of LEMMA HE6-2's own dichotomy and names no \(\beta\), and at \(\delta=\kappa\) the guard puts \(\beta\) in the unit case, where the source's parenthetical at 545 — \(r_0\) never a label's factor, hence \(\iota_\rho(r_0)(\beta_\rho)\ne0\) — closes it. What the master consumes is the guard, at (b)'s "exactly one thing", and the widened census above is the complete record of where. The third name in that sentence's list, THEOREM HE6.A, adds no route: this master's only consumption of HE6.A's proof is item 1 at HE6:644–648 inside `HE6-PROOF`, which takes \(\beta_\rho\) from COROLLARY HE6-2″ (`HE6-ROOT-LABEL`) at the root's own slope — r11 (c)'s unit case, where \(\beta\) is defined without appeal to LEMMA HE6-2's guard — and cites LEMMA HE6-3, HE6-4 and HE6-5 by conclusion only.

**Nothing downstream of the census moves.** The conclusion this master takes from LEMMA HE6-2 is byte-unchanged under the correction, exactly as r11 (a) and the r11 counter say, and that is as true at the transitive route as at the two direct sites: the displayed value \(dv(\Psi_\kappa(\rho))=\ell d_0\cdot\min(\lambda_\rho,\kappa)\) at 546 and the class decomposition at 589–591 read the same before and after the re-scoping, the correction supplying the hypothesis under which they are well posed rather than a new fact. So r11 (d)'s bookkeeping stands unchanged — the pin count stays 36 with the same \(19+4+13\) taxonomy, and the cured-consumption census stays at nine, this rider curing no consumption but recording a further route to one already cured — and r11 (e) and (f) stand unchanged, (f)'s enumeration of the six HE6 dated appends included. The two named open obligations, `(LB1)` and `(MP1)`, are unchanged, as is the status of (RISE) as a stipulated carrier hypothesis discharged per instance.

**Landmarks.** Under the convention above, on 2026-08-13: the amended census sentence `Those are the only two sites.` — **1** (line 4088, inside r11 repair 1(c)); the (b) pin note's closing sentence `The unit's remaining content, the source's own site check over HE6-3, HE6-4 and THEOREM HE6.A, is the source's business and is not consumed here.` — **1** (line 4070, classified above and NOT amended); the two consuming-item tails the widened census points to, `with the proof of LEMMA HE6-4 that runs on it.` — **1** (line 3871, item 3) and `the two evaluations at the bullets` — **1** (line 3877, item 9); and the source line item 8 already quotes, `* ρ-side: by LEMMA HE6-2 (with r₀ never a label's factor)` — **1** (line 3876). Source, `grep -cFx` in `HE6_PROOF_2026-08-08.md`: line 545 — **1**; line 589 — **1**; line 1044 — **1**. Lines 545 and 589 both lie strictly inside `HE6-ACCOUNT` [525,604], whose delimiters 525 and 604 r10 repair 1(b) and the r11 sol pass have each verified at **1**; line 1044 lies strictly inside `HE6-BETA-GUARD` [1036,1047]. No source file is edited, no pin is added, and no `T2V2PIN` line is created.
...
## Post-pass rider 3 (2026-08-13, after the r12 clean pair — the Opus/Claude confirmation pass's single minor)

**Dated rider (2026-08-13 — the single minor of the Opus/Claude confirmation hostile pass on the r12 effective text, `runs/comp13/T2P10_output.log`, returning 0 CRITICAL + 0 GAP + 1 minor, taken after the gpt-5.6-sol high hostile pass on the same effective text, `runs/comp13/T2SOL5_output.log`, returned 0 CRITICAL + 0 GAP + 0 minors. Folded per the campaign's minors-only precedent — T4's post-pass gloss-direction rider, T5's two post-pass-1 locator riders, T1's five post-pass-1 riders and its two final riders, and this note's own two post-pass rider blocks above, each folded on a 0 CRITICAL + 0 GAP pass and none of them a grade event. NO r-section, no grade event, NO counter reset.)** The rider RE-GROUNDS one disposition sentence and does nothing else. It replaces the closing clause of r12 repair 1(g)'s HE7:2478 sweep entry — which grounds the disposition of rider M-B on the claim that this master NAMES neither of the two units M-B is a provenance rider on — by a clause that grounds the same disposition on CONSUMPTION, the relation the gate actually tracks. No display of the statement suite S1.2–S1.8 is amended, no hypothesis is added to or removed from HE7.A's suite, no pin is added or removed, no span, fence, obligation, Source-pin line or consumption check is touched, and no new source consumption is created; no source file is quoted or edited by this rider at all. The expected pin count stays **37** with taxonomy \(20+4+13\), the expected synthetic-marker count stays zero, the expected source edit count stays zero, and the cured-consumption census stays at ten. In particular the DISPOSITION is unchanged and no mathematical conclusion of this master moves: M-B is not consumed either way, and what the rider corrects is the ground on which r12's own sweep record says so. The pass's verdict slot reads CLEAN with one minor; the campaign precedent above governs the fold, and the counter statement at the end of this block is the operative one.

**Landmark convention (rider).** Counts are taken over the note AS IT STOOD BEFORE THIS BLOCK — lines 1–4439, i.e. through the last line of the r12 counter — because each landmark is necessarily quoted a second time here. Each master-internal count was taken on 2026-08-13 in the form

```
sed -n '1,4439p' T2_SIGMA_LADDER_MASTER_2026-08-12.md | grep -cF -- '<string>'
```

No source line is quoted by this rider, so no `grep -cFx` source count is owed.

### Rider 3.1 (m-1P10) — r12 repair 1(g)'s HE7:2478 disposition re-grounded on consumption

**The finding folded.**

> **m-1 (MINOR, on r12's own text) — r12 repair 1(g)'s ground for the HE7:2478 disposition is false as written.** The text: "M-B is a provenance rider on THEOREM HE7.D's §S2 clause and COROLLARY HE7.B(ii), **and this master names neither.**" Why it is false: "The master names both, verbatim, at line 2136 (r3 repair 1(b)) … `grep -n 'HE7\.D\|HE7\.B'` over the master returns exactly two lines: 2136 and 4333 itself. The sentence is wrong in the note's **own vocabulary**: the standing S7 check r4 repair 13 installed is worded 'every source display the master text **quotes, names, or leans on**', so 'names' is a relation the gate tracks, and the master bears it to both units at 2136." Why the disposition nevertheless stands, so that this is a minor and not a gap: "the operative test is consumption, as r12's own restated S5 item 3 says in terms ('Consumption, not citation, is the gated relation'). The 2136 naming is a unit-identification parenthetical describing what the pinned endpoint `ANNEX-COROLLARY R1-d` re-proves — it is drawn from HE7's own R1 heading (HE7:1990) and leans on neither THEOREM HE7.D's §S2 clause nor COROLLARY HE7.B(ii)'s inductive-step clause. Obeying M-B … changes nothing the master reads: the master reads the cocycle chain for `(COC-DEF)`/`(BETA)`, and R1-b, R1-c and R1-d all lie inside `HE7-COCYCLE-2P-THROUGH-R1D` [2013, 2253]. So M-B is not consumed — on the consumption ground, not on the naming ground." Classification: MINOR, "of the bookkeeping-on-r12's-own-text kind. It is one sentence in this round's own new sweep record; nothing depends on it … No display, hypothesis, pin, fence or obligation moves."

**Accepted, and it is a re-grounding, not a change of disposition.** The finding is right on both legs. On the first, the master does name both units, once, at line 2136, in r3 repair 1(b)'s identification of `HE7-COCYCLE-2P-THROUGH-R1D`'s endpoint — and naming is a relation the standing S7 consumption check tracks by its own wording, so a sweep entry that denies the naming is false in this note's vocabulary. On the second, the disposition it grounds is nonetheless correct, because the gated relation is consumption and not naming: r12 repair 3's own restated S5 item 3 says so in terms, and it is that same restatement that creates the unit-identification / boundary-fixing class into which the 2136 parenthetical falls. The cure is therefore confined to the ground clause of one bullet in r12's sweep record; the entry's verdict — M-B not consumed — is unchanged, and so is everything downstream of it.

**Cure.** In r12 repair 1(g)'s HE7:2478 sweep entry, replace the closing clause

> M-B is a provenance rider on THEOREM HE7.D's §S2 clause and COROLLARY HE7.B(ii), and this master names neither.
...
**Landmarks.** Under the convention above, on 2026-08-13: the amended clause `M-B is a provenance rider on THEOREM HE7.D's §S2 clause and COROLLARY HE7.B(ii), and this master names neither.` — **1** (line 4333); its enclosing bullet head `- HE7:2478 (post-annex-acceptance, riders M-1, M-A, M-B)` — **1** (line 4333); the naming site the cure now cites, `(THEOREM HE7.D and COROLLARY HE7.B(ii), re-proved)` — **1** (line 2136, inside r3 repair 1(b)); and the governing sentence of the restated S5 item 3, `Consumption, not citation, is the gated relation` — **1** (line 4381, inside r12 repair 3). Independently, `grep -n 'HE7\.D\|HE7\.B'` over the whole note returns exactly two lines, 2136 and 4333, which is the finding's own census and is what makes the cure's "exactly once" exact. No source file is quoted or edited, no pin is added, and no `T2V2PIN` line is created.

### Counter state (SET BY THIS BLOCK)

This rider folds the single MINOR of a hostile pass that returned **0 CRITICAL + 0 GAP + 1 minor**, taken over the same r12 effective text on which the immediately preceding hostile pass returned **0 CRITICAL + 0 GAP + 0 minors**. S7's reset rule is stated of gap-or-worse repairs only ("A gap-or-worse repair resets the clean count"), and a minors-only pass is a clean pass whose minors fold as post-pass riders (T4, T5, T1, and this note's own two earlier rider blocks). Folding riders is not a repair round: the clean-pass counter is NOT reset, and with these two passes it **STANDS AT 2 OF 2**. The two passes are on model-diverse arms and both are fresh-context passes over the r12 effective text. The S7 acceptance gate — two consecutive clean hostile passes over one effective text — is therefore MET, and the ACCEPTANCE RECORD below is the operative record.

---

## ACCEPTANCE RECORD (2026-08-13): T2 v2 ACCEPTED 2/2 (model-diverse pair)
...
Per the campaign's minors-only precedent (T4's gloss-direction rider, T5's two locator riders, T1's five plus its two final riders, and this note's own two earlier rider blocks), a 0 CRITICAL + 0 GAP pass is a clean pass and its minor folds as a post-pass rider without a grade event or a counter reset. **The clean-pass counter reaches 2 OF 2**, on model-diverse arms, over one effective text.

**One rider postdates both passes.** Post-pass rider 3 above, folding pass 2's single minor, is wording-grade and one sentence long: it re-grounds one clause of r12's own dated-append sweep record on consumption rather than on naming, changing no display, hypothesis, pin, span, fence, obligation or count, and leaving the disposition it grounds unchanged. Like T1's final riders it has not been read by a third fresh context; unlike them it touches only this note's internal bookkeeping about itself, quotes no source, and could be deleted without altering a single claim of the mathematics.
...
## Dated consumption record (2026-08-14) — HE6's four A3 corrections adjudicated against this master's HE6 consumption

**Dated append (2026-08-14 — a CONSUMPTION RECORD, raised by `spec/EFF-HE6.md` OPEN-CALL 2. That compilation observes that HE6's four 2026-08-12 A3 corrections — F-1 at HE6:1061–1113, F-2 at 1115–1154, F-3 at 1156–1186, F-4 at 1188–1204 — all lie OUTSIDE every one of this master's nine HE6 pin spans; that two of them correct heavily-pinned text; and that F-1 declares the slot residue displayed inside `HE6-SLOT-SEAM` [215,238] VACUOUS, a span this master cites 48 times. It could not determine from HE6's side whether this master's reading predates the A3 fold, and left the remedy — two new pins, two widened spans, or a record that the consumption misses the corrected clauses — as a decision for this note's owner. This block records the answer. NO r-section, no grade event, no counter reset, and NO supersession: not one sentence of the accepted text is withdrawn or replaced. No display of the statement suite S1.2–S1.8 is amended, no hypothesis is added to or removed from HE7.A's clause suite, no case, conclusion or truth value moves, no pin is added or removed, no span, fence, obligation, Source-pin line or consumption check is touched, no new source consumption is created, and no source file is edited. The expected pin count stays **37** with taxonomy \(20+4+13\), the expected synthetic-marker count stays zero, the expected source edit count stays zero, and the cured-consumption census stays at **ten**. Nothing below is new mathematics; it collects in one place an adjudication this note already carries across r4, r10, r11 and its acceptance pass of record, so that a consumer who meets HE6's dated appends need not re-derive it.)**

**(a) The four corrections, one by one, against what this master consumes.**

- **F-1** (HE6:1061, `**Dated correction (2026-08-12, post-acceptance challenge F-1 — normalized slot coefficients in LEMMA HE6-0″).** The displayed definition`) — the one sharp case. It rewrites the slot residue of LEMMA HE6-0″ **inside** `HE6-SLOT-SEAM`: the span's bare display, `> where γ = Σ_t res(a_{i₀+e₁t})η_θ^{t} ∈ K is the level-1 slot residue at θ` (HE6:222), is replaced by \(\gamma_k(A)\) carrying the coefficient normalizer \(\pi^{-(k-(i+e_1t)h)/e_1}\), and the span's conclusion by \(\operatorname{res}(A(\xi)/\varpi(\xi)^k)=\iota_\xi(\gamma_k(A))\,\eta_\xi^{-q(k)}\). **This DOES change what the pinned span asserts** — it is not proof-internal material the master re-derives, and it is not classifiable with F-2/F-3/F-4 on that ground. **The master is immune because its own form of the clause IS the corrected form.** F-1's \(\gamma_k(A)\) is letter-for-letter S1.2's `(GAMMA)` and F-1's corrected conclusion is letter-for-letter S1.2's `(HE6-SEAM)`, agreeing in the index convention (\(0\le i<e_1\) with \(ih\equiv k\bmod e_1\)), the summation range (\(i+e_1t<D'\)), the normalizer exponent, the \(-q(k)\) sign, and the vanishing clause ("Terms strictly above height \(k\) contribute zero"). **The 2026-08-12 correction governs**, and obeying it changes not one character of what this master reads.
- **F-2** (HE6:1115) — replaces an inference inside LEMMA HE6-3(a)'s PROOF by a length argument. Proof-internal; the lemma's statement is unchanged and the append says so ("Part (b) and all consumers are unchanged"). Already dispositioned in the accepted text: r10 repair 1(b) records it as a reading directive at pin `HE6-ACCOUNT` — "**The 2026-08-12 correction governs.** Nothing this master consumes from the span is touched by it" — on the ground that the correction operates strictly below (†) at HE6:551 and that this master derives its own clause 1 from `(ACCOUNT)` in S2's proof rather than consuming HE6-3(a). r11 repair 1(f) re-affirms the declination to pin on the consumption test.
- **F-3** (HE6:1156) — reads §S3.1's "with equality exactly at the slots \(i=e_1t\)" as "equality possible only at". §S3.1 is the proof of LEMMA HE6-0, the disk criterion, which this master DECLINES outright: r10 repair 1(c) records that it "consumes no disk criterion", what S1.7 clause 1 reads as the intended-disk property being LEMMA HE6-1's own conclusion inside `HE6-TEST-PACKAGE`. Proof-internal to a declined unit; no assertion this master reads moves.
- **F-4** (HE6:1188) — reads §S6.1's "\(\varpi\in O[x]\)" as "\(\varpi\in K_0[x]\)", with \(g\)-fixed coefficients. LEMMA HE6-5's STATEMENT, inside `HE6-TEST-PACKAGE`, is untouched; only its proof is corrected, and the corrected reading is the one this master already declares carrier-side at S1.3 — "Integrality is required of the constructed lifts and test polynomials, not of the auxiliary functions used to read residues." The correction moves HE6 towards this master's frame, not away from it.

**(b) F-1's vacuity claim, re-derived here, with the regime in which it bites.** Let \(A=\sum_{j<D'}a_jx^j\) with \(a_j\in O\), let \(k=dv(A)=\min_j\{e_1v(a_j)+jh\}\), and let \(i_0\) be the attaining index, \(0\le i_0<e_1\) with \(i_0h\equiv k\bmod e_1\). At the attaining index the minimum is attained with equality, \(e_1v(a_{i_0})+i_0h=k\), so \(v(a_{i_0})=(k-i_0h)/e_1\), which is STRICTLY POSITIVE as soon as \(k>i_0h\); then \(\operatorname{res}(a_{i_0})=0\) and the span's bare sum loses the very term it is a display of. Sharper: every summand index \(j=i_0+e_1t<D'\) has \(jh\le(D'-1)h\), and \(e_1v(a_j)+jh\ge k\) forces \(v(a_j)\ge(k-jh)/e_1>0\) whenever \(k>(D'-1)h\). **So at every height \(k>(D'-1)h\) EVERY term of the bare sum vanishes and the bare display asserts \(\operatorname{res}(A(\xi)/\varpi(\xi)^k)=0\)** — against the same lemma's own \(dv(A(\xi))=k\) EXACTLY, which makes \(A(\xi)/\varpi(\xi)^k\) a unit with nonzero residue. The bare reading is therefore not merely lossy but false, and false precisely in HE6's consumed regime: DEFINITION HE6-1's standing \(\kappa>D'h>(D'-1)h\). F-1's "vacuous" is upheld as written. Why the defect survived inside the span for four rounds is visible in the span's own `[r2]` witness: at \((e_1,f_1,h)=(2,2,3)\), \(k=3\), \(A=x\) one has \(i_0=1\) and \(k-i_0h=0\), so the normalizer is \(\pi^0\) and bare \(\gamma=\gamma_k(A)=1\) — the witness sits in the coincidence regime \(k\le(D'-1)h=9\), where the two readings agree, and so cannot discriminate them. This master's consumption never enters that regime as a ground: it reads `(GAMMA)` with the normalizer written out, and S1.2 declares it non-optional — "The coefficient normalizers inside \(\gamma_k(A)\) and the negative sign on \(q(k)\) are mandatory."

**(c) The consumption sites, sampled across the rounds, and why each is immune.** Nine sites, chosen where the seam content does real work:

1. **S1.2**, the definitional consumption — `(GAMMA)` and `(HE6-SEAM)` as displayed above, i.e. the post-F-1 forms, with the mandatory-normalizer sentence attached. Immune: it IS the correction.
2. **S3.2**, `(HE6-INSTANCE)` — "Its coherent slot digit is exactly `(GAMMA)`", the print-frame read \(\operatorname{res}(A/\varpi^k)=\iota(\gamma_k(A))\eta^{-q(k)}\), and then the refusal in terms: "The bare sum of coefficient residues is not an admissible substitute." Immune: it refuses the pre-correction reading explicitly.
3. **S3.1's `(HE3-INSTANCE)` box** — \(\operatorname{dig}(A)=\gamma_{dv(A)}(A)\), the same corrected object.
4. **The standing pass-1 gate list** — "3. **HE6 seam gate.** Verify normalized coefficients in \(\gamma_k(A)\) and the exact factor \(\eta^{-q(k)}\)." The master's own gate demands F-1's form of the display.
5. **r4 repair 1**, the adjudication of this pin — it fixes what is consumed here to exactly two clauses, "the exact height law `(SLOT-V)`" and the \(\varpi\)-read conversion written with \(\gamma_k(A)\). F-1 leaves the height law untouched and corrects the conversion to the form already written.
6. **r4 repair 9**, the decisive leg — it derives the normalized reading from a DIFFERENT pinned span, quoting three grep-verified lines of LEMMA HE3-1L's proof inside `HE3-HE3-1L` that compute \(c_i\pi^{-(\kappa-(i_0+e_1t)h)/e_1}\), and it reaches F-1's own diagnosis independently: "Read literally at a slot with \(v(a)>0\) the bare residue is \(0\)". So the master's supplier for the corrected reading does not route through HE6-0″'s display at all, and the immunity is pin-covered rather than coincidental.
7. **S1.8C's discharge of `(MP1′)`** (r6 repair 2) — it quotes HE6:217–219, the bare display among them, but consumes only the law's KEY-FREENESS: "a law about an \(A\) with \(\deg A<D'\) and about a point \(\xi\), whose parameters are \(e_1\), \(h\), \(\varpi\), \(\eta\), \(\iota\) and \(\psi\), and in which no key occurs". F-1 adds one STAGE datum, \(\pi\), to that parameter list and no key, so conclusions (i)–(v) stand verbatim under the corrected display.
8. **r9 repair 1(b)**, the coherent-frame rebuild — the sharpest test, because it is where the seam does the most work. It consumes `(SLOT-V)`, the coherent label read from `HE3-A-PROOF`, and master-side properties of `(GAMMA)`: "`(GAMMA)`'s formula is additive in the coefficient array at a fixed height" together with the vanishing clause. All three survive F-1 unchanged, and the rebuild's deliberate move INTO the coherent frame means the print-frame residue enters only through the fixed unit \(\eta^{q(\lambda)}\).
9. **r9 repair 1(a)'s `(IDX-TWIST)` and 1(e)'s frame authority** — the print/coherent mismatch computed there is the \(\eta^{q(\kappa)}\) twist of `(HE6-SEAM)`, i.e. the span's `[r2]` SIGN leg, which F-1 leaves exactly as it stands; and 1(e) names the supplier "whose supplier pin is `HE6-SLOT-SEAM` (LEMMA HE6-0″, adjudicated at r4 repair 1 for exactly the two clauses" of item 5.

At no sampled site does a conclusion depend on the pre-correction reading, and at four of them the master refuses that reading in terms. The two remaining classes of citation — the Source-pin lines of S3.1–S3.5 and the negative claims that this pin is not asserted to prove `(LB1)` or `(MP1)` — consume nothing that F-1 touches.

**(d) The remedy OPEN-CALL 2 asks for already exists in this note, in three rounds and one pass.** (i) r10 repair 1(b) installs the F-2 directive at `HE6-ACCOUNT`, worded on the same model this note later used for `HE6-BETA-GUARD`, and declines the pin on the consumption test. (ii) r11 repair 1(f) enumerates the A3 fold by date — "PE4 Correction 1 (the §S9 headline) and the 2026-08-12 F-1 (LEMMA HE6-0″, inside `HE6-SLOT-SEAM`)", with F-3 and F-4 in the same sentence — and disposes all of them; the same round quotes the A3 fold's own heading at HE6:1051 as a boundary line, `grep -cFx`-verified. (iii) The acceptance pass 2 of record (`runs/comp13/T2P10_output.log`, Opus/Claude, 2026-08-13) re-attacked F-1 as "the hardest case, recorded in full because it is the one that could have been a GAP" at its §D.1a, and upheld the disposition on r4 repair 9's ground. **The question OPEN-CALL 2 could not settle is therefore settled: this master's reading does not predate the A3 fold and does not depend on it either — it reached the corrected display independently at r4 repair 9 and read the fold itself at r11.** No tenth HE6 pin is owed for F-1 under S5 item 3's governing sentence, "Consumption, not citation, is the gated relation", and none is added here; the two clauses this master consumes at `HE6-SLOT-SEAM` are stated in its own S1.2 displays, which are the corrected ones.

**(e) One precision residue, recorded and deliberately NOT cured.** r11 repair 1(f) groups F-1 with F-3, F-4 and PE4 Correction 1 as corrections that "touch nothing this master consumes". For F-3, F-4 and Correction 1 that is exact — they correct units this master declines or never reads. For F-1 it is loose in this note's own vocabulary: F-1 lands ON the clause this master consumes, the \(\varpi\)-read conversion, and the accurate statement is (a)'s — the master's form of that clause is already F-1's corrected form, with a pinned supplier at `HE3-HE3-1L` that does not route through the corrected display. The DISPOSITION is unchanged, nothing this master reads moves, and no conclusion is touched, so this is recorded and not cured, in the same class as the four non-counted nits of pass 2 of record, which the acceptance record above deliberately leaves uncured "so that later work need not re-raise them" — §F.3 of that pass records the sibling residue at r4 repair 9's "No pinned display is asserted to be wrong". A round that reopens the hostile arc for other reasons may cure both in one clause; nothing in this note is conditional on either.

**Landmark census (this block).** Counts are taken over the note AS IT STOOD BEFORE THIS BLOCK — lines 1–4507, i.e. through the last line of the ACCEPTANCE RECORD — because each landmark is necessarily quoted a second time here; each was taken on 2026-08-14 in the form `sed -n '1,4507p' T2_SIGMA_LADDER_MASTER_2026-08-12.md | grep -cF -- '<string>'`. Eleven master-internal landmark strings, on eleven lines, each returning **1**: S1.2's mandatory-normalizer sentence (249); S1.3's integrality sentence (269); S3.2's refusal of the bare sum (871); the pass-1 HE6 seam gate (1236); r4 repair 1's consumption sentence (2274); r4 repair 9's literal-reading sentence (2522); S1.8C's key-freeness clause (3059); r9 repair 1(b)'s additivity clause (3739); r9 repair 1(e)'s supplier clause (3757); r10 repair 1(b)'s reading directive (3914); and r11 repair 1(f)'s A3 enumeration (4142). Three further strings are quoted with their true counts rather than at 1: r10 repair 1(c)'s "consumes no disk criterion" returns **1** (3918); S1.2's vanishing clause "Terms strictly above height \(k\) contribute zero" returns **2** (231, the display's own sentence, and 3739, where r9 repair 1(b) quotes it); and S5 item 3's governing sentence "Consumption, not citation, is the gated relation" returns **4** (4381 in r12 repair 3, and 4459/4469/4471 in post-pass rider 3, which quotes the same restatement). Five source lines, each returning **1** under `grep -cFx` in `lean/notes/openmath/HE6_PROOF_2026-08-08.md` on 2026-08-14: the F-1 heading (1061), the F-3 heading (1156), the F-4 heading (1188), the A3 fold heading (1051), and the bare-\(\gamma\) line inside `HE6-SLOT-SEAM` (222). The F-2 heading (1115) is cited by number only, being already quoted and verified at r10 repair 1(b). No source file is edited, no pin is added, and no `T2V2PIN` line is created.
