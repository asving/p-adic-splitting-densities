...
> **[ANNEX 2026-08-08 #2, orchestrator, post-acceptance] The "numbering re-verification still owed" parenthetical on the FGMN cites is DISCHARGED at the local print (`docs/references/fgmn_residual_ideals_1305.0775v3_layout.txt`): Thm 4.2 @ 1028, Thm 4.8 @ 1136, Cor 2.7 @ 525, Cor 4.7 @ 1121, eq (14) @ 1004, Lemma 5.13 @ 1403 (the 2026-08-07 numbering-leg preliminaries) + **Cor 4.9 @ 1153 (this fold)**: the print states "Fᵢ₊₁ = Fᵢ[zᵢ] = F₀[z₀,…,zᵢ], deg ψᵢ = fᵢ"; this note's gloss "Δ/(ψ_m) ≅ K_{m+1}" is the composite of Thm 4.8 (Δ ≅ F_m[y], y ↦ y_m) + the print's §3.3 DEFINITION "ψᵢ ∈ Fᵢ[y], minimal polynomial of zᵢ over Fᵢ" (@ 862, giving F_m[y]/(ψ_m) ≅ F_m[z_m]) + Cor 4.9 (F_m[z_m] = F_{m+1}) — FAITHFUL as the pair-cite, no extra hypotheses. Caveat (extraction-decorrelation rule): verified against the local text extraction; the cited formulas' non-textual legs are the sealed batteries (R1-DMULT etc.). No text of this note is edited.**
...
>
> 1. **Cor 2.7 — printed p. 10 — VERIFIED.** Print: *"For non-zero g, h ∈ K[x], we have S_λ(gh) = S_λ(g) + S_λ(h)."* Its proof opens by displaying the equivalent component form *"s(gh) = s(g) + s(h), s′(gh) = s′(g) + s′(h), and u(gh) = u(g) + u(h)"* (for sides of the same slope or of length zero). This note's (IN-3)(e) renders it as *"the minimal on-line Φ_m-abscissa is additive: s(fg) = s(f)+s(g) [Cor 2.7]"* — the left-endpoint-abscissa component, and by Lemma 2.6(2) (`s(g) = ord_{μ′,φ}(g)`, same page) this is the ord-additivity the corpus §S6a quote names. §4's JC-SPAN consumption (L838–841: minimal on-line abscissa of φ_γφ_{γ′} is s_NR(γ)+s_NR(γ′)) uses exactly this component. Display-precision note, not drift: the s-additivity is a display inside Cor 2.7's proof rather than its headline segment identity; a maximally precise cite is "Cor 2.7 (component form) / Lemma 2.6(2)". Consumption survives unchanged.
> 2. **Thm 4.2 — printed p. 20 — VERIFIED.** Print: *"Let g ∈ K[x] be a non-zero polynomial and let α = μ(g). Then, H_μ(g) = φ_r(α)R_{r,α}(g)(y_r) = φ_r(g)R_r(g)(y_r). In particular, 𝒫_α(μ)/𝒫_α^+(μ) = φ_r(α)Δ is a free Δ-module of rank one."* This note's (IN-3)(c): *"gr_γ(w) is FREE OF RANK ONE over Δ ≅ K_m[y] on the anchor basis in(φ_γ) at attained γ [Thm 4.2]"*. The graded piece 𝒫_α(μ)/𝒫_α^+(μ) is gr_α, the rank-one freeness and the anchor generator both match, and α = μ(g) for non-zero g is the "attained γ" side condition. Two disclosed renderings, neither adding a hypothesis: (i) "Δ ≅ K_m[y]" imports Thm 4.8 (item 3), cited in the same block; (ii) the generator is printed as φ_r(α) := x_r^{s(α)}p_r^{u(α)} (Lemma 4.1, same page), which is associate to a power of in(φ_r) since x_r and H_{μ_r}(φ_r) are associate (Lemma 3.14, p. 17) and p_r ∈ 𝒢r(μ_r)^*; identifying it with THIS note's own anchor in(φ_γ) is the transport, already graded W-1 ATTEMPT and fenced in JC-BOX-3 [r1] item 1 — no undisclosed gap. Level renaming r ↔ m and K_i ↔ 𝔽_i is uniform throughout the note.
