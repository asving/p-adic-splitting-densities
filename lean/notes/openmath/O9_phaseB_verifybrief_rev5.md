# O9 — the order-r stratum census (CL-6 at OM order ≥ 2) — Phase-B verify brief REV 5 (Fable, 2026-07-31)
...
gap* (assume and continue); fix nothing. This is REV 5 = rev 4
(`O9_phaseB_verifybrief_rev4.md`) byte-stable except the two-site fix of
pass 4's single justification gap (`O9_pass4_verify.md`, fresh context,
2026-07-31: **SOUND — 0 critical, 1 gap**; Finding 1: the rev-4-withdrawn
"per-slot bijection of F_{r+1}" (GR-B) gloss survived verbatim at two
byte-carried, non-load-bearing SUMMARY sites — the §0 status-table
OL-A(iii) row and §1's (P5) pin; both now re-pointed at the LINE clause
with the onto ⟺ FULL-attainment qualifier, matching §5.1's actual
consumption). Rev 4 repaired the pass-3
hostile verification (`O9_pass3_verify.md`, fresh context, 2026-07-31:
UNSOUND — 1 critical, 0 gaps) of rev 3 (`O9_phaseB_verifybrief_rev3.md`);
the earlier chain: rev 3 repaired pass 2 of rev 2, rev 2 repaired pass 1 of
rev 0. Pass 3 explicitly VERIFIED, each **as displayed**: all four rev-3
repairs (the corrected value-attainment (ADM) with its LED(iii) derivation;
the automaticity rescope incl. the r = 1 value inequality re-derived by
hand; K6 clause by clause against its script and log; the CEN-J scope +
padding clause) and the entire census-layer record (§3 of the pass report:
logs byte-matched, the §6.4 source cite checked, K5's independent membership
layer confirmed). Its one CRITICAL: the attainment ladder goes ONE RUNG
DEEPER — value-attainment (dim G_{β_k} ≥ 1) is still strictly weaker than
what §5.1/§6.3/§6.4 consume (fiber-uniformity ONTO F_{r+1}, which by the
proved LED(ii)/(iii) forces FULL attainment dim G_{β_k} = d); machine
countermodel at a PARTIALLY attained slot: 73728 vs 49152 at q = 2. Rev 4
...
Rev 5 touches ONLY the §0 OL-A(iii) row and the §1 (P5) parenthetical
(pass-4 Finding 1); every other byte, §§2–11 included, is verbatim rev 4,
which pass 4 verified as displayed in full.

## 0-pre. Rev-5 delta, then the rev-4 changelog (the pass-3 finding → disposition → repair sites)

*Rev-5 delta (pass 4, `O9_pass4_verify.md`: SOUND — 0 critical, 1
justification gap).* Finding 1: the rev-4 withdrawal of the "per-slot
bijection of F_{r+1}" (GR-B) gloss (§5.1) had not been propagated into two
byte-carried, non-load-bearing summary sites, where the gloss survived
verbatim — and at the §0 row, read unqualified, is REFUTABLE by the proved
LED(ii)/(iii) at any partially attained slot. Repair (nothing else
touched): the §0 OL-A(iii) row's statement cell re-pointed at the LINE
clause with the onto ⟺ FULL-attainment qualifier (what §5.1 actually
consumes); the §1 (P5) parenthetical re-scoped to §5.1's onto-ness at
FULLY attained β, the withdrawn surjectivity clause's name removed. The
grep audit of the pass-4 report confirms these were the only two surviving
consumption-free sites; §§6–8 were already clean.

Pass-1/pass-2 history: all findings of both passes were repaired in revs
2–3, and pass 3 re-verified every rev-3 repair correct **as displayed**
(`O9_pass3_verify.md` §2) together with the full census layer (§3 there);
the rev-3 changelog table is superseded by that report and not repeated.
Pass 3's single finding is the third rung of one ladder: rev 2's (ADM) was
CLASS-attainment (pass-2 CRITICAL: class ⊊ value), rev 3's was
VALUE-attainment (pass-3 CRITICAL: value ⊊ full), rev 4's is FULL
attainment — the first form strong enough for what the count consumes.

| pass-3 finding | class | disposition | repair site |
|---|---|---|---|
| F1: the corrected (ADM) (value-attainment, G_{β_k} ≠ 0, i.e. dim G_{β_k} ≥ 1) is still strictly weaker than what §5.1/§6.3/§6.4 consume: fiber-uniformity of the digit read ONTO F_{r+1} forces dim G_{β_k} = d (its z = 0 fiber is {w > β_k}, of the PROVED LED(ii) size), and (ADM) admits PARTIALLY attained slots (0 < dim < d), where the read's image is a proper F_q-subspace — the identical phenomenon rev 3 itself recorded at LED-top(ii) for the sub-ledger. §5.1's "(GR-B)(2′)'s slot clause (bijection of F_{r+1})" gloss cannot rescue the count against the proved LED(ii)/(iii). Machine countermodel from the note's own LED-top(ii) type (r = 1, f₀ = 1, e₁ = 1, f₁ = 2, h₁ = 2; d = 2): datum (0,6)→(1,5), N = 7 — (ADM)-value HOLDS, CEN-W display 73728 vs truth 49152 at q = 2, ratio (q+1)/q per partial slot. Every prior VALUE gate ran at d = 1, where value = full | CRITICAL | **REPAIRED, per the pass-3 displayed direction** — (a) (ADM) restated at FULL attainment: dim_{F_q} G_{β_k} = s(β_k+1) − s(β_k) = d, still a p-free decidable lattice check (§2, with the (CLASS-d) max-weight reading); (b) the r = 1 decidable inequality re-derived for the FULL predicate: β_k ≥ h₁·((h₁^{−1}β_k mod e₁) + (f₁−1)e₁), collapsing to rev 3's value inequality exactly at f₁ = 1 (§2); (c) LED-top(ii)'s image-subspace phenomenon propagated to the FULL ledger at the consumption site: §5.1's surjectivity bullet restated — read injective on G_β (the (GR-B) line clause), image V_β of dim s(β+1) − s(β), fiber-uniform onto V_β always, onto F_{r+1} ⟺ FULL attainment; the "bijection of F_{r+1}" (GR-B) gloss WITHDRAWN; §6.3/§6.4 consumption lines re-pointed at FULL (§5.1, §6.3, §6.4 step 3); (d) the pass-3 countermodel imported as §2's second warning display AND the falsifier extended to it: NEW K7 evaluates truth = formula under the FULL predicate, including a d ≥ 2 POSITIVE-CONTROL cell ((ADM)-FULL holds: census = q^{14}(q²−1)² EXACT at p ∈ {2,3} — the first d ≥ 2 VALUE gate) and the countermodel's refutation of the rev-3 display (§10 K7); (e) automaticity re-scoped to the FULL predicate: r = 0 STILL automatic (dim G_β = f₀ = d on nonnegative on-line data — the pass-3 verifier's own note); r ≥ 1 the displayed decidable check (§2) | §2; §5.1; §6.3; §6.4 step 3; §0 row + headline; §10 K7; §11 |
| (pass-3 blast-radius note adopted) | — | NOT infected and carried unchanged, per the pass report: the whole r = 0 layer (M08 recovery, (E″) validation, K5), the unconditional lemma stock A1/TRI/LED/LED-top, TYPE-MARCH, (DET), §5.3, the K6 dichotomy, the CEN-J scope clause, and the census-layer record | — |

Statement-change disclosure (per the repo statement-fence): (ADM) is
restated in this rev — value-attainment → FULL attainment, a STRICTLY
STRONGER hypothesis (full ⇒ value ⇒ class), so every consumer is now
conditional on the stronger form; §5.1's surjectivity bullet is restated
(the (GR-B) "bijection of F_{r+1}" gloss withdrawn — it overstated (GR-B)
against the proved LED(ii)/(iii); what remains consumed of (GR-B) there is
the LINE clause only, which is weaker). Both are this unit's own displays
with no external consumer; the pass-3 verifier's repair direction demanded
exactly these. CEN-W's and CEN-J's statement texts are untouched (their
"(ADM)" hypothesis re-reads through the new definition — strengthening the
hypothesis narrows the asserted scope, it does not alter the asserted
equation). No pinned literature statement and no imported theorem is
touched.
...
| OL-A(iii) line/digit structure | weight-β initial forms of deg < m elements live on the line ϕ_β·F_{q^d} (the LINE clause — all §5.1 consumes of (GR-B) here); the graded digit read embeds G_β ↪ F_{q^d}, onto ⟺ FULL attainment dim G_β = d — a proved LED(ii)/(iii) lattice fact, not a (GR-B) clause (§5.1; the unqualified "per-slot F_{q^d}-bijections" gloss was withdrawn in rev 4, this row synced in rev 5) | = **GD-3/OL-B**, consumed not re-proved; order 1 PROVED (GD23 RES-1); orders ≥ 2 = (GR-B) [OPEN there] |
...
| (ADM) admissibility | every on-line lattice slot's β_k is FULLY attained by the ledger: dim_{F_q} G_{β_k} = s(β_k+1) − s(β_k) = d, i.e. #{𝐣 : wt(𝐣) ≡ β_k (mod e), wt(𝐣) ≤ β_k} = d (rev 4 — pass-3 repair; the rev-3 value-only form G_{β_k} ≠ 0 is REFUTED at d ≥ 2, §2/K7; the rev-2 mod-e form was REFUTED by K6) | **HYPOTHESIS** on the datum: automatic at r = 0 on data with nonnegative on-line heights (dim = f₀ = d there, §2); NOT automatic at r ≥ 1 — two explicit r = 1 countermodels (§2) — but a p-free per-datum decidable lattice check (displayed FULL inequality at r = 1) |
...
| falsifier | order-2 census harness (K1–K4, T2) + rev-2 K5 (predict-from-ledger, r = 0) + rev-3 K6 (the class-vs-value (ADM) dichotomy at r = 1, d = 1) + rev-4 K7 (the value-vs-FULL dichotomy at r = 1, d = 2 — the first d ≥ 2 VALUE gate) | RUN: 0 violations / ALL PASS; scope split (form vs value) displayed; §10 |
...
hypothesis (ADM) (FULL attainment of every on-line β_k, dim G_{β_k} = d:
automatic at r = 0, NOT automatic at r ≥ 1 — §2's two countermodels — but
a p-free decidable lattice check per cell, not a research-open input). No
research-open input is new to this unit.** Everything else is proved
below; the r = 0 instance on k₀ = 0 data recovers M08 Theorem 2 exactly,
and the repaired ledger formula now PREDICTS brute-force censuses (K5 at
r = 0; K6's r = 1 positive-control cell at d = 1; K7's r = 1
positive-control cell at d = 2, §10).

## 1. Setting, indexing, and the pinned imports

[Byte-stable = rev 0 §1 verbatim, except the (P5) parenthetical — re-scoped
in rev 5 (pass-4 Finding 1).]
...
* (P5) GMN Prop 2.10: realizability of any prescribed residual value at any
  admissible weight by some P with deg P < m (corroboration, at FULLY
  attained β only, for §5.1's onto-F_{r+1} law — no surjectivity clause of
  (GR-B) is consumed, that gloss being withdrawn in rev 4; not load-bearing
  — see §5.1).
...
**(ADM) — ledger admissibility (hypothesis; REPAIRED in rev 4 — pass-3
CRITICAL 1; rev 3's value-attainment form repaired the pass-2 CRITICAL and
was itself refuted one rung deeper).** Say D is *ledger-admissible* if
every on-line lattice slot's β_k is FULLY attained by the slot ledger:

    (ADM)  for every on-line lattice slot k:
           dim_{F_q} G_{β_k} = d   — equivalently  s(β_k + 1) − s(β_k) = d,
           equivalently  #{𝐣 : wt(𝐣) ≡ β_k (mod e), wt(𝐣) ≤ β_k} = d.

The equivalences are LED(iii): dim G_β = s(β+1) − s(β) = #{𝐣 : wt(𝐣) ≡ β
(mod e), wt(𝐣) ≤ β} (per 𝐣, the max(0, ⌈(β − wt)/e⌉) summand increments
from β to β+1 exactly when β ≡ wt (mod e) and β ≥ wt). This is a p-free
lattice condition on D, decidable from the type's lattice data. *The
attainment ladder (two prior defects, both repaired here or in rev 3):*
CLASS-attainment (β_k mod e ∈ {wt(𝐣) mod e}; rev 2) ⊊ VALUE-attainment
(G_{β_k} ≠ 0, i.e. dim ≥ 1; rev 3) ⊊ FULL attainment (dim = d; rev 4).
Value-attainment governs EMPTINESS (below); FULL attainment is what the
COUNT consumes (§5.1: the digit read is onto F_{r+1} ⟺ dim G_{β_k} = d;
at a PARTIALLY attained slot, 0 < dim < d, the read's image is a proper
F_q-subspace — LED-top(ii)'s phenomenon on the full ledger — and CEN-W's
per-digit charge q^{−d} is wrong by q^{d−dim} on the attained digits).
Facts:

* **(CLASS-d)** Every residue class c mod e contains exactly d ledger
  indices: #{𝐣 : wt(𝐣) ≡ c (mod e)} = d. PROVED at r ≤ 1 (r = 0: all m =
  f₀ = d weights are 0 and e = 1; r = 1: the class of c is {j₁ = j₁* +
  t·e₁ : 0 ≤ t < f₁} with j₁* := (h₁^{−1}c) mod e₁ — gcd(h₁, e₁) = 1 —
  each carrying the f₀ choices of j₀ at weight contribution 0: f₀f₁ = d).
  At general order it follows from (GR-B): the line clause gives
  G_β ↪ ϕ_β·F_{r+1}, so dim G_β ≤ d for every β; by LED(iii), for
  β ≥ max_𝐣 wt(𝐣) in class c, dim G_β = #{class c}; the e class counts
  sum to m = e·d with each ≤ d, forcing each = d. Every consumer of (ADM)
  already assumes (GR-B), so the following reading is available wherever
  (ADM) is consumed:  **(ADM) ⟺ every on-line β_k ≥ max{wt(𝐣) : wt(𝐣) ≡
  β_k (mod e)}** — the deep-stratum reading (all class weights reached).
* By TRI, the w-values attained on {deg < m} \ {0} are exactly
  {e·v + wt(𝐣) : v ≥ 0, 𝐣 a ledger index}; a SPECIFIC β is value-attained
  (G_β ≠ 0) iff ∃𝐣 with wt(𝐣) ≡ β (mod e) and wt(𝐣) ≤ β. Value-attainment
  remains the EMPTINESS threshold: a value-UNATTAINED VERTEX β_k empties
  the stratum (a nonzero vertex digit needs w(a_k) = β_k): census = 0,
  the §8 padding value. A value-unattained INTERIOR on-line slot forces
  its digit to 0 (the slot behaves as strictly-above; the displayed
  formula's −d and its digit index would have to be dropped — that
  variant is NOT displayed here). And at a value-attained but PARTIALLY
  attained slot the stratum can be NONEMPTY while the displayed count is
  FALSE — the second warning display below. (ADM) = FULL excludes all
  three failure shapes.
* **Warning display 1 (the pass-2 countermodel: class ⊋ value;
  machine-checked, K6 §10).** r = 1 type: f₀ = f₁ = 1, e₁ = 3, h₁ = 2
  (λ₁ = −2/3): m = 3, d = 1, e = 3; ledger {1, φ₁, φ₁²} with weights
  {0, 2, 4} (w(φ₁) = e₁V₁ + h₁ = 2, V₁ = 0, per the pinned (P3));
  V̂ = e₁f₁·w(φ₁) = 6. Attained w-values: {3v} ∪ {3v+2} ∪ {3v+4} =
  {0, 2, 3, 4, 5, …} — β = 1 is NOT attained although its CLASS is
  (4 ≡ 1 mod 3). Datum D: single face (0,8)→(1,7) (e_S = 1, d(S) = 1,
  λ = {(1,1)}), free box, ℓ = 1, N = 3 ((DET): 8 ≤ eN − 1 = 8). Then
  β₀ = 8 is attained but β₁ = 7 − 6 = 1 is NOT (G_1 = 0: s(2) = s(1) =
  1), and the stratum is EMPTY by TRI alone (the vertex needs w(a₁) = 1).
  The mod-e reading calls D admissible and CEN-W's free-box display would
  give q^{E′}(q^d−1)∏M = q⁸(q−1)² > 0 — false. Under (ADM) (full ⇒
  value fails here too), D is inadmissible and the census is the padding
  value 0 = truth (K6: brute force 0; mod-e display 256 at p = 2, 26244
  at p = 3, REFUTED).
* **Warning display 2 (the pass-3 countermodel: value ⊋ FULL;
  machine-checked, K7 §10).** r = 1 type — the LED-top(ii) counterexample
  type at the FULL ledger: f₀ = 1, e₁ = 1, f₁ = 2, h₁ = 2 (λ₁ = −2):
  m = 2, d = 2, e = 1; ledger {1, φ₁} with weights {0, 2} (w(φ₁) =
  e₁V₁ + h₁ = 2, V₁ = 0 per (P3)); V̂ = e₁f₁·w(φ₁) = 4. EVERY β ≥ 0 is
  value-attained (e = 1, weight 0 present), but dim G_β = #{wt ≤ β} is 1
  for β ∈ {0, 1} (only wt = 0; the missed digit line is exactly
  LED-top(ii)'s V_1 = F_q·in(π) ⊊ F_{q²}) and 2 = d only for β ≥ 2.
  Datum D: single face (0,6)→(1,5) (slope −1, e_S = 1, d(S) = 1,
  λ = {(1,1)}), free box, ℓ = 1, N = 7 ((DET): max(6, 1) ≤ eN − 1 = 6;
  reads β_k ≤ 6). Then β₀ = 6 is FULL (dim G_6 = 2) but β₁ = 5 − 4 = 1
  is PARTIAL (dim G_1 = 1 < 2 = d). The rev-3 value-(ADM) calls D
  admissible and CEN-W's free-box display gives q^{E′}(q^d−1)·M_{(1,1)}
  (q²) with E′ = (14−10) + (14−1) − 2·2 = 13: q^{13}(q²−1)² — FALSE. The
  truth (stratum membership is purely valuation-level here: d(S) = 1
  vertices force the type) is #{w = 6}·#{w = 1} = (q⁴−q²)(q^{13}−q^{12})
  = q^{14}(q²−1)(q−1): display/truth = (q+1)/q ≠ 1 — at q = 2, display
  73728 vs truth 49152 (K7, both by v-census convolution and the naive
  joint loop). The discrepancy localizes at the partial slot: slot 0's
  factor (q²−1)q² = 12 matches its display; slot 1's truth
  (q^{dim}−1)q^{mN−s(1)−dim} = q^{12}(q−1) (the §5.1 image law) ≠ the
  displayed (q²−1)q^{11}. Under (ADM) = FULL, D is fenced OUT of CEN-W's
  scope (no count is asserted; the stratum is nonempty, so no padding
  claim either). The companion K7 POSITIVE control (0,7)→(1,6), N = 8 has
  β = (7, 2) both FULL, and the display q^{14}(q²−1)² is EXACT at
  p ∈ {2, 3} — the first d ≥ 2 VALUE gate.
* **Automaticity, RE-SCOPED to the FULL predicate (rev 3's r = 0 claim
  survives; its r = 1 value inequality is superseded — it decides the
  now-insufficient predicate).**
  - r = 0: all ledger weights are 0 and e = 1, so dim G_β = #{wt ≤ β} =
    m = f₀ = d for EVERY β ≥ 0: full attainment is automatic on every
    datum with nonnegative on-line line values — every datum realizable
    as an actual polygon (heights are w-values ≥ 0), in particular all
    M08 data and all K5 data. (A datum declaring a negative on-line
    height is empty and (ADM) correctly fails: 0 = 0 under the padding.)
  - r = 1: NOT automatic. With V₁ = 0 the ledger weights are {j₁h₁ :
    0 ≤ j₁ < e₁f₁}, each with the f₀-fold j₀-multiplicity at weight
    contribution 0. The class of β mod e₁ is {j₁ = j₁* + t·e₁ : 0 ≤ t <
    f₁}, j₁* = (h₁^{−1}β) mod e₁, with weights h₁·(j₁* + t·e₁) strictly
    increasing in t; the class count reaches d = f₀f₁ iff ALL f₁ values
    of t satisfy wt ≤ β, i.e. iff β clears the class MAXIMUM:

        (ADM) at r = 1  ⟺  every on-line β_k ≥ h₁·((h₁^{−1}β_k mod e₁)
                                                     + (f₁ − 1)·e₁)

    (K7 verifies the equivalence numerically at seven (e₁, h₁, f₁, f₀)
    tuples, β ≤ 40, together with (CLASS-d)). At f₁ = 1 the (f₁−1)e₁
    term vanishes and this IS rev 3's value inequality β ≥ h₁·((h₁^{−1}β)
    mod e₁) — at r = 1, full = value exactly when f₁ = 1 (each class is
    a single weight), which is why both K6 cells (d = 1) were blind to
    the pass-3 defect. Countermodels: warning display 2 has threshold
    h₁·(0 + 1·1) = 2 > 1 = β₁ (value threshold 0 — passes, wrongly);
    warning display 1 has value threshold 2·((2^{−1}·1) mod 3) = 4 > 1
    (fails already at value).
  - General order: (ADM) is a genuine per-datum condition at EVERY order
    ≥ 1. It is decidable from the type's lattice data: compute the weight
    multiset by the (P3) V-recursion, and per on-line slot count
    #{𝐣 : wt(𝐣) ≡ β_k (mod e), wt(𝐣) ≤ β_k} and compare with d —
    equivalently, under (CLASS-d), check β_k ≥ the class maximum. It
    HOLDS whenever every on-line β_k ≥ max_𝐣 wt(𝐣) (the deep-stratum
    regime). Consumers (§8) evaluate it per constructed cell.
...
### 5.1 The digit read: F_q-linear, injective on the graded piece, fiber-uniform onto its image; onto F_{r+1} ⟺ FULL attainment [(GR-B) line clause + LED]

[REPAIRED in rev 4 (pass-3 CRITICAL 1: this is the defect's consumption
site). The definition of r_β and the additivity bullet are verbatim rev 3;
the surjectivity and fiber-uniformity bullets are RESTATED — rev 3's
"Surjective onto F_{r+1} for attained β: (GR-B)(2′)'s slot clause (the
per-slot digit read is a BIJECTION of F_{r+1})" is WITHDRAWN: no reading
of (GR-B) can defeat the PROVED LED(ii)/(iii) lattice counts, which pin
the z = 0 fiber at q^{mN − s(β+1)} and hence the image dimension at
s(β+1) − s(β). This is LED-top(ii)'s image-subspace phenomenon on the
FULL ledger.]
...
* **Injective on the graded piece (the line clause's true content
  here):** r_β factors through G_β = {w ≥ β}/{w > β} and, by the line
  clause, embeds it: G_β ↪ ϕ_β·F_{r+1} ≅ F_{r+1}. Hence the IMAGE
  V_β := r_β({w ≥ β}) is an F_q-subspace of F_{r+1} with

      dim_{F_q} V_β = dim_{F_q} G_β = s(β+1) − s(β)     (LED(iii)),

  and 0 ≤ dim V_β ≤ d. [Rev 4: the exact full-ledger analogue of
  LED-top(ii)'s sub-ledger V_β of dim s′(β+1) − s′(β).]
* **Fiber-uniform onto its image (the counting consequence,
  unconditional in z ∈ V_β):** a homomorphism of finite abelian groups is
  fiber-uniform onto its image, so #{a ∈ W_N : r_β(a) = z} =
  q^{mN − s(β) − dim V_β} for every z ∈ V_β (zero included), and = 0 for
  z ∉ V_β.
* **Onto F_{r+1} ⟺ FULL attainment (a LATTICE fact, not a (GR-B)
  clause):** V_β = F_{r+1} ⟺ dim G_β = d ⟺ β is FULLY attained. Only
  then does the per-digit count take the value the census consumes,
  q^{mN − s(β) − d} for EVERY z ∈ F_{r+1} — this is exactly what (ADM)
  supplies at on-line slots. At a partially attained β the count is
  q^{mN − s(β) − dim} on the q^{dim} attained digits and 0 on the rest
  (K7's warning-display-2 slot: dim = 1 < d = 2, truth q^{12}(q−1) vs
  the onto-law's q^{11}(q²−1)). The pinned (P5) (GMN Prop 2.10)
  corroboration is re-scoped accordingly: it is cited only at fully
  attained β, and was never load-bearing.

At FULLY attained β this is the exact order-r analogue of M08 Lemma A's
second display, and the only place the residue-field structure enters the
counts.
...
(rev 4: the on-line bullet's consumption line re-pointed at FULL
attainment; everything else verbatim rev 3)
...
* each on-line lattice slot contributes, per prescribed digit
  ...
  z-independent; the onto-ness behind it is dim G_{β_k} = d, i.e. FULL
  attainment, which is exactly (ADM) [rev 4] + (GR-B)'s line clause; at a
  merely value-attained slot, 0 < dim < d, this per-digit charge is FALSE
  — §2 warning display 2, K7).
...
cross-check — REPAIRED (rev 4: step 3's consumption parenthetical + one
validation parenthetical re-pointed at FULL; everything else verbatim
rev 3)
...
3. **(reads)** By (FRESH)(b) each on-line read in K_D except the chain's
   ...
   the fiber; onto-ness = FULL attainment = (ADM) [rev 4] + (GR-B)'s
   line clause); there are L′ = L − [k_s ∈ K_D]
...
order-1 instance is proved, (ADM) automatic — at FULL: dim = f₀ = d, §2).
...
rev 0 verbatim. Rev 4: text untouched — every "(ADM)" here re-reads through
§2's FULL-attainment definition.]
...
| O-9 OL-C | CEN-W/CEN-J + TYPE-MARCH (the junction pin = (FRESH)(c), OPEN) | (GR-B) + (FRESH); (ADM) per cell [rev 2; FULL attainment per rev 4] |
| O-9 OL-D | §7 remark, conditional on (GR-B) + (FRESH) under (ADM) [rev 3; FULL per rev 4] | table build |
...
## 10. Falsifier record (REV 4: K7 added; REV 3: K6; REV 2: form/value split + K5)
...
VALUE = K5 (r = 0, clean) + K6 (the class-vs-value (ADM) dichotomy at
r = 1, d = 1, incl. one r = 1 nonzero-census cell, clean) + K7 (the
value-vs-FULL dichotomy at r = 1, d = 2, incl. one d = 2 nonzero-census
cell, clean — closing the "all VALUE gates ran at d = 1" blindness pass 3
identified) + a queued order-2 predictor (evaluate E′/E″ from the ledger,
compare to tabulation). A FORM caveat carried from pass 3: K1–K4's atom
set {p, p^g−1, p−1} can FIT a partial-attainment factor (q^{dim}−1) at
dim = 1 as the atom (p−1), so the form checks are structurally blind to
the pass-3 failure mode — VALUE gates (K7) are the fence.
...
**K7 (NEW, rev 4): the (ADM) value-vs-FULL dichotomy at r = 1, d = 2 —
the pass-3 countermodel datum, extended per the falsifier-first rule to a
d ≥ 2 POSITIVE control (the first d ≥ 2 VALUE gate).**
`verification/openmath/o9_rev4_fulladm_check.py` (run 2026-07-31, log
`verification/openmath/logs/o9_rev4_fulladm_20260731_202140.log`). At the
§2 warning-display-2 type (f₀ = 1, e₁ = 1, f₁ = 2, h₁ = 2; m = 2, d = 2,
e = 1; weights {0, 2}; V̂ = 4), with w computed from the pinned recursion
(P1)/(P2) directly — min(v_π(c₀), v_π(c₁) + 2), not TRI as a black box —
for p ∈ {2, 3}: (i) CH1: attained w-set = {v + wt}; measured dim G_β
equals the LED(iii) count at every β; FULL ⟺ β ≥ 2 ⟺ the §2 r = 1
full-attainment inequality; β ∈ {0, 1} PARTIAL (dim 1) yet value-attained
— rev 3 admits them, rev 4 does not; (ii) CH2: the countermodel datum
D1 = (0,6)→(1,5), λ = {(1,1)}, free box, N = 7: brute-force truth =
q^{14}(q²−1)(q−1) (49152 at p = 2, 76527504 at p = 3; at p = 2 also by
the naive joint pair loop; membership purely valuation-level: d(S) = 1
vertices force the type) — the rev-3 display q^{13}(q²−1)² (73728 /
102036672) is **REFUTED**, with display·q = truth·(q+1) verified exactly
(the (q+1)/q ratio) and the discrepancy LOCALIZED: the FULL slot matches
its displayed factor, the PARTIAL slot matches §5.1's image law
(q^{dim}−1)q^{mN−s−dim} and not the onto law; rev 4 fences D1 OUT (**no
false assertion remains**); (iii) CH3: positive control D2 = (0,7)→(1,6),
same type, N = 8, (ADM)-FULL HOLDS (β = (7,2), both dim = 2 = d): census
= q^{E′}(q^d−1)M_{(1,1)}(q^d) = q^{14}(q²−1)² **EXACTLY** (147456 at
p = 2, 306110016 at p = 3) — an r = 1, d = 2 full-census VALUE check of
CEN-W's free-box display; (iv) CH4: K6 regression under the FULL
predicate (both m = 3, d = 1, f₁ = 1 cells: FULL ⟺ VALUE there; 0 = 0
and p⁴(p−1)² stand); (v) CH5: the r = 1 FULL inequality ⟺ class count
= d at seven (e₁, h₁, f₁, f₀) tuples for β ≤ 40, with the VALUE-inequality
regression, (CLASS-d) (every class mod e₁ has exactly d indices), and
the f₁ = 1 collapse to rev 3's inequality all verified; the T2 thresholds
FULL = 2 vs VALUE = 0 exhibited at β₁ = 1. RESULT: **ALL PASS**.
Independence disclosure (as for K6): the census layers share the one-step
pinned w-recursion with the formula side; the naive loops and the
v-census convolution independently check every count and the slot-product
structure, not w itself; CH2c's image-law leg is an arithmetic-consistency
display against LED(ii) with the enumeration as the independent leg.
...
VALUE check remains the queued predictor). K7 lifts exactly the d = 1
limit (its two cells are d = 2) but keeps the others: K7's cells are
single-face, ℓ = 1, valuation-level-forced types with e = 1 (no
fractional slopes) and, like every census layer here, share the pinned
one-step w-recursion with the formula side; a d ≥ 2 cell with a
residual-type condition that is NOT valuation-forced (genuine digit
reads over F_{q^d}, d ≥ 2) remains uncovered by any VALUE gate and is
folded into the queued order-2 predictor's scope. The march itself
at g ≥ 2 is separately covered by the depth-3 harness's 470,736-instance
run (CU-1's gate).

## 11. Attack list for the hostile pass (rev 4)

1. The restated (ADM): re-derive the equivalence chain dim G_{β_k} = d ⟺
   s(β_k+1) − s(β_k) = d ⟺ #{𝐣 : wt ≡ β_k (mod e), wt ≤ β_k} = d from
   LED(iii). Is FULL attainment + the (GR-B) line clause EXACTLY what
   §5.1's onto-F_{r+1} fiber-uniformity consumes — nothing weaker
   sufficing, nothing stronger smuggled in?
2. (CLASS-d): check the r ≤ 1 proofs by hand (r = 0 trivial; r = 1 the
   j₁* + t·e₁ argument with the f₀-multiplicity). At general order it is
   derived FROM (GR-B) (line clause ⟹ dim ≤ d; LED(iii) at large β;
   pigeonhole over e classes summing to ed) — is that derivation sound,
   and is the max-weight reading of (ADM) used ONLY where (GR-B) is
   already assumed (never to define the hypothesis unconditionally)?
3. The r = 1 FULL inequality: is the class maximum really
   h₁·(j₁* + (f₁−1)e₁) against the weight multiset {j₁h₁ : 0 ≤ j₁ <
   e₁f₁} with f₀-fold weight-0 j₀-multiplicity? Does it collapse to
   rev 3's value inequality exactly at f₁ = 1, and is "at r = 1,
   full = value ⟺ f₁ = 1" right (f₀ > 1 included)?
4. §5.1 as restated: does the line clause honestly give INJECTIVITY of
   the read on G_β (so dim V_β = dim G_β), and is the withdrawn
   "bijection of F_{r+1}" gloss consumed NOWHERE else (grep §§6–8 and
   the (P5) remark)? Is the partial-attainment per-digit law
   (q^{mN−s−dim} on q^{dim} digits, 0 elsewhere) consistent with
   LED(ii) at β and β+1?
5. Consumption audit under the restated (ADM): §6.3's on-line bullet and
   §6.4 step 3 now cite FULL attainment — is value-attainment still
   consumed anywhere beyond its two legitimate uses (vertex emptiness /
   interior forced-zero, §2)? Does CEN-J's coset-top leg (s′, LED-top)
   need a SUB-ledger full-attainment clause anywhere, or is the
   ρ-pinned right end ((FRESH)(c)) + LED-top(i) genuinely all it uses?
6. Warning display 2's arithmetic: s(6) = 10, s(7) = 12, s(1) = 1,
   s(2) = 2, E′ = 13, truth q^{14}(q²−1)(q−1), the (q+1)/q ratio, and
   the per-slot localization — all re-derivable by hand?
7. K7's semantics: is membership at D1/D2 really valuation-level-forced
   (d(S) = 1 vertices, twist-invariance of nonvanishing)? Is the d ≥ 2
   positive control a genuine full-census check of CEN-W's display
   (free box: ε = 1, (q^d−1)·M_{(1,1)}(q^d) = (q²−1)²)? Do the v-census
   convolution and the naive joint loop really check the counts
   independently (given the shared w-formula, honestly disclosed)?
8. The automaticity rescope: r = 0 FULL automaticity (dim = f₀ = d for
   every β ≥ 0) — right scope, all M08/K5 data inside? Is §6.4's r = 0
   validation still sound verbatim under the FULL reading?
9. (Carried from rev 2.) The K4 atom set {p, p^g−1, p−1}: could a
   genuine census produce atoms outside it that the harness would
   mis-flag — and note pass 3's observation that (q^{dim}−1) at dim = 1
   IS the atom (p−1), i.e. the FORM checks cannot see partial
   attainment. Is the §10 disclosure of that blindness accurate?

— O9 Phase-B rev 5 (= rev 4 + the pass-4 two-site gloss-propagation fix).
Verdict: PARTIAL — the order-r census REDUCED to
...
FULL-attainment form dim G_{β_k} = d (rev 4, closing the pass-3 ladder:
class ⊊ value ⊊ full; automatic at r = 0; at r ≥ 1 a decidable per-datum
lattice check with the displayed r = 1 inequality — §2's two warning
displays are the two lower rungs' countermodels); the reduction's own
mathematics — A1/TRI/LED/LED-top, (DET), TYPE-MARCH, the STRICT-threshold
CEN-W over explicit boxes with §5.1 restated at the honest image-subspace
granularity, CEN-J with the DISPLAYED (E″) and its explicit scope/padding
clause — proved above; r = 0 on k₀ = 0 data = M08 Thm 2 recovered exactly,
(E″) validated against M08's printed formula; falsifier: 1.13M order-2
instances clean (FORM) + K5 predict-from-ledger exact at r = 0, K6 exact
at the r = 1 class-vs-value dichotomy, and K7 exact at the r = 1, d = 2
value-vs-FULL dichotomy (VALUE — the first d ≥ 2 gate), with the rev-0
formula refuted by K5 at every integral left-tail case, the rev-2 mod-e
(ADM) refuted by K6, and the rev-3 value (ADM) refuted by K7 at the
pass-3 countermodel (73728 vs 49152 at q = 2; fenced out, positive
control exact, under the rev-4 reading).
