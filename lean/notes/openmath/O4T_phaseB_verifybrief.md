# O4T — the O-4 drainage transfer — Phase-B verify brief, rev 1 (Fable, 2026-07-31)
...
| TR-Q | quantitative transfer: a decision-level majorant N(f) ≤ Γ(v_p(disc f)) converts the discriminant tail bound into an explicit envelope for c | **PROVED here** (consumes I-TAIL, displayed) |
| N3 | the canonical order-≤1 classifier at n ≤ 3: termination, soundness, stability, read ceiling **L(f) ≤ v_p(disc f) + 1** at every prime including wild, hence pointwise OL1 and drainage with explicit envelope | **PROVED here** ((c) as the assembled quantified statement; ingredients (a)/(b) classical — see §6) |
...
predicate; n ≥ 4; OM order ≥ 2; any consumption of Bauch–Nart–Stainsby (BNS).
...
extensions), `v(0) = ∞`. For separable monic `f`, `σ(f)` = its splitting type over
...
`f` with roots `α_1, …, α_n` (integral since `f` is monic over `Z_p`),
...
**Theorem TR-Q (quantitative transfer).** Let `c` be stable and suppose there is a
nondecreasing `Γ : Z_{≥0} → Z_{≥0}` with: for every separable monic `f`, `c` decides
the level-`N` class of `f` for every `N ≥ Γ(v_p(disc f))`. Then for every `N`:

    env^c(N) ≤ μ{ a : disc f_a = 0 } + μ{ a : v_p(disc f_a) ≥ Γ⁻(N) }
             ≤ (n−1)·p^{−max(0, ⌈(Γ⁻(N)−w)/(n−1)⌉)},
    where Γ⁻(N) := min{ v : Γ(v) > N } (so v < Γ⁻(N) ⇒ Γ(v) ≤ N; Γ⁻(N) → ∞).
...
`U^c_N ⊆ {disc = 0} ∪ {v ≥ Γ⁻(N)}`; apply (I-TAIL). `Γ⁻(N) → ∞` since each `Γ(v)` is
finite. ∎

## 3. The canonical order-≤1 classifier at n ≤ 3, and Theorem N3
...
image of `p`), a domain, so initial forms are multiplicative: `in_w(g) = in_w(g_<) ·
...
`e | e(Q_p(β)/Q_p)` and `[Q_p(β):Q_p] ≥ e`; the slope factor `g_{h/e} ∈ Z_p[x]` (Fact
SF) has degree `e·1 = e` and contains `β`: it is irreducible with `e = e`, `f = 1`.
...
`u ≡ z₀ (mod 𝔪)`, i.e. `v(β − z₀p^h) > h`; no other root of `g` does (other sides have
`v(β) ≠ h`; other residual classes reduce elsewhere). ∎

**Lemma REC (refinement invariant).** Opening `(c′ = c + z₀p^h, m′, λ_prev = h)` from a
repeated residual root restores (I1): exactly the `m′` roots of Lemma SIDE(ii) satisfy
`v(α − c′) > h`. *Proof.* Those `m′` roots do, by SIDE(ii). Any other root: if
`v(α − c) < h` then `v(α − c′) = v(α − c) ≤ h` (ultrametric, `v(z₀p^h) ≥ h`); if
`v(α − c) = h` with a different residual class, `v(α − c′) = h`; if `v(α − c) > h` on a
different side — impossible, sides partition by valuation and `v(α−c) > h` roots lie on
steeper sides, which the certificate processes separately (they are NOT on the slope-`h`
side). For those steeper-side roots: `v(α − c′) = min(v(α−c), h) = h` unless `z₀ = 0`;
if `z₀ = 0` then `c′ = c` and steeper-side roots keep `v(α − c′) > h` — but then they
lie on sides of slope `> h` which were already processed as separate sides of THIS
step's hull, and the residual class of the slope-`h` side at `z₀ = 0` is not a root of
`R` at `0` unless the side's own roots reduce there; the count in SIDE(ii) is exact
either way. Each certified or recursed group is disjoint, and degrees add to `m`. ∎
...
**Corollary N3-ENV (drainage at n ≤ 3, every prime, with envelope).** By TR-Q with
`Γ(v) = v+1` (so `Γ⁻(N) = N`), for `2 ≤ n ≤ 3`, every prime including wild:
...
the canonical classifier is at least as fast as the semantic optimum is required to be,
and at most as slow as the Krasner decider run at DOUBLE the level — the transfer shift
is multiplicative in the classifier's favor, not an additive lag.
...
`N^OM(f) ≤ Γ(n, v_p(disc f))` with `Γ` nondecreasing in `v` (the OL2 shape), TR-Q gives
the explicit envelope with `Γ⁻(N)` in the exponent.
...
* Fact NP/SF/HEN: (a) classical (Newton polygons over complete discretely valued
  fields; Galois stability of valuation fibers; Hensel). Standard references suffice;
  no numbered citation is consumed by a downstream step.
* Lemma RES: (b) — the order-1 shadow of the GD23 unit's GAUSS-g/VAL-g/DEVID layer
  (V26-verified); proved here directly and self-containedly for `e = 1`.
* Theorem N3 + corollaries: (c) AS AN ASSEMBLED, EXPLICITLY QUANTIFIED STATEMENT about
  ...
  every prime, `n ≤ 3`); the mathematical substance is (a/b)-classical (order-1
...
3. **Lemma REC** — the `z₀ = 0` corner and the exact-count claim (`m′` and no others);
   check against SIDE(ii)'s Hensel factor bookkeeping.
...
6. **(D) and the pairwise bounds** — `v(disc) = 2Σδ_{ij}` for monic integral-root
   ...
   breaks (CEIL)).
7. **TR-Q's `Γ⁻`** — off-by-one at the tail threshold (`v < Γ⁻(N) ⇒ Γ(v) ≤ N`).
8. **§4 (AGR)** — confirm nothing in §§2–3 silently consumes (AGR) or any CU-*/GD-≥2
   item; §3 must stand on Facts NP/SF/HEN alone.
