...
## S2. THE BRIDGE LEMMAS (all proved) [r1 2026-08-09 (passPE1 F-1/F-2/F-4): HETOW-1 and HETOW-2 restated and re-proved at the WRAP-CORRECTED key — the pass refuted their gauge-naive forms at η ≠ 1 frames]
...
≡ mod e₁ and 0 ≤ i(·) < e₁). At a (T1)/(T2) point res(x₀^{e₁}/π^h) =
η_{x₀}, so the wrap carries a residue factor: the ϖ-residue of
n̂((f₂−t)u₂) is η^{−(f₂−t)Q(u₂)−W(t)} — NOT the (f₂−t)-th power of
n̂(u₂)'s. W(t) is non-linear in t, so no K₂^×-rescale absorbs it.
...
residue η^{−Q(k)}·η^j. The gauge-naive display (plain lifts ĉ_t of
...
t-coefficient, by the preamble's two displays; the gauge-naive
coefficient omits the η^{W(t)} factor, leaving the residue
−c_t·η^{−(f₂−t)Q−W(t)}. CONSISTENCY (now a check, not a definition):
at a root ξ of the corrected Φ₂, dividing 0 = Φ₂(ξ) by ϖ(ξ)^{f₂u₂}
and taking residues gives r̃(β_ξ) = 0 — the letter clause of
T(b)′(i) at the corrected key, matching (b). At the naive key the
same division shows β_ξ kills the SKEWED polynomial
Z^{f₂} − Σ_t c_t·η^{−(f₂−t)Q−W(t)}Z^t; when that is reducible (frame
X: (Z+1)², β_ξ ∈ K) the roots generate too small a residue field and
the naive key is not the composed carrier — passPE1's measured
σ = {(4,1)} ≠ {(2,2)}. ∎
...
Pre-reduction terms sit at slots e₂t at value T₂ (exact-height part;
DEFINITION HE6-1's line e₂(f₂−t)u₂ + e₂t·u₂ = T₂) or above. Value
never decreases along the reduction, and value-T₂ material occurs
only at slots e₂t (moved pieces land at slot e₂t + 1 with value
> T₂ and are inert). Sort the terminal (= literal reduced)
...
slot e₂t's exact-height residue after reduction equals the unreduced
coefficient's, = r̃'s t-coefficient (HETOW-1(c)), and B_t ≠ 0 ⟺
c_t ≠ 0 survives. Everything else goes to −w: above-height stationary
...
* **P2 ✓ (HEADLINE) 300/300, zero mis-decisions** — the FROZEN readers
  ...
  decides the tower box with no change. **[r1 2026-08-09 (passPE1
...
## S7. THE r1 REPAIR RECORD (2026-08-09, from passPE1 @ 5f821dc) + THE SUPP MACHINE LEG
...
**F-2 (GAP, repaired inside LEMMA HETOW-2's proof):** the sealed carry
estimate priced only the moved piece of x^{D′+r′} = x^{r′}(Φ′ − tail);
the stationary tail piece sits AT value T₂ and lands in B_t. Re-derived
accounting: value never decreases; exact-height value-T₂ parts land in
B_t with residues unchanged (reduction is an identity; moved material
sits strictly higher); w collects only value ≥ T₂ + min(e₂, u₂ − e₂D′h)
material, so dv₂(w) > T₂ still.
**F-3 (GAP):** HETOW-BOX-3 [r1] now names the η = 1 gauge of the
battery and the η ≠ 1 coverage on record.
**F-4 (MINOR):** the coefficient object pinned — the (LIFT)-slot form,
under which no x-overflow occurs at any f₁ (w = 0); the carry algebra
retained for representation independence.
**F-5 (MINOR):** §S6 P2 gauge caveat added.
