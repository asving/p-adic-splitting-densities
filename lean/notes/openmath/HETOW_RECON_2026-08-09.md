# HETOW — [GENHN-HE(μ₂ ≥ 3)] AT TOWER STAGES: the tower-stage σ read is a **LITERAL INSTANCE of THEOREM HE7.A** at the composed carrier — the instantiation map with every hypothesis checked, THREE bridge lemmas (HETOW-1 the ϖ-twist label translation; HETOW-2 the composed key is a Ψ-family member up to a LEMMA HE7-12 perturbation, with the f₁ ≥ 2 carry PROVED > T₂ from the node floor; HETOW-3 full-side block identities) — minimal live degree n = 12 derived — machine leg: the FIRST inner-μ₂ ≥ 3 tower battery (both branches (e₂,f₂) = (2,1)/(1,2)), run through the FROZEN he7/he6r1 readers unmodified
...
## S0. THE QUESTION, AND THE ANSWER'S SHAPE
...
HE3→HE6→HE7 ladder (COROLLARY HE7.B). The HE6R1 precedent: a box that
looks new can be a LITERAL INSTANCE of HE7's frozen machinery once the
instantiation map is written and every hypothesis checked. Tested here:
...
hypothesis on ℓ or d_r (HE6R1 §S3.1). What needs proof is only the
...
## S1. THE INSTANTIATION MAP (every hypothesis of DEFINITION HE7-1 + THEOREM HE7.A at the composed data)
...
Φ₂ := Φ′^{e₂f₂} − Σ_{t<f₂} ĉ_t·n̂(u₂(f₂−t))·Φ′^{e₂t}, deg D₂ = D′e₂f₂;
composed ladder dv₂ = e₁e₂v.

**The map** (HE7 datum ← tower datum), row by row against DEFINITION HE7-1:
...
## S2. THE BRIDGE LEMMAS (all proved)
...
> **LEMMA HETOW-1 (label translation).** Set Q := Q(u₂) and define
> r̃ ∈ K[Z] as the monic degree-f₂ polynomial whose t-coefficient is the
> ϖ-residue of Φ₂'s slot-e₂t development coefficient at height (f₂−t)u₂
> (equivalently: r̃ := the minimal polynomial of β := η_θ^{−Q}·η₂ over K,
> η₂ a fixed root of ψ₂). Then: (a) r̃ is monic irreducible of degree f₂
> with r̃(0) ≠ 0, and K[Z]/(r̃) = K(β) = K(η₂) ≅ F_{Q^{f₁f₂}};
...
*Proof.* (b) Φ′^{e₂}/ϖ^{u₂} = (Φ′^{e₂}/n̂(u₂))·(n̂(u₂)/ϖ^{u₂}); the first
factor's residue is η₂(x₀) by (T2₂)'s definition, the second is
η_{x₀}^{−Q} by the display above. η^{−Q} ∈ ι(K^×), so applying the fixed
K-automorphism-free rescaling Z ↦ η^{Q}Z: β is a root of
Z ↦ η^{−Qf₂}ψ₂(η^{Q}Z) ⟺ η₂ is a root of ψ₂. (a) K(β) = K(η₂) because
η^{−Q} ∈ K^×; [K(η₂):K] = f₂ (ψ₂ irreducible, T(b)′(i)); so β's minimal
polynomial is monic irreducible of degree f₂, equal to
η^{−Qf₂}ψ₂(η^{Q}Z), with constant term η^{−Qf₂}ψ₂(0) ≠ 0 — ψ₂(0) ≠ 0
because at a FULL side the left endpoint (0, dv(A_0)) is an attained pin
(A_0 ≠ 0), so the side residual R_{κ₂} = ψ₂^{μ₂} has nonzero constant
term. That this minimal polynomial has as coefficients exactly the
ϖ-residues of Φ₂'s exact-height slot parts is the residual division at a
root ξ of Φ₂ (T(b)′(i)'s proof, ϖ-normalized instead of n̂-normalized):
0 = res(Φ₂(ξ)/ϖ(ξ)^{e₂f₂·... }) gives 0 = β_ξ^{f₂}-combination with those
coefficients, i.e. β_ξ is a root of the displayed degree-f₂ polynomial;
since it is monic of degree f₂ = [K(β):K] and kills β, it IS the minimal
polynomial. ∎

> **LEMMA HETOW-2 (the composed key is a key-family member up to an
> above-the-line perturbation).** Let Ψ := the Ψ_{κ₂,r̃} obtained from
> Φ₂'s own exact-height slot parts (so B_t := the height-(f₂−t)u₂ part of
> Φ₂'s slot e₂t, reduced mod Φ′ to degree < D′). Then Φ₂ = Ψ − w with
> deg w < D″ and **either w = 0 (in particular whenever f₁ = 1) or
> dv₂(w) > T₂**; hence by LEMMA HE7-12(a)–(d) the entire §S5 package,
> THEOREM HE7.A(1)(2), the refine package (LEMMA HE7-12/13, LEMMA HE7-8)
> and the §S1 CONVENTION with LEMMA HE6R1-3's peel hold VERBATIM with
> key Φ₂.

*Proof.* At f₁ = 1: ĉ_t ∈ O is a scalar lift and n̂(u₂(f₂−t)) is an
x-monomial of degree i(·) < e₁ = D′, so each B_t := −ĉ_t·n̂(u₂(f₂−t))
already has deg < D′, dv = (f₂−t)u₂ exactly (monomial), ϖ-residue = r̃'s
t-coefficient (HETOW-1); Φ₂ is LITERALLY of DEFINITION HE6-1's displayed
form (the definition's proofs consume only deg/dv/residue of the B_t,
which the n̂-monomial choice supplies — it is itself a (LIFT) output: all
f₁ slots are available at these heights by HE6's [r1] NON-PROPAGATION box,
since (f₂−t)u₂ ≥ u₂ > e₂D′h > (D′−1)h). So w = 0.
At f₁ ≥ 2 the literal products ĉ_t·n̂ can overflow x-degree D′ (T(b)′'s
parenthetical); reduce mod Φ′. **The carry estimate, derived (not
transcribed):** replacing x^{D′+r'} by x^{r'}·(Φ′ − tail) moves a
monomial contribution at Φ′-slot s and dv-height m to slot s+1 at
dv-height m − D′h; its slot VALUE (in the level-2 grading
value(s, m) = e₂m + su₂) moves from e₂m + su₂ to
e₂(m − D′h) + (s+1)u₂ = (e₂m + su₂) + (u₂ − e₂D′h) — a STRICT increase
by u₂ − e₂D′h > 0 (the [r1] node floor κ₂ > D′h). The pre-reduction
terms of Φ₂ sit at slot value exactly T₂ (DEFINITION HE6-1's line:
e₂(f₂−t)u₂ + e₂t·u₂ = T₂), so every carry — and every iterated carry —
sits at slot value ≥ T₂ + (u₂ − e₂D′h) > T₂. Collect the exact-height
(value-T₂) parts into Ψ and the carries into −w: deg w < D″, and w's
every occupied slot has value > T₂, so dv₂(w) > T₂ (slotwise ultrametric,
LEMMA HE7-12(d)'s computation). LEMMA HE7-12 [r2, r3] applies (its W = 0
split covers the f₁ = 1 case). ∎
...
> (full inner side, any μ₂ ≥ 2, any (e₁, f₁, h), any e₂f₂ ≥ 2):
>
...
[GENHN-HE(≥3)] instance exists at n ≤ 11. The two inner branches at
(e₁,f₁,h) = (2,1,1) are the battery's two cases (§S5); the σ menus at
μ₂ = 3 (from (ii)+(iii)):
...
## S4. WHAT REMAINS BOXED (inherited, none new)
...
> **HETOW-BOX-3 (machine coverage).** The battery exercises μ₂ = 3,
> (e₁,f₁,h) = (2,1,1), (e₂,f₂) ∈ {(2,1),(1,2)}, u₂ ∈ {5, 3}, p ∈ {3,5},
> char 0 only (PARI oracle asymmetry, inherited HE6-BOX-4). NOT
> exercised: μ₂ ≥ 4 (level-3-capable towers), e₂f₂ ≥ 4 (both ℓ ≥ 2 AND
> d_r ≥ 2 — first live n = 24), f₁ ≥ 2 outer frames (HETOW-2's carry leg
> is proof-only, cf. GENHN-ERR-1's separate f₁ ≥ 2 residue), organic
...
## S5. THE MACHINE LEG (`verification/openmath/hetow_checks.py`) — sealed at commit 1
...
**VERDICT — appended at commit 2 from the committed artifacts: §S6.**
