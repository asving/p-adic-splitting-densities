# The per-site read package (REV 3): lawful-site locality, the realized tier with window closure, and the cylinder law
...
and §8: **(M6b)(iii)** (harvested records satisfy AUX) and, at states of order ≥ 1
beyond first transitions over degree-1 root keys, the two named citation-grade
inputs **(V1)/(V2)** of §2.5 (MacLane augmentation multiplicativity; the key
initial-form/development identity). The rev-2 assumption's checkable kernel
((M6b)(i) vertex nonzero; (M6b)(ii)'s consumed upper half) and the rev-3-exposed
window-harvest completeness **(M6c)** are PROVED in §2.5 within that same
perimeter — outright at root states and degree-1 first transitions, modulo
(V1)/(V2) elsewhere. The honest conditionality ledger is §8. An exhaustive machine
...
residue ((M6b)(iii) + the (V1)/(V2) perimeter; for genuine MacLane-key policies
these are the key-polynomial content of OM theory — seam S-4; machine-checked
at depths 1 and 2 in §9). Existence of policies is
...
* **(M6b)** Harvest, substantive residue: at every harvested child, (i) the
  ...
  compose by the policy's degree law.) **Status at rev 3:** (i) and the upper
  half of (ii) — the only half any proof consumes (Lemma CEIL, (†′)) — are
  PROVED: Corollary VTX (§2.5), outright at root states and first transitions
  over degree-1 root keys, modulo the named inputs (V1)/(V2) at general states;
...
* **(M6c)** Window-harvest completeness — the rev-3-exposed second interface
  condition, now named and discharged: (i) harvested children satisfy (R1) at
  ALL slots of the full development (not only the window slots that harvesting
  inspects); (ii) every REALIZED child at a REALIZED non-halted state arises
  from a (window face, ψ) pair with exactly the harvested records. Precise
  statement, proof, and status: §2.5. [PROVED at root states and degree-1 first
  transitions; PROVED MODULO (V1)/(V2) in general; machine-checked at depth 2,
  §9 T-F.]

### 2.5 Window-harvest completeness: (M6c), Lemma WGEO, realization coherence *(new at rev 3)*

Fix a monic degree-n f and a REALIZED non-halted state o; write Φ := Φ_o (the
...
**Condition (M6c) (window-harvest completeness).**
...
(M6a); strictness is what (R1) demands off the recorded strides).
(ii) *No phantom:* every REALIZED child at o arises from a (window principal
face, ψ′) candidate pair, with records equal to that pair's harvest.

**Named inputs** (consumed ONLY at states of order ≥ 1 other than first
transitions over degree-1 root keys; both citation-grade, attributions flagged
for lookup):

* **(V1) augmentation multiplicativity:** every w_i of §1.1 is a valuation,
  w_i(AB) = w_i(A) + w_i(B). [MacLane, Trans. AMS 40 (1936), the augmented-
  valuation/key-polynomial theorems; the §1.1 chain is the MacLane composite.
  Proved below at degree-1 first transitions (Lemma VAL1).]
* **(V2) key initial form, development identity, residual dictionary:** in the
...
**Lemma KEY1 (key initial form, degree-1 root key).** *(b — proof displayed)*
In the setting of VAL1, with ψ of degree g (ord_ψ(R_0) = μ) and the key lift
Φ_1 = Σ_κ ψ̂_κ p^{(g−κ)h} Φ_0^{eκ}: every monomial of Φ_1 has w_1-weight exactly
...
degree D = eg), induct on deg A: the top reduction step subtracts
a_d·X^{d−D}·Φ_1 whose every monomial has w_1-weight = e·v_p(a_d·ψ̂_κ p^{(g−κ)h})
+ (d−D+eκ)h ≥ e·v_p(a_d) + dh ≥ w_1(A) (using D = eg and the on-line weights of
Φ_1), so the remainder chain keeps w_1 ≥ w_1(A); hence w_1(B) ≥ w_1(A) and
...
**Lemma WGEO (window geometry).** Let o be a non-root REALIZED state as above.
Then, with γ the creating read's side value:

* (a) u_k ≥ γ for every k with C_k ≠ 0, and w(f) = γ;
* (b) min_k u_k = γ, attained at k = μ and at NO k < μ; in particular C_μ ≠ 0;
...
key (via VAL1/KEY1/DIV1); PROVED MODULO (V1)/(V2) in general (the proof below
...
γ is in(f) = Σ_{k∈S_0} in(C_k)·in(Φ)^k, and it is nonzero (else w(f) > γ). Each
in(C_k) has Φ-degree < deg Φ, hence ψ̂_hom ∤ in(C_k) ((V2a)/KEY1's degree
block), so ord_{ψ̂_hom} of the k-th summand is exactly k. Factoring
ψ̂_hom^{min S_0} out of the sum leaves a unit-plus-multiple-of-ψ̂_hom bracket,
so ord_{ψ̂_hom}(in f) = min S_0. By ((V2c)/KEY1), ord_{ψ̂_hom}(in f) =
...
**Corollary VTX (vertex laws; (M6b)(i) and (N4)-upper discharged).** At every
...
the face, so no equality is permitted or needed). (ii) Let ν′ be a REALIZED
child at o with side [s0, s0+wSide], line ℓ′. By (R1) on f, ℓ′ supports ALL
...
i.e. a factor of F's residual polynomial. Hence (F, ψ′) is a candidate pair
and ν′'s records are its harvest. ∎
...
*Proof.* (⟸) Induction along H·ν. Each prefix state is non-saturated (the
record-only clause of REALIZED) and non-ns-halted (its next node's side is a
window principal face by (M6c)(ii), so a principal face exists); the next node
arises from a candidate pair with harvested records ((M6c)(ii) again), which
the execution harvests ((M6a) totality). (⟹) Induction along the walk: a
harvested child satisfies full READ ((M6a) window clauses + (M6c)(i) beyond-
window + (M6b)(i) for (R5)) and is LAWFUL ((N1)–(N3) displayed at harvest;
...
* (F1) forward (chains REALIZED at x′): a chain H has L(H) ≤ t ≤ N; Theorem
  2(a). Pruning: IrrHalts is record-only; a proper prefix's non-ns-halt is
  witnessed by its tree child, REALIZED at x′ by Theorem 2(a), whose side is a
  window principal face by **(M6c)(ii)** — so the prefix's window has a
...
* (F1) backward (no strays at x′): suppose K is a REALIZED pruned run on f_{x′}
  ...
  REALIZED pruned run is one: READ and the per-node laws restrict, and the
...
box-free AUX completion of LAWFUL: Lemmas DEV, POS, R-LOC, NAT, CEIL, ROOT-M;
Theorem 1; Theorem 2(a) (as restated at rev 3 — REALIZED, no execution
reading); and, at root states and at first transitions over degree-1 root keys,
Lemmas VAL1/KEY1/DIV1/WGEO, Corollary VTX, and condition (M6c). Proved from the
...
at states beyond the perimeter above, the named citation-grade inputs (V1)/(V2)
of §2.5 (which carry WGEO/VTX/(M6c) there): Lemma SPAN; Lemma REAL≡; Theorem
...
* **(S-4) Harvest realization ((M6b)/(M6c)/(V1)/(V2)).** Rev 3 splits this seam
  into proved and assumed parts. PROVED (§2.5, Lemmas WGEO + Corollary VTX +
  condition (M6c)): vertex coefficient nonzero, the (N4) UPPER bound,
  beyond-window (R1), and no-phantom — outright at root states and at first
  transitions over degree-1 root keys; modulo (V1)/(V2) at all other states;
  machine-checked at depth 1 (§9 T-E: 33,043 instances) and depth 2 (§9 T-F:
  41,720 instances), zero violations. ASSUMED: (M6b)(iii) AUX-at-harvest; the
  LOWER (N4) band (consumed only inside Lemma REAL≡'s lawfulness step); and
  (V1)/(V2) themselves — MacLane's augmentation theorem and the GMN
  key-form/development/residual dictionary, citation-grade with attributions
  flagged for lookup. Consumers: Lemma SPAN's roster totality, Lemma REAL≡,
...
* Named inputs (V1)/(V2): (a) — MacLane 1936 / GMN Trans. AMS 364 (2012);
  attributions flagged for lookup (seam S-4).
