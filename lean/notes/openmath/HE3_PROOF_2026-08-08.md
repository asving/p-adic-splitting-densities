...
G(Z) := Σ_j g_jZ^j = Z^{T}·R_λ(Z) up to the twist diagonal **[r2, F1:
with the g_j read against the COHERENT normalizers M_j = n(c)/n(λ)^j
(c the λ-line intercept at abscissa 0; DEFINITION 1 [r2]),
G(Z) = Z^T·R_λ(Z) up to ONE global nonzero K-factor — no diagonal;
and in the substitution below the (i → j) summand's normalizer ratio
M_i·n(λ)^{i−j}/M_j = 1 identically, so the wrap constants cancel
term-by-term (the coboundary, §S3 Remark [r2]) rather than needing a
folding]**. The
