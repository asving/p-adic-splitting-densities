...
exactly (per root, dv(Φ′(ρ) − C(ρ)) = min(λ_ρ, κ): dv(C(ρ)) = κ by the
ξ-form slot read at ρ — §S6.1 step (2) [r2, F3] — and at λ_ρ = κ the
residues cannot cancel, since β_ρ is a root of ι_ρ(R_κ) by HE3-0(iii)
while ι_ρ(s) is not, ι_ρ being a field embedding and R_κ(s) ≠ 0); the
