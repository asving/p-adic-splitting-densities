...
> root set S (LEMMA HE7-6). Put μ₂ := deg f_S / D″ ∈ ℤ_{≥1} and let
...
>
> 3. at a slope with a repeated residual factor and ℓ₂ = 1 the level-2
>    α-refine Ψ ↦ Ψ − ŝ₂·n₂(λ₂) exists (by (LIFT₂)) and the refine chain is
>    FINITE (LEMMA HE7-8), so the read continues; at a slope with a repeated
>    residual factor and ℓ₂ ≥ 2 the read passes to level 3, which is the same
>    statement one level further up (THEOREM HE7.D) and cannot occur unless
>    μ₂ ≥ 4 (THEOREM HE7.C).
>
> The proof uses **no base change, no irreducibility of Ψ, and no order-≥2
> citation**.

> **THEOREM HE7.C (TERMINATION — the ladder is FINITE at every n).** A node
> requiring a level jump (a side with ℓ ≥ 2 carrying a residual factor of
> multiplicity ≥ 2 — exactly HE6-BOX-1's configuration) satisfies μ ≥ 4, and
> the level-2 problem it opens has
>
>     μ₂ ≤ μ / (ℓ·d_r) ≤ μ/2 .
>
...
> COROLLARY HE6.B loses its exclusion clause there. (ii) At every degree, the
...
> **COROLLARY HE7-7″ (the level-2 label of a root; HE6-2″ one level up).**
...
*Proof of THEOREM HE7.C.* A node needs a level jump exactly in HE6-BOX-1's
configuration: a side of slope λ = u/ℓ with **ℓ ≥ 2** carrying a residual
factor r of **multiplicity m ≥ 2**. Then

    μ ≥ L_λ = ℓ·deg R_λ ≥ ℓ·m·d_r ≥ 2·2·1 = 4 ,
...
μ₂ ≥ 4, impossible. The first degree at which J = 2 is possible is
n = D′μ with μ ≥ 8, i.e. **n = 16**. ∎
...
> **THEOREM HE7.A′ (the bite, unconditionally).** At every member of the
> n = 8 box configuration with disc f ≠ 0, σ(f) is decided by the level-2
> read of §S1–S5, with the dictionary above; the argument uses no base
> change, no irreducibility of Ψ, and no order-≥2 citation. In particular
> HE6-BOX-1's machine-certified live configurations are all decided.
...
> **HE7-BOX-1 (arc grade).** Attempt grade **0/2** — no hostile pass has run
> on this note. Every consequence inherits the minimum over its suppliers
> {GENHN 0/2 (battery GREEN), HE6 0/2 (battery GREEN), HE3 1/2 at μ ≤ 3,
> GENIND 0/2, W-12 0/2, classical}. COROLLARY HE7.B is therefore a
...
octics with 0 exceptions, in both characteristics and at both trivial and
nontrivial twist; that (SLOT₂)'s exactness holds on 12,632 instances; that
