...
### Dated convention append (2026-08-15 — MQ, Q1's battery table: the σ-column convention; and one companion finding WITHDRAWN)

*Provenance: corpus-maintenance queue (MQ), acting on the two Q1 battery-table
findings of `spec/EFF-HE7.md` §5.2 (Phase-0a effective-spec compilation; homed in
shard 2 at commit `8e3a583c`, promoted to the merge at `6b447566`). Both were
re-checked against this note before landing: **one reproduces and is documented
here; the other does not reproduce and is withdrawn.** Edit shape append-only; the
body above stays BYTE-FROZEN.*

**(1) REPRODUCES — Q1's three σ columns are keyed to READ-OK members, and the
table never says so.** In Q1's thirteen-row table the columns `{(8,1)}`,
`{(4,2)}`, `{(4,1),(4,1)}` sum to the `members` column on **nine** rows and fall
short on **four**:

| row | s | members | σ-column sum | short by |
|---|---|---|---|---|
| ℤ₃ λ=5/2 twist live | 2 | 124 | 102+10+10 = 122 | 2 |
| F₃[[t]] λ=5/2 | 1 | 79 | 61+13+2 = 76 | 3 |
| F₅[[t]] λ=5/2 | 1 | 57 | 45+7+3 = 55 | 2 |
| F₅[[t]] λ=5/2 twist | 3 | 57 | 49+5+2 = 56 | 1 |

Column totals: members **1,587**, σ-columns **1,579**, difference **8** — and 8 is
exactly the degenerate count this section's `[r1]` re-reading names, distributed
over precisely these four rows (ℤ₃ s=2 → 2, F₃[[t]] s=1 → 3, F₅[[t]] s=1 → 2,
F₅[[t]] s=3 → 1). **So the convention is: `members` counts every member of the
label class; the three σ columns count only the READ-OK members, the degenerate
ones being excluded.** That convention is stated in this note for the **PARI**
column only — "every row has PARI = read-OK members (the one 124/122 row is
exactly the 2 key-divides members, which never reach the oracle)" — and nowhere
for the σ columns, where it also silently applies, including on the four
`F_p[[t]]` rows that have no oracle at all (PARI = 0). A reader who sums the σ
columns against `members` finds four rows apparently short and no stated reason;
the reason is above. **This append supplies the missing convention line. No number
in the table changes, and no verdict, law or grade is affected** — Q1's headline
"1,335 PARI jobs, 1,335 agree, 0 mismatch" is the exact sum of the PARI column
(1,335) and is unaffected.

**(2) DOES NOT REPRODUCE — WITHDRAWN: the alleged "F₅[[t]] λ=5/2 s=2" row
mislabel is not in this note.** The spec's §5.2 first bullet reports that the
`[r1]` degenerate distribution "reads 'F₅[[t]] λ=5/2 **s=2** → 2'" while Q1's
F₅[[t]] rows carry only s = 1 and s = 3. **This note reads `s=1`**: the
distribution line above is "ℤ₃ λ=5/2 s=2 → 2, F₃[[t]] λ=5/2 s=1 → 3, **F₅[[t]]
λ=5/2 s=1 → 2**, F₅[[t]] λ=5/2 s=3 → 1", which is correct against the table (the
F₅[[t]] s=1 row is short by exactly 2, per the table above), and the string
"F₅[[t]] λ=5/2 s=2" occurs **nowhere** in this file at any point in its git
history. The `s=2` exists only inside the spec's own quotation of this passage —
i.e. the compiler corrupted one character while quoting and then flagged its own
corruption as a source defect. **Nothing in this note requires correction on that
count**; the correction is owed to `spec/EFF-HE7.md` (unit `.82`'s "verbatim"
quote, §5.2's first bullet, and §8's item 17) and is routed to the codex
cross-read. Recorded here so the withdrawal travels with the note.

*(A third, spec-local arithmetic slip found while checking (1), recorded for the
same cross-read and requiring nothing of this note: the merge's parenthetical
"`members` and `PARI` columns coincide on nine of thirteen rows, diverging exactly
on the four degenerate rows" is wrong on both halves — they coincide on **eight**
rows and diverge on **five** (the ℤ₃ s=2 row plus all four `F_p[[t]]` rows, which
have no oracle), and that diverging set is not the degenerate set. Shard 2's
original formulation — "`members − (σ-column sum)` is 0 on nine rows and nonzero
on exactly four" — is the correct one and is what (1) above confirms.)*

BYTE-FREEZE: pre-append body md5 2eab5113a3fd622732f82b8ee068c64b, re-verified
by prefix after this append; edit shape append-only; all statement displays,
battery figures and sealed artifacts byte-untouched.
