...
HE6R1-1 itself proves only

\[
\mu_{i+1}\le \mu_i/2,\qquad \mu_i\ge4
\]

at a jump. Thus, for \(J\) jumps,
...
Globally \(n=D'\mu\) with only \(D'\ge1\), yielding

\[
J\le\log_2 n-1.
\]

The stronger \(\log_2 n-2\) bound requires \(D'\ge2\). The supplied \(D'=1,\ n=\mu=4\) example attains \(J=1\), directly contradicting the accepted display’s bound \(J\le0\).

An honest correction must either:

- replace \(\log_2 n-2\) by \(\log_2 n-1\), moving the first possible jump to \(n=4\) and two jumps to \(n=8\); or
- restrict HE6R1-1 to composite-stage starts \(D'\ge2\), leaving initial degree-one events outside its global claim.

Either change weakens the accepted displayed theorem. HEAD’s “Dated acceptance record” says the note is “ACCEPTED (2/2) and FROZEN,” and the present instruction forbids weakening accepted theorem displays. The counterexample prevents proving the existing display, so no permitted honest repair exists.
...
it need not occur at every such index. Indeed, if equality holds, then
\[
ih\equiv D'h\equiv0\pmod{e_1},
\]
because \(D'=e_1f_1\); since \(\gcd(h,e_1)=1\), this forces
\(e_1\mid i\). Conversely, divisibility \(e_1\mid i\) alone does not force
the coefficient to be nonzero or its slot to lie on the line.

For \(i=e_1t\), the slot inequality gives
\[
...
*(The two corrections and two obstruction records above were derived
by the A4 certification run (gpt-5.6-sol high,
runs/cert/A4_he6r1_output.log, 2026-08-12); orchestrator verification
before transcription — finding 1: HE6R1-1's proof re-read: it derives
...
global bound J ≤ log₂4 − 2 = 0 is contradicted by J = 1. Finding 2:
GENHN-ERR-1 located verbatim at HEAD (the S5 box block); the
completion statement re-read — its qualification (ii) NAMES
GENHN-ERR-1 yet asserts the consumed boxes are "none of them an open
piece of mathematics", while ERR-1 is precisely an unsettled
mathematical check (whether GENHN's f₁ ≥ 2 count-side displays
survive the corrected node floor {dv(A_j) ≥ (μ−j)D′h + 1}, which R9
proved and GENHN's own [r1] erratum adopted); no append re-runs the
count argument, and A2's LEMMA GENHN-CAP-GEN (the band projection)
does not touch it — the two residues are disjoint. Finding 3: the
"equality exactly at" span located (the R4 row of the repair table);
the divisibility scoping and the normalized ā_t display verified —
identical mathematics to HE6's own F-3 correction folded at A3,
mutually consistent. Finding 4: P3's table re-read (onset k = λ in
all five frames, threshold λ+1, "sufficient, and by 1 not tight" in
P3's own words) against the §S6 sentence "exactly the closed form" —
the internal contradiction is real and the correction restates P3's
numbers exactly. Transcribed unaltered per the standing division of
labor. Ledger: HE6R1's row → 2 repairs folded awaiting sol
discharge-confirmation; the 2 obstructions close only by re-displayed
statements (owner sign-off) or new mathematics (the D′ ≥ 2 scoping /
log₂n − 1 re-display decision for HE6R1-1; the f₁ ≥ 2 count re-run
lemma for GENHN-ERR-1).)*
...
discharge-confirmation in the cert2 pass). R7 finding 2 is NOT closed
