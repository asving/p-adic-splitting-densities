# CU-1 (K-RUN-C composition, unit NM) — no-missing + interior adequacy + state binding — verify brief, REV 3 (Fable, 2026-07-31)

(Companion record: `CU1_phaseB_attempt_rev3.md`. This is REVISION 3.
Rev 2 was the STRUCTURAL repair of the V27 hostile verification of rev 1
(`V27_cu1.jsonl`, verdict UNSOUND); its disposition table is kept verbatim
as §0. The V29 hostile verification of rev 2 (`V29_cu1r2.jsonl`, verdict
SOUND-WITH-GAPS) CONFIRMED the rev-2 structure: the interior/terminal split
PASSES ("neither a gap nor overlap"; its μ = 1 leaf half explicitly
conditional through O5triple, admitted in the text), the dressing
construction PASSES ("creation-versus-recovery confusion is repaired"),
the read-back's GLOBAL quantification is sound, the induction is N-free
("no box, decode, D-14, REAL≡, or N\* leaks into §3"), and the width route,
the record-only non-saturation step, and the top-site D-14 application are
coherent. Every remaining V29 finding is a CONSUMPTION-ACCOUNTING defect:
§5's map failed its own exactness claim at five named points, and §4's
SURJ dependency sentence understated the walk clause's inputs. REV 3
REPAIRS EXACTLY THOSE (disposition table: §0-R3). The §3 induction, the
§2 statements and proofs, §4's constructions and split, §§6–7 are
byte-identical to rev 2; no input is ADDED anywhere — two listed
consumptions are DROPPED as unconsumed (OL-2 clause (a); O-2a Thm 2(A)),
one consumption site is RECORDED (ADEQ's W4 use of the (L3) fact displayed
in the OL-3 note), and six inputs the text already consumed get explicit
rows ((L2), (L3), Def 2.21, (C1), the W-laws, [F.2]). The primary gate
(depth-3 falsifier, §7) ran before rev 1 and SURVIVED; rev 3 adds no
mechanism and no input, so no new gate is owed — rev 2's own mapping §7.4
carries.)
...
> (f, c). **Status:** at order 1 (root states under (P-ROOT)) this is
...
value), dressing := ψ̂ — fields all present on the harvested record via
(FR≡)'s emitted-record clause. Theorem CU-1 produces, for each admissible
𝔈-chain 𝐇, a CONSTRUCTED machine history M_𝐇 with π(M_𝐇) = 𝐇.
...
(M6a)–(M6c) at their grades, the §1 box/decode display, and (N\*-MAJ)
whenever the level is taken from N\* (rows 7–11, 15, 16, 18, 22; nothing
...
| 6 | Lemma R | §2 scope note (3) + §4 canonicity remark ONLY — produces and identifies NOTHING | PROVED (O-2a) |
...
| 21 | **(FR≡)** | step 1, 2, 3(d) (polygon/residual/record identification); clause (ii) frame clause | order 1: PROVED ( = (D3) by inspection); order ≥ 2: DECLARED pin, OPEN (R-6); slot-fresh reading REFUTED by §7.2 |
...
per V29 finding 4): every input consumed by any sentence of §§2–4 appears in
a row above. Rows 7, 17, 18 cover O-1thr's definitional vocabulary (states,
windows, halts, READ/REALIZED, harvesting, π's machine side) and the PROVED
key-degree identity; rows 24–29 (NEW at rev 3) cover the O-2a-side
definitional and literature vocabulary that the text — §4's checks above
all — consumes by name ((L2)/(L3)/Def 2.21, the (C1) carrier with its
W-laws, [F.2]). (Rev 2 asserted rows 7/17/18 covered "the two home briefs'"
vocabulary, which misassigned the O-2a side — V29.) The remaining rows are
the substantive citations, each with its locations audited against the
text. In particular: OL-2 is consumed ONLY at clauses (b)/(c)/(d) (row 2 —
clause (a) is consumed NOWHERE); O-2a Thm 2(A) is consumed NOWHERE (dropped
at rev 3 — row 5); NO OL-4, OL-5, OL-6 (dropped at rev 2 — §0); NO
...
CU-2 and is not consumed here at all).
...
7. **CU-1-LVL:** is ONE D-14 application at the top site really sufficient
   ...
   L(M) ≥ every per-read ceiling by Lemma CEIL)? Is f_N's monicity/degree
   display faithful to O-1thr §1?
...
9. **CU-1-SURJ:** is the global statement really the displayed ∀-composition
   ...
   rows 7–11, 15, 16, 18, 22) for the walk clause, and NOTHING more (no
...
— CU-1 rev 3 complete. PROVED-UNVERIFIED (conditional composition, §5
grades) until hostile passes accept it; V29 on rev 2 = SOUND-WITH-GAPS, all
gaps consumption-accounting, repaired above (§0-R3); primary gate SURVIVED
(§7, carried); rev-2 falsifier accounting §7.4 carries (rev 3 adds no input
and no mechanism).
