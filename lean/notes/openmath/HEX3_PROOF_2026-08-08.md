...
0/2** (composed; no hostile arc yet). **Instrument:** sealed two-commit
...
2^{12} + 1504 + 64 (k = 1 and k = 2 terms both alive). The committed
...
* **(β3) = VERT2(u₀,t)-⟨1²⟩**, 1 ≤ t, u₀ > 3t, u₀ ≡ t (mod 2),
  ...
  k := t + j... (u₀−t)/2, interior lattice point (1, (u₀+t)/2);
...
(3,0)), and (3−j)(N−1)/3 < N−(3−j)k for k ≤ (N−1)/3 — so consulted
digits sit strictly below the ghost zones {≥ N−2k} (B₂), {≥ N−k}
(B₁). Updates: shift3 needs B₁ mod π^{N−v(s)} ⊆ mod π^{N−k−1} and B₂
...
* **HEX3-BOX-2 (attempt grade; the transports' ledger detail).** 0/2 —
  composed this session, no hostile arc. The window-coherence and
  ...
  consulted-height bound at the k = ⌊(N−1)/3⌋ corner.
...
* **P-1 [the law]** HEX3-LAW 0 violations on all 46 rows: measured
  ...
  (9,3): 59,049; (11,3): 161,051; (13,3): 371,293. (The N ≤ 3 entries
...
2. **The hExhaust program** — n = 2 exact (W-11), n = 3: DBL term
   ...
   normalized head U₃^conv/q^{2N} = q^{−1}(1 + o(1))·(1 + Θ(N q^{−3}))
   — NOTE: u has an N-linear subleading family (the R-tails), so the
   conservative residue is Θ(N·q^{2N−4}) above its head — consistent
...
* P-1 ✓ [the law] HEX3-LAW 92 checks / 0 — measured U₃^conv = q·u(q,N)
  on ALL 46 rows. Every one of the ELEVEN never-measured (q,N) hit its
  preregistered value EXACTLY: (2,8) 50,176; (2,9) 206,336 (16.7M
...
bound", (hExhaust-3)) is DISCHARGED at attempt grade 0/2 (the hostile
arc has not begun); the σ-residue exact law remains open exactly at
