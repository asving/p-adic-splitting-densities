...
* **P-1 [the law]** HEX3-LAW 0 violations on all 46 rows: measured
  ...
  11-set~~ [r2: itself one pair short — see the r2 bracket below]
  of (q,N) pairs first measured by the sealed battery, with
  ...
  correction.]** **[r2, PE2 F1: the [r1] census is ITSELF miscounted
...
* P-1 ✓ [the law] HEX3-LAW 92 checks / 0 — measured U₃^conv = q·u(q,N)
  ...
  never-measured, force.] **[r2, PE2 F1: the honest never-measured
  set has TWELVE pairs, not eleven — (8,3), omitted by the [r1]
  census too, was measured 32,768 = 8⁵ in this very run (it is a
  roster row) and INDEPENDENTLY by the PE2 verifier's own enumerator,
  hitting the law exactly; the full corrected census with its basis
  is the S7 P-1 r2 bracket.]**
...
**[r2] arc cont.: → PE2 NOT CLEAN 0C+1G+1m @ f60348b (every
theorem-level claim and both r1 mathematical repairs re-derived and
CONFIRMED; machine leg bit-reconciled GREEN 158,512/0, seal
byte-intact; fresh route: polynomial-identity proof of the
telescoping for N ≤ 40, own W-11 block reader — R(M) 39 cases both
parities — and own-field-table enumeration incl. the recovered
(8,3) = 32,768 and a first-ever q = 27 row, 19,683 = 27³ exact; the
gap was the [r1] census itself, one pair short) → r2 THIS REPAIR
(F1 the census 11 → TWELVE + m1 the S8.2 k = 1 factor; note-only,
[r2] tags, minimality contract; runner + artifacts byte-frozen at
seal e3f3459); counter 0/2; PE3 next**
**[r3] arc cont.: → PE3 CLEAN 0C+0G+0m @ fca1a97 (FIRST clean pass
of the arc: the twice-burned census audited from primary sources and
FINALLY RIGHT at twelve; β₁ route re-derived in full + letter-
resolution fibers machine-checked; fresh route: renewal-GF series
inversion + nine never-instantiated DVR types incl. first-ever
q = 25/49; counter 1/2) → PE4 NOT CLEAN 0C+0G+1m @ 34a946b
(cross-note verdict POSITIVE: the (hExhaust-3) discharge SURVIVES
W-12's r3 σ-pin (b17238a) and is parse-ROBUST — U₃^conv =
(σ-undecided TRP) ∪ (certified TRP boundary tails) exactly, HEX3.B
dominating both r(3,N) TRP legs however BOX-1 resolves; α-route
re-derived from scratch; machine leg fresh full run GREEN exit 0
158,512/0, runner md5 == seal e3f3459; fresh route: partial-fraction
GF with structural minimal denominator + six further never-
instantiated DVR types, all law+species exact; the finding: the S8.2
aside's W-12.B consistency clause asserted BOX-1's open lemma as
fact — counter RESETS 0/2) → r3 THIS REPAIR (m1 the S8.2 consistency
clause conditionalized; note-only, [r3] tag, minimality contract;
runner + artifacts byte-frozen at seal e3f3459); counter 0/2; PE5
next**);
...
### Dated corrections (2026-08-09, post-PE6)

*The rider owed at the wave-16 adjudication (ledgered in
`lean/notes/BRIDGE_ADJUDICATIONS_2026-07-30.md`, wave-16 fold @
91ec856: the charge bar — nothing GAP-or-worse = CLEAN — is the
standing convention; HEX3's clean count HOLDS AT 1/2). Discipline:
dated append only — the sealed body above (lines 1–832) is
byte-untouched by this rider; runner + artifacts stay frozen at seal
e3f3459 (runner md5 83faf7edb554811cd657691d7ea256cc, results JSON
ce6f41162a22b0bd66a641e98cb4d56e); the PE6 report is byte-frozen at
md5 42d86c7aeb3f5652657d44d6608fb247. Two corrections, one per PE6
MINOR (`HEX3_passPE6_report.md` @ d9c5ef8).*
...
### Dated acceptance record + corrections (2026-08-09, post-PE7)
...
### Dated corrections (2026-08-12 — sol certification A0: the ratification annex folded)

The sol-5.6 chain ratification of this note (gpt-5.6-sol high,
runs/residues/R5_hex3_output.log, 2026-08-12) returned
RATIFIED-WITH-ANNEX: the theorems, battery, and discharge stand; four
annex minors owed. The A0 fold run (gpt-5.6-sol high,
runs/cert/A0_hex3_output.log) re-adjudicated all four against HEAD:
all four remain REAL defects, each minor and locally repairable, none
changing HEX3.A/B/C or the discharge. The four corrections below fold
the annex. Everything above this heading is byte-untouched by this
append.
