...
**[r2] S-STATUS (CURRENT): attempt grade, acceptance counter 0/2 (ZERO clean
passes), TWO hostile passes RUN and TWO repair rounds applied. Arc: composed
at 5463f2a → W2-PE1 (`GRTW2_passPE1_report.md`, 083adff — 0 CRITICAL + 2
JUSTIFICATION GAPS + 5 minor, NOT CLEAN; core mathematics CONFIRMED by full
independent re-derivation; machine leg bit-identical, 6,403/0, exit 0) → r1
(8d5e3fa, NOTE-ONLY: the W2-C3 HONEST RE-SCOPE — proved to each chamber's
first ξ ≠ 1 junction, MEASURED above it — with the open box W2-OPEN-2 after
a four-route derivability adjudication; the W2-C1a chamber qualifier; minors
F3–F7; round block §7) → W2-PE2 (`GRTW2_passPE2_report.md`, 79e796b — 0
CRITICAL + 2 JUSTIFICATION GAPS (P1 moderate, P2 minor) + 1 minor (P3), NOT
CLEAN; the four-route adjudication HELD at source under a deliberate
fifth-route hunt; machine leg re-run byte-identical; fresh leg: the §3.4
closed form evaluated against the committed ξ-ledger by from-scratch
arithmetic, 16/16 at m = 1 plus hand T3B/T3F r = 4) → r2 (this round,
NOTE-ONLY: **P1** — the re-scope's boundary identification "first recorded
ξ ≠ 1 junction = first byte-divergence" made honest: its m = 1 case PROVED
and displayed as LEMMA W2-ID1 (§5.2), its m ≥ 2 case folded into W2-OPEN-2
as that box's own premise sub-clause (OPEN-2a), and the general grade
sentence scoped to the instance census (recorded ξ ≠ 1 ⟺ byte-RED at 22/22
walked junctions; every below-r₀ junction byte-GREEN — PE2's recomputation);
**P2** — the [r1, F6] §4.3 sign flip REVERTED: the r0 sign t ↦ t − k·u_i is
CORRECT under tex 1431's literal Def-`t(i)`, and the r1 bracket (like PE1's
F6 before it) rode W2-L2's r0 naming conflation of Def-`t(i)` with
Prop-`construct`'s child exponent (tex 1201) — parenthetical split at §2.3;
**P3** — W2-OPEN-2's premise pairing corrected to φ_q^tw ↔ Φ_{q−1} per the
§1.2 dictionary; round block §8). A repair round is NOT a pass, so the
counter stays 0/2; W2-PE3 — a fresh hostile pass on THIS post-r2 text — is
the next acceptance attempt. The runner `grt_w2_checks.py` (sha256
71f8fc90f7543fd3…, PE2-verified byte-identical across working tree, 5463f2a,
and HEAD) and both artifacts remain BYTE-FROZEN; r2 touches only this note.**
...
§5.2) and at m ≥ 2 is W2-OPEN-2's own premise sub-clause (OPEN-2a) — on the
...
the premise sub-clause (OPEN-2a) of the box below. [Machine: instance-exact
...
q < r₀ and byte-DIFFERENT for r₀ ≤ q < r —
...
closes the m = 1 slice of that identification.**
...
* **P1 (moderate gap) → the boundary identification made honest.** The
  ...
  rk. 3's byte law). (b) The m ≥ 2 case is folded INTO W2-OPEN-2 as its
...
STALE** — W2 now has post-5463f2a commits (8d5e3fa r1, this r2). The pin
