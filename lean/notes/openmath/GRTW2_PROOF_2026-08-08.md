...
### Dated arithmetic append (2026-08-15 — MQ, §6.1's 6,403 headline: RECOUNT SETTLES IT)

*Provenance: corpus-maintenance queue (MQ). Finding of record: `spec/EFF-GRTW2.md`
**source defect 4** (Phase-0a effective-spec compilation; merged at commit
`7ce478e2`, which recorded the gap as "not reconcilable from the displayed table"
and carried it as an MQ-candidate row). This append closes it: the gap IS
reconciled here, from the sealed artifacts. Edit shape append-only; the body above
stays BYTE-FROZEN.*

**THE DISCREPANCY.** §6.1's header says "0 violations, **6,403** samples" (as do
the four Freeze paragraphs of r1–r4 and the S-STATUS line); §6.1's own sample
column sums to **6,396** (4,396 + 156 + 156 + 55 + 44 + 16 + 22 + 28 + 44 + 44 +
44 + 684 + 684 + 12 + 7 + 2 + 2). Gap: 7. The family count (17) reproduces exactly.

**THE RECOUNT (this append's own work, against `grt_w2_checks.py` and the sealed
`grt_w2_checks_results.json`).**

1. Re-summing the sealed JSON's own `families` map over all 17 declared families
   gives **exactly 6,396**, in per-family agreement with §6.1's table. The table
   is a faithful transcription of the instrument's per-family counters.
2. The `6,403` is likewise faithful — to the instrument's printed `TOTAL samples:`
   line, which computes `sum(COUNTS.values())` (`grt_w2_checks.py:946`–`947`),
   i.e. it sums **every** key of the `COUNTS` dict.
3. `COUNTS` carries two non-family bookkeeping keys, both written by the same
   `note()` helper the family counters use: **`CK-XI-BOUNDARY`** (the recorded,
   deliberately-unscored monomial-collapse boundary junction) and
   **`CK-REC-SKIP`** (`grt_w2_checks.py:523` — bumped once per junction where the
   two chains do NOT share key bytes below the junction, so CK-REC is skipped
   rather than scored).
4. Their values on this run: `CK-XI-BOUNDARY` = **1** (the sealed JSON's
   `boundary_junctions`, = the "1 recorded boundary = I4B r=5" this section's
   CK-XI row already names); `CK-REC-SKIP` = **6**, independently recovered from
   the sealed JSON by counting junctions with `shared_below: false` — exactly six
   (`Zp,p=2,T3A` r3, `Zp,p=3,T3C` r3, `Fpt,p=2,T3D` r3, `Zp,p=2,T3E` r3,
   `Fpt,p=2,I4C` r3 and r4), against 22 with `shared_below: true`.
5. **1 + 6 = 7, and 6,396 + 7 = 6,403 exactly.**

**VERDICT — the table is right; the headline over-counts by 7 non-samples.** The
6,396 is the true number of scored samples across the 17 families. The 6,403 is
the runner's own total, inflated by two bookkeeping counters that count
*deliberately-unscored* events: one recorded boundary junction and six
CK-REC-skipped junctions. Notably these are the very events this note documents as
NOT scored — so the headline silently re-admits into its sample count exactly what
§6.1's CK-XI row and the r1/F1 scope split exclude. Neither figure is a
transcription error by this note; the defect is instrument-side, in the conflation
of sample counters and bookkeeping counters inside one `COUNTS` dict.

**Nothing in the note's fences moves.** All 17 families are GREEN with 0
violations at every per-family count, the four r1–r4 re-runs remain
bit-identical, and no claim, box, grade or scope decision reads the headline
total. Operative figures: **6,396 samples across 17 families**, plus 1 recorded
boundary and 6 skipped CK-REC junctions.

**Corpus-wide rider (recorded, not repaired).** The
`sum(COUNTS.values())`-as-total idiom appears in **24** runners under
`verification/openmath/`. The sibling case is confirmed with the identical
mechanism: GRTJB §7.1's 6,700-vs-6,575 = 125 JB-CREAD census rows (see that
note's 2026-08-15 MQ append). Any headline sample total quoted from a runner's
`TOTAL samples:` line should be re-derived from its per-family counters before it
is cited.

BYTE-FREEZE: pre-append body md5 1f548ce6f0961bfd86f5af1303a212a8, re-verified
