# Verification brief O14ap — realizer existence at recentering nodes of coherent stage towers
...
### 5.4 The corrected (faithful) support law, with proof

Setting: the intended tower. Stage σ_r has key Φ_r produced from the previous key
Φ_{r−1} by a read of data (e, h, g) := (e_{r−1}, h_{r−1}, g_{r−1}) and descend
polynomial ψ := ψ_{r−1} with root z̄ generating K_r = K_{r−1}(z̄) = K_{r−1}[z]/(ψ), a
degree-g extension. Coefficients t ∈ C_{Φ_r} are developed in the PREVIOUS key:
t = Σ_j B_j·Φ_{r−1}^j, slots j < e·g, deg B_j < deg Φ_{r−1}. "Parent scale" means the
wPrev_r-scale. The *forced class* of a target weight W is the unique
j₀(W) ∈ [0, e) with h·j₀ ≡ W (mod e) (well-defined: gcd(e, h) = 1). Two standard
facts of the intended apparatus are consumed: (F1) the slot positions of the
progression j₀, j₀+e, …, j₀+e(g−1) in the previous stage's residual variable are
consecutive: position(j₀ + ek) = m + k for a fixed offset m = m(W); (F2) the previous
stage satisfies its own threshold inequality wPrev_{r−1}(Φ_{r−1}) < h/e ≤ h (this is
the inequality proved in Lemma 3.1 one stage down; at the base stage realizers are
p-power multiples of unit lifts and no threshold is needed).

> **(S6b′-SUPP).** For every parent-scale weight W with W > wPrev_r(Φ_r) = e·h·g and
> every c̃ ∈ K_rˣ there is a realizer t of digit scalar c̃ at weight W whose
> Φ_{r−1}-slot support is contained in {j₀(W) + e·k : 0 ≤ k < g}, and whose occupied
> slot coefficients are, recursively, (S6b′-SUPP)-realizers one stage down at the
> integral target weights W_k := (W − (j₀+ek)·h)/e, down to the base (where realizers
> are p^m·û, û a unit lift, support {0} for a linear base key).

*Proof.* On the forced progression the W_k are integers (choice of j₀); off it no slot
can contribute at weight W (weights of slot j are ≡ j·h mod e). Let P ∈ K_{r−1}[z] be
the unique polynomial of degree < g with z̄^{m(W)}·P(z̄) = c̃ (the powers z̄^{m}, …,
z̄^{m+g−1} form a K_{r−1}-basis of K_r, being a unit multiple of 1, z̄, …, z̄^{g−1}). Set
t := Σ_{k<g, P_k≠0} t_k·Φ_{r−1}^{j₀+ek} with t_k a lower-stage realizer of scalar P_k
at weight W_k. *Threshold clearing:* W_k ≥ (W − (eg−1)h)/e > (ehg − (eg−1)h)/e = h/e >
wPrev_{r−1}(Φ_{r−1}) by (F2), so the lower-stage (S6b′-SUPP) applies; the recursion
terminates at the base. *Weight:* every summand has parent-scale weight
e·W_k + (j₀+ek)h = W, so wPrev_r(t) = W (slot-minimum over equal values, at least one
slot occupied: P ≠ 0 since c̃ ≠ 0). *Digit:* by (F1) the slot digits assemble as
z^m·P(z) in the previous residual variable; reduction mod ψ and evaluation at z̄ (the
passage to the new residue field) give the scalar z̄^m·P(z̄) = c̃; at a recentering
consumer (stage pair e = 1, t-Bézout 0) the position is 0, so R_r(t) = C(c̃)·T(0),
matching (CR3). *Support:* clauses are the literal index set and the recursion. ∎

*Tightness.* §5.2 also shows the progression length g is sharp: single-slot realizers
have digit scalars confined to one coset K_{r−1}ˣ·z̄^{const}, and c̃ = (1+i)·i^m needs
P = (a unit multiple of) 1 + z — two slots. So the faithful "monomial support" law is a
forced-class PROGRESSION law (one graded monomial per slot per level), keyed at the
FRAME-CREATING read's (e, g); the compiled (CR2) keys it at the recentering node's own
(e, g) = (1, 1) — the refuted reading.
...
3. §5.4: a valid proof of (S6b′-SUPP) given the two consumed facts (F1)/(F2), each
   flagged; and the tightness claim.
