# CU-1 (K-RUN-C composition, unit NM) — no-missing + interior adequacy + state binding — verify brief, REV 2 (Fable, 2026-07-31)

(Companion record: `CU1_phaseB_attempt_rev2.md`. This is REVISION 2,
repairing the V27 hostile verification of rev 1 (`V27_cu1.jsonl`, verdict
UNSOUND). V27 CONFIRMED: step 1's IH(ii)/OL-2a-1/Theorem-2(A) use ("as
advertised"), the k = 1 corner of the saturation range ("not the problem;
no k′ = 0 use is necessary"), the absence of a literal same-index IH circle
in the record match, and the per-prefix sufficiency of the majorant property
for the D-14 ceilings. Those verdicts are honored; everything else was
findings, and rev 2 is a STRUCTURAL repair — the disposition table is §0.
The primary gate (depth-3 falsifier, §7) ran before rev 1 and SURVIVED;
rev 2 adds no new per-site mechanism, so no new gate is owed — the mapping
of rev 2's newly DECLARED inputs onto the existing gate is §7.4.)
...
finding 3c); 2(A)/(B) remain cited at the base and in §4's split.
...
which understated it. Its dependencies are exactly ∀-instances of Theorem
CU-1 and Corollary CU-1-ADEQ (induction on path length; no input beyond
theirs). Non-claims, displayed: no leaf statement (the split above), no
...
| 2 | **FULL OL-2 (a)–(d)** (GD-2) | step 2 ((b): whole-polygon identity — the width tie's semantic half); step 3(d) ((c)/(d): emitted side fields + positional transport) | OPEN; the full form is advertised at home to exactly this consumer (rev-5 §4 "the O-3 design seam") — rev 1 under-cited OL-2-min, repaired |
| 3 | **FULL OL-3** (GD-3) | steps 2, 3(d) (residual identity ⟹ factor/multiplicity transport) | OPEN; same advertisement (shared with the O-9 leaf) |
...
| 5 | O-2a Thm 2(A)/(B) | ADEQ/split ONLY ((A): placement/non-completeness vocabulary; (B): complete ⟺ μ = 1 for the leaf split). NOT consumed in the step (rev-2 drop) | conditional [Cons_f + OL-1], displayed |
...
Exhaustiveness claim (precise form, V27 finding 4): every input consumed by
any sentence of §§2–4 appears in a row above, where rows 7, 17, 18 cover the
two home briefs' DEFINITIONAL vocabulary (carriers, W-laws, states, windows,
halts, READ/REALIZED, π) and the PROVED key-degree identity; the remaining
rows are the substantive citations, each with its locations audited against
the text. In particular: NO OL-4, OL-5, OL-6 (dropped at rev 2 — §0); NO
...
9. **CU-1-SURJ:** is the global statement really a pure ∀-composition of
   CU-1 + ADEQ (no hidden tree object, no leaf claim, no box claim)?
...
— CU-1 rev 2 complete. PROVED-UNVERIFIED (conditional composition, §5
grades) until hostile passes accept it; primary gate SURVIVED (§7, carried);
rev-2 falsifier accounting §7.4.
