# BRIDGE CAMPAIGN — ORCHESTRATOR ADJUDICATIONS (2026-07-30, pre-Codex-review)
BP1: Q1 Architecture B ratified. Q2 ramIdx/resDeg repair path signed off CONDITIONAL on
IB-E0's compiled countermodel (fence event, recorded; Asvin digest). Q3 hdet+hStateNe
ratified as named premises (hStateNe derivation attempt first). Q4 boundary confirmed:
UCarriers/KernelCarriers/Cl7Kernel/UpstreamTyped owner-corpora; SolveData + KT wiring
owned by BP1. Q5 proceed now, statement-only, Codex audit before consumers. Q6 keep as
kernel rows; S-area proves at real pack later. Q7 count_tie stays a named row.
Q8 overflow units pre-authorized (2-4).
BP2: Q1 repair candidate (S-a) ratified, gated by HK-05 (refutation stall) + HK-22
(compiled 2-node instance) before any consumer. Unadjudicated BP2 questions: provers
must surface, never improvise.
BP3: Q1 E5 hoist ratified. Q2 named StableInputs hypothesis (not KBTotTower fields).
Q3 G1 ruling waits on TV-C1; HistoryCoherent is BP2-EXCLUSIVE. Q4 W17ii: summability
guard preferred; a PLAUSIBLE countermodel does NOT suffice for a statement change —
blocked TV-H1 => hoist to named hypothesis. Q5 shared witness allowed; per-cell
cp.count wiring. Q6 DELETE the CtsmConclusions tautology fields (fence event, digest).
Q7 extend toy if cheap, else census-backed record. Q8 extraction-lemma route first.
Q9 priority order confirmed.
BP4: Q1 Kernels/ mini-corpus ratified + lakefile/AxChk sweep entry mandatory. Q2 prove
over the transcribed enum model + seam unit. Q3 N3 carriers DEFERRED. Q4 confirmed:
hypothesis/carrier shape only, never new fields on frozen structures. Q5 annotate at
discharge. Q6 probe units count, seal discipline applies. Q7 instance-vs-interface
adjudication queued now. Q8 priority confirmed.
BP5: Q-1 decide on CL-01/CL-04 reports. Q-2 compiled countermodel preferred; Lean-core
certificates suffice with record if cost insane. Q-3 VARIANT A. Q-4 typed-only-with-
record. Q-5 quarantine-with-record. Q-6 bank NOTHING beyond the three conjuncts (the
scaffold lesson). Q-7 yes, quarantine the fifth file. Q-8 hygiene quarantines execute
immediately.
...
## R11 DESIGN REVISION 2 ON FILE (2026-08-04, in place; the 5 criticals + finding 9 folded)
...
## R11 DESIGN REVISION 3 FOLD (e8f50e8, 2026-08-04)
Evidence pass done: (K1-NSF) SPLIT — proved at the interface pin (q-1..q-6
displayed) BUT the as-built Lean keying violates the pin (the M02 count_tie
refutation, on file) => named obligation (NSF-KEY) (= M02's sanctioned
minimal repair, MovesT.NsFree conjoined; executes at Unit C under the
definition-change authority WITH Codex diff confirmation); Σ=1 exhaustive
...
## ITER-LAW hostile pass PE2 (Fable, fresh-context, execution + content) — CLEAN (2026-08-03)

Pass 2 of the post-errata pair on the AMENDED ITERLAW_2026-08-08.md (errata
commit 7566ccd); report `lean/notes/openmath/ITERLAW_passPE2_report.md`.
**FINDINGS: 0 critical, 0 gaps — VERDICT: CLEAN.** (1) ERRATA FIDELITY: the
four brackets (S0 / S6.4 / S8 / S9) are additive — note diff 242ba1e..7566ccd
= +49/−1 where the −1 is a pure text-preserving line split around the S9
cross-ref bracket (both fragments retained verbatim); S2–S5 zero changed
lines; S6 zero modified/deleted lines (S6.4 bracket purely appended); the PC1
CRITICAL is cured — no unqualified verbatim-§S2-match claim survives (grep
sweep: :81–82 original immediately struck by the :85–98 bracket; other
"verbatim" hits are D-REAL/value-set claims, correct); the DITER_RESTATE:33
`E₂ := e₀e₁ > 1` pin verified against the file. (2) SEAL: iterlaw_check.py
md5 bfd5ef71257a79aee827b516fccfc03d == ab7c31c blob, diff-to-HEAD empty;
re-run exit 0, 35,232/0, ALL family counts == sealed JSON (F1 468 · F2 2,604 ·
F3 2,604 · F4 516 · F5 4,108 · F6 2,604 · F7 156 · F8 1,736 · F9 20,000 ·
F10 4 · CERT 432), Case-II coverage MET, F10 4/4 exact value sets, results
JSON regenerated byte-identically. (3) SUPPLEMENT read hostilely: all four
mixed blocks + W4 both ways + CF-at-actual-mixed-args + CERT Y/Z confirmed in
code; ROWS/closed_form/windows transcription verbatim vs the seal, no semantic
divergence; sample arithmetic reconciled to the digit (Σ E₂² = 381/ring/block);
re-run exit 0, 18,396/0 (M1–M4 1,524/0 · M5 3,048/0 · CF 9,144/0 · CERT 108/0,
PARI fired). (4) F2c reimpl re-run exit 0, 89,056/0, mutations 4/4 DETECTED,
duties 5/5 MET (seed 20260803 confirmed). (5) FRESH CONTENT READ: L1–L7 + EXP
re-derived by hand; Case I/II partition exhaustive (e₀=1 in Case I; Case II's
e₁=1 on-grid A₁ leg correctly reads 0 — strictly above the line); corollaries
1–5 + S6.6 check; W-MULT recovered-not-consumed confirmed (S4 is direct
development bookkeeping); S7 residuals honest; corrected §S2 relationship
(coincidence-on-the-nose at E₂>1, harmless E₂=1 extension) accurate. NOTES
(non-findings): the fenced DITER_RESTATE §S2 phrasing correction stays OWED at
...
## WELDMASTER PE1: 0C / 5G+3m — WM-COB and PERIM-μ re-derived SOUND on their exact footprints; the gaps are freshness + the same-computation species jumping notes — 2026-08-06

**WELDMASTER PE1 (report df00707) — VERDICT 0C/5G+3m; counter 0/2.**
SURVIVED: WM-COB(i)/(ii)/(iii) + WM-RHO re-derived sound on exactly the
claimed footprints ([ILN] S2 "γ,γ′ ∈ ℤ arbitrary" verbatim); WM-CLASS
honestly display-graded, consumed nowhere; the face table CURRENT at HEAD
(the JB r4 leaves re-scope touches no cited face; (DMULT) two-tier and
(VD-m) m ≤ 1 carried verbatim); PERIM-μ (a)/(b)/(c) exact; machine leg
bit-identical, seal verified. GAPS: F-1 the JB NO-LEG frontier carried as
"rows 5/12/22" at 3 sites — STALE BY CONCURRENCY (JB r4 recounted to four
WHILE the composer sealed; and JB r5, in flight, may shrink it again via
the u ≥ 0 discharge — **r1 therefore WAITS for JB r5 to land**); F-2
WM-SHEAR billed as "M1 re-key" is SAME-COMPUTATION (Hpts ≡ shearU via the
engine's own recursion) — the species' FOURTH appearance, now jumping
notes after being taxonomized: **standing rule minted: every machine
family must carry an evidence-class label (IND / SAME / NONE) at its
prereg row, and "re-key"/"independent" claims require naming the
decorrelated computation path**; F-3 the M2 cell's "all ... are values"
vs the note's own χ-chain fence (verb → "generated by"); F-4 JA-EPS cited
at off-window arguments (correct carrier [ILN] EXP-KIT, uncited); F-5 the
stack omits the (ITER-LAW-LIFT) residual + [T] pins that [JC]'s own stack
...
## ★★★★ (hMenu-3) DISCHARGED, NO BOXED FAMILY — W-12.D FIRES AT n=3: THE EXACT CUBIC DENSITIES (one R_τ ∈ ℚ(q) per type, both characteristics, wild p=2 AND p=3 included, Σ=1 exactly) — CUBICS = THE SECOND COMPLETE DEGREE AT COMPOSITION GRADE — 2026-08-08

**HMENU3 ADJUDICATED** (seal 64d3ff9, verdict 625fa49; attempt 0/2):
the complete (A1) menu at n=3 = 33 aggregate families (3 SEP + 4 DBL +
26 TRP), history-resolved keys generated by ONE recursion (187 realized
at N=9, every one at its closed form). All four clauses PROVED: (a)
exhaustiveness (LEMMA HM3-1 two-phase chain — α self-similar + β
one-time n=2 re-entry, EXACTLY W12-BOX-2's two species, nothing else);
(b) exact counts (HM3-2/3/4 leaf-resolved transports over HEX3 +
W12-L1(b)); (c) disjointness (structural); (d) q-uniformity (one
polynomial per family, both characteristics). Only open residue =
inherited HEX3-BOX-1 (U₃^σ exact law), PROVED off W-12.D's critical
path (§S5.4). COROLLARY (conditional on arc grades W-12/HEX3/HMENU3
all 0/2): with Φ = q⁴+q³+q²+q+1 — R_split = q³(q²−q+1)/(6(q+1)Φ);
R_{(1,2)(1,1)} = q³(q²+q+1)/(2(q+1)Φ); R_{(1,3)} = q³(q+1)/(3Φ);
R_{(2,1)(1,1)} = q(q³+q+1)/((q+1)Φ) [wild p=2]; R_{(3,1)} = (q²+1)/Φ
[wild p=3]; Σ = 1 EXACTLY; same expressions both characteristics.
Triple-tied (symbolic; om_density_engine β(2)/β(3) BCFG-reconciled;
exact partial sums). Battery 183,387/0 over 94,824,902 states, 3/3
never-measured rows exact (first q=27), PARI 9,952 deep-decided incl.
972 wild p=3 deep-RAM3 (first independent deep-TRP σ-check), teeth
21/15/10/4 exact. Annex drafts for W12-BOX-2 + HEX3 §S8.2 in §S8.1.

**W-12 PE2 ADJUDICATED (1be15d2): 0C + 3G + 2m, counter 0/2, r2 owed**
— the A2/conv-vs-σ HEADLINE CHECK CLEAN (HEX3.B carries the σ-side
transfer; no surface overstates); G1 oracle cover cite still
incomplete (a≡b mod p sub-genre; conservative convention = the true
warrant); G2 the S3.4 r(N) line undercounts by the transported DBL
SPLIT-TAIL boundary families (one line from W12-L0×L1(c),
undisplayed); G3 m_i integrality implication false-as-stated
(conclusion stands). All display-level.

**r40b CURED (e92d5b4)**: 3/3 arc-count sites + 11/11 filed tokens +
7 sweep extras (18 dated brackets), standing 4th-recurrence
compose-time-grep flag added; VC40b owed; acceptance citation stays
REVISION 38 + r39b.

**THE n≤3 CHAIN, restated: W-12.D (proved) ∘ W-11 (n=2) ∘
HEX3+HMENU3 (n=3) — composition COMPLETE; the owed surface is the
HOSTILE ARCS (all at 0/2: W-11 PE3 next, W-12 r2→PE3, HEX3 PE1,
HMENU3 PE1, SIGMALAW PE1) + the general-n program (the HM3-1
