# MovesR Lean blueprint REV 3 — Codex FINAL CONFIRMATION pass (2026-07-28)

Prompt = CAMPAIGN_AUDIT_CHARGE_2026-07-28.md (§R-LEDGER instantiation, statements-only note, final-confirmation addendum) + MOVES_2026-07-24.md lines 9290-9763 (§R-LEDGER, rev 5) + MOVES_2026-07-24.md lines 3700-3760 (Lemma LST source display) + MOVESR_LEAN_BLUEPRINT_2026-07-28.md rev 3 (1058 lines, full text). Verdict: REJECT (15 crit / 5 gap).
...
session id: 019fa3f7-a9ba-7290-8a16-e5f984cc0118
--------
user
...
NOTE: this corpus is statements-only by design; vacuity applies to TYPING (a Prop trivially true regardless of its subject is a critical). This is the FINAL CONFIRMATION pass: rev 2 repaired your 15/6 rev-1 findings; a fresh independent auditor rejected rev 2 at 2/5 (LST mistyping, free count field); rev 3 repaired those (findings table F1–F21 + FF1–FF7 in the blueprint). Verify the FF repairs are genuine — especially FF1: the lst field must state Lemma LST (level-set scale-typing: slot-minimum ht-typing, interior floor constancy, floor-downset selection), NOT count stabilization — and sweep the whole blueprint end to end one last time. A clean pass = ACCEPTED for formalization.
...
THE LST SOURCE DISPLAY (context — the §C lemma the ledger references):
...
THE BLUEPRINT UNDER FINAL CONFIRMATION (REV 3):
...
codex
1. **CRITICAL — R8/R22 is vacuously satisfiable by the advertised zero-mass singleton.**  
   Offending specs: **“R8 SettingsFamily … nonempty + root δ=1 + typed descent closure”** and **“R22 SIBOdelta … guard `0 < cellMass`.”**  
   The supplied inhabitation story explicitly permits a singleton family whose only setting has `cellMass = 0` and whose descendant sets are empty. Then `SIBOdelta F` is true without producing REL.1, REL.2(a)/(b)/(d), or any \(O_{\delta_j}\)-typing. The note quantifies over every realized positive branching cell; merely requiring some root member does not ensure such cells are represented.

2. **CRITICAL — R8/R22 does not express literal composition across nested base changes.**  
   Offending spec: **“R8 SettingsFamily … `desc_amb : … S'.δ = S.δabs`”** together with **“R22 SIBOdelta … `∃ Sp AD A D RB`”** separately for every `S`.  
   Index equality and membership closure do not make the independently existential `REL2a` coordinate maps, read dictionaries, lift policies, trees, or re-based theories commute from parent to descendant. The note requires the obligations to *compose* across nested base changes, not merely to have the correct absolute numerical index.

3. **CRITICAL — R18 silently excludes empty menus.**  
   Offending spec: **“R18 SpeciesSyntax + EQ1 … `menuNe : ∀ δ, (menu δ).Nonempty`.”**  
   The note asserts equality of finite syntax/menus across bases but does not assert that every menu is nonempty. This strengthens the display and loses the empty-menu edge case explicitly required by the audit.

4. **CRITICAL — R18 does not type the stated species grammar.**  
   Offending spec: **“R18 SpeciesSyntax + EQ1 … `shape : Letter → ℕ × ℕ × ℕ × ℕ`.”**  
   The note says letters are defined by stage tuples *and side/lattice data and polygon conventions*. Those fields and their validity conditions are absent; even `(0,0,0,0)` junk letters are admitted. Equality of arbitrary `Finset Letter` menus is weaker than invariance of the note’s species syntax.

5. **CRITICAL — R19’s “alphabet span” is not an attainable span.**  
   Offending spec: **“R19 AlphabetData … `alpha`, `alpha_zero`, `alpha_add`, `card_eq`.”**  
   No re-based slot maps, attainable images, or span-generation equality occur in the statement. `alpha` may be any additively closed finite subset with the desired cardinality. This is weaker than the unconditional definition that \(\mathbb A_\delta\) is the attainable additive image/span of the re-based slot maps.

6. **CRITICAL — R19 drops half of the open EQ-2 prediction.**  
   Offending spec: **“R19 … `EQ2law := … aDim δ x = δ * aDim 1 x`.”**  
   The note predicts both that the re-based piece maps are \(F_\delta\)-linear **and** that \(a_\delta=\delta a\) on the pinned stable-position domain. The Lean statement contains only the dimension equality and has no piece maps on which to state linearity.

7. **CRITICAL — FF1’s LST repair is still incomplete.**  
   Offending spec: **“R12 … `lst` … `wSlot` is the infimum of `ht` over nonzero block digits.”**  
   Lemma LST(i) first states that `ht(b,l)` is the actual K1-chain weight of the basis monomial and that the weight-\(\gamma\) initial form depends on exactly \(L_\gamma(B)\), typed in the appropriate graded piece. The blueprint supplies arbitrary `ht` and `wSlot` fields constrained only to agree with each other; it explicitly fences away the chain-weight and initial-form clauses. Thus it proves a weaker, freely modelled statement, not Lemma LST.

8. **CRITICAL — R12’s LST interior clauses can be vacuous.**  
   Offending spec: **“R12 ReBased … `interiorB : … → Prop`”** and the `lst` clauses guarded by `K.interiorB H B`.  
   Nothing identifies `interiorB` with the note’s factor interior or requires any relevant block to be interior. Taking it constantly false makes floor constancy and floor-downset selection automatic. This fails the statements-only non-vacuity requirement.

9. **CRITICAL — FF3 substitutes alphabet exponents for PIN-WELLDEF status.**  
   Offending spec: **“R12 … `c05PinWelldef` … pinned-set cardinality and the multiset of `aDim` values.”**  
   The note requires frame invariance of pin *cardinality and status*. No pin-status carrier or status-preservation statement appears. Alphabet dimensions are ledger weights, not the stated pin-status datum, so this is a different and weaker clause.

10. **CRITICAL — R14(a5) remains satisfiable using only an identity transport.**  
    Offending spec: **“R14 REL2a … `FrameIdx`, `frameIdxNe`, `ambT`, `tgtT`, … `frame_compat`.”**  
    `FrameIdx := PUnit` with identity maps satisfies all fields for every `Θ`. Nothing says this family is the actual complete family of C.0.5 transports. Hence the structure does not deliver compatibility with C.0.5’s transports as required by (a5).

11. **CRITICAL — R14(a6) is not letter-for-letter transport per read field.**  
    Offending spec: **“R14 REL2a … `readDict_inj` and `readDict_letter`.”**  
    The note explicitly requires a displayed dictionary for every read field: shape, anchor, side endpoints/height, stride positions, digits, branch factor, and recentering lifts. The blueprint preserves only a species-letter label between two opaque types.

12. **CRITICAL — R16 does not state the required lift-policy/node-data correspondence.**  
    Offending spec: **“R16 REL2d … `corr_word`, `corr_reads`, `corr_verdict`, `posOf`; (a3) policy via `A.teich_pin`.”**  
    Merely having `A.teich_pin` available does not relate `nodeCorr`, `tgtTcan`, or the square to canonical residue-field lifts. Likewise `posOf` is unconstrained and does not identify the target alphabets. Since R14’s reads are opaque, `corr_reads` also cannot preserve the specified node datum fields. This is weaker than REL.2(d)’s acceptance square.

13. **CRITICAL — R17 silently promotes an optional e4 candidate into an obligation.**  
    Offending spec: **“R17 … `firstIdxCandidate := E.entryFirst = A.dRel`”** and **“hyp_fields: … `firstIdxCandidate` (e4) — open.”**  
    The note calls \(d_j^{\mathrm{rel}}\) only a *candidate* for \(e_j\); it requires the eventual definition to agree with [3]’s convention. Requiring equality to this candidate strengthens the frozen display without justification.

14. **CRITICAL — R17(e5) is vacuous through the free `consumed` set.**  
    Offending spec: **“R17 … `consumed : Set S.PTree`, `consumedNe`; `determines` says all members of `consumed` are equal.”**  
    Nothing connects `consumed` to the prescriptions actually consumed by [3] at the table key. Choosing an arbitrary singleton makes `determines` true. It therefore does not establish that the entry’s actual indices determine the actual prescribed subtree.

15. **CRITICAL — R10 does not characterize normalized Haar measure.**  
    Offending spec: **“R10 BoxVol … a normalized finitely-additive content … `vol_cyl`.”**  
    Finite additivity and correct digit-cylinder masses do not characterize Haar measure on an event class that may contain additional events. REL.2 and (e3) price realization events from that larger class, where `vol` can differ from Haar measure. The note specifically uses normalized Haar mass, so the proposed statements permit a different measure.

16. **GAP — R9/R12 leave an explicit REL.1 dependency untyped.**  
    Offending specs: **“R9 StageCarrierLaws … graded internals … fenced”** and **“R12 `secB1`.”**  
    REL.1 says §B1 and the re-based graded pieces/localization/degree-one unit hold over the new base. Finite stage carriers and `CharP` do not type that clause. This is an underspecified dependency on the absent §B1 API.

17. **GAP — EQ-1 and EQ-2 are not attached to REL.1 as the note requires.**  
    Offending specs: **“R13 `REL1 := REL1Clauses ∧ REL1b`,” “R18 `EQ1`,” and “R19 `EQ2law`.”**  
    The note places EQ-1’s check and EQ-2’s prediction in the REL.1 re-scoping pass. Neither is a field/consequence of `REL1`; they are unrelated standalone Props. The required REL.1-conditional dependency is therefore missing.

18. **GAP — R12 violates the one-display decomposition rule.**  
    Offending spec: **“R12 ReBased + count + REL1Clauses … ten TYPED clause fields.”**  
    This combines a large carrier, the count construction, and ten logically separate §A/§B/§C obligations. Its own “largest unit” warning confirms a hidden multi-lemma unit.

19. **GAP — R14 violates the one-display decomposition rule.**  
    Offending spec: **“R14 REL2a … six field-blocks.”**  
    REL.2(a1)–(a6) are six separately checkable deliverables involving measurability, divisibility, embeddings, formulas, transports, and reads. One hard structure obscures which item failed to elaborate or was supplied.

20. **GAP — R17 violates the one-display decomposition rule.**  
    Offending spec: **“R17 REL2e … structure + e4agrees/firstIdxCandidate/determines/massEqEntry.”**  
    The five displayed e-requirements are split inconsistently between data and several independent Props, with no single conjunction collecting them. This is both a hidden multi-lemma unit and an ambiguous hypothesis interface.

Counts: **15 critical / 5 gaps**

**REJECT (15 crit / 5 gap)**
tokens used
45,151
1. **CRITICAL — R8/R22 is vacuously satisfiable by the advertised zero-mass singleton.**  
   Offending specs: **“R8 SettingsFamily … nonempty + root δ=1 + typed descent closure”** and **“R22 SIBOdelta … guard `0 < cellMass`.”**  
