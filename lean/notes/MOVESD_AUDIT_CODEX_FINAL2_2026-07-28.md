# MovesD Lean blueprint (rev 3) — SECOND FINAL-CONFIRMATION pass (Codex, 2026-07-28)
Verdict: REJECT (8 crit / 2 gap). Prompt: /tmp/leanaudit/movesd_final2_prompt.txt.
...
session id: 019fa403-a7e6-7a81-8d97-5d97cc493f51
--------
user
...
This is the SECOND FINAL-CONFIRMATION pass, on REV 3. History: you rejected rev 1 (8/2); a fresh auditor accepted rev 2 (0/0); you then rejected rev 2 at 7/2 (offSlots_forced unstated in general; lawless Canon; assumed L8/henc; extra hn; fixed-N quantifiers; empty-case; cyl restatement; missing tree/verdict laws). Rev 3 claims: offSlots_forced resolved by the note's own L11 mechanism with only the note-displayed monic-lead residue (adjudication report §0.5 — check it against the note's §D4-R.6 display); the prefix family CONSTRUCTED with the L11 injection proved (E1–E9); D4R2′ hypothesis-free; ∀ N ≥ Nshape quantifiers; the empty case per the note; cyl → pinned D4R_CYL deferral; TreeModel child laws; VerdictModel halt dichotomy. Verify each repair against the note and sweep end to end. A clean pass = ACCEPTED for formalization.
...
SPIRAL-CROSSING ADDENDUM (context):
...
CONSUMED INTERFACE (MovesC, PROVED):
...
codex
I’m checking the REV 3 definitions and all 43 unit specs directly against the frozen displays, including the empty-prefix ontology, L11 encoding, uniform-in-\(N\) claims, and the tree/verdict interfaces. I’ll report only offending units, with severity and final counts.
codex
1. **CRITICAL — E9 / D1c / D3a use an unsupported reduction of the digit-slot data.**
   ...
   L3 retains the complete stride-position/slot sets \(p_j\), explicitly “hence the digit count \(d_r\).” The single D4-n3 example—root full-span with one monic lead omitted—does not prove that every general read has either \(\ell_r\) or \(\ell_r+1\) recorded digits, nor that the only omitted slot can be identified by a Boolean. Moreover, `monicTop = true` is not constrained to the root or to the degree-\(n\) box’s actual monic slot. Thus artificial non-root fixed-one slots are admitted, changing \(d_r\), \(W\), the encoding target, and ultimately D4R.2′.

2. **CRITICAL — F1 and D2a count a quotient of prefixes, not the note’s prefixes.**

   > `def PrefIdx ... := ↥(etaData P '' PrefSet n pol P)`

   > `theorem card_classes : Nat.card (PrefIdx n pol P) = Nat.card (etaData P '' PrefSet n pol P)`

   The note defines \(\operatorname{Pref}(\hat P)\) as the set of full realized lift-carrying prefixes. REV 3 instead identifies histories having the same ambient-field digit and \(\psi\)-coefficient data. No unit proves
   `Function.Injective (etaData P)` on `PrefSet`. `HistoryCoherent` also does not by itself fix the root stage/key or reconstruct every stage datum. E9 proves only that the already-formed quotient classes inject into an encoding; it does not prove that two histories in one class are the same \(\eta\). Consequently `CD`, D10, D12, D13, and D15 can count fewer objects than \(C_{\hat P}(p)\).

3. **CRITICAL — E7/E8 use an invalid irreducibility transport to the ambient field.**

   > `theorem selRank_lt ... : selRank ν < R.mbar`

   > “the ψImage transport: eligibility over ↥σ.K ↔ over the F-image, via E3 and the coercion ring embedding”

   A polynomial irreducible over the frame field need not remain irreducible over the ambient extension field \(F\). Hence the advertised `EligibleImage` predicate—described as monicity, irreducibility, degree and order in `Polynomial F`—does not contain `ψImage ν` in general and is not equivalent to the note’s eligible-factor set over \(F_i\). E3 identifies the coefficient subfield but does not preserve irreducibility after scalar extension. E7, E8, and therefore E9 are unsupported as stated.

4. **CRITICAL — the canonical-policy interface remains lawfully empty/incorrect.**

   > `structure CanonPolicy ... where`
   >
   > `  liftOf : Node p F → Polynomial ℤ_[p]`
   >
   > `  blind : ∀ ν t, liftOf { ν with lift := t } = liftOf ν`

   > `def CanonPolicy.IsCanon ... := ∀ r hr, ...lift = pol.liftOf ...`

   `blind` says only that `liftOf` ignores the existing `lift` field. It does not say that the output is an offset-P realizer, has the required weight and residual class, or is defined compatibly with the node’s recentering data. An arbitrary constant `liftOf` is allowed and can make `PrefSet` empty. Applying it to every node also adds a normalization condition on non-recentering `lift` fields that the note does not state. No instance/pinning unit establishes that `pol` is the note’s offset-P policy. Thus the claimed non-vacuity repair is incomplete.

5. **CRITICAL — the \(M(\hat P)\le n^{|\hat P|}\) half of D4R.2′ is still conditional and missing from the advertised theorem.**

   > `theorem Mfac_le (hwf : P.Wellformed n) : P.Mfac ≤ n ^ P.reads.length`

   > `theorem D4R2' : Nat.card (PrefIdx n pol P) ≤ P.Mfac * p ^ P.W`

   The note displays both inequalities for every shape-prefix:
   \[
   C_{\hat P}(p)\le M(\hat P)p^{W(\hat P)},\qquad
   M(\hat P)\le n^{|\hat P|}.
   \]
   `ShapePrefix` does not enforce `Wellformed`. W1 supplies it only when an actual member `H` exists. For an unrealizable nonempty shape—the precise L12 empty-menu case—`PrefIdx` may be empty while `Mfac ≤ n^length` remains unproved and can be false for the unrestricted structure. Saying that the second inequality is unnecessary when \(C=0\) does not match the displayed D4R.2′ claim.

6. **CRITICAL — D14a explicitly gives the wrong empty-prefix set, and the general mass units silently use that wrong ontology.**

   > `theorem Pref_empty_shape (hP : P.reads = []) : PrefSet n pol P = ∅ ∧ P.CD n pol = 1`

   The note says \(\operatorname{Pref}(\varnothing)=\{\varnothing\}\), not the empty set. Hard-coding `CD = 1` and defining a separate `emptyFiber` does not make `PrefSet`, `PrefIdx`, or `Presented.event` contain the empty prefix.

   In particular, these purported general laws lack `P.reads ≠ []`:
   ...
   At `P.reads = []`, they sum over an empty `PrefIdx` and use an empty `Presented.event`, yielding a vacuous zero law instead of the note’s full-box summand of mass \(1\). D14a proves a disconnected box identity but does not repair those statements.

7. **CRITICAL — D10/D12/D13 do not establish the note’s “every \(N\ge N(\hat P)\)” assertion non-vacuously.**

   > `theorem D4R1_SUM (N) (hN : P.Nshape ≤ N) (S : Presented ...) : ...`

   `Presented` requires a `JetSetup` for every index, but no existence unit constructs such an `S` at any level, much less at every \(N\ge Nshape\). Thus the theorem is true only for levels where an externally supplied presentation happens to exist and can be vacuous at all other levels. Nor is there a unit proving that `Nshape` dominates every note-level cutoff \(N(\eta,\top)\). This is weaker than L12’s unconditional quantification over every sufficiently large level.

8. **CRITICAL — T1a is vacuously satisfiable because `TreeModel` still admits the empty tree.**

   > `theorem tree_prefix_closed (T : TreeModel ...) : ∀ H H' x, H.IsPrefixOf H' → T.mem H' x → T.mem H x`

   Taking `mem := False` and `child := False` satisfies `mem_single`, `mem_snoc`, and `mem_realizable`; T1a then holds vacuously. L2 instead requires every input’s tree to contain the empty chain and to contain all and only eligible children computed from its digit cells. The generation equivalences relate `mem` to `child` but impose neither root inhabitation nor exhaustive eligible-child semantics. The non-vacuity repair therefore has not landed.

9. **GAP — D15 has no in-corpus L6 dependency and its deferred predicate covers representatives only.**

   > `theorem mult_eq_tree ... (hcyl : D4R_CYL S T) ...`

   > `theorem event_iff_tree ... (hcyl : D4R_CYL S T) ...`

   The frozen note proves D4R-CYL for every \(\eta\). No unit is specified to prove `D4R_CYL`; D15 merely assumes it. Furthermore, because it ranges over `PrefIdx`, it covers only one chosen representative of each `etaData` class. The missing representative-independence/reconstruction theorem leaves this dependency underspecified.

10. **GAP — `VerdictModel` does not encode the full residual VP/τ interface it claims to represent.**

   > `halt_iff : ... (vdict H x ≠ none ↔ ∀ ν hν, ¬ T.child (some H) ν x)`

   > `cap_law : ...`

   The halt dichotomy and one-sided cap stability are present, but there is no law making `vdict`/τ a function of the fixed next-outcome digit cell, nor any link from `levelOf` to the presented coordinate charts. The blueprint itself defers both to MovesT, so `VerdictModel` is not yet the stated complete typed surface for kernel part (c).
