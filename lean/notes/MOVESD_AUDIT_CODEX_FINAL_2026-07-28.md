# MovesD Lean blueprint — FINAL CONFIRMATION pass (Codex, 2026-07-28)
Verdict: REJECT (7 crit / 2 gap). Prompt: /tmp/leanaudit/movesd_final_prompt.txt.
...
session id: 019fa3ef-eee4-72c1-989e-80adfb2cab45
--------
user
...
This is the FINAL CONFIRMATION pass: rev 2 repaired your 10 rev-1 findings and a fresh independent auditor then returned ACCEPT (0/0). Sweep the whole blueprint end to end one last time — verify the 10 repairs are genuine and hunt for anything both prior passes missed. AUX units (declared informal-statement auxiliaries) are acceptable design. A clean pass = the corpus is ACCEPTED for formalization.
...
THE BLUEPRINT UNDER FINAL CONFIRMATION (REV 2):
...
codex
1. **CRITICAL — D2a relies on an invented off-slot constraint.**

   > `theorem encOf_inj (S : PrefFamily p F n N m Canon P) : Function.Injective S.encOf`

   Its sketch extends equality on retained slots using:

   > `Matches.offSlots_forced`  
   > `(∀ k ≤ R.len, k ∉ R.slots → ν.sideDigit k = 1)`

   The note never states that every digit outside the retained slot set equals `1`. It only says the shape retains the stride-position/slot sets and forgets digit values. The monic lead equals `1` in the pinned D4-n3 instance, not uniformly at every read and every excluded position. This strengthens `shape(η)=P̂`, can remove legitimate prefixes, and invalidates the claimed reconstruction of L11.

2. **CRITICAL — D2b counts an arbitrary predicate, not the note’s fixed canonical policy.**

   > `theorem range_hist (S : PrefFamily p F n N m Canon P) : Set.range S.hist = PrefSet Canon P`  
   > `theorem C_eq_card_prefSet ... : S.C = Nat.card (PrefSet Canon P)`

   `Canon : History p F → Prop` has no law connecting it to the note’s deterministic offset-P-lift policy. Taking `Canon := fun _ => False` makes `PrefSet` empty for every shape. Thus these statements can identify `C` with zero even when the note’s actual `Pref(P̂)` is nonempty. “Pinned later by HC-1” is an underspecified dependency, not a faithful statement of the displayed `Pref(P̂)`.

3. **CRITICAL — D2a/L8 assumes the finiteness mechanism it is supposed to prove.**

   > `theorem L8_finite (S : PrefFamily p F n N m Canon P) : Finite S.idx`

   `PrefFamily` already assumes `complete` and the injectivity field `henc`. No unit constructs such a family from `PrefSet`, proves the actual factor-rank semantics of `sel`, or proves `henc` from L1/L4/L11. If `PrefSet` is infinite or the proposed encoding fails, no `S` exists and D2a is vacuous. Consequently this is not L8’s assertion that `Pref(P̂)` itself is finite.

4. **CRITICAL — D3a silently assumes the main L11 injection and adds a hypothesis to the coarse bound.**

   > `theorem D4R2' (S : PrefFamily ...) : S.C ≤ P.Mfac * p ^ P.W`  
   > `theorem Mfac_le (hn : ∀ R ∈ P.reads, R.len ≤ n) : P.Mfac ≤ n ^ P.reads.length`

   The first theorem consumes `S.henc`, which is precisely the substantive L11 injectivity conclusion rather than a genuine open kernel. The second makes `deg R_anch,r ≤ n` an extra hypothesis, whereas L11 states it holds “always” for a degree-`n` classifier history. Hence the combined displayed conclusion  
   `C_P̂(p) ≤ M(P̂)p^{W(P̂)}` and `M(P̂) ≤ n^{|P̂|}` is formalized only under stronger assumptions.

5. **CRITICAL — D10 formalizes only one presented cutoff, not D4R.4’s uniform quantifier.**

   > `theorem D4R1_SUM ... : (∑ i : S.idx, Nat.card (S.fiber i)) * p ^ (P.A n N) = S.C * p ^ (n * N)`

   D4R.4 says that after defining one `N(P̂)`, the count holds **for every** `N ≥ N(P̂)`. D10 fixes one `N` inside a separately assumed `PrefFamily`; there is no `N(P̂)`, no quantification over all larger levels, and no compatibility between the families or fibers at different levels. D12 and D13 inherit the same loss. Defining `A` as `A(P̂,n,N)` and explicitly omitting N-stability does not encode the note’s display.

6. **CRITICAL — D14a does not state the empty-prefix identification it cites.**

   > `theorem emptyShape_law ... : Nat.card (Set.univ : Set (Box p m)) * p ^ (P.A n N) = 1 * p ^ (n * N)`

   This proves a numerical full-box identity but never states or defines  
   `Pref(∅) = {∅}`, `C_∅ = 1`, or `S(∅,⊤)` as the fiber of that unique empty prefix. Because the empty prefix is excluded from both `History` and `PrefFamily`, the literal `1` on the right is disconnected data, not the note’s cardinality conclusion.

7. **CRITICAL — D15 assumes D4R-CYL instead of formalizing it.**

   > `theorem mult_eq_tree ... : S.mult x = Nat.card {i : S.idx // T.mem (S.hist i) x}`  
   > `theorem event_iff_tree ... : x ∈ S.event ↔ ∃ i : S.idx, T.mem (S.hist i) x`

   Both follow immediately from the hypothesis field:

   > `TreeModel.cyl : ∀ i x, x ∈ S.fiber i ↔ mem (S.hist i) x`

   But `(D4R-CYL)` is L6’s theorem, conditional on the named kernel and Theorem C; it is not an open kernel field. D15 therefore merely rewrites an assumed copy of its cited mathematical result.

8. **GAP — `TreeModel` does not specify the maximal canonical branch tree of L2.**

   > D15 hypothesis fields:  
   > ``T.prefix_closed`/`T.mem_realizable` (carried)`

   The structure has no digit-cell outcome data, child relation, “children are all eligible `(side,ψ)` continuations,” uniqueness of the tree for each input, or maximal-branch law. `cyl + complete` constrains only enumerated chains of one fixed shape; it does not determine membership for other shapes or extensions. Thus the Defs dependency called `T_can(f)` is materially underspecified.

9. **GAP — the advertised VP residual has no detection-cap or halting-rule law.**

   The blueprint says no MovesD unit consumes `VerdictModel`, while its only cap condition is:

   > `cap : ℕ`  
   > `hcap : 1 ≤ cap`

   Nothing relates `cap` to `vdict`, digit cells, verdict classes, or detection time, and no field encodes the halting rule `τ`. `leaf := False` remains satisfiable whenever the chosen tree has no finite maximal member. This does not expose the stated open kernel D4R.0-K(c): “alphabet V + halting rule τ + the uniform per-class detection cap.”

Counts: **7 CRITICAL / 2 GAP**
