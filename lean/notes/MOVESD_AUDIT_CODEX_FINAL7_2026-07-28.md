...
session id: 019fa4ca-2a37-7680-815d-0b86b9eaba62
--------
user
...
SEVENTH pass, REV 8 — the acceptance rev. Your rev-7 verdict (3/3): residue-degree laws missing; SW1 assuming MonicTie; the empty prefix in E9/D3a/W3; order/TreeModel/VerdictModel gaps. Rev 8: ShapeWF gains w0/wchain (§D4-R.6 quoted; the w=2 adversary fails); SW1 DERIVES the derivable monicTop structure with the residue honestly named ENC-MONIC (weakest sufficient hmt0, owner = the D4-n3 gate + menu wave — adjudicate whether the note supports keeping it as a named gap); the ∅ dispatch through the F3/CD bridge makes D4R2'' AT the empty shape the note's C_∅ = 1 arithmetic; strS hoisted (order elaborates); TreeModel gains L2's all-and-only children law (typed OPEN); VerdictModel gains the cap laws (cellOf := id fails). VERIFY each cure + the adversary block + the gate; sweep all 48 units. A clean pass = ACCEPTED.
...
codex
I’ll audit the 48 statement blocks against the displays, with special attention to the rev-8 cures, empty-prefix routing, and whether the newly named open surfaces are actually sufficient and non-vacuous.
codex
1. **CRITICAL — `ShapeWF.gamTie` changes the retained γ-law.**

   Offending spec:

   > `gamTie : ... gam = e * (P.strS r * ustar) + (s0 + wSide) * h`

   The frozen L3 display states `γ_r := e_r·u*_r + j*_r·h_r`, without the accumulated `strS` factor. For histories following an earlier read with `e > 1`, these are different conditions. The gate examples all have `strS = 1`, so they do not test this discrepancy.

2. **GAP — SW1 does not account for the new residue-degree conclusions.**

   Offending unit:

   > `theorem ShapeWF_of_matches ... : ShapeWF n Q`

   `ShapeWF` now includes `w0` and `wchain`, but SW1’s hypotheses and sketch only transport species, window, degree, lattice, and anchor laws. No cited dependency establishes from `HistoryCoherent` and `Matches` that the root field has degree `w₀ = 1` or that successive field cardinalities satisfy `w_{r+1} = w_r·g_r`. A residue-field tower/cardinality lemma or explicit hypothesis is missing.

3. **GAP — ENC-MONIC remains an extra dictionary hypothesis, not a general consequence of the note.**

   Offending unit:

   > `(hmt0 : ∀ h0 : 0 < Q.reads.length,  
   > ((Q.reads[0]'h0).monicTop = true ↔ ...s0 + ...wSide = n))`

   The note retains the actual slot sets; it does not leave their root monic-slot interpretation as an open kernel. The D4-n3 gate verifies only one shape, and a future menu check does not justify SW1 for every matched history. Keeping `hmt0` honestly exposes the unresolved Boolean reduction, but it remains an encoding GAP rather than a discharged cure.

4. **GAP — `Shape.NP` is not tied to L12’s actual cutoff.**

   Offending definitions/spec:

   > `Shape.NP ... := if Nonempty (PrefIdx ...) then NPband ... else 1`

   > `theorem D4R4_all ... : ∀ N, P.NP pol ≤ N → ...`

   The blueprint explicitly leaves as “documented semantic reading” the required identification of `NPband` with `maxη N(η,⊤)`. Neither a theorem nor a hypothesis field connects fresh-band coordinates with every equation level in the transported systems. Consequently the displayed range is not yet verified to be exactly L12’s `N ≥ N(P̂)` range.

5. **CRITICAL — `TreeModel`’s all-and-only cure is vacuous.**
   ...
   Because `eligible` has no law connecting it to polygon sides, fixed digit cells, residual factors, or canonical lifts, every `child` relation satisfies the requirement by setting `eligible := child`. In particular, the root-only tree with no children remains admissible for every input. This does not state L2’s “ALL eligible continuations” law.

6. **CRITICAL — `VerdictModel.cell_local` silently strengthens the note.**

   Offending field:

   > `cell_local : ...  
   > (∀ c, (∃ v, levelOf c < cap v) → x c = x' c) → cellOf o x = cellOf o x'`

   The uniform detection cap belongs to the terminal verdict rule. The note does not say that L5’s full polygon-and-digit outcome cell is determined by the verdict-cap prefix. Requiring the entire `cellOf` datum to factor through that envelope can exclude faithful cell partitions and conflates kernel parts (b) and (c).

7. **CRITICAL — `VerdictModel` remains degenerately satisfiable.**

   Offending structure: `VerdictModel`.

   Even with `capBound`, `hcapB`, and `hlevel`, one can use singleton `V` and `Cell`, constant terminal `vdict`/`rootVdict`, constant `cellOf`, zero caps, and a tree with no children. All locality, cap, cell-factorization, and halt laws then hold. Thus the structure still does not encode the particular verdict alphabet and halting rule asserted by kernel (c); the promised “semantic pin” is only prose and has no hypothesis field or instance unit.
