# MovesS blueprint (rev 8) — Codex final-confirmation pass 5
Verdict: REJECT (6 crit / 3 gap). Prompt: /tmp/leanaudit/movess_final5_prompt.txt. Raw output below.
...
session id: 019fa45d-2189-7531-ba13-16f65a811675
--------
user
...
FIFTH final pass, on REV 8. History: your rev-7 verdict (6/4) and the independent auditor's (2/2) CONVERGED on the PolyGeom denominator class. Rev 8 resolves it as a CONVENTION FIX with the note quoted (§2.C.1): the note states the class in q⁻¹ ("geometric denominators (1 − q^{−a})"), so the faithful X = q class is denom ∣ X^b·∏(1−X^a); PolyGeom.qpow added; the gate masses AND the CTS count/q^level instance re-verified representable. VERIFY THE CONVENTION READING against the note's (iv)-POLY display yourself — this is the load-bearing decision; if you believe the note's class genuinely excludes q-powers, say so with the quote. Your other findings: W2/W3 de-circularized (now obligations over the raw family that CREATE the bundle fields, S.1/S.2 packages quoted); W7 = full CUT-1/2/3 laws; W17ii/shWeight typed and fenced; separate tg/j bounds; U-24b in full Lean; U-29a split; U-29d5 scoped; the gate window data corrected to ℓ=g=μ=1 (the independent auditor's SCS-law finding — CHECK window_comp/cluster_child now hold and the masses/det/βs are unchanged). Sweep end to end. A clean pass = ACCEPTED for formalization.
...
codex
I’m sweeping the formal signatures against the displays now, with special attention to the q⁻¹ denominator convention, the raw-family W2/W3 pins, CUT-WD, and the n = 2 inhabitation gate.
codex
Load-bearing checks: the REV-8 denominator convention is faithful. The note says “geometric denominators \((1-q^{-a})\)” and uses monomials in \(q^{-1}\); with Lean’s variable \(X=q\), reduced denominators may divide \(X^b\prod(1-X^a)\). Thus `PolyGeom.qpow` is required. The corrected \(n=2\) window data \(\ell=g=\mu=1\) also satisfies `window_comp`, `res_sum`, and `cluster_child`, without changing the displayed masses or solved β-values.

1. **CRITICAL — W2 still drops every displayed hypothesis from its formal signature.**

   Offending unit:

   > `def W2_xrb (B₀ : RS1Meas T M) : Prop :=`
   > `  ∀ e he h h' τ σ', B₀.βmeas e he h τ σ' = B₀.βmeas e he h' τ σ'`

   The note proves XRB only given:

   > “XHD (w/u/d/s) + (JC-INV) + (SIB)/(JC-multi) + TB-CAP/VP + REL.2(a)/(b)/(d) + (ns-null).”

   Those inputs occur only in comments, not as typed hypothesis fields or arguments. Consequently the pin asserts an unconditional property of `B₀`; it does not formalize the theorem-under-hypotheses displayed in S.1. Splitting off `RS1Meas` removes the circular projection, but does not restore hypothesis fidelity.

2. **CRITICAL — W3 likewise omits its complete GIVEN-package.**

   Offending unit:

   > `def W3_recursion (B₀ : RS1Meas T M) : Prop :=`
   > `  ∀ e he τ σ' q₀ (h : q₀ ∈ M.Pools), M.activeState q₀ e τ →`
   > `    ∀ h_ent, B₀.βmeas ... = evalRe ...`

   The note attaches TREE-EXP fin/ns, TREE-N, ONE-F, SIB, TB-CAP/VP, the nine-input ledger, XRB, PCI, and REL.2 at every base-changed leg. None is a formal argument or field of this proposition. Quoting them in comments does not make them Lean hypotheses. The statement is therefore stronger than the note’s RS.1-DEEP derivation.

3. **CRITICAL — `W7_cutWD` remains weaker than the displayed CUT-1/2/3 laws and is vacuous on an empty tree carrier.**

   Offending unit:

   > `def W7_cutWD ... : Prop :=`
   > `(∀ t (ν ν' : TI.Node t), ... TI.onPath t ν ν' → TI.onPath t ν' ν → ν = ν') ∧`
   > `(∀ t (ν : TI.Node t), ... ) ∧`
   > `... ∧ (∀ t, TI.shapeOf t ∈ F.Sh)`

   Problems:

   - The first conjunct is only antisymmetry; CUT-1 says entrance nodes on each branch are linearly ordered and that the first one, when present, is unique.
   - `TreeIface` gives no order laws for `onPath`, so the formula does not establish the intended root-path semantics.
   - The “deep splits are never re-classified as shallow entrances” law is only prose; no field relates `isEntrance` to deep split nodes.
   - `TreeIface.Tree` has no `Nonempty` field. Taking `Tree := Empty` makes the complete CUT pin vacuous, with no explicit interface-instance unit in S5.

4. **CRITICAL — `W17ii` formalizes only a height sum, not CL-17(ii)’s stated production machinery.**
   ...
   The note’s open duty is that \(W_{\hat S}\) is produced by C.1.5 node volumes × CTS/[1] counts × XHD-s height sums × SIB’s COUNT face across shallow splits. The formal statement contains no CTS-count or SIB-count production relation. The blueprint explicitly leaves that “residual” only in prose.

   It is also degenerately satisfiable with `shDom Ŝ = ∅`, empty events, zero weights, `WshVal = 0`, and `WshP = 0`. Thus the claimed CL-17(ii) pin neither states nor non-vacuously enforces the note’s open kernel.

5. **CRITICAL — the cofinitely-all-active polynomial burden is weakened to an unrelated infinitude field.**
   ...
   > “at all-active primes (cofinitely many — only finitely many primes are roots of some not-identically-zero cell-size polynomial)”

   The blueprint requires only infinitely many prime-power pools, potentially powers of one prime, and supplies no law connecting `activeState` to nonvanishing of the fixed `countS` polynomials. It also permits those polynomials to be identically zero. This is a weaker hypothesis than the displayed (iv)-POLY/cofiniteness burden and can satisfy U-9b/U-22 in models not covered by the note.

6. **CRITICAL — XRB and exactness quantify over every rational parameter, not merely relevant pools.**
   ...
   > `rexact : nsNull → ∀ e he h_ent τ σ' q₀,`
   > `  βfull ... q₀ = βmeas ... q₀`

   The first is equality of entire functions `ℚ → ℝ`; the second explicitly ranges over every `q₀ : ℚ`. The note’s measured conditional probabilities exist at the relevant prime-power theories/pools. Other measured claims correctly carry `q₀ ∈ M.Pools`; these two do not. They are therefore strictly stronger than the displays and impose arbitrary requirements at non-pool rationals.

7. **GAP — REV-8’s `PolyGeom` refactor leaves a stale, ill-typed field reference.**

   Offending field:

   > `((ιP e τ ε).count.eval q₀ : ℚ) = M.entCount e τ ε q₀`

   `PolyGeom` now has `countT` and `countS`; it has no `count`. The intended ENT-COUNT polynomial is therefore unspecified, and the Defs skeleton cannot elaborate as written.

8. **GAP — the three new S5 construction units remain hidden multi-obligation units.**

   Offending unit specs:

   > `U-29a1 n2_shape` — “TableShape ... Wloc/Wstate, DegCons/SCSData DATA”

   > `U-29a2 n2_measured` — “MeasuredSide: level-N boxes; cell events ... heights/entrance shapes ... pools”

   > `U-29a3 n2_burdens_shapes` — “RatBurdens data ... + ShapeFam”

   `MeasuredSide` and `RatBurdens` contain numerous proof fields: pool closure and infinitude, nonempty boxes and representatives, interpolation at every pool, degree pins, OK-at-pool proofs, and all-active infinitude. These are not mere data definitions and cannot honestly fit the advertised single medium construction sketch. The split does not yet meet the one-display decomposition rule.

9. **GAP — several S5 “one-display” units still combine multiple independent fields.**

   Offending specs:

   > `n2_kstep_hmc` — “kstep_one + hmc”

   > `n2_init` — “init_agg/init_count/ent_count_card”

   > `n2_xrb_rexact` — “h_ent-independence and βfull = βmeas”

   These are respectively two, three, and two distinct formal obligations, drawn from different displayed clauses. They remain hidden multi-lemma units under the stated decomposition criterion.

Counts: **6 CRITICAL / 3 GAP**
