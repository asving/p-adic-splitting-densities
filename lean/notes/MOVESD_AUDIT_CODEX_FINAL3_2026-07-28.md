# Codex THIRD FINAL CONFIRMATION audit — MovesD rev 4 (§D4-R)
Verdict: REJECT (7 crit / 1 gap). Run 2026-07-27, foreground, prompt = /tmp/leanaudit/movesd_final3_prompt.txt.
...
session id: 019fa422-86ff-7542-a40f-7f90a4ee8769
--------
user
...
THIRD final pass, on REV 4. History: your rev-3 final (8/2) partially CONFLICTED with an independent auditor who re-derived the E-chain and gate numbers; the independent auditor separately caught a gate-refuted granularity bug you had flagged from the other direction. Rev 4: the η-granularity FIXED via canonical-root normalization with the full gate re-derivation displayed in §0.6 (C_P̂*(2) = 3, C_P̂*(3) = 48, mult ≡ 1 — CHECK THE ARITHMETIC YOURSELF against the note's §D4-R.6 gate); EligibleImage now the explicit transported predicate; four of your findings received evidence-backed PUSHBACKS (§11 table: monicTop root-only + guard; CanonPolicy determinism; the ∅ decomposition; TreeModel with a displayed falsifying computation for the root-inhabitation law you requested). ADJUDICATE each pushback on the evidence — if the note supports the pushback, accept it; if not, say exactly why. Also verify the fixes for your findings 5, 7, 9, 10. Sweep end to end. A clean pass = ACCEPTED for formalization.
...
codex
I’m auditing the actual theorem statements against the displays, including the four pushbacks and the sealed-gate arithmetic. I’ll treat “documented for a future corpus” as a gap unless the current unit’s hypotheses really carry it.
codex
1. **CRITICAL — `monicTop` does not faithfully encode L3’s retained slot data.**
   ...
   The note retains the actual stride-position/slot sets `p_j`, “hence the digit count `d_r`.” Rev 4 replaces that data by a Boolean. The root guard is insufficient: it does not require the side’s top slot to be the degree-`n` monic coefficient, e.g. `s0 + wSide = n` at the root. Thus junk root shapes may set `monicTop = true`, omit a genuine digit, and obtain an artificially smaller `W`.

   The sealed instance supports `monicTop = true` for its particular full-span root; it does not justify the reduction for every shape. Pushback C-1 is rejected.

2. **CRITICAL — the arbitrary `CanonPolicy` parameter can still make the formal corpus empty or different from the note’s fixed offset-P policy.**
   ...
   and every mass/count unit quantified over arbitrary:

   > `{pol : CanonPolicy p F}`

   `blind` proves only independence from the existing lift field. It does not state that `liftOf` is the offset-P lift, is a valid realizer, has the prescribed weight/class, or is compatible with landing. A constant junk polynomial therefore remains legal and can eliminate all presentable histories.

   The note fixes one particular offset-P policy. The future `POL-PIN` named in prose is not a hypothesis of D3a, D10, D12, D13, or `D4R4_all`. Pushback C-4 is rejected.

3. **CRITICAL — PRESENT-NORM is declared in prose but absent from the Lean statements, so `PrefIdx` may be a strict subset of the note’s `Pref(P̂)`.**
   ...
   > “classes ↔ η's is the PRESENT-NORM pin”

   This theorem only counts the image of the already root-normalized `PrefSet`. It neither states nor consumes existence of a normalized presentation for every note-η. Consequently the source type can be empty even when the note’s `Pref(P̂)` is nonempty, and D3a/D10/D12 can prove zero-count laws for a truncated family.

   The canonical-root gate repair itself is numerically consistent, but its required two-sided dictionary is not wired as a hypothesis. This is the same non-vacuity defect in a new location.

4. **CRITICAL — `D4R4_all` is weaker than the displayed L12 quantifier and remains vacuously satisfiable.**

   Offending unit spec:

   > `theorem D4R4_all ...`  
   > `(hex : ∀ N, P.Nshape ≤ N → Nonempty (Presented ...)) :`  
   > `∀ N, P.Nshape ≤ N → ∃ S : Presented ..., ...`

   L12 defines the exact threshold
   `N(P̂) = maxη N(η,⊤)` and asserts the identities for every `N ≥ N(P̂)`. Rev 4 instead begins at the potentially larger `Nshape`; `A_stable` proves ledger stability, not `Nshape = N(P̂)` or coverage of levels between them.

   It also concludes existence of one presentation `S`, rather than stating the classifier count itself. Because `Presented` has only an empty-indexed `jet` field, `hex` is trivially satisfiable when the incorrectly normalized `PrefIdx` is empty. The PRESENT-EXIST repair is therefore only partial; finding C-7 is not closed.

5. **CRITICAL — the empty-prefix unit directly states the wrong `Pref` set.**

   Offending unit spec:

   > `theorem Pref_empty_shape (hP : P.reads = []) : PrefSet n pol P = ∅ ∧ P.CD n pol = 1`

   Its quoted display says:

   > “`Pref(∅) = {∅}`”

   Encoding the missing empty history through an unrelated `if` branch in `CD` does not make `PrefSet = ∅` faithful to that display. It splits one mathematical object into incompatible formal objects: `PrefSet` has zero elements while its claimed cardinality surrogate is one.

   The note’s separate proof mechanism for the empty summand does not support saying the underlying prefix set is empty. Pushback C-6 first half is rejected. Adding `hne` to D10/D12/D13/D3b correctly prevents those units from using the zero-sized `PrefIdx` at `P̂ = ∅`, but D14a itself remains wrong.

6. **CRITICAL — `TreeModel` still omits the always-present empty root and exhaustive-child semantics.**
   ...
   > `theorem tree_prefix_closed ... : ∀ H H' x, H.IsPrefixOf H' → T.mem H' x → T.mem H x`

   L2 says every tree’s root is the empty chain and its children are **all** eligible next-node data. `History` is nonempty, so `mem` cannot even express membership of the empty root. `Option.none` labels root children but does not assert that the empty root exists.

   The pushback confuses “the empty root is a leaf” with “the empty root need not exist.” The `(ns)` case permits no children; it never removes the root. Moreover, `D4R_CYL` constrains membership only for one enumerated shape and does not force all other eligible children. Pushback C-8 is rejected.

7. **CRITICAL — the representative-independence repair is not present in D15’s statement.**

   Offending unit spec:

   > `theorem mult_eq_tree (S) (T) (hcyl : D4R_CYL S T) ...`

   and

   > `theorem event_iff_tree (S) (T) (hcyl : D4R_CYL S T) ...`

   `ClassFiberWelldef` is mentioned in the ledger but is neither shown as a definition nor included as a hypothesis of these units. `D4R_CYL` concerns only `reprOf i`, so D15 proves a chosen-representative bridge, not L6’s statement for every η/presentation in the class.

   Thus the advertised fix for C-9 is not wired into any theorem.

8. **GAP — `VerdictModel` still does not encode verdicts as functions of the actual digit cells.**

   Offending Defs unit:

   > `vdict_local : ∀ H x x',`  
   > `  (∀ c, (∃ v, levelOf c < cap v) → x c = x' c) → vdict H x = vdict H x'`

   together with only:

   > `levelOf_lt : ∀ c, levelOf c < N`

   This gives locality in an abstract coordinate prefix, but no equality with a presented jet’s `coordOf`, no frame transport, and no factorization through L5’s full next-outcome digit cell. The blueprint explicitly defers those ties to MovesT. That is an honest declaration, but it means fix C-10 is incomplete as a Defs representation of VP/kernel-(c).

Gate arithmetic independently checks out:
...
- `C_P̂*(2) = 1·1·3 = 3`
- `C_P̂*(3) = 3·2·8 = 48`
- canonical-root normalization removes the displayed conjugate doubling, so the sealed shape has `mult ≡ 1`.

The explicit transported `EligibleImage` repair is also semantically aligned with L11. W3 correctly removes the coarse bound’s exposed `Wellformed` hypothesis on the C-carrying inequality, but it cannot repair the misidentified/truncated `C`.

Counts: **7 CRITICAL / 1 GAP**
