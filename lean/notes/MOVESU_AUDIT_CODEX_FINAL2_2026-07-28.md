# MovesU rev 5 — Codex SECOND FINAL-CONFIRMATION audit (2026-07-28)
Verdict: REJECT (3 crit / 0 gap). Raw run saved from /tmp/leanaudit/movesu_final2_verdict.txt.
...
session id: 019fa406-5c8c-72e3-8b7b-18185d2037a0
--------
user
...
This is the SECOND FINAL-CONFIRMATION pass, on REV 5. History: you rejected rev 4 at 3/0 — U3/U4 consumed the full CapstoneLedger where SQ.2's displays need only slices, and U6 dragged the rate-form CL-2/CL-3 into the non-rate bracket; a fresh independent auditor separately ACCEPTED rev 4 (0/0) on all other axes. Rev 5 repaired exactly your three findings via Prop sub-bundles (FinStack/SolveStack/LowerStack) whose contents claim to match the note's per-display citation lists, with CL-2/CL-3 confined to U10's full ledger. VERIFY: (a) each sub-bundle's field list against the note's displays — SQ.2's partial claim, the (U-n) upper citation list, RS.4's inherited checksum + CL-4 for the lower leg; too much OR too little in any bundle is a critical; (b) U10 still consumes the full CapstoneLedger (that IS Theorem U); (c) the projections change no unit's mathematical content. Then sweep end to end. A clean pass = ACCEPTED for formalization.
...
THE BLUEPRINT UNDER FINAL CONFIRMATION (REV 5):
...
codex
1. **CRITICAL — U3 drops part of the cited CL-10 package.**

   Offending spec:

   > `theorem sq2_partial (FS : FinStack n p X F K) (σ N) : ...`

   `FinStack.cl10` asserts only:

   > `K.sib ∧ K.jcMulti ∧ K.treeExpTreeN`

   But the frozen note cites the `[3t] package (CL-10)`, whose displayed contents also include VP/VP-SOUND. In the blueprint, that obligation is represented separately by:

   > `cl10_vpsound : VPSound X`

   It is absent from `FinStack`. Thus U3 is satisfiable without the complete CL-10 hypothesis attached to TREE-N in the note.

2. **CRITICAL — U4 inherits the same incomplete finiteness stack.**

   Offending spec:

   > `theorem sq2_upper (FS : FinStack n p X F K) (SS : SolveStack n p X F S D M K) ...`

   The frozen `(U-n)` upper citation list requires the finiteness stack containing “the `[3t] package (CL-10)`.” Neither `FS` nor `SS` asserts `VPSound X`. Consequently U4’s premises are weaker than the displayed upper-side hypothesis stack.

3. **CRITICAL — U6’s non-rate bracket likewise omits that CL-10 component.**

   Offending spec:

   > `theorem un_bracket (FS : FinStack n p X F K) (SS : SolveStack n p X F S D M K) (LS : LowerStack n p S D K) ...`

   `LowerStack` correctly adds CL-4 and the two RS.4 checksum forms, and CL-2/CL-3 are correctly excluded from this non-rate unit. Nevertheless, the combined premises still omit `cl10_vpsound`, so they do not contain the full CL-10 package cited by `(U-n)`.

U10 itself still consumes the complete `CapstoneLedger`, including `cl10_vpsound`, CL-2/CL-3, O3, and all operative fields. The projection definitions otherwise preserve their selected fields exactly. No additional gaps were found.
