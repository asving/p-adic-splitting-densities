# Codex FINAL CONFIRMATION audit — MovesU rev 4 (§U-SQUEEZE)
Verdict: REJECT (3 crit / 0 gap). Run 2026-07-27, foreground, prompt = /tmp/leanaudit/movesu_final_prompt.txt.
...
session id: 019fa3fb-d758-7811-a8e9-c85bd257804c
--------
user
...
This is the FINAL CONFIRMATION pass on REV 4. History: you rejected rev 1 (10/3) and rev 2 (4/3); a fresh independent auditor rejected rev 3 (2/4, menu layer only, spine verified faithful); rev 4 repaired those (cumulative findings table §7, 26 findings). Settled architecture (audit execution, not the design): concrete Box of monic coefficient tuples over ZMod (p^N); conclusion split into classifier-keyed clause (i) + VPSound-conditional identification clause (ii) with the second squeeze U11; open kernels as typed hypothesis structures with declared owner-audited fidelity (the D9 boundary is DECLARED: an unfaithful-but-lawful instance is out of scope, a lawless one must violate a field). Verify the rev-4 menu-layer repairs (KsubM1C1 continuation-only scope; ctsmSyntax full CTS-M(i) carrier; M/K in the p-uniform layer; menu pins; equation pins replacing unsupported positivity) and sweep end to end. A clean pass = ACCEPTED for formalization.
...
codex
1. **CRITICAL — U3 `MovesU.U3_sq2_partial` silently requires the complete capstone ledger.**

   Offending spec:

   > `theorem sq2_partial (L : CapstoneLedger n p X F S D M K) ...`

   and:

   > `theorem sq2_partial_le …`

   The quoted display requires only the TREE-N/finiteness interface represented by `finiteness_stack` (CL-7 + CL-10). In Lean, accepting `L : CapstoneLedger ...` assumes every field of that structure, including CL-1–CL-19, VP-SOUND, O3, the solve stack, RS.4, and `env → 0`. Merely projecting `L.finiteness_stack` in the proof does not remove those hypotheses. Thus the unit is weaker than the note’s displayed conditional claim, and its `hypothesis_fields` declaration is inaccurate.

2. **CRITICAL — U4 `MovesU.U4_sq2_upper` silently strengthens SQ.2’s hypothesis stack.**

   Offending spec:

   > `theorem sq2_upper (L : CapstoneLedger n p X F S D M K) ...`

   SQ.2 uses the finite-tree/fiber stack and the stated solve stack. The full `CapstoneLedger` additionally requires unrelated hypotheses such as CL-2/CL-3’s quantitative-envelope machinery, CL-4’s qualitative limit, RS.4’s checksum, `cl10_vpsound`, the O3 declaration, and `cl4_env_tendsto`. These are not side conditions of the SQ.2 display. The statement therefore proves SQ.2 only under a strictly stronger premise than the note specifies.

3. **CRITICAL — U6 `MovesU.U6_un_bracket` requires hypotheses reserved for the rate form and capstone.**

   Offending spec:

   > `theorem un_bracket (L : CapstoneLedger n p X F S D M K) ...`

   The frozen `(U-n)` bracket requires the displayed upper stack plus RS.4’s inherited stack for the lower side. It explicitly says the **rate form** “additionally inherits CL-2/CL-3 + the `(X2-*)` family.” Nevertheless, `CapstoneLedger` makes CL-2 and CL-3 mandatory for the non-rate bracket, along with other capstone-only fields. Hence this Lean statement has stronger side conditions than `(U-n)`.

Counts: **3 CRITICAL / 0 GAP**
