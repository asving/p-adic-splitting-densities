# MovesD blueprint (rev 6) — Codex fifth final-confirmation pass, 2026-07-28
Verdict: REJECT (2 crit / 3 gap). Raw codex output below (input transcript + response).
...
session id: 019fa45a-e627-7113-93d3-dedb42e35d52
--------
user
...
FIFTH final pass, on REV 6 — the domain-subtype rev. History: five consecutive verifier splits; your rev-5 verdict (4/3) diagnosed junk-shape vacuity, policy-trivialization, and the smuggled Wellformed — the orchestrator ruled all three are ONE disease (the Lean type exceeded the note's universe) and rev 6 implements the cure: Shape n := {P // ShapeWF n P} (your "malformed shape" is now NOT A SHAPE by type; MonicTie a certificate component), SW1 proves no universe loss, W2 is unconditional over the subtype, W3 states both displayed bounds verbatim, the five standalone units carry hnorm (the note's normalized-presentation domain), NP is DEFINED by L12's exact formula with hstab derived, VerdictModel gains the typed open root-(ns)/cell fields, forward references resolved, every unit statement inlined. The subtype gate re-derivation (§0.6 addendum) reproduces all sealed values. VERIFY: (a) the certificate's clauses against the note's L3 dictionary (too weak = junk shapes return; too strong = universe loss — check SW1's proof sketch); (b) NP's definition against L12 verbatim including the max-over-η collapse via L9; (c) each of your seven rev-5 findings against its cure; (d) the gate numbers. Sweep end to end. A clean pass = ACCEPTED for formalization.
...
codex
1. **CRITICAL — the subtype certificate still exceeds the note’s shape universe, so junk-shape vacuity survives.**
   ...
   > `theorem D4R4_all ... (T : Threshold p F n pol P) : ∀ N, ...`

   `SW1` does not prove the advertised no-universe-loss result: it assumes `MonicTie`, the disputed certificate component, rather than deriving it from a matched note-history. More importantly, `ShapeWF` is only a partial dictionary-image certificate. It omits conditions forced by being a value-forgotten `Node` chain, including `h ≥ 1`, `gcd e h = 1`, the increment/recentering species laws, and the `u*`/`γ`/anchor lattice compatibility.

   For example, a one-read root record with `h = 0` can satisfy every displayed `ShapeWF` field but cannot match any `MovesC.Node`, whose `hh` requires `1 ≤ h`. For such a subtype inhabitant, `PrefSetAny = ∅`; consequently `PresentNorm` is vacuous, `PrefIdx` is empty, `Presented.jet` is an empty function, and the mass-law wrappers reduce to `0 = 0`. Thus malformed shapes remain shapes by type, exactly the rev-5 failure the subtype was meant to remove.

2. **CRITICAL — `ShapePrefix.NP` is not L12’s exact piecewise threshold.**

   Offending spec:

   > `noncomputable def ShapePrefix.NP ... := 1 + ... bandS ... .sup ...`

   and:

   > `theorem D4R4_all ... : ∀ N, P.NP n ≤ N → ...`

   L12 defines `N(P̂) := 1` whenever `Pref(P̂) = ∅`, irrespective of the potential geometric band of the shape. The proposed definition does not inspect `Pref`; a nonempty shape with no realizable value assignments may still have band coordinates at positive levels, producing `NP > 1`. The note explicitly says realizability can remove every assignment, so this is a live edge case.

   Moreover, L9 proves constancy of the fresh-volume exponent, not the asserted general identity between `maxη N(η,⊤)` and the maximum of the shape-level `bandS`. No unit states or proves that identification. Therefore `D4R4_all` can omit levels `1 ≤ N < NP` where L12 requires the empty-family law.

3. **GAP — the rev-6 Defs skeleton is not type-consistent after changing `PrefSet` to the subtype.**

   Affected unit example:

   > `theorem encIdx_inj (hnorm : PresentNorm n pol P) : Function.Injective ...`

   `PrefSet` and `PrefSetAny` take `P : Shape n`, but the displayed definitions of `PresentNorm`, `PrefOpt`, `PrefIdx`, `Presented`, `D4R_CYL`, and `ClassFiberWelldef` still bind `P : ShapePrefix` and pass it directly to those subtype-taking definitions. There is no coercion from an arbitrary `ShapePrefix` to `Shape n`. Consequently the stated units cannot elaborate from the supplied Defs skeleton without an unrecorded signature rewrite.

4. **GAP — the open normalization hypothesis is missing or incorrectly reported in unit metadata.**

   Offending specs include:

   > `theorem CD_eq (hnorm : PresentNorm n pol P) ...`
   ...
   `PresentNorm` is explicitly HC-2/open-surface content. The same omission occurs around the standalone E9/F1 declarations, while only W3 correctly records `hnorm`. The statements visibly depend on an open dictionary pin, so the hypothesis ledger is not faithful.

5. **GAP — `VerdictModel` still does not type the claimed digit-cell or root-cap semantics.**

   Offending fields:

   > `Cell : Type`  
   > `cellOf : Option (History p F) → Box p m → Cell`  
   > `vdict_cell : ... cellOf ... x = cellOf ... x' → ...`  
   > `rootVdict_cell : ...`

   `cellOf` has no field tying it to L5’s full polygon/digit outcome, no finiteness requirement, and no exhaustiveness/exclusivity law. Taking `Cell := Box p m` and `cellOf _ x := x` makes both factorization fields tautological for an arbitrary verdict function. This therefore does not encode the advertised “function of CELL data alone.”

   In addition, `cap_law` and `vdict_local` cover only nonempty histories; `rootVdict` has no corresponding uniform detection-cap/locality law. The root portion of the residual VP kernel remains underspecified.

The sealed arithmetic itself checks out: `W = 7`, `M = 1`, `A = 11`, `NP = 5` for the pinned inhabited gate shape, `C(2) = 3`, `C(3) = 48`, and the four displayed finite-level counts are arithmetically correct. That gate does not expose the malformed-subtype or empty-`Pref` threshold failures above.
